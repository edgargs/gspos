...
--  (SELECT LLAVE_TAB_GRAL FROM PBL_TAB_GRAL WHERE ID_TAB_GRAL = 470)||':'
  ...
  --Fecha       Usuario	  Comentario
  --19/03/2015  ERIOS     Creacion    
  FUNCTION GET_MONTO_NCR(cCodGrupoCia_in IN CHAR,
                         cCodCia_in      IN CHAR,
                         cCodLocal_in    IN CHAR,
                         cNumPedVta_in   IN CHAR) RETURN VARCHAR2 IS
    vRetorno VARCHAR2(20);
  BEGIN
    -- KMONCADA 25.04.2016 [PERCEPCION] NO SE UTILIZA LA PERCEPCION COMO PAGO 
    SELECT TRIM(TO_CHAR(SUM(VAL_NETO_COMP_PAGO -
                            (NVL(VAL_PERCEP_COMP_PAGO, 0) - NVL(VAL_REDONDEO_PERCEPCION, 0)) +
  ...
  AS
    ...
    -- KMONCADA 01.10.2015 VARIABLE PARA DETERMINAR SI SE ACUMULO TODOS LOS PUNTOS PERMITIDOS EN EL PEDIDO
    vRedimeTodoPermitido CHAR(1) := 'N';
    vMontoProdPermitido VTA_PEDIDO_VTA_DET.VAL_PREC_TOTAL%TYPE;
    curProdRedime FARMACURSOR;
    vListaRedime VARCHAR2(2000);
    vSecProdDet VTA_PEDIDO_VTA_DET.SEC_PED_VTA_DET%TYPE;
    vCodProd VTA_PEDIDO_VTA_DET.COD_PROD%TYPE;
    vPrimerSecPermitido VTA_PEDIDO_VTA_DET.SEC_PED_VTA_DET%TYPE;
    vValTotalProdExcluye VTA_PEDIDO_VTA_DET.VAL_PREC_TOTAL%TYPE;
  BEGIN
    -- KMONCADA 01.10.2015 SE OBTIENE TODOS LOS PRODUCTOS PERMITIDOS PARA LA REDENCION
    curProdRedime := FARMA_PUNTOS.F_CUR_PRODUCTO_APLICA_REDIME(cCodGrupoCia_in, cCodLocal_in, cNumPedVta_in);
    LOOP
      FETCH curProdRedime INTO vSecProdDet, vCodProd;
      EXIT WHEN curProdRedime%NOTFOUND;
      
      IF vListaRedime IS NULL THEN 
        vListaRedime := vSecProdDet||','||vCodProd;
      ELSE
        vListaRedime := vListaRedime||'@'||vSecProdDet||','||vCodProd;
      END IF;
    END LOOP;
    CLOSE curProdRedime;
    ...
    SELECT NVL(SUM(DET.VAL_PREC_TOTAL),0)
    INTO vValTotalProdExcluye
    FROM VTA_PEDIDO_VTA_DET DET
    WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
    AND DET.COD_LOCAL = cCodLocal_in
    AND DET.NUM_PED_VTA = cNumPedVta_in
    -- KMONCADA 01.10.2015 NO SE CONSIDERA LOS PRODUCTOS EXCLUIDOS O QUE LA EQUIVALENCIA DE REDIMIDOS SEA 0
    AND (DET.SEC_PED_VTA_DET, DET.COD_PROD) NOT IN (
            SELECT TO_NUMBER(SUBSTR(VAL,0,INSTR(VAL,',')-1),'999990.00') SEC_PED_VTA_DET, 
                   SUBSTR(VAL,INSTR(VAL,',')+1) COD_PROD 
            FROM (
    ...
    v_nValNetoPedVta := v_nValNetoPedVta - vValTotalProdExcluye;
    -- KMONCADA 01.10.2015 VALIDA CUANTOS PRODUCTO SE ENCUENTRAN AFECTOS AL IGV
    SELECT COUNT(1)
    INTO vContarAfecto
    FROM VTA_PEDIDO_VTA_DET DET
    WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
    ...
    ELSE
      ...
      END IF;
    END IF;
    -- KMONCADA 01.10.2015 VALIDA SI TODO EL PEDIDO FUE REDIMIDIDO CON LOS PTOS PERMITIDOS
    vRedimeTodoPermitido := FARMA_PUNTOS.F_VAR_VALIDA_TODO_REDIMIDO(cCodGrupoCia_in => cCodGrupoCia_in,
    ...
    --SET DET.FACTOR_PUNTOS = ROUND((DET.VAL_PREC_TOTAL/nImTotalPago_in),2)
    WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
    AND DET.COD_LOCAL = cCodLocal_in
    AND DET.NUM_PED_VTA = cNumPedVta_in
    -- KMONCADA 01.10.2015 NO SE CONSIDERA LOS PRODUCTOS EXCLUIDOS O QUE LA EQUIVALENCIA DE REDIMIDOS SEA 0
    AND (DET.SEC_PED_VTA_DET, DET.COD_PROD) IN (
            SELECT TO_NUMBER(SUBSTR(VAL,0,INSTR(VAL,',')-1),'999990.00') SEC_PED_VTA_DET, 
                   SUBSTR(VAL,INSTR(VAL,',')+1) COD_PROD 
            FROM (
              SELECT EXTRACTVALUE(xt.column_value, 'e') VAL
    ...
    -- KMONCADA 01.10.2015 SE OBTIENE EL SECUENCIAL DEL PRIMER PRODUCTO PERMITIDO DE REDIMIR
    SELECT SEC_PED_VTA_DET 
    INTO vPrimerSecPermitido
    FROM (
      SELECT TO_NUMBER(SUBSTR(VAL,0,INSTR(VAL,',')-1),'999990.00') SEC_PED_VTA_DET, 
             SUBSTR(VAL,INSTR(VAL,',')+1) COD_PROD 
      FROM (
    ...
    SET DET.FACTOR_PUNTOS = 1-NVL((SELECT SUM(NVL(AUX.FACTOR_PUNTOS,0))
                              ...
                              AND AUX.SEC_PED_VTA_DET > vPrimerSecPermitido),0)
    WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
    AND DET.COD_LOCAL = cCodLocal_in
    AND DET.NUM_PED_VTA = cNumPedVta_in
    AND DET.SEC_PED_VTA_DET = vPrimerSecPermitido;
    ...
    SET DET.AHORRO_PUNTOS = CASE 
                              ...
                              WHEN vRedimeTodoPermitido = 'S' THEN
                                VAL_PREC_TOTAL
                              ELSE  
                                ROUND((nImTotalPago_in * NVL(DET.FACTOR_PUNTOS,0)),2)
                            END
    WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
    AND DET.COD_LOCAL = cCodLocal_in
    AND DET.NUM_PED_VTA = cNumPedVta_in
    ...
    SET DET.VAL_PREC_TOTAL = CASE 
                               WHEN vRedimeTodoPermitido = 'S' THEN
                                 0.01
                               WHEN DET.VAL_PREC_TOTAL-NVL(DET.AHORRO_PUNTOS,0) < 0 THEN
                                 0
                               ELSE
                                 DET.VAL_PREC_TOTAL-NVL(DET.AHORRO_PUNTOS,0)
                             END,
        ...
        IF vRestoElectronico = 'S' THEN
          ...
          vPosicion := 1;
            ...
            LOOP
              ...
              AND DET.NUM_PED_VTA = cNumPedVta_in
              AND DET.SEC_PED_VTA_DET = vPosicion
              AND DET.VAL_IGV != 0-- KMONCADA 01.10.2015 NO SE CONSIDERA LOS PRODUCTOS EXCLUIDOS O QUE LA EQUIVALENCIA DE REDIMIDOS SEA 0
              AND (DET.SEC_PED_VTA_DET, DET.COD_PROD) IN (
                      SELECT TO_NUMBER(SUBSTR(VAL,0,INSTR(VAL,',')-1),'999990.00') SEC_PED_VTA_DET, 
                             SUBSTR(VAL,INSTR(VAL,',')+1) COD_PROD 
                      FROM (
    ...
    AND DET.COD_LOCAL = cCodLocal_in
    AND DET.NUM_PED_VTA = cNumPedVta_in
    -- KMONCADA 01.10.2015 SOLO APLICARA A LOS PRODUCTOS QUE TUVIERON FACTOR DE REDENCION
    AND DET.FACTOR_PUNTOS > 0;
