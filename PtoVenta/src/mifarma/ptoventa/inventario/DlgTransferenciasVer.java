...
public class DlgTransferenciasVer extends JDialog {
    ...
    private void initTable() {
        tableModel =
                new FarmaTableModel(ConstantsInventario.columnsListaProductosTransferenciasVer, ConstantsInventario.defaultValuesListaProductosTransferenciasVer,
                                    0);
        FarmaUtility.initSimpleList(tblListaProductos, tableModel,
                                    ConstantsInventario.columnsListaProductosTransferenciasVer);
    }
    ...
    private void chkKeyPressed(KeyEvent e) {
        ...
        if (mifarma.ptoventa.reference.UtilityPtoVenta.verificaVK_F2(e)) {
            ...
            else if (UtilityInventario.validaGuias(this, vEstadoNota, btnRelacionProductos,
                                                   VariablesInventario.vNumNota)) {
                if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this, "¿Está seguro de reimprimir?")) {
                    FarmaVariables.vAceptar = false;
                    ...
                    if (UtilityInventario.reimprimir(this, tblListaProductos, btnRelacionProductos,
                                                     VariablesInventario.vNumNota)) {
    ...
    private boolean anular() {
        ...
        try {
            DBInventario.grabaInicioFinAnulaTransferencia(VariablesInventario.vNumNota,
                                                          "I"); //JCHAVEZ 10122009 graba inicio de anulacion de transferencia
            if (VariablesInventario.vTipoNotaOrigen.equalsIgnoreCase(ConstantsPtoVenta.LISTA_MAESTRO_LOCAL)) {
                if (vEstadoNota.equalsIgnoreCase("P")) {
                    ...
                    DBInventario.grabaInicioFinAnulaTransferencia(VariablesInventario.vNumNota,
                                                                  "F"); //JCHAVEZ 10122009 graba fin de anulacion de transferencia
    ...
    private boolean verificaComprobante() {
        ...
        if (VariablesInventario.vTipoNotaOrigen.equalsIgnoreCase(ConstantsPtoVenta.LISTA_MAESTRO_LOCAL)) {
            ...
            if (vEstadoNota.equals("P") || vEstadoNota.equals("A") || vEstadoNota.equals("X") ||
                vEstadoNota.equals("R")) //Se puede anular en estado pendiente
            {
                try {
                    String estadoProcesado =
                        DBInventario.getEstadoProcesoSap(VariablesInventario.vNumNota, ConstantsPtoVenta.TIP_NOTA_SALIDA);
          ...
          FarmaUtility.showMessage(this,"Esta transferencia ha sido enviada al local destino.",btnRelacionProductos);
                ...
                retorno = false;
                FarmaUtility.showMessage(this, "Esta transferencia ya está anulada.", btnRelacionProductos);
            } else
            //retorno = true;
            //JSANTIVANEZ 21.10.2010
                ...
                vEstadoNota.equals("R")) //Se puede anular en estado pendiente
            {
                ...
                retorno = false;
                FarmaUtility.showMessage(this, "Esta transferencia ha sido enviada al local destino.",
                                         btnRelacionProductos);
            } else if (vEstadoNota.equals("N")) {
                retorno = false;
                FarmaUtility.showMessage(this, "Esta transferencia ya está anulada.", btnRelacionProductos);
        ...
        if (!FarmaVariables.dlgLogin.verificaRol(FarmaConstants.ROL_SUPERVISOR_VENTAS)) {
            ...
            if (verificaComprobante()) {
                if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                                                                                "¿Está seguro de anular la Transferencia?")) {
                    if (anular()) {
