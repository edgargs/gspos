...
public interface MapperEncuesta {
    ...
    @Select(value="{call " +
                    "#{listado, mode=OUT, jdbcType=CURSOR, resultMap=encuesta} := " +
                    "PTOVENTA_ENCUESTAS.VERIFICA_ENCUESTA(" +
                    "   #{cCodGrupoCia_in}," +
                    "   #{cCodCia_in}," +
                    "   #{cCodLocal_in}," +
                    "   #{cCodProd_in}" +
    ...
    @Select(value="{call " +
                    "#{listado, mode=OUT, jdbcType=CURSOR, resultMap=preguntaEncuesta} := " +
                    "PTOVENTA_ENCUESTAS.OBTENER_PREGUNTAS(" +
                    "   #{nCodEncuesta_in}" +
                    ")}")
    @Options(statementType = StatementType.CALLABLE)
    public void obtenerPreguntas(HashMap<String, Object> object);
    
    @Select(value="{call PTOVENTA_ENCUESTAS.GRABA_DETALLE_ENCUESTA(" +
                    "   #{cCodGrupoCia_in}," +
                    "   #{cCodCia_in}," +                  
                    "   #{cCodLocal_in}," +                  
                    "   #{cSecuencial}," +                  
    ...
    @Select(value="{call #{vSecuencial, mode=OUT, jdbcType=NUMERIC} := "+    
                        "PTOVENTA_ENCUESTAS.GRABA_CABECERA_ENCUESTA(" +
                        "   #{cCodGrupoCia_in}," +
                        "   #{cCodCia_in}," +                  
                        "   #{cCodLocal_in}," +                  
                        "   #{cUsuId_in}," +                  
