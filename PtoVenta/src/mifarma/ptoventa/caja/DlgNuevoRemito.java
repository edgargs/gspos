...
public class DlgNuevoRemito extends JDialog {
    ...
    private void jbInit() throws Exception {
        this.setSize(new Dimension(734, 513));
    ...
    private void initialize() {
        //RHERRERA: bloquea F11 - Sí es local Marrket
        ...
        if (VariablesPtoVenta.vIndTico.equals("N") && VariablesPtoVenta.vIndLocalPadre.equals("S")) {
            //rherrera 17.09.2014
            // deshabilito proceso de sobres;
            UtilitySobres.actualizarProceso("S");
            traerSobres(); // trae sobres
        }
    ...
    private void initTableListaVentasVendedor() {
        ...
        tableModeSobres =
                new FarmaTableModel(ConstantsCaja.columnsListaSobresDet, ConstantsCaja.defaultListaSobresDet, 0);
        // FarmaUtility.initSelectList(tblLista,tableModelLista,ConstantsCaja.columnsListaFechas);
        FarmaUtility.initSimpleList(tblSobres, tableModeSobres, ConstantsCaja.columnsListaSobresDet);
        //listarFechaVentas();

        if (listar())
            listaSobres(""); //ASOSA, 22.04.2010
        ...
        if (e.getKeyCode() == KeyEvent.VK_ENTER) {
            if (txtNumRemito.getText().trim().length() > 0) {
                ...
                if (validaMontoIngresado(txtNumRemito)) {
                    ...
                    FarmaUtility.showMessage(this, "Valor ingresado invalido!!!", txtNumRemito);
            }
        }
        chkKeyPressed(e);
    }
    ...
    private void chkKeyPressed(KeyEvent e) {
        ...
        } else if (mifarma.ptoventa.reference.UtilityPtoVenta.verificaVK_F11(e) && lblF11.isEnabled()) {
            if (validarCampos()) {
                //if(com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,"Esta seguro de asignar el nuevo remito a las fechas selecionadas?"))
                //    {
                if (todos) {
                ...
                //RHERRERA 29.08.2014
                if (VariablesPtoVenta.vIndTico.equals("N") && VariablesPtoVenta.vIndLocalPadre.equals("S")) {
                    updateRemitoTico_t(VariablesCaja.vArrayFechasSeleccinadas);

                    VariablesCaja.vArrayFechasSeleccinadas.clear(); // 29.08.2014 rherrera
                }
    ...
    private boolean validarCampos() {
        ...
        if (txtPrecinto.isVisible() == true && txtPrecinto.getText().trim().length() == 0) {
            ...
            result = false;
        } else if (txtNumRemito.getText().trim().length() < 10) {
   ...
   result=false;
    ...
    private void asignarLocales() {
        try {
            ...
            DBCajaElectronica.agregaRemito_AS(VariablesCaja.NumRemito, VariablesCaja.vArrayFechasSeleccinadas,
                                              vPrecinto); //INI ASOSA, 22.04.2010
    ...
    private void cerrarVentana(boolean pAceptar) {
        if (VariablesPtoVenta.vIndTico.equals("N") && VariablesPtoVenta.vIndLocalPadre.equals("S")) {
            //rherrera 17.09.2014
            // Habilito proceso de sobres;
            UtilitySobres.actualizarProceso("N");
    ...
    private boolean listar() {
        ...
        String ind = "";
        try {
            ind = DBCajaElectronica.getIND_PROSEGUR();
        } catch (SQLException e) {
            log.error("", e);
            FarmaUtility.showMessage(this, "ERROR AL OBTENER INDICADOR PROSEGUR: " + e.getMessage(), txtNumRemito);
        }
    ...
    /**
     * RHERRERA 16.09.2014
     * Obtiene sobres pendientes de envio al local Farma
     */
    private void traerSobres() {
        UtilitySobres.traerSobresTico();
    }

    private void updateRemitoTico_t(ArrayList pLista) {
        VariablesCaja.NumRemito = txtNumRemito.getText().trim();
        String vPrecinto = txtPrecinto.getText().trim();

        UtilitySobres.updateRemitoTico(pLista, VariablesCaja.NumRemito, vPrecinto);
