...
/* *********************************************************************** */
 ...
 RETURN NUMBER IS
    ...
    vIndActFuncionalidad varchar2(5);
 BEGIN
   
 begin
 SELECT DESC_CORTA
 INTO   vIndActFuncionalidad
 FROM   PBL_TAB_GRAL
 WHERE  ID_TAB_GRAL = 690;
 exception
   when others then
     vIndActFuncionalidad := 'N';
  end;   

    SELECT case 
            when vIndActFuncionalidad = 'N' then c.valor_cupon
            else 
                 NVL(U.VALOR_CUPON_PROD,C.VALOR_CUPON) 
           end      
    INTO   vValor_Cupon
