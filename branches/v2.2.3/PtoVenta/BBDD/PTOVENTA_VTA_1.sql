--------------------------------------------------------
--  DDL for Package Body PTOVENTA_VTA
--------------------------------------------------------
      ...
      SELECT
        CASE WHEN PROD.VAL_FRAC_VTA_SUG IS NOT NULL AND PROD_LOCAL.IND_PROD_FRACCIONADO = 'S' AND
          ...
          -- TO_CHAR(PROD_LOCAL.PORC_DCTO_1,'990.000') || 'Ã' ||
           ...
           DECODE(PROD_LOCAL.IND_PROD_FRACCIONADO,'N',' ',PROD_LOCAL.UNID_VTA)
...
--           TO_CHAR(PROD.VAL_BONO_VIG,'990.00') || 'Ã' ||
           ...
           PROD_LOCAL.VAL_FRAC_LOCAL|| 'Ã' ||
           NVL(PROD.IND_ZAN,' ')--JCORTEZ 16/07/08
      FROM   LGT_PROD PROD,
           LGT_PROD_LOCAL PROD_LOCAL,
...
PROCEDURE VTA_GRABAR_PEDIDO_VTA_CAB(cCodGrupoCia_in         IN CHAR,
     ...
     vNomCliPedVta V_CON_BENEFICIARIO.DES_CLIENTE%TYPE;
     vRucCliPedVta V_CON_BENEFICIARIO.DNI%TYPE;
  ...
  BEGIN
   ...
   -- JCHAVEZ 23092009.sn
    IF (cTipCompPago_in = '01' or cTipCompPago_in = '05') THEN
    ...
    IF cIndConvBTLMF = 'S' THEN
        ...
        SELECT
          ...
          INTO vCodCliente,vRucCliPedVta,vNomCliPedVta
...
-- 2009-11-09 JOLIVA: Se agrega el porcentaje de ZAN
    ...
    -- 19-NOV-13, TCT, obtiene Zan segun Cia
    CASE 
    WHEN vc_cod_cia='001' THEN
     NVL(p.porc_zan,0)
    WHEN vc_cod_cia='002' THEN
     NVL(p.porc_zan_fasa,0) 
    ELSE
     -1                           -- tct, indica error 
    END porc_zan
  ...
  BEGIN
       OPEN curVta FOR
          SELECT PROD.COD_PROD || 'Ã' ||
                 ...
                 --NVL(TRIM(lab.ind_lab_propio||to_char(prod.val_bono_vig,'990.00')),' ')
                 NVL(TRIM(prod.ind_prod_propio||to_char(prod.val_bono_vig,'990.00')),' ')
          FROM   LGT_PROD PROD,
                 LGT_PROD_LOCAL PROD_LOCAL,
                 LGT_LAB LAB,
                 PBL_IGV IGV,
...
/* ***************************************************************** */
  ...
  BEGIN
      ...
      SELECT   cCodGrupoCia_in,cCodLocal_in,cNumPedVta_in,cSecProdDet_in,cCodProd_in,cCantAtend_in,
               ...
               PROD.PORC_ZAN   -- 2009-11-09 JOLIVA
      FROM     LGT_PROD PROD,
               LGT_PROD_LOCAL PROD_LOCAL,
               pbl_igv igv
      WHERE    PROD_LOCAL.COD_GRUPO_CIA = cCodGrupoCia_in
    ...
    BEGIN
      BEGIN
        IF cIndFid_in ='S' THEN
          ...
          AND LLAVE_TAB_GRAL = (SELECT COD_CIA FROM PBL_LOCAL WHERE COD_GRUPO_CIA = cCodGrupoCia_in AND COD_LOCAL = cCodLocal_in)
          AND EST_TAB_GRAL = 'A';
...
/* *********************************************************** */
  ...
  BEGIN
    ...
    SELECT C.NUM_PED_VTA||';'|| to_char(C.VAL_NETO_PED_VTA,'9999999.00')
      INTO vRpta
      FROM VTA_PEDIDO_VTA_CAB C
     WHERE C.COD_GRUPO_CIA = cCodGrupoCia_in
       AND C.COD_LOCAL = cCod_Local_in
       AND C.NUM_PED_VTA IN (
                            SELECT CP.NUM_PED_VTA
                             ...
                             WHERE CP.COD_GRUPO_CIA = cCodGrupoCia_in
                               ...
                               AND CP.NUM_COMP_PAGO = cNum_Comp_Pago_in
                               AND CP.VAL_NETO_COMP_PAGO+CP.VAL_REDONDEO_COMP_PAGO = to_number(cMonto_Neto_in,'999999.000'));
                               --AND CP.VAL_NETO_COMP_PAGO = to_number(cMonto_Neto_in,'999999.000'));
  RETURN vRpta;
