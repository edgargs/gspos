...
public class StringTools {
    ...
    private static String BASE_DIGITS =
        "0123456789abcdefghijklmnopqrstvwxyzABCDEFGHIJKLMNOPQRSTVWXYZ()[]{}<>!@#&-=_+:~";
    ...
    // ------------------------------------------------------------------------
    public enum HTMLFilterType { VALUE, TEXT }
    ...
    public static String getCharacterEncoding() {
        try {
            ...
            return !StringTools.isBlank(charSet)
                   ? charSet
                   : CharEncoding_UTF_8;
        } catch (Throwable th) {
        ...
        if ((s == null) || s.equals("")) {
            ...
            for (int i = 1; i < ch.length; i++) {
                if (Character.isLetterOrDigit(ch[i])) {
                    continue;
                } else if ((x != null) && ListTools.contains(x, ch[i])) {
                    continue;
                }
        ...
        if (s == null) {
            ...
            for (int i = 0; i < ch.length; i++) {
                if (Character.isDigit(ch[i])) {
                    ...
                    // continue;
                } else if (inclHex && StringTools.isHexDigit(ch[i])) {
    ...
    public static String blankDefault(String target, String dft) {
        return StringTools.isBlank(target)
               ? dft
               : target;
    }
    ...
    public static String trimLeading(Object s) {
        return (s != null)
               ? StringTools.trimLeading(s.toString())
               : "";
    }
    ...
    public static String trimTrailing(Object s) {
        return (s != null)
               ? StringTools.trimTrailing(s.toString())
               : "";
    }
    ...
    public static <T> Number parseNumber(Object data, Class<?> numberClass, Number dft) {
        if (data == null) {
            return dft;
        } else if ((numberClass == null) ||!Number.class.isAssignableFrom(numberClass)) {
            return dft;
        } else if (numberClass.isAssignableFrom(data.getClass())) {
            return (Number) data;
        } else {
    ...
    public static double parseDouble(byte b[], int ofs, boolean isBigEndian, double dft) {
        ...
        if (isBigEndian) {
            doubleLong = (((long) b[i + 0] & 0xFF) << (7 * 8)) + (((long) b[i + 1] & 0xFF) << (6 * 8))
                         + (((long) b[i + 2] & 0xFF) << (5 * 8)) + (((long) b[i + 3] & 0xFF) << (4 * 8))
                         + (((long) b[i + 4] & 0xFF) << (3 * 8)) + (((long) b[i + 5] & 0xFF) << (2 * 8))
                         + (((long) b[i + 6] & 0xFF) << (1 * 8)) + ((long) b[i + 7] & 0xFF);
        } else {
            doubleLong = (((long) b[i + 7] & 0xFF) << (7 * 8)) + (((long) b[i + 6] & 0xFF) << (6 * 8))
                         + (((long) b[i + 5] & 0xFF) << (5 * 8)) + (((long) b[i + 4] & 0xFF) << (4 * 8))
                         + (((long) b[i + 3] & 0xFF) << (3 * 8)) + (((long) b[i + 2] & 0xFF) << (2 * 8))
                         + (((long) b[i + 1] & 0xFF) << (1 * 8)) + ((long) b[i + 0] & 0xFF);
        }
    ...
    public static float parseFloat(byte b[], int ofs, boolean isBigEndian, float dft) {
        ...
        if (isBigEndian) {
            floatInt = (((int) b[i + 0] & 0xFF) << (3 * 8)) + (((int) b[i + 1] & 0xFF) << (2 * 8))
                       + (((int) b[i + 2] & 0xFF) << (1 * 8)) + ((int) b[i + 3] & 0xFF);
        } else {
            floatInt = (((int) b[i + 3] & 0xFF) << (3 * 8)) + (((int) b[i + 2] & 0xFF) << (2 * 8))
                       + (((int) b[i + 1] & 0xFF) << (1 * 8)) + ((int) b[i + 0] & 0xFF);
        }
    ...
    public static boolean isBoolean(String data, boolean strict) {
        if (data != null) {
            ...
            for (int i = 0; i < BooleanTRUE.length; i++) {
                boolean ok = strict
                             ? v.equals(BooleanTRUE[i])
                             : v.startsWith(BooleanTRUE[i]);
            ...
            for (int i = 0; i < BooleanFALSE.length; i++) {
                boolean ok = strict
                             ? v.equals(BooleanFALSE[i])
                             : v.startsWith(BooleanFALSE[i]);
     ...
     * @return The hex representation in the form of a StringBuffer
     */
    public static StringBuffer formatHexString(byte b[], int bOfs, int bLen, int blockLen, boolean showAscii,
            StringBuffer sb) {
        int headerLen = 0;
        ...
        int bi      = (bOfs >= 0)
                      ? bOfs
                      : 0;    // byte index
        int bMaxNdx = ((bLen >= 0) && ((bi + bLen) <= b.length))
                      ? (bi + bLen)
                      : b.length;

        /* validate block length */
        if (blockLen <= 0) {
            blockLen = ((bMaxNdx - bi) < 16)
                       ? bLen
                       : 16;
        }

        /* position ruler */
        int rulerLen = (headerLen > blockLen)
                       ? headerLen
                       : blockLen;
    ...
    public static String toHexString(long val, int bitLen) {
        ...
        if (bitLen <= 0) {
            if ((val & 0xFFFFFFFF00000000L) != 0L) {
                bitLen = 64;
            } else if ((val & 0x00000000FFFF0000L) != 0L) {
                bitLen = 32;
            } else if ((val & 0x000000000000FF00L) != 0L) {
                bitLen = 16;
            } else {
                bitLen = 8;
            }
    ...
    public static String[] parseArray(String s, char arrayDelim) {
        ...
        for (int a = 0; ; ) {
            ...
            } else if (ch[a] == arrayDelim) {    // token == ','

                // blank entry
                v.add("");                                                                  // add blank entry
                a++;                                                                        // skip delimiter
            } else if ((ch[a] == ARRAY_DOUBLE_QUOTE) || (ch[a] == ARRAY_SINGLE_QUOTE)) {    // token = '\"'
    ...
    public static String encodeArray(Object list[], int ofs, int max, char delim, boolean alwaysQuote) {
        ...
        if (list != null) {
            ...
            for (int i = ((ofs >= 0)
                ...
                String s = (list[i] != null)
                           ? list[i].toString()
                           : "";

                if (alwaysQuote || (s.indexOf(' ') >= 0) || (s.indexOf('\t') >= 0) || (s.indexOf('\"') >= 0)
                        || (s.indexOf(delim) >= 0)) {
    ...
    public static String encodeArray(Object list, int ofs, int max, char delim, boolean alwaysQuote) {
        if (list == null) {
            ...
            if ((ofs <= 0) && (max != 0)) {
                ...
                return alwaysQuote
                       ? StringTools.quoteString(list.toString())
                       : list.toString();
            } else {
    ...
    public static Map<String, String> parseProperties(String props) {
        return StringTools.parseProperties(props, StringTools.PropertySeparatorChar, StringTools.KeyValSeparatorChars,
    ...
    public static Map<String, String> parseProperties(String props, char propSep) {
        return StringTools.parseProperties(props, propSep, StringTools.KeyValSeparatorChars,
                                           (Map<String, String>) null);
    }
    ...
    public static Map<String, String> parseProperties(String props, char propSep, Map<String, String> properties) {
        return StringTools.parseProperties(props, propSep, StringTools.KeyValSeparatorChars, properties);
    }
    ...
    public static Map<String, String> parseProperties(String props, char propSep, char keyValSep[]) {
        return StringTools.parseProperties(props, propSep, keyValSep, (Map<String, String>) null);
    }
    ...
    public static Map<String, String> parseProperties(String props, char propSep, char keyValSep[],
        ...
        for (; a < argsLen; ) {
            ...
            while ((a < argsLen) && (Character.isWhitespace(args[a]) || _isSepChar(args[a], propSep))) {
                a++;
            }
            ...
            for (; (a < argsLen) &&!Character.isWhitespace(args[a]) &&!_isSepChar(args[a], propSep)
                    &&!_isSepChar(args[a], keyValSep);
                    a++) {
            ...
            while ((a < argsLen) &&!_isSepChar(args[a], propSep)) {
                a++;
            }    // trailing junk

            if ((a < argsLen) &&!_isSepChar(args[a], propSep)) {
                a++;
    ...
    public static String replaceKeys(String text, KeyValueMap keyMap, ValueFilter filter, String keyStart,
        ...
        for (; (ks >= 0); ) {
            ...
            int ksi = ((ks > 0) && (repText.charAt(ks - 1) == KEY_START_ESC))
                      ? ks - 1
                      : ks;
            ...
            String kv = (keyMap != null)
                        ? keyMap.getKeyValue(key, arg, null)
                        : null;
            String fv = (kv != null)
                        ? ((filter != null)
                           ? filter.getFilteredValue(kv)
                           : kv)
                        : dftStr;
    ...
    public static StringBuffer encodeNewline(StringBuffer sb) {
        if (sb != null) {
            for (int c = 0; c < sb.length(); ) {
                ...
                // HCM_MOD: JPMS 2012/04/19 - below lines commented, i think that is only necessary with mysql
    ...
    public static String insertKeyValues(String text, String startDelim, String endDelim, String dftDelim,
        ...
        if (text != null) {
            ...
            return insertKeyValues(text, startDelim, endDelim, dftDelim, repMap, htmlFilter);
        } else {
            return text;
        }
    }
    ...
    public static String insertKeyValues(String text, String startDelim, String endDelim, String dftDelim,
        ...
        if (text != null) {
            ...
            return insertKeyValues(text, startDelim, endDelim, dftDelim, rm, htmlFilter);
        } else {
            return text;
        }
    }
            ...
            StringTools.KeyValueMap rmap, boolean htmlFilter)    // ReplacementMap
    {
        ...
        while (s < sb.length()) {
            ...
            String val = (rmap != null)
                         ? rmap.getKeyValue(key, arg, dftStr)
                         : ("?" + key + "?");

            if (val != null) {

                // Print.sysPrintln("Found Key: " + key + " [replace with '" + val + "']");
                sb.replace(s, e + endDelimLen, (htmlFilter
                                                ? StringTools.htmlFilterText(val)
                                                : val));
                s += val.length();
            } else {
    ...
    public static int compare(byte b1[], String s) {
        return StringTools.compare(b1, ((s != null)
                                        ? StringTools.getBytes(s)
                                        : null), -1);
    }
    ...
    public static boolean compareEquals(byte b[], String s) {
        return StringTools.compareEquals(b, ((s != null)
                ? StringTools.getBytes(s)
                : null), -1);
    }
    ...
    public static String format(long val, String fmt, int fieldSize) {
        ...
        if (fmt == null) {
            ...
            if (val > 0L) {
                int      p  = fmt.indexOf(':');
                TimeZone tz = (p > 0)
                              ? DateTime.getTimeZone(fmt.substring(p + 1))
        ...
        for (int i = 0; i < ch.length; i++) {
            if ((i == 0) && (ch[i] == ' ')) {
                ...
                case '"' :
                    ...
                    break;             // seems to break some html display
    ...
    public static class FilterNumber {
        ...
        public FilterNumber(String val, Class<?> type) {
            ...
            if (hex) {
                ...
                for (ps = p; (p < vlen) && ("0123456789ABCDEF".indexOf(Character.toUpperCase(v.charAt(p))) >= 0); ) {
                    p++;
                }
            } else {
        ...
        public <T> boolean supportsType(Class<T> ct) {
            if ((this.numStr != null) && (this.type != null) && (ct != null)) {
                ...
                if (Short.class.isAssignableFrom(ct)) {
                ...
                } else if (Integer.class.isAssignableFrom(ct)) {
                    ...
                    return this.type.isAssignableFrom(Integer.class) || this.type.isAssignableFrom(Short.class)
                           || this.type.isAssignableFrom(Byte.class);
                } else if (BigInteger.class.isAssignableFrom(ct)) {

                    // return this.supportsType(Long.class);       // ct is 'BigInteger', check for promotion from 'Long'
                    return this.type.isAssignableFrom(Long.class) || this.type.isAssignableFrom(Integer.class)
                           || this.type.isAssignableFrom(Short.class) || this.type.isAssignableFrom(Byte.class);
                    ...
                    return this.type.isAssignableFrom(BigInteger.class) || this.type.isAssignableFrom(Long.class)
                           || this.type.isAssignableFrom(Integer.class) || this.type.isAssignableFrom(Short.class)
                           || this.type.isAssignableFrom(Byte.class);
                } else if (Double.class.isAssignableFrom(ct)) {

                    // return this.supportsType(Float.class);      // ct is 'Double', check for promotion from 'Float'
                    return this.type.isAssignableFrom(Float.class) || this.type.isAssignableFrom(BigInteger.class)
                           || this.type.isAssignableFrom(Long.class) || this.type.isAssignableFrom(Integer.class)
                           || this.type.isAssignableFrom(Short.class) || this.type.isAssignableFrom(Byte.class);
                }
            }
        ...
        public Number toNumber(Number dft) {
            if ((this.numStr != null) && (this.type != null)) {
                try {
                    ...
                    } else if (BigInteger.class.equals(this.type)) {
