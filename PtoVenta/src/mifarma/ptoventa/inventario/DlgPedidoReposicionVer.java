...
public class DlgPedidoReposicionVer extends JDialog {
    ...
    private void initTable() {
        tableModel =
                new FarmaTableModel(ConstantsInventario.columnsListaProductosPedidoReposicionVer, ConstantsInventario.defaultValuesListaProductosPedidoReposicionVer,
                                    0);
        FarmaUtility.initSimpleList(tblListaProductos, tableModel,
                                    ConstantsInventario.columnsListaProductosPedidoReposicionVer);
        //cargaListaProductos();
    }
        ...
        if (mifarma.ptoventa.reference.UtilityPtoVenta.verificaVK_F1(e)) {
            ...
            if (tableModel.data.size() > 0)
                if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                                                                                "¿Está Seguro de enviar el Pedido de Reposicion?")) {
                    if (enviarPedido()) {
    ...
    private void cargaListaProductos_filtro() {
        try {
            tableModel.clearTable();
            DBInventario.cargaListaPedidoReposicionDetalle_filtro(tableModel, VariablesPtoVenta.vTipoFiltro,
        ...
        if (tblListaProductos.getRowCount() > 0) {
            ...
            if (tblListaProductos.getValueAt(row, 6).toString().trim().equals("")) {
                if (validaVentaUltimosMeses(row)) {
                    cargarCabecera();
                    DlgPedidoReposicionIngresoCantidad dlgPedidoReposicionIngresoCantidad =
                        new DlgPedidoReposicionIngresoCantidad(myParentFrame, "", true);
    ...
    private void funcionF10() {
        ...
        FarmaLoadCVL.loadCVLfromArrays(dlgOrdenar.getCmbCampo(), VariablesInventario.vNombreInHashtable, IND_CAMPO,
        ...
        if (tableModel.data.size() != 0)
            try {
                ...
                FarmaUtility.showMessage(this,
                                         "Ocurrió un error al obtener información del pedido (adicionales) : \n" +
                        sql.getMessage(), txtBuscar);
            }
    }
    ...
    private void funcionF8() {
        ...
        log.debug("VariablesInventario.vCantMax_PedRep " +
                  FarmaUtility.getDecimalNumber(VariablesInventario.vCantMax_PedRep));
        log.debug("VariablesInventario.vCant_PedRep    " +
                  FarmaUtility.getDecimalNumber(VariablesInventario.vCant_PedRep.trim()));
        ...
        if (FarmaUtility.getDecimalNumber(VariablesInventario.vCant_PedRep.trim()) > 0) {
            if (FarmaUtility.getDecimalNumber(VariablesInventario.vCantMax_PedRep.trim()) ==
                FarmaUtility.getDecimalNumber(VariablesInventario.vCant_PedRep.trim())) {
                /////
                DlgIngresoCantidadAdicional dlgIngresoCantAdic =
                    new DlgIngresoCantidadAdicional(myParentFrame, "", true);
                dlgIngresoCantAdic.setVisible(true);

                if (FarmaVariables.vAceptar) {
                    tblListaProductos.setValueAt(VariablesInventario.vCantAdicional,
                                                 tblListaProductos.getSelectedRow(), 9);
                    tblListaProductos.repaint();
                    FarmaVariables.vAceptar = false;
                }
                /////
                ...
                FarmaUtility.showMessage(this,
                                         "Para agregar adicional la cantidad solicitada debe ser igual a la cantidad maxima.",
                                         txtBuscar);
        } else
            FarmaUtility.showMessage(this, "Para agregar adicional la cantidad solicitada debe ser mayor a cero.",
                                     txtBuscar);
    }
