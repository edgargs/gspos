          ...
          else
          {
            ...
            if(VariablesVentas.vEsPedidoConvenio)generarPedidoConvenio();
                ...
                else if (e.getKeyCode() == KeyEvent.VK_F5)
                {
                    ...
                    if (myJTable.getName().equalsIgnoreCase(ConstantsVentas.NAME_TABLA_PRODUCTOS))
                        VariablesVentas.vCodProdFiltro = FarmaUtility.getValueFieldJTable(myJTable, 
                                                                                        vFila, 
                                                                                        COL_COD);
                    else
...
(ArrayList)VariablesVentas.vArrayList_Prod_Promociones.get(i);
    ...
    private boolean validaStockDisponible() {
        if (FarmaUtility.isInteger(stkProd) && Integer.parseInt(stkProd) > 0)
            ...
            if(FarmaUtility.isInteger(stkProd) && Integer.parseInt(stkProd) == 0&& UtilityVentas.getIndVtaNegativa())
              return true;
          else
            return false;
        }
    ...
    private void muestraProductoPromocion(int pColumna, JLabel pLabel) {
        ...
        if (myJTable.getRowCount() > 0) {
            String indProdProm = 
                ((String)(myJTable.getValueAt(myJTable.getSelectedRow(), 
                                              17))).trim();
            log.debug("indProdProm: " + indProdProm + " myJTable: " + 
                               myJTable);
