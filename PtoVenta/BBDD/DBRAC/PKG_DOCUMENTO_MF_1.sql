            ...
            SELECT * FROM
                         (
                          SELECT
                                ...
                                -- KMONCADA FACTURACION ELECTRONICA
                              (SELECT COD_TIPODOC
                                 FROM BTLPROD.AUX_TIPO_DOC_CONV
                                WHERE TIP_COMP_PAGO = CP.TIP_COMP_PAGO
                                  AND COD_GRUPO_CIA = CP.COD_GRUPO_CIA) COD_TIPO_DOCUMENTO,
...
CURSOR C_DET(pNum_Ped_Vta_in CHAR, pCodGrupo_Cia_in CHAR,pCod_local_in CHAR) IS
       SELECT
           (SELECT CASE -- KMONCADA 23.10.2014 FACTURACION ELECTRONICA
               WHEN R.COD_TIP_PROC_PAGO = '1' AND  COD_TIPODOC='TKB' THEN 'BOL'
               ELSE COD_TIPODOC END
            FROM  BTLPROD.AUX_TIPO_DOC_CONV
            WHERE TIP_COMP_PAGO=R.TIP_COMP_PAGO
            AND COD_GRUPO_CIA=R.COD_GRUPO_CIA) COD_TIPO_DOCUMENTO,
            R.NUM_COMP_PAGO NUM_DOCUMENTO
    ...
    cursor c_datos_liquidaciones is
        SELECT DISTINCT COD_LOCAL,
          ...
          FROM (SELECT (SELECT L.Cod_Local
                           FROM NUEVO.AUX_MAE_LOCAL l
                          WHERE /*COD_LOCAL_SAP BETWEEN '001' AND '599' */
                          (COD_LOCAL_SAP BETWEEN '001' AND '599' or
 ...
 ******************************************************************************/
    ...
    BEGIN
      SELECT L.COD_LOCAL, ML.CIA
       ...
       WHERE L.COD_LOCAL_SAP = pCod_local_in
         AND (L.FCH_MIGRACION IS NOT NULL OR (L.COD_LOCAL_SAP BETWEEN '001' AND '599'));
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20000, 'No se ha encontrado el local: ' || pCod_local_in);
    END;
 ...
 BEGIN
   ...
   WHERE (L.COD_LOCAL_SAP BETWEEN '001' AND '599' OR L.FCH_MIGRACION IS NOT NULL)
