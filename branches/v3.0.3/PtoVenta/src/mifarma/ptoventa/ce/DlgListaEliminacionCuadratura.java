 ...
 *
 */
public class DlgListaEliminacionCuadratura extends JDialog {
    /* ********************************************************************** */
    ...
    private void jbInit() throws Exception {
        ...
        this.setSize(new Dimension(514, 297));
    ...
    private void initTableDineroFalso() {
        tableModel =
                new FarmaTableModel(ConstantsCajaElectronica.columsListaEliminacionDineroFalso, ConstantsCajaElectronica.defaultListaEliminacionDineroFalso,
                                    0);
        FarmaUtility.initSelectList(tblListaEliminacion, tableModel,
                                    ConstantsCajaElectronica.columsListaEliminacionDineroFalso);
    }

    private void initTableRobo() {
        tableModel =
                new FarmaTableModel(ConstantsCajaElectronica.columsListaEliminacionRobo, ConstantsCajaElectronica.defaultListaEliminacionRobo,
                                    0);
        FarmaUtility.initSelectList(tblListaEliminacion, tableModel,
                                    ConstantsCajaElectronica.columsListaEliminacionRobo);
    }

    private void initTableOtrasCuadraturas() {
        tableModel =
                new FarmaTableModel(ConstantsCajaElectronica.columsListaEliminacionOtrasCuadraturas, ConstantsCajaElectronica.defaultListaEliminacionOtrasCuadraturas,
                                    0);
        FarmaUtility.initSelectList(tblListaEliminacion, tableModel,
                                    ConstantsCajaElectronica.columsListaEliminacionOtrasCuadraturas);
    }

    private void initTableCotizacionCompetencia() {
        log.debug("BBBBBBBBBBB");
        tableModel =
                new FarmaTableModel(ConstantsCajaElectronica.columnsEliminacionCotizacionCompetencia_Turno, ConstantsCajaElectronica.defaultValuesEliminacionCotizacionCompetencia_Turno,
                                    0);
        FarmaUtility.initSelectList(tblListaEliminacion, tableModel,
                                    ConstantsCajaElectronica.columnsEliminacionCotizacionCompetencia_Turno);
    ...
    private void cargaLista() {
        try {
            DBCajaElectronica.listaEliminacionCuadratura(tableModel, VariablesCajaElectronica.vSecMovCaja,
    ...
    private void obtieneVariablesEliminacion() {
        ...
        if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                                                                        "¿Está seguro de eliminar la(s) cuadratura(s)?")) {
            if (VariablesCajaElectronica.vCodCuadratura.equalsIgnoreCase(ConstantsCajaElectronica.CUADRATURA_REGULARIZACION_ANULADO_PENDIENTE) ||
                VariablesCajaElectronica.vCodCuadratura.equalsIgnoreCase(ConstantsCajaElectronica.CUADRATURA_REGULARIZACION_COMP_MANUAL)) {
                if (!cargaLoginOper()) {
                    FarmaUtility.liberarTransaccion();
                    FarmaUtility.showMessage(this,
                                             "No se realizó la operación. Solo un usuario con Rol de\nOperador de Sistemas puede eliminar esta cuadratura.",
                                             tblListaEliminacion);
                    return;
                }
            }
            try {
                for (int i = 0; i < VariablesCajaElectronica.listaEliminacion.size(); i++) {
                    VariablesCajaElectronica.vSecMovCaja =
                            (String)(((ArrayList)VariablesCajaElectronica.listaEliminacion.get(i)).get(0));
                    VariablesCajaElectronica.vIndVBCajero =
                            (String)(((ArrayList)VariablesCajaElectronica.listaEliminacion.get(i)).get(1));
                    VariablesCajaElectronica.vSecCuadratura =
                            (String)(((ArrayList)VariablesCajaElectronica.listaEliminacion.get(i)).get(2));
                    if (VariablesCajaElectronica.vIndVBCajero.equalsIgnoreCase(FarmaConstants.INDICADOR_N)) {
                        if (!evaluaValidacionEliminacion()) {
                            ...
                            FarmaUtility.showMessage(this,
                                                     "No puede eliminar el registro. Primero elimine la(s)\ncuadratura(s) que han sido originadas por esta.",
                                                     tblListaEliminacion);
                        } else {
                        ...
                        FarmaUtility.showMessage(this,
                                                 "No se puede eliminar el registro. El Movimiento de Caja ya cuenta con VB de Cajero.",
                                                 tblListaEliminacion);
                        break;
                    }
                }
    ...
    private boolean evaluaValidacionEliminacion() {
        ...
        try {
            VariablesCajaElectronica.vValidaEliminacionCuadratura =
                    DBCajaElectronica.validacionEliminacion(VariablesCajaElectronica.vCodCuadratura,
                                                            VariablesCajaElectronica.vSecMovCaja,
                                                            VariablesCajaElectronica.vSecCuadratura);
            log.debug("VariablesCajaElectronica.vValidaEliminacionCuadratura : " +
                      VariablesCajaElectronica.vValidaEliminacionCuadratura);
            if (VariablesCajaElectronica.vValidaEliminacionCuadratura == 0)
                return true;
            else
                return false;
        } catch (SQLException sql) {
    ...
    private void cargaVariables() {
        ...
        VariablesCajaElectronica.vSecMovCaja =
                tblListaEliminacion.getValueAt(row, ConstantsCajaElectronica.INDICE_COLUMNA_SEC_MOV_CAJA).toString().trim();
        VariablesCajaElectronica.vIndVBCajero =
                tblListaEliminacion.getValueAt(row, ConstantsCajaElectronica.INDICE_COLUMNA_IND_VB).toString().trim();
        VariablesCajaElectronica.vSecCuadratura =
                tblListaEliminacion.getValueAt(row, ConstantsCajaElectronica.INDICE_COLUMNA_SEC_CUADRATURA).toString().trim();
