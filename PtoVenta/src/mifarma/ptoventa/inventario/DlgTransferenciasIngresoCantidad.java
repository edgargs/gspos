...
public class DlgTransferenciasIngresoCantidad extends JDialog {
    ...
    private boolean isLoteValido = false;
    ...
    private void jbInit() throws Exception {
        ...
        txtFechaVenc.addFocusListener(new java.awt.event.FocusAdapter() {
            public void focusGained(FocusEvent e) {
                txtFechaVenc_focusGained(e);
            }
        });
    ...
    private void txtCantidad_keyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_ENTER) {
            log.debug("vTransfMatriz: "+VariablesInventario.vTransfMatriz);
            //if (VariablesInventario.vTransfMatriz && !VariablesPtoVenta.vTipoLocalVenta.equals(ConstantsPtoVenta.TIPO_LOCAL_VTA_MAYORISTA)) {
            //KMONCADA 05.08.2016 [PROYECTO M] FUNCION PARA VALIDAR LOCAL MAYORISTA
            if (VariablesInventario.vTransfMatriz && !UtilityPtoVenta.isLocalVentaMayorista()) {
                ...
                //{
                //RHERRERA 10.09.2014 // valida la cantidad ingresada antes de toda operación
                log.debug("Validar CANTIDAD");
            ...
            } else if (!txtFraccion.isEditable()){
                log.debug("txtLote");
    ...
    private boolean validarCampos() {
        ...
        if (txtCantidad.getText().trim().length() == 0) {
            ...
            retorno = false;
        } else if (txtFraccion.getText().trim().length() == 0 && !VariablesInventario.vTransfMatriz &&
                   productoFraccionado) {
    ...
    // gmoran-14012016 -->  busqueda del lote 
    private String buscarProductoLote() {
        ...
        String fechaVencLote = "";
        // String fecVencLote = txtFechaVenc.getText().trim();
        try {
            fechaVencLote = DBInventario.obtieneProductoLote(cadena);
            log.debug("fechaVencLote: "+fechaVencLote);
            return fechaVencLote;
    ...
    private void aceptaCantidadIngresada() {
        if (validarCampos()) {
            ...
            //if(com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,"¿Está seguro de guardar los datos?"))
            {
                cargarDatos();
        ...
        } else //(VariablesInventario.vTransfMatriz)//si es transferencia a matriz
        {
            ...
            if (result % frac != 0) {
                ...
                FarmaUtility.showMessage(this,
                                         "Al enviar una Transferencia a Matriz, debe ingresar \nla cantidad expresada en Unidad de Presentacion del Producto.\nEjemplo:" +
                                         frac * 1 + "," + frac * 2 + "," + frac * 10 + "...", txtCantidad);
            }
        }
        return valor;
    }
    ...
    } else*/VariablesInventario.vCantIngreso =
                Integer.parseInt(cantidad) * Integer.parseInt(lblValorFrac.getText()) + Integer.parseInt(fraccion);
    ...
    private void obtieneStockProducto() {
        ...
        if (myArray.size() == 1) {
            ...
            if(UtilityPtoVenta.isLocalVentaMayorista()){ //KMONCADA 17.03.2016 MENSAJE PARA LOCALES M
                FarmaUtility.showMessage(this, "No se pudo obtener Stock del Producto y Lote.", null);
            }else{
    ...
    private void obtieneStockProducto_ForUpdate(ArrayList pArrayList) {
        try {
            if(UtilityPtoVenta.isLocalVentaMayorista()){
                DBInventario.obtieneStockProducto_ForUpdate_Lote(pArrayList, VariablesInventario.vCodProd_Transf, 
                                                                 txtLote.getText().trim(), txtFechaVenc.getText().trim());
            }else{
    ...
    // gmoran obtiene la fecha de vencimiento segun lo y producto y tambien si pertenece al local(003)
    private void validaLoteProducto(){
        if(UtilityPtoVenta.isLocalVentaMayorista()){
            isLoteValido = false;
            String fechaVencLote =  buscarProductoLote();
            if(!fechaVencLote.equals(" ")){
                txtFechaVenc.setText(fechaVencLote);
                txtLote.setEditable(true);
                isLoteValido = true;
            }else{
                FarmaUtility.showMessage(this, "Verificar el Lote", txtLote);
    ...
    private void txtFechaVenc_focusGained(FocusEvent e){
        if(UtilityPtoVenta.isLocalVentaMayorista()){
            if(!isLoteValido){
                FarmaUtility.showMessage(this, "Verificar el Lote", txtLote);
            }else{
