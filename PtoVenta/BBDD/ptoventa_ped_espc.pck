CREATE OR REPLACE PACKAGE PTOVENTA."PTOVENTA_PED_ESPC" AS
  ...
  --Fecha       Usuario   Comentario
  --10/09/2008  JCORTEZ     Creación
  --13.01.2015  KMONCADA    MODIFICACION : CAMBIO PARA QUE LISTE SEGUN LOCAL TIPO VENTA M
 ...
 /******************************************************************************/
  ...
  BEGIN
    ...
    SELECT P.COD_PROD || 'Ã' ||
          ...
          CASE
            WHEN LOCAL.COD_TIP_LOCAL_VENTA = PKG_CONSTANTES.LOCAL_TIPO_VTA_M THEN
              TO_CHAR(L.VAL_PREC_VTA * L.VAL_FRAC_LOCAL,'999990')
            ELSE
              TO_CHAR(L.VAL_PREC_VTA,'999990')
          END
          --TO_CHAR(SYSDATE,'dd/MM/yyyy HH24:MI:SS')
    FROM LGT_PROD P, LGT_PROD_LOCAL L, LGT_LAB B, PBL_LOCAL LOCAL
    WHERE P.COD_GRUPO_CIA = cGrupoCia_in
          ...
          AND L.COD_GRUPO_CIA = LOCAL.COD_GRUPO_CIA
          AND L.COD_LOCAL = LOCAL.COD_LOCAL;
