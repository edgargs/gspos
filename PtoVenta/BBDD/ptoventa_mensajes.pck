CREATE OR REPLACE PACKAGE PTOVENTA."PTOVENTA_MENSAJES" AS
  ...
  */
  ...
  FUNCTION F_VAR2_GET_MENSAJE(cGrupoCia_in  IN CHAR,
                            cCod_Rol_in   IN CHAR )
  RETURN VARCHAR2;
    FUNCTION GET_MSG_FIDEICOMIZO(cCodGrupoCia in char,
                               cCodLocal in char, 
                               cObligado IN CHAR DEFAULT 'N')
    RETURN VARCHAR2;
...
CREATE OR REPLACE PACKAGE BODY PTOVENTA."PTOVENTA_MENSAJES" IS
  ...
  */
  ...
  nNumPedDel number;
   ...
   n_CodGrupo   NUMBER;
   nCTD_Recargas_PendCobro number;
  BEGIN
			--ERIOS 2.4.7 Se agrega limite (JLUNA)
            SELECT count(D.NUM_PED_VTA) into nCTD_Recargas_PendCobro
              FROM vta_pedido_vta_det d, lgt_prod_virtual p
             WHERE (d.cod_grupo_cia,d.cod_local) in (select ip.cod_grupo_cia,ip.cod_local from vta_impr_ip ip)
               AND d.num_ped_vta IN
                   (SELECT VTA_CAB.NUM_PED_VTA
                      FROM VTA_PEDIDO_VTA_CAB VTA_CAB
                     WHERE (VTA_CAB.cod_grupo_cia,VTA_CAB.cod_local) in (select ip.cod_grupo_cia,ip.cod_local from vta_impr_ip ip)
                       AND VTA_CAB.FEC_PED_VTA <
                           (SYSDATE - (10 / 24 / 60 / 60))
...
*/
          ...
          DBMS_OUTPUT.put_line('n_IdColorLetra=' || n_IdColorLetra||'---'||nCTD_Recargas_PendCobro);
    ...
    --'<FONT FACE = "' || T.DESC_TIPO_LETRA || '" color="' || C.DESC_COLOR  || '">' || NVL(M.TEXTO,' ') || '</FONT></HTML>'
          SELECT '<HTML> '||
                 ...
                 '<div align="center"><font color="#FFFFFF" '||
                  'size ="4" face="' || T.DESC_TIPO_LETRA || '">' ||
                 --'<br>'||
                 '<b>'||NVL(M.TEXTO,' ')||'</b>'||
                 --'<br>'||
                 (case 
                 when nCTD_Recargas_PendCobro > 0 then
                 '  .............................................<br>'||
                   (case
                   when nCTD_Recargas_PendCobro = 1 then
                   '<b>'||NVL(''||1 ||' Recarga Pendiente por COBRAR.',' ')||'</b>'
                   else
                   '<b>'||NVL(''||nCTD_Recargas_PendCobro||' Recargas Pendientes por COBRAR.',' ')||'</b>'
                   end)
                 end)                 ||
                 '</font></div></td> '||
                 ' </tr>'||
  ...
  -- KMONCADA 24.07.2015 SE AGREGA PARAMETRO cObligado PARA IMPRESION DE BTLAMAZONIA EN TICKET BOLETA
  FUNCTION GET_MSG_FIDEICOMIZO(cCodGrupoCia in char,
                               cCodLocal in char, 
                               cObligado IN CHAR DEFAULT 'N') 
    RETURN VARCHAR2
