...
*/
  ...
  END;
  /* *************************************************** */
  PROCEDURE P_GENERA_TARJETA_DNI(cCodGrupoCia          IN CHAR,
                                 cCodLocal_in          IN CHAR,
                                 cDni_in               IN CHAR,
                                 cNroTarjetaLealtad_in IN CHAR DEFAULT NULL) AS
    PRAGMA AUTONOMOUS_TRANSACTION;

    nCantReniec  number;
    nCantCliente number;

    vPrefijo_tarjeta varchar2(20) := 'V99999';
    vCodTarjeta      FID_TARJETA.COD_TARJETA%TYPE;

    vSecuencialNumera         VARCHAR2(20);
    vConcatenado              VARCHAR2(50);
    vNuevaTarjetaFidelizacion FID_TARJETA.COD_TARJETA%TYPE;
    -- KMONCADA 12.01.2015 NUEVO FORMATO DE CORRELATIVO DE TARJETA VIRTUAL DE FIDELIZADO
    ...
    vNroTarjetaLealtadFinal VARCHAR2(20);
    iy INTEGER := 0;
    vImportadoOrbis CHAR(1) := 'N';
  BEGIN
    -- KMONCADA 14.08.2015 VERIFICA SI EL CLIENTE SE IMPORTE DE ORVEE
    SELECT DECODE(COUNT(1),0,'N','S')
    INTO vImportadoOrbis
    FROM PBL_CLIENTE A
    WHERE A.DNI_CLI = cDni_in
    AND A.IND_ENVIADO_ORBIS = 'E';
  
  
    IF cNroTarjetaLealtad_in IS NOT NULL THEN 
      IF FARMA_PUNTOS.F_VAR_TARJ_VALIDA(cCodGrupoCia, cCodLocal_in, cNroTarjetaLealtad_in) = 'S' THEN
        vNroTarjetaLealtadFinal := cNroTarjetaLealtad_in;
      ELSE
        vNroTarjetaLealtadFinal := NULL;
      END IF;
    ELSE
      vNroTarjetaLealtadFinal := NULL;
    END IF;
    ...
    */
    -- vDatosDNI := utility_dni_reniec.aux_datos_existe_dni('001','000',cDni_in);
    -- KMONCADA 24.09.2014 VERIFICA LONGITUD DE CADENA DNI
    IF LENGTH(TRIM(cDni_in)) = 8 AND vImportadoOrbis = 'N' THEN
      vDatosDNI := utility_dni_reniec.aux_datos_existe_dni('001', '000', cDni_in);
    ELSE
      vDatosDNI := 'N';
    END IF;
    ...
    VAL_DNI := cDni_in;
    IF vNroTarjetaLealtadFinal IS NOT NULL THEN
      /*select count(1)
      into iy
      from PTOVENTA.vta_rango_tarjeta
      WHERE DESDE<= substr(vNroTarjetaLealtadFinal,1,12)
      AND HASTA>= substr(vNroTarjetaLealtadFinal,1,12)
    ...
    if nCantCliente = 0 then
      ...
      if nCantReniec > 0 then 
        IF vImportadoOrbis = 'N' THEN
          ...
          */
            SELECT VAL_DNI dni,
              ...
              FROM DUAL;
        END IF;
        IF vNroTarjetaLealtadFinal IS NULL THEN 
        ...
        ELSE
          vNuevaTarjetaFidelizacion := vNroTarjetaLealtadFinal;
        END IF;
        vNuevaTarjetaFidelizacion := TRIM(vNuevaTarjetaFidelizacion);
        begin
        ...
        vCodTarjeta := TRIM(vCodTarjeta);
        update FID_TARJETA
           set DNI_CLI    = VAL_DNI
               /*IND_PUNTOS = CASE
                              WHEN vNroTarjetaLealtadFinal IS NULL THEN
                               'N'
                              ELSE
                               'S'
                            END,
               IND_ENVIADO_ORBIS = CASE
                                     WHEN vNroTarjetaLealtadFinal IS NOT NULL THEN
                                       'P'
                                   END*/
    ...
    ELSE
      --ini ASOSA, 06.04.2010
      if nCantReniec > 0 AND vImportadoOrbis = 'N' THEN
      ...
      ELSE
        ...
        WHERE t.Dni_Cli = VAL_DNI
        AND T.IND_PUNTOS = 'S'
        AND T.COD_TARJETA = vNroTarjetaLealtadFinal;
      END IF;
