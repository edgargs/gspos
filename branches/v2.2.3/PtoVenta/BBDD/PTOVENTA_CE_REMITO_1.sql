...
dbms_output.put_line('hito05');
  ...
  /*
   ...
   into   vCliente
   from   pbl_tab_gral t
   where  t.id_tab_gral = 384;
  */
   select g.raz_soc_cia||' '|| e.cod_local||' - '||e.desc_corta_local,
          nvl(e.DIREC_LOCAL,e.DIREC_LOCAL_CORTA), G.COD_CIA
   into   vRecibidoDe,vCalle, vCodCia
   from   PBL_LOCAL E
    --Agregado por Luigy Terrazos 
    --Fecha 28/02/2012
    --Motivo Obtener la razon social
          ,PBL_CIA G
   where  E.COD_GRUPO_CIA = cCodGrupoCia_in
   AND    E.COD_LOCAL     = cCodLocal_in
    --Agregado por Luigy Terrazos 
  ...
  BEGIN
    ...
    IF cant=0 THEN
        ...
        (COD_REMITO,COD_GRUPO_CIA,COD_LOCAL,ESTADO,USU_CREA_REMITO,FEC_CREA_REMITO,IND_INT_REMITO,PRECINTO)
