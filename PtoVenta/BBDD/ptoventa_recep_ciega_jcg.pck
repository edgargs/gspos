CREATE OR REPLACE PACKAGE PTOVENTA."PTOVENTA_RECEP_CIEGA_JCG" is
  ...
  --Descripcion: Obtiene los datos para imprimir las diferencias de forma termica
  --Fecha       Usuario   Comentario
  --16/08/2016  CCASTILLO   Creación
  FUNCTION RECEP_F_VAR2_IMP_DATOS_DIFE_T(cGrupoCia_in  IN CHAR,
                                         cCodLocal_in  IN CHAR,
                                          cNroRecepcion IN CHAR)
  RETURN FARMACURSOR;
...
-- JMIRANDA 11.05.10
 ...
 /*************************************************************************************************************/
  ...
  /*** INICIO CCASTILLO 16/08/2016 ***/
    ...
    RETURN FARMACURSOR
    IS
    curVoucherDiferencia    FARMACURSOR;
    vIdDoc VARCHAR2(30);
    vIpPC VARCHAR2(30);
    ...
    v_titulo varchar2(1000):='RECONTEO DE DIFERENCIAS';
    v_textprod  varchar2(32767);
    v_textbulto varchar2(32767);                                                                                   
  ...
  BEGIN
	...
	LOOP
      FETCH curDiferencias INTO vProducto,vDescProd,vUnidadPresentacion, vLaboratorio, v_nCant2,v_vUnidadCorta;
      EXIT WHEN curDiferencias%NOTFOUND;
...
*/
      ...
      v_textprod:=vProducto||' - '||RPAD(vDescProd,30) ||' - '|| RPAD(vUnidadPresentacion,11) ||' - '||/*RPAD(*/vLaboratorio/*,10)*/;
      v_vBultos := GET_LISTA_BULTOS_IMP(cGrupoCia_in,cCodLocal_in,cNroRecepcion,vProducto);


      FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc,
                                        vIpPc_in => vIpPC,
                                        vValor_in => v_textprod,--vProducto||' - '||vDescProd ||' - '|| vUnidadPresentacion ||' - '||vLaboratorio ||CHR(10),
      ...
      v_textbulto:=RPAD('En G/R: '||v_nCant2||' '||v_vUnidadCorta||' '|| v_vBultos ||' ',40)||' Cant. Fisica : _______'/*||CHR(10)*/;
      
      FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc,
                                        vIpPc_in => vIpPC,
                                        vValor_in => v_textbulto,--'En G/R: '||v_nCant2||' '||v_vUnidadCorta||' '|| v_vBultos ||' '||CHR(10),
                                        vTamanio_in => FARMA_PRINTER.TAMANIO_0,
                                        vAlineado_in => FARMA_PRINTER.ALING_IZQ);
    ...
    END LOOP;
      ...
      curVoucherDiferencia := FARMA_PRINTER.F_CUR_OBTIENE_DOC_IMPRIMIR(vIdDoc_in => vIdDoc, vIpPc_in => vIpPC);
      RETURN curVoucherDiferencia;
 ...
 END;
  ...
  BEGIN
	...
	TAB AS (SELECT CEIL(rownum/v_nColmunas) GRUPO,
			...
			AND COD_PROD = cCodProd_in )
	--FILAS EN COLUMNAS		
	SELECT listagg (nvl(VALOR,' '), 'Ã') WITHIN GROUP (ORDER BY VALOR) RESULTADO 
