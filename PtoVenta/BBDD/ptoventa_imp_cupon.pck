CREATE OR REPLACE PACKAGE "PTOVENTA_IMP_CUPON" AS
 ...
 FUNCTION IMP_PROCESA_CUPON_1_OLD(cCodGrupoCia_in 	IN CHAR,
                            cCodLocal_in    	IN CHAR,
            								cNumPedVta_in   	IN CHAR,
                            cIpServ_in        IN CHAR,
                            cCodCupon_in      IN CHAR
 ...
 RETURN VARCHAR2;
  ...
  --Descripcion: Genera encabezado de cupon
  --Fecha       Usuario		Comentario
  --16/02/2016  ERIOS       Creacion
  FUNCTION F_ENCABEZADO_CUPON(cCodGrupoCia_in IN CHAR, cCodLocal_in IN CHAR, cCodCupon_in IN CHAR)
  RETURN FarmaCursor;
  
END;
/
CREATE OR REPLACE PACKAGE BODY "PTOVENTA_IMP_CUPON" AS
 ...
 FUNCTION IMP_GET_CUPONES_PEDIDO(cCodGrupoCia_in 	IN CHAR,
  ...
  BEGIN
              ...
              if vIndManual = 'N' then
               ...
               from   PTOVENTA.vta_campana_cupon c
               ...
               and    TRUNC(sysdate) between c.fech_inicio and c.fech_fin
                AND
                    (
                     C.IND_CADENA = 'S'
                     OR
                     (C.IND_CADENA = 'N' AND EXISTS (SELECT 1 FROM PTOVENTA.VTA_CAMP_X_LOCAL CL WHERE CL.COD_CAMP_CUPON = C.COD_CAMP_CUPON AND CL.COD_LOCAL = cCodLocal_in))
                    );
             ...
             vIndDocLocked_in    := PTOVENTA_FIDELIZACION.F_IS_DOC_LOCKED(dDniCli);
               ...
               open vCursor for
               SELECT C.COD_CUPON
               FROM   PTOVENTA.VTA_CAMP_PEDIDO_CUPON C,
                      PTOVENTA.VTA_CAMPANA_CUPON D
               WHERE  C.COD_GRUPO_CIA = cCodGrupoCia_in
               AND    C.COD_LOCAL = cCodLocal_in
               AND    C.NUM_PED_VTA = cNumPedVta_in
               AND    C.ESTADO = 'E'
               ...
               -- dubilluz 18.08.2015
               and   vIndDocLocked_in = 'N'
               union
               -- CUPONES DE SORTEO
               select cp.cod_cupon
               from   PTOVENTA.vta_cupon cp
               where  cp.cod_grupo_cia = cCodGrupoCia_in
               --and    cp.cod_cupon like  cCodLocal_in || SUBSTR(cNumPedVta_in,-7) || '%'
               --and    1 = ctdCampanaSorteo
               -- DUBILLUZ 15.03.2016
               and    cp.cod_cupon like   'C'||cp.cod_campana|| cCodLocal_in || SUBSTR(cNumPedVta_in,-8) || '%'
               -- DUBILLUZ 15.03.2016
               and    cp.cod_campana in
               (
               select cod_camp_cupon from PTOVENTA.vta_campana_cupon C
               where ind_sorteo = 'S'
                 AND ESTADO = 'A'
                AND
                    (
 ...
 ENd;
  ...
  BEGIN
   ...
   and    c.cod_cupon = cCodCupon_in;
     ...
     vFila_IMG_Cabecera_MF:= ''; --ERIOS 16.02.2016 El logo se genera e imprime, con el farmaprinter. 
...
PROCEDURE PROCESA_CUPON_SORTEO(cCodGrupoCia_in 	IN CHAR,
 ...
 CURSOR curCampanaSorteo is
  ...
  BEGIN
    ...
    into   valNeto
    from   PTOVENTA.vta_pedido_vta_cab ca
      ...
      LOOP
       ...
       v_MaxCupones := fSC.Mmaxcupones;
        ...
        if cIndTodosProductos = 'S' then
          ...
          from   PTOVENTA.vta_pedido_vta_det De
          ...
          and   exists (
                        select 1
                        from   PTOVENTA.vta_campana_prod p
           ...
           if nCantidadCupones >= 0 then
             ...
             --Codigo de Cupon es VARCHAR2(20)
                ...
                loop
                  ...
                  nCodigoCupon :=  'C'||cCodCampanaSorteo||cCodLocal_in|| SUBSTR(cNumPedVta_in,-8) || LPAD(i,2,'0');
...
/*-------------------------------------------------------------------------------------------------------------------
GOAL :Procesar cupon
Hist : 2) se agrega campos adicional para incluir texto 
---------------------------------------------------------------------------------------------------------------------- */
 FUNCTION IMP_PROCESA_CUPON_1(cCodGrupoCia_in 	IN CHAR,
                            cCodLocal_in    	IN CHAR,
            								cNumPedVta_in   	IN CHAR,
...
-- 2010-04-22 JOLIVA: Nuevas variables
 ...
 vFila_MsgOTRAFORMA_PAGO varchar2(2800):= '';
  ...
  vc_Desc_Adic vta_campana_cupon.desc_adic%type;
     ...
     --inicio 09.06.2011
       ...
       if vCountFPActivo > 0 then
         ...
         */
         ...
         --- < 13-JUL-15   TCT  Reasigna texto hasta  >
         vIndHasta:='';
         IF TRIM(vc_Desc_Adic)IS NOT NULL THEN
          vIndHasta:=TRIM(vc_Desc_Adic)||' ';
         ELSIF TRIM(vc_Desc_Adic) IS NULL THEN
             IF vIndCostoPromedio ='N' THEN
                vIndHasta:= 'Hasta ';
             END IF;         
         END IF;
        --- < /13-JUL-15   TCT  Reasigna texto hasta  >
...
/*************************************************************************************************************************/
  ...
  BEGIN
	...
	FARMA_PRINTER.P_AGREGA_BARCODE_CODE39(vIdDoc_in => vIdDoc,
                                          ...
                                          vValor_in => cCodCupon_in);
    END IF;										  
	curCupon := FARMA_PRINTER.F_CUR_OBTIENE_DOC_IMPRIMIR(vIdDoc_in => vIdDoc,
                                                                         vIpPc_in => vIpPc);
    RETURN curCupon;
  END;
