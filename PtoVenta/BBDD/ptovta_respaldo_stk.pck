...
FUNCTION F_EXISTE_STK_PED_PROD_FINAL(cCodGrupoCia_in IN CHAR,
  ...
  --Fecha          07/10/2014
  --Author         ASOSA
  --06/07/2010  ERIOS       Se agregan los parametros para convenios
      PROCEDURE CAJ_UPD_STK_PROD_COMP(cCodGrupoCia_in 	   IN CHAR,
                                           ...
                                           cCodMotKardex_in IN CHAR,    --ASOSA - 17/10/2014
										                       ...
										                       cCodNumeraKardex_in IN CHAR,
							 			                       cUsuModProdLocal_in IN CHAR,
									cTipComp_in         IN CHAR DEFAULT NULL,
                                    cNumComp_in         IN CHAR DEFAULT NULL);
...
--          cantCompConStock number(3) := 0;
  ...
  /*****************************************************************/
    ...
    RETURN VARCHAR2
           IS
          rpta  VARCHAR2(1000) := 'S'||'Ã'||'N';
          cantCompWithoutStock number(3) :=  0;
		  cantCompOpcional INTEGER := 0;
		  CURSOR curStock IS
		  SELECT PROD.COD_PROD,PROD.DESC_PROD,V.CANTIDAD,V.IND_VTA_STOCK
           FROM
                  (SELECT pl.cod_prod,
                  ...
                  AND TT.COD_PROD_PADRE = cCodProd_in) V,
				  LGT_PROD PROD
            WHERE CANTIDAD < 0
			AND PROD.COD_PROD = V.COD_PROD
			AND PROD.COD_GRUPO_CIA = '001';
			v_vCadena VARCHAR2(1000);
			v_vCadena2 VARCHAR2(1000);
     BEGIN
			--ERIOS 19.09.2016 Se evalua los productos obligatorios para la venta
           FOR fila IN curStock
		   LOOP
			IF fila.IND_VTA_STOCK = 'M' THEN
				cantCompWithoutStock := cantCompWithoutStock+1;
				v_vCadena := v_vCadena||fila.COD_PROD||'-'||fila.DESC_PROD||CHR(10);
			ELSIF fila.IND_VTA_STOCK = 'C' THEN
				cantCompOpcional := cantCompOpcional+1;
				v_vCadena2 := v_vCadena2||fila.COD_PROD||'-'||fila.DESC_PROD||CHR(10);
			END IF;
		   END LOOP;
            ...
            IF cantCompWithoutStock > 0 THEN
                         rpta := 'N'||'Ã'||
						 '================================================'||CHR(10)||
											'No hay stock suficiente para vender el producto.'||CHR(10)||
											v_vCadena
						 ;
            ELSIF cantCompOpcional > 0 THEN
				rpta := 'S'||'Ã'||
						'================================================'||CHR(10)||
											'Faltan los siguientes producto. ¿Desea continuar?'||CHR(10)||
											v_vCadena2
						 ;
			END IF;
...
AND C.COD_PROD_PADRE = A.COD_PROD_PADRE
AND C.COD_PROD_HIJO = A.COD_PROD_HIJO;

    v_nCantAtendida VTA_PEDIDO_VTA_DET.CANT_ATENDIDA%TYPE;
    stkFis  lgt_prod_local.stk_fisico%TYPE;
    fecMod  lgt_prod_local.fec_mod_prod_local%TYPE;
    usuMod  lgt_prod_local.usu_mod_prod_local%TYPE;
	bGrabaKardex BOOLEAN;
  BEGIN
  /**
  ALTA_VTA_PROD CONSTANT CHAR(3) := '113';
  ...
  **/
    ...
    FOR productos_K IN prod_comp_Kardex
	  LOOP
		...
		--ERIOS 19.09.2016 Determina componentes a registrar kardex
		v_nCantAtendida := productos_K.CANT_ATENDIDA;
		bGrabaKardex := TRUE;
		IF productos_K.IND_VTA_STOCK <> 'M' THEN
			v_nCantAtendida := (productos_K.CANT_ATENDIDA*productos_K.VAL_FRAC_LOCAL)/productos_K.VAL_FRAC_VTA;
			IF productos_K.STK_FISICO = 0 THEN
				bGrabaKardex := FALSE;
			ELSIF productos_K.STK_FISICO < v_nCantAtendida THEN
				v_nCantAtendida := (productos_K.STK_FISICO*productos_K.VAL_FRAC_VTA)/productos_K.VAL_FRAC_LOCAL;
			ELSE
				v_nCantAtendida := productos_K.CANT_ATENDIDA;
			END IF;
		END IF;
		IF bGrabaKardex THEN
          ...
          WHERE COD_GRUPO_CIA = cCodGrupoCia_in
    			AND	   COD_LOCAL = cCodLocal_in
    			AND	   COD_PROD = productos_K.COD_PROD FOR UPDATE;
          ...
          --ACTUALIZA STK PRODUCTO
          v_nCantAtendida := (productos_K.CANT_ATENDIDA*productos_K.VAL_FRAC_LOCAL)/productos_K.VAL_FRAC_VTA;
			--ERIOS 19.09.2016 Determina la cantidad a descontar
			IF productos_K.IND_VTA_STOCK <> 'M' THEN
				IF productos_K.STK_FISICO < v_nCantAtendida THEN
					v_nCantAtendida := productos_K.STK_FISICO;
				END IF;
			END IF;
  ...
  EXCEPTION
    WHEN OTHERS THEN
         ...
         FARMA_UTILITY.envia_correo(cCodGrupoCia_in,
                                    cCodLocal_in,
                                    FARMA_GRAL.F_VAR_GET_FARMA_EMAIL('22'),
                                    'ERROR AL COBRAR PEDIDO',
                                    'ERROR',
                                    mesg_body,
                                    '');
