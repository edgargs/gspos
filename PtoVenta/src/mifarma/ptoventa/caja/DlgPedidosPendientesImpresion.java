    ...
    private void reimprimirPedido() {
        ...
        try {
            ...
            VariablesCaja.vNumSecImpresionComprobantes =
                    Integer.parseInt(((String)tblListaPendientes.getValueAt(filaActual, 7)).trim());
            ...
            VariablesCaja.vNumComp =
                    ((String)tblListaPendientes.getValueAt(filaActual, 1)).trim(); //LTAVARA 25.09.2014 OBTIENE EL NUMERO ELECTRONICO
            ...
            try {
                ...
                VariablesCaja.vIndDeliveryAutomatico =
                        FarmaUtility.getValueFieldJTable(tblListaPendientes, filaActual, COL_IND_DELIVERY);
                ...
                if ("S".equalsIgnoreCase(DBCaja.getIndPedConvenio(VariablesCaja.vNumPedVta))) {
                    ...
                    if (ConstantsVentas.TIPO_PEDIDO_INSTITUCIONAL.equals(VariablesVentas.vTipoPedido)) {
                        ...
                        VariablesConvenioBTLMF.vInstitucion =
                                ((String)tblListaPendientes.getValueAt(filaActual, 14)).trim();
                        VariablesConvenioBTLMF.vDireccion =
                                ((String)tblListaPendientes.getValueAt(filaActual, 15)).trim();
                        VariablesConvenioBTLMF.vRuc = ((String)tblListaPendientes.getValueAt(filaActual, 16)).trim();
                        VariablesConvenioBTLMF.vNumOrdeCompra =
                                ((String)tblListaPendientes.getValueAt(filaActual, 17)).trim();
                        VariablesConvenioBTLMF.vPuntoLlegada =
                                ((String)tblListaPendientes.getValueAt(filaActual, 18)).trim();
                        VariablesVentas.vMensajeBotiquinBTLVtaInstitucional =
                                ((String)tblListaPendientes.getValueAt(filaActual, 19)).trim();
                    } else {
                    ...
                    //CESAR HUANES 19/09/2014
                    //SOLO PARA REIMPRESION DE NOTA DE CREDITO
                    /* if (VariablesVentas.vTip_Comp_Ped.equalsIgnoreCase(ConstantesDocElectronico.TIPNOTACREDITO)) {
                        //if (indicElectronico.equalsIgnoreCase(ConstantesDocElectronico.INDELECTRONICO)) {
                        //KMONCADA 22.01.2015 NUEVA INDICADOR DE VALIDACION DE COMPROBANTE ELECTRONICO
                        if (ConstantesDocElectronico.IND_COMPROBANTE_ELECTRONICO.equalsIgnoreCase(indComprElectronico)) {
                            //flag=electronico.printDocumento(this,VariablesCaja.vNumPedVta, VariablesCaja.vSecComprobante,  VariablesVentas.vTip_Comp_Ped,  VariablesConvenioBTLMF.vTipClienConvenio,  VariablesCaja.vNumComp);
                            flag = electronico.printDocumento(VariablesCaja.vNumPedVta, VariablesCaja.vSecComprobante, false, false);
                            if (flag) {
                            ...
                            FarmaUtility.showMessage(this, ConstantesDocElectronico.ERRORNOTCRED, null);
                        }
                    } else {
                    ...
                    /* if (VariablesVentas.vTip_Comp_Ped.equalsIgnoreCase(ConstantesDocElectronico.TIPNOTACREDITO)) {
                        ...
                        UtilityCaja.procesoImpresionComprobante(this, tblListaPendientes, true, indComprElectronico);
                    } */
                    UtilityCaja.procesoImpresionComprobante(this, tblListaPendientes, false);
                }
                cargaListaPendientes();
