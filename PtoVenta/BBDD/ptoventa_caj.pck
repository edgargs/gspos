...
TYPE FarmaCursor IS REF CURSOR;
  ...
  COD_FORMA_PAGO_PUNTOS CHAR(5) := '00090';
  COD_FORMA_PAGO_NCR CHAR(5) := '00091';
  COD_FORMA_PAGO_REDONDEO_PTOS CHAR(5) := '00092';
...
*/
 ...
 FUNCTION CAJ_LISTA_MOVIMIENTOS_CAJA(cCodGrupoCia_in IN CHAR,
  ...
  --Fecha       Usuario		Comentario
  --06/03/2006  LMESIA     	Creación
  --24/07/2015  ASOSA     	Modificacion
  FUNCTION CAJ_AGRUPA_IMPRESION_DETALLE(cCodGrupoCia_in IN CHAR,
						   	   		 	cCodLocal_in    IN CHAR,
										cNumPedVta_in 	IN CHAR,
										nCantMaxImpr_in IN NUMBER,
                    cUsuModImpr_in  IN CHAR,
                    cTipoDoc_in in varchar2 default null
										)
  	RETURN NUMBER;
  
  --Descripcion: Agrupa el detalle del pedido para local-mayorista
  --Fecha       Usuario		Comentario
  --22/01/2016  LMESIA     	Creacion
  FUNCTION CAJ_AGRUPA_MAYORISTA_DETALLE(cCodGrupoCia_in IN CHAR,
                                           cCodLocal_in    IN CHAR,
                                        cNumPedVta_in   IN CHAR,
										cTipCompBol_in CHAR,
                                        cUsuModImpr_in  IN CHAR)
  ...
  --Fecha       Usuario		Comentario
  --10/03/2006  LMESIA     	Creacion
  --10/01/2007  LREQUE		Modificacion
  --22/01/2016  ERIOS       Se modifica el parametro cSecCompPago_in
 ...
 --Descripcion: Obtiene los datos de un movimientos de caja y de su cajero
  ...
  --Fecha       Usuario		Comentario
  --12/01/2007  LMesia      Creacion
  --16/09/2015  ERIOS       Retorna COD_COMERCIO, NOM_COMERCIO
  --23/09/2015  ERIOS       Se agrega nombre del operador  
...
/*  PROCEDURE ACTUALIZA_CUPONES_MATRIZ(cCodGrupoCia_in IN CHAR,cCodLocal_in IN CHAR,
 ...
 RETURN FarmaCursor;
  ...
  FUNCTION CAJ_OBTIENE_FORMAS_PAG_SINCONV(cCodGrupoCia_in        IN CHAR,
                                          ...
                                          cIsCobroProforma_in    IN CHAR DEFAULT 'N')
  RETURN FarmaCursor;
...
--08/01/2009    ASOLIS   Creacion
  ...
  --Fecha       Usuario		      Comentario
  --20/03/2013  Luigy Terrazos  Creacion
  --02/03/2015  Se agrega la NCR de uso
...
FUNCTION GET_MODELO_IMP_TERMICA(cCodGrupoCia_in  IN CHAR,
  ...
  */
  ...
  --Descripcion : Marca las NCR usadas
  --Fecha       Usuario		Comentario
  --15/05/2015  JCORTEZ  	Creacion  
  PROCEDURE MARCA_USO_NCR(cCodGrupoCia_in IN CHAR,cCodLocal_in IN CHAR,cNumPedVtaOrigen_in IN CHAR,cNumPedVta_in IN CHAR);

  --Descripcion : Obtener partida arancelaria
  --Fecha       Usuario		Comentario
  --20/07/2015  ASOSA  	Creacion 
  FUNCTION F_GET_PARTIDA_ARANCELARIA(cCodGrupoCia_in  IN CHAR,
                                       cCodLocal_in     IN CHAR,
                                       cCodCia_in       in char,
                                       cCodProd_in   IN CHAR)
  RETURN varchar2;
  
  --Descripcion : OBTIENE CANTIDAD DE INTENTOS PARA OBTENER NRO DE COMPROBANTE MANUAL
  --Fecha       Usuario		Comentario
  --14/09/2015  KMONCADA  Creacion 
  FUNCTION F_GET_NRO_INTENTO_COMPROBANTE
   RETURN NUMBER;

/******************************************************************************/
  --Descripcion : LISTADO DE LOGO DE TARJETAS ASIGNADOS AL LOCAL
  --Fecha       Usuario		Comentario
  --16.05.2016  KMONCADA  Creacion    
  FUNCTION F_LST_TARJETAS_LOGO(cCodGrupoCia_in   IN PBL_LOCAL.COD_GRUPO_CIA%TYPE,
                              cCodLocal_in      IN PBL_LOCAL.COD_LOCAL%TYPE)
  RETURN FARMACURSOR;
  
/************************************************************************************/
	--Descripcion: DEVUELVE SECUENCIA DE IMPRESORA SEGUN EL TIPO DE COMPROBANTE E IP
 	--Fecha       Usuario		Comentario
  --25.05.2016  KMONCADA  Creación    
  FUNCTION F_GET_SECUENCIAL_IMPRESORA(cCodGrupoCia_in IN VTA_IMPR_LOCAL.COD_GRUPO_CIA%TYPE,
                                      cCodLocal_in    IN VTA_IMPR_LOCAL.COD_LOCAL%TYPE,
                                      cTipoCP_in      IN VTA_IMPR_LOCAL.TIP_COMP%TYPE,
                                      cIpPC_in        IN VTA_IMPR_IP.IP%TYPE,
                                      cIsPedManual_in IN CHAR DEFAULT 'N')
  RETURN VARCHAR2;  
END;
/
  ...
  BEGIN
    OPEN curCaj FOR
         SELECT VTA_CAB.NUM_PED_VTA || 'Ã' || -- 0
               ...
               TO_CHAR((VTA_CAB.VAL_NETO_PED_VTA / VTA_CAB.VAL_TIP_CAMBIO_PED_VTA) + DECODE(VTA_CAB.IND_DISTR_GRATUITA,'N',DECODE(VTA_CAB.VAL_NETO_PED_VTA,0,0,0.05),0.00),'999,990.00') || 'Ã' || -- 2
               ...
               CASE
                 ...
                 ELSE
                   DECODE(VTA_CAB.TIP_COMP_PAGO,COD_TIP_COMP_BOLETA,'BOLETA',COD_TIP_COMP_FACTURA,'FACTURA',COD_TIP_COMP_TICKET,'TICKET','OTRO') END || 'Ã' || -- 5
               ...
               (CASE
                 -- dubilluz 15.09.2014
                  ...
                  ELSE to_char(VTA_CAB.PCT_BENEFICIARIO,'99999990.00') END) || 'Ã'|| -- 20
              --ERIOS 05.03.2015 Mensaje de ahorro
             (SELECT LLAVE_TAB_GRAL FROM PBL_TAB_GRAL WHERE ID_TAB_GRAL = 467)||' : S/.'||'@'|| 
...
*\
  ...
  */
  ...
  IF cIsCobroProforma_in = 'S' THEN
    IF cNumPed_in IS NOT NULL THEN
      select NVL(IND_FP_FID_EFECTIVO,'N'),
             NVL(IND_FP_FID_TARJETA,'N'),
...
----- CAMBIA DE FECHA LOS PEDIDOS ---
          ...
          -- KMONCADA 09.06.2016 [PROYECTO M] VALIDAREMOS QUE NO EXISTA PROFORMAS PENDIENTE DE ENTREGA
          IF FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in,cCod_Local_in)= 'S' THEN
            ...
            IF vCantPedInc > 0 THEN
              RAISE_APPLICATION_ERROR(-20030,'No se puede cerrar caja ya que existen pedidos pendientes o en proceso de cobro. Vuelva a intentar!!!');
            END IF;
          END IF;
  ...
  IS
    curCaj FarmaCursor;
  BEGIN
    --ERIOS 12.02.2016 Se agrega secuencial de impresora Nota Credito para local-mayorista.
    OPEN curCaj FOR
    SELECT NVL(
          ...
          NVL(
            ...
            AND TRIM(X.IP)= (SELECT TRIM(SYS_CONTEXT('USERENV','IP_ADDRESS')) FROM DUAL)),0) || 'Ã' || --SERIE TICKET
         (SELECT SEC_IMPR_LOCAL
          FROM VTA_IMPR_LOCAL
          WHERE TIP_COMP = '04') --NOTA CREDITO
  ...
  IS
    ...
    v_nContador     NUMBER;
    v_cIndicador    PBL_IGV.COD_IGV%TYPE;
    v_nCantidad     NUMBER;
...
/*    SELECT COUNT(*) CANTIDAD,
    ...
    GROUP BY VTA_DET.IND_EXONERADO_IGV;*/
      ...
      SELECT COUNT(*) CANTIDAD, X.*
      FROM (SELECT P.COD_IGV INDICADOR,
                   DECODE(NVL(VTA_DET.VAL_PCT_PERCEPCION, 0), 0, 'N', 'S') PERCEPCION
              FROM VTA_PEDIDO_VTA_DET VTA_DET, LGT_PROD P
             WHERE VTA_DET.COD_GRUPO_CIA = P.COD_GRUPO_CIA
               AND VTA_DET.COD_PROD = P.COD_PROD
               AND VTA_DET.COD_GRUPO_CIA = cCodGrupoCia_in
               AND VTA_DET.COD_LOCAL = cCodLocal_in
               AND VTA_DET.NUM_PED_VTA = cNumPedVta_in) X
     GROUP BY X.INDICADOR, X.PERCEPCION;


    CURSOR productos(p_indicador CHAR, p_percepcion CHAR) IS
      ...
      SELECT *
      FROM (SELECT VTA_DET.COD_PROD PRODUCTO,
             ...
             WHERE VTA_DET.COD_GRUPO_CIA = P.COD_GRUPO_CIA
               ...
               AND VTA_DET.COD_GRUPO_CIA = cCodGrupoCia_in
               AND VTA_DET.COD_LOCAL = cCodLocal_in
               AND VTA_DET.NUM_PED_VTA = cNumPedVta_in) K
     WHERE K.INDICADOR = p_indicador
       AND K.PERCEPCION = p_percepcion;
    vPercepcion CHAR(1);
    vSecPedVtaDet VTA_PEDIDO_VTA_DET.SEC_PED_VTA_DET%TYPE;
  BEGIN
    v_nSecGrupoImpr := 0;
    ...
    LOOP
        ...
        vPercepcion := secGrupoImpr_rec.PERCEPCION;
      v_nContador := 0;
      --IF(v_nSecGrupoImpr = 0) THEN
      v_nSecGrupoImpr := v_nSecGrupoImpr + 1;
      --END IF;
      FOR producto_rec IN productos(v_cIndicador, vPercepcion)
        LOOP
        ...
        END IF;
          ...
          vSecPedVtaDet := producto_rec.SEC_PED_VTA_DET;
        UPDATE VTA_PEDIDO_VTA_DET 
        SET FEC_MOD_PED_VTA_DET = SYSDATE, 
            USU_MOD_PED_VTA_DET = cUsuModImpr_in,
            SEC_GRUPO_IMPR = v_nSecGrupoImpr
        WHERE  COD_GRUPO_CIA  = cCodGrupoCia_in
        AND  COD_LOCAL      = cCodLocal_in
        AND  NUM_PED_VTA    = cNumPedVta_in
        AND  COD_PROD       = v_cCodProd
        AND SEC_PED_VTA_DET = vSecPedVtaDet;
  ...
  IS
    v_nSecGrupoImpr NUMBER;
  v_nContador      NUMBER;
  nCantMaxImpr_in NUMBER;
  
  CURSOR agrupaFarma IS
  WITH
COMP AS (SELECT '01' TIP,TO_NUMBER(LLAVE_TAB_GRAL) CANT_MAX FROM PBL_TAB_GRAL WHERE ID_TAB_GRAL = 230
         UNION
         SELECT '02' TIP,TO_NUMBER(LLAVE_TAB_GRAL) CANT_MAX FROM PBL_TAB_GRAL WHERE ID_TAB_GRAL = 232
         UNION
         SELECT '05' TIP,TO_NUMBER(LLAVE_TAB_GRAL) CANT_MAX FROM PBL_TAB_GRAL WHERE ID_TAB_GRAL = 269
         )
SELECT IND_PROD_FARMA,TIP_COMP,
       (SELECT CANT_MAX FROM COMP WHERE TIP = TIP_COMP) CANT_MAX
FROM (
	  SELECT DISTINCT PROD.IND_PROD_FARMA,
			 CASE WHEN PROD.IND_PROD_FARMA = 'S' THEN cTipCompBol_in
				  ELSE '02' END TIP_COMP
		FROM VTA_PEDIDO_VTA_DET DET JOIN LGT_PROD PROD ON (DET.COD_PROD = PROD.COD_PROD)
		WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
		AND DET.COD_LOCAL = cCodLocal_in
		AND DET.NUM_PED_VTA = cNumPedVta_in
	);
    CURSOR indicadores(cIndFarma_in IN CHAR) IS
           SELECT COUNT(*) CANTIDAD,
             VTA_DET.IND_EXONERADO_IGV INDICADOR
        FROM   VTA_PEDIDO_VTA_DET VTA_DET JOIN LGT_PROD PROD ON (VTA_DET.COD_PROD = PROD.COD_PROD
																AND PROD.IND_PROD_FARMA = cIndFarma_in)
        WHERE  VTA_DET.COD_GRUPO_CIA = cCodGrupoCia_in
        AND     VTA_DET.COD_LOCAL = cCodLocal_in
        AND     VTA_DET.NUM_PED_VTA = cNumPedVta_in
        GROUP BY VTA_DET.IND_EXONERADO_IGV;
		
  CURSOR productos(cIndFarma_in IN CHAR,p_indicador CHAR) IS
	...
	LINEAS_BOLETA_SIN_CONVENIO NUMBER := 230;
    LINEAS_FACTURA_SIN_CONVENIO NUMBER := 232;
    LINEAS_TICKET NUMBER := 269;
  BEGIN
    v_nSecGrupoImpr := 0;
	...
	FOR indFarma IN agrupaFarma
	LOOP
	  nCantMaxImpr_in := indFarma.CANT_MAX;
    FOR secGrupoImpr_rec IN indicadores(indFarma.IND_PROD_FARMA)
    LOOP
        
      v_nContador := 0;      
      v_nSecGrupoImpr := v_nSecGrupoImpr + 1;
      
      FOR producto_rec IN productos(indFarma.IND_PROD_FARMA, secGrupoImpr_rec.INDICADOR)
        LOOP
        ...
        INSERT INTO VTA_PEDIDO_VTA_DET_IMPR(COD_GRUPO_CIA,COD_LOCAL,NUM_PED_VTA,SEC_PED_VTA_DET,SEC_GRUPO_IMPR,TIP_COMP_PAGO)
               VALUES(cCodGrupoCia_in,cCodLocal_in,cNumPedVta_in,producto_rec.SEC_PED_VTA_DET,v_nSecGrupoImpr,indFarma.TIP_COMP);
	...
	UPDATE PTOVENTA.VTA_PEDIDO_VTA_DET DET
	SET FEC_MOD_PED_VTA_DET = SYSDATE, USU_MOD_PED_VTA_DET = cUsuModImpr_in,
			SEC_GRUPO_IMPR = (SELECT SEC_GRUPO_IMPR FROM VTA_PEDIDO_VTA_DET_IMPR TMP 
								  WHERE TMP.COD_GRUPO_CIA = DET.COD_GRUPO_CIA
									AND TMP.COD_LOCAL = DET.COD_LOCAL
									AND TMP.NUM_PED_VTA = DET.NUM_PED_VTA
									AND TMP.SEC_PED_VTA_DET = DET.SEC_PED_VTA_DET)
	WHERE  COD_GRUPO_CIA  = cCodGrupoCia_in
            AND  COD_LOCAL      = cCodLocal_in
            AND  NUM_PED_VTA    = cNumPedVta_in;
...
----------------------------------------------------------------------------------------------------------------------*/
 FUNCTION CAJ_COBRA_PEDIDO(cCodGrupoCia_in        IN CHAR,
  ...
  IS
	...
	v_vDniCli         VTA_PEDIDO_VTA_CAB.DNI_CLI%TYPE;
	...
	v_cFlgDigemid CHAR(1);

    CURSOR totalesComprobante(TipComp_in IN CHAR, operaTemp_in IN INTEGER) IS
	       SELECT VTA_DET.SEC_GRUPO_IMPR "SECUENCIA",
                 ...
                 TO_CHAR(SUM(VTA_DET.VAL_PREC_TOTAL / (1 + (I.PORC_IGV/*VTA_DET.VAL_IGV*//100))),'999,990.00') "VALOR_AFECTO",
                 TO_CHAR(SUM(VTA_DET.VAL_PREC_TOTAL - (VTA_DET.VAL_PREC_TOTAL / (1 + (I.PORC_IGV/*VTA_DET.VAL_IGV*//100)))),'999,990.00') "VALOR_IGV",
                 TO_CHAR(I.PORC_IGV/*VTA_DET.VAL_IGV*/,'990.00') "PORC_IGV",
				 CASE WHEN operaTemp_in>0 THEN (SELECT DISTINCT TIP_COMP_PAGO FROM VTA_PEDIDO_VTA_DET_IMPR TMP 
				                 WHERE TMP.COD_GRUPO_CIA = cCodGrupoCia_in
								       AND TMP.COD_LOCAL = cCodLocal_in
									   AND TMP.NUM_PED_VTA = cNumPedVta_in
									   AND TMP.SEC_GRUPO_IMPR = VTA_DET.SEC_GRUPO_IMPR)
				 ELSE TipComp_in END "TIP_COMP",
                 TO_CHAR(SUM(NVL(VTA_DET.VAL_MONTO_PERCEPCION,0)),'999,990.00') "VALOR_PERCEPCION",
                 TO_CHAR(MAX(NVL(VTA_DET.VAL_PCT_PERCEPCION,0)),'999,990.00') VAL_PCT_PERCEPCION
      FROM   VTA_PEDIDO_VTA_DET VTA_DET,
             LGT_PROD P, -- KMONCADA 19.04.2016 EVALUA EL IGV EN BASE AL MAESTRO DE PRODUCTOS
             PBL_IGV I
      WHERE  VTA_DET.COD_GRUPO_CIA = P.COD_GRUPO_CIA
      AND VTA_DET.COD_PROD = P.COD_PROD
      AND I.COD_IGV = P.COD_IGV
      AND VTA_DET.COD_GRUPO_CIA = cCodGrupoCia_in
      AND VTA_DET.COD_LOCAL = cCodLocal_in
      AND VTA_DET.NUM_PED_VTA = cNumPedVta_in
      AND VTA_DET.SEC_GRUPO_IMPR <> 0
      GROUP BY SEC_GRUPO_IMPR, I.PORC_IGV--VAL_IGV
    ...
    -- KMONCADA 16.09.2015 CONTROL DE CAMPAÑAS LIMITADAS DE USO
    CURSOR curCampAplicadas IS
      SELECT A.VAL COD_CAMP_CUPON
      FROM (SELECT TRIM(EXTRACTVALUE(xt.column_value, 'e')) VAL
              FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' ||
                                                     REPLACE((SELECT LLAVE_TAB_GRAL
                                                               FROM PBL_TAB_GRAL T
                                                              WHERE T.ID_TAB_GRAL = 556),
                                                             '@',
                                                             '</e><e>') ||
                                                     '</e></coll>'),
                                             '/coll/e'))) xt
     ...
     WHERE A.VAL IN (SELECT DISTINCT TRIM(D.COD_CAMP_CUPON) COD_CAMP_CUPON
    ...
    filaCurCampAplicadas curCampAplicadas%ROWTYPE;
    vDniClienteFid FID_TARJETA_PEDIDO.DNI_CLI%TYPE;
    operaTemp INTEGER;
    ...
    vAplicoRedondeoPercep CHAR(1) := 'N';
    vNumPedVtaProforma VTA_PEDIDO_VTA_CAB.NUM_PED_VTA%TYPE;
  BEGIN
        ...
        */
        ...
        SELECT VTA_CAB.VAL_REDONDEO_PED_VTA,
			   ...
			   VTA_CAB.DNI_CLI,
               NVL(VTA_CAB.VAL_PERCEPCION_PED_VTA,0),
               NVL(VTA_CAB.VAL_REDONDEO_PERCEPCION,0)
        INTO v_nValRedondeo,
             v_cCodCliLocal,
             v_cNomCliPedVta,
             v_cRucCliPedVta,
		...
		--ERIOS 22.01.2016 Para local-mayorista, factura, no cliente-digemid
		--                 Se divide los productos farma en boletas.
		operaTemp := 0;
    -- KMONCADA 08.06.2016 [PROYECTO M] NO SE REALIZARA DIVISION DE COMPROBANTES
		/*IF FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in, cCodLocal_in) ='S' AND TipComp = COD_TIP_COMP_FACTURA THEN
		  
		  IF v_cRucCliPedVta IS NULL THEN
		     RAISE_APPLICATION_ERROR(-20021,'No existe el codigo de cliente. ¡Verifique!');
		  END IF;
		  
		  SELECT FLG_DIGEMID
		    INTO v_cFlgDigemid
			FROM PBL_CLIENTE 
			WHERE DNI_CLI = v_cRucCliPedVta;
		  IF v_cFlgDigemid = 'N' THEN
		    operaTemp := CAJ_AGRUPA_MAYORISTA_DETALLE(cCodGrupoCia_in,
                                           cCodLocal_in,
                                        cNumPedVta_in,
										TipCompAux,
                                        cUsuCreaCompPago_in);
        ...
        FOR totalesComprobante_rec IN totalesComprobante(TipComp,operaTemp)
        LOOP
            v_nContador := v_nContador + 1;
            ...
            IF vAplicoRedondeoPercep = 'N' THEN
              IF TRIM(totalesComprobante_rec.VALOR_PERCEPCION) != '0.00' THEN
                --v_nValRedondeo := v_nValRedondeo + vRedondeoPercepcion;
                vAplicoRedondeoPercep := 'S';
              ELSE
                vAplicoRedondeoPercep := 'N';
              END IF;
            ELSE
              vAplicoRedondeoPercep := 'N';
            END IF;
            ...
            --ERIOS 22.01.2016 Para retail, el tipo de comprobante es el mismo de la cabecera.
            INSERT INTO VTA_COMP_PAGO(COD_GRUPO_CIA,
                                      COD_LOCAL,
                                      NUM_PED_VTA,
                                      SEC_COMP_PAGO,
            ...
            VALUES(cCodGrupoCia_in,
                   ...
                   cNumPedVta_in,
                   v_nSecCompPago,
                   totalesComprobante_rec.TIP_COMP, --TIPO_COMPROBANTE_99,
                   v_nSecCompPago,
                   cSecMovCaja_in,
                   ...
                   -- KMONCADA 20.04.2016 PERCEPCION: SE AGREGA EL MONTO DE PERCEPCIONAL TOTAL DEL CP
                   (TO_NUMBER(totalesComprobante_rec.VALOR_NETO,'999,990.00') + 
                    TO_NUMBER(totalesComprobante_rec.VALOR_PERCEPCION,'990.00')),
                   ...
                   TO_NUMBER(totalesComprobante_rec.VALOR_PERCEPCION,'990.00'),
                   DECODE(vAplicoRedondeoPercep,'S',vRedondeoPercepcion,0),
                   TO_NUMBER(totalesComprobante_rec.VAL_PCT_PERCEPCION,'990.00')
                   );
       ...
       close curComprobantes;
        ...
        pkg_sol_stock.sp_aprueba_sol(cCodGrupoCia_in,
                                     ...
                                     cNumPedVta_in);
        --- dubilluz
        -- KMONCADA 17.02.2016 SOLO RETORNARA DEL TEMPORAL PARA EL CASO DE LOCALES M
		    IF FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in,cCodLocal_in) = 'S' THEN
          SELECT A.NUM_PED_VTA
          INTO vNumPedVtaProforma
          FROM TMP_VTA_PEDIDO_VTA_CAB A
          WHERE A.COD_GRUPO_CIA = cCodGrupoCia_in
          AND A.COD_LOCAL = cCodLocal_in
          AND A.NUM_PED_VTA_ORIGEN = cNumPedVta_in;
		      --ERIOS 12.01.2016 Retorna el stock para proformas de local-mayorista
		      PTOVENTA_PROFORMA.P_RETORNA_STOCK_TEMPORAL(cCodGrupoCia_in,cCodLocal_in,vNumPedVtaProforma,cUsuCreaCompPago_in);
		    END IF;
    ...
    --INI ASOSA - 07/10/2014 - PANHD
          ...
          --FIN ASOSA - 07/10/2014 - PANHD
            ...
            */
            ...
            ---------------------------------------------------
            -- KMONCADA 08.06.2016 [PROYECTO M] ACTUALIZA A COBRADO LA PROFORMA
            IF(v_cIndDelivAutomatico = INDICADOR_SI OR
               FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in, cCodLocal_in) = 'S') THEN
            ...
            IF v_Resultado='EXITO' THEN
                -- dubilluz 13.04.2015
                IF PTOVENTA_MDIRECTA.GET_VAL_GEN_STRING(COD_IND_EMI_CUPON) = 'S' THEN                   
                ...
                ELSE
                 -- dubilluz 13.04.2015
                    FARMA_CUPON.nvo_algoritmo_cupon(cCodGrupoCia_in,cCodLocal_in,cNumPedVta_in);
               END IF;
                ...
                select NVL(sum(NVL(D.AHORRO_CAMP,d.ahorro)),0)
                into   vAhorroPedido
                ...
                if vAhorroPedido > 0 then
                    ...
                    select c.cod_grupo_cia,
                           c.cod_local,
                           c.num_ped_vta,
                           sum(NVL(D.AHORRO_CAMP,d.ahorro)),
                           t.dni_cli,
                           c.fec_ped_vta
                    from   vta_pedido_vta_det d,
      ...
      --  dubilluz 17.07.2015
          update VTA_PEDIDO_VTA_CAB c
           ...
           where c.cod_grupo_cia = cCodGrupoCia_in
            ...
            SELECT COUNT(1)
            INTO   nDifCabDetMonedero
            FROM   (
            select c.num_ped_vta,c.pt_acumulado ACUM_CAB,
                   sum(d.ctd_puntos_acum)  ACUM_DET
            from   vta_pedido_vta_cab c,
            ...
            AND   C.COD_LOCAL = D.COD_LOCAL 
            AND   C.NUM_PED_VTA = D.NUM_PED_VTA
            GROUP BY c.num_ped_vta,c.pt_acumulado
                  ) T
            WHERE  ABS(ACUM_CAB-ACUM_DET)>0; 
            
            
            -- graba historico de unidades con descuento 
            ...
            select cod_grupo_cia,cod_local,num_ped_vta,num_doc_id,DIA_VTA,
                   cod_prod,sum(cant_atendida),val_frac,usu_crea_ped_vta_cab
            from  (
                  select c.cod_grupo_cia,c.cod_local,c.num_ped_vta,
                         (
                         select t.dni_cli
                         from  fid_tarjeta_pedido t
                         where t.cod_grupo_cia = c.cod_grupo_cia
                         and   t.cod_local = c.cod_local
                         and   t.num_pedido = c.num_ped_vta
                         ) num_doc_id,
                         trunc(c.fec_ped_vta) as "DIA_VTA",
                         d.cod_prod,
                         d.cant_atendida,
                         d.val_frac,
            ...
            GROUP BY cod_grupo_cia,cod_local,num_ped_vta,num_doc_id,DIA_VTA,
                   cod_prod,val_frac,usu_crea_ped_vta_cab;
            
            -- si hay diferencia error
            if nDifCabDetMonedero >0 then
               v_Resultado := 'ERROR';
               RAISE_APPLICATION_ERROR(-20050,
               'NO SE PUEDE COBRAR.Porque no cuadra los campos de monedero Cabecera y Detalle');
            end if;
                        
      --  dubilluz 17.07.2015      

      -- KMONCADA 16.09.2015 REGISTRO DE CAMPAÑAS LIMITADAS DE USO
      BEGIN
        ...
        IF NVL(LENGTH(TRIM(vDniClienteFid)),0)>0 THEN
          OPEN curCampAplicadas;
          LOOP
            FETCH curCampAplicadas INTO filaCurCampAplicadas;
            EXIT WHEN curCampAplicadas%NOTFOUND;
              PTOVENTA_CAJ.CAJ_REG_CAMP_LIM_CLIENTE(cCodGrupoCia_in => cCodGrupoCia_in,
                                                    cCodLocal_in => cCodLocal_in,
                                                    vCodCampCupon_in =>  filaCurCampAplicadas.Cod_Camp_Cupon,
                                                    vDniCliente_in => vDniClienteFid,
                                                    vIdUsuario_in => cUsuCreaCompPago_in);
          END LOOP;
          CLOSE curCampAplicadas;
        END IF;
      EXCEPTION 
        WHEN OTHERS THEN
          NULL;
...
/* ********************************************************************************************** */
  ...
  BEGIN
    ...
    IF indConvenio='S' and vTipoDocumento != COD_TIP_COMP_NOTA_CRED THEN
      ...
      ;
    ELSE
	  --ERIOS 22.01.2016 Se retorna el tipo de documento
      OPEN curCaj FOR        
      SELECT ((select count(*)
                FROM   VTA_COMP_PAGO COMP_PAGO
                WHERE COMP_PAGO.COD_GRUPO_CIA = cCodGrupoCia_in
      ...
      AND     VTA_DET.NUM_PED_VTA = cNumPedVta_in
      --AND     VTA_DET.SEC_GRUPO_IMPR <> 0
      GROUP BY VTA_DET.SEC_GRUPO_IMPR, VTA_DET.SEC_COMP_PAGO, P.TIP_COMP_PAGO
      ORDER BY VTA_DET.SEC_GRUPO_IMPR;
  ...
  BEGIN
    ...
    IF v_cTipVta = '03'  THEN
      OPEN curCaj FOR
      SELECT ABS(VTA_DET.CANT_ATENDIDA)||DECODE(VTA_DET.VAL_FRAC,1,'','/'||VTA_DET.VAL_FRAC)|| 'Ã' ||
                     ...
                     VTA_DET.UNID_VTA                                      || 'Ã' ||
                     LAB.NOM_LAB                                           || 'Ã' ||
                     TO_CHAR((ABS(VTA_DET.Val_Prec_Total + nvl(vta_det.ahorro,0)))/(ABS(vta_det.cant_atendida) * prod_local.VAL_FRAC_LOCAL / vta_det.val_frac),'999,990.000')           || 'Ã' ||

                     TO_CHAR(ABS(VTA_DET.VAL_PREC_TOTAL),'999,990.00')          || 'Ã' ||
                     VTA_DET.COD_PROD                                      || 'Ã' ||
                     ...
                     TO_CHAR(VTA_DET.AHORRO,'999,990.000')  || 'Ã' ||
                      ...
                      -- precio nuevo de institucional
                       ...
                       case
                        ...
                        then
                             TO_CHAR(
                                     ABS(VTA_DET.Val_Prec_Total + nvl(vta_det.ahorro,0))/
                                     (ABS(vta_det.cant_atendida) * prod_local.VAL_FRAC_LOCAL / vta_det.val_frac),
                                     '999,990.000')
                        else
                             case
                              when  (
                       ...
                       case
                        when pp.val_prec_vta is null
                        then
                             TO_CHAR(ABS(VTA_DET.VAL_PREC_TOTAL),'999,990.00')
                        else
                             case
                              when  (
                                     ABS(VTA_DET.Val_Prec_Total + nvl(vta_det.ahorro,0))/
                              ...
                              then
                                    ...
                                    TO_CHAR(
                                     (pp.val_prec_vta/vta_det.val_frac)*ABS(vta_det.cant_atendida),
                                     '999,990.00')
                             end
                       end
    ...
    ELSIF v_cTipVta = '02' AND v_cIndConvEnteros = INDICADOR_SI THEN
      OPEN curCaj FOR
        SELECT DECODE(MOD(ABS(VTA_DET.CANT_ATENDIDA),VTA_DET.VAL_FRAC),0,(ABS(VTA_DET.CANT_ATENDIDA)/VTA_DET.VAL_FRAC)||'',
                                                          ABS(VTA_DET.CANT_ATENDIDA)||DECODE(VTA_DET.VAL_FRAC,1,'','/'||VTA_DET.VAL_FRAC))|| 'Ã' ||
             CASE  WHEN VTA_DET.VAL_PREC_TOTAL=0
             THEN  SUBSTR(v_promocion,1,9) ||' '|| PROD.DESC_PROD
             ELSE  PROD.DESC_PROD
             END   || 'Ã' ||
               DECODE(MOD(ABS(VTA_DET.CANT_ATENDIDA),VTA_DET.VAL_FRAC),0,PROD.DESC_UNID_PRESENT,
                                                                  VTA_DET.UNID_VTA                                      )|| 'Ã' ||
               LAB.NOM_LAB                                           || 'Ã' ||
               DECODE(MOD(ABS(VTA_DET.CANT_ATENDIDA),VTA_DET.VAL_FRAC),0,
                          TO_CHAR(ABS(VTA_DET.VAL_PREC_TOTAL+ nvl(vta_det.ahorro,0) )/(ABS(VTA_DET.CANT_ATENDIDA)/VTA_DET.VAL_FRAC),'999,990.000'),
               ...
               NVL(VTA_DET.VAL_COD_APROBACION,' ')                    || 'Ã' ||
               NVL(TO_CHAR(VTA_DET.FEC_VENCIMIENTO_LOTE,'dd/MM/yyyy'),' ')                  || 'Ã' ||
               DECODE(MOD(ABS(VTA_DET.CANT_ATENDIDA),VTA_DET.VAL_FRAC),0,PROD.DESC_PROD || ' / ' || PROD.DESC_UNID_PRESENT,
                                                                 PROD.DESC_PROD || ' / ' || VTA_DET.UNID_VTA)  || 'Ã' ||
                TO_CHAR(VTA_DET.AHORRO,'999,990.000')
              || 'Ã' ||  TO_CHAR(PROD_LOCAL.VAL_PREC_VTA*ABS(VTA_DET.CANT_ATENDIDA),'999,990.00')
        FROM   VTA_PEDIDO_VTA_DET VTA_DET,
               LGT_PROD_LOCAL PROD_LOCAL,
               LGT_PROD PROD,
               LGT_LAB LAB,
    ...
    ELSE --IMPRESION NORMAL
      OPEN curCaj FOR
        SELECT ABS(VTA_DET.CANT_ATENDIDA)||DECODE(VTA_DET.VAL_FRAC,1,'','/'||VTA_DET.VAL_FRAC)|| 'Ã' ||
               ...
               PROD.DESC_PROD || ' / ' || VTA_DET.UNID_VTA || 'Ã' ||
               TO_CHAR(VTA_DET.AHORRO,'999,990.000') || 'Ã' ||  
			   TO_CHAR(PROD_LOCAL.VAL_PREC_VTA*ABS(VTA_DET.CANT_ATENDIDA),'999,990.00')
        FROM   VTA_PEDIDO_VTA_DET VTA_DET,
               LGT_PROD_LOCAL PROD_LOCAL,
               LGT_PROD PROD,
               LGT_LAB LAB,
  ...
  BEGIN
    OPEN curCaj FOR
    SELECT TO_CHAR(ABS(COMP_PAGO.VAL_BRUTO_COMP_PAGO),'999,990.00') || 'Ã' ||
         TO_CHAR(ABS(COMP_PAGO.VAL_NETO_COMP_PAGO + COMP_PAGO.VAL_REDONDEO_COMP_PAGO),'999,990.00') || 'Ã' ||
         TO_CHAR(ABS(COMP_PAGO.VAL_DCTO_COMP_PAGO),'999,990.00') || 'Ã' ||
         TO_CHAR(ABS(COMP_PAGO.VAL_IGV_COMP_PAGO),'999,990.00') || 'Ã' ||
         TO_CHAR(ABS(COMP_PAGO.VAL_AFECTO_COMP_PAGO),'999,990.00') || 'Ã' ||
         TO_CHAR(ABS(COMP_PAGO.VAL_REDONDEO_COMP_PAGO),'999,990.00') || 'Ã' ||
         ...
         TO_CHAR(ABS(COMP_PAGO.VAL_DCTO_COMP),'999,990.00')
    FROM   VTA_COMP_PAGO COMP_PAGO
    WHERE  COMP_PAGO.COD_GRUPO_CIA = cCodGrupoCia_in
    AND     COMP_PAGO.COD_LOCAL     = cCodLocal_in
...
END IF;
  ...
  IS
    ...
    vContador INTEGER;
    vHayDif char(1) := 'S';
    vNumPedido CHAR(10) := '0';
    CURSOR cpPendientes is
      SELECT CP.*
        FROM VTA_COMP_PAGO CP
       WHERE (CP.COD_GRUPO_CIA, CP.COD_LOCAL, CP.NUM_PED_VTA) IN
             (SELECT W.COD_GRUPO_CIA, W.COD_LOCAL, W.NUM_PED_VTA
                FROM (SELECT CAB.COD_GRUPO_CIA,
                             CAB.COD_LOCAL,
                             CAB.NUM_PED_VTA,
                             (SELECT COUNT(1)
                                FROM VTA_COMP_PAGO
                               WHERE COD_GRUPO_CIA = CAB.COD_GRUPO_CIA
                                 AND COD_LOCAL = CAB.COD_LOCAL
                                 AND NUM_PED_VTA = CAB.NUM_PED_VTA) TOTAL,
                             (SELECT COUNT(1)
                                FROM VTA_COMP_PAGO
                               WHERE COD_GRUPO_CIA = CAB.COD_GRUPO_CIA
                                 AND COD_LOCAL = CAB.COD_LOCAL
                                 AND NUM_PED_VTA = CAB.NUM_PED_VTA
                                 AND FECH_IMP_COBRO IS NOT NULL) IMPR
                        FROM VTA_PEDIDO_VTA_CAB CAB
                       WHERE CAB.COD_GRUPO_CIA = cCodGrupoCia_in
                         AND CAB.COD_LOCAL = cCodLocal_in
                         AND CAB.EST_PED_VTA = 'S') W
               WHERE W.TOTAL = W.IMPR);
    fila     cpPendientes%ROWTYPE;
  BEGIN
    -- KMONCADA 04.05.2015 EN CASO DE EXISTIR PEDIDOS EN ESTADO S Y TODOS LOS CP FIGUREN IMPRESOS, SE 
    -- MARCARA EL PRIMERO PARA REIMPRESION
    OPEN cpPendientes;
    LOOP
      FETCH cpPendientes
      INTO fila;
      EXIT WHEN cpPendientes%NOTFOUND;
      IF vNumPedido = fila.NUM_PED_VTA THEN 
        vHayDif := 'N';
      ELSE vHayDif := 'S'; END IF;

      IF vHayDif = 'S' THEN 
        UPDATE VTA_COMP_PAGO 
        SET FECH_IMP_COBRO = NULL 
        WHERE COD_GRUPO_CIA = fila.COD_GRUPO_CIA 
        AND COD_LOCAL = fila.COD_LOCAL 
        AND NUM_PED_VTA = fila.NUM_PED_VTA 
        AND SEC_COMP_PAGO = fila.SEC_COMP_PAGO; 
      END IF; 
      vNumPedido := fila.NUM_PED_VTA;
    END LOOP;
    CLOSE cpPendientes;
...
WHERE MCFP.COD_GRUPO_CIA  = cCodGrupoCia_in   AND
 ...
 END;
  ...
  BEGIN
    /*UPDATE VTA_PEDIDO_VTA_CAB 
    SET USU_MOD_PED_VTA_CAB = cUsuModPedVtaCab_in, 
        FEC_MOD_PED_VTA_CAB = SYSDATE,
        COD_CLI_LOCAL   = '0000000',--cCodCliLocal_in,
           ...
           (CAB.RUC_CLI_PED_VTA, 
            ...
            CAB.DIR_CLI_PED_VTA) =
                                   (SELECT P.DNI_CLI, 
                                           ...
                                           P.DIR_CLI
                                      FROM PBL_CLIENTE P
                                     WHERE P.DNI_CLI = cCodCliLocal_in
                                       AND P.COD_TIP_DOCUMENTO = PKG_CONSTANTES.TIPO_DOC_RUC
                                       AND P.IND_ESTADO = 'A')
     WHERE CAB.COD_GRUPO_CIA = cCodGrupoCia_in
       AND CAB.COD_LOCAL = cCodLocal_in
       AND CAB.NUM_PED_VTA = cNumPedVta_in;
  ...
  /* ********************************************************************************************** */
  --ERIOS 25.01.2016 Se actualiza el kardex por lote.
  PROCEDURE CAJ_ACT_KARDEX_COMPROBANTE(cCodGrupoCia_in     IN CHAR,
                              cCodLocal_in        IN CHAR,
                        cNumPedVta_in       IN CHAR,
                     cSecCompPago_in     IN CHAR,
  ...
  IS
  CURSOR productos IS
           SELECT VTA_DET.COD_PROD, NVL(VTA_DET.LOTE,'ES_VACIO') LOTE
        FROM   VTA_PEDIDO_VTA_DET VTA_DET
        WHERE  VTA_DET.COD_GRUPO_CIA = cCodGrupoCia_in
        AND     VTA_DET.COD_LOCAL = cCodLocal_in
        AND     VTA_DET.NUM_PED_VTA = cNumPedVta_in
  ...
  BEGIN
    ...
    LOOP
        UPDATE LGT_KARDEX SET FEC_MOD_KARDEX = SYSDATE, USU_MOD_KARDEX = cUsuModKardex_in,
         ...
         WHERE COD_GRUPO_CIA = cCodGrupoCia_in
         AND   COD_LOCAL = cCodLocal_in
		 AND   TIP_DOC_KARDEX = '01'
         AND   NUM_TIP_DOC = cNumPedVta_in
         AND   COD_PROD = productos_rec.COD_PROD
		 AND (('ES_VACIO' = productos_rec.LOTE AND  NRO_LOTE IS NULL)
              OR NRO_LOTE = productos_rec.LOTE);
    END LOOP;
  END;
...
FUNCTION  CAJ_VERIFICA_TOTAL_PED_FOR_PAG(cCodGrupoCia_in IN CHAR,
  ...
  --16/09/2015  ERIOS       Retorna COD_COMERCIO, NOM_COMERCIO
  --23/09/2015  ERIOS       Se agrega nombre del operador
  --25/05/2016  ERIOS       Se agrega proveedor de recargas. Por defecto es SIX.
  FUNCTION CAJ_OBT_INFO_PROD_VIRTUAL(cCodGrupoCia_in IN CHAR,
                                       cCodLocal_in    IN CHAR,
                                     cNumPedVta_in   IN CHAR)
    RETURN FarmaCursor
	...
	AND COD_LOCAL = cCodLocal_in;
       ...
       OPEN curCaj FOR
            SELECT DET.COD_PROD || 'Ã' ||
                   ...
                   VIR.COD_TIPO_PROD || 'Ã' || --ASOSA - 14/07/2014
				   COM.COD_COMERCIO || 'Ã' ||
                   COM.NOM_COMERCIO || 'Ã' ||
				   (SELECT NOM_LAB
					FROM PTOVENTA.LGT_PROD NATURAL JOIN PTOVENTA.LGT_LAB
					WHERE COD_GRUPO_CIA = cCodGrupoCia_in
					AND COD_PROD = VIR.COD_PROD) || 'Ã' ||
                   NVL(IND_GESTOR,'S') || 'Ã' ||
                   NVL(COD_PROVEEDOR,'SIX')
            FROM   LGT_PROD_VIRTUAL VIR,
                   VTA_PEDIDO_VTA_DET DET,
                   (SELECT '6'||COD_CIA COD_COMERCIO,
                    'Boticas '||NOM_CIA NOM_COMERCIO
                    FROM PTOVENTA.PBL_CIA
                    WHERE COD_CIA = v_cCodCia) COM,
				   (SELECT SUBSTR(DESC_CORTA,5) IND_GESTOR,
                    SUBSTR(DESC_LARGA,6) COD_PROVEEDOR,
                    LLAVE_TAB_GRAL TIP_RCD, COD_CIA
                    FROM PTOVENTA.PBL_TAB_GRAL
                    WHERE COD_TAB_GRAL = 'RECARGA_TIPO_EMPRE'
                    AND COD_CIA = v_cCodCia
                    AND EST_TAB_GRAL = 'A') PROV	
            WHERE  DET.COD_GRUPO_CIA = cCodGrupoCia_in
            AND    DET.COD_LOCAL = cCodLocal_in
            AND    DET.NUM_PED_VTA = cNumPedVta_in
            AND    DET.COD_GRUPO_CIA = VIR.COD_GRUPO_CIA
            AND    DET.COD_PROD = VIR.COD_PROD
			AND PROV.TIP_RCD(+) = VIR.TIP_RCD ;
       RETURN curCaj;
  END;
...
END CAJ_F_VAR_LINEA_DOC;
 ...
 FUNCTION CAJ_F_VAR_IND_PED_CONVENIO(pCodGrupoCia_in       IN CHAR,
  ...
  BEGIN
        ...
        SELECT C.VAL_NETO_PED_VTA + NVL(C.VAL_PERCEPCION_PED_VTA,0)  , 
               C.VAL_REDONDEO_PED_VTA + NVL(C.VAL_REDONDEO_PERCEPCION,0)
        ...
        SELECT SUM(D.VAL_PREC_TOTAL) + SUM(NVL(D.VAL_MONTO_PERCEPCION,0))
        into v_nSumaValorDet
        FROM VTA_PEDIDO_VTA_DET D
        WHERE D.COD_GRUPO_CIA = cCodGrupoCia_in
        AND   D.COD_LOCAL     = cCodLocal_in
        AND   D.NUM_PED_VTA   = cNumPedVta_in;
...
*/
      ...
      IF cTipMov_in = 'C' THEN--TIPO DE CAJA CIERRE
           ...
           SET (MM.CANT_BOL_EMI, MM.MON_BOL_EMI) =
                    (
                     /*SELECT COUNT(*) CANTIDAD,
                      ...
                      WHERE TIP_COMP_PAGO='01'
                        -- AND EXISTS (SELECT 1
                        -- KMONCADA 14.09.2015 VALIDA QUE PEDIDO ESTE COBRADO
                        AND CP.NUM_PED_VTA IN (SELECT C.NUM_PED_VTA
                             FROM VTA_PEDIDO_VTA_CAB C
                             WHERE C.COD_GRUPO_CIA = cCodGrupoCia_in
                               AND C.COD_LOCAL = cCod_Local_in
                               AND C.FEC_PED_VTA BETWEEN TO_DATE(v_vFecha || ' 00:00:00','DD/MM/YYYY HH24:MI:SS') AND
                    ...
                    (
                     /*SELECT COUNT(*) CANTIDAD,
                       ...
                       WHERE TIP_COMP_PAGO='02'
                         /*AND EXISTS (SELECT 1*/
                       -- KMONCADA 14.09.2015 VALIDA QUE PEDIDO ESTE COBRADO
                        AND NUM_PED_VTA IN (SELECT C.NUM_PED_VTA
                              FROM VTA_PEDIDO_VTA_CAB C
                              WHERE C.COD_GRUPO_CIA = cCodGrupoCia_in
                                AND C.COD_LOCAL = cCod_Local_in
                                AND C.FEC_PED_VTA BETWEEN TO_DATE(v_vFecha || ' 00:00:00','DD/MM/YYYY HH24:MI:SS') AND
               ...
               (MM.MON_BOL_ANU) =
                    (
                     /*SELECT NVL(SUM(C.VAL_NETO_PED_VTA),0)*-1 TOTAL_NETO_REDONDEO
                       ...
                       SELECT NVL(SUM(CP.VAL_NETO_COMP_PAGO- NVL(CP.VAL_PERCEP_COMP_PAGO,0)+CP.VAL_REDONDEO_COMP_PAGO),0) TOTAL_NETO_REDONDEO
                       FROM VTA_COMP_PAGO CP
                       WHERE TIP_COMP_PAGO='01'
                         /*AND EXISTS (SELECT 1*/
                        -- KMONCADA 14.09.2015 VALIDA QUE PEDIDO ESTE COBRADO                            
                         AND CP.NUM_PED_VTA IN (SELECT C.NUM_PED_VTA_ORIGEN
                              FROM VTA_PEDIDO_VTA_CAB C
                              WHERE C.COD_GRUPO_CIA = cCodGrupoCia_in
                                AND C.COD_LOCAL = cCod_Local_in
                                AND C.FEC_PED_VTA BETWEEN TO_DATE(v_vFecha || ' 00:00:00','DD/MM/YYYY HH24:MI:SS') AND
            ...
            SELECT COUNT(1) CANTIDAD,NVL(SUM(VCP2.VAL_NETO_COMP_PAGO- NVL(VCP2.VAL_PERCEP_COMP_PAGO,0)+VCP2.val_redondeo_comp_pago),0) TOTAL_NETO_REDONDEO--,VCP2.TIP_COMP_PAGO
            FROM  (
            ...
            -- KMONCADA 02.05.2016 PERCEPCION
                       SELECT COUNT(1) CANTIDAD, NVL(SUM(VCP2.VAL_NETO_COMP_PAGO- NVL(VCP2.VAL_PERCEP_COMP_PAGO,0)+VCP2.val_redondeo_comp_pago),0) TOTAL_NETO_REDONDEO
            FROM  (
            SELECT DISTINCT VPC.COD_GRUPO_CIA,VPC.COD_LOCAL,VPC.NUM_PED_VTA_ORIGEN,VPC.sec_mov_caja
            FROM VTA_COMP_PAGO VCP JOIN VTA_PEDIDO_VTA_CAB VPC ON (VCP.COD_GRUPO_CIA = cCodGrupoCia_in
                                        AND VCP.COD_LOCAL = cCod_Local_in
...
RETURN FarmaCursor IS
 ...
 /****************************************************************************************************************************/
  ...
  BEGIN
	...
	IF cCodFormaPago_in = COD_FORMA_PAGO_PUNTOS THEN
...
--		RETURN;
    ...
    INSERT INTO VTA_FORMA_PAGO_PEDIDO(COD_GRUPO_CIA,
      ...
      IF cCodFormaPago_in = COD_FORMA_PAGO_NCR THEN
	    ...
	    MARCA_USO_NCR(cCodGrupoCia_in,cCodLocal_in,v_cNumPedVtaOrigen,cNumPedVta_in);
...
--    AND    X.TIP_COMP_PAGO     = cTipComp_in
 ...
 END;
  ...
  PROCEDURE MARCA_USO_NCR(cCodGrupoCia_in IN CHAR,cCodLocal_in IN CHAR,cNumPedVtaOrigen_in IN CHAR,cNumPedVta_in IN CHAR) IS
  BEGIN  
    UPDATE VTA_PEDIDO_VTA_CAB CAB
        SET NUM_PED_VTA_USO_NC = cNumPedVta_in,
        FEC_MOD_PED_VTA_CAB = SYSDATE,
        USU_MOD_PED_VTA_CAB = 'USO_NCR'
        WHERE CAB.COD_GRUPO_CIA = cCodGrupoCia_in
        AND CAB.COD_LOCAL = cCodLocal_in
        AND CAB.NUM_PED_VTA_ORIGEN = cNumPedVtaOrigen_in;
  END;
...
/******************************************************************************/  
 ...
 BEGIN
   BEGIN
     SELECT TO_NUMBER(A.LLAVE_TAB_GRAL,'9999990.00')
     INTO vCantidad
     FROM PBL_TAB_GRAL A
     WHERE A.ID_TAB_GRAL=555;
   EXCEPTION
     WHEN OTHERS THEN
       vCantidad := 1;
   END;
...
/******************************************************************************/
 ...
 FUNCTION F_LST_TARJETAS_LOGO(cCodGrupoCia_in   IN PBL_LOCAL.COD_GRUPO_CIA%TYPE,
                              cCodLocal_in      IN PBL_LOCAL.COD_LOCAL%TYPE)
   RETURN FARMACURSOR IS
   curTarjetas FARMACURSOR;
 BEGIN
   OPEN curTarjetas FOR
     SELECT A.NOM_TARJETA, 
            A.IMG_LOGO, 
            A.IMG_AYUDA, 
            A.COD_FORMA_PAGO
     FROM REL_TARJ_LOGO_LOCAL R,
          MAE_TARJETA_LOGO_FORMA_PAGO A
     WHERE R.ID_TARJETA_LOGO = A.ID_TARJETA_LOGO
     AND R.COD_GRUPO_CIA = cCodGrupoCia_in
     AND R.COD_LOCAL = cCodLocal_in
     AND R.ESTADO = 'A'
     ORDER BY R.ORDEN_TARJETA
     ;
   RETURN curTarjetas;
 END;
...
/************************************************************************************/
  ...
  FUNCTION F_GET_SECUENCIAL_IMPRESORA(cCodGrupoCia_in IN VTA_IMPR_LOCAL.COD_GRUPO_CIA%TYPE,
                                      ...
                                      cIsPedManual_in IN CHAR DEFAULT 'N')
    RETURN VARCHAR2 IS
    vSecImpresora VARCHAR2(5);
  BEGIN
    IF cIsPedManual_in = 'S' AND cTipoCP_in NOT IN (COD_TIP_COMP_BOLETA, COD_TIP_COMP_FACTURA) THEN
      RETURN NULL;
    END IF;
    BEGIN 
      SELECT TO_CHAR(IL.SEC_IMPR_LOCAL)
      INTO vSecImpresora
      FROM VTA_IMPR_LOCAL IL,
           VTA_IMPR_IP IP 
      WHERE IL.COD_GRUPO_CIA = cCodGrupoCia_in
      AND IL.COD_LOCAL = cCodLocal_in
      AND TRIM(IP.IP) = TRIM(cIpPC_in)
      AND IL.COD_GRUPO_CIA = IP.COD_GRUPO_CIA
      AND IL.COD_LOCAL = IP.COD_LOCAL
      AND IL.TIP_COMP = cTipoCP_in
      AND IL.SEC_IMPR_LOCAL = CASE 
                                WHEN cTipoCP_in IN (COD_TIP_COMP_BOLETA, COD_TIP_COMP_TICKET) THEN
                                  IP.SEC_IMPR_LOCAL
                                WHEN cTipoCP_in = COD_TIP_COMP_FACTURA THEN
                                  IP.SEC_IMPR_LOCAL_FAC
                                ELSE
                                  IL.SEC_IMPR_LOCAL
                               END
      AND IL.EST_IMPR_LOCAL = 'A'
      AND IP.EST_IP = 'A';
    EXCEPTION 
      WHEN OTHERS THEN
        vSecImpresora := '-1';
