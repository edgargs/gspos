  ...
  EXCEPTION
	  WHEN OTHERS THEN
		--ERIOS 14.08.2014 Control de errores
	     ROLLBACK;
	     RAISE_APPLICATION_ERROR(-20050,'SP_GRABA_DOCUMENTOS:' || sqlerrm);
  END;	      
END;
...
SELECT DISTINCT COD_LOCAL,COD_MAQUINA,NUM_IP,COD_CIA,COD_USUARIO,FCH_EMISION
    ...
    (  SELECT
             ...
             --P.IP_PC COD_MAQUINA,
             (SELECT COD_MAQUINA FROM BTLPROD.AUX_MAQUINA_X_LOCAL WHERE COD_MAQUINA=P.IP_PC) COD_MAQUINA,
             P.IP_PC NUM_IP,
             (SELECT ML.CIA FROM NUEVO.AUX_MAE_LOCAL L INNER JOIN NUEVO.Mae_Local ML ON L.COD_LOCAL=ML.COD_LOCAL
...
function sp_anula_documento(pcodtipodoc char,
                            pnumdoc     char,
                            pcodlocal   char default null) return char as
    ...
    for reg in c_datos(v_cia) loop
      ...
      if reg.cod_tipo_documento = 'GRL' then
        ...
        begin
         ...
         where   cg.num_guia = reg.num_documento ;
