...
--                       (SELECT MAX(DECODE(CP.TIP_COMP_PAGO,'01','BOL-','02','FAC-','05','TB-') || SUBSTR(CP.SEC_COMP_PAGO,1,3) ||'-'|| SUBSTR(CP.SEC_COMP_PAGO,-7)) FROM VTA_COMP_PAGO CP WHERE CP.COD_GRUPO_CIA = C.COD_GRUPO_CIA AND CP.COD_LOCAL = C.COD_LOCAL AND CP.NUM_PED_VTA = C.NUM_PED_VTA) "COMPROB",
 ...
 /* ********************************************************* */
  ...
  IS
    ...
    textoCompletoReniec varchar2(200) := '';
  ...
  vFilaCliente varchar2(6000):= '';


    v_vCabecera2 VARCHAR2(500);
    
    codigo_texto_legal number(3) := 687;
    texto_legal varchar2(2000) := '';
    
  BEGIN
    
  textoCompletoReniec := UTILITY_DNI_RENIEC.AUX_DATOS_EXISTE_DNI(cCodGrupoCia_in,
                                   cCodLocal_in,
                                   cDNI_in);
                                   
  textoLegal := PTOVENTA_MDIRECTA.GET_VAL_GEN_STRING(codigo_texto_legal);
                                   
--==================== DATOS RENIEC ==============<<<<<<<<<<<<<

    select substr(textoCompletoReniec,1,instr(textoCompletoReniec,'@',1,1)-1)
    into dni_cli
    from dual;    
    textoCompletoReniec := REPLACE(textoCompletoReniec,substr(textoCompletoReniec,1,instr(textoCompletoReniec,'@',1,1)),'');
    ...
    select decode(substr(textoCompletoReniec,1,instr(textoCompletoReniec,'@',1,1)-1),
    ...
    select substr(textoCompletoReniec,7,2)|| '/' || substr(textoCompletoReniec,5,2) || '/' ||substr(textoCompletoReniec,1,4)
    INTO fec_nac_cli
    from dual;
...
--otros datos
    ...
    if dni_cli is null or dni_cli = '' then
        
        select nvl(a.nom_cli || ' ' || a.ape_pat_cli || ' ' || a.ape_mat_cli,'_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _'),
               nvl(decode(a.sexo_cli,'M','MASCULINO','FEMENINO'),'_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _'),
               nvl(to_char(a.fec_nac_cli,'dd/MM/yyyy'),'_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _'),
               a.dni_cli
        INTO  nom_cli,
              sexo_cli,
              fec_nac_cli,
...
-------------
   ...
   v_vCabecera2 := PTOVENTA_GRAL.GET_DIRECTORIO_RAIZ||'\'||PTOVENTA_GRAL.GET_DIRECTORIO_IMAGENES||'\'||PTOVENTA_GRAL.GET_RUTA_IMG_CABECERA_2||PTOVENTA_GRAL.GET_RUTA_IMAGEN_MARCA(cCodGrupoCia_in,cCodCia_in,cCodLocal_in);
     ...
     vFilaCliente := '
            ...
            <B>Firma: </B>_________________</td> </tr>' ||
            ...
            '<tr>
            <td height="30" colspan="30"  align="lef" style="font:Arial, Helvetica, sans-serif">
            <B>Vendedor: </B>' || vendedor || '</td> </tr>' ||
            
            '<tr>
            <td height="30" colspan="30"  align="lef" style="font:Arial, Helvetica, sans-serif">
            <B>Local: </B>' || descLocal || '</td> </tr>';
