    ...
    OPEN curCli FOR
          SELECT LOC.DESC_CORTA_LOCAL || 'Ã' ||
                 ...
                 CASE
                   ...
                   ELSE NVL(TMP_CAB.DIR_CLI_PED_VTA,' ') END || 'Ã' ||
...
FUNCTION CLI_LISTA_TRANSFERENCIA(
 ...
 --@@@ se coloca el un lote , que contenga la mayot cantidad solicitada
  ...
  BEGIN
    ...
    INSERT INTO TMP_COMANDA_DELIVERY VALUES('PEDIDO DELIVERY','1','C','B','N');
    ...
    SELECT REPLACE((REPLACE((EXTRACTVALUE(xt.column_value, 'e')),'Ã','&')),'Ë','<') VAL ,'1','I','N','N'
     ...
     INSERT INTO TMP_COMANDA_DELIVERY VALUES('CLIENTE','1','I','B','N');
     ...
     SELECT REPLACE((REPLACE((EXTRACTVALUE(xt.column_value, 'e')),'Ã','&')),'Ë','<') VAL ,'1','I','N','N'
      ...
      IF vCodConvenio IS NOT NULL THEN
        INSERT INTO TMP_COMANDA_DELIVERY VALUES('TIPO DE VENTA','1','I','B','N');
        ...
        INSERT INTO TMP_COMANDA_DELIVERY VALUES('VERIFICACION DE DOCUMENTOS','1','I','B','N');

        INSERT INTO TMP_COMANDA_DELIVERY
          SELECT
          CASE
            WHEN TEXT IS NOT NULL THEN RPAD(' ',vEspaciosTab)||RPAD(TEXT,vEspacios)||': '||VERIFICAR
            ELSE RPAD(' ',vEspaciosTab)||RPAD(' ',vEspacios+2)||VERIFICAR END,'1','I','N','N'
          FROM (
            SELECT
              CASE
        ...
        INSERT INTO TMP_COMANDA_DELIVERY VALUES('*','1','I','N','N');
      END IF;

      -- KMONCADA 15.09.2014 DESCRIPCION DE PRODUCTOS EN COMANDA
      -- DOCUMENTOS
      INSERT INTO TMP_COMANDA_DELIVERY VALUES('DETALLE','1','I','B','N');
      INSERT INTO TMP_COMANDA_DELIVERY VALUES('CODIGO  CANTIDAD  DESCRIPCION - LABORATORIO.','1','I','N','N');
      INSERT INTO TMP_COMANDA_DELIVERY
      SELECT RPAD(DET.COD_PROD,8)|| LPAD(DET.CANT_ATENDIDA||DECODE(DET.VAL_FRAC,1,'','/'||DET.VAL_FRAC),7)||'  '|| PROD.DESC_PROD||' '||PROD.DESC_UNID_PRESENT||' - '|| SUBSTR(LAB.NOM_LAB,0,5),'1','I','N','N'
      FROM TMP_VTA_PEDIDO_VTA_DET DET,
      LGT_PROD PROD,
      LGT_LAB LAB
      WHERE
      ...
      INSERT INTO TMP_COMANDA_DELIVERY VALUES('DOCUMENTOS','1','I','B','N');

      INSERT INTO TMP_COMANDA_DELIVERY
      SELECT  RPAD(' ',vEspaciosTab)||RPAD(TCP.DESC_COMP,30) || CP.NUM_COMP_PAGO,'1','I','N','N'
      ...
      INSERT INTO TMP_COMANDA_DELIVERY VALUES('*','1','I','N','N');
      INSERT INTO TMP_COMANDA_DELIVERY VALUES('FORMA DE PAGO','1','I','B','N');
      INSERT INTO TMP_COMANDA_DELIVERY VALUES(RPAD(' ',vEspaciosTab)||RPAD(' ', 15) || LPAD('PAGO', 14) || LPAD('VUELTO', 10),'1','I','N','N');

      INSERT INTO TMP_COMANDA_DELIVERY
      SELECT Q.FORMA_PAGO, '1', 'I', 'N', 'N'
      FROM (
      ...
      IF vCodConvenio IS NOT NULL THEN
        INSERT INTO TMP_COMANDA_DELIVERY
        select LPAD('TOTAL A PAGAR',25)|| TO_CHAR(sum(a.im_total_pago-a.val_vuelto), '999,999,990.00') , '1', 'I', 'B', 'N'
        from tmp_vta_forma_pago_pedido a
        WHERE A.COD_GRUPO_CIA = cCodGrupoCia_in
      ...
      SELECT Q.TOTAL, '1', 'I',
      CASE
        WHEN INSTR(Q.TOTAL,'TOTAL CRED.CONVENIO')=0 THEN 'B'
        ELSE 'N' END , 'N'
        ...
        INSERT INTO TMP_COMANDA_DELIVERY
        SELECT (EXTRACTVALUE(xt.column_value, 'e')) , '1' , 'I' ,
         CASE
...
--      COMMIT;
  ...
  BEGIN
    ...
    SELECT MAX(COMP.FECH_IMP_COBRO) 
    ...
    AND   TMP.NUM_PED_VTA   = CAB.NUM_PEDIDO_DELIVERY
