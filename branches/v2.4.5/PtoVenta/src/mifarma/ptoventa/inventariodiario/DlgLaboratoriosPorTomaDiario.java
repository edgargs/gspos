...
public class DlgLaboratoriosPorTomaDiario extends JDialog {
	FarmaTableModel tableModelLaboratorioTomaDiario;
        private static final Logger log = LoggerFactory.getLogger(DlgLaboratoriosPorTomaDiario.class);
	...
	// Constructores
	// **************************************************************************
	public DlgLaboratoriosPorTomaDiario() {
		this(null, "", false);
	}

	public DlgLaboratoriosPorTomaDiario(Frame parent, String title, boolean modal) {
	...
	private void initTableLaboratoriosToma() {
		tableModelLaboratorioTomaDiario = new FarmaTableModel(ConstantsInvDiario.columnsLaboratoriosToma,ConstantsInvDiario.defaultValuesLaboratoriosToma, 0);
		FarmaUtility.initSimpleList(tblRelacionLaboratoriosXToma,tableModelLaboratorioTomaDiario,ConstantsInvDiario.columnsLaboratoriosToma);
  ...
  private void btnRelacionLaboratoriosToma_actionPerformed(ActionEvent e)
  {
	...
	private void cargaLaboratoriosToma() {
		try {
			DBInvDiario.getListaLabsXToma(tableModelLaboratorioTomaDiario);
			if (tblRelacionLaboratoriosXToma.getRowCount() > 0)
				FarmaUtility.ordenar(tblRelacionLaboratoriosXToma,tableModelLaboratorioTomaDiario, 1,FarmaConstants.ORDEN_ASCENDENTE);
	...
	private void cargarRegistroSeleccionado() {
		VariablesInvDiario.vCodLab = tblRelacionLaboratoriosXToma.getValueAt(tblRelacionLaboratoriosXToma.getSelectedRow(), 0).toString().trim();
		VariablesInvDiario.vNomLab = tblRelacionLaboratoriosXToma.getValueAt(tblRelacionLaboratoriosXToma.getSelectedRow(), 1).toString().trim();
	}
  ...
  private void imprimir() {
      ...
      vPrint.printBold("Toma #: " + VariablesInvDiario.vSecToma, true);
      vPrint.printBold("Tipo         : " + VariablesInvDiario.vDescTipoToma, true);
      vPrint.printBold("Fecha Inicio : " + VariablesInvDiario.vFecIniToma, true);
      vPrint.printBold("Fecha Fin    : " + VariablesInvDiario.vFecFinToma, true);
      vPrint.printBold("Estado       : " + VariablesInvDiario.vDescEstadoToma, true);      
