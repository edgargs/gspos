...
/**=-_Author: Alfredo Sosa Dordán_-=**/
  ...
  PROCEDURE INV_AGREGA_DET_TRANSFERENCIA(cCodGrupoCia_in  IN CHAR,
                                         ...
                                         ,-- GMORAN 18/01/2016
										                     cIndLote_in      IN CHAR default null
										                     --
                                         );
  ...
  --Author         ASOSA 
      PROCEDURE CAJ_UPD_STK_PROD_COMP(cCodGrupoCia_in 	   IN CHAR,
                                           ...
                                           cCodMotKardex_in IN CHAR,    --ASOSA - 17/10/2014
											...
											cTipDoc_in          IN CHAR DEFAULT NULL,
                                           cNumDoc_in          IN CHAR DEFAULT NULL);
  --Descripcion: Actualiza stock (por lote)
  --Fecha       Usuario		  Comentario
  --13/01/2016  ERIOS         Creacion
  PROCEDURE P_ACT_STOCK_PRODUCTO(cCodGrupoCia_in IN CHAR,
								cCodLocal_in IN CHAR,
								cCodProd_in IN CHAR,
								nCantAtendida_in IN NUMBER,
								nValFracLocal_in IN NUMBER,
								vValFracVta_in IN NUMBER,
								vUsuModProdLocal_in IN VARCHAR2,
								cNumLote_in IN CHAR DEFAULT NULL);
...
PROCEDURE PVTA_P_GRAB_PED_VTA_DET(cCodGrupoCia_in 	 	  IN CHAR,
                                      ...
                                      vCodProdLealtad_in     IN CHAR DEFAULT NULL,
                                      -- KMONCADA 2016.01.22 GENERA PROFORMA [LOCAL M]
                                      cGeneraProforma_in  IN  CHAR DEFAULT 'N' --34
                                      ) IS
...
-- 2009-11-09 JOLIVA: Se agrega el porcentaje de ZAN
  ...
  -- 2009-11-09 JOLIVA: Se agrega el porcentaje de ZAN
          ...
          END IF;
     ELSE 
       INSERT INTO TMP_VTA_PEDIDO_VTA_DET (COD_GRUPO_CIA,
            COD_LOCAL,
            NUM_PED_VTA,
            SEC_PED_VTA_DET,
            COD_PROD,
...
PROCEDURE CAJ_UPD_STK_PROD_DETALLE(cCodGrupoCia_in 	   IN CHAR,
  ...
  --ERIOS 13.01.2016 Se agrega el lote de producto
  CURSOR productos_Respaldo IS
		WITH 
		VTA_DET AS (SELECT COD_GRUPO_CIA,COD_LOCAL,NUM_PED_VTA,COD_PROD,VAL_FRAC,LOTE,FECHA_VENCIMIENTO_LOTE,
		                   SUM(CANT_ATENDIDA) CANT_ATENDIDA
            FROM PTOVENTA.VTA_PEDIDO_VTA_DET
            WHERE COD_GRUPO_CIA = cCodGrupoCia_in
             AND	  COD_LOCAL = cCodLocal_in
             AND	  NUM_PED_VTA = cNumPedVta_in
            GROUP BY COD_GRUPO_CIA,COD_LOCAL,NUM_PED_VTA,COD_PROD,VAL_FRAC,LOTE,FECHA_VENCIMIENTO_LOTE)
        SELECT VTA_DET.COD_PROD,
				...
				WHERE COD_GRUPO_CIA = VTA_DET.COD_GRUPO_CIA AND COD_LOCAL = VTA_DET.COD_LOCAL
				AND COD_PROD = VTA_DET.COD_PROD AND LOTE = VTA_DET.LOTE) 
				, PROD_LOCAL.STK_FISICO) STK_FISICO,
			    PROD_LOCAL.VAL_FRAC_LOCAL,
			    PROD_LOCAL.UNID_VTA,
			    VTA_DET.LOTE,
				VTA_DET.FECHA_VENCIMIENTO_LOTE,
          NVL2( (SELECT 1 FROM LGT_PROD_VIRTUAL VIR 
                 WHERE PROD_LOCAL.COD_GRUPO_CIA = VIR.COD_GRUPO_CIA
				         AND      PROD_LOCAL.COD_PROD = VIR.COD_PROD) ,'S','N') IND_PROD_VIR
		 FROM    VTA_DET,
				LGT_PROD_LOCAL PROD_LOCAL
	     WHERE   VTA_DET.COD_GRUPO_CIA = PROD_LOCAL.COD_GRUPO_CIA
				 AND	  VTA_DET.COD_LOCAL = PROD_LOCAL.COD_LOCAL
				 AND	  VTA_DET.COD_PROD = PROD_LOCAL.COD_PROD
         --INI ASOSA - 07/10/2014 - PANHD
             AND  NOT EXISTS (
	...
	v_cEmailErrorPtoVenta PBL_TAB_GRAL.LLAVE_TAB_GRAL%TYPE:='programacion';
  -- KMONCADA 04.08.2016 [PROYECTO M] AJUSTE AUTOMATICO
  vCantLote NUMBER;
  vStockLotePendiente NUMBER;
  BEGIN
  	
   	FOR productos_K IN productos_Respaldo
	  LOOP
	  	  --GRABAR KARDEX
        ...
        IF v_cIndProdVirtual = INDICADOR_SI THEN
    		  Ptoventa_Inv.INV_GRABAR_KARDEX_VIRTUAL(cCodGrupoCia_in,
                            										 ...
                            										 cUsuModProdLocal_in,
                            										 cCodNumeraKardex_in,
																	 NULL,cTipDoc_in,cNumDoc_in);
        ELSIF v_cIndProdVirtual = INDICADOR_NO THEN
          --KMONCADA [PROYECTO M] VERIFICARA PARA REALIZAR EL AJUSTE AUTOMATICO
          IF PTOVENTA_VTA_MAYORISTA.F_PERMITE_AJUSTE_AUTOMATICO = 'S' THEN
            IF FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in, cCodLocal_in) = 'S' THEN
              SELECT COUNT(1), (productos_k.CANT_ATENDIDA - NVL(SUM(STK_FISICO),0))
              INTO vCantLote, vStockLotePendiente
              FROM LGT_PROD_LOCAL_LOTE
              WHERE COD_GRUPO_CIA = cCodGrupoCia_in
              AND COD_LOCAL = cCodLocal_in
              AND COD_PROD = productos_K.COD_PROD
              AND LOTE = productos_K.LOTE;
              -- CUANDO NO EXISTA EL LOTE O EL STOCK DEL LOTE NO CUMPLA SE GENERAA UN MOVIMIENTO DE AJUSTE AUTOMATICO
              IF vCantLote = 0 OR vStockLotePendiente > 0 THEN
                PTOVENTA_PROFORMA.P_AJUSTE_POR_ENTEGRA(cCodGrupoCia_in => cCodGrupoCia_in,
                                                       cCodLocal_in => cCodLocal_in,
                                                       cCodProd_in => productos_K.COD_PROD,
                                                       nCantidad_in => vStockLotePendiente,
                                                       vLote_in => productos_K.LOTE,
                                                       vFechaVenc_in => to_char(productos_K.FECHA_VENCIMIENTO_LOTE,'dd/mm/yyyy'),
                                                       vUsuCrea_in => cUsuModProdLocal_in);
              END IF;
            END IF;
          END IF;
          Ptoventa_Inv.INV_GRABAR_KARDEX(cCodGrupoCia_in,
		    								               cCodLocal_in,
                    										 ...
                    										 cCodNumeraKardex_in,
															 NULL,cTipDoc_in,cNumDoc_in,
															 to_char(productos_K.FECHA_VENCIMIENTO_LOTE,'dd/mm/yyyy'),
															 productos_K.LOTE
															 );
        
          P_ACT_STOCK_PRODUCTO(cCodGrupoCia_in,
								cCodLocal_in,
								productos_K.COD_PROD,
								(productos_k.CANT_ATENDIDA * -1),
								productos_K.VAL_FRAC_LOCAL,
...
/***********************************************************************************************************************/
  ...
  cantExceso, cantUsadaExceso                             --ASOSA - 22/08/2014
	...
	vIndLote_in := FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in, cCodLocal_in);
	IF(vIndLote_in = 'S')THEN
      SELECT COUNT(*), NVL(SUM(STK_FISICO),0)
      INTO v_count_prod, v_nStkFisico
      FROM LGT_PROD_LOCAL_LOTE
      WHERE  COD_GRUPO_CIA = cCodGrupoCia_in  
          AND    COD_LOCAL     = cCodLocal_in    
          AND    COD_PROD      = cCodProd_in
...
PROCEDURE RECEP_P_OPERAC_ANEXAS_PROD(cCodGrupoCia_in IN CHAR,
                                       ...
                                       nDiferencia_in  IN NUMBER DEFAULT NULL,
                                       nNumLoteProd_in   IN CHAR DEFAULT NULL,
                                       nFecVctoProd_in  IN CHAR DEFAULT NULL) AS
    v_nValFrac     LGT_NOTA_ES_DET.VAL_FRAC%TYPE;
    v_vDescUnidVta LGT_NOTA_ES_DET.DESC_UNID_VTA%TYPE;
    v_nStkFisico   LGT_PROD_LOCAL.STK_FISICO%TYPE;
    ...
    --KMONCADA 20.07.2016 [PROYECTO M]
    vExisteLote CHAR(1) := 'N';
  ...
  BEGIN
    ...
    -- KMONCADA 20.07.2016 [PROYECTO M]
    IF (FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in, cCodLocal_in) = 'S') THEN
      SELECT DECODE(COUNT(1),0,'N','S'), NVL(SUM(A.STK_FISICO),0)
      INTO vExisteLote, v_nStkFisico
      FROM LGT_PROD_LOCAL_LOTE A
      WHERE A.COD_GRUPO_CIA = cCodGrupoCia_in
      AND A.COD_LOCAL = cCodLocal_in
      AND A.COD_PROD = cCodProd_in
      AND A.LOTE = nNumLoteProd_in;
        
      IF vExisteLote = 'N' THEN
        INSERT INTO LGT_PROD_LOCAL_LOTE
          (COD_GRUPO_CIA, COD_LOCAL, COD_PROD, LOTE, STK_FISICO, FECHA_VENCIMIENTO_LOTE, USU_CREA_PROD_LOCAL_LOTE, FEC_CREA_PROD_LOCAL_LOTE)
        VALUES
          (cCodGrupoCia_in, cCodLocal_in, cCodProd_in, nNumLoteProd_in, (nCantMov_in * v_nValFrac), to_date(nFecVctoProd_in, 'dd/mm/yyyy'), cIdUsu_in, SYSDATE);
        v_nStkFisico := 0;
      ELSE
        --ACTUALIZA PROD LOCAL_LOTE
        UPDATE LGT_PROD_LOCAL_LOTE
        SET USU_MOD_PROD_LOCAL_LOTE = cIdUsu_in,
            FEC_MOD_PROD_LOCAL_LOTE = SYSDATE,
            STK_FISICO              = STK_FISICO + nCantMov_in * v_nValFrac
        WHERE COD_GRUPO_CIA = cCodGrupoCia_in
        AND COD_LOCAL = cCodLocal_in
        AND COD_PROD = cCodProd_in
        AND LOTE = nNumLoteProd_in;  
      END IF;
    END IF;
...
AND A.COD_GRUPO_CIA = cCodGrupoCia_in;
  ...
  **/
  ...
  PROCEDURE P_ACT_STOCK_PRODUCTO(cCodGrupoCia_in IN CHAR,
								...
								cNumLote_in IN CHAR DEFAULT NULL)
  IS
    v_nCantAtendida PTOVENTA.VTA_PEDIDO_VTA_DET.CANT_ATENDIDA%TYPE;
	stkFis  lgt_prod_local.stk_fisico%TYPE;
	stkLoteFis  lgt_prod_local.stk_fisico%TYPE;
  BEGIN
    ...
    SELECT STK_FISICO
	  ...
	  v_nCantAtendida := (nCantAtendida_in*nValFracLocal_in)/vValFracVta_in;
		  
    IF cNumLote_in IS NOT NULL THEN 
	  ...
	  WHERE COD_GRUPO_CIA = cCodGrupoCia_in
			AND	   COD_LOCAL = cCodLocal_in
			AND	   COD_PROD = cCodProd_in 
			AND    LOTE = cNumLote_in FOR UPDATE; 
			
	  UPDATE LGT_PROD_LOCAL_LOTE SET USU_MOD_PROD_LOCAL_LOTE = vUsuModProdLocal_in, FEC_MOD_PROD_LOCAL_LOTE = SYSDATE,
				 STK_FISICO = STK_FISICO + v_nCantAtendida
				WHERE  COD_GRUPO_CIA = cCodGrupoCia_in
				AND	   COD_LOCAL = cCodLocal_in
				AND	   COD_PROD = cCodProd_in
				AND    LOTE = cNumLote_in;
	END IF;
	
	  UPDATE LGT_PROD_LOCAL SET USU_MOD_PROD_LOCAL = vUsuModProdLocal_in, FEC_MOD_PROD_LOCAL = SYSDATE,
				 STK_FISICO = STK_FISICO + v_nCantAtendida
				WHERE  COD_GRUPO_CIA = cCodGrupoCia_in
				AND	   COD_LOCAL = cCodLocal_in
				AND	   COD_PROD = cCodProd_in;
