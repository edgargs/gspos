    ...
    public void imprimirTextoExpert(FarmaPrinterFacade printer,
        ...
        if (flag) {
            ...
            if (cod.equals("A")) {
                ...
                valor = "Llevas S/." + valor + " soles de ahorro en los ultimos 12 meses";  //
                //valor = "Llevas S/." + valor + " soles de ahorro";
...
ruta + fechaImpresion + "_" + nomDocumento.substring(0, 1) + "_E_" + VariablesCaja.vNumPedVta + "_" +
 serieCorrelativo + ".TXT";
    ...
    /**
     * Realiza la impresion del documento en impresora termica configurada a la PC.
     * @author KMONCADA
     * @since 01.06.2015
     * @param listDocumento documento a imprimir en la impresora termica
     * @param pRutaFile ruta del archivo a grabar en caso se desee, sino null
     * @return 
     * @throws Exception
     */
    public boolean impresionTermica(List listDocumento, String pRutaFile)throws Exception {
        ...
        if (!printer.startPrintService()) {
            ...
            printer.inicializate(); 
            Map lineaDocumento = new HashMap();
            String valor, tipoDato, tamanio, alineacion, negrita, subrayado, interlineado, inverso, saltoLinea;
            for (int i = 0; i < listDocumento.size(); i++) {
                lineaDocumento = (HashMap)listDocumento.get(i);
                valor = ((String)lineaDocumento.get("VALOR"));
                tipoDato = ((String)lineaDocumento.get("TIPO_DATO"));
                tamanio = ((String)lineaDocumento.get("TAMANIO"));
                alineacion = ((String)lineaDocumento.get("ALINEACION"));
                negrita = ((String)lineaDocumento.get("NEGRITA"));
                subrayado = ((String)lineaDocumento.get("SUBRAYADO"));
                interlineado = ((String)lineaDocumento.get("INTERLINEADO"));
                inverso = ((String)lineaDocumento.get("COLOR_INVERSO"));
                saltoLinea = ((String)lineaDocumento.get("SALTO_LINEA"));
                log.info("-->"+tipoDato+" \t "+valor);
                ArrayList lstValor = new ArrayList();
                //TIPO DE ALINEACION JUSTIFICADO
                if("J".equalsIgnoreCase(alineacion)){
                    lstValor = aplicarJustificacion(valor, tamanio);
                }else{
                    lstValor.add(valor);
                }
                for(int k = 0;k<lstValor.size();k++){
                    printer.printer((String)lstValor.get(k), tipoDato, tamanio, alineacion, negrita, subrayado, interlineado, inverso, saltoLinea);
                }
            }
