   ...
   * ****************************************************** */
    ...
    */
    ...
    private void initTable() {
        tableModel =
                new FarmaTableModel(ConstantsControlAsistencia.columnsListaHistTemp, ConstantsControlAsistencia.defaultValuesListaHistTemp,
                                    0);
        FarmaUtility.initSimpleList(tblHistorico, tableModel, ConstantsControlAsistencia.columnsListaHistTemp);
        cargaListaHistorico();
    ...
    private void chkKeyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_F3) {
            if (!ValidaRolQF()) {
                FarmaUtility.showMessage(this, ConstantsControlAsistencia.MENSAJE_ROL, tblHistorico);
            } else {
                VariablesControlAsistencia.vTipoAccionTemp = FarmaConstants.ACCION_INSERT;
            ...
            if (tblHistorico.getRowCount() > 0) {
                if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                                                                                "Esta seguro de exportar el histórico?")) {
                    exportar();
                }
            } else {
    ...
    private void cargaListaHistorico() {
        try {
            ...
            DBControlAsistencia.cargaListaHistoricoTemp(tableModel);
    ...
    private void exportar() {
        ...
        FarmaUtility.saveFileRuta(ruta, ConstantsControlAsistencia.columnsListaHistTemp, tblHistorico,
                                  new int[] { 8, 25, 25, 24, 24, 24 });
    ...
    private boolean ValidaRolQF() {
        ...
        try {
            result =
                    DBControlAsistencia.verificaRolUsuario(FarmaVariables.vNuSecUsu, ConstantsControlAsistencia.ROL_QF_ADMINLOCAL);
    ...
    private void mostrarhistoricoFiltro() {
        ...
        try {
            DBControlAsistencia.ListarHistoricoTempFiltro(tableModel, fecini, fecfin);
            if (tblHistorico.getRowCount() > 0) {
                FarmaUtility.ordenar(tblHistorico, tableModel, 0, FarmaConstants.ORDEN_DESCENDENTE);
                FarmaUtility.moveFocus(tblHistorico);
            } else {
