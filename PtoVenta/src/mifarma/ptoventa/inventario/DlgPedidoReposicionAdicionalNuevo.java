...

{
    ...
    private JLabelWhite lblRpta = new JLabelWhite();
    ...
    private void jbInit() throws Exception {
        ...
        txtCantidad.setBounds(new Rectangle(430, 350, 105, 25));
        ...
        lblCantidad.setBounds(new Rectangle(425, 325, 325, 15));
        ...
        lblRpta.setText("");
        lblRpta.setBounds(new Rectangle(425, 375, 350, 30));
        lblRpta.setForeground(Color.black);
        lblRpta.setVisible(false);
        lblRpta.setFont(new Font("SansSerif", 1, 13)); 
    ...
    private void initTable() {
        tableModel =
                new FarmaTableModel(ConstantsInventario.columnsListaProductosPedidoReposicionAdicional, ConstantsInventario.defaultListaProductosPedidoReposicionAdicional,
                                    0);
...
*/
    ...
    private void cargarMeses() {
        ...
        ArrayList<Object> myLista = new ArrayList<>();
        try {
            DBInventario.cargarMeses(lista);
        } catch (SQLException f) {
            log.error("", f);
        }
        myLista = (ArrayList<Object>)lista.get(0);
    ...
    private void txtCantidad_keyReleased(KeyEvent e) {
        String vCantidadPvm = txtCantidad.getText().trim();
        String vStkMax = "0";
        if (vCantidadPvm.trim().length() > 0) {                                    
            try {
                cargarCabecera();
                vStkMax = DBInventario.getStkMaximo(VariablesInventario.vCodProd_PedRep,vCantidadPvm);
                if (vStkMax.equals("N")){ //Si el valor es N indica que el flag se encuentra inactivo
                    lblRpta.setText("");
                    lblRpta.setVisible(false);
                }
                else{
                    lblRpta.setText("Su local contará con "+ vStkMax.trim() +" unidades.");
                    log.info("COD_PROD = "+VariablesInventario.vCodProd_PedRep+" - PVM = "+vCantidadPvm);                    
                }
                vCantidadPvm = "";                
            } catch (SQLException sql) {
                log.error("", sql);
                FarmaUtility.showMessage(this, "Error al obtener el Stock Maximo del Producto", txtBuscar);
            }     
        }
        else{
             lblRpta.setText("");   
            ...
            if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                ...
                try {
                    if (FarmaVariables.vEconoFar_Matriz) {
                        ...
                        UtilityInventario.guardarCantidadPedidoAdicionalLocal(VariablesInventario.vCodProd_PedRep,
                                                                              VariablesInventario.vCantAdicional, "0",
                                                                              "N");
                ...
                tblListaProductos.setRowSelectionInterval(VariablesInventario.vPos_PedRep,
                                                          VariablesInventario.vPos_PedRep);
                FarmaUtility.moveFocus(txtBuscar);
                txtCantidad.setText("");
                setIngresaCantidad(false);
            } else {
    ...
    private void muestraDetalleOrd() {
        ...
        FarmaLoadCVL.loadCVLfromArrays(dlgOrdenar.getCmbCampo(), VariablesInventario.vNombreInHashtable,
                                       ConstantsInventario.vCodCampoPedPVM, ConstantsInventario.vDescCampoPedPVM,
                                       true);
        dlgOrdenar.setVisible(true);
        if (FarmaVariables.vAceptar) {
            FarmaVariables.vAceptar = false;
            FarmaUtility.ordenar(tblListaProductos, tableModel, VariablesInventario.vCampo,
