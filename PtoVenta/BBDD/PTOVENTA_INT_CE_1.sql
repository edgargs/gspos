--------------------------------------------------------
--  DDL for Package Body PTOVENTA_INT_CE
--------------------------------------------------------
...
--           NVL(TRAB.COD_TRAB_RRHH,' ') ASIG_1,
           ---jquispe 11.04.2011 cambio interfaz para conv mifarma
           NVL(LPAD(TRAB.NUM_DOC_IDEN,10,'0'),' ') ASIG_1,
-- 2011-11-21 JOLIVA: Se cambia para que se muestre el código del local en lugar del nombre.
--           TO_CHAR(MOV.FEC_DIA_VTA,'MON') || '-' || LOCAL.DESC_CORTA_LOCAL || '-' || TRAB.APE_PAT_TRAB || ',' || TRAB.NOM_TRAB TEX_DET_1,
...
--    AND    F.COD_FORMA_PAGO NOT IN ('00047','00025','00051')
    AND    F.COD_FORMA_PAGO NOT IN ('00055', '00047','00025','00051')
...
-- 2013-07-24 JOLIVA: SE AGREGAN COMPROBANTES POR CONVENIOS BTL
      ...
      SELECT
            ...
            ' ' TEX_CAB,
            'PEN'                  MONEDA,
            TO_CHAR(-1 * (CP_ORIG.VAL_NETO_COMP_PAGO + CP_ORIG.VAL_REDONDEO_COMP_PAGO),'999999990.00') IMPORTE,
             CASE
...
--*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-
  ...
  BEGIN
       ...
       IF nSumaSap IS NULL OR nSumaSap = 0 THEN
       ...
       SELECT
              NVL(SUM(TOTAL*TIPO_CAMBIO),0) INTO nSumaRecSap
       FROM (
