CREATE OR REPLACE PACKAGE PTOVENTA."FARMA_GRAL" AS
  ...
  */
  ...
  C_COD_PROV_DIRECTV CHAR(3) := 'DTV';
	...
	C_COD_PROV_TELF_BITEL CHAR(1) := 'B';
    ...
    g_TIPO_CTRL_ASISTENCIA INTEGER := 19;
...
--                                 cCodCupon_in      IN CHAR,
 ...
 RETURN FarmaCursor;
  ...
  --Fecha       Usuario		    Comentario       
  --08/09/2015  KMONCADA      Creación              
   procedure P_CUR_DATOS_REP_ASISTENCIA(cCodGrupoCia_in   IN CHAR,
                                     cCodLocal_in      IN CHAR,
  ...
  --Fecha       Usuario		    Comentario       
  --08/09/2015  KMONCADA      Creación              
  FUNCTION F_CUR_RPT_ASISTENCIA_x_DIA(cCodGrupoCia_in   IN CHAR,
                                      cCodLocal_in      IN CHAR,
                                      cCodHorario_in    IN CHAR,
                                      vCadenaDias       IN CHAR)
  RETURN FarmaCursor;
   ...
   /**********************************************************************************************/
  --Descripcion: Retorna texto para imprimir el voucher de verificacion de recaudaciones
  --Fecha       Usuario		    Comentario       
  --12/08/2016  ASOSA         Creación 
  FUNCTION F_IMPR_VOU_VERIF_RECAUDACION(cCodGrupoCia_in   IN CHAR,
                                 cCodLocal_in      IN CHAR,
                                 cIpServ_in        IN CHAR,
                                 cCodCia_in in char,
                                 vDniCe  IN VARCHAR2,
      ...
      ELSIF (v_cod_prov_tel = C_COD_PROV_TELF_MOVISTAR) THEN --MOVISTAR
        ...
        EXCEPTION
      		WHEN NO_DATA_FOUND THEN
    			dbms_output.put_line('no se encontro informacion!!!');
        END;
      --ERIOS 27.04.2016 Se agrega el operador BITEL
	  ELSIF (v_cod_prov_tel = C_COD_PROV_TELF_BITEL) THEN 
        BEGIN

          SELECT G.DESC_LARGA
          INTO   DESC_CABECERA
          FROM   PBL_TAB_GRAL G
          WHERE  G.COD_TAB_GRAL = 'BITEL_CABECERA'
		  AND    G.LLAVE_TAB_GRAL = 'CABECERA';

          SELECT G.DESC_LARGA
          INTO   DESC_LINEA_01
          FROM   PBL_TAB_GRAL G
          WHERE  G.COD_TAB_GRAL = 'BITEL_LINEA_01'
		  AND    G.LLAVE_TAB_GRAL = 'DESC_LINEA_01';

          SELECT G.DESC_LARGA
          INTO   DESC_LINEA_02
          FROM   PBL_TAB_GRAL G
          WHERE  G.COD_TAB_GRAL = 'BITEL_LINEA_02'
		  AND    G.LLAVE_TAB_GRAL = 'DESC_LINEA_02';
     ...
     FROM
          (
             ...
             WHERE C.COD_GRUPO_CIA = cGrupoCia_in
               ...
               AND C.FEC_PED_VTA BETWEEN TO_DATE(TO_CHAR(Add_months(TRUNC(SYSDATE,'MM'), -3),'dd/MM/yyyy') || ' 00:00:00', 'dd/MM/yyyy HH24:mi:ss') AND
               --DUBILLUZ 10.05.2016
   ...
   BEGIN
       ...
       if pFechaApertura is not null then
		...
		select count(1) cantidad
				 ...
				 and    CP.COD_TIP_PROC_PAGO='1' and cp.NUM_COMP_PAGO_E=cNumCmpr
				 and    cp.val_neto_comp_pago = TO_NUMBER(cMontoCmpr,'9999900.00')
		union all
		select count(1)
				 from   ptoventa.vta_comp_pago cp
				 where  cp.cod_grupo_cia = cCodGrupoCia
				 and    cp.cod_local = cCodLocal
				 and    cp.tip_comp_pago = cTipComp
				 and    CP.COD_TIP_PROC_PAGO<>'1' and cp.NUM_COMP_PAGO=cNumCmpr
				 and    cp.val_neto_comp_pago = TO_NUMBER(cMontoCmpr,'9999900.00')
		)
		;
...
BEGIN
    IF codMaestro = g_TIPO_CTRL_ASISTENCIA THEN
     OPEN curProvs FOR
     SELECT A.VALOR1 || 'Ã' || 
                     A.DESCRIPCION
     ...
     AND A.COD_MAES_DET NOT IN (156, 152);
...
procedure P_CUR_DATOS_REP_ASISTENCIA(cCodGrupoCia_in   IN CHAR,
                                     cCodLocal_in      IN CHAR,
                                     cCodHorario_in    IN CHAR) is
  vFilaHorario pbl_cab_horario%rowtype; 
  cant number(3) := 0;
BEGIN
  ...
  AND    C.Cod_Horario   = cCodHorario_in;
  ...
  if cant = 1 then
    ...
    -- agrega datos
    delete tmp_data_usu_rep;
    delete tmp_data_tick_rep;

          insert into tmp_data_usu_rep
          (num_dia, dia, sec_usu_local, login_usu, dni_usu, nombre, entrada, salida, entrada_2, salida_2, tipo, de_7_8, 
          de_8_9, de_9_10, de_10_11, de_11_12, de_12_13, de_13_14, de_14_15, de_15_16, de_16_17, de_17_18, de_18_19, 
          de_19_20, de_20_21, de_21_22, de_22_23, de_23_24, orden)
          SELECT farma_utility.obtien_num_dia(DIA) num_dia,DIA,SEC_USU_LOCAL,LOGIN_USU,DNI_USU,NOMBRE,ENTRADA,SALIDA,ENTRADA_2,SALIDA_2,TIPO,
                 DE_7_8,DE_8_9,DE_9_10,DE_10_11,DE_11_12,DE_12_13,DE_13_14,DE_14_15,DE_15_16,
                 DE_16_17,DE_17_18,DE_18_19,DE_19_20,DE_20_21,DE_21_22,DE_22_23,DE_23_24,
                 RANK() OVER (PARTITION BY tipo ORDER BY sec_usu_local) orden
          FROM   (
          SELECT 
          V.DIA,V.SEC_USU_LOCAL,V.LOGIN_USU,V.DNI_USU,V.NOMBRE,V.ENTRADA,V.SALIDA,V.ENTRADA_2,V.SALIDA_2,
          CASE 
            WHEN V.US_QF > 0 THEN 'QF'
            WHEN V.US_TV > 0 THEN 'TV'
            ELSE 'N'
          END TIPO,    
          V.DE_7_8,V.DE_8_9,V.DE_9_10,V.DE_10_11,V.DE_11_12,V.DE_12_13,V.DE_13_14,V.DE_14_15,V.DE_15_16,
          V.DE_16_17,V.DE_17_18,V.DE_18_19,V.DE_19_20,V.DE_20_21,V.DE_21_22,V.DE_22_23,V.DE_23_24
          FROM (
          select 
                 trunc(i.fecha) dia,d.sec_usu_local,u.login_usu,u.dni_usu,
                 u.nom_usu||' '||u.ape_pat||' '||u.ape_mat nombre,
                 i.entrada,i.salida,i.entrada_2,i.salida_2,
                 (
                 select count(1)
                 from  pbl_rol_usu r
                 where r.COD_ROL = '001'
                 ...
                 nvl(case when 7 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end ,case when 7 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end ) de_7_8,
                 nvl(case when 8 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end ,case when 8 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end ) de_8_9,
                 nvl(case when 9 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end ,case when 9 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end ) de_9_10,
                 nvl(case when 10 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 10 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_10_11,
                 nvl(case when 11 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 11 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_11_12,
                 nvl(case when 12 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 12 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_12_13,
                 nvl(case when 13 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 13 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_13_14,
                 nvl(case when 14 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 14 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_14_15,
                 nvl(case when 15 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 15 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_15_16,
                 nvl(case when 16 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 16 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_16_17,
                 nvl(case when 17 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 17 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_17_18,
                 nvl(case when 18 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 18 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_18_19,
                 nvl(case when 19 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 19 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_19_20,
                 nvl(case when 20 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 20 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_20_21,
                 nvl(case when 21 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 21 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_21_22,
                 nvl(case when 22 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 22 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_22_23,
                 nvl(case when 23 between to_char(nvl(i.entrada,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida,nvl(i.entrada,trunc(i.fecha)-2)),'HH24')+(case when i.entrada-i.salida > 0 then + 20 else 0 end)  then 'x' else null  end,case when 23 between to_char(nvl(i.entrada_2,trunc(i.fecha)-2),'HH24') and to_char(nvl(i.salida_2,nvl(i.entrada_2,trunc(i.fecha)-2)),'HH24')+(case when i.entrada_2-i.salida_2 > 0 then + 20 else 0 end)  then 'x' else null  end) de_23_24
          from   pbl_det_horario d,
                 pbl_cab_horario c,
                 pbl_ingreso_personal i,
                 pbl_usu_local u
          where  c.cod_grupo_cia = cCodGrupoCia_in
          and    c.cod_local = cCodLocal_in
          and    c.cod_horario = cCodHorario_in
          ...
          and    trunc(i.fecha) between c.fec_inicio  and c.fec_fin
          and    d.cod_grupo_cia = i.cod_grupo_cia
          and    d.cod_local = i.cod_local
          and    d.cod_grupo_cia = u.cod_grupo_cia
          and    d.cod_local = u.cod_local
          and    d.sec_usu_local = u.sec_usu_local
          and    u.dni_usu = i.dni) V ) F;


          insert into tmp_data_tick_rep 
          (num_dia, dia, ntck_7_8, ntck_8_9, ntck_9_10, ntck_10_11, ntck_11_12, ntck_12_13, ntck_13_14, ntck_14_15, ntck_15_16, 
           ntck_16_17, ntck_17_18, ntck_18_19, ntck_19_20, ntck_20_21, ntck_21_22, ntck_22_23)
          select farma_utility.obtien_num_dia(trunc(c.fec_ped_vta) ) num_dia, trunc(c.fec_ped_vta) dia,
                  sum(case when 7 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_7_8,
                  sum(case when 8 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_8_9,
                  sum(case when 9 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_9_10,
                  sum(case when 10 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_10_11,
                  sum(case when 11 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_11_12,
                  sum(case when 12 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_12_13,
                  sum(case when 13 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_13_14,
                  sum(case when 14 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_14_15,
                  sum(case when 15 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_15_16,
                  sum(case when 16 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_16_17,
                  sum(case when 17 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_17_18,
                  sum(case when 18 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_18_19,
                  sum(case when 19 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_19_20,
                  sum(case when 20 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_20_21,
                  sum(case when 21 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_21_22,
                  sum(case when 22 = to_char(c.fec_ped_vta,'hh24')*1 then 1 else 0 end) ntck_22_23
          from   vta_pedido_vta_cab c
          where  c.cod_grupo_cia = cCodGrupoCia_in
          and    c.cod_local = cCodLocal_in
          and    c.fec_ped_vta between vFilaHorario.Fec_Inicio and vFilaHorario.Fec_Fin+1-1/24/60/60
          and    c.est_ped_vta = 'C'
...
(    
          ...
          '1' POS_FILA,
          tipo||orden as "TIPO",
          decode(sum((case when de_7_8 = 'x' then 1 else 0 end)),0,' ','x') as  h_7_8,
          decode(sum((case when de_8_9 = 'x' then 1 else 0 end)),0,' ','x') as  h_8_9,
          decode(sum((case when de_9_10 = 'x' then 1 else 0 end)),0,' ','x') as  h_9_10,
          decode(sum((case when de_10_11 = 'x' then 1 else 0 end)),0,' ','x') as  h_10_11,
          decode(sum((case when de_11_12 = 'x' then 1 else 0 end)),0,' ','x') as  h_11_12,
          decode(sum((case when de_12_13 = 'x' then 1 else 0 end)),0,' ','x') as  h_12_13,
          decode(sum((case when de_13_14 = 'x' then 1 else 0 end)),0,' ','x') as  h_13_14,
          decode(sum((case when de_14_15 = 'x' then 1 else 0 end)),0,' ','x') as  h_14_15,
          decode(sum((case when de_15_16 = 'x' then 1 else 0 end)),0,' ','x') as  h_15_16,
          decode(sum((case when de_16_17 = 'x' then 1 else 0 end)),0,' ','x') as  h_16_17,
          decode(sum((case when de_17_18 = 'x' then 1 else 0 end)),0,' ','x') as  h_17_18,
          decode(sum((case when de_18_19 = 'x' then 1 else 0 end)),0,' ','x') as  h_18_19,
          decode(sum((case when de_19_20 = 'x' then 1 else 0 end)),0,' ','x') as  h_19_20,
          decode(sum((case when de_20_21 = 'x' then 1 else 0 end)),0,' ','x') as  h_20_21,
          decode(sum((case when de_21_22 = 'x' then 1 else 0 end)),0,' ','x') as  h_21_22,
          decode(sum((case when de_22_23 = 'x' then 1 else 0 end)),0,' ','x') as  h_22_23
          from   tmp_data_usu_rep t
          where  t.num_dia in (SELECT to_number((EXTRACTVALUE(xt.column_value, 'e')),'99990') MASCARA
                                FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' ||
                                                                       REPLACE((vCadenaDias), '@', '</e><e>') ||
                                                                       '</e></coll>'),
                                                               '/coll/e'))) xt
                              )
          group by tipo||orden 
          ...
          '2' POS_FILA,
          'Total VD' ,
          TRIM(TO_NUMBER(sum(case  when H_7_8 = 'x' then 1 else 0 end),'99990')) t_vd_7_8,
          TRIM(TO_NUMBER(sum(case  when H_8_9 = 'x' then 1 else 0 end),'99990')) t_vd_8_9,
          TRIM(TO_NUMBER(sum(case  when H_9_10 = 'x' then 1 else 0 end),'99990')) t_vd_9_10,
          TRIM(TO_NUMBER(sum(case  when H_10_11 = 'x' then 1 else 0 end),'99990')) t_vd_10_11,
          TRIM(TO_NUMBER(sum(case  when H_11_12 = 'x' then 1 else 0 end),'99990')) t_vd_11_12,
          TRIM(TO_NUMBER(sum(case  when H_12_13 = 'x' then 1 else 0 end),'99990')) t_vd_12_13,
          TRIM(TO_NUMBER(sum(case  when H_13_14 = 'x' then 1 else 0 end),'99990')) t_vd_13_14,
          TRIM(TO_NUMBER(sum(case  when H_14_15 = 'x' then 1 else 0 end),'99990')) t_vd_14_15,
          TRIM(TO_NUMBER(sum(case  when H_15_16 = 'x' then 1 else 0 end),'99990')) t_vd_15_16,
          TRIM(TO_NUMBER(sum(case  when H_16_17 = 'x' then 1 else 0 end),'99990')) t_vd_16_17,
          TRIM(TO_NUMBER(sum(case  when H_17_18 = 'x' then 1 else 0 end),'99990')) t_vd_17_18,
          TRIM(TO_NUMBER(sum(case  when H_18_19 = 'x' then 1 else 0 end),'99990')) t_vd_18_19,
          TRIM(TO_NUMBER(sum(case  when H_19_20 = 'x' then 1 else 0 end),'99990')) t_vd_19_20,
          TRIM(TO_NUMBER(sum(case  when H_20_21 = 'x' then 1 else 0 end),'99990')) t_vd_20_21,
          TRIM(TO_NUMBER(sum(case  when H_21_22 = 'x' then 1 else 0 end),'99990')) t_vd_21_22,
          TRIM(TO_NUMBER(sum(case  when H_22_23 = 'x' then 1 else 0 end),'99990')) t_vd_22_23
          from   (
          select 
          tipo||orden as "TIP",
          decode(sum((case when de_7_8 = 'x' then 1 else 0 end)),0,' ','x') as  h_7_8,
          ...
          group by tipo||orden) vv
          union
          select  '3' POS_FILA,
                  'Tickets' ,
                  to_char(sum(ntck_7_8),'99990') t_7_8, 
                  to_char(sum(ntck_8_9),'99990') t_8_9, 
                  to_char(sum(ntck_9_10),'99990') t_9_10, 
                  to_char(sum(ntck_10_11),'99990') t_10_11, 
                  to_char(sum(ntck_11_12),'99990') t_11_12,  
                  to_char(sum(ntck_12_13),'99990') t_12_13,  
                  to_char(sum(ntck_13_14),'99990') t_13_14,  
                  to_char(sum(ntck_14_15),'99990') t_14_15,  
                  to_char(sum(ntck_15_16),'99990') t_15_16, 
                  to_char(sum(ntck_16_17),'99990') t_16_17, 
                  to_char(sum(ntck_17_18),'99990') t_17_18, 
                  to_char(sum(ntck_18_19),'99990') t_18_19, 
                  to_char(sum(ntck_19_20),'99990') t_19_20, 
                  to_char( sum(ntck_20_21),'99990') t_20_21, 
                  to_char(sum(ntck_21_22),'99990') t_21_22, 
                  to_char(sum(ntck_22_23),'99990') t_22_23 
          from   tmp_data_tick_rep v
          where  v.num_dia in (SELECT to_number((EXTRACTVALUE(xt.column_value, 'e')),'99990') MASCARA
                                FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' ||
                                                                       REPLACE((vCadenaDias), '@', '</e><e>') ||
                                                                       '</e></coll>'),
                                                               '/coll/e'))) xt
          ...
          '4' ,
          'Tickets / VD' ,
          --trunc(round(to_number(trim(bb.t_7_8),'9990')/aa.t_vd_7_8)) prom_7_8,
          to_char(case when aa.t_vd_7_8> 0 then trunc(round(to_number(trim(bb.t_7_8),'9990')/aa.t_vd_7_8)) else 0 end ,'9990') prom_7_8,
          to_char(case when aa.t_vd_8_9> 0 then trunc(round(to_number(trim(bb.t_8_9),'9990')/aa.t_vd_8_9)) else 0 end,'9990') prom_8_9,
          to_char(case when aa.t_vd_9_10> 0 then trunc(round(to_number(trim(bb.t_9_10),'9990')/aa.t_vd_9_10)) else 0 end,'9990') prom_9_10,
          to_char(case when aa.t_vd_10_11> 0 then trunc(round(to_number(trim(bb.t_10_11),'9990')/aa.t_vd_10_11)) else 0 end,'9990') prom_10_11,
          to_char(case when aa.t_vd_11_12> 0 then trunc(round(to_number(trim(bb.t_11_12),'9990')/aa.t_vd_11_12)) else 0 end,'9990') prom_11_12,
          to_char(case when aa.t_vd_12_13> 0 then trunc(round(to_number(trim(bb.t_12_13),'9990')/aa.t_vd_12_13)) else 0 end,'9990') prom_12_13,
          to_char(case when aa.t_vd_13_14> 0 then trunc(round(to_number(trim(bb.t_13_14),'9990')/aa.t_vd_13_14)) else 0 end,'9990') prom_13_14,
          to_char(case when aa.t_vd_14_15> 0 then trunc(round(to_number(trim(bb.t_14_15),'9990')/aa.t_vd_14_15)) else 0 end,'9990') prom_14_15,
          to_char(case when aa.t_vd_15_16> 0 then trunc(round(to_number(trim(bb.t_15_16),'9990')/aa.t_vd_15_16)) else 0 end,'9990') prom_15_16,
          to_char(case when aa.t_vd_16_17> 0 then trunc(round(to_number(trim(bb.t_16_17),'9990')/aa.t_vd_16_17)) else 0 end,'9990') prom_16_17,
          to_char(case when aa.t_vd_17_18> 0 then trunc(round(to_number(trim(bb.t_17_18),'9990')/aa.t_vd_17_18)) else 0 end,'9990') prom_17_18,
          to_char(case when aa.t_vd_18_19> 0 then trunc(round(to_number(trim(bb.t_18_19),'9990')/aa.t_vd_18_19)) else 0 end,'9990') prom_18_19,
          to_char(case when aa.t_vd_19_20> 0 then trunc(round(to_number(trim(bb.t_19_20),'9990')/aa.t_vd_19_20)) else 0 end,'9990') prom_19_20,
          to_char(case when aa.t_vd_20_21> 0 then trunc(round(to_number(trim(bb.t_20_21),'9990')/aa.t_vd_20_21)) else 0 end,'9990') prom_20_21,
          to_char(case when aa.t_vd_21_22> 0 then trunc(round(to_number(trim(bb.t_21_22),'9990')/aa.t_vd_21_22)) else 0 end,'9990') prom_21_22,
          to_char(case when aa.t_vd_22_23> 0 then trunc(round(to_number(trim(bb.t_22_23),'9990')/aa.t_vd_22_23)) else 0 end,'9990') prom_22_23
          from   (
          select
          '2' POS_FILA,
          'Total VD' ,
...
/****************************************************************************/
  ...
  IS
    ...
    vIpPc  VARCHAR2(50);
    vValor VARCHAR2(100);
    v_vDescTipoRcd VARCHAR2(100);
    v_vDescProd VARCHAR2(100);
    v_vSignoTipoMoneda VARCHAR2(10);
  BEGIN
    vIdDoc := FARMA_PRINTER.F_GENERA_ID_DOC;
    vIpPc := FARMA_PRINTER.F_GET_IP_SESS;
    
    IF vTipoMoneda = 'S' THEN
       v_vSignoTipoMoneda := 'S/.';
    ELSE
       v_vSignoTipoMoneda := '$';
    END IF;    
    ...
    AND EST_TAB_GRAL = 'A'
    AND A.LLAVE_TAB_GRAL = vTipoRcd;
    
    IF vTipoRcd = '21' THEN
       v_vDescTipoRcd := 'VERIFICACIÓN DE DONACIÓN';
    ELSE
       v_vDescTipoRcd := 'VERIFICACION DE RECAUDACION';
    END IF;
    
    FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc,
                                      vIpPc_in => vIpPc,
                                      vValor_in => v_vDescTipoRcd,
                                      vTamanio_in => FARMA_PRINTER.TAMANIO_1,
                                      vAlineado_in => FARMA_PRINTER.ALING_CEN, 
                                      vNegrita_in => FARMA_PRINTER.BOLD_ACT);
    ...
    FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc,
                                      vIpPc_in => vIpPc,
                                      vValor_in => v_vSignoTipoMoneda || trim(to_char(vMonto,'999990.00')),
                                      vTamanio_in => FARMA_PRINTER.TAMANIO_4,
                                      vAlineado_in => FARMA_PRINTER.ALING_CEN, 
                                      vNegrita_in => FARMA_PRINTER.BOLD_ACT);
