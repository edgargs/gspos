...
public class DlgPedidosBuscar extends JDialog {
    ...
    private void initTableListaPendientes() {
        tableModelListaPendientes =
                new FarmaTableModel(ConstantsCaja.columnsListaPendientesUnir, ConstantsCaja.defaultListaPendientesUnir,
                                    0);
        FarmaUtility.initSimpleList(tblListaPendientes, tableModelListaPendientes,
                                    ConstantsCaja.columnsListaPendientesUnir);
    }
    ...
    private void initTableDetallePedido() {
        tableModelDetallePedido =
                new FarmaTableModel(ConstantsCaja.columnsDetallePedidoUnir, ConstantsCaja.defaultDetallePedidoUnir, 0);
    ...
    private void btnCabecera_keyPressed(KeyEvent e) {
        ...
        if (tblListaPendientes.getSelectedRow() > -1) {
            VariablesCaja.vNumPedPendiente =
                    tblListaPendientes.getValueAt(tblListaPendientes.getSelectedRow(), 1).toString();
            cargaDetallePedido();
        }
        ...
        if (e.getKeyCode() == KeyEvent.VK_F9) {
            ...
            if (validarPedidos()) {
                if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                                                                                "¿Esta seguro que desea unir los pedidos?")) {
                    if (unirPedidos()) {
                        ...
                        VariablesVentas.vNum_Ped_Diario = numPedDiaNuevo.trim();
                        VariablesVentas.vVal_Neto_Ped = lblSoles.getText();
    ...
    private void buscarPedido(String numeroPedidoDia, String fechaPedido) {
        try {
            ...
            if (array.size() == 0) {
                FarmaUtility.showMessage(this, "El Pedido No existe o No se encuentra pendiente de pago.",
                                         txtNumeroPedidoDia);
            } else {
    ...
    private boolean unirPedidos() {
        ...
        try {
            ...
            if (e.getErrorCode() == 20032)
                FarmaUtility.showMessage(this,
                                         "No puede unir estos pedidos porque contienen productos virtuales. Verifique!!!",
                                         txtFechaPedido);
            else
                FarmaUtility.showMessage(this, e.getMessage(), txtFechaPedido);
        }
        ...
        if (tblListaPendientes.getRowCount() < 2) {
            ...
            for (int i = 0; i < tableModelListaPendientes.data.size(); i++) {
                ...
                if (!tipDoc.equals(tipDocAux)) {
                    FarmaUtility.showMessage(this, "No puede unir pedidos de distintos tipos de documentos.",
                                             txtNumeroPedidoDia);
                    retorno = false;
                    break;
                }
            ...
            if (retorno) {
                ...
                for (int i = 0; i < tableModelListaPendientes.data.size(); i++) {
                    ...
                    if (!tipPed.equals(tipPedAux)) {
                        FarmaUtility.showMessage(this, "No puede unir pedidos de distintos tipos.",
                                                 txtNumeroPedidoDia);
                        retorno = false;
                        break;
                    }
                    if (tipConvAux.equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                        FarmaUtility.showMessage(this, "No puede unir pedidos pedidos del tipo Convenio.",
                                                 txtNumeroPedidoDia);
                        retorno = false;
                        break;
                    }
            ...
            if (retorno && tipDoc.equals(ConstantsPtoVenta.TIP_COMP_FACTURA)) {
                ...
                for (int i = 0; i < tableModelListaPendientes.data.size(); i++) {
                    ...
                    if (!rucDoc.equals(rucDocAux)) {
                        FarmaUtility.showMessage(this, "No puede unir pedidos de distintos clientes.",
                                                 txtNumeroPedidoDia);
                        retorno = false;
                        break;
                    }
    ...
    private boolean buscarPedidoEnListado(String vNumPedPendiente, String vNumPedVta) {
        ...
        for (int i = 0; i < tableModelListaPendientes.data.size(); i++) {
            if (((ArrayList)tableModelListaPendientes.data.get(i)).contains(vNumPedPendiente) &&
                ...
                retorno = false;
                FarmaUtility.showMessage(this, "EL Pedido seleccionado, ya está agregado para unir. Seleccione otro.",
                                         txtNumeroPedidoDia);
            }
        }
        return retorno;
