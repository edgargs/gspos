...
public class FacadeDaemon {
    ...
    void enviaDocumentoE(List<MonVtaCompPagoE> lstNotas, BeanConexion beanConexion) {
        ...
        try{
            ...
            //2.1 Graba Cabecera
            for(MonVtaCompPagoE monVtaCompPagoE:lstNotas){
        ...
        actualizaDocumentoE(lstNotas);              
    }

    public void actualizaDocumentoE(List<MonVtaCompPagoE> lstNotas) {
        try{
            dao.openConnection();
            //3.1 Actualiza envio
            for(MonVtaCompPagoE monVtaCompPagoE:lstNotas){
    ...
    public void actualizaDocumentoEPendientes(BeanLocal beanLocal) {
        try{
            dao.openConnection();
            
            dao.updateMonVtaCompPagoEPend(beanLocal.getCodGrupoCia(),beanLocal.getCodCia(),beanLocal.getCodLocal());
            //log.info(String.format("Documento enviado loc: [%s|%s|%s]",beanLocal.getCodLocal()));
