...
public interface MapperConvenioBTLMF {
    ...
    @Select(value="{call #{respuesta, mode=OUT, jdbcType=CHAR} := " +
        "BTLPROD.PKG_DOCUMENTO_MF.SP_GRABA_DOCUMENTOS(" +
            "#{pCodGrupo_Cia_in}," + 
            "#{pCod_local_in}," + 
            "#{pNum_Ped_Vta_in}," + 
            "#{pIndNotaCred}," + 
    ...
    @Select(value="{call  " +
        "PTOVENTA.PTOVENTA_CONV_BTLMF.BTLMF_P_ACT_FECHA_PROC_RAC(" +
            "#{CodGrupoCia_in}," + 
            "#{cCodLocal_in}," + 
            "#{cNroPedido}" + 
            ")}")
    ...
    @Select(value="{call #{listado, mode=OUT, jdbcType=CURSOR, resultMap=resultado} := " +
        "PTOVENTA.PTOVENTA_MATRIZ_CONV_BTLMF.BTLMF_F_CUR_OBT_BENIFICIARIO(" +
            "#{CCODGRUPOCIA_IN}," + 
            "#{CCODLOCAL_IN}," + 
            "#{CSECUSULOCAL_IN}," + 
            "#{VCODCONVENIO_IN}," + 
    ...
    @Select(value="{call  " +
        "DELIVERY.PKG_DAEMON_UPD_PROF_FV.P_ACT_ENVIO_PROF(" +
            "#{a_cod_cia}," + 
            "#{a_cod_local}," + 
            "#{a_num_proforma}," + 
            "#{a_cod_local_sap}," + 
