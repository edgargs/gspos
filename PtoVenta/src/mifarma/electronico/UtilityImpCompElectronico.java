...
import printerStar.FarmaPrinterStarInterface;
...
public class UtilityImpCompElectronico {
    ...
    public UtilityImpCompElectronico() {
        super();
         
    }
    
    public boolean printDocumento(String pNumPedidoVta,String pSecCompPago) throws Exception{
        /*boolean pImpResult = printDocumentoMALO(pJDialog, pNumPedidoVta,pSecCompPago,pTipoOld,pTipoClienteConvenio ,pNumComprobante);
        FarmaUtility.showMessage(pJDialog, "Se imprimo algoritmo MALO de Impresion.", null);*/
        
        //printDocElectronico(pJDialog, pNumPedidoVta,pSecCompPago,pTipoOld,pTipoClienteConvenio ,pNumComprobante);
        String rutaFile = UtilityPtoVenta.obtieneDirectorioComprobantes();
        rutaFile = rutaFile+UtilityCaja.getNombreElectronico(FarmaVariables.vCodGrupoCia, FarmaVariables.vCodLocal, pNumPedidoVta, pSecCompPago);

        List listComanda = new ArrayList();
        listComanda =  DBComprobanteElectronico.getDatosImpresion(VariablesPtoVenta.vVersion, pNumPedidoVta, pSecCompPago);
        if (listComanda.size() > 0) {
            boolean rest = impresionDocumentoEnTermica(listComanda,VariablesPtoVenta.vTipoImpTermicaxIp, VariablesPtoVenta.vImpresoraActual.getName(), true, rutaFile);
            if(!rest){
                VariablesCaja.vEstadoSinComprobanteImpreso = "S";
            }else{
                VariablesCaja.vEstadoSinComprobanteImpreso = "N";
            }
            //FarmaUtility.showMessage(pJDialog, "Se imprimo NUEVO algoritmo de Impresion.", null);
        }else{
            VariablesCaja.vEstadoSinComprobanteImpreso = "S";
            throw new Exception("Alerta Local:\n" +
                    "La cadena para Impresión es vacía." +
    ...
    public boolean impresionDocumentoEnTermica(List listComanda, String tipoImpresora, String ruta, boolean isPonerLogo, String pRutaFile)throws Exception{
        String impresora = "";
        ...
        FarmaPrinterFacade printer=new FarmaPrinterFacade(tipoImpresora,ruta);//manda imprimir segun el modelo de impresora
        ...
        if(!printer.startPrintService()){
            ...
            if(isPonerLogo){
                String codMarca = DBComprobanteElectronico.getMarcaLocal(FarmaVariables.vCodGrupoCia, FarmaVariables.vCodLocal);
                if(codMarca == null){
            ...
            for (int i = 0; i < listComanda.size();i++) {
                ...
                if("-".equalsIgnoreCase(valor)){
                    ...
                    printer.printGeneral(valor, tamanio, alineacion, isNegrita);
                    if(pRutaFile!=null){
            ...
            //ABRIR GABETA
            if(tipoImpresora.equalsIgnoreCase(FarmaPrinterConstants.MODELOEPSON)) {
                boolean vImpr = imprimirTMU950DK(ruta);
            }
            if(tipoImpresora.equalsIgnoreCase(FarmaPrinterConstants.MODELOSTAR)) {
                boolean vImpr2 = imprimirTSP700DK(ruta);
            }
    ...
    public boolean  printDocumentoMALO(JDialog pJDialog, String pNumPedidoVta,String pSecCompPago,String pTipoOld,String pTipoClienteConvenio ,String pNumComprobante)
    throws Exception
    {
        ...
        if(pNumComprobante==null||pNumComprobante.trim().length()>0){
                ...
                try {
                    printer = new FarmaPrinterFacade(this.getModelo(), this.getRuta());
...
ruta + fechaImpresion + "_" + nomDocumento.substring(0, 1) + "_E_" + VariablesCaja.vNumPedVta + "_" +
 serieCorrelativo + ".TXT";
                    ...
                    if (!printer.startPrintService()) {
                        ...
                        printer.printLineCenter(razSocialEmisor + printer.printEspacioBlanco(1) + ConstantesDocElectronico.RUCEMISOR + rucEmisor);
                        vPrintArchivo.printLine(razSocialEmisor + printer.printEspacioBlanco(1) + ConstantesDocElectronico.RUCEMISOR + rucEmisor, true); //
                        ...
                        vPrintArchivo.printLine(ConstantesDocElectronico.FECHAEMISION + fecEmision,
                                                true); //
                        /**DOCUMENTO DE REFERENCIA*/
                        if (UtilityConvenioBTLMF.esActivoConvenioBTLMF(new JDialog(), null) &&
                            VariablesConvenioBTLMF.vHayDatosIngresadosConvenioBTLMF) {

                            if ((numCompCoPagoRef.trim() != ConstantesDocElectronico.CERO ||
                                 numCompCoPagoRef != null) && tipComPagoRef.equals(ConstantsPtoVenta.TIP_COMP_GUIA)) {
                        ...
                        if (tipPedVta.equals(ConstantesDocElectronico.TIPVTAINSTU)) {
                            if (!ordCompra.equals(ConstantesDocElectronico.CERO) && !"".equals(ordCompra)) {
                                ...
                                vPrintArchivo.printLine(ConstantesDocElectronico.ORDCOMP + ordCompra,
                                                        true); //
                            }
                        ...
                        if (tipCompPago.equals(ConstantesDocElectronico.TIPNOTACREDITO)) {
                            ...
                            vPrintArchivo.printLine(ConstantesDocElectronico.MOTIVO + sustAnulacion,
                                                    true); //
                        ...
                        if (this.getModelo().equalsIgnoreCase(FarmaPrinterConstants.MODELOSTAR)) {
                            printer.printLineBold(ConstantesDocElectronico.CODIGO + "    \t" + ConstantesDocElectronico.DESCRIPCION + "           \t" + ConstantesDocElectronico.CANTIDAD + "  \t" + ConstantesDocElectronico.PREUNIT + " \t" + ConstantesDocElectronico.DESCUENT + " \t" + ConstantesDocElectronico.IMPORTE);
                            vPrintArchivo.printLine(ConstantesDocElectronico.CODIGO + "    \t" + ConstantesDocElectronico.DESCRIPCION + "           \t" + ConstantesDocElectronico.CANTIDAD + "  \t" + ConstantesDocElectronico.PREUNIT + " \t" + ConstantesDocElectronico.DESCUENT + " \t" + ConstantesDocElectronico.IMPORTE,
                                                    true); //
                        } else {
                            printer.printLineBold(ConstantesDocElectronico.CODIGO + "   \t" + ConstantesDocElectronico.DESCRIPCION + "   \t" + ConstantesDocElectronico.CANTIDAD + "  " + ConstantesDocElectronico.PREUNIT + " " + ConstantesDocElectronico.DESCUENT + " " + ConstantesDocElectronico.IMPORTE);
                            vPrintArchivo.printLine(ConstantesDocElectronico.CODIGO + "   \t" + ConstantesDocElectronico.DESCRIPCION + "   \t" + ConstantesDocElectronico.CANTIDAD + "  " + ConstantesDocElectronico.PREUNIT + " " + ConstantesDocElectronico.DESCUENT + " " + ConstantesDocElectronico.IMPORTE,
                        ...
                        if (lstDetalle != null && lstDetalle.size() > 0) {
                            ...
                            if (indCoPagoCliente.trim().equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                                printer.printLine(printer.alineaDetalle(pCodServicio, pDescServicio, "1", monTot, ConstantesDocElectronico.CERODECIMAL, monTot));
                                vPrintArchivo.printLine(printer.alineaDetalle(pCodServicio, pDescServicio, "1", monTot, ConstantesDocElectronico.CERODECIMAL,
                                                                              monTot),
                                                        true); //
                            } else {
                                for (int i = 0; i < lstDetalle.size(); i++) {
                                    ...
                                    desProd =
                                            ((ArrayList)lstDetalle.get(i)).get(1).toString(); // descripcion del producto
                                    canProd =
                                            ((ArrayList)lstDetalle.get(i)).get(2).toString(); // cantidad del producto
                                    preUnit =
                                    ...
                                    printer.printLine(printer.alineaDetalle(codProd, desProd, canProd, preUnit,
                                                                            descItem, monItem));
                        ...
                        if (!indCoPagoEmpresa.trim().equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                            ...
                            if (!redond.equalsIgnoreCase(ConstantesDocElectronico.CERODECIMAL)) {
                                ...
                                vPrintArchivo.printLine(ConstantesDocElectronico.REDONDEO + ConstantesDocElectronico.SOLES + printer.alineaMontos(redond),
                                                        true);

                            }
                            printer.printLineRigth(ConstantesDocElectronico.TOTALPAGAR + ConstantesDocElectronico.SOLES + printer.alineaMontos(totalPagar));
                            vPrintArchivo.printLine(ConstantesDocElectronico.TOTALPAGAR + ConstantesDocElectronico.SOLES + printer.alineaMontos(totalPagar),
                                                    true);
                        }
                        ...
                        if (!opGrab.equalsIgnoreCase(ConstantesDocElectronico.CERODECIMAL)) {
                            printer.printLineRigth(ConstantesDocElectronico.OPGRABADAS + ConstantesDocElectronico.SOLES + printer.alineaMontos(opGrab));
                            vPrintArchivo.printLine(ConstantesDocElectronico.OPGRABADAS + ConstantesDocElectronico.SOLES + printer.alineaMontos(opGrab),
                                                    true);
                        }
                        if (!opInaf.equalsIgnoreCase(ConstantesDocElectronico.CERODECIMAL)) {
                            printer.printLineRigth(ConstantesDocElectronico.OPINAFECTAS + ConstantesDocElectronico.SOLES + printer.alineaMontos(opInaf));
                            vPrintArchivo.printLine(ConstantesDocElectronico.OPINAFECTAS + ConstantesDocElectronico.SOLES + printer.alineaMontos(opInaf),
                                                    true);
                        }
                        if (!opExon.equalsIgnoreCase(ConstantesDocElectronico.CERODECIMAL)) {
                            printer.printLineRigth(ConstantesDocElectronico.OPEXONERDAS + ConstantesDocElectronico.SOLES + printer.alineaMontos(opExon));
                            vPrintArchivo.printLine(ConstantesDocElectronico.OPEXONERDAS + ConstantesDocElectronico.SOLES + printer.alineaMontos(opExon),
                                                    true);
                        }
                        if (!opGrat.equalsIgnoreCase(ConstantesDocElectronico.CERODECIMAL)) {
                            printer.printLineRigth(ConstantesDocElectronico.OPGRATUITAS + ConstantesDocElectronico.SOLES + printer.alineaMontos(opGrat));
                            vPrintArchivo.printLine(ConstantesDocElectronico.OPGRATUITAS + ConstantesDocElectronico.SOLES + printer.alineaMontos(opGrat),
                                                    true);
                        }
                        ...
                        if (!monDes.equalsIgnoreCase(ConstantesDocElectronico.CERODECIMAL)) {
                            printer.printLineRigth(ConstantesDocElectronico.DESCUENTO + ConstantesDocElectronico.SOLES + printer.alineaMontos(monDes));
                            vPrintArchivo.printLine(ConstantesDocElectronico.DESCUENTO + ConstantesDocElectronico.SOLES + printer.alineaMontos(monDes),
                                                    true);
                        }
                        if (!monTot.equalsIgnoreCase(ConstantesDocElectronico.CERODECIMAL)) {
                            if (indCoPagoEmpresa.trim().equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                                printer.printLineRigth(ConstantesDocElectronico.IMP_TOTAL + ConstantesDocElectronico.SOLES + printer.alineaMontos(monTot));
                                vPrintArchivo.printLine(ConstantesDocElectronico.IMP_TOTAL + ConstantesDocElectronico.SOLES + printer.alineaMontos(monTot),
                                                        true);
                            } else {
                        ...
                        if (indCoPagoEmpresa.trim().equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                            ...
                            vPrintArchivo.printLine("--------------------------------------------------------------------",
                                                    true);
                            printer.printLineRigth(ConstantesDocElectronico.CO_PAGO.replace("#", pctoCoPagoEmpresa) + ConstantesDocElectronico.SOLES + printer.alineaMontos(valorCoPago));
                            vPrintArchivo.printLine(ConstantesDocElectronico.CO_PAGO.replace("#", pctoCoPagoEmpresa) + ConstantesDocElectronico.SOLES + printer.alineaMontos(valorCoPago),
                                                    true);
                            //CESAR HUANES 23/10/2014
            ...
            vPrintArchivo.printLine(ConstantesDocElectronico.REDONDEO+ConstantesDocElectronico.SOLES+printer.alineaMontos(redond),true);
                            ...
                            for (int i = 0; i < lstFormaPago.size(); i++) {
                                ...
                                if (indCoPagoEmpresa.trim().equalsIgnoreCase(FarmaConstants.INDICADOR_N) &&
                                    ...
                                    if (FarmaUtility.getDecimalNumber(montVuelto) > 0)
                                        printer.printLineRigth(ConstantesDocElectronico.VUELTO + ConstantesDocElectronico.SOLES +
                                    ...
                                    if (indCoPagoEmpresa.trim().equalsIgnoreCase(FarmaConstants.INDICADOR_N) &&
                                        ...
                                        if (FarmaUtility.getDecimalNumber(montVuelto) > 0) {
                                            printer.printLineRigth(ConstantesDocElectronico.VUELTO + ConstantesDocElectronico.SOLES +
                                                                   printer.alineaMontos(montVuelto));
                                            vPrintArchivo.printLine(ConstantesDocElectronico.VUELTO + ConstantesDocElectronico.SOLES +
                        ...
                        if (!tipCompPago.equalsIgnoreCase(ConstantesDocElectronico.NOTACREDITO))
                            if (!ahorro.equals(ConstantesDocElectronico.CERODECIMAL)) {
                                printer.printLineBold(ConstantesDocElectronico.AHORRO + "" + ConstantesDocElectronico.SOLES + ahorro);
                                vPrintArchivo.printLine(ConstantesDocElectronico.AHORRO + "" + ConstantesDocElectronico.SOLES + ahorro, true);
                            }
                        /**DATOS DE CONVENIO*/
                            ...
                            if (lstConvenio != null && lstConvenio.size() > 0) {
                                ...
                                if (tipCompPago.equalsIgnoreCase(ConstantesDocElectronico.BOLETA) ||
                                    ...
                                    if (!razSocialRecep.equals(ConstantesDocElectronico.CERO) &&
                                        ...
                                        vPrintArchivo.printLine(ConstantesDocElectronico.NOMBRECLIENTE +
                                                                razSocialRecep, true);
                                    }
                                ...
                                if (tipCompPago.equalsIgnoreCase(ConstantesDocElectronico.FACTUA) ||
                                    ...
                                    if (!razSocialRecep.equals(ConstantesDocElectronico.CERO) &&
                                        ...
                                        vPrintArchivo.printLine(ConstantesDocElectronico.RAZSOCIACLIENTE +
                                                                razSocialRecep, true);
                                    }
                                    if (!rucReceptor.equals(ConstantesDocElectronico.CERO) &&
                                        !"".equals(rucReceptor)) {
                                        printer.printLine(ConstantesDocElectronico.RUCCLIENTE + rucReceptor);
                                        vPrintArchivo.printLine(ConstantesDocElectronico.RUCCLIENTE + rucReceptor,
                                                                true);
                                    }
                                }
               ...
               vPrintArchivo.printLine(ConstantesDocElectronico.REFERENCIA+refDirDeli,true);
                                ...
                                if (tipCompPago.equalsIgnoreCase(ConstantesDocElectronico.BOLETA) ||
                                    ...
                                    if (montoBoleta >= ConstantesDocElectronico.IMPLIMITE) {
                                        if (!razCliDeli.equals(ConstantesDocElectronico.CERO) &&
                                            !"".equals(razCliDeli)) {
                                        ...
                                        if (diCliDeli != null && !"".equals(diCliDeli) &&
                                            ...
                                            printer.printLine(ConstantesDocElectronico.DIRCLIENTE + diCliDeli);
                                            vPrintArchivo.printLine(ConstantesDocElectronico.DIRCLIENTE + diCliDeli,
                                                                    true);
                                        }
                                    }
                                ...
                                if (tipCompPago.equalsIgnoreCase(ConstantesDocElectronico.FACTUA) ||
                                    ...
                                    if (rucCliDeli != null && !"".equals(rucCliDeli) &&
                                        ...
                                        vPrintArchivo.printLine(ConstantesDocElectronico.RUCCLIENTE + rucCliDeli,
                                                                true);
                                    }
                        ...
                        if (cantCupo != 0) {
                            if (cantCupo > 1) {
                                printer.printLineDotted(30);
                                printer.printLineBold(ConstantesDocElectronico.GANO + cantCupo + ConstantesDocElectronico.CUPONES);
                                vPrintArchivo.printLine("--------------------------------------------------------------------",
                                                        true);
                                vPrintArchivo.printLine(ConstantesDocElectronico.GANO + cantCupo + ConstantesDocElectronico.CUPONES, true);
                            } else {
                                printer.printLineBold(ConstantesDocElectronico.GANO + cantCupo + ConstantesDocElectronico.CUPON);
                                vPrintArchivo.printLine(ConstantesDocElectronico.GANO + cantCupo + ConstantesDocElectronico.CUPON, true);
                            }
                        }
                            ...
                            if (indCoPagoCliente.trim().equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                                ...
                                for (int i = 0; i < lstDetalle.size(); i++) {
                                    ...
                                    printer.printLine(printer.alineaDetalle(codProd, desProd, canProd, "", descItem,
                                                                            ""));
                                    vPrintArchivo.printLine(printer.alineaDetalle(codProd, desProd, canProd, "",
                                                                                  descItem, ""), true);
                                    indProVirtual =
                                            ((ArrayList)lstDetalle.get(i)).get(6).toString(); //Indicador de producto virtual
                                    tiProdVirtual =
    ...
    * */
     ...
     * 08/09/2014
     * */
    public void prinInforVirtual(FarmaPrinterFacade printer, String pTipoProdVirtual,
                                             String pCodigoAprobacion, String pNumeroTarjeta, String pNumeroPin,
                                             String pNumeroTelefono, String pMonto, String pNumPedido,String pCodProd,
    ...
    public void prinDatosCliente(FarmaPrinterFacade printer,FarmaPrintServiceTicket vPrintArchivo){
        ...
        vPrintArchivo.printLine(ConstantesDocElectronico.NOMBRE+"______________________________________________",true);
        vPrintArchivo.printLine(ConstantesDocElectronico.DNI+"_________________________________________________",true);
        vPrintArchivo.printLine(ConstantesDocElectronico.FIRMA+"_______________________________________________",true);
