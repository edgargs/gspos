...
public class DlgDatosImpresoras extends JDialog {
  ...
  private JButtonLabel btnNroComp = new JButtonLabel();
    ...
    private void initComboModelo(){        
        ...
        FarmaLoadCVL.loadCVLFromSP(cmbModelo, "cmbModelo","PTOVENTA_ADMIN_IMP.IMP_LISTA_MODELOS(?,?)", parametros, false);        
		...
		if (UtilityPtoVenta.verificaVK_F1(e)) // Reservado para ayuda
		{
                    ...
                    if (datosValidados()){
                        ...
                        if (VariablesImpresoras.vTipoComp.trim().equalsIgnoreCase(ConstantsPtoVenta.TIP_COMP_TICKET)){
                            ...
                            if(valor)
                            if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,"¿Está seguro que desea grabar los datos ?")) {
                                    try {
                                            if (existenDatos) {
                                                    actualizarImpresora();
                                            } else {
