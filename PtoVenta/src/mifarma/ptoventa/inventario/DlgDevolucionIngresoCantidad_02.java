...
*/
    ...
    private void initCabecera_02() {       
        ...
        */
        ...
        int cantidad = 0;
        if (VariablesInventario.vRecepTotal.trim().equals("") ||
            VariablesInventario.vRecepTotal.trim().equals("0")){
    ...
    private void aceptaCantidadIngresada_01(){
      if(validarCampos()){
        ...
        int cantidadIngresada = (int)Integer.parseInt(txtCantidad.getText()) + (int)Integer.parseInt(txtFraccion.getText());
        ...
        if (cantidadIngresada > cantidadExceso) {
           FarmaUtility.showMessage(this, 
                                    "La cantidad a devolver no puede ser mayor a " + cantidadExceso, 
                                    txtCantidad);
            return;
        }else if ((cantidadIngresada <= cantidadExceso) && (cantidadIngresada > stockDisponible)) {
           FarmaUtility.showMessage(this, 
                                    "Solo hay disponible en stock la cantidad de " + stockDisponible, 
                                    txtCantidad);
            return;
        }else{           
