    ...
    private void funcionVkEnter(){
        ...
        if (UtilityFidelizacion.validarDocIdentidad(txtBuscar.getText().trim())) {
            if (isSeleccionFormaPago()) {
                // KMONCADA 05.02.2015 
                if(UtilityPuntos.isActivoFuncionalidad() && !isVtaManual){
                    ...
                    if(consultarOrbis){
                        UtilityPuntos.consultarTarjetaOrbis(myParentFrame, this, txtBuscar, false, false);
                    }else{
                        try{
                            ...
                            if(FarmaPuntosConstante.NO_CONEXION_ORBIS.equalsIgnoreCase(tarjetaBean.getEstadoOperacion())){
                                ...
                                if( nroDocumento.trim().length() > 0 && !txtBuscar.getText().trim().equalsIgnoreCase(nroDocumento)){
