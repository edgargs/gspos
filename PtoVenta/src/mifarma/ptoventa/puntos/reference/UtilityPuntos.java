...
public class UtilityPuntos {
     ...
     * @throws Exception
     */
    public static int cantidadTieneTarjetasAdicionales(Frame myParentFrame, JDialog pJDialog, String nroDocumento, boolean solicitaValidaDocumento, boolean actualizaAfiliado)throws Exception{
        int cantidadTarjeta = -1;
        ...
        if(afiliado != null){
            if(afiliado.getDni().trim().length()==0){
                ...
                for(int i=0;i<lstTarjeta.size();i++){
                    ...
                    if(nroDocAux.trim().length()>0 && !afiliado.getDni().trim().equalsIgnoreCase(nroDocAux)){
                        contador++;
                        if(isBloqueAsociarTarjOrbis){
                            if(!isValidaTarjetaConvenio){
                                isValidaTarjetaConvenio = evaluaTarjeta(nroDocAux, ConstantsPuntos.TARJETA_PTOS_TIPO_TC);
                            }
                        }
                    }
                }
