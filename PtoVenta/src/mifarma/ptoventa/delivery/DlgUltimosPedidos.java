...
public class DlgUltimosPedidos extends JDialog {
  ...
  JLabelFunction lblF9 = new JLabelFunction();
    ...
    lblF5.setBounds(new Rectangle(225, 465, 140, 20));
...
// **************************************************************************
  ...
  private void cargaDatosCabecera()
  {
    ...
    private void generaPedidoDeliveryAutomatico()
    {
        ...
        try
        {
            ...
            if(!com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this, "Está seguro de generar el pedido?")) 
                return;
  ...
  public void actualizaListaPedidos()
  {
    cargaListaCabecera();
    cargaDatosCabecera();
  }
    ...
    for(int i=0; i<tblDetalle.getRowCount(); i++)
    {
      ...
      if( error )
      {
        codigoErrado = FarmaUtility.getValueFieldJTable(tblDetalle, i, 0);
          ...
          codigoErrado = codigoErrado+" : "+cantidadPedido+" "+unidadVenta;
        if(cadenaCodigos.length() == 0)
          cadenaCodigos.append(codigoErrado);
        else
