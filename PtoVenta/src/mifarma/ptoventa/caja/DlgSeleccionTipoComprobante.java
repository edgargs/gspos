...
public class DlgSeleccionTipoComprobante extends JDialog {
    ...
    private String tipCompSinRUC;
    private String tipCompConRUC;
    private boolean isSeleccionTipoClienteM = false;
    
    // KMONCADA 07.06.2016 [PROYECTO M]
    private ButtonGroup btngTipoDocumentos = new ButtonGroup();
    private BeanClienteJuridico beanClientePedido;
    ...
    private void jbInit() throws Exception {
        ...
        rbtFactura.addItemListener(new ItemListener() {
        ...
        rbtBoleta.addItemListener(new ItemListener() {
                public void itemStateChanged(ItemEvent e) {
                    cambiaTipoComprobante();
                }
            });
        ...
        btngTipoDocumentos.add(rbtBoleta);
        btngTipoDocumentos.add(rbtFactura);
    }
    ...
    private void this_windowOpened(WindowEvent e) {
        ...
        if (pTipoBoletaTicketPC.trim().equalsIgnoreCase(ConstantsVentas.TIPO_COMP_BOLETA)) {
            rbtBoleta.setText("BOLETA");
            tipCompSinRUC = ConstantsVentas.TIPO_COMP_BOLETA;
        }
        //si el tipo de comprobante configurado para la IP es un ticket, se mostrara dicho texto
            ...
            tipCompSinRUC = ConstantsVentas.TIPO_COMP_TICKET;
        }
        ...
        /* if (UtilityPtoVenta.isLocalTipoVentaM()){
            if (rbtBoleta.isSelected()){
                if (e.getKeyCode() == KeyEvent.VK_ENTER){
                    if (txtRazonSocial_Ruc.getText().trim().equals("")){
    ...
    private void chkKeyPressed(KeyEvent e) {
        if (UtilityPtoVenta.verificaVK_F2(e) && lblF2.isVisible()) {
            ...
            if(isSeleccionTipoClienteM){
                VariablesVentas.vTip_Comp_Ped = tipCompSinRUC;
                beanClientePedido = new BeanClienteJuridico();
                beanClientePedido.setIsClienteMayorista(false);
            }
            cerrarVentana(false);
        }
    }
     ...
     * 
     */
    private void cambiarSeleccionRadioBtn(){
        if (rbtBoleta.isSelected()) {
            rbtFactura.setSelected(true);
    ...
    private void cambiaTipoComprobante() {
        if (lblF2.isVisible()){
            if (rbtFactura.isSelected()) {
                VariablesVentas.vTip_Comp_Ped = tipCompConRUC;//ConstantsVentas.TIPO_COMP_FACTURA;
            } else if (rbtBoleta.isSelected()) {
                VariablesVentas.vTip_Comp_Ped = tipCompSinRUC;//setTipoComprobante(ConstantsVentas.TIPO_COMP_BOLETA);
            }
            limpiaDatosCliente();
            bloqueaDatosCliente();
    ...
    private void validaTextoIngresado(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_ENTER) {
            (new UtilityCliente()).buscarCliente(myParentFrame, this, txtRazonSocial_Ruc, rbtFactura.isSelected());
            if (FarmaVariables.vAceptar) {
                if (VariablesCliente.vArrayList_Cliente_Juridico.size() == 1) {
                    VariablesVentas.vCod_Cli_Local = ((String)((ArrayList)VariablesCliente.vArrayList_Cliente_Juridico.get(0)).get(1)).trim();
                    ...
                    if (UtilityPtoVenta.isLocalVentaMayorista()){
                        beanClientePedido = (BeanClienteJuridico)(((ArrayList)VariablesCliente.vArrayList_Cliente_Juridico.get(0)).get(4));
                        VariablesCliente.vDigemid = beanClientePedido.getClienteDigemid();
                    }
                }            
                FarmaVariables.vAceptar = false;
            }
                ...
                if(isNumero){
                    if(rbtFactura.isSelected()){
                        if(ConstantsCliente.RESULTADO_RUC_VALIDO.equalsIgnoreCase(DBCliente.verificaRucValido2(textoBusqueda))){
                            ...
                            FarmaUtility.showMessage(this, "Nro de RUC INVALIDO", txtRazonSocial_Ruc);
                        }
                    }else{
    ...
    private void buscaClienteJuridico(String pTipoBusqueda, String pBusqueda) {
        ...
        if (FarmaVariables.vAceptar) {
            if (VariablesCliente.vArrayList_Cliente_Juridico.size() == 1) {
                ...
                txtDireccionCliente.setText(((String)((ArrayList)VariablesCliente.vArrayList_Cliente_Juridico.get(0)).get(3)).trim());
                if (UtilityPtoVenta.isLocalVentaMayorista()){
                    VariablesCliente.vDigemid = ((String)((ArrayList)VariablesCliente.vArrayList_Cliente_Juridico.get(0)).get(4)).trim();
                }
            }            
            FarmaVariables.vAceptar = false;
        } */
    ...
    private void guardaValoresCliente() {
        //log.info("*********************** VariablesPtoVenta.vTipoLocalVenta: " + VariablesPtoVenta.vTipoLocalVenta);
        ...
        if (UtilityPtoVenta.isLocalVentaMayorista()){
            if ( rbtFactura.isSelected() && UtilityFidelizacion.validarRUC(VariablesVentas.vRuc_Cli_Ped)){
                beanClientePedido.setIsClienteMayorista(true);
            }else{
                beanClientePedido = new BeanClienteJuridico();
                beanClientePedido.setNumDocumento(VariablesVentas.vRuc_Cli_Ped);
                beanClientePedido.setRazonSocial(VariablesVentas.vNom_Cli_Ped);
                beanClientePedido.setDireccion(VariablesVentas.vDir_Cli_Ped);
                beanClientePedido.setIsClienteMayorista(false);
            }
        }
    }
    ...
    private boolean validaDatosCliente() {
        ...
        String nroDocCliente = txtRUC.getText().trim();
        String direccionCliente = txtDireccionCliente.getText().trim();
        
        if(rbtFactura.isSelected()){
            if(nombreCliente == null || (nombreCliente!=null && "".equalsIgnoreCase(nombreCliente))){
                FarmaUtility.showMessage(this, "Falta Razon Social del Cliente.Verifique!!!", txtCliente);
                return false;
            } else if(nroDocCliente == null || (nroDocCliente!=null && "".equalsIgnoreCase(nroDocCliente))){
...
"" : VariablesFidelizacion.vDniCliente.trim();
    ...
    //KMONCADA 20.11.2014 INDICADOR SI ACTIVA OPCION DE CAMBIO DE TIPO DE DOCUMENTO
    ...
    public void setIsSeleccionTipoClienteM(boolean isAtencionLocalM) {
        this.isSeleccionTipoClienteM = isAtencionLocalM;
    }

    public BeanClienteJuridico getBeanClientePedido() {
        return beanClientePedido;
    }
}