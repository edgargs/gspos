...
public class DlgListaIPSImpresora extends JDialog {
    ...
    private void initTable() {
        tableModel = 
                new FarmaTableModel(ConstantsImpresoras.columnsListaIPS, ConstantsImpresoras.defaultValuesListaIPS, 
                                    0);
    ...
    private void chkKeyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
            ...
            if (tblMaestro.getSelectedRow() >= 0) {
                if (!(FarmaUtility.findTextInJTable(tblMaestro, 
                                                    txtIP.getText().trim(), 0, 
                                                    1))) {
            ...
            if (tblMaestro.getSelectedRow() > -1) {
                VariablesImpresoras.vSecIP = 
                        tblMaestro.getValueAt(tblMaestro.getSelectedRow(), 
                                              0).toString().trim();
                log.debug("VariablesImpresoras.vSecIP-->" + 
                                   VariablesImpresoras.vSecIP);
                DlgListaImpresoraMaquina dlglista = 
                    new DlgListaImpresoraMaquina(this.myParentFrame, "", true);
                ...
                FarmaUtility.showMessage(this, "Debe seleccionar un registro.", 
                                         tblMaestro);
            ...
            if (tblMaestro.getRowCount() > 0) {
                if (tblMaestro.getSelectedRow() > -1) {
                    if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this, 
                                                       "Esta seguro de eliminar la IP seleccionada?")) {
                        eliminarIP();
                    }
                } else
                    FarmaUtility.showMessage(this, 
            ...
            if (txtIP.getText().trim().length() == 0) {
                FarmaUtility.showMessage(this, "Debe ingresar una IP válida.", 
                                         txtIP);
            } else {
                if (esUnaDireccionIPValida(txtIP.getText().trim())) {
                    if (validarPingIP(txtIP.getText().trim())) {
                        if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this, 
                                                           "Esta seguro de agregar la nueva IP?")) {
                            guardarIP();
                            cagarIPs();
                        }
                    } else {
                        FarmaUtility.showMessage(this, 
                                                 "La ip ingresada no esta disponible.", 
                                                 txtIP);
                    }
                } else {
                    FarmaUtility.showMessage(this, 
                                             "La ip ingresada no es válida. Verifique!!!", 
                                             txtIP);
                }
            }
        } else if (e.getKeyCode() == KeyEvent.VK_F3) //JCHAVEZ 25.06.09.sn
        {
            if (tblMaestro.getSelectedRow() > -1) {
                ...
                log.debug("VariablesImpresoras.vSecIP-->" + 
                                   VariablesImpresoras.vSecIP);
                DlgListaImpresoraTermica dlglista = 
                    new DlgListaImpresoraTermica(this.myParentFrame, "", true);
                dlglista.setVisible(true);
