...
public class DlgRecaudacionesRealizadas extends JDialog {
    ...
    public void reimprimirComprobante(){
            ...
            boolean rpta;
            if(bEstadoImp){
               rpta=com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,"Desea reimprimir el voucher?");
               if(rpta){
                   if(ConstantsRecaudacion.TIPO_COBRO_RECAUDACION.equals(tipoCobro)){
    ...
    */    
    private ArrayList verificarEstadoTrssc(){                
        ...
        boolean bRpt=false;
        String codTrsscSix = tableModel.getValueAt(tblCabeceraPedido.getSelectedRow(), COL_EST_TRSSC_SIX).toString().trim();
