...
--------------------------------------------------------
  ...
  BEGIN
    OPEN curConv FOR
		...
		SELECT m.cod_convenio || 'Ã' || ' '|| m.des_convenio|| 'Ã' ||
                NVL(m.cod_convenio_rel, 'N') || 'Ã' ||
                NVL(m.flg_creacion_cliente, 'N')
         FROM MAE_CONVENIO m WHERE m.flg_activo='1' and COD_CONVENIO = '0000000834'
		union
           SELECT m.cod_convenio || 'Ã' || DECODE(m.cod_tipo_convenio,2,'(%)'|| m.des_convenio,m.des_convenio)|| 'Ã' ||
                NVL(m.cod_convenio_rel, 'N') || 'Ã' ||
                NVL(m.flg_creacion_cliente, 'N')
         FROM MAE_CONVENIO m WHERE m.flg_activo='1' and COD_CONVENIO <> '0000000834'
  ...
  BEGIN
    ...
    IF flgDataRimac = '1' THEN
      ...
      OPEN curConv FOR
         SELECT
                BENIF.NOMBRES  "DES_NOMBRE"
           FROM V_CON_BENEFIC_RIMAC BENIF
    ...
    ELSE
      ...
      OPEN curConv FOR
        ...
        UNION
         SELECT
            BENIF.DNI "DNI",
            BENIF.DES_CLIENTE "DES_NOMBRE"
       FROM V_CON_BENEFICIARIO BENIF
      WHERE
            BENIF.DNI*1 > 1
        AND BENIF.DNI IS NOT NULL
        AND BENIF.COD_CONVENIO = vCodConvenio_in
        AND BENIF.DNI  = vCodBenif_in;
  ...
  BEGIN
    ...
    ELSE
        ...
        SELECT B.DNI             || 'Ã' ||
               B.DES_NOM_CLIENTE || ' ' ||
               B.DES_APE_CLIENTE || 'Ã' ||
               NVL(TO_CHAR(B.LINEA_CREDITO),'0')   || 'Ã' ||
               'A'         || 'Ã' ||
               ' ' || 'Ã' ||
               ' ' || 'Ã' ||
               ' ' || 'Ã' ||
...
/*SELECT COD_CONVENIO,
       ...
       estado,
       cod_cliente_sap
  FROM V_CON_BENEF_TOTAL;
*/
FROM (
SELECT TT.COD_CONVENIO,
       TT.cod_cliente,
       TT.des_cliente,
       TT.flg_activo,
       TT.dni,
       ...
       TT.num_contrato,
       TT.tipo_seguro,
       TT.imp_linea_credito,
       TT.estado,
       TT.cod_cliente_sap
  FROM V_CON_BENEF_TOTAL TT
WHERE  (TT.COD_CONVENIO,TT.COD_CLIENTE) IN 
                                            (
                                            SELECT  T.COD_CONVENIO,T.COD_CLIENTE
                                            FROM    V_CON_BENEF_TOTAL T
                                            WHERE   T.COD_CONVENIO = vCodConvenio_in
                                            MINUS  
                                            SELECT  P.COD_CONVENIO,P.COD_CLIENTE
                                            FROM    V_CON_BENEF_PARCIAL   P
                                            WHERE   P.COD_CONVENIO = vCodConvenio_in
                                            )
UNION
SELECT  PP.COD_CONVENIO,
       PP.cod_cliente,
       PP.des_cliente,
       PP.flg_activo,
       PP.dni,
       ...
       PP.num_contrato,
       PP.tipo_seguro,
       PP.imp_linea_credito,
       PP.estado,
       PP.cod_cliente_sap
FROM    V_CON_BENEF_PARCIAL   PP
WHERE   PP.COD_CONVENIO = vCodConvenio_in) BENIF
      ...
      WHERE
         ...
         --BENIF.DNI*1 > 1
         --AND
         --BENIF.DNI IS NOT NULL
           BENIF.COD_CONVENIO = vCodConvenio_in
  ...
  BEGIN
    OPEN curDiagno FOR
      SELECT d.COD_CIE_10, d.DES_DIAGNOSTICO
        FROM MAE_DIAGNOSTICO d
       WHERE d.flg_activo = '1'
  ...
  BEGIN
    ...
    IF vFlg_retencion_in = '0' THEN
      ...
      v_htmlDerecho := '<P>' || '<html>' || '  <table width="' ||
  ...
  BEGIN
    ...
    OPEN curBenif FOR
        ...
        SELECT B.COD_CLIENTE,
               B.DNI,
               B.DES_NOM_CLIENTE ||
               B.DES_APE_CLIENTE DES_NOM_CLIENTE,
               ' ' DES_APE_CLIENTE,
               NVL(TO_CHAR(B.LINEA_CREDITO),'0') LCREDITO,
               'A' ESTADO,
               ' ' num_poliza,
               ' ' num_plan,
               ' ' cod_asegurado,
         ...
         WHERE B.CIA = cCodGrupoCia_in
           AND B.COD_LOCAL = cCodLocal_in
           AND B.COD_CONVENIO = cCodConvenio
           AND B.COD_CLIENTE = pCodCliente
           AND B.FLG_CREACION = 'C'
           AND TO_CHAR(B.FEC_CREACION,'DD/MM/YYYY') = TO_CHAR(SYSDATE,'DD/MM/YYYY')
      UNION
    ...
    <tbody>
        <tr>
            ...
            <table width="293" height="128" cellspacing="1" cellpadding="1" border="0" align="" summary="">
                <tbody>
                    <tr>
                        <td align="center" style="font-family: Arial, Verdana,Sans-Serif" colspan="4">
                        <h5><strong>' ||
  ...
  BEGIN
   ...
   /*     vMensaje_uno := vMensaje_uno || '     <tr>';
        ...
        vMensaje_dos := vMensaje_dos ||'    <td style="font-family: Arial, Verdana" colspan="4"> ';
         ...
         SELECT SUM(COMP.VAL_NETO_COMP_PAGO)
          ...
          --MONTO CREDITO
           IF vCodTipoConvenio = '1' THEN
              vMsj_vtas_voucher_dos := vMsj_vtas_voucher_dos ||'<tr>';
              vMsj_vtas_voucher_dos := vMsj_vtas_voucher_dos ||'    <td align = "left" style="font-family: Arial, Verdana" colspan="4">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font size="4"><strong>'||vDocBenif||'  S/.'||TO_CHAR(vMontoTotalBenif,'99,990.00')||'</strong></font></td>';
              vMsj_vtas_voucher_dos := vMsj_vtas_voucher_dos ||'</tr>';

              vMsj_vtas_voucher_dos := vMsj_vtas_voucher_dos ||'<tr>';
              vMsj_vtas_voucher_dos := vMsj_vtas_voucher_dos ||'    <td align = "left" style="font-family: Arial, Verdana" colspan="4">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font size="4"><strong>'||vDocEmpre||'  S/.'||TO_CHAR(vMontoTotalEmpre,'99,990.00')||'</strong></font></td>';
              vMsj_vtas_voucher_dos := vMsj_vtas_voucher_dos ||'</tr>';
           ELSE
          ...
          vMsj_vtas_voucher_dos := vMsj_vtas_voucher_dos ||
                           '   <td align = "center" colspan="4" style="font-family: Arial, Verdana"><strong><h5>*** Documentos Sin Valor Fiscal ***</h5></strong></td>';
          vMsj_vtas_voucher_dos := vMsj_vtas_voucher_dos || '  </tr>';
    /*      vMsj_vtas_voucher_dos := vMsj_vtas_voucher_dos || '  <tr>';
 ...
 --1.Validaciones para el realizar el cobro pedido
  BEGIN
   SELECT VTA.COD_CONVENIO,VTA.VAL_NETO_PED_VTA
    ...
    WHERE VTA.NUM_PED_VTA = cNumPedVta_in;
  EXCEPTION
...
PROCEDURE BTLMF_P_INSERT_COMP_PAGO(cCodGrupoCia_in IN CHAR,
 ...
 dbms_output.put_line('PORC_IGV_COMP_PAGO     = '||PPORC_IGV_COMP_PAGO);
    ...
    INSERT INTO VTA_COMP_PAGO(COD_GRUPO_CIA,
          ...
          EXCEPTION
          WHEN OTHERS THEN
          RAISE_APPLICATION_ERROR(-20999,'ERROR AL INSERTAR BTLMF_P_INSERT_COMP_PAGO.' ||SQLERRM);
      ...
      BEGIN
        ...
        OPEN curComp FOR
          SELECT P.NUM_COMP_PAGO || 'Ã' ||
                 ...
                 NVL(TO_CHAR(P.VAL_REDONDEO_COMP_PAGO,'999,990.00'),0)   VAL_REDONDEO_COMP_PAGO 
            FROM VTA_COMP_PAGO P,
...
FUNCTION BTLMF_F_CHAR_ES_DIA_VIG_RECTA(fechaReceta_in IN varchar2)
    ...
    IF  vDiaVigencia < vLimiteDiaVigencia or vDiaVigencia = vLimiteDiaVigencia THEN
        vResp := 'S' ;
    END IF;
...
FUNCTION BTLMF_F_VARCHAR_MSG_COMP(cCodGrupoCia_in varchar2,cCodConvenio varchar2,  montoTotalPedido varchar2 )
    RETURN varchar2 IS
    vResp varchar2(3000);
    vPct_beneficiario mae_convenio.pct_beneficiario%TYPE;
    vPct_empresa      mae_convenio.pct_empresa%TYPE;
...
BEGIN
           /*BEGIN            
                SELECT 'S'--DISTINCT DECODE(MC.FLG_VALIDA_LINCRE_BENEF,'1','S','N')
                INTO  vIndCompCredito
                FROM VTA_PEDIDO_VTA_CAB COMP INNER JOIN MAE_CONVENIO MC
                ON COMP.COD_CONVENIO=MC.COD_CONVENIO
                WHERE COMP.COD_GRUPO_CIA = cCodCia_in
                AND COMP.COD_LOCAL     = cCodLocal_in
                AND COMP.NUM_PED_VTA   = cNroPedido_in;
           EXCEPTION
                    WHEN NO_DATA_FOUND THEN
                                           vIndCompCredito := 'N';
           END;*/
