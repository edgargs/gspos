  ...
  BEGIN
    IF cIndVerificaSug = 'N' THEN
      ...
      SELECT (PROD_LOCAL.STK_FISICO) || 'Ã' ||
           ...
           ' ' || 'Ã' ||
           ' ' || 'Ã' ||
           PROD_LOCAL.EST_PROD_LOC -- kmoncada 02.07.2014 para obtener esl estado del producto en el local
      FROM LGT_PROD PROD,
           LGT_PROD_LOCAL PROD_LOCAL
      WHERE  PROD_LOCAL.COD_GRUPO_CIA = cCodGrupoCia_in
      AND     PROD_LOCAL.COD_LOCAL = cCodLocal_in
...
/* *********************************************************** */
  ...
  BEGIN
    ...
    SELECT --C.NUM_PED_VTA||';'|| to_char(C.VAL_NETO_PED_VTA,'9999999.00')
           -- LLEIVA 01.04.2014
           -- SE AUMENTO EL CAMPO DE FECHA PARA ENVIAR AL METODO DE RECARGA DE MATRIZ
           C.NUM_PED_VTA ||';'||
