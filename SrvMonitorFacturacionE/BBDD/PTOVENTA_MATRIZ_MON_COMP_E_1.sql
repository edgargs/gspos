...
--------------------------------------------------------
  ...
  BEGIN
    INSERT INTO MON_VTA_COMP_PAGO_E(COD_GRUPO_CIA,
	  ...
	  --ERIOS 02.02.2015 No consulta Boletas
      CASE WHEN C.TIP_COMP_PAGO = '01' THEN 'C'
	  ELSE 'R' END,
	  'JOB_CARGA_COM_E',      
      ...
      ELSE 'S' END,	  
	  CASE WHEN C.TIP_COMP_PAGO = '01' THEN '0'
	  ELSE NULL END
  ...
  BEGIN
	...
	FROM VTA_COMP_PAGO C
	  ...
	  END IF;
	  --ERIOS 29.01.2015
	  RECUPERA_COMP_E_PENDIENTE(cCodGrupoCia_in, cCodCia_in, loc.COD_LOCAL);
	END LOOP;
  END;
