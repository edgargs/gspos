CREATE OR REPLACE PACKAGE PTOVENTA.FARMA_LEALTAD AS
  ...
  FUNCTION GET_USO_NCR(cCodGrupoCia_in IN CHAR, cCodCia_in IN CHAR, cCodLocal_in IN CHAR, cNumPedVta_in IN CHAR)
  RETURN VARCHAR2;
...
--           ' '||'@'||
  ...
  BEGIN
	...
	(SELECT COUNT(1)
		...
		AND ROWNUM = 1)||'@'||
   TRIM(TO_CHAR((SELECT SUM(NVL(PTOS_AHORRO,0))
		FROM VTA_PEDIDO_VTA_DET DET
		WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
		AND DET.COD_LOCAL = cCodLocal_in
		AND DET.NUM_PED_VTA = cNumPedVta_in) ,'999999990.00'))
			INTO vRetorno
	from VTA_PEDIDO_VTA_CAB CAB JOIN PBL_USU_LOCAL USL ON (USL.COD_GRUPO_CIA = CAB.COD_GRUPO_CIA
							AND USL.COD_LOCAL = CAB.COD_LOCAL
							AND USL.SEC_USU_LOCAL = CAB.SEC_USU_LOCAL)
  ...
  BEGIN
    ...
    SET DET.FACTOR_PUNTOS = DET.VAL_PREC_TOTAL/v_nValNetoPedVta
    WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
    AND DET.COD_LOCAL = cCodLocal_in
    AND DET.NUM_PED_VTA = cNumPedVta_in;
    ...
    SET DET.AHORRO_PUNTOS = CASE 
                              WHEN vRedimeTodo = 'S' THEN
                                VAL_PREC_TOTAL
                              ELSE  
                                nImTotalPago_in*DET.FACTOR_PUNTOS
                            END
    WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
    AND DET.COD_LOCAL = cCodLocal_in
    AND DET.NUM_PED_VTA = cNumPedVta_in;
