CREATE OR REPLACE PACKAGE "PTOVENTA_VTA" AS
  ...
  FUNCTION VTA_OBTIENE_INFO_PROD(cCodGrupoCia_in IN CHAR,
                                 ...
                                 cTipoCliente_in IN CHAR DEFAULT PTOVENTA.PTOVENTA_VTA_MAYORISTA.TIPO_CLI_FINAL)
  RETURN FarmaCursor;
...
*/
  ...
  PROCEDURE GRABA_CUPON_PEDIDO_USO(cCodGrupoCia_in IN CHAR, 
                                    cCodLocal_in IN CHAR,
                                    cNumPedVta_in IN CHAR, 
                                    cCodCupon_in IN CHAR, 
                                    cCodCamp_in IN CHAR, 
...
--Fecha:       06/08/2014
  ...
  FUNCTION F_GET_STOCK_PRODUCTO(cCodGrupoCia_in    IN CHAR,
                                cCodLocal_in       IN CHAR,
                                cCodProd_in        IN CHAR,
                                cIndVerificaSug    IN CHAR)
  RETURN NUMBER;
  /**************************************************************************************************************************/
...
AND     TO_DATE( TO_CHAR(P.FEC_PROMOCION_FIN,'dd/MM/YYYY') || ' 23:59:59','dd/MM/yyyy HH24:mi:ss')
  ...
  IS
    curVta FarmaCursor;
    vValPrecioVta NUMBER(8,3);
  BEGIN
    
    vValPrecioVta := PTOVENTA.PTOVENTA_VTA_MAYORISTA.VTA_OBTIENE_VAL_PREC_VTA(cCodGrupoCia_in => cCodGrupoCia_in,
                                                                              cCodLocal_in => cCodLocal_in,
                                                                              cCodProd_in => cCodProd_in,
                                                                              cTipoCliente_in => cTipoCliente_in);
    
    IF cIndVerificaSug = 'N' THEN
      OPEN curVta FOR
      SELECT 
        CASE
          -- KMONCADA 11.01.2016 DETALL DE PRECIOS SEGUN FRACCIONAMIENTO ENTERO
          WHEN LOCAL.COD_TIP_LOCAL_VENTA = PKG_CONSTANTES.LOCAL_TIPO_VTA_M THEN
             --(PROD_LOCAL.STK_FISICO) || 'Ã' ||
             F_GET_STOCK_PRODUCTO(cCodGrupoCia_in, cCodLocal_in, cCodProd_in, cIndVerificaSug)|| 'Ã' ||
             PROD.DESC_UNID_PRESENT || 'Ã' ||
             PKG_CONSTANTES.VAL_FRAC_ENTERO || 'Ã' ||
             TO_CHAR(VTA_F_CHAR_PREC_REDONDEADO((vValPrecioVta/*PROD_LOCAL.VAL_PREC_VTA*/ * PROD_LOCAL.VAL_FRAC_LOCAL)),'999,990.000') || 'Ã' ||
             PROD_LOCAL.IND_PROD_CONG || 'Ã' ||
             TO_CHAR(VTA_F_CHAR_PREC_REDONDEADO((vValPrecioVta/*PROD_LOCAL.VAL_PREC_VTA*/ * PROD_LOCAL.VAL_FRAC_LOCAL)) ,'999,990.000') || 'Ã' ||
             PROD.DESC_UNID_PRESENT || 'Ã' ||
      ...
      AND     PROD.COD_GRUPO_CIA = PROD_LOCAL.COD_GRUPO_CIA
      AND     PROD.COD_PROD = PROD_LOCAL.COD_PROD
      AND     PROD_LOCAL.COD_GRUPO_CIA = LOCAL.COD_GRUPO_CIA
      AND     PROD_LOCAL.COD_LOCAL = LOCAL.COD_LOCAL
      ;
    ELSE --ERIOS 29/05/2008 Se muestran los datos de venta sugerida
      ...
      SELECT
        CASE 
          -- KMONCADA 11.01.2016 DETALL DE PRECIOS SEGUN FRACCIONAMIENTO ENTERO
          WHEN LOCAL.COD_TIP_LOCAL_VENTA = PKG_CONSTANTES.LOCAL_TIPO_VTA_M THEN
               -- 0 STOCK SIEMPRE EN UNIDAD DE PRESENTACION.
               TRUNC(F_GET_STOCK_PRODUCTO(cCodGrupoCia_in, cCodLocal_in, cCodProd_in, cIndVerificaSug))|| 'Ã' ||
               --TRUNC((PROD_LOCAL.STK_FISICO/PROD_LOCAL.VAL_FRAC_LOCAL))|| 'Ã' ||
               -- 1 DESCRIPCION DE PRODUCTO
               PROD.DESC_UNID_PRESENT || 'Ã' ||
               -- 2 FRACCIONAMIENTO PARA CASO DE LOCAL M SIEMPRE ES ENTERO
               PKG_CONSTANTES.VAL_FRAC_ENTERO || 'Ã' ||
               -- 3 PRECIO DE PRESENTACION DE PRODUCTO (UNIDAD ENTERA)
               TO_CHAR(VTA_F_CHAR_PREC_REDONDEADO( ( CEIL((vValPrecioVta/*PROD_LOCAL.VAL_PREC_VTA*/ * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/100 +
                   CASE WHEN (CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10)-TRUNC(CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10) = 0.0 THEN 0.0
                        WHEN (CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10)-TRUNC(CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10) <= 0.5 THEN
                             (0.5 -( (CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10)-TRUNC(CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10) ))/10
                        ELSE (1.0 -( (CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10)-TRUNC(CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10) ))/10 END )) ,'999,990.000') || 'Ã' ||
               -- 4
               PROD_LOCAL.IND_PROD_CONG || 'Ã' ||
               -- 5
               TO_CHAR(VTA_F_CHAR_PREC_REDONDEADO( ( CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/100 +
                   CASE WHEN (CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10)-TRUNC(CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10) = 0.0 THEN 0.0
                        WHEN (CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10)-TRUNC(CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10) <= 0.5 THEN
                             (0.5 -( (CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10)-TRUNC(CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10) ))/10
                        ELSE (1.0 -( (CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10)-TRUNC(CEIL((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL)*100)/10) ))/10 END )) ,'999,990.000') || 'Ã' ||
               ...
               -- 11
               CASE 
                 WHEN PROD_LOCAL.IND_PROD_FRACCIONADO = 'S' AND PROD_LOCAL.VAL_FRAC_LOCAL > 1 THEN
                   TO_CHAR((  (PROD_LOCAL.STK_FISICO)-
                              (( TRUNC(((PROD_LOCAL.STK_FISICO) * PKG_CONSTANTES.VAL_FRAC_ENTERO)/ PROD_LOCAL.VAL_FRAC_LOCAL) * 
                                 PROD_LOCAL.VAL_FRAC_LOCAL) / PKG_CONSTANTES.VAL_FRAC_ENTERO ) 
                           ),'9990' )
                 ELSE
                   ' '
                 END || 'Ã' ||
               ...
               -- 13
               PROD_LOCAL.EST_PROD_LOC  || 'Ã' ||
               -- 14
               nvl(PROD.IND_TIPO_CONSUMO,'-')   
        WHEN PROD.VAL_FRAC_VTA_SUG IS NOT NULL AND PROD_LOCAL.IND_PROD_FRACCIONADO = 'S' AND
  ...
  IS
    curVta FarmaCursor;
    vValPrecioVta NUMBER(8,3);
    vTipoLocalVenta PTOVENTA.PBL_LOCAL.COD_TIP_LOCAL_VENTA%TYPE;
  BEGIN
    
    SELECT L.COD_TIP_LOCAL_VENTA
    INTO vTipoLocalVenta
    FROM PTOVENTA.PBL_LOCAL L
    WHERE L.COD_GRUPO_CIA = cCodGrupoCia_in
    AND L.COD_LOCAL = cCodLocal_in;
    
    IF vTipoLocalVenta = PKG_CONSTANTES.LOCAL_TIPO_VTA_M THEN
      vValPrecioVta := PTOVENTA.PTOVENTA_VTA_MAYORISTA.VTA_OBTIENE_VAL_PREC_VTA(cCodGrupoCia_in => cCodGrupoCia_in,
                                                                              cCodLocal_in => cCodLocal_in,
                                                                              cCodProd_in => cCodProd_in,
                                                                              cTipoCliente_in => cTipoCliente_in);
      ...
      SELECT
        -- 0
        --KMONCADA 23.03.2016 OBTIENE STOCK SOLO DE LOS LOTES DISPONIBLES
        F_GET_STOCK_PRODUCTO(cCodGrupoCia_in, cCodLocal_in, cCodProd_in, cIndVerificaSug)
        /*CASE 
          WHEN cIndVerificaSug = 'S' THEN
            TRUNC(PROD_LOCAL.STK_FISICO/PROD_LOCAL.VAL_FRAC_LOCAL)
          ELSE
        ...
        CASE 
          WHEN cIndVerificaSug = 'S' THEN
            TO_CHAR(vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL,'999,990.000') 
          ELSE
            TO_CHAR(vValPrecioVta,'999,990.000')
        END || 'Ã' || --ERIOS 18/06/2008
        -- 4
        DECODE(cIndVerificaSug,'S',PROD.DESC_UNID_PRESENT,PROD_LOCAL.UNID_VTA) || 'Ã' ||
        -- 5
        '-' || 'Ã' ||
        -- 6
        CASE 
          WHEN cIndVerificaSug = 'N' THEN
            '0' 
          ELSE 
            TO_CHAR(NVL(PROD_LOCAL.PORC_DCTO_1,0),'990.000') 
        END || 'Ã' ||
        -- 7
        ...
        CASE 
          WHEN cIndVerificaSug = 'S' THEN
            TO_CHAR(((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL) *(1+LOC.PORC_DSCTO_CASTIGO/100)),'990.000') 
          ELSE
            TO_CHAR(((vValPrecioVta) *(1+LOC.PORC_DSCTO_CASTIGO/100)),'990.000')
        END || 'Ã' ||--por si acaso
        -- 9
        DECODE(cIndVerificaSug,'S',PKG_CONSTANTES.VAL_FRAC_ENTERO, PROD_LOCAL.VAL_FRAC_LOCAL)|| 'Ã' ||
        -- 10
...
PROCEDURE VTA_GRABAR_PEDIDO_VTA_CAB(cCodGrupoCia_in         IN CHAR,
  ...
  BEGIN
   ...
   -- JCHAVEZ 23092009.sn
    ...
    -- KMONCADA 2016.01.21 GENERA PROFORMAS
    IF cGeneraProforma_in = 'S' THEN
      -- JCHAVEZ 23092009.en
      INSERT INTO TMP_VTA_PEDIDO_VTA_CAB (COD_GRUPO_CIA,
                                          COD_LOCAL,
                                      NUM_PED_VTA,
                                      COD_CLI_LOCAL,
                                      SEC_MOV_CAJA,
  ...
  IF nCantCampFid > 0 then
    ...
    v_cSecuencial := PTOVENTA_CUPON.F_GET_SECUENCIAL_CUPON(cCodCupon_in); --SUBSTR(cCodCupon_in,9,5);
        ...
        IF v_cEstadoCupon = 'A' THEN
          -- KMONCADA 22.01.2016 PROFORMAS PARA LOCALES DE VENTA M
           ...
           ELSE
             INSERT INTO TMP_VTA_CAMP_PEDIDO_CUPON(COD_GRUPO_CIA,COD_LOCAL,COD_CUPON,
                                               NUM_PED_VTA,ESTADO,USU_CREA_CUPON_PED,COD_CAMP_CUPON,IND_USO)
             VALUES(cCodGrupoCia_in,cCodLocal_in,cCodCupon_in,
             cNumPedVta_in,'S',vIdUsu_in,cCodCamp_in,vIndUso_in);
           END IF;
 ...
 /* FUNCTION VTA_F_CUR_OBT_DCTO_PROD(cCodGrupoCia_in IN CHAR,
  ...
  PROCEDURE VTA_P_VALIDAR_VALOR_VTA(cCodGrupoCia_in IN CHAR,
    ...
    IS
        ...
        nPctoMin_campana number;--clarico 15.02.2016        
    BEGIN
     ...
     DBMS_OUTPUT.put_line('cIndicador:'||cIndicador);
      ...
      IF cIndicador = 'N' THEN--QUIER DECIR QUE NO DEBE PERMITIR VENDER POR DEBAJO DEL COSTO PROMEDIO
        ...
        --clarico 15.02.2016 - valida descuento minimo
            nAhorro_calculado := nPrecioVta_in - nPreciofinal;
            nPcto_ahorro := nAhorro_calculado*100/nPrecioVta_in;
            
            SELECT nvl(nvl(p.valor_dcto_min,c.valor_dcto_min),-999)
            INTO   nPctoMin_campana
            FROM VTA_CAMPANA_CUPON C,
                 vta_campana_prod_uso p
            WHERE C.COD_GRUPO_CIA = cCodGrupoCia_in
            AND   C.COD_CAMP_CUPON = cCodCamp_in
            ...
            if nPctoMin_campana <> -999 and nPcto_ahorro < nPctoMin_campana then
            -- es menor que el minimo x tanto calculara con el valor del minimo
              nPreciofinal:= round(nPrecioVta_in*(100-nPctoMin_campana)/100,3);
            end if;
         --fin clarico 15.02.2016
...
/***************************************************************************************************************************/
  ...
  BEGIN
       --HALLO CODIGO DE PRODUCTO, FRACCIONAMIENTO Y DESCRIPCION DE UNIDAD DE ADICIONALES
        ...
        -- KMONCADA 14.01.2015 CAMBIO PARA LOCAL DE TIPO M
        SELECT CASE 
                 WHEN L.COD_TIP_LOCAL_VENTA = PKG_CONSTANTES.LOCAL_TIPO_VTA_M THEN 
                   PKG_CONSTANTES.VAL_FRAC_ENTERO
                 ELSE
                   P.VAL_FRAC_LOCAL
               END, 
               CASE 
                 WHEN L.COD_TIP_LOCAL_VENTA = PKG_CONSTANTES.LOCAL_TIPO_VTA_M THEN 
                   ROUND(P.VAL_PREC_VTA * P.VAL_FRAC_LOCAL,3)
                 ELSE
                   P.VAL_PREC_VTA
               END,
               CASE 
                 WHEN L.COD_TIP_LOCAL_VENTA = PKG_CONSTANTES.LOCAL_TIPO_VTA_M THEN 
                   TRUNC(p.stk_fisico / P.VAL_FRAC_LOCAL)
                 ELSE
                   p.stk_fisico
               END,
               P.VAL_FRAC_LOCAL
  ...
  FUNCTION F_GET_STOCK_PRODUCTO(cCodGrupoCia_in    IN CHAR,
                                ...
                                cIndVerificaSug    IN CHAR)
    RETURN NUMBER IS
    vCantidad LGT_PROD_LOCAL.STK_FISICO%TYPE;
  BEGIN
    IF FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in, cCodLocal_in) = 'S' THEN
      
      SELECT CASE 
               WHEN cIndVerificaSug = 'N' THEN
                 NVL(SUM(LOTE.STK_FISICO),0)
               ELSE 
                 NVL(SUM(LOTE.STK_FISICO),0)/PROD.VAL_FRAC_LOCAL
               END 
      INTO vCantidad
      FROM LGT_PROD_LOCAL PROD
           LEFT OUTER JOIN LGT_PROD_LOCAL_LOTE LOTE 
                ON  PROD.COD_GRUPO_CIA = LOTE.COD_GRUPO_CIA 
      ...
      WHERE PROD.COD_GRUPO_CIA = cCodGrupoCia_in
      AND PROD.COD_LOCAL = cCodLocal_in
      AND PROD.COD_PROD = cCodProd_in
      /*AND PROD.COD_GRUPO_CIA = LOTE.COD_GRUPO_CIA
      AND PROD.COD_LOCAL = LOTE.COD_LOCAL
      AND PROD.COD_PROD = LOTE.COD_PROD
      AND LOTE.LOTE NOT IN
                    (SELECT LLAVE_TAB_GRAL 
                     FROM PBL_TAB_GRAL 
                     WHERE ID_TAB_GRAL = '597')*/
      GROUP BY PROD.VAL_FRAC_LOCAL;
    ELSE 
      SELECT PROD.STK_FISICO
      INTO vCantidad
      FROM LGT_PROD_LOCAL PROD
      WHERE PROD.COD_GRUPO_CIA = cCodGrupoCia_in
      AND PROD.COD_LOCAL = cCodLocal_in
