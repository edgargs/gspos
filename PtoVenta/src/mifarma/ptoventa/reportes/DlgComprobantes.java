        ...
        if (e.getKeyCode() != KeyEvent.VK_ESCAPE) {
            if (tblComprobantes.getRowCount() > 0) {
                VariablesReporte.vNComprobante =
                        ((String)tblComprobantes.getValueAt(tblComprobantes.getSelectedRow(), 1)).trim();
                log.debug("variables reporte vNComprobante" + VariablesReporte.vNComprobante);
                cargaDetalleComprobantesVenta();

                if (e.getKeyCode() == KeyEvent.VK_F11 && lblReimprimir.isVisible()) {
                    if (cargarLogeoAdmLocal()) {

                        String nroPedidoVta = VariablesReporte.vCorrelativo;
                        String secCompPago =
                            (String)tableModelComprobantes.getValueAt(tblComprobantes.getSelectedRow(), 11);
    ...
    private void cargaDetalleComprobantesVenta() {
        try {
            ...
            DBReportes.obtieneComprobantes_Venta_Detalle(tableModelDetalleComprobantes, VariablesReporte.vNComprobante,
                                                         VariablesReporte.vCorrelativo, tipcCompr);
            FarmaUtility.ordenar(tblDetalleComprobantes, tableModelDetalleComprobantes, 0,
                                 FarmaConstants.ORDEN_ASCENDENTE);
            // KMONCADA 15.12.2014
            String flgCompElectronico =
                ((String)tblComprobantes.getValueAt(tblComprobantes.getSelectedRow(), 12)).trim();
