 ...
 *
 */
public class DlgProductosInvDiario extends JDialog {
    /* ************************************************************************ */
    ...
    private void initTableListaProductos() {
        tableModelProductos =
                new FarmaTableModel(ConstantsInvDiario.columnsListaProductosInv, ConstantsInvDiario.defaultValuesListaProductosInv,
                                    0);
    ...
    private void cargaListaProductos() {
        try {
            ...
            if (tblRelacionProductos.getRowCount() > 0) {
                FarmaUtility.ordenar(tblRelacionProductos, tableModelProductos, 2, FarmaConstants.ORDEN_ASCENDENTE);
            }
    ...
    private void txtProductos_keyPressed(KeyEvent e) {
        FarmaGridUtils.aceptarTeclaPresionada(e, tblRelacionProductos, txtProductos, 2);

        if (e.getKeyCode() == KeyEvent.VK_ENTER) {
            e.consume();
            if (tblRelacionProductos.getSelectedRow() >= 0) {
                if (!(FarmaUtility.findTextInJTable(tblRelacionProductos, txtProductos.getText().trim(), 1, 2))) {
    ...
    private void txtProductos_keyReleased(KeyEvent e) {
        FarmaGridUtils.buscarDescripcion(e, tblRelacionProductos, txtProductos, 2);
    }
    ...
    private void aceptar() {
        ...
        try {
            ...
            VariablesInvDiario.vCodLabProdCruce =
            ...
            VariablesInvDiario.vStockProdD =
                    Double.parseDouble(pDatosDel[0].trim()) * (Double.parseDouble(VariablesInvDiario.vValFrac) /
                                                               ValFracMax1) + fraccion;
            ...
            DlgIngresoCantidadCruce dlgIngresoCantidadCruce =
                new DlgIngresoCantidadCruce(this.myParentFrame, "", true);
            ...
            log.error("", e);
            FarmaUtility.showMessage(this, "Ha ocurrido un error al insertar los productos.\n" +
    ...
    private void cargarRegistroSeleccionado() {
        ...
        VariablesInvDiario.vCodigoAux =
                tblRelacionProductos.getValueAt(tblRelacionProductos.getSelectedRow(), 1).toString().trim();
        VariablesInvDiario.vNomProdAux =
                tblRelacionProductos.getValueAt(tblRelacionProductos.getSelectedRow(), 2).toString().trim();
        VariablesInvDiario.vUnidPresentaAux =
        ...
        VariablesInvDiario.vStockProdCruce =
    ...
    private boolean validaSeleccion() {
        ...
        if (!retorno)
            FarmaUtility.showMessage(this, "Debe seleccionar, al menos un producto.", txtProductos);
