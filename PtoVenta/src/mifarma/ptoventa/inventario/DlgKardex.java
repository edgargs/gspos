...
public class DlgKardex extends JDialog {
    ...
    private void jbInit() throws Exception {
        ...
        lblF13.setText("[ F7 ] Ver Detalle x Lote");
        lblF13.setBounds(new Rectangle(15, 390, 140, 20));
    }
    ...
    private void initTable() {
        log.debug("TIPO_LOCAL:"+ConstantsPtoVenta.TIPO_LOCAL_VTA_MAYORISTA + " "+ VariablesPtoVenta.vTipoLocalVenta);
        tableModel =
    ...
    private void chkKeyPressed(KeyEvent e) {
        if (UtilityPtoVenta.verificaVK_F2(e)) {
                    ...
                    if (tieneRegistroSeleccionado(tblListaProductos)) {
                        ...
                        if (VariablesInventario.vIndProdVirtual.equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                            ...
                            if (!validarAsistAudit()) { //ASOSA 20.01.2010
                                ...
                                log.debug("tipo local "+ VariablesPtoVenta.vTipoLocalVenta);
                                log.debug("tipo local...  "+ ConstantsPtoVenta.TIPO_LOCAL_VTA_MAYORISTA);
                                ...
                                if(UtilityPtoVenta.isLocalVentaMayorista()){
                                    log.debug("local M");
                                    DlgAjusteKardexLote dlgAjusteKardexLote = new DlgAjusteKardexLote(myParentFrame, "", true);
                                    dlgAjusteKardexLote.setVisible(true);
                                }else {
            ...
            if (tieneRegistroSeleccionado(tblListaProductos)) {
                cargarRegistroSeleccionado();
                DlgLoteKardex dlgLoteKardex = new DlgLoteKardex(myParentFrame, "", true);
                dlgLoteKardex.setVisible(true);
            }
        }
    }
    // **************************************************************************
