CREATE OR REPLACE PACKAGE "PTOVENTA_VTA" AS
  ...
  IND_TIPO_REDONDEDO_CASO3 CONSTANT CHAR(1) := '3'; --JCHAVEZ 29102009
   ...
   MOT_CONSUMO_INTERNO CHAR(3)     :='522';             --CCASTILLO 21/04/2016
   USU_CONSUMO_AUTO    VARCHAR2(15):='CONSUMO_AUTO';    --CCASTILLO 21/04/2016
   TIP_DOC_KARDEX_AJUSTE CHAR(2)   :='03';              --CCASTILLO 21/04/2016
...
--Descripcion: obtengo el  dcto de los productos de un pedido
 ...
 /* FUNCTION VTA_F_CUR_OBT_DCTO_PROD(cCodGrupoCia_in IN CHAR,
  ...
  --04/11/2008  JCALLO     Creacion
   ...
   --21/07/2015  ASOSA      Modificacion
    FUNCTION VTA_F_GET_MONTO_VALIDA_DATOS(cCodCia_in in char)
    RETURN VARCHAR2;
...
--Fecha:       06/08/2014
  ...
  /**************************************************************************************************************************/
  --Descripcion: VALIDA SI DEBE APLICAR PERCEPCION AL PEDIDO O LISTADO DE PRODUCTOS
  --Fecha       Usuario    Comentario
  --21.04.2016  KMONCADA   CREACION
  FUNCTION F_GET_APLICA_PERCEPCION(cCodGrupoCia_in   IN CHAR,
                                   cCodLocal_in      IN CHAR,
                                   cLstProductos_in  IN VARCHAR2 DEFAULT NULL,
                                   cCodConvenio_in   IN CHAR DEFAULT NULL,
                                   cNumPedVta_in     IN CHAR DEFAULT NULL)
    RETURN CHAR;

  /**************************************************************************************************************************/
  --Descripcion: CALCULA LOS MONTOS DE PERCEPCION A APLICAR EN CADA PRODUCTOS DEL PEDIDO
  --Fecha       Usuario    Comentario
  --21.04.2016  KMONCADA   CREACION
  PROCEDURE P_OPERA_PERCEPCION(pCodGrupoCia_in IN CHAR,
                               pCodLocal_in    IN CHAR,
                               pNumPedVta_in   IN CHAR);

  /**************************************************************************************************************************/
  --Descripcion: DETERMINA QUE PRODUCTOS APLICA LA PERCEPCION Y SUS PORCENTAJES
  --Fecha       Usuario    Comentario
  --21.04.2016  KMONCADA   CREACION
  FUNCTION F_GET_PROD_APLICA_PERCEPCION(cCodGrupoCia_in IN CHAR,
                                        cCodLocal_in IN CHAR,
                                        cNumPedVta_in IN CHAR DEFAULT NULL,
                                        cTipoDocumento_in IN CHAR DEFAULT PKG_CONSTANTES.COD_TIPO_BOLETA,
                                        cLstProductos_in IN VARCHAR2 DEFAULT NULL,
                                        cIsAgenteRetencion_in IN CHAR DEFAULT 'N',
                                        cIsAgentePercepcion_in IN CHAR DEFAULT 'N'
                                        )
    RETURN FARMACURSOR;

  /**************************************************************************************************************************/
  --Descripcion: VALIDA SI UN PRODUCTO ES DE UN CENTRO MEDICO
  --Fecha       Usuario    Comentario
  --06.05.2016  KMONCADA   CREACION

  FUNCTION F_IS_PROD_CENTRO_MEDICO(cCodGrupoCia_in IN CHAR,
                                     cCodLocal_in    IN CHAR,
                                     cCodProd_in     IN CHAR)
  RETURN CHAR;

  /**************************************************************************************************************************/
  --Descripcion: ACTUALIZARA EL CAMPO CMP DE LA CABECERA PARA CASOS DE PEDIDOS CON PROD FARMA
  --Fecha       Usuario    Comentario
  --13.05.2016  KMONCADA   CREACION  
  PROCEDURE P_REGISTRA_CMP_FARMA(cCodGrupoCia_in IN VTA_PEDIDO_VTA_CAB.COD_GRUPO_CIA%TYPE,
                                cCodLocal_in    IN VTA_PEDIDO_VTA_CAB.COD_LOCAL%TYPE,
                                cNumPedVta_in   IN VTA_PEDIDO_VTA_CAB.NUM_PED_VTA%TYPE,
                                cNumCmp_in      IN VTA_PEDIDO_VTA_CAB.NUM_CMP%TYPE);

  --Descripcion: Calcula el tipo y número de bolsas a utlizar en una venta
  --Fecha       Usuario    Comentario
  --21/04/2016  CCASTILLO  Creacion
  FUNCTION VTA_CALCULA_BOLSAS(cCodGrupoCia_in IN CHAR,
                              cCodLocal_in    IN CHAR,
                              cUsu_in         IN CHAR,
                              cNumPedVta_in   IN CHAR)
  RETURN FarmaCursor;
  ...
  --Descripcion: Registra el kardex del papel termico a utilizado en una venta
  --Fecha       Usuario    Comentario
  --05/05/2016  CCASTILLO  Creacion
   
   PROCEDURE VTA_ACT_KARDEX_PAPEL_TERMICO(cCodGrupoCia_in  IN CHAR,
                                          cCodLocal_in     IN CHAR,
                                          iCantidad_in     IN NUMBER);  
...
END;
/
CREATE OR REPLACE PACKAGE BODY "PTOVENTA_VTA" AS
...
--           TO_CHAR(PROD.VAL_BONO_VIG,'990.00') || 'Ã' ||
           ...
           --MODIFICADO POR DVELIZ 10.10.08
           '0' || 'Ã' ||
           TO_CHAR(NVL(PROD_LOCAL.VAL_PREC_LISTA,0),'999,990.000')|| 'Ã' ||
...
--                      TO_CHAR(PROD.VAL_BONO_VIG,'990.00') || 'Ã' ||
           ...
           --MODIFICADO POR DVELIZ 10.10.08
           '0' || 'Ã' ||
                      TO_CHAR(NVL(PROD_LOCAL.VAL_PREC_LISTA_SUG,0),'999,990.000') || 'Ã' ||
...
--                      TO_CHAR(PROD.VAL_BONO_VIG,'990.00') || 'Ã' ||
                      '-' || 'Ã' ||
                      TO_CHAR(NVL(PROD_LOCAL.PORC_DCTO_1,0),'990.000') || 'Ã' ||
                      TO_CHAR(NVL(PROD_LOCAL.VAL_PREC_LISTA,0),'999,990.000') || 'Ã' ||
                      TO_CHAR((PROD_LOCAL.VAL_PREC_VTA*(1+LOC.PORC_DSCTO_CASTIGO/100)),'990.000') || 'Ã' ||--por si acaso
                      PROD_LOCAL.VAL_FRAC_LOCAL|| 'Ã' ||
                      NVL(PROD_LOCAL.IND_ZAN,' ')--JCORTEZ 16/07/08
  ...
  BEGIN
   ...
   -- JCHAVEZ 23092009.sn
    ...
    IF cIndConvBTLMF = 'S' THEN
     ...
     -- KMONCADA 08.07.2015 INDICARON QUE VENTA MANUAL SI ACUMULA PTOS
...
--  AND    C.IND_FID = 'N';
  ...
  IF nCantCampFid > 0 then
    -- KMONCADA 23.02.2016 NUEVOS FORMATOS DE CUPONES
    v_cCodigoCamp := PTOVENTA_CUPON.F_GET_COD_CAMPANA_CUPON(cCodCupon_in); --SUBSTR(cCodCupon_in,1,5);
    v_cCodLocalEmi := PTOVENTA_CUPON.F_GET_LOCAL_CUPON(cCodCupon_in); --SUBSTR(cCodCupon_in,6,3);
    v_cSecuencial := PTOVENTA_CUPON.F_GET_SECUENCIAL_CUPON(cCodCupon_in); --SUBSTR(cCodCupon_in,9,5);
...
---------------------------------------------------------------------------------------------------------------------*/
 FUNCTION VTA_OBTIENE_IND_CAMP(cCodGrupoCia_in IN CHAR,
  ...
  BEGIN
        ...
        --- < /18-JUL-14 TCT Nuevo Modo de Leer Codigo Cupon >
        -- KMONCADA 23.02.2016 NUEVO FORMATO DE CUPONES
        v_cCodLocalEmi := PTOVENTA_CUPON.F_GET_LOCAL_CUPON(cCodCupon_in);
...
FUNCTION VALIDA_COD_BARRA(cCodGrupoCia_in IN CHAR,
  ...
  PROCEDURE VTA_P_UPDATE_DET_PED_VTA( cNumPedVta IN CHAR,
    ...
    BEGIN
        ...
        -- KMONCADA 11.08.2015 SE AGREGARA EL AHORRO DE LA CAMPAÑA AL CAMPO
        AHORRO = NVL(AHORRO,0) + NVL(cAhorro,0),
        COD_CAMP_CUPON = cCodCamp,
        PORC_DCTO_CALC = cPorcDctoCalc,
        -- KMONCADA 28.05.2015 SE REGISTRARA TAMBIEN EN EL CAMPO DE AHORRO POR CAMPAÑAS CON EL PROGRAMA PUNTOS
        AHORRO_CAMP = cAhorro
 ...
 /* FUNCTION VTA_F_CUR_OBT_DCTO_PROD(cCodGrupoCia_in IN CHAR,
  ...
  PROCEDURE VTA_P_VALIDAR_VALOR_VTA(cCodGrupoCia_in IN CHAR,
    ...
    IS
        ...
        --dubilluz 16.08.2015
        nAhorro_calculado number;
        nPcto_ahorro number;
        nPctoMin_campana_locked number;
    ...
    BEGIN
     ...
     DBMS_OUTPUT.put_line('cIndicador:'||cIndicador);
      ...
      IF cIndicador = 'N' THEN--QUIER DECIR QUE NO DEBE PERMITIR VENDER POR DEBAJO DEL COSTO PROMEDIO
         ...
         -- calculo de ahorro y de % descuento para ver si cumple algunos parametros
         -- dubilluz 16.08.2015
         if vIndDocLocked_in = 'S' then
          nAhorro_calculado := nPrecioVta_in - nPreciofinal;
          nPcto_ahorro := nAhorro_calculado*100/nPrecioVta_in;
          
          SELECT nvl(nvl(p.LOCKED_valor_dcto_min,c.LOCKED_valor_dcto_min),0)
          INTO   nPctoMin_campana_locked
          FROM VTA_CAMPANA_CUPON C,
               vta_campana_prod_uso p
          WHERE C.COD_GRUPO_CIA = cCodGrupoCia_in
          AND   C.COD_CAMP_CUPON = cCodCamp_in
          ...
          if nPcto_ahorro < nPctoMin_campana_locked then
          -- es menor que el minimo x tanto calculara con el valor del minimo
            nPreciofinal:= round(nPrecioVta_in*(100-nPctoMin_campana_locked)/100,3);
          end if;
        end if;
         -- dubilluz 16.08.2015
...
----------------------------------------------------------------------------------------------------------------------------*/
 FUNCTION VTA_F_CURSOR_DATOS_CUPON ( cCodGrupoCia_in IN CHAR,
    ...
    vIndDocLocked_in char(1) := 'N';

    BEGIN

    vIndDocLocked_in    := PTOVENTA_FIDELIZACION.F_IS_DOC_LOCKED(vDNI_in);
 ...
 OPEN curCupon FOR
       SELECT C.COD_CAMP_CUPON AS COD_CAMP_CUPON,
              ...
              case
                when vIndDocLocked_in = 'N' then C.VALOR_CUPON
                else nvl(c.locked_valor_cupon,C.VALOR_CUPON)
              end AS VALOR_CUPON,
              TRIM(to_char(NVL(C.MONT_MIN_USO, 0),'99999999.999')) AS MONT_MIN_USO,
              NVL(C.UNID_MIN_USO, 0) AS UNID_MIN_USO,
              case
              when vIndDocLocked_in = 'N' then NVL(C.UNID_MAX_PROD, 0)
              else
                nvl(c.locked_unid_max_prod,NVL(C.UNID_MAX_PROD, 0))  
              end AS UNID_MAX_PROD,
              case
              when vIndDocLocked_in = 'N' then NVL(C.MONTO_MAX_DESCT, 0) 
              else
                nvl(c.locked_monto_max_desct ,NVL(C.MONTO_MAX_DESCT, 0) )  
              end AS MONTO_MAX_DESCT,
              ...
              LPAD(C.RANKING, 8, '0') ||
              TIP_CUPON || TO_CHAR(100000 - (case
                when vIndDocLocked_in = 'N' then C.VALOR_CUPON
                else nvl(c.locked_valor_cupon,C.VALOR_CUPON)
              end), '00000.000') AS ORDEN
       FROM   VTA_CAMPANA_CUPON C
       WHERE  C.COD_GRUPO_CIA = cCodGrupoCia_in
      ...
      ELSE
       ...
       SELECT C.COD_CAMP_CUPON AS COD_CAMP_CUPON,
              ...
              --NVL(fpc.descuento_personalizado,0) AS 
             case
                when vIndDocLocked_in = 'N' then NVL(fpc.descuento_personalizado,0)
                else nvl(c.locked_valor_cupon,NVL(fpc.descuento_personalizado,0))
              end AS VALOR_CUPON, -- dubilluz 09.06.2011
              TRIM(to_char(NVL(C.MONT_MIN_USO, 0),'99999999.999')) AS MONT_MIN_USO,
              NVL(C.UNID_MIN_USO, 0) AS UNID_MIN_USO,
  ...
  Select --ltavara 26/01/2015
     ...
     WHERE C.COD_GRUPO_CIA = cCodGrupoCia_in
       ...
       AND exists (select 1
            ...
            AND ABS(CP.VAL_NETO_COMP_PAGO+NVL(CP.VAL_REDONDEO_PERCEPCION,0)+CP.VAL_REDONDEO_COMP_PAGO) = to_number(cMonto_Neto_in,'999999.000')
      )
      union 
      SELECT     C.NUM_PED_VTA ||';'||
           to_char(ABS(C.VAL_NETO_PED_VTA),'9999999.00') ||';'||
...
BEGIN
 ...
 IF  vc_Cod_Camp_Cupon IS NULL THEN
   -- KMONCADA 18.02.2015 SE EVALUA SEGUN NUEVO FORMATO DE CUPON
   vc_Cod_Camp_Cupon := PTOVENTA_CUPON.F_GET_COD_CAMPANA_CUPON(cCodCupon_in);
...
/***************************************************************************************************************************/
  ...
  FUNCTION F_GET_APLICA_PERCEPCION(cCodGrupoCia_in   IN CHAR,
    ...
    vCantAplica NUMBER;
    vAplica CHAR(1) := 'N';
    vTipoPercepcion PBL_TIPO_PERCEPCION.ID_TIPO_PERCEPCION%TYPE;
    vSecPedVta VTA_PEDIDO_VTA_DET.SEC_PED_VTA_DET%TYPE;
    vCodProd VTA_PEDIDO_VTA_DET.COD_PROD%TYPE;
    vCantAtendida VTA_PEDIDO_VTA_DET.CANT_ATENDIDA%TYPE;
    vValPrecVta   VTA_PEDIDO_VTA_DET.VAL_PREC_TOTAL%TYPE;
    vValPorcentaje VTA_PEDIDO_VTA_DET.VAL_PCT_PERCEPCION%TYPE;
    vValMinAplica PBL_TIPO_PERCEPCION.VAL_MIN_APLICA%TYPE;
    vTotalGrupo NUMBER(10,3);

    curDetalle FARMACURSOR;
    vContFila NUMBER := 0;
    vValidaPercepcion CHAR(1):='N';
    vAplicaPercepcion CHAR(1);
  BEGIN
    vAplicaPercepcion := FARMA_UTILITY.F_GET_LOCAL_APLICA_PERCEPCION(cCodGrupoCia_in => cCodGrupoCia_in,cCodLocal_in => cCodLocal_in);
    
    IF vAplicaPercepcion = 'N' THEN
      RETURN 'N';
    END IF;
  
    IF cCodConvenio_in IS NULL THEN
      vValidaPercepcion := 'S';
    ELSE
      BEGIN 
        SELECT DECODE(COUNT(1), 0, 'N', 'S') 
        INTO vValidaPercepcion
        FROM MAE_CONVENIO C
        WHERE C.COD_CONVENIO = cCodConvenio_in
        AND C.FLG_TIPO_CONVENIO = PKG_CONSTANTES.TIPO_CONV_CONTADO
        AND C.PCT_BENEFICIARIO = 100
        AND NVL(C.PCT_EMPRESA, 0) = 0
        AND TRIM(C.COD_TIPDOC_CLIENTE) IS NULL
        AND C.COD_TIPDOC_BENEFICIARIO IN
                                   (SELECT M.COD_TIPODOC
                                    FROM MAE_TIPO_COMP_PAGO_BTLMF M, 
                                         VTA_TIP_COMP P
                                    WHERE P.COD_GRUPO_CIA = cCodGrupoCia_in
                                    AND P.TIP_COMP = M.TIP_COMP_PAGO
                                    AND P.TIP_COMP IN (PKG_CONSTANTES.COD_TIPO_BOLETA, 
                                                       PKG_CONSTANTES.COD_TIPO_FACTURA, 
                                                       PKG_CONSTANTES.COD_TIPO_TICKET_BOLETA));
      EXCEPTION 
        WHEN OTHERS THEN
          vValidaPercepcion:= 'N';
      END;
    ...
    IF vValidaPercepcion = 'S' THEN
      IF cLstProductos_in IS NULL THEN
        curDetalle := PTOVENTA_VTA.F_GET_PROD_APLICA_PERCEPCION(cCodGrupoCia_in => cCodGrupoCia_in, 
                                                                cCodLocal_in => cCodLocal_in, 
                                                                cNumPedVta_in => cNumPedVta_in);
      ELSE
        curDetalle := PTOVENTA_VTA.F_GET_PROD_APLICA_PERCEPCION(cCodGrupoCia_in => cCodGrupoCia_in, 
                                                                cCodLocal_in => cCodLocal_in, 
                                                                cLstProductos_in => cLstProductos_in);
      END IF;
      LOOP
        FETCH curDetalle INTO vTipoPercepcion, vSecPedVta, vCodProd, vCantAtendida, vValPrecVta, vValPorcentaje, vValMinAplica, vTotalGrupo;
        EXIT WHEN curDetalle%NOTFOUND;
          vContFila := vContFila + 1;
      END LOOP;
      CLOSE curDetalle;
    ELSE
      vContFila := 0;
  ...
  PROCEDURE P_OPERA_PERCEPCION(pCodGrupoCia_in IN CHAR,
                               ...
                               pNumPedVta_in   IN CHAR)
    IS
    vCodClienteLocal PTOVENTA.VTA_PEDIDO_VTA_CAB.COD_CLI_LOCAL%TYPE;
    vIndConvenio PTOVENTA.VTA_PEDIDO_VTA_CAB.IND_CONV_BTL_MF%TYPE;
    vNumDocCliente PTOVENTA.VTA_PEDIDO_VTA_CAB.RUC_CLI_PED_VTA%TYPE;
    vIndAgenteRetencion PBL_CLIENTE.IND_AGENTE_RETENCION%TYPE;
    vIndAgetenPercepcion PBL_CLIENTE.IND_AGENTE_PERCEPCION%TYPE;
    vTipoDocumento CHAR(2);
    vAplicaPercepcion CHAR(1) := 'N';
    
    vTipoPercepcion NUMBER;
    vSecPedVta NUMBER;
    vCodProd CHAR(6);
    vCantAtendida VTA_PEDIDO_VTA_DET.CANT_ATENDIDA%TYPE;
    vValPrecVta   VTA_PEDIDO_VTA_DET.VAL_PREC_TOTAL%TYPE;
    ...
    vTotalGrupo NUMBER(10,3);
    curDetalle FARMACURSOR;
    vMontoPercepcion NUMBER(9,3);
    vMontoResiduo NUMBER(9,3) := 0;
    vTipoRedondeoPercepcion CHAR(1);
    vCodConvenio VTA_PEDIDO_VTA_CAB.COD_CONVENIO%TYPE;
  BEGIN
    SELECT C.COD_CLI_LOCAL, NVL(C.IND_CONV_BTL_MF,'N'), C.RUC_CLI_PED_VTA, C.COD_CONVENIO
    INTO vCodClienteLocal, vIndConvenio, vNumDocCliente, vCodConvenio
    FROM VTA_PEDIDO_VTA_CAB C
    WHERE C.COD_GRUPO_CIA = pCodGrupoCia_in
    AND C.COD_LOCAL = pCodLocal_in
    AND C.NUM_PED_VTA = pNumPedVta_in;
    
    IF vIndConvenio = 'N' THEN
      IF vCodClienteLocal IS NOT NULL THEN -- FACTURA
        SELECT C.IND_AGENTE_RETENCION, C.IND_AGENTE_PERCEPCION
        INTO vIndAgenteRetencion, vIndAgetenPercepcion
        FROM PBL_CLIENTE C
        WHERE C.DNI_CLI = vNumDocCliente
        AND C.COD_TIP_DOCUMENTO = '05';
        vTipoDocumento:='02';
        vAplicaPercepcion := 'S';
      ELSE
        vIndAgenteRetencion := 'N';
        vIndAgetenPercepcion := 'N';
        vTipoDocumento := '01';
        vAplicaPercepcion := PTOVENTA_VTA.F_GET_APLICA_PERCEPCION(cCodGrupoCia_in => pCodGrupoCia_in, 
                                                                  cCodLocal_in => pCodLocal_in, 
                                                                  cNumPedVta_in => pNumPedVta_in);
      END IF;
    ELSE
      vIndAgenteRetencion := 'N';
      vIndAgetenPercepcion := 'N';
      ...
      vAplicaPercepcion := PTOVENTA_VTA.F_GET_APLICA_PERCEPCION(cCodGrupoCia_in => pCodGrupoCia_in, 
                                                                cCodLocal_in => pCodLocal_in, 
                                                                cNumPedVta_in => pNumPedVta_in,
                                                                cCodConvenio_in => vCodConvenio);
    END IF; 
    
    IF vAplicaPercepcion = 'S' THEN
      curDetalle := PTOVENTA_VTA.F_GET_PROD_APLICA_PERCEPCION(cCodGrupoCia_in => pCodGrupoCia_in, 
                                                              cCodLocal_in => pCodLocal_in,
                                                              cNumPedVta_in => pNumPedVta_in,
                                                              cTipoDocumento_in => vTipoDocumento,
                                                              cIsAgenteRetencion_in => vIndAgenteRetencion,
                                                              cIsAgentePercepcion_in => vIndAgetenPercepcion);
      LOOP
        FETCH curDetalle INTO vTipoPercepcion, vSecPedVta, vCodProd, vCantAtendida, vValPrecVta, vValPorcentaje, vValMinAplica, vTotalGrupo;
        EXIT WHEN curDetalle%NOTFOUND;

          UPDATE VTA_PEDIDO_VTA_DET D
          SET D.VAL_PCT_PERCEPCION = vValPorcentaje,
              D.VAL_MONTO_PERCEPCION = ROUND(vValPrecVta * (vValPorcentaje/100), 2)
          WHERE D.COD_GRUPO_CIA = pCodGrupoCia_in
          AND D.COD_LOCAL = pCodLocal_in
          AND D.NUM_PED_VTA = pNumPedVta_in
          AND D.SEC_PED_VTA_DET = vSecPedVta;

      END LOOP;
      CLOSE curDetalle;
      
      UPDATE VTA_PEDIDO_VTA_CAB C
      SET C.VAL_PERCEPCION_PED_VTA = (SELECT SUM(NVL(D.VAL_MONTO_PERCEPCION,0))
                                      FROM VTA_PEDIDO_VTA_DET D
                                      WHERE D.COD_GRUPO_CIA = pCodGrupoCia_in
                                      AND D.COD_LOCAL = pCodLocal_in
                                      AND D.NUM_PED_VTA = pNumPedVta_in)
      ...
      SELECT C.VAL_PERCEPCION_PED_VTA
      ...
      BEGIN
        ...
        INTO vTipoRedondeoPercepcion
        FROM PBL_TAB_GRAL L
        WHERE ID_TAB_GRAL = 609;
      EXCEPTION
        WHEN OTHERS THEN
          vTipoRedondeoPercepcion := 1;
      END;
      
      IF vTipoRedondeoPercepcion = '1' THEN
        vMontoResiduo := vMontoPercepcion MOD 0.1;
      ELSIF vTipoRedondeoPercepcion = '2' THEN
        vMontoResiduo := vMontoPercepcion MOD 0.05;
      END IF;
      
      UPDATE VTA_PEDIDO_VTA_CAB C
      SET C.VAL_PERCEPCION_PED_VTA = vMontoPercepcion - vMontoResiduo,
          C.VAL_REDONDEO_PERCEPCION = vMontoResiduo * (-1)
      WHERE C.COD_GRUPO_CIA = pCodGrupoCia_in
      AND C.COD_LOCAL = pCodLocal_in
      AND C.NUM_PED_VTA = pNumPedVta_in;
  ...
  BEGIN
    OPEN curDetalle FOR
    SELECT *
      FROM (SELECT Y.ID_TIPO_PERCEPCION, X.SEC_PED_VTA_DET, X.COD_PROD, X.CANT_ATENDIDA,
                   X.VAL_PREC_TOTAL, Y.VAL_PORCENTAJE, Y.VAL_MIN_APLICA,
                   SUM(X.VAL_PREC_TOTAL) OVER(PARTITION BY Y.ID_TIPO_PERCEPCION) TOTAL_GRUPO
              FROM (SELECT CASE
                             WHEN cTipoDocumento_in = PKG_CONSTANTES.COD_TIPO_FACTURA THEN 0
                             ELSE P.ID_TIPO_PERCEPCION
                           END ID_TIPO_PERCEPCION_D,
                           DET.SEC_PED_VTA_DET,
                           DET.COD_PROD,
                           DET.CANT_ATENDIDA,
                           DET.VAL_PREC_TOTAL
                     ...
                     WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
                       ...
                       AND DET.COD_GRUPO_CIA = P.COD_GRUPO_CIA
                       AND DET.COD_PROD = P.COD_PROD
                       AND P.IND_CON_PERCEPCION = 'S'
                    UNION
                    SELECT P.ID_TIPO_PERCEPCION ID_TIPO_PERCEPCION_D,
                           0 SEC_PED_VTA_DET,
                           P.COD_PROD,
                           TO_NUMBER(X.CANT_ATENDIDA, '999990.000') CANT_ATENDIDA,
                           TO_NUMBER(X.VAL_PREC_TOTAL, '99999990.000') VAL_PREC_TOTAL
                      FROM LGT_PROD P,
                           (SELECT TRIM(SUBSTR(val, 1, INSTR(val, '|', 1, 1) - 1)) COD_PROD,
                                   TRIM(SUBSTR(val, INSTR(val, '|', 1, 1) + 1, INSTR(val, '|', 1, 2) - INSTR(val, '|', 1, 1) - 1)) CANT_ATENDIDA,
                                   TRIM(SUBSTR(val, INSTR(val, '|', 1, 2) + 1)) VAL_PREC_TOTAL
                              FROM (SELECT EXTRACTVALUE(xt.column_value, 'e') VAL
                                      FROM TABLE(XMLSEQUENCE(
                                                  EXTRACT(
                                                     XMLTYPE('<coll><e>' || 
                                                             REPLACE((cLstProductos_in),'@','</e><e>') ||
                                                             '</e></coll>'), '/coll/e'))) xt)) X,
                           PBL_TIPO_PERCEPCION T
                     WHERE P.COD_GRUPO_CIA = cCodGrupoCia_in
                       AND P.COD_PROD = X.COD_PROD
                       AND P.ID_TIPO_PERCEPCION = T.ID_TIPO_PERCEPCION
                       AND P.IND_CON_PERCEPCION = 'S'
                    ) X,
                   
                   (SELECT CASE
                             WHEN cTipoDocumento_in = PKG_CONSTANTES.COD_TIPO_FACTURA THEN 0
                             ELSE T.ID_TIPO_PERCEPCION
                           END ID_TIPO_PERCEPCION,
                           T.VAL_PORCENTAJE,
                           T.VAL_MIN_APLICA
                      FROM PBL_TIPO_PERCEPCION T
                     WHERE T.TIP_COMP_PAGO = cTipoDocumento_in
                       AND T.IND_APLICA_AGENTE_RETENCION = cIsAgenteRetencion_in
                       AND (cIsAgenteRetencion_in = 'S' OR T.IND_APLICA_AGENTE_PERCEPCION = cIsAgentePercepcion_in)
                       AND T.ESTADO = 'A') Y
             WHERE X.ID_TIPO_PERCEPCION_D = Y.ID_TIPO_PERCEPCION) K
     WHERE K.TOTAL_GRUPO >= K.VAL_MIN_APLICA;
  ...
  FUNCTION F_IS_PROD_CENTRO_MEDICO(cCodGrupoCia_in IN CHAR,
    ...
    vRspta CHAR(1) := 'N';
    vIpPc VARCHAR2(20);
    vCodTipoLocalVenta MAE_TIP_LOCAL_VENTA.COD_TIP_LOCAL_VENTA%TYPE;
  BEGIN
    ...
    INTO vRspta
    FROM VTA_IMPR_IP IP,
         REL_TIP_LOCAL_VENTA_PROD P
    WHERE IP.COD_GRUPO_CIA = cCodGrupoCia_in
    AND IP.COD_LOCAL = cCodLocal_in
    AND IP.IP = vIpPc
    AND IP.COD_TIP_LOCAL_VENTA = '004'
    AND P.COD_TIP_LOCAL_VENTA = IP.COD_TIP_LOCAL_VENTA
    AND P.COD_PROD = cCodProd_in;*/
    -- KMONCADA 07.05.2016 CONSULTA MEDICA (242126) SE DEBE DE VENDER EN FARMA ARCANGEL COMO VIRTUAL
    SELECT DECODE(SUM(CONT), 0, 'N', 'S')
    INTO vRspta
    FROM (SELECT COUNT(1) CONT
          ...
          AND IP.IP = vIpPc
          AND IP.COD_TIP_LOCAL_VENTA = '004'
          AND*/ P.COD_TIP_LOCAL_VENTA = '004'--IP.COD_TIP_LOCAL_VENTA
          AND P.COD_PROD = cCodProd_in
          UNION ALL
          SELECT COUNT(1) CONT
          FROM /*VTA_IMPR_IP IP, 
          ...
          AND IP.COD_LOCAL = cCodLocal_in
          AND IP.IP = vIpPc
          AND IP.COD_TIP_LOCAL_VENTA = '001'
          AND P.COD_TIP_LOCAL_VENTA = IP.COD_TIP_LOCAL_VENTA
          AND P.COD_PROD = cCodProd_in
          AND */cCodProd_in/*P.COD_PROD*/ = '242126');
...
FUNCTION VTA_CALCULA_BOLSAS(cCodGrupoCia_in IN CHAR,
  ...
  curBolsaVta FarmaCursor;
  vcharBolsaVta varchar2(32767);
  

  CURSOR c_detpedido(p_grupo_cia char,p_cod_local char,p_num_pedidovta varchar2) is
  select c.num_ped_vta,c.cant_atendida,c.val_frac,a.cod_prod,a.val_ancho,a.val_alto,a.val_largo,a.desc_prod,a.desc_unid_present
  ,(1/c.val_frac)*c.cant_atendida fraccionunidad,val_alto*val_largo*val_ancho volumen, ((1/c.val_frac)*c.cant_atendida)*(val_alto*val_largo*val_ancho) fraccionvolumen
  FROM lgt_prod a,lgt_prod_local b,vta_pedido_vta_det c
  ...
  AND b.COD_GRUPO_CIA=p_grupo_cia
  AND b.COD_LOCAL=p_cod_local
  and c.num_ped_vta=p_num_pedidovta
  and c.est_ped_vta_det='A'
  order by fraccionvolumen desc;
  --ORDER BY c.NUM_PED_VTA DESC;
  
  l_reg c_detpedido%ROWTYPE;
  
  
  CURSOR c_bolsas(p_grupo_cia char,p_cod_local char) is
  select a.COD_PROD,a.DESC_PROD,a.VAL_MEDIDA AS VAL_VOLUMEN,a.ORD_TAMANIO num_bolsa
  from vta_insumo a,lgt_prod_local b
  ...
  and  b.ind_reponer='S'
  and  a.tip_prod='01'-- Tipo Bolsas
  and  a.est_prod='A'
  --order by a.ORD_TAMANIO asc;
  order by a.ORD_TAMANIO desc;
  
  l_regbolsas c_bolsas%ROWTYPE;
  
  cantfilas integer:=0;
  
  TYPE bol IS RECORD 
    (
      cod_prod     VTA_INSUMO.COD_PROD%TYPE,
      val_volumen  VTA_INSUMO.VAL_MEDIDA%TYPE,
      desc_prod    VTA_INSUMO.DESC_PROD%TYPE, 
      num_bolsa    VTA_INSUMO.ORD_TAMANIO%TYPE,
      cantidad     integer
    );  
  TYPE bolsas IS TABLE OF bol INDEX BY BINARY_INTEGER ;
  ...
  volumenrestante number:=0;
  fraccionunidad number:=0;
  fraccionvolumen number:=0;
  ...
  TYPE volu IS RECORD 
   (
      cod_prod     VTA_INSUMO.COD_PROD%TYPE,
      val_volumen  VTA_INSUMO.VAL_MEDIDA%TYPE,
      /*** INICIO CCASTILLO 18/08/2016 ***/
      cant_atendida  vta_pedido_vta_det.cant_atendida%TYPE,
      /*** FIN CCASTILLO 18/08/2016 ***/
      indicador    varchar2(10)--boolean
   );  
    
  TYPE typevolumen IS TABLE OF volu INDEX BY BINARY_INTEGER ;
  tvolumenes typevolumen;
  ...
  contx integer:=0;
  tbolsasx bolsas;
  cantbolsas integer:=0;
  
  v_cantotal     NUMBER:=0;
  v_cociente     NUMBER:=0;
  v_resto        NUMBER:=0;
  v_can_utilizada VTA_INSUMO_AUX.CANT_UTILIZADA%TYPE;
  v_grupo_cia pbl_usu_local.cod_grupo_cia%TYPE;
  v_local     pbl_usu_local.cod_local%TYPE;
  v_cant_unid_pqte lgt_prod.VAL_MAX_FRAC%TYPE;


  begin
       OPEN c_detpedido(cCodGrupoCia_in,cCodLocal_in,cNumPedVta_in);
       FETCH c_detpedido INTO l_reg;
       WHILE c_detpedido%FOUND
       LOOP   
                fraccionunidad:=abs(l_reg.fraccionunidad);--(1/l_reg.val_frac)*l_reg.cant_atendida;
                volumen:=abs(l_reg.volumen);--l_reg.val_alto*l_reg.val_largo*l_reg.val_ancho; 
                
                fraccionvolumen:=round(fraccionunidad*volumen,3);--redondea a 3 decimales
                
                if(fraccionvolumen is not null and fraccionvolumen<>0) then
                   volumentotal:=volumentotal+fraccionvolumen;


                   /*** ***/
                   contvol:=contvol+1;
                   tvolumenes(contvol).cod_prod         := l_reg.cod_prod;
                   tvolumenes(contvol).val_volumen      := fraccionvolumen;
                   tvolumenes(contvol).indicador        := 'false';
                   tvolumenes(contvol).cant_atendida    := l_reg.cant_atendida;
                   /*** ***/
                end if;
       ...
       /**** Deshabilita el calculo de bolsas ***/
       --volumentotal:=0;
       /**** Deshabilita el calculo de bolsas ***/
       

    IF NOT (volumentotal=0 OR volumentotal IS NULL ) THEN
       
       OPEN c_bolsas(cCodGrupoCia_in,cCodLocal_in);
       FETCH c_bolsas INTO l_regbolsas;
       WHILE c_bolsas%FOUND
        LOOP
       ...
       WHILE c_bolsas%FOUND
         LOOP
             ...
             IF(volumenrestante>=l_regbolsas.val_volumen or (fila=cantfilas and resto<>0) or (fila=cantfilas and flag=false) ) THEN                                                                                        
                 
                 flag:=true;-- Por lo menos asigno una bolsa 
                 cont:=cont+1;
                 cociente:=volumenrestante/l_regbolsas.val_volumen;
                 cociente:=trunc(cociente);
                 resto:=mod(volumenrestante,l_regbolsas.val_volumen);
                              
                 tbolsas(cont).cod_prod    := l_regbolsas.cod_prod;
                 tbolsas(cont).desc_prod   := l_regbolsas.desc_prod;
                 tbolsas(cont).val_volumen := l_regbolsas.val_volumen;
                 tbolsas(cont).num_bolsa   := l_regbolsas.num_bolsa;
             
                 IF (fila=cantfilas and resto<>0) THEN
                      tbolsas(cont).cantidad    := cociente+1;
                      resto:=0;
                 ELSE     
                      tbolsas(cont).cantidad    := cociente;
                 END IF;
  
                 volumenrestante:=resto;
             END IF;                                                  
		...
		FOR x IN tvolumenes.FIRST..tvolumenes.LAST
         LOOP
          IF tvolumenes.EXISTS(x) THEN
             IF(tvolumenes(x).indicador='false') THEN
                         OPEN c_bolsas(cCodGrupoCia_in,cCodLocal_in);
                         FETCH c_bolsas INTO l_regbolsas;
                         ...
                         LOOP
                          fila:=fila+1;
                           ...
                           IF(l_regbolsas.val_volumen>=tvolumenes(x).val_volumen OR fila=cantfilas ) THEN
                               tvolumenes(x).indicador:='true';
                               
                               /*dbms_output.put_line('*********************************');
                               dbms_output.put_line(x||') - l_regbolsas.val_volumen   : '||l_regbolsas.val_volumen);
                               dbms_output.put_line(x||') - tvolumenes(x).val_volumen : '||tvolumenes(x).val_volumen);
                               dbms_output.put_line(x||') - fila                      : '||fila);*/
                               
                               IF(fila=cantfilas AND tvolumenes(x).val_volumen>l_regbolsas.val_volumen) THEN 
                               --SI EL(LOS) PRODUCTO(S) SUPERAN EL VOLUMEN DE LA BOLSA MÁS GRANDE ENTONCES DAR UNA BOLSA POR CADA PRODUCTO
                               cociente:=tvolumenes(x).cant_atendida;
                               volrespro:=0;
                               
                               ELSE
                               cociente:=tvolumenes(x).val_volumen/l_regbolsas.val_volumen;
                               cociente:=ceil(cociente);
                               
                               volrespro := l_regbolsas.val_volumen*cociente - tvolumenes(x).val_volumen;

                               END If;

                               /*dbms_output.put_line(x||') - cociente '||cociente);
                               dbms_output.put_line(x||') - volrespro '||volrespro);
                               dbms_output.put_line('*********************************');*/
                               
                               
                               contx:=contx+1;
                               tbolsasx(contx).cod_prod    := l_regbolsas.cod_prod;
                               tbolsasx(contx).desc_prod   := l_regbolsas.desc_prod;
                               tbolsasx(contx).val_volumen := l_regbolsas.val_volumen;
                               tbolsasx(contx).num_bolsa   := l_regbolsas.num_bolsa;
                               tbolsasx(contx).cantidad    := cociente;


                               IF(volrespro<>0)THEN 
                                     --dbms_output.put_line('$$$$ Entra al Loop Verficar otros volumenes $$$$');                                         
                                     FOR y IN tvolumenes.FIRST..tvolumenes.LAST  
                                     LOOP
                                        /*dbms_output.put_line('#####################');
                                        dbms_output.put_line(y||') - tvolumenes(y).cod_prod    : '||tvolumenes(y).cod_prod);
                                        dbms_output.put_line(y||') - tvolumenes(y).val_volumen : '||tvolumenes(y).val_volumen); 
                                        dbms_output.put_line(y||') - tvolumenes(y).indicador   : '||tvolumenes(y).indicador);  
                                        dbms_output.put_line('#####################');*/
                                       IF tvolumenes.EXISTS(y) THEN
                                         IF(tvolumenes(y).indicador='false') THEN
                                              IF(volrespro>=tvolumenes(y).val_volumen) THEN   
                                                  --dbms_output.put_line(y||') - volrespro incial   : '||volrespro); 
                                                  tvolumenes(y).indicador:='true';
                                                  volrespro:=volrespro-tvolumenes(y).val_volumen;
                                                  --dbms_output.put_line(y||') - volrespro despues  : '||volrespro); 
                                              END IF;                                 
                                         END IF;     
                                       END IF;                                                       
                                     END LOOP; 
                                     --dbms_output.put_line('$$$$ Sale del Loop Verficar otros volumenes $$$$');                                                               
                               END IF;
        ...
        END LOOP;
         ...
         /*dbms_output.put_line('');
         dbms_output.put_line('$$$$$$$$$$$$ Cantidad de bolsas a utilizar $$$$$$$$$$$$');
         dbms_output.put_line('Pedido               : '||vNumPedidoVenta_in);
         dbms_output.put_line('Volumen Total        : '||volumentotal);*/
         
         --OPEN curVta FOR select * from TABLE(tbolsas);


       /*for h in tbolsasx.first..tbolsasx.last
       loop
       dbms_output.put_line('tbolsasx(h).cod_prod : '||tbolsasx(h).cod_prod||'  -   tbolsasx(h).tbolsasx : '||tbolsasx(h).val_volumen||' tbolsasx(h).cantidad '||tbolsasx(h).cantidad);
                  
       end loop;*/
         
         /**** Agrupar las bolsas a entregar por codigo ****/
         OPEN c_bolsas(cCodGrupoCia_in,cCodLocal_in);
         FETCH c_bolsas INTO l_regbolsas;
           WHILE c_bolsas%FOUND
           LOOP
              cantbolsas:=0;
              FOR z IN tbolsasx.FIRST..tbolsasx.LAST
              LOOP
                 if(l_regbolsas.cod_prod=tbolsasx(z).cod_prod) then
                    cantbolsas:=cantbolsas+tbolsasx(z).cantidad;
                 end if; 
              END LOOP; 
              
              if(cantbolsas<>0) then
                 cont:=cont+1;
                 tbolsas(cont).cod_prod    := l_regbolsas.cod_prod;
                 tbolsas(cont).desc_prod   := l_regbolsas.desc_prod;
                 tbolsas(cont).val_volumen := l_regbolsas.val_volumen;
                 tbolsas(cont).num_bolsa   := l_regbolsas.num_bolsa;
                 tbolsas(cont).cantidad    := cantbolsas;  
              end if;
       ...
       /**** Agrupar las bolsas a entregar por codigo ****/
        ...
        /***** Cambios Produción 1.1 *****/
        SELECT distinct cod_grupo_cia,cod_local into v_grupo_cia,v_local from vta_impr_local;
        /***** Cambios Produción 1.1 *****/

            
         FOR i IN tbolsas.FIRST..tbolsas.LAST
           LOOP
           IF tbolsas.EXISTS(i) THEN
             --dbms_output.put_line(i||') Codigo Bolsa : '||tbolsas(i).cod_prod||' Volumen Bolsa '||tbolsas(i).val_volumen||' Cantidad a usar '||tbolsas(i).cantidad);
             vcharBolsaVta:=vcharBolsaVta||'Ã'||tbolsas(i).cod_prod||'Ã'||tbolsas(i).val_volumen||'Ã'||tbolsas(i).num_bolsa||'Ã'||tbolsas(i).cantidad||'Ã'||tbolsas(i).desc_prod||'@';
             
              /***** Cambios Produción 1.1 *****/
             
                 BEGIN
                   SELECT  val_max_frac into v_cant_unid_pqte from LGT_PROD
                      WHERE COD_GRUPO_CIA=v_grupo_cia
                    AND COD_PROD=tbolsas(i).cod_prod;
                 EXCEPTION
                   WHEN OTHERS THEN
                   v_cant_unid_pqte:=0;
                 END;
                 ...
                 BEGIN
                    SELECT a.cant_utilizada INTO  v_can_utilizada
                    FROM   VTA_INSUMO_AUX a
                    WHERE  a.COD_GRUPO_CIA = v_grupo_cia
                    AND    a.COD_LOCAL  = v_local
                    AND    a.cod_prod = tbolsas(i).cod_prod
                    AND    a.est_prod='A' FOR UPDATE;
                    
                    IF ( v_can_utilizada IS NULL ) THEN
                      v_can_utilizada := 0;
                    END IF;
                    
                 EXCEPTION
                    WHEN NO_DATA_FOUND THEN
                     INSERT INTO VTA_INSUMO_AUX(COD_GRUPO_CIA,COD_LOCAL,COD_PROD,CANT_UTILIZADA,EST_PROD,FEC_CREA_PROD,USU_CREA_PROD,FEC_MOD_PROD,USU_MOD_PROD)
                      VALUES
                      (v_grupo_cia,v_local,tbolsas(i).cod_prod,0,'A',SYSDATE,'SISTEMAS',NULL,NULL);
                     v_can_utilizada := 0;
                 END;
               
               v_cantotal:=v_can_utilizada+tbolsas(i).cantidad;
               /*dbms_output.put_line('v_cantotal  : '||v_cantotal);
               dbms_output.put_line('v_can_utilizada  : '||v_can_utilizada);
               dbms_output.put_line('tbolsas(i).cantidad  : '||tbolsas(i).cantidad);*/
               
           IF (v_cantotal>=v_cant_unid_pqte) THEN
              
                v_cociente:=trunc(v_cantotal/v_cant_unid_pqte);
                v_resto:=mod(v_cantotal,v_cant_unid_pqte);
                
                
                 /*UPDATE vta_insumo_aux
                 SET CANT_UTILIZADA=v_resto,
                     FEC_MOD_PROD=SYSDATE,
                     USU_MOD_PROD=USU_CREA_PROD
                 WHERE COD_GRUPO_CIA=v_grupo_cia
                 and   COD_PROD=tbolsas(i).cod_prod;*/ 
                   
             /***** Cambios Producción 1.1 *****/
             
             
             
             /**Inicio Logica del kardex**/
             SELECT STK_FISICO INTO   v_nStkFisico FROM   LGT_PROD_LOCAL
              WHERE  COD_GRUPO_CIA = cCodGrupoCia_in  AND
                     COD_LOCAL     = cCodLocal_in    AND
                     COD_PROD      = tbolsas(i).cod_prod;
             ...
             /***** Cambios Producción 1.1 *****/
               ...
               v_nStkNew:=v_nStkFisico-v_cociente;--tbolsas(i).cantidad;
               
               IF(v_nStkNew<=0) THEN -- El nuevo Stock no puede ser negativo
                  v_nStkNew:=0;
               END IF;
            ...
            /**Fin Logica del kardex**/
            
         /*** cambios 1.1 Producción ***/  
         ELSE 
              SELECT STK_FISICO INTO   v_nStkFisico FROM   LGT_PROD_LOCAL
              WHERE  COD_GRUPO_CIA = cCodGrupoCia_in  AND
                     COD_LOCAL     = cCodLocal_in    AND
              ...
              END IF;
                 ...
                 --dbms_output.put_line('Actualiza el valor : '||v_cantotal);
         END IF;
      ...
      OPEN curBolsaVta FOR
        SELECT (EXTRACTVALUE(xt.column_value, 'e')) VAL 
        FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' ||
                           REPLACE( ( volumentotal||'@'|| --opcional
                           vcharBolsaVta
                           --'1a'||'Ã'||'1b'||'Ã'||'1c@2a'||'Ã'||'2b'||'Ã'||'2c@3a'||'Ã'||'3b'||'Ã'||'3c@4a'||'Ã'||'4b'||'Ã'||'4c'
                           ),'@','</e><e>') ||'</e></coll>'),'/coll/e'))) xt;
                         
    ELSE


        IF curBolsaVta%ISOPEN then
             CLOSE curBolsaVta;
        END IF;
    OPEN curBolsaVta FOR SELECT 0 FROM DUAL;
...
/***************************************************************************************************************************/
   ...
   PROCEDURE VTA_ACT_KARDEX_PAPEL_TERMICO(cCodGrupoCia_in  IN CHAR,
    ...
    IS
    PRAGMA AUTONOMOUS_TRANSACTION;
    CURSOR C_RESTROLLO IS
    SELECT a.cod_grupo_cia,a.cod_local,a.cod_prod,a.val_medida/*,b.cant_utilizada*/ FROM VTA_INSUMO a,VTA_INSUMO_AUX b
    where a.cod_grupo_cia=b.cod_grupo_cia
    ...
    l_reg C_RESTROLLO%ROWTYPE;
    
    v_cantotal     NUMBER:=0;
    v_cociente     NUMBER:=0;
    v_resto        NUMBER:=0;
    v_nStkFisico   NUMBER:=0;
    v_nStkNew      NUMBER:=0;
    v_can_utilizada VTA_INSUMO_AUX.CANT_UTILIZADA%TYPE;
  ...
  BEGIN
       ...
       CLOSE C_RESTROLLO;
         ...
         BEGIN
            SELECT a.cant_utilizada INTO  v_can_utilizada
            FROM   VTA_INSUMO_AUX a
            WHERE  a.COD_GRUPO_CIA = l_reg.cod_grupo_cia
            AND    a.COD_LOCAL  = l_reg.cod_local
            AND    a.cod_prod = l_reg.cod_prod
            AND    a.est_prod='A' FOR UPDATE;
         ...
         EXCEPTION
            WHEN NO_DATA_FOUND THEN
             INSERT INTO VTA_INSUMO_AUX(COD_GRUPO_CIA,COD_LOCAL,COD_PROD,CANT_UTILIZADA,EST_PROD,FEC_CREA_PROD,USU_CREA_PROD,FEC_MOD_PROD,USU_MOD_PROD)
              VALUES
              (l_reg.cod_grupo_cia,l_reg.cod_local,l_reg.cod_prod,0,'A',SYSDATE,'SISTEMAS',NULL,NULL);
             v_can_utilizada := 0;
         END;
       
       v_cantotal:=v_can_utilizada+iCantidad_in;
       
       IF (v_cantotal>=l_reg.val_medida) THEN
       
          v_cociente:=trunc(v_cantotal/l_reg.val_medida);
          v_resto:=mod(v_cantotal,l_reg.val_medida);
           ...
           SET CANT_UTILIZADA=v_resto,
               FEC_MOD_PROD=SYSDATE,
               USU_MOD_PROD=USU_CREA_PROD
           WHERE COD_GRUPO_CIA=l_reg.cod_grupo_cia
           and   COD_PROD=l_reg.cod_prod; 
           
           /**Inicio Logica del kardex**/
               SELECT STK_FISICO INTO   v_nStkFisico FROM   LGT_PROD_LOCAL
                WHERE  COD_GRUPO_CIA = cCodGrupoCia_in  AND
                       COD_LOCAL     = cCodLocal_in    AND
                       COD_PROD      = l_reg.cod_prod;
               
               
               IF (v_nStkFisico>0) THEN
               
                 v_nStkNew:=v_nStkFisico-v_cociente;
          ...
          /*dbms_output.put_line('l_reg.val_medida : '||l_reg.val_medida);
          dbms_output.put_line('v_cantotal : '||v_cantotal);
          dbms_output.put_line('cociente : '||v_cociente);
          dbms_output.put_line('v_resto  : '||v_resto);*/
