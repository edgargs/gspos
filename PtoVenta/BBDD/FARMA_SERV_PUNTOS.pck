...
/* ******LTAVARA 18.08.2015************************************************************ */
   FUNCTION P_UPDATE_FEC_EJECUTA_DEMONIO(cCodGrupoCia_in IN CHAR,
                           cCodLocal_in    IN CHAR,
                           cCodDemonio_in   IN VARCHAR) RETURN CHAR;

   FUNCTION F_VAR_DEMONIO_VERSION(
                             cCodLocal_in    IN CHAR,
                              cCodDemonio     IN VARCHAR,
                              cVersion       IN VARCHAR) RETURN CHAR;

  FUNCTION F_VAR_DEMONIO_INTERVALO(
                               cCodLocal_in    IN CHAR,
                               cCodDemonio     IN VARCHAR) RETURN VARCHAR2;
/* ******LTAVARA 14.09.2015************************************************************ */

   PROCEDURE F_ACTUALIZAR_PED_NC_DESCARTADO(cCodGrupoCia_in IN CHAR,
                             cCodLocal_in    IN CHAR);
  ...
  BEGIN
    ...
    -- AND C.NUM_TARJ_PUNTOS IS NOT NULL
      AND C.NUM_PED_VTA_ORIGEN IN (
                  ...
                  and cAB.NUM_TARJ_PUNTOS IS NOT NULL
  ...
  BEGIN
     OPEN vCur for
      select C.NUM_PED_VTA,
             ...
             NVL(C.PT_REDIMIDO,0)  AS "PTO_CANJEADO"
      from   VTA_PEDIDO_VTA_CAB C
      where  C.COD_GRUPO_CIA = cCodGrupoCia_in
      AND    C.COD_LOCAL = cCodLocal_in
      AND    C.Fec_Ped_Vta >= to_date('01/01/2015','dd/mm/yyyy')
  ...
  begin
   if cIndOnline_in = 'S' and cIndCancela = 'N' then
     ...
     set    c.fec_proc_puntos = sysdate,
            ...
            c.usu_mod_ped_vta_cab = 'DEMONIO_MONED',
 ...
 SELECT NVL(C.NUM_PED_VTA_ORIGEN,0)
  ...
  BEGIN
    IF cAnulacion_in = 'N' THEN
      ...
      CAB.USU_MOD_PED_VTA_CAB='DEMONIO_MONED',
      CAB.FEC_MOD_PED_VTA_CAB = SYSDATE
      WHERE CAB.COD_GRUPO_CIA = cCodGrupoCia_in
      AND CAB.COD_LOCAL = cCodLocal_in
...
FUNCTION P_UPDATE_FEC_EJECUTA_DEMONIO(cCodGrupoCia_in IN CHAR,
                           cCodLocal_in    IN CHAR,
                           cCodDemonio_in   IN VARCHAR)
    RETURN CHAR IS
    vIndObservado CHAR(1) :='N';
     vCantAfiliacionP number :=0;
     vCantVentaP number :=0;
     vCantAnulacionP number :=0;
  ...
  BEGIN
   ...
   select  nvl(count(*),0)
      ...
      --ANULACIONES PENDIENTES
       select nvl(count(DISTINCT C.NUM_PED_VTA_ORIGEN),0)
       into vCantAnulacionP
      from VTA_PEDIDO_VTA_CAB C
      where C.COD_GRUPO_CIA = cCodGrupoCia_in
      AND C.COD_LOCAL = cCodLocal_in
      AND C.Fec_Ped_Vta >= to_date('01/01/2015', 'dd/mm/yyyy')
      ...
      if vCantAfiliacionP>0 or
        vCantVentaP>0 or
        vCantAnulacionP>0 then
        vIndObservado:='S';
        end if;

  UPDATE PTOVENTA.PBL_DEMONIO D
    SET D.FEC_ULTIMA_EJEC = SYSDATE,
        D.FEC_MOD_DEMONIO=SYSDATE,
        D.USU_MOD_DEMONIO='DEMONIO_MONED',
        D.IND_OBSERVADO=vIndObservado,
        D.OBSERVACION='Afiliados Pendientes:'||vCantAfiliacionP||CHR(13)||
                      'Ventas Pendientes:'||vCantVentaP||CHR(13)||
                      'Anulaciones Pendientes:'||vCantAnulacionP||CHR(13)

            WHERE D.COD_LOCAL = cCodLocal_in
     AND D.COD_DEMONIO=cCodDemonio_in;
     COMMIT;
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
      return 'S';
 ...
 /* *********************************************************************** */
  ...
  IS
    nRetorno CHAR(1) := 'N';
    nVersion VARCHAR2(10) := 'N';
  BEGIN

      SELECT NVL(D.VERSION,'N')
      INTO nVersion
      FROM  PTOVENTA.PBL_DEMONIO D
      WHERE
      D.COD_DEMONIO=cCodDemonio
      AND
      D.COD_LOCAL=cCodLocal_in;

      IF nVersion!='N' THEN
       IF UPPER(nVersion) = UPPER(cVersion) THEN
          nRetorno:='S';
         END IF;
      END IF;
  ...
  FUNCTION F_VAR_DEMONIO_INTERVALO(
                             ...
                             cCodDemonio     IN VARCHAR) RETURN VARCHAR2
  IS
    nIntervalo VARCHAR2(10) := 'N';
  BEGIN

   SELECT NVL(D.Intervalo,'N')
    INTO nIntervalo
    FROM  PTOVENTA.PBL_DEMONIO D
    WHERE
    D.COD_DEMONIO=cCodDemonio
  ...
  END;
  /**
  *PEDIDO Y NC COBRADOS Y DESCARTADOS:
  * Se actualiza a P el indicador orbis del pedido origen y nc.
  */
  PROCEDURE F_ACTUALIZAR_PED_NC_DESCARTADO(cCodGrupoCia_in IN CHAR,
                             cCodLocal_in    IN CHAR) AS
    CURSOR ListaPedidoOrigen IS
    select DISTINCT C.NUM_PED_VTA_ORIGEN NUM_PED_VTA
      from VTA_PEDIDO_VTA_CAB C
      where C.COD_GRUPO_CIA =cCodGrupoCia_in
      AND C.COD_LOCAL = cCodLocal_in
      --INICIA LA VALIDACION
      AND C.Fec_Ped_Vta >= to_date('01/09/2015', 'dd/mm/yyyy')
      and C.est_ped_vta =PEDIDO_COBRADO
      and C.num_ped_vta_origen is not null
      AND C.EST_TRX_ORBIS =TRX_DESCARTADA
      AND C.NUM_PED_VTA_ORIGEN IN (
                  select CAB.NUM_PED_VTA
                  from VTA_PEDIDO_VTA_CAB CAB
                  where CAB.COD_GRUPO_CIA =cCodGrupoCia_in
                  ...
                  AND CAB.NUM_PED_VTA = C.NUM_PED_VTA_ORIGEN
                  and CAB.est_ped_vta = PEDIDO_COBRADO
                  AND CAB.EST_TRX_ORBIS =TRX_DESCARTADA
                  );
  BEGIN
  FOR pedidoOrigen IN ListaPedidoOrigen
    LOOP
      BEGIN

     --ACTUALIZA EL PEDIDO ORIGEN
     UPDATE PTOVENTA.VTA_PEDIDO_VTA_CAB CAB
     set CAB.EST_TRX_ORBIS=TRX_PENDIENTE,
         CAB.fec_mod_ped_vta_cab=SYSDATE,
         CAB.USU_MOD_PED_VTA_CAB ='DEMONIO_MONED'
      where CAB.COD_GRUPO_CIA = '001'
      AND CAB.COD_LOCAL = cCodLocal_in
      AND CAB.NUM_PED_VTA =pedidoOrigen.NUM_PED_VTA;
     ...
     set CAB.EST_TRX_ORBIS=TRX_PENDIENTE,
      ...
      where CAB.COD_GRUPO_CIA = '001'
      AND CAB.COD_LOCAL = cCodLocal_in
      AND CAB.NUM_PED_VTA_ORIGEN =pedidoOrigen.NUM_PED_VTA;

      COMMIT;
      EXCEPTION
        WHEN OTHERS THEN
          ROLLBACK;
          DBMS_OUTPUT.PUT_LINE('ERROR PEDIDO ORIGEN- F_ACTUALIZAR_PED_NC_DESCARTADO - '||pedidoOrigen.NUM_PED_VTA||' - '||SQLERRM);
      END;
    END LOOP;

  END;

  FUNCTION F_VAR_EXISTE_FID_TARJETA(
                               cCodLocal_in    IN CHAR,
                               cNroTarjeta_in  IN CHAR)
    RETURN VARCHAR2 IS
    cCodTarjeta FID_TARJETA.COD_TARJETA%TYPE;
  BEGIN
    BEGIN
      SELECT FID.COD_TARJETA
        INTO cCodTarjeta
        FROM FID_TARJETA FID
       WHERE FID.COD_LOCAL = cCodLocal_in
         AND (FID.COD_TARJETA = cNroTarjeta_in
             or FID.DNI_CLI=cNroTarjeta_in)
         and ROWNUM <=1;
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        cCodTarjeta := 'N';
