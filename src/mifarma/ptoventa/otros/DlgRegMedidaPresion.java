...
public class DlgRegMedidaPresion extends JDialog 
{
  ...
  {
	  System.out.println("seteando valores:"+VariablesOtros.vDniCliente);
  ...
  private void chkKeyPressed(KeyEvent e)
  {
    ...
    if(e.getKeyCode() == KeyEvent.VK_F11){
        if(validarDatos()){
        	...
        	if(FarmaUtility.rptaConfirmDialog(this, "Desea Registrar Datos de la Medida Presion ?")){
  ...
  private String generarHtmlImprimir(){
	...
	try {
		...
		System.err.println("HTML GENERADO : "+html.toString());
		
	} catch (SQLException e) {
		System.err.println("error al generar el html a imprimir");
	}
  ...
  private void traerHistorialDeMatriz(FarmaTableModel farmaTblModel, String pDNI){
		try {
			if(VariablesOtros.vIndLineaMatriz.equals(FarmaConstants.INDICADOR_S)){
				System.err.println("invocando procedimiento de matriz traer datos de matriz a local");
			...
			FarmaUtility.liberarTransaccion();
			//FarmaUtility.showMessage(this, "error al registrar la medida de Presion :"+e, txtDNI);
			System.err.println("ERROR AL TRAER INFORMACION DE MATRIZ");
		} finally {
