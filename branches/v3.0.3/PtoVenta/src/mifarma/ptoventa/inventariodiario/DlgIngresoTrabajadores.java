...
import com.gs.mifarma.componentes.UpperCaseDocument;
...
public class DlgIngresoTrabajadores extends JDialog {
    ...
    private void initTableListaTrabajadores() {
        tableModelListaTrabajadores =
                new FarmaTableModel(ConstantsInvDiario.columnsListaTrabajadores, ConstantsInvDiario.defaultValuesListaTrabajadores,
                                    0);

        FarmaUtility.initSimpleList(tblListaTrabajadores, tableModelListaTrabajadores,
    ...
    private void this_windowOpened(WindowEvent e) {
        ...
        if (VariablesInvDiario.vListaTrabParaDescuento.size() > 0) {
            ...
            FarmaUtility.ordenar(tblListaTrabajadores, tableModelListaTrabajadores, 2,
                                 FarmaConstants.ORDEN_ASCENDENTE);
        }

        FarmaUtility.setearTextoDeBusqueda(tblListaTrabajadores, txtProductos, 1);
        ...
        if (VariablesInvDiario.vAccion.equalsIgnoreCase("A")) {
            ...
            lblDiferencia.setText(FarmaUtility.formatNumber(FarmaUtility.getDecimalNumber(jLabel3.getText().trim()) -
                                                            FarmaUtility.getDecimalNumber(lblTotalMonto.getText().trim())));
            ...
            for (int i = 0; i < tableModelListaTrabajadores.data.size(); i++) {
                pParcial =
                        FarmaUtility.getDecimalNumber(FarmaUtility.getValueFieldArrayList(tableModelListaTrabajadores.data,
                                                                                          i, 3));
                pTotal += pParcial;
            }
    ...
    private void txtProductos_keyPressed(KeyEvent e) {
        FarmaGridUtils.aceptarTeclaPresionada(e, tblListaTrabajadores, txtProductos, 2);
    ...
    private void chkKeyReleased(KeyEvent e) {
        FarmaGridUtils.buscarDescripcion(e, tblListaTrabajadores, txtProductos, 2);
        getAcumulado();
    ...
    public void cambiarMonto() {
        ...
        if (vFila > -1) {
            ...
            VariablesInvDiario.vCodRRHH =
                    FarmaUtility.getValueFieldArrayList(tableModelListaTrabajadores.data, vFila, 1);
            VariablesInvDiario.vNombreCompleto =
                    FarmaUtility.getValueFieldArrayList(tableModelListaTrabajadores.data, vFila, 2);
            VariablesInvDiario.vCodigoTrab =
                    FarmaUtility.getValueFieldArrayList(tableModelListaTrabajadores.data, vFila, 3);
            VariablesInvDiario.vMontoIngresado =
                    FarmaUtility.ShowInput(this, "DNI: " + VariablesInvDiario.vDNI + "\n" +
                        "" + VariablesInvDiario.vNombreCompleto + "\n" +
    ...
    public void cargaSeleccionPersonal() {
        ...
        DlgListaTrabajadores dlgListaTrab = new DlgListaTrabajadores(myParentFrame, "", true);
        ...
        if (FarmaVariables.vAceptar) {
            ...
            for (int i = 0; i < vltrab.size(); i++) {
                if (FarmaUtility.getValueFieldArrayList(vltrab, i,
                                                        1).trim().equalsIgnoreCase(VariablesInvDiario.vCodRRHH.trim())) {
                    agrega = false;
    ...
    public void eliminaTrabajador() {
        if (tblListaTrabajadores.getRowCount() > 0) {
            ...
            if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                                                                            "¿Seguro de quitar el descuento al personal?")) {
        ...
        if (VariablesInvDiario.vAccion.equalsIgnoreCase("A")) {
            ...
            if ((VariablesInvDiario.dTotalProds * -1) != VariablesInvDiario.dTotalDescTrab) {
                ...
                FarmaUtility.showMessage(this,
                                         "El monto total ingresado no coincide con el monto total " + " de los productos. Verifique !!!",
                                         txtProductos);
            } else {
                if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                                                                                "Esta seguro de ingresar los trabajadores?")) {
                    log.info("Trabajadores a Descontar: " + VariablesInvDiario.vListaTrabParaDescuento);
                    cerrarVentana(true);
                }
            }
