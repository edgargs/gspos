...
/* ******************************************************************* */
  ...
  BEGIN
     ...
     SELECT COD_LOCAL,
            ...
            MIN(CASE 
                 WHEN NVL(CP.COD_TIP_PROC_PAGO,'0') = '1' THEN 
                   CP.NUM_COMP_PAGO_E
                 ELSE  
                   CP.NUM_COMP_PAGO 
               END)
             FROM VTA_COMP_PAGO CP WHERE CP.COD_GRUPO_CIA = cGrupoCia_in AND CP.COD_LOCAL = CD.COD_LOCAL AND CP.TIP_COMP_PAGO = CD.TIP_COMP_PAGO AND --SUBSTR(CP.NUM_COMP_PAGO,1,3)
            --FAC-ELECTRONICA :09.10.2014
            FARMA_UTILITY.GET_T_SERIE_COMPROBA
            (CP.COD_TIP_PROC_PAGO,CP.NUM_COMP_PAGO_E,CP.NUM_COMP_PAGO) = CD.SERIE AND CP.NUM_PED_VTA = CD.NUM_PED_MIN) MIN_COMP,
            ...
            --FAC-ELECTRONICA :09.10.2014
            FARMA_UTILITY.GET_T_SERIE_COMPROBA
            (CP.COD_TIP_PROC_PAGO,CP.NUM_COMP_PAGO_E,CP.NUM_COMP_PAGO)= CD.SERIE AND CP.NUM_PED_VTA = CD.NUM_PED_MAX) MAX_COMP
     FROM
          (
             SELECT
                    C.COD_LOCAL,
          ...
          FROM
               (
                 ...
                 WHERE CP.COD_GRUPO_CIA = cGrupoCia_in
                   ...
                   --AND SUBSTR(CP.NUM_COMP_PAGO,1,3) = CD.SERIE --rherrera 11.11.2014
                   AND (CASE  NVL(CP.COD_TIP_PROC_PAGO,'0')
                         WHEN '1' THEN SUBSTR(NUM_COMP_PAGO_E,1,4)
                         WHEN '0' THEN SUBSTR(NUM_COMP_PAGO,1,3)
                        END 
                       ) = CD.SERIE
                   --AND CP.NUM_COMP_PAGO BETWEEN CD.MIN_COMP AND CD.MAX_COMP ----rherrera 11.11.2014
                   AND (CASE  NVL(CP.COD_TIP_PROC_PAGO,'0')
                         WHEN '1' THEN NUM_COMP_PAGO_E
                         WHEN '0' THEN NUM_COMP_PAGO
                        END                          
                       ) BETWEEN CD.MIN_COMP AND CD.MAX_COMP
                 GROUP BY
                        CD.COD_LOCAL, CD.TIP_COMP_PAGO, SERIE,
                        (MAX_COMP - MIN_COMP)
               )
...
/* C_INICIO_MSG  VARCHAR2(2000) := '
 ...
 C_FIN_MSG     VARCHAR2(2000) := '
  ...
  BEGIN
    ...
    INSERT INTO TMP_DOCUMENTO_ELECTRONICOS        
    SELECT vNumTelefono ,'2','C','S','N' FROM DUAL;

    INSERT INTO TMP_DOCUMENTO_ELECTRONICOS        
    SELECT 'MONTO: ' ,'0','I','N','N' FROM DUAL;

    INSERT INTO TMP_DOCUMENTO_ELECTRONICOS        
    SELECT 'S/. '||vMonto ,'2','C','S','N' FROM DUAL;
