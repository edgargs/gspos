                ...
                if (esCompCredito) {
                    ...
                    try {
                        ...
                        if (indConexionRac.equals("S")) {
                          ...
                          //  pedidoAnuladoRac = DBConvenioBTLMF.anularPedidoRac(null, FarmaConstants.INDICADOR_N);//comentado 11.09.2014
                            ...
                            if (estaGrabado){
                                ...
                                */
                                ...
                                for(int k=0;k<pListaPedidos.length;k++){ //kmoncada
                                    ...
                                    if (resMatriz.equals("S")) {
                                        log.debug("-------------------GRABO EXITOSAMENTE MATRIZ-------- ");
                                        String rspta = DBConvenioBTLMF.grabarCobrarPedidoRacNC2(numPedVtaNC, FarmaConstants.INDICADOR_S);
                            ...
                            /*} else {
                               /* FarmaUtility.showMessage(this, "No se pudo anular el pediddo Convenio en el RAC",
                                                         tblListaProductos);*/
                               /* FarmaUtility.showMessage(this, "No se pudo anular GUIA Convenio en el RAC",
                                                         tblListaProductos);//LTAVARA 11.09.2014
                                
                              FarmaUtility.liberarTransaccion();//LTAVARA 12.09.2014
                            }*/
                ...
                //JCHAVEZ 10.07.2009.sn
                    try{
                        if (esConvenioBTLMF && indConvenioBTLMF != null && indConvenioBTLMF.equals("S")) {
                            ...
                            if(UtilityCaja.getImpresionTicketAnulado(this,VariablesCaja.vNumPedVta_Anul,"",VariablesCaja.vNumCaja,VariablesCaja.vNumTurnoCaja)){
                                FarmaUtility.showMessage(this,"El ticket se ha reimpreso con éxito .",null);
                            }
                        }
                        FarmaUtility.aceptarTransaccion();
        ...
        try {
            ...
            if (FarmaVariables.vAceptar) {
                ...
                * */
                ...
                dlgDetalleAnularPedido.evaluaPedidoRecarVirtual(VariablesCaja.vNumPedVta_Anul);
                if (VariablesCaja.vIndPedidoConRecarVirtual) {
                    if (dlgDetalleAnularPedido.cargarLogeoOperador()) { //ASOSA - 24/07/2014
                        DlgProcesarAnulacionRecarga dlgProcesarAnulacionRecarga = new DlgProcesarAnulacionRecarga(myParentFrame, "Virtual", true);
                        dlgProcesarAnulacionRecarga.mostrar();
                        retorno = dlgProcesarAnulacionRecarga.isVProcesoRecarga();
                ...
                * */
                if (dlgDetalleAnularPedido.anularPedidofidelizado(FarmaConstants.INDICADOR_S)) { // LTAVARA 10.09.2014
                    ...
                    for (int i = 0; i < pListaPedidos.length; i++) {
                        ...
                        String nroNC = DBCaja.agregarDetalleNotaCredito(VariablesCaja.vNumPedVta_Anul, numera, "0", "0", "0.0",i + ""); //ERIOS 2.4.3 Indica el secuencial de NC
                                ...
                                if (UtilityEposTrx.validacionEmiteElectronico()) {
                                    ...
                                    if (UtilityCaja.obtieneCompPago()) {
                                        for (int j = 0; j < VariablesVentas.vArray_ListaComprobante.size(); j++) {
                                            ...
                                            VariablesCaja.vNumCompImprimir = UtilityEposTrx.getNumCompPagoE(VariablesConvenioBTLMF.vTipoCompPago, VariablesCaja.vNumPedVta, VariablesConvenioBTLMF.vSecCompPago,"", conexionRAC);
                                    ...
                                    nroCorrecto = nroCorrecto && !DBConvenioBTLMF.isExisteComprobanteEnRAC(FarmaVariables.vCodGrupoCia,
                                                                                                     FarmaVariables.vCodLocal,
                    ...
                    /**Actualiza el pedido virtual con el trace de anulación
    ...
    if(FarmaVariables.vAceptar)
      {
          ...
          if(VariablesCaja.vIndPedidoConRecarVirtual){
              ...
              if (dlgDetalleAnularPedido.cargarLogeoOperador()) {    //ASOSA - 24/07/2014
                  ...
                  return retorno = false;   //ASOSA - 13/11/2014 - RECAR - PARA QUE AL PRESIONAR ESC NO CONTINUE CON LA GENERACION DE NOTA DE CREDITO
              }
          ...
          String tipoCambio = FarmaVariables.vTipCambio+"";
                  ...
                  //SE OBTIENE LA LISTA QUE ALAMACENA LOS NUMEROS DE PEDIDOS
                    ...
                    for(int i=0;i<pListaPedidos.length;i++ ){
                        ...
                        // los parametros de monto,cantidad,total,items NO NECESITA porque se va sacar todo la copia del ORIGINAL de La NC
                        //dubilluz 27.09.2013
                        DBCaja.agregarDetalleNotaCredito(VariablesCaja.vNumPedVta_Anul,numera,"0","0","0.0",i+"");
                         ...
                         if (UtilityEposTrx.validacionEmiteElectronico()){
                            ...
                            if(UtilityCaja.obtieneCompPago()){
                                    ...
                                    //obtener  secComprobante por documento
                                        ...
                                        VariablesConvenioBTLMF.vTipoCompPago = ((String)((ArrayList)VariablesVentas.vArray_ListaComprobante.get(j)).get(2)).trim();
                                        // obtener el numero del comprobante electrónico
                                        VariablesCaja.vNumCompImprimir = UtilityEposTrx.getNumCompPagoE(VariablesConvenioBTLMF.vTipoCompPago,VariablesCaja.vNumPedVta,   VariablesConvenioBTLMF.vSecCompPago,"", false);
                                        // KMONCADA 20.10.2014 VALIDACION DE DUPLICIDAD DE COMPROBANTES ELECTRONICOS
                                        // DUPLICADO EN LOCAL
                                        /*if("L".equalsIgnoreCase(VariablesCaja.vNumCompImprimir)){
