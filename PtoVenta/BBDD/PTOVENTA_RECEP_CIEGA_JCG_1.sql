...
--------------------------------------------------------
 ...
 /*(SELECT CASE
                      ...
                      END
                 FROM DUAL)*/
  CANTIDAD || 'Ã' || PRODUCTO || 'Ã' || SEC_CONTEO
  FROM (SELECT Q1.PROD PRODUCTO,
          ...
          FROM (SELECT A.COD_PROD PROD,
                       ...
                       NVL(TO_CHAR(A.CANT_SEG_CONTEO), ' ') CANTIDAD
...
SELECT H.COD_PROD || 'Ã' ||
       H.DESC_PROD || 'Ã' || H.DESC_UNID_PRESENT || 'Ã' ||
       (SELECT L.NOM_LAB FROM LGT_LAB L WHERE L.COD_LAB = H.COD_LAB) || 'Ã' ||
       NVL(TO_CHAR(E.CANT_SEG_CONTEO), ' ') || 'Ã' || E.COD_PROD || 'Ã' ||
       E.SEC_CONTEO
...
SELECT K.COD_PROD || 'Ã' ||
       K.DESC_PROD || 'Ã' || K.DESC_UNID_PRESENT || 'Ã' ||
       (SELECT L.NOM_LAB FROM LGT_LAB L WHERE L.COD_LAB = K.COD_LAB) || 'Ã' || /*I.CANT_ENVIADA_MATR*/
       ' ' || 'Ã' || K.COD_PROD || 'Ã' || 0
 ...
 WHERE I.COD_GRUPO_CIA = cGrupoCia_in
   ...
   AND I.COD_PROD NOT IN
    ...
    /*  SELECT \*COD_BARRA || 'Ã' ||*\ DESCRIP_PROD || 'Ã' || UNIDAD || 'Ã' || LAB || 'Ã' ||
             \*(SELECT CASE
                       WHEN CANT < 0 THEN
                        CANT * (-1)
                       ELSE
                        CANT
                     END
                FROM DUAL)*\CANTIDAD || 'Ã' || PRODUCTO || 'Ã' || SEC_CONTEO
        FROM (SELECT Q1.PROD PRODUCTO,
                     (Q2.CANT2 - Q1.CANT1) CANT,
                     --Q1.COD_BARRA,
                     Q3.DESC_PROD DESCRIP_PROD,
      ...
      SELECT\* E.COD_BARRA || 'Ã' ||*\ H.DESC_PROD || 'Ã' ||
             H.DESC_UNID_PRESENT || 'Ã' ||
             (SELECT L.NOM_LAB FROM LGT_LAB L WHERE L.COD_LAB = H.COD_LAB) || 'Ã' ||
             NVL(TO_CHAR(E.CANT_SEG_CONTEO), ' ') || 'Ã' || E.COD_PROD || 'Ã' ||
             E.SEC_CONTEO
        FROM LGT_PROD_CONTEO E, LGT_PROD H
       WHERE E.COD_GRUPO_CIA = cGrupoCia_in
        ...
        SELECT K.DESC_PROD || 'Ã' ||K.DESC_UNID_PRESENT || 'Ã' ||  (SELECT L.NOM_LAB FROM LGT_LAB L WHERE L.COD_LAB = K.COD_LAB) || 'Ã' || \*I.CANT_ENVIADA_MATR*\' ' || 'Ã' ||K.COD_PROD || 'Ã' || 0
        FROM LGT_NOTA_ES_DET I, LGT_PROD K
        WHERE I.COD_GRUPO_CIA = cGrupoCia_in
        AND I.COD_LOCAL       = cCodLocal_in
