...
public class DlgConsultarRecargaCorrelativo_AS extends JDialog {
    ...
    private void buscarRecarga() throws SQLException {
       ...
       if (infoRecarga.size() > 0){// si hay pedido
        ...
        if(VariablesVirtual.vConProductoVirtual){ //hay producto virtual
            log.info("VariablesCaja.vIndLineaADMCentral : " + 
                                                VariablesCaja.vIndLineaADMCentral);
            log.debug("VariablesCaja.vIndLineaADMCentral : " + 
                                                VariablesCaja.vIndLineaADMCentral);
    
          if(VariablesPtoVenta.vIndFarmaSix.equals(FarmaConstants.INDICADOR_S)){ //con el six 
              ...
              String strCodeResponse = facadeCaja.obtenerEstadoTrssc(txtCorrelativo.getText().trim());
              String strMsjSix = "";
              if(strCodeResponse != null && !strCodeResponse.equals("")){
                  ArrayList arrayMsjSix  = facadeCaja.obtenerDescErrorSix(strCodeResponse); 
                  strMsjSix       = FarmaUtility.getValueFieldArrayList(arrayMsjSix, 0, 0);
                  String strMsjFarma     = FarmaUtility.getValueFieldArrayList(arrayMsjSix, 0, 1);
              }
              if(ConstantsRecaudacion.COD_SOLICITUD_EXITOSA.equals(strCodeResponse)){
                  strMsjSix = "La recarga se realizó correctamente."; 
              }else{
                  strMsjSix = "No se pudo realizar la recarga.";
              }
              
                  //if( ConstantsRecaudacion.COD_SOLICITUD_EXITOSA.equals(strCodeResponse) ){

                      String msg = "<font face=Arial color=red>"+strMsjSix+"</font>"; 
            ...
            if (VariablesCaja.vIndLineaADMCentral.equals(FarmaConstants.INDICADOR_S)) {
                ...
                log.info("VariablesCaja.vRespuestaExito :" + 
    ...
    public void validarConexionADMCentral(){            
        ...
        if(VariablesCaja.vIndLineaADMCentral.trim().
           ...
           //log.debug("No existe linea se cerrara la conexion en ADMCentral ...");
           log.info("***cerrar toda conexion remota ... " +
