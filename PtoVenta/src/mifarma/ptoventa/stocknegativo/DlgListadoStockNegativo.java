...
public class DlgListadoStockNegativo extends JDialog {
    ...
    private void initialize() {
        ftm_stockNeg =
                new FarmaTableModel(ConstantsStockNegativo.columnsListaStockNeg, ConstantsStockNegativo.defaultListaStockNeg,
                                    0);
        FarmaUtility.initSimpleList(tbl_stockNeg, ftm_stockNeg, ConstantsStockNegativo.columnsListaStockNeg);

        //se indica la fecha final como el presente dia, y la inicial dos meses antes
        DecimalFormat f = new DecimalFormat("00");
        Calendar c = Calendar.getInstance();
        String fecFin =
            f.format(c.get(Calendar.DAY_OF_MONTH)) + "/" + f.format(c.get(Calendar.MONTH) + 1) + "/" + c.get(Calendar.YEAR);
    ...
    private void listarSolicitudes() {
        ...
        ArrayList<ArrayList<String>> resultado =
            facade.listadoSolStockNeg(txtSolicitud.getText(), tmpEst, txtSolicitante.getText(), txtAprobador.getText(),
    ...
    private boolean validar() {
        ...
        if (((txtFechaInicio.getText() == null || txtFechaInicio.getText().trim().equals("")) &&
            ...
            FarmaUtility.showMessage(this,
                                     "ATENCIÓN: Para realizar busquedas de fecha \ndeberá llenar los dos campos de fecha o ninguno",
                                     null);
            flag = false;
        }
        if (flag) {
            if (txtFechaInicio.getText() != null && !txtFechaInicio.getText().trim().equals("")) {
                try {
                    fecha_inicial.setTime(FarmaUtility.getStringToDate(txtFechaInicio.getText().trim(), "dd/MM/yyyy"));
        ...
        if (flag) {
            if (FarmaUtility.diferenciaEnDias(fecha_inicial, fecha_final) > 0) {
                flag = false;
                FarmaUtility.showMessage(this, "ERROR: La fecha inicial no debe ser mayor a la fecha final", null);
            }
        }
        return flag;
    }
