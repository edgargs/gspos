...
import mifarma.ptoventa.controlingreso.reference.ConstantsControlIngreso;
import mifarma.ptoventa.controlingreso.reference.DBControlIngreso;
import mifarma.ptoventa.controlingreso.reference.VariablesControlIngreso;
...
public class DlgControlIngreso extends JDialog {
    ...
    private void initTable() {
        tableModel =
                new FarmaTableModel(ConstantsControlIngreso.columnsListaRegistro, ConstantsControlIngreso.defaultValuesListaRegistro,
                                    0);
    ...
    private void txtDni_keyPressed(KeyEvent e) {
        ...
        //CHUANES 24.02.2015
        //PERMITE SOLO EL USO DEL ESCANEO 
         VariablesControlIngreso.vflag=false;
        this.setIndicador(indicadorIngreso());
        if(this.getIndicador().toUpperCase().equalsIgnoreCase("S")){
            if(isTeclaPermitida(e)){
            if (isLectoraLazer) {
           txtDni.setText("");
            ...
            isEnter = true;
            tmpT1 = 0;
            VariablesControlIngreso.vIndicador="S";
            VariablesControlIngreso.vflag=true;
            if (buscaDni()) {
            ...
            if (e.getKeyCode() == KeyEvent.VK_ENTER) {
                ...
                if ((tmpT2 - tmpT1) <= vTiempoMaquina && txtDni.getText().length() > 0 ) {
                    ...
                    VariablesControlIngreso.vIndicador="N";
    ...
    private void chkKeyPressed(KeyEvent e) {
        ...
        if ((mifarma.ptoventa.reference.UtilityPtoVenta.verificaVK_F11(e)) || (e.getKeyCode() == KeyEvent.VK_ENTER)) {
            ...
            if(this.getIndicador().toUpperCase().equalsIgnoreCase("S")){
                if(VariablesControlIngreso.vflag){
                grabarRegisto();
                }else{
                    txtDni.setText("");
                    limpiarDatos();
    ...
    private void determinaTipoRegistro() {
        ...
        FarmaLoadCVL.setSelectedValueInComboBox(cmbTipo, ConstantsControlIngreso.CMB_TIPO_REG,
                                                VariablesControlIngreso.vSugTipo);
    }
    ...
    private void limpiarDatos() {
        ...
        lblIndFiscalizado.setText("");
        FarmaLoadCVL.setSelectedValueInComboBox(cmbTipo, ConstantsControlIngreso.CMB_TIPO_REG,
                                                ConstantsControlIngreso.TIPO_ENTRADA);
        VariablesControlIngreso.vCodCia = "";
        VariablesControlIngreso.vCodTrab = "";
        VariablesControlIngreso.vCodHor = "";
        VariablesControlIngreso.vSugTipo = "";
    ...
    private void grabarRegisto() {
        ...
        if (buscaDni()) {
            if (existeRegistro()) {
                String vTipo =
                    FarmaLoadCVL.getCVLCode(ConstantsControlIngreso.CMB_TIPO_REG, cmbTipo.getSelectedIndex());
                        ...
                        if (VariablesControlIngreso.vExisteCarne.trim().equalsIgnoreCase("S")) {
                            if (VariablesControlIngreso.vFechaVencCarne.trim().equals("NV")) //No ha vencido el carne o no está proximo de vencer
                                ...
                                if (VariablesControlIngreso.vMensajeTiempoVencimiento.trim().equalsIgnoreCase("V"))
                                    vAvisoTrabLocal =
                                            "Le recordamos que su Carné de Sanidad caducó el :" + VariablesControlIngreso.vFechaVencCarne;

                                else
                                    vAvisoTrabLocal =
                                            "Le recordamos que su Carné de Sanidad caduca el :" + VariablesControlIngreso.vFechaVencCarne;
                            }
                    ...
                    if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this, vAvisoTrabLocal + "\n" +
                        ...
                        try {
                            ...
                            DBControlIngreso.grabarRegistro(txtDni.getText().trim(), vTipo,
                                                            VariablesControlIngreso.vCodCia,
                                                            VariablesControlIngreso.vCodTrab,
                                                            VariablesControlIngreso.vCodHor,VariablesControlIngreso.vIndicador);//chuanes 03.03.2015
    ...
    private boolean ValidaRolTrabLocal() {
        ...
        try {
            ...
            result =
                    DBControlIngreso.ValidaRolTrabLocal(SecUsu, ConstantsControlIngreso.ROL_CAJERO, ConstantsControlIngreso.ROL_VENDEDOR,
                                                        ConstantsControlIngreso.ROL_QF_ADMINLOCAL);
            if (result.equalsIgnoreCase("N")) {
                valor = false;
    ...
    public String indicadorIngreso(){
       ...
       String indicador="";
        try {
            indicador = DBControlIngreso.indicadorIngreso().trim();
        } catch (SQLException e) {
