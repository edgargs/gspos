CREATE OR REPLACE PACKAGE PTOVENTA."PTOVENTA_CAJ_ANUL" AS
  ...
  --                      devolvera N si presenta error, G cuando se trate de una guia y el nro de nc en los demas casos
   FUNCTION CAJ_AGREGAR_DET_NC(cCodGrupoCia_in IN CHAR,
                                       ...
                                       )RETURN VTA_COMP_PAGO.NUM_COMP_PAGO%TYPE
   ;
...
--                   IF v_rCurProdVirtual.FECHA > 0.007 THEN --0.007 INDICA 10 MINUTOS
  ...
  AS
    ...
    vCodCia CHAR(3);
    vNumPedNC VARCHAR2(1000);
    -- KMONCADA 28.06.2016 [JORSA] ANULACION CON JORSA DEBE SER CON NC
    vValTipoCambio VTA_PEDIDO_VTA_CAB.VAL_TIP_CAMBIO_PED_VTA%TYPE;
    vNumCompPago VARCHAR2(10);
    vCont number := 0;
  BEGIN
    ...
    WHERE COD_GRUPO_CIA = cCodGrupoCia_in
    AND COD_LOCAL = cCodLocal_in;
    -- KMONCADA 28.06.2016 [JORSA] ANULACION CON JORSA DEBE SER CON NC SINO DEBE SER EL METODO DE ANULACION NORMAL
    IF vCodCia='008' THEN
      SELECT VAL_TIP_CAMBIO_PED_VTA
      INTO vValTipoCambio
      FROM VTA_PEDIDO_VTA_CAB
      WHERE COD_GRUPO_CIA = cCodGrupoCia_in
      AND COD_LOCAL = cCodLocal_in
      ...
      vNumPedNC := CAJ_AGREGAR_CAB_NOTA_CREDITO(cCodGrupoCia_in => cCodGrupoCia_in,
                                                cCodLocal_in => cCodLocal_in,
                                                cNumVtaAnt_in => cNumPedVta_in,
                                                nTipoCam_in => vValTipoCambio,
                                                vIdUsu_in => vIdUsu_in,
                                                nNumCajaPago_in => nNumCajaPago_in,
                                                cMotivoAnulacion => cMotivoAnulacion_in);
       
      
       FOR LISTA IN(
             SELECT REPLACE((REPLACE((EXTRACTVALUE(xt.column_value, 'e')),'Ã','&')),'Ë','<') NUM_PEDIDO --, '9','I','N','N'
        FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' ||
          ...
          vNumCompPago := CAJ_AGREGAR_DET_NC(cCodGrupoCia_in => cCodGrupoCia_in,
                             cCodLocal_in => cCodLocal_in,
                             cNumVtaAnt_in => cNumPedVta_in,
                             cNumVta_in => LISTA.NUM_PEDIDO,
                             cCodProd_in => '0',
                             nCantProd_in => 0,
                             nTotal_in => 0.00,
                             vIdUsu_in => vIdUsu_in,
           ...
           WHERE COD_GRUPO_CIA =  cCodGrupoCia_in
           AND COD_LOCAL = cCodLocal_in
           AND NUM_PED_VTA = LISTA.NUM_PEDIDO;
        END LOOP;
        RETURN ;
    END IF;
...
-- dubilluz 27.09.2013
  ...
  /* *********************************************************************** */
   FUNCTION CAJ_AGREGAR_DET_NC(cCodGrupoCia_in     IN CHAR,
     ...
     RETURN VTA_COMP_PAGO.NUM_COMP_PAGO%TYPE IS
     V_NUM_COMP_PAGO_NC VTA_COMP_PAGO.NUM_COMP_PAGO%TYPE DEFAULT 'N';
  --AS
    CURSOR curProd IS
        ...
        if vSecCompPago_in = v_rCurProd.Sec_Comp_Pago then
            ...
            INSERT INTO VTA_PEDIDO_VTA_DET(COD_GRUPO_CIA,
                ...
                VALUES(v_rCurProd.COD_GRUPO_CIA,
                     ...
                     v_rCurProd.PTOS_AHORRO_PACK/*,
                     -- KMONCADA 20.04.2016 PERCEPCION
                     v_rCurProd.VAL_PCT_PERCEPCION,
                     v_rCurProd.VAL_MONTO_PERCEPCION*/
                  );
            end if;
    ...
    IF vTipoComprobantePago <> '03' THEN
	 ...
	 v_nNumCompPago := F_GET_SERIE_AUX_LOCAL(cCodGrupoCia_in, cCodLocal_in)||TRIM(v_nNumCompPago);
      ...
      INSERT INTO VTA_COMP_PAGO(
        ...
        SEC_COMP_PAGO,             TIP_COMP_PAGO,                  NUM_COMP_PAGO,
        SEC_MOV_CAJA,              CANT_ITEM,                      COD_CLI_LOCAL, 
        NOM_IMPR_COMP,             DIREC_IMPR_COMP,                NUM_DOC_IMPR, 
        VAL_BRUTO_COMP_PAGO,       VAL_NETO_COMP_PAGO,             
        VAL_DCTO_COMP_PAGO,
        VAL_AFECTO_COMP_PAGO,      VAL_IGV_COMP_PAGO,              VAL_REDONDEO_COMP_PAGO, 
        USU_CREA_COMP_PAGO,        PORC_IGV_COMP_PAGO,             IND_AFECTA_KARDEX,
        /* INICIO LTAVARA SE AGREGO PARA COMPROBANTE ELECTRONICO 30-07-2014   */
        COD_TIP_PROC_PAGO,         COD_TIP_IDENT_RECEP_E,          
        NUM_COMP_PAGO_EREF,-- NUMERO DE COMPROBANTE DE REFERENCIA
        COD_TIP_COMP_PAGO_EREF,    
        COD_TIP_MOTIVO_NOTA_E,          VAL_TOTAL_E,
        TOTAL_GRAV_E,              TOTAL_INAF_E,                   TOTAL_GRATU_E,
        TOTAL_EXON_E,              TOTAL_DESC_E,                   TOTAL_IGV_E,
        COD_TIP_MONEDA,
        /* FIN LTAVARA SE AGREGO PARA COMPROBANTE ELECTRONICO 30-07-2014   */
        -- 23/12/2014 ltavara
        VAL_COPAGO_E,              VAL_COPAGO_COMP_PAGO,           TIP_CLIEN_CONVENIO,
        PCT_BENEFICIARIO,          PCT_EMPRESA/*,
        --KMONCADA 20.04.2016 PERCEPCION
        VAL_PERCEP_COMP_PAGO,      VAL_REDONDEO_PERCEPCION*/
      )
      select  
        cCodGrupoCia_in,           cCodLocal_in,                   cNumVta_in,
        v_nSecCompPago,            '04',                           vNumCompPago_in,
        v_cSecMovCaja_in,          CANT_ITEM,                      cp.COD_CLI_LOCAL, 
        NOM_IMPR_COMP,             DIREC_IMPR_COMP,                NUM_DOC_IMPR, 
        --KMONCADA 25.04.2016 [PERCEPCION] LA ANULACION NO FIGURA CON LA PERCEPCION
        VAL_BRUTO_COMP_PAGO*-1,    (VAL_NETO_COMP_PAGO-NVL(CP.VAL_PERCEP_COMP_PAGO,0))*-1,
        VAL_DCTO_COMP_PAGO*-1,
        VAL_AFECTO_COMP_PAGO*-1,   VAL_IGV_COMP_PAGO*-1,           VAL_REDONDEO_COMP_PAGO*-1, 
        USU_CREA_COMP_PAGO,        PORC_IGV_COMP_PAGO,             IND_AFECTA_KARDEX,
        /* INICIO LTAVARA SE AGREGO PARA COMPROBANTE ELECTRONICO 30-07-2014  */
        CP.COD_TIP_PROC_PAGO,      CP.COD_TIP_IDENT_RECEP_E,   
        (CASE
         ...
         END),
        DECODE(cp.TIP_COMP_PAGO,'02','1',
                                '06','1',
                                '01','3',
                                '05','3'),
        cCodTipMotivoNotaE,        cp.VAL_TOTAL_E,                 cp.TOTAL_GRAV_E,
        cp.TOTAL_INAF_E,           cp.TOTAL_GRATU_E,               cp.TOTAL_EXON_E,
        cp.TOTAL_DESC_E,           cp.TOTAL_IGV_E,                 cp.COD_TIP_MONEDA,
        /* FIN LTAVARA SE AGREGO PARA COMPROBANTE ELECTRONICO 30-07-2014   */
        -- 23/12/2014 ltavara
        cp.val_copago_e,           cp.val_copago_comp_pago,        cp.tip_clien_convenio,
        cp.pct_beneficiario,       cp.pct_empresa/*,
        -- KMONCADA 20.04.2016 PERCEPCION
        CP.VAL_PERCEP_COMP_PAGO,   CP.VAL_REDONDEO_PERCEPCION*/
     FROM VTA_COMP_PAGO CP
     WHERE CP.COD_GRUPO_CIA = cCodGrupoCia_in
     AND cp.cod_local = cCodLocal_in
     AND cp.num_ped_vta = cNumVtaAnt_in
     ...
     /* *********************** FIN ******************************************* */
      ...
      V_NUM_COMP_PAGO_NC := vNumCompPago_in;
    ELSE
...
/*
       ...
       -- SI ES CONVENIO
       if isConvenio = 'S' then
       -- como : LAS NC es TOTAL DE TODO EL PEDIDO
       -- SE HARA UNA COPIA DE LAS FORMAS DE PAGO del ORIGINAL
       -- SOLES y CREDITO (PINPAD pasa a Soles, Dolares a Soles,Credito a Credito)
        -- cantidad de monto pagado FORMA DE PAGO NC
		-- agrega forma de pago de PEDIDO
       select  ca.val_neto_ped_vta*-1
       into    vNetoPedido_in
       from    vta_pedido_vta_cab ca
       where   ca.cod_grupo_cia = cCodGrupoCia_in
       and     ca.cod_local = cCodLocal_in
       and     ca.num_ped_vta = cNumVtaAnt_in;
        ...
        select c.im_total_pago*-1
        into   vImptotalCredito
        from   vta_forma_pago_pedido c
        where  c.cod_grupo_cia =cCodGrupoCia_in
        and    c.cod_local = cCodLocal_in
        and    c.num_ped_vta = cNumVtaAnt_in
        and    c.cod_forma_pago = '00080';-- CREDITO BTL MF

        EXCEPTION
        WHEN NO_DATA_FOUND THEN
            vImptotalCredito:=0.000; --RHEREREA : si no encuentra forma de pago 00080,
                                     -- resta valor 0.000
        END;
        vImptotalSoles := vNetoPedido_in - vImptotalCredito;

        select count(1)
        into   nEx_pedNC_DOBLE
        from  vta_pedido_vta_cab c
        where  c.cod_grupo_cia =cCodGrupoCia_in
        and    c.cod_local = cCodLocal_in
        and    c.num_ped_vta_origen = cNumVtaAnt_in ;
      --ERIOS 2.4.3 Valida el primer documento
	  --if nEx_pedNC_DOBLE = 1 then
      if nSecDetPed_in = 0 then
              if abs(vImptotalCredito) > 0 then
                 INSERT INTO VTA_FORMA_PAGO_PEDIDO(COD_GRUPO_CIA,COD_LOCAL,
                COD_FORMA_PAGO,NUM_PED_VTA,IM_PAGO,TIP_MONEDA,
                VAL_TIP_CAMBIO,VAL_VUELTO,IM_TOTAL_PAGO,
                USU_CREA_FORMA_PAGO_PED  )
                VALUES(cCodGrupoCia_in,cCodLocal_in,
                '00080',cNumVta_in,vImptotalCredito,'01',--SOLES
                0,0,vImptotalCredito,vIdUsu_in);
             end if;

             if abs(vImptotalSoles) > 0 then
             -- kmoncada 26.06.2014  para grabar forma_pago credito en vta institucional
                SELECT a.tip_ped_vta
                INTO   vTipoPedido
                FROM   vta_pedido_vta_cab a
                WHERE  a.COD_GRUPO_CIA = cCodGrupoCia_in
                AND    a.COD_LOCAL = cCodLocal_in
                AND    a.NUM_PED_VTA = cNumVtaAnt_in;

                IF vTipoPedido = '03' THEN
                   ...
                   USU_CREA_FORMA_PAGO_PED  )
                   VALUES(cCodGrupoCia_in,cCodLocal_in,
                   '00050',cNumVta_in,vImptotalSoles,'01',--SOLES
                   0,0,vImptotalSoles,vIdUsu_in);
                ELSE
                   INSERT INTO VTA_FORMA_PAGO_PEDIDO(COD_GRUPO_CIA,COD_LOCAL,
                   COD_FORMA_PAGO,NUM_PED_VTA,IM_PAGO,TIP_MONEDA,
                   VAL_TIP_CAMBIO,VAL_VUELTO,IM_TOTAL_PAGO,
                   USU_CREA_FORMA_PAGO_PED  )
                   VALUES(cCodGrupoCia_in,cCodLocal_in,
                   g_cFormPagoEfectivo,cNumVta_in,vImptotalSoles,'01',--SOLES
                   0,0,vImptotalSoles,vIdUsu_in);
                END IF;
             end if;
	 ELSE
       ...
       -- SI ES MEZON
          ...
          USU_CREA_FORMA_PAGO_PED  )
          VALUES(cCodGrupoCia_in,cCodLocal_in,
          g_cFormPagoEfectivo,cNumVta_in,vNetoPedido_in,'01',--SOLES
          0,0,vNetoPedido_in,vIdUsu_in);
       end if;
*/
      --SUMAR CABECERA
...
--- hace update de cabecera igual a suma de detalle ---
  ...
  END;
  /* ********************************************************************************************** */
  /*PROCEDURE CAJ_AGREGAR_DET_NOTA_CREDITO(cCodGrupoCia_in IN CHAR,cCodLocal_in IN CHAR,cNumVtaAnt_in IN CHAR,
  cNumVta_in IN CHAR,cCodProd_in IN CHAR,nCantProd_in IN NUMBER,nTotal_in IN NUMBER,vIdUsu_in IN VARCHAR2,
  nSecDetPed_in IN NUMBER)
  AS
    CURSOR curProd IS
    SELECT D.COD_GRUPO_CIA,
            ...
            D.IND_ORIGEN_PROD,
            L.VAL_FRAC_LOCAL,
            D.PORC_ZAN,       -- 2009-11-09 JOLIVA
            --JCORTEZ 11.11.09
            NVL(D.COD_GRUPO_REP,' ') COD_GRUPO_REP,
            NVL(D.COD_GRUPO_REP_EDMUNDO,' ') COD_GRUPO_REP_EDMUNDO,
            D.COD_PROM, --ASOSA, 21.05.2010
            D.COD_CAMP_CUPON, --ASOSA, 21.05.2010
            --Agregado Por FRAMIREZ 12.06.2012
            D.AHORRO_CONV,
            D.NUM_COMP_PAGO,
            D.SEC_COMP_PAGO_BENEF,
    ...
    WHERE D.COD_GRUPO_CIA = cCodGrupoCia_in
          ...
          AND D.SEC_PED_VTA_DET = nSecDetPed_in
          AND D.COD_GRUPO_CIA = L.COD_GRUPO_CIA
          AND D.COD_LOCAL = L.COD_LOCAL
          AND D.COD_PROD = L.COD_PROD;
...
\*    SELECT NVL(X.COD_GRUPO_REP,' '),NVL(X.COD_GRUPO_REP_EDMUNDO,' ')
    ...
    AND X.COD_PROD=v_rCurProd.COD_PROD;
      *\
      ...
      VALUES(v_rCurProd.COD_GRUPO_CIA,
            ...
            v_rCurProd.PORC_DCTO_TOTAL,
            v_rCurProd.EST_PED_VTA_DET,
            (v_rCurProd.VAL_TOTAL_BONO*nCantProd_in*-1)/v_rCurProd.CANT_ATENDIDA,
            v_rCurProd.VAL_FRAC,
            NULL,
            v_rCurProd.SEC_USU_LOCAL,
            vIdUsu_in,
            ...
            v_rCurProd.IND_ORIGEN_PROD,
            v_rCurProd.VAL_FRAC_LOCAL, --ERIOS 05/06/2008
            ( (nCantProd_in*v_rCurProd.VAL_FRAC_LOCAL)/v_rCurProd.VAL_FRAC )*-1,
            v_rCurProd.COD_GRUPO_REP,v_rCurProd.COD_GRUPO_REP_EDMUNDO, --JCORTEZ 19.10.09
            v_rCurProd.PORC_ZAN,      -- 2009-11-09 JOLIVA
            v_rCurProd.Cod_Prom,      --ASOSA, 21.05.2010
            v_rCurProd.Cod_Camp_Cupon, --ASOSA, 21.05.2010
    ...
    --ANULAR DETALLE
      UPDATE VTA_PEDIDO_VTA_DET SET USU_MOD_PED_VTA_DET = vIdUsu_in, FEC_MOD_PED_VTA_DET = SYSDATE,
           CANT_USADA_NC = CANT_USADA_NC + nCantProd_in
      WHERE COD_GRUPO_CIA = cCodGrupoCia_in
            AND COD_LOCAL = cCodLocal_in
            AND NUM_PED_VTA = cNumVtaAnt_in
            AND COD_PROD = cCodProd_in
            AND SEC_PED_VTA_DET = nSecDetPed_in;

      --SUMAR CABECERA
      UPDATE VTA_PEDIDO_VTA_CAB SET USU_MOD_PED_VTA_CAB = vIdUsu_in,FEC_MOD_PED_VTA_CAB = SYSDATE,
      --SET --VAL_BRUTO_PED_VTA = VAL_BRUTO_PED_VTA+(nCantProd_in*-1*v_rCurProd.VAL_PREC_LISTA),
          --VAL_NETO_PED_VTA = VAL_NETO_PED_VTA+(nCantProd_in*-1*v_rCurProd.VAL_PREC_VTA),
          --VAL_DCTO_PED_VTA = (VAL_BRUTO_PED_VTA+(nCantProd_in*-1*v_rCurProd.VAL_PREC_LISTA))-(VAL_NETO_PED_VTA+(nCantProd_in*-1*v_rCurProd.VAL_PREC_VTA)),
          --VAL_IGV_PED_VTA=VAL_IGV_PED_VTA+((nCantProd_in*-1*v_rCurProd.VAL_PREC_VTA)-((nCantProd_in*-1*v_rCurProd.VAL_PREC_VTA)/((100+v_rCurProd.VAL_IGV)/100))),
          CANT_ITEMS_PED_VTA = v_nSec
      WHERE COD_GRUPO_CIA = cCodGrupoCia_in
            AND COD_LOCAL = cCodLocal_in
            AND NUM_PED_VTA = cNumVta_in;
