...
public class DlgListado extends JDialog {
    ...
    private int indTipoListado = 0;
    private List lstDatos;
    private ArrayList rowSelected;
    ...
    public DlgListado(Frame parent, String title, boolean modal, int indTipoListado) {
        super(parent, title, modal);
        this.indTipoListado = indTipoListado;
        constructorClase(parent, title);
    }
    
    private void constructorClase(Frame parent, String title){
        myParentFrame = parent;
        try {
            jbInit();
            initialize();
    ...
    private void initTable() {
        tableModel =
                new FarmaTableModel(ConstantsProducto.columnsListaLaboratorios, ConstantsProducto.defaultValuesListaLaboratorios,
                                    0);
    ...
    private void iniciarContenido(){
        if(this.indTipoListado == 1 ){
            FarmaColumnData[] columnas = { new FarmaColumnData("Piso", 70, JLabel.CENTER), 
                                           new FarmaColumnData("Descripcion", 280, JLabel.LEFT) ,
                                           new FarmaColumnData("Posicion", 0, JLabel.LEFT) 
                                         };
            Object[] defaultValues = { " ", " ", " " };
            tableModel = new FarmaTableModel(columnas, defaultValues, 0);
            FarmaUtility.initSimpleList(tblListaLaboratorios, tableModel, columnas);
            
            for(int i= 0; i<lstDatos.size();i++){
                String nroPiso = (String)((Map)((HashMap)lstDatos.get(i))).get("PISO");
                ArrayList<String> aux = new ArrayList<String>(Arrays.asList(nroPiso, "Comanda Piso "+nroPiso, i+""));
                tableModel.data.add(aux);
            }
            tblListaLaboratorios.repaint();
        }
    }
    ...
    private void seleccion() {
        ...
        if (!(FarmaUtility.findTextInJTable(tblListaLaboratorios, txtLaboratorio.getText().trim(), COL_COD,
                                            COL_NOMB))) {
        ...
        if (e.getKeyCode() == KeyEvent.VK_ENTER) {
            ...
            if (txtLaboratorio.getText().trim().length() < DIG_COD) {
                if (FarmaUtility.isInteger(txtLaboratorio.getText().trim())) {
                    if (indTipoListado == 0) {
                        txtLaboratorio.setText(FarmaUtility.completeWithSymbol(txtLaboratorio.getText(), DIG_COD, "0",
                                                                               "I"));
                    }
                }
    ...
    private void seleccionarElemento() {
        if (tblListaLaboratorios.getRowCount() > 0) {
            if (!(FarmaUtility.findTextInJTable(tblListaLaboratorios, txtLaboratorio.getText().trim(), COL_COD,
                                                COL_NOMB))) {
                FarmaUtility.showMessage(this, "Elemento no encontrado según criterio de búsqueda !!!",
                                         txtLaboratorio);
                return;
            } else {
                ...
                if (row > -1) {
                    if(indTipoListado==0){
                        ...
                        rowSelected = (ArrayList)tableModel.data.get(row);
                    }
    ...
    public void cargarDatosTabla(List lstDatos){
        this.lstDatos = lstDatos;
    }
    
    public ArrayList getRowSelected(){
