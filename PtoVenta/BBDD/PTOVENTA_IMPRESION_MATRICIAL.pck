...
CREATE OR REPLACE PACKAGE BODY PTOVENTA_IMPRESION_MATRICIAL is
  ...
  FUNCTION F_GET_GUIA_ARCANGEL(cCodGrupoCia_in    IN VTA_COMP_PAGO.COD_GRUPO_CIA%TYPE,
      ...
      FROM (
        SELECT REPLACE((REPLACE((EXTRACTVALUE(xt.column_value, 'e')),'Ã','&')),'Ë','<') VAL --, '9','I','N','N'
          ...
          SELECT ' ' ||'@@'||
                 ...
                 FARMA_PRINTER.F_GET_TEXTO_MATRICIAL(X.FECHA, 45) ||
                 FARMA_PRINTER.F_GET_TEXTO_MATRICIAL(X.NRO_COMPROBANTE, 15)||'@@'||
                 FARMA_PRINTER.F_GET_TEXTO_MATRICIAL(' ', 17) ||
          ...
          FROM (
            SELECT
               ...
               CASE
                 WHEN CAB.TIP_PED_VTA = '03' THEN  SUBSTR(CAB.DIR_CLI_PED_VTA,0,72)
                 ELSE   SUBSTR(CP.DIREC_IMPR_COMP,0,72)
               END      AS "DIR_CLIENTE",               
               -- SUBSTR(CP.NOM_IMPR_COMP,0,73) NOM_CLIENTE,
               CASE
                 WHEN CAB.TIP_PED_VTA = '03' THEN  SUBSTR(CAB.NOM_CLI_PED_VTA,0,73)
                 ELSE   SUBSTR(CP.NOM_IMPR_COMP,0,73)
               END      AS "NOM_CLIENTE",
               SUBSTR(CP.NUM_DOC_IMPR,0,73) DOC_CLIENTE,
               TO_CHAR(CP.FEC_CREA_COMP_PAGO, 'DD/MM/YYYY HH24:MI:SS') FECHA,
               farma_utility.get_t_comprobante_2(CP.TIP_COMP_PAGO, CP.NUM_COMP_PAGO, CP.NUM_COMP_PAGO) NRO_COMPROBANTE,
               NVL(( SELECT 'DOC.REF.: '||TCP.COD_TIPODOC||' '||
                            DECODE(REF.COD_TIP_PROC_PAGO,'1', REF.NUM_COMP_PAGO_E, REF.NUM_COMP_PAGO)
                            || '     '||
                            ' OC: '|| 
                            (
                            select nvl(g.num_oc,'')
                            from   vta_pedido_vta_cab_emp  g
                            where  g.cod_grupo_cia = ref.cod_grupo_cia
                            and    g.cod_local = ref.cod_local
                            and    g.num_ped_vta = ref.num_ped_vta
                            )
    ...
    CURSOR curDetalle IS
      SELECT
        case
          when vIsVtaInstitucional = 1 then
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL(X.COD_PROD, 11, pAlineacion_in => 'C') || ' '||
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL('('||X.nom_lab||')', 6, pAlineacion_in => 'D') || ' '||
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL(X.DESCRIPCION,61) || ' ' ||
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL(X.UNID_VTA, 20) || ' ' ||
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL(X.CANTIDAD, 10, pAlineacion_in => 'C') || ' '||                
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL(X.NUM_LOTE, 15, pAlineacion_in => 'C') || ' ' ||
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL(X.FECHA_LOTE, 15, pAlineacion_in => 'C') 
          else  
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL(X.COD_PROD, 11, pAlineacion_in => 'C') || ' '||
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL(X.CANTIDAD, 10, pAlineacion_in => 'C') || ' '||
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL(X.DESCRIPCION,61) || ' ' ||
                ...
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL(X.SUB_TOTAL, 15, pAlineacion_in => 'D') 
        END        
          as  VALOR,
                MTO_SUB_TOTAL
      FROM (
          SELECT
             ...
             CASE
               ...
               ELSE
                 ROUND(DET.VAL_PREC_TOTAL * 100 / (DET.VAL_IGV + 100), 3)
             END MTO_SUB_TOTAL,
             det.num_lote_prod NUM_LOTE,
             to_char(det.fec_vencimiento_lote,'dd/mm/yyyy') FECHA_LOTE,
             la.nom_lab
            FROM VTA_PEDIDO_VTA_CAB CAB,
  ...
  BEGIN
    ...
    if vIsVtaInstitucional > 0 then
        ...
        FARMA_PRINTER.P_GRABA_LINEA_DOC(vIdDoc_in => vIdDoc,
                                        vIpPc_in => vIpPc,
                                        vValor_in => 
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL('DESCRIPCION', 11, pAlineacion_in => 'C') || ' '||
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL('LAB)', 6, pAlineacion_in => 'D') || ' '||
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL('PRODUCTO',61) || ' ' ||
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL('UNIDAD', 20) || ' ' ||
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL('CANTIDAD', 10, pAlineacion_in => 'C') || ' '||                
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL('LOTE', 15, pAlineacion_in => 'C') || ' ' ||
                FARMA_PRINTER.F_GET_TEXTO_MATRICIAL('FCH VENCIMIENTO', 15, pAlineacion_in => 'C')
                                        );
    end if;
    ...
    -- DUBILLUZ 16.08.2016
    ----- NO DEBE IMPRIMIR SI ES INSITITUCIONAL ------ 
