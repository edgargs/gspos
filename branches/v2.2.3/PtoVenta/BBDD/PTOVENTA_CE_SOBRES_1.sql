--------------------------------------------------------
--  DDL for Package Body PTOVENTA_CE_SOBRES
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "PTOVENTA"."PTOVENTA_CE_SOBRES" AS
   ...
   IF cantidad = 0 THEN
    ...
    if nExiteRegistro = 0 then
     ...
     IF(LENGTH(TRIM(cCodSobre_in))>0)THEN
      ...
      ,COD_FORMA_PAGO,SEC_USU_QF,cod_remito --DUBILLUZ 27.07.2010
      )
      SELECT A.COD_SOBRE,A.COD_GRUPO_CIA,A.COD_LOCAL,A.FEC_DIA_VTA,cSecCaja_in,cSecuencialFormaPago,
             ...
             ,COD_FORMA_PAGO,SEC_USU_QF,a.cod_remito --DUBILLUZ 27.07.2010
      FROM CE_SOBRE_TMP A
...
--       AND S.ESTADO = 'A'
  ...
  FUNCTION SEG_F_VAR_IMP_HTML_SOBRES(cCodGrupoCia_in IN CHAR,
    ...
    CURSOR curSobres IS
      SELECT S.COD_SOBRE,
             DECODE(S.TIP_MONEDA, '01', 'SOLES', 'DOLARES') AS MONEDA,
             --TRIM(to_char(S.MON_ENTREGA, '999999999.00')) AS MONTO antes
             TRIM(to_char(S.MON_ENTREGA, '999,999,990.00')) AS MONTO --ASOSA, 04.08.2010
        FROM CE_SOBRE_TMP S--, CE_FORMA_PAGO_ENTREGA FPE
       WHERE S.COD_GRUPO_CIA = cCodGrupoCia_in
         AND S.COD_LOCAL = cCodLocal_in
...
--      DBMS_OUTPUT.put_line('html:' || msgHTML);
      msgHTMLDet := msgHTMLDet || '<tr><td class="cajero">Nro Sobre: ' ||trim(rSobre.COD_SOBRE) ||'</td></tr>'
    ...
    UPDATE CE_SOBRE_TMP TT --/ ASOSA, 26.07.2010
    SET TT.IND_IMP='S'
    WHERE TT.COD_GRUPO_CIA=cCodGrupoCia_in
    AND TT.COD_LOCAL=cCodLocal_in
    AND TT.SEC_MOV_CAJA=cMovCaja_in
    AND TT.COD_SOBRE=cCodSobre_in
    AND TT.ESTADO='A';
    COMMIT;
  ...
  /******************************************************************************/
  --Ahora estos son los cambios que se solicitaron hace 15minutos para ahorita

/*     - Imprimir con clave de QF en la creacion de sobres en las dos pantallas de control de sobres
     	   y de cierre parcial, e inmediatamente despues se imprima los sobres y se creen aprobados.
       - Los sobres que ya se imprimieron altoq que ya no se impriman en cierre de turno.*/
  FUNCTION SEG_F_CUR_GET_SOBRES(cCodGrupoCia_in IN CHAR,
                                cCodLocal_in    IN CHAR,
                                cMovCaja_in     IN CHAR,
                                cCodSobre_in    IN CHAR) RETURN FarmaCursor IS
 ...
 BEGIN
  ...
  BEGIN
     ...
     IF(cIndicador='S' OR indProsegur='S')THEN
       ...
       IF nCantidad>0 OR nCantidad3 >0THEN -- Se verifica la existencia del unico sobre

       --solo por locales con prosegur, ya que para ellos unicamente se crearan remitos
       SELECT COUNT(*) INTO nCantidad2
       FROM CE_REMITO A
       WHERE A.COD_GRUPO_CIA=cCodGrupoCia_in
       AND A.COD_LOCAL=cCodLocal_in
       AND TRUNC(A.FEC_CREA_REMITO)=cFechaCierreDia;

       SELECT nvl(P.LLAVE_TAB_GRAL, 'NO SE PUEDE ELIMINAR ESTE SOBRE YA CERRADO, ASOCIADO A UN REMITO.')
          INTO mensaje
          FROM PBL_TAB_GRAL P
       ...
       SELECT nvl(P.LLAVE_TAB_GRAL, 'SE ESTA ELIMINANDO UN SOBRE YA CERRADO')
         ...
         mensaje:=mensaje||'                                                               '||'.'||'.';--rango permitido 63 espac
         mensaje2:=mensaje2||'                                                               '||'.';
...
/* ******************************************************************************* */
  ...
  BEGIN
    ...
    if cTipoAccion_in = ACC_APRUEBA then
      ...
      AND    T.SEC_MOV_CAJA = cSecMovCaja_in;
              ...
              AND    T.SEC_MOV_CAJA = cSecMovCaja_in;
                ...
                -- Se grabara el secuencial del quimico que aprobo el sobre. SEC_USU_QF -- DUBILLUZ 27.07.2010

                INSERT INTO CE_SOBRE_TMP
                (COD_SOBRE,COD_GRUPO_CIA,COD_LOCAL,FEC_DIA_VTA,SEC_MOV_CAJA,SEC,COD_FORMA_PAGO,
                 TIP_MONEDA,MON_ENTREGA,MON_ENTREGA_TOTAL,USU_CREA_SOBRE,USU_MOD_SOBRE,FEC_MOD_SOBRE,ESTADO,IND_IMP,SEC_USU_QF)
                VALUES
                (vCodigoSobre,cCodGrupoCia_in,cCodLocal_in,fecdiaVenta,cSecMovCaja_in,v_numSec,cCodFormaPago_in,
                --cTipMoneda_in,cMonEntrega_in,cMonEntregaTotal_in,cIdUsu_in,NULL,NULL,ESTADO_PENDIENTE);
                cTipMoneda_in,cMonEntrega_in,cMonEntregaTotal_in,cIdUsu_in,NULL,NULL,'A','N',secUsuQf_in); --edmundo queria esto 'ASOSA, 26.07.2010'
   ...
   /* SELECT SUM(X.VAL_NETO_PED_VTA)
    ...
    return TRIM(nvl(vCodigoSobre,'N'));
 ...
 /******************************************************************************/
  ...
  v_nValorTipCambio := FARMA_UTILITY.OBTIENE_TIPO_CAMBIO3(cCodGrupoCia_in,vCodCia,TO_DATE(fechadiaventa,'DD/MM/YYYY'),'V');
...
/** ************************************************************** */
  ...
  FUNCTION SEG_F_CUR_GET_SOBRES_02(cCodGrupoCia_in IN CHAR,
                                cCodLocal_in    IN CHAR,
  ...
  FUNCTION SEG_F_VAR_IMP_HTML_SOBRES_02(cCodGrupoCia_in IN CHAR,
                                     cCodLocal_in    IN CHAR,
                                     cMovCaja_in     IN CHAR,
                                     cCodSobre_in    IN CHAR) RETURN VARCHAR2 IS
    msgHTMLCab VARCHAR2(4000) := '';
...
FUNCTION SEG_F_VAR_IMP_HTML_SOBRES_04(cCodGrupoCia_in IN CHAR,
                                     cCodLocal_in    IN CHAR,
                                     cMovCaja_in     IN CHAR,
                                     cCodSobre_in    IN CHAR) RETURN VARCHAR2 IS
    msgHTMLCab VARCHAR2(4000) := '';
    ...
    CURSOR curSobres IS
      SELECT ee.COD_SOBRE,
             DECODE(xx.TIP_MONEDA, '01', 'SOLES', 'DOLARES') AS MONEDA,
             --TRIM(to_char(xx.mon_entrega_total, '999,999,990.00')) AS MONTO
             TRIM(to_char(xx.mon_entrega, '999,999,990.00')) AS MONTO
        FROM CE_SOBRE ee, ce_forma_pago_entrega xx
       WHERE ee.COD_GRUPO_CIA = cCodGrupoCia_in
         AND ee.COD_LOCAL = cCodLocal_in
         AND ee.SEC_MOV_CAJA = cMovCaja_in
         AND ee.COD_SOBRE = cCodSobre_in
         AND ee.ESTADO = 'A'
         AND ee.cod_grupo_cia = xx.cod_grupo_cia
         AND ee.cod_local = xx.cod_local
         AND ee.sec_mov_caja=xx.sec_mov_caja
         AND ee.sec_forma_pago_entrega=xx.sec_forma_pago_entrega;
    rSobre curSobres%ROWTYPE;
  BEGIN
...
--      DBMS_OUTPUT.put_line('html:' || msgHTML);
  ...
  FUNCTION SEG_F_VAR_IMP_HTML_SOBRES_03(cCodGrupoCia_in IN CHAR,
                                     cCodLocal_in    IN CHAR,
                                     cMovCaja_in     IN CHAR,
                                     cCodSobre_in    IN CHAR) RETURN VARCHAR2 IS
    msgHTMLCab VARCHAR2(4000) := '';
    ...
    CURSOR curSobres IS
      SELECT S.COD_SOBRE,
             DECODE(S.TIP_MONEDA, '01', 'SOLES', 'DOLARES') AS MONEDA,
             --TRIM(to_char(S.MON_ENTREGA, '999999999.00')) AS MONTO
             TRIM(to_char(S.MON_ENTREGA, '999,999,990.00')) AS MONTO --ASOSA, 04.08.2010
        FROM CE_SOBRE_TMP S--, CE_FORMA_PAGO_ENTREGA FPE
       WHERE S.COD_GRUPO_CIA = cCodGrupoCia_in
         AND S.COD_LOCAL = cCodLocal_in
