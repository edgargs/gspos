CREATE OR REPLACE PACKAGE PTOVENTA."PTOVENTA_CE_LMR" is
  ...
  -- KMONCADA 22.06.2016 [CONV FAMISALUD]
  COD_FP_REDONDEO_FAMISALUD      VTA_FORMA_PAGO.COD_FORMA_PAGO%TYPE := '00094';
...
/************************CIERRE DE TURNO*****************************/
 ...
 END;
  ...
  AS
    ...
    vMontoRedondeoFamisalud VTA_FORMA_PAGO_PEDIDO.IM_TOTAL_PAGO%TYPE;
  BEGIN
    ...
    vMontoRedondeoPuntos := PTOVENTA_CE.GET_MONTO_TURNO_PTOS_REDONDEO(cCodGrupoCia_in => cCodGrupoCia_in, 
                                                         ...
                                                         cCodFormaPago_in => COD_FORMA_PAGO_REDONDEO_PTOS);
    -- KMONCADA 22.06.2016 [CONV FAMISALUD]
    vMontoRedondeoFamisalud := PTOVENTA_CE.GET_MONTO_TURNO_PTOS_REDONDEO(cCodGrupoCia_in => cCodGrupoCia_in, 
                                                         cCodLocal_in => cCodLocal_in, 
                                                         cSecMovCaja_in => cMovCajaCierre_in,
                                                         cCodFormaPago_in => COD_FP_REDONDEO_FAMISALUD);
    OPEN curCE FOR
        ...
        SELECT FP.DESC_CORTA_FORMA_PAGO || 'Ã' ||
               'SOLES' || 'Ã' ||
               TO_CHAR(vMontoRedondeoPuntos,'999,990.00') || 'Ã' ||
               TO_CHAR(vMontoRedondeoPuntos,'999,990.00') || 'Ã' ||
               FP.COD_FORMA_PAGO
        ...
        SELECT FP.DESC_CORTA_FORMA_PAGO || 'Ã' ||
               'SOLES' || 'Ã' ||
               TO_CHAR(vMontoRedondeoFamisalud,'999,990.00') || 'Ã' ||
               TO_CHAR(vMontoRedondeoFamisalud,'999,990.00') || 'Ã' ||
               FP.COD_FORMA_PAGO
        FROM VTA_FORMA_PAGO FP
        WHERE vMontoRedondeoFamisalud > 0 
        AND FP.COD_GRUPO_CIA = cCodGrupoCia_in
        AND FP.COD_FORMA_PAGO = COD_FP_REDONDEO_FAMISALUD
...
FUNCTION CE_LISTA_HIST_CIERRE_DIA(cCodGrupoCia_in IN CHAR,
 ...
 SELECT to_number(nvl(A.LLAVE_TAB_GRAL,'60'))
  ...
  BEGIN
    ...
    -- KMONCADA 22.06.2016 [CONV FAMISALUD]
    vMontoRedondeoFamisalud := PTOVENTA_CE.GET_MONTO_DIA_PTOS_REDONDEO(cCodGrupoCia_in => cCodGrupoCia_in, 
                                                                    cCodLocal_in => cCodLocal_in, 
                                                                    cCierreDia_in => cCierreDia_in,
                                                                    cCodFormaPago_in => COD_FP_REDONDEO_FAMISALUD);
    OPEN curCE FOR
...
---FIN
    ...
    -- KMONCADA 23.05.2016 DOS IMPRESORAS POR TIPO DE COMPROBANTE
      SELECT A.TIP_COMP ||'Ã'||
             ...
             A.NUM_INICIAL ||'Ã'||
             A.NUM_FINAL
      FROM CE_COMP_MOV_CAJA A
      WHERE A.COD_GRUPO_CIA = cCodGrupoCia_in
      AND A.COD_LOCAL = cCodLocal_in
      AND A.SEC_MOV_CAJA = cMovCajaCierre_in;
             /*SELECT  trim('01Ã'||
                     \*cCodLocal_in*\
                     (
...
--FIN
  ...
  BEGIN
   -- KMONCADA 26.05.2016
    IF NOT (cNumSerie_in = cCodLocal_in AND cRangoIni_in = '0000000' AND cRangoFin_in = '0000000') THEN
      ...
      if cTipComp_in != '05' then
        ...
        and    ip.sec_impr_local = PTOVENTA_CAJ.F_GET_SECUENCIAL_IMPRESORA(cCodGrupoCia_in, 
                                                                           cCodLocal_in, 
                                                                           cTipComp_in,
    ...
    OPEN curCE FOR
        SELECT     to_date(mc.fec_dia_vta, 'dd/MM/yyyy')|| 'Ã' ||
            ...
            AND    C.EST_CUADRATURA_CAJA='A' -- RHERRERA: FILTRAR LAS CUADRATURAS PARA ESTADO ACTIVO
             ...
             -- KMONCADA 15.01.2015 PARA EL CASO DE MIGRACION DE DATA SOLO VERIFICAR HACIA UN AÑO ATRAS.
  ...
  BEGIN
    OPEN curCE FOR
            ...
            SELECT (SELECT A.DESC_COMP
                    ...
                    AND   A.TIP_COMP = CP.TIP_COMP_PAGO ) || 'Ã' ||
                SUBSTR(CP.NUM_COMP_PAGO_E,1,4) || 'Ã' ||
                SUBSTR(MIN(CP.NUM_COMP_PAGO_E),-8) || 'Ã' ||
...
--FIN
 ...
 /******************************************************************/
  ...
  BEGIN
    ...
    OPEN curCE FOR
      SELECT CAJA || 'Ã' ||CAJERO || 'Ã' || NOM_COMP|| 'Ã' ||COUNT(TIPO)|| 'Ã' ||to_char(SUM(abs(IMPORTE)),'999,999.00')
      FROM  (SELECT
  ...
  BEGIN
    ...
    OPEN curCE FOR
     SELECT 
    RPAD(DECODE(CP.COD_TIP_PROC_PAGO,'1',cp.num_comp_pago_e, '-'),12,' ') || 'Ã' ||   
    RPAD(DECODE(NVL(DECODE(cp.COD_TIP_COMP_PAGO_EREF,'3','01','1','02', '7','04'), pc.tip_comp_pago),'01','BOLETA','02',
    'FACTURA','04','NOTA CREDITO','05','TICKET BOLETA','03','GUIA'),14,' ') || 'Ã' || 
    RPAD(nvl(cp.num_comp_pago_eref,cp.num_comp_pago),13,' ') || 'Ã' || 
    ...
    (SELECT  VTA.USU_CREA_PED_VTA_CAB   FROM  VTA_PEDIDO_VTA_CAB VTA where VTA.NUM_PED_VTA=pc.num_ped_vta_origen)) ),12,' ') || 'Ã' ||         
...
/**********************************************************************/
 ...
 FUNCTION LISTA_DOCANULADOS_IMPRIME(cCodGrupoCia_in IN CHAR, cCodLocal_in    IN CHAR, cSecMovCaja  IN CHAR)
  ...
  BEGIN
    ...
    'FACTURA','04','NOTA CREDITO','05','TICKET BOLETA','03','GUIA'),14,' ') || 'Ã' || 
    RPAD(DECODE(CP.COD_TIP_PROC_PAGO,'1',cp.num_comp_pago_e, '-'),12,' ') || 'Ã' || 
    RPAD(to_char( abs(pc.val_neto_ped_vta),'999,999.00'),11,' ')  || 'Ã' || 
    RPAD(DECODE(CP.COD_TIP_PROC_PAGO,'1',PC.USU_CREA_PED_VTA_CAB,
    DECODE(CP.TIP_COMP_PAGO,'03',(SELECT  VTA.USU_CREA_PED_VTA_CAB   FROM  VTA_PEDIDO_VTA_CAB VTA where VTA.NUM_PED_VTA=cp.num_pedido_anul),
...
FUNCTION LISTA_CIERRETURNO_SINVB(cCodGrupoCia_in IN CHAR,
  ...
  BEGIN
    ...
    CE.NUM_CAJA_PAGO || 'Ã' ||
    CE.NUM_TURNO_CAJA|| 'Ã' ||
    TO_CHAR(CE.FEC_DIA_VTA,'dd/mm/yyyy') || 'Ã' ||
    USU.NOM_USU||' '||USU.APE_PAT || 'Ã' ||
    CE.IND_VB_CAJERO || 'Ã' ||
    CE.IND_VB_QF || 'Ã' ||
    NVL((SELECT  X.NOM_USU||' '||X.APE_PAT FROM   PBL_USU_LOCAL  X WHERE X.LOGIN_USU=CE.USU_MOD_MOV_CAJA ),'-') || 'Ã' ||
  ...
  BEGIN
    ...
    OPEN curCE FOR
     ...
     SELECT CAJERO|| 'Ã' ||ANULADOR|| 'Ã' || NOM_COMP|| 'Ã' ||COUNT(TIPO)|| 'Ã' ||SUM(abs(IMPORTE))
      FROM  (
      SELECT
  ...
  BEGIN
    ...
    SELECT   
    DECODE(CAB.COD_ORIGEN_NOTA_ES,cCodLocal_in,'ENVIADO','RECIBIDO')|| 'Ã' ||  
    CAB.COD_ORIGEN_NOTA_ES||' - '||(SELECT  NVL(LC.DESC_CORTA_LOCAL,'-') FROM PBL_LOCAL   LC WHERE  LC.COD_LOCAL =NVL(CAB.COD_ORIGEN_NOTA_ES,'-'))|| 'Ã' ||  
    CAB.COD_DESTINO_NOTA_ES||' - '||(SELECT NVL(LC.DESC_CORTA_LOCAL,'-') FROM PBL_LOCAL   LC WHERE  LC.COD_LOCAL = NVL(CAB.COD_DESTINO_NOTA_ES,'-'))|| 'Ã' ||  
     NVL(COUNT(DISTINCT CAB.NUM_NOTA_ES),0)|| 'Ã' ||  
...
--CHUANES HUANES 18.01.2015
  ...
  Dbms_Output.put_line('g_dFechaInicio'||g_dFechaInicio);
   ...
   v_vNombreArchivo := 'Reporte de Sucesos '||cCodLocal_in||' - '||TO_CHAR(vfecha,'dd-mm-yyyy')||'.htm';
    ...
    UTL_FILE.PUT_LINE(ARCHIVO_TEXTO,'      <th><small>FECHA ANULACIÓN</small></th>');
    ...
    UTL_FILE.PUT_LINE(ARCHIVO_TEXTO,'      <th><small>N° GUÍA</small></th>');
    UTL_FILE.PUT_LINE(ARCHIVO_TEXTO,'      <th><small>COD PROD</small></th>');
    UTL_FILE.PUT_LINE(ARCHIVO_TEXTO,'      <th><small>DESCRIPCIÓN  -  UNID.PRESENT</small></th>');
    ...
    UTL_FILE.PUT_LINE(ARCHIVO_TEXTO,'      <th><small>FECHA CREACIÓN</small></th>');
    ...
    UTL_FILE.PUT_LINE(ARCHIVO_TEXTO,'      <th><small>USU. MODIFICACIÓN</small></th>');
    UTL_FILE.PUT_LINE(ARCHIVO_TEXTO,'      <th><small>FECHA DE MODIFICACIÓN</small></th>');
