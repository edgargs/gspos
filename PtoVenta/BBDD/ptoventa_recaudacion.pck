CREATE OR REPLACE PACKAGE PTOVENTA."PTOVENTA_RECAUDACION" AS
  ...
  TIPO_REC_VENTA_CMR CHAR(2):='06';
    ...
    COD_RCD_RECAU_TELETON CHAR(2) := '21';   --ASOSA - 11/08/2016 - TELETON
...
--================================================================================
 ...
 FUNCTION RCD_IMP_COMPAGO_RECAUDACION(cCodGrupoCia_in         IN CHAR,
   ...
   C_FIN_MSG     VARCHAR2(2000) := '</table></td></tr></table></body></html>';
    ...
    --INI ASOSA - 11/08/2016 - TELETON
    vComodin01 varchar2(200) := '';
    vComodin03 varchar2(200) := '';
    vComodin04 varchar2(200) := '';
    vComodin05 varchar2(200) := '';
    vComodin06 varchar2(200) := '';
    vComodin07 varchar2(200) := '';
    --FIN ASOSA - 11/08/2016 - TELETON
    ...
    BEGIN
          ...
          --INI ASOSA - 11/08/2016 - TELETON
          IF vTipoRecaudacion = COD_RCD_RECAU_TELETON THEN 
            
              Select Rcd.Dni_Cliente, RCD.TIP_MONEDA, RCD.IM_TOTAL, Rcd.Cod_Autorizacion, Sum(Det.Im_Total_Pago), Sum(Det.Val_Vuelto)
              ...
              GROUP BY Rcd.Dni_Cliente, RCD.TIP_MONEDA, RCD.IM_TOTAL, Rcd.Cod_Autorizacion;
              
              vTipoDoc:='DNI/CE: ';
              
              vComodin01 := 'DONACIÓN TELETÓN';
              vComodin03 := '<br>'||'NUM. DONACIÓN: '|| cNum_Recaudacion_in;
              vComodin04 := '<span style="font-weight:bold;">';
              vComodin05 := '</span>';
              vComodin06 := '';
              vComodin07 := '';
              
              SELECT 
              '<span style="font-weight:bold;">' || '#unidoshacemosmas' || '</span>' ||
          ...
          ELSE
              vComodin01 := 'RECAUDACIÓN';
              vComodin03 := '<br>'||'NUM. RECAUDACIÓN: '|| cNum_Recaudacion_in;
              vComodin04 := '';
              vComodin05 := '';
              vComodin06 := '<br>'||'COD. AUTORIZACIÓN: '|| vCodAutorizacion;
              vComodin07 := '<br><span style="font-weight:bold;">'||'EL PAGO SE REFLEJARÁ HASTA EL 2DO. DÍA ÚTIL';
              
              SELECT 
              vTextoMarca||MG.NOM_MARCA ||
              '<br>'|| PC.RAZ_SOC_CIA || ' RUC: '||PC.NUM_RUC_CIA ||
              '<br>'|| PC.DIR_CIA
              ,PL.COD_LOCAL
              INTO vMensajeLocal, vlocal
       ...
       END IF ;
        ...
        IF vTipoRecaudacion='04' OR
          vTipoRecaudacion = COD_RCD_RECAU_RAIZ  --ASOSA - 28/08/2015 - RAIZ
          THEN
          vTipoDoc:='DOCTO : ';
            Select Rcd.Cod_Cliente, RCD.TIP_MONEDA, RCD.IM_TOTAL, Rcd.Cod_Autorizacion, Sum(Det.Im_Total_Pago), Sum(Det.Val_Vuelto)
            INTO vNumTarjeta, vTipoMonRcd, vMontoPago,vCodAutorizacion, vMontoTotalPago, vValVuelto
       ...
       vcMontoTotalPago := GET_FORMAS_PAGO(cCodGrupoCia_in,cCod_Cia_in,cCod_Local_in,cNum_Recaudacion_in);
          ...
          '<br>'||'----------------------------------------------------------------------------' ||
          '<br>'|| vComodin04 ||'SERVICIO: '|| DECODE(RCD.TIPO_RCD, COD_RCD_RECAU_TELETON, '', RCD.TIPO_RCD) || '  ' || EMP.DESC_LARGA_RCD ||
          '<br>'|| vTipoDoc || vNumTarjeta ||
          vComodin06 ||          
          '<br>'||'MONTO: '||decode(vTipoMonRcd,'02',' $ ','S/.')||'  ***'|| TO_CHAR(vMontoPago,'999,990.00') ||'***' || vComodin05 ||
          '<br>'||'----------------------------------------------------------------------------' ||
          vcMontoTotalPago ||
  ...
  BEGIN
      ...
      TIEMPO_MAXIMO :=
       ...
       FOR v_curRecaudacion IN curRecaudacion
        ...
        SELECT A.TIPO_RCD
        INTO v_cTipoRecau
        FROM RCD_RECAUDACION_CAB A
        WHERE A.COD_GRUPO_CIA = cCodGrupoCia_in
        AND A.COD_CIA = cCod_Cia_In
        AND A.COD_LOCAL = cCod_Local_in
        AND A.COD_RECAU = cNum_Recaudacion_In;
        
        IF v_cTipoRecau = COD_RCD_RECAU_TELETON THEN
           cRepta := 'N';
        END IF;          
