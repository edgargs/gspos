CREATE OR REPLACE TRIGGER PTOVENTA."TR_U_VTA_PEDIDO_VTA_CAB" AFTER UPDATE OF TIP_COMP_PAGO OR INSERT ON VTA_PEDIDO_VTA_CAB
 FOR EACH ROW
...
BEGIN
  ...
  if vIP_pc = '10.10' then
      IF INSERTING THEN
        IF (:NEW.TIP_COMP_PAGO = '01' OR :NEW.TIP_COMP_PAGO = '05') AND :NEW.NUM_PED_VTA_ORIGEN IS NULL THEN
          SELECT X.TIP_COMP
            INTO v_vTipComp
            FROM VTA_IMPR_IP X
          ...
          IF (v_vTipComp <> :NEW.TIP_COMP_PAGO) THEN
            ...
            vAsunto   := 'ALERTA POR CAMBIO DEL TIPO DE COMPROBANTE EN LA VENTA: - TR_U_VTA_PEDIDO_VTA_CAB';
            vTitulo   := 'ALERTA';
