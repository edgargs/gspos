...
/*******************************************************************************************************************************************/
  ...
  FUNCTION F_CHAR_ACTIVA_REPORTE_GIGANTE
    ...
    --Rafael Bullon Mucha 25/01/2016
    FUNCTION LTA_OBTIENE_INFO_PROD_PREC_VOL (cCodGrupoCia_in IN CHAR,
        ...
        FROM
           (
            SELECT
                   ...
                   SUM(ROUND((DET.VAL_PREC_TOTAL/(1+(DET.VAL_IGV/100))),3)) MON_TOT_VTA,
...
/*******************************************************************************************************************************/
  ...
  --Rafael Bullon Mucha 25/01/2016
  -- KMONCADA 07.04.2016 FORMATO A LOS NUMEROS  
  FUNCTION LTA_OBTIENE_INFO_PROD_PREC_VOL (cCodGrupoCia_in IN CHAR,
                                           cCod_Local_in IN CHAR)
  RETURN FarmaCursor
  IS
    curLstaPrecVol FarmaCursor;

  BEGIN
  
    OPEN curLstaPrecVol FOR
        select 
        prod.cod_prod ||'Ã' ||  
        prod.desc_prod ||'Ã' || 
        lab.nom_lab ||'Ã' || 
        loc.val_frac_local ||'Ã' || 
        --trunc(loc.stk_fisico/loc.val_frac_local) ||'Ã' || 
        TRUNC(PTOVENTA_VTA.F_GET_STOCK_PRODUCTO(cCodGrupoCia_in, cCod_Local_in, PROD.COD_PROD, 'S'))||'Ã' ||
        TO_CHAR(loc.val_prec_vta,'999,990.00') ||'Ã' || 
        TO_CHAR((prod.val_prec_prom * (1 + igv.porc_igv/100)) ,'999,990.00')
        from ptoventa.lgt_prod prod, 
             ptoventa.lgt_lab lab, 
             ptoventa.lgt_prod_local loc, 
             ptoventa.pbl_igv igv
        where prod.cod_grupo_cia=loc.cod_grupo_cia 
        and prod.cod_prod=loc.cod_prod 
        and prod.cod_lab=lab.cod_lab 
        and prod.cod_igv=igv.cod_igv
        and loc.cod_grupo_cia = cCodGrupoCia_in --'001'
        and loc.cod_local = cCod_Local_in --'A28'
        and loc.est_prod_loc='A'
        order by prod.desc_prod;
     -- and prod.cod_prod=cCodProd_in;
      RETURN curLstaPrecVol;
  END;    
