...
public class UtilityPuntos {
    ...
    public static boolean consultarTarjetaOrbis(Frame myParentFrame, JDialog pJDialog, JTextField campoTexto, boolean isLectorTarjeta, boolean pTarjetaAnterior){
        ...
        try{
            if(isActivoFuncionalidad()){
                ...
                if(pTarjetaAnterior){
                    nroTarjetaPuntos = campoTexto.getText();
                    ...
                    campoTexto.setText(nroTarjetaPuntos);
                }
                ...
                if(DBPuntos.requerieTarjetaAsociada()){
                    ...
                    if(!isTarjetaValida(campoTexto.getText())){
                        log.info("PROGRAMA PUNTOS [CONSULTA TARJETA] : NO ES UNA TARJETA DE PUNTOS --> "+campoTexto.getText());
                        String nroDocumentoAux = campoTexto.getText();
                        try{
                            int cantidadTarjetaAdicional = cantidadTieneTarjetasAdicionales(myParentFrame, pJDialog, nroDocumentoAux, false);
                            log.info("PROGRAMA PUNTOS [CONSULTA TARJETA] : CANTIDAD DE TARJETAS ASOCIADAS --> "+campoTexto.getText()+" -- "+cantidadTarjetaAdicional);
                            if(cantidadTarjetaAdicional == 0){
                                ...
                                if(JConfirmDialog.rptaConfirmDialog(pJDialog, mensajeAviso)){
                                    ...
                                    if(FarmaVariables.vAceptar){
                                        campoTexto.setText(dlgVerifica.getTextTarjeta().trim());
                                    }else
                                        return false;
                                }else{
                                    return false;
                ...
                log.info("PROGRAMA PUNTOS [CONSULTA TARJETA] : NRO DE TARJETA --> "+campoTexto.getText());
                TarjetaBean tarjetaPuntos =  VariablesPuntos.frmPuntos.validarTarjetaAsociada(campoTexto.getText().trim(),  getDNI_USU());
                ...
                if(FarmaPuntosConstante.EstadoTarjeta.INACTIVA.equalsIgnoreCase(tarjetaPuntos.getEstadoTarjeta())){
                    ...
                    if(pTarjetaAnterior){
                        ...
                        registroAfiliadoAutomatico(nroDni, nroTarjetaPuntos, nroTarjetaFidelizado);
                        return true;
                    }
                    return false;
                }
                ...
                if(FarmaPuntosConstante.NO_CONEXION_ORBIS.equalsIgnoreCase(tarjetaPuntos.getEstadoOperacion())){
                    log.info("CONSULTA DE CLIENTE OFFLINE --> VENTA OFFLINE "+campoTexto.getText());
                  //  FarmaUtility.showMessage(pJDialog, "PROGRAMA DE PUNTOS:\nVenta en modo OFFLINE", campoTexto);
                    ...
                    log.info("ERROR EN INIT DE ORBIS --> \n CODIGO DE ERROR "+estadoOperacion+"\n MENSAJE "+mensaje+
                             "NRO TARJETA "+campoTexto.getText()+
     ...
     * @throws Exception
     */
    private static boolean procesarQuote(TarjetaBean tarjetaPuntos, String pNumPedVta, boolean isAdicionarBonificado)throws Exception{
    ...
    public static boolean validaTarjetaAdicional(TarjetaBean tarjetaPuntos, Frame myParentFrame, JDialog pJDialog, String nroDocumento)throws Exception{
        ...
        if(FarmaPuntosConstante.EstadoTarjeta.INACTIVA.equalsIgnoreCase(tarjetaPuntos.getEstadoTarjeta()) &&
            ...
            int cantidadAdicional = cantidadTieneTarjetasAdicionales(myParentFrame, pJDialog, nroDocumento, true);
    ...
    public static int cantidadTieneTarjetasAdicionales(Frame myParentFrame, JDialog pJDialog, String nroDocumento, boolean solicitaValidaDocumento)throws Exception{
