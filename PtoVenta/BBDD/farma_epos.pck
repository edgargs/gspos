             ...
             ELSE
              ...
              CASE PED.TIP_PED_VTA
                WHEN '02' --PEDIDO DELIVERY
                 THEN
                 LOCAL.UBDEP || LOCAL.UBPRV || LOCAL.UBDIS || '|' || -- 32 Dirección del Punto de Partida, Código de Ubigeo
                 REEMPLAZA_CARACTERES(LOCAL.DIREC_LOCAL) || '|' || -- 33 Dirección del Punto de Partida, Dirección completa y detallada
                 '' || '|' || -- 34 Dirección del Punto de Partida, Urbanización
                 UBI_LOCAL.NOPRV || '|' || -- 35 Dirección del Punto de Partida, Provincia
                 UBI_LOCAL.NODEP || '|' || -- 36 Dirección del Punto de Partida, Departamento
                 UBI_LOCAL.NODIS || '|' || -- 37 Dirección del Punto de Partida, Distrito
    ...
    */ 
    BEGIN
      SELECT 'DN|'||
             ...
             --- Glosa de la leyenda	3
           'SON: ' ||  TRIM(UPPER(TRIM(FARMA_UTILITY.LETRAS(COMP.VAL_TOTAL_E +
                                             COMP.TOTAL_IGV_E))) || ' ' ||
  ...
  BEGIN
   ...
   IF cTipoClienteConvenio = DOC_EMPRESA AND v_vIndCopago='S' THEN
      ...
      --IMPUESTO GLOBAL POR DOCUMENTO
       ...
       DECODE(DECODE(PAGO.TOTAL_GRAV_E,0,0,(PAGO.TOTAL_GRAV_E - PAGO.VAL_COPAGO_E)*0.18), NULL, '', 1000) || '|' || -- 3 Identificación del tributo
       DECODE(DECODE(PAGO.TOTAL_GRAV_E,0,0,(PAGO.TOTAL_GRAV_E - PAGO.VAL_COPAGO_E)*0.18), NULL, '', 'IGV') || '|' || -- 4 Nombre del Tributo
       DECODE(DECODE(PAGO.TOTAL_GRAV_E,0,0,(PAGO.TOTAL_GRAV_E - PAGO.VAL_COPAGO_E)*0.18), NULL, '', 'VAT') || CHR(13) -- 5 Código del Tipo de Tributo
   ...
   -- dubilluz 13.01.2015    
    ...
    ELSE
      ...
      SELECT 
        ...
        LPAD(NVL(TRIM(TO_CHAR(NVL(ABS(DET.AHORRO_CAMP),0),'999,999,990.000')),0),8) ||
        ...
        LPAD(NVL(TRIM(TO_CHAR(NVL(ABS(ROUND((DET.VAL_TOTAL_DESC_ITEM_E*(1+(DET.VAL_IGV/100))),2)),0),'999,999,990.000')),0),8) ||
    ...
    END IF;
     ...
     IF FARMA_LEALTAD.IND_IMPR_PUNTOS_REDI(vCodGrupoCia_in,vCodLocal_in,vAuxNumPedVta) = 'S' THEN
       ...
       SELECT LPAD('SUBTOTAL: S/.',25)||
             LPAD(TRIM(TO_CHAR(SUM(NVL(ABS(DET.VAL_PREC_TOTAL+DET.AHORRO_PUNTOS),0)),'999,999,990.00')),15)
              ,'9','D','N','N'
       FROM VTA_PEDIDO_VTA_DET DET
       WHERE DET.COD_GRUPO_CIA = vCodGrupoCia_in
        AND   DET.COD_LOCAL     = vCodLocal_in
       ...
       SELECT FARMA_LEALTAD.MSJ_IMPR_PUNTOS_REDI||
...
*/
      ...
      IF vEstadoOrbis = ESTADO_SIN_TRX_ORBIS THEN
        IF vTipoDocumento != COD_TIP_COMP_NOTA_CRED THEN
...
/*
                      PTOVENTA_FIDELIZACION.INS_IMP_PTOS(vCodGrupoCia_in,
                                                    ...
                                                    vNumPedVta_in);
                                                    */
        -- KMONCADA 21.04.2015 SE CAMBIA METODO POR CORRECCION DE DATOS.
        INSERT INTO TMP_DOCUMENTO_ELECTRONICOS VALUES ('__________________________________________', '9','I','N','N');
        INSERT INTO TMP_DOCUMENTO_ELECTRONICOS VALUES (' ', '9','I','N','N');

        INSERT INTO TMP_DOCUMENTO_ELECTRONICOS        
        SELECT 'Tarjeta : ' || vNroTarjetaPuntos ,'0','I','S','N' FROM DUAL;
                            
        INSERT INTO TMP_DOCUMENTO_ELECTRONICOS        
        SELECT 'DNI : ' || vNroDNIPto ,'0','I','S','N' FROM DUAL;
                            
        INSERT INTO TMP_DOCUMENTO_ELECTRONICOS        
        SELECT 'Nombre Cliente : ' || vNombreClientePto ,'0','I','S','N' FROM DUAL;
                    
        IF ahorroPtoSoles > 0 THEN
          SELECT NVL(T.LLAVE_TAB_GRAL, ' ')
            INTO vSufijoAhorroSoles
            FROM PBL_TAB_GRAL T
           WHERE T.ID_TAB_GRAL = 487;
        
          INSERT INTO TMP_DOCUMENTO_ELECTRONICOS  
          SELECT V_BOLD_I || 
                 NVL(T.LLAVE_TAB_GRAL,' ') || ' : ' ||
                 V_BOLD_F || 
                 V_SEPARADOR || 
                 V_BOLD_I_2 ||
                 PTOVENTA_FIDELIZACION.FID_F_GET_NUM_FORMATED_02(ahorroPtoSoles) || 
                 V_BOLD_F_2 || 
                 V_SEPARADOR || 
                 ' ' || 
                 vSufijoAhorroSoles
                 ,'0','I','N','N' 
          FROM PBL_TAB_GRAL T
          WHERE T.ID_TAB_GRAL = 471;  
        END IF;
        
        IF vPtoAcumulado > 0 THEN
          SELECT NVL(T.LLAVE_TAB_GRAL, ' ')
            INTO vSufijoPtoAcumula
            FROM PBL_TAB_GRAL T
           WHERE T.ID_TAB_GRAL = 477;
          ...
          SELECT V_BOLD_I || 
                 ...
                 V_SEPARADOR || 
                 V_BOLD_I_2|| 
                 PTOVENTA_FIDELIZACION.FID_F_GET_NUM_FORMATED(vPtoAcumulado) || 
                 V_BOLD_F_2 || 
                 V_SEPARADOR || ' ' || 
                 vSufijoPtoAcumula
                 ,'0','I','N','N' 
          FROM PBL_TAB_GRAL T
          WHERE T.ID_TAB_GRAL = 469;  
        END IF;
        ...
        IF vPtoRedimido > 0 THEN
          ...
          SELECT V_BOLD_I || 
                 ...
                 V_SEPARADOR || 
                 V_BOLD_I_2|| 
                 PTOVENTA_FIDELIZACION.FID_F_GET_NUM_FORMATED(vPtoRedimido) || 
                 V_BOLD_F_2 || 
                 V_SEPARADOR || ' ' || 
                 vSufijoPtoAcumula
                 ,'0','I','N','N' 
          FROM PBL_TAB_GRAL T
          WHERE T.ID_TAB_GRAL = 470; 
        END IF;
        
        IF vPtoTotal > 0 THEN
          SELECT NVL(T.LLAVE_TAB_GRAL, ' ')
            INTO vSufijoPtoTotal
            FROM PBL_TAB_GRAL T
           WHERE T.ID_TAB_GRAL = 486;
          ...
          SELECT V_BOLD_I || 
                 ...
                 V_SEPARADOR || 
                 V_BOLD_I_2|| 
                 PTOVENTA_FIDELIZACION.FID_F_GET_NUM_FORMATED(vPtoTotal) || 
                 V_BOLD_F_2 || 
                 V_SEPARADOR || ' ' || 
                 vSufijoPtoTotal
                 ,'0','I','N','N' 
          FROM PBL_TAB_GRAL T
          WHERE T.ID_TAB_GRAL = 485; 
        END IF;
        ...
        SELECT REPLACE((REPLACE((EXTRACTVALUE(xt.column_value, 'e')),'Ã','&')),'Ë','<') VAL ,'0','I','N','N'
               FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' || REPLACE( REPLACE((REPLACE((
                    ...
                    (SELECT P.LLAVE_TAB_GRAL
                     FROM PBL_TAB_GRAL P
                     WHERE P.ID_TAB_GRAL = 661)
                    ||'@'||' '||'@'||
                    (SELECT P.LLAVE_TAB_GRAL
                     FROM PBL_TAB_GRAL P
                     WHERE P.ID_TAB_GRAL = 458)
                    ||'@'||' '
               ),'&','Ã')),'<','Ë'),'@','</e><e>') ||'</e></coll>'),'/coll/e'))) xt;
      END IF;
