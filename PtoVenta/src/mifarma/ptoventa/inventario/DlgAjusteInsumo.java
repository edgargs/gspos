  ...
  { FarmaUtility.showMessage(this, "Debe presionar la tecla ESC para cerrar la ventana.", null);
                        ...
                        FarmaUtility.showMessage(this,"ERROR: LA CANTIDAD A INGRESAR DEBE SER MENOR AL STOCK DEL SISTEMA",txtStockModifEntero);
                            ...
                            if (datosValidados())
                                    if (JConfirmDialog.rptaConfirmDialog(this,
                                                    ...
                                                    +txtStockModifEntero.getText().trim()+"     Fracción: "
    ...
    /* 	if (cmbMotivoAjuste.getSelectedIndex()==0) {
		...
		if (txtStockModifEntero.getText().trim().length() == 0) {
			FarmaUtility.showMessage(this, "Ingrese una cantidad válida del stock",
					txtStockModifEntero);
			return false;
		}
    ...
    if (txtStockFraccion.getText().trim().length() == 0) {
			FarmaUtility.showMessage(this, "Ingrese una cantidad válida a fraccionar" ,
					txtStockFraccion);
			return false;
		}
     ...
     if(FarmaUtility.getDecimalNumber(txtStockModifEntero.getText().trim()) * Integer.parseInt(VariablesInventario.vFrac) + FarmaUtility.getDecimalNumber(txtStockFraccion.getText().trim()) == FarmaUtility.getDecimalNumber(lblStock.getText().trim()))
    {
      /* FarmaUtility.showMessage(this, "La cantidad ingresada debe ser distinto del Stock Actual.",
					txtStockModifEntero);
			return false; */
      return JConfirmDialog.rptaConfirmDialog(this,
                                 "La cantidad ingresada es el mismo al Stock Actual \n" +
                                "¿Está seguro de continuar? ");
                        
    }
	    if (txtCantSuge.getText().trim().length() == 0) {
	                        FarmaUtility.showMessage(this, "Ingrese una cantidad válida a reponer",
	                                        txtCantSuge);
	                        return false;
	                }
