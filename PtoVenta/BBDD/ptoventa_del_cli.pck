...
--          AND	   TVTA_DET.COD_LOCAL = cCodLocal_in -- kmoncada
          ...
          AND    (cFlagFiltro_in = 'N'
                  OR TVTA_DET.COD_PROD NOT  IN (
                      ...
                      FROM (SELECT DET.COD_PROD COD_PROD,
                                   (SELECT --SUM(INS.CANT_ATENDIDA) -- KMONCADA 28.06.2016 LOTE CON VAL_FRAC
                                           SUM(
                                              CASE
                                                 WHEN INS.VAL_FRAC=1 THEN
                                                   (INS.CANT_ATENDIDA/INS.VAL_FRAC)
                                                 ELSE
                                                   ((INS.CANT_ATENDIDA*PL.VAL_FRAC_LOCAL)/INS.VAL_FRAC)
                                                   END
                                            )
                                     ...
                                     WHERE INS.COD_GRUPO_CIA = DET.COD_GRUPO_CIA
                                       ...
                                       AND INS.COD_GRUPO_CIA = PL.COD_GRUPO_CIA
                                       AND INS.COD_LOCAL = PL.COD_LOCAL
                                       AND INS.COD_PROD = PL.COD_PROD) nCantProdLote,
                                   (SELECT SUM(CASE
 ...
 END;
  ...
  IS
    ...
    vFracLocal LGT_PROD_LOCAL.VAL_FRAC_LOCAL%TYPE;
  BEGIN
       ...
       -- KMONCADA 27.06.2016
	   --ERIOS 01.07.2016 Hay Kenny, te copias todas las de Gogo
       SELECT DET.VAL_FRAC
       INTO vFracLocal
       FROM TMP_VTA_PEDIDO_VTA_DET DET
       WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
       AND DET.COD_LOCAL = cCodLocal_in
       AND DET.COD_PROD = cCodProd_in
	   AND    DET.NUM_PED_VTA = cNumPedido_in;

  	   INSERT INTO
              TMP_VTA_INSTITUCIONAL_DET (COD_GRUPO_CIA, COD_LOCAL, NUM_PED_VTA, SEC_VTA_INST_DET,
                                         COD_PROD, NUM_LOTE_PROD, CANT_ATENDIDA, USU_CREA_VTA_INST_DET, Fec_Vencimiento_Lote, VAL_FRAC)-- paulo
	   		  	   	                 VALUES (cCodGrupoCia_in, cCodLocal_in, cNumPedido_in, v_nNumeroSecMax,
                                         cCodProd_in, cNumLote_in, cCant_in, cUsuCrea_in ,to_date(cFechaVencimiento_in,'dd/MM/yyyy'), vFracLocal);
  END;
  ...
  BEGIN
       OPEN curCli FOR
            SELECT --TMP.CANT_ATENDIDA || 'Ã' || -- KMONCADA 28.06.2016 LOTE CON VAL_FRAC
                   SUM( CASE
                          WHEN TMP.VAL_FRAC=1 THEN
                            (TMP.CANT_ATENDIDA/TMP.VAL_FRAC)
                          ELSE
                            ((TMP.CANT_ATENDIDA*PL.VAL_FRAC_LOCAL)/TMP.VAL_FRAC)
                        END
                   ) || 'Ã' ||
            ...
            AND    TMP.NUM_PED_VTA = cNumPedido_in
            AND    TMP.COD_PROD = cCodProd_in
            AND    TMP.COD_GRUPO_CIA = PL.COD_GRUPO_CIA
            AND    TMP.COD_LOCAL = PL.COD_LOCAL
            AND    TMP.COD_PROD = PL.COD_PROD;
    RETURN CURCLI;
  END;
...
--        AND    INS.SEC_VTA_INST_DET = TMP_DET.SEC_PED_VTA_DET -- DUBILLUZ - 18.12.2009
        ORDER BY TMP_DET.SEC_PED_VTA_DET; --rherrera 27.08.2014 order de productos
		...
		v_nSumaPro NUMBER;
		v_nSumaIns NUMBER;
    vCantSinValFrac NUMBER := 0;
  BEGIN
    -- KMONCADA 24.06.2016 CARGA DE VAL FRAC EN EL CASO DE LOS ERRORES REPORTADOS
    /*SELECT COUNT(1)
    INTO vCantSinValFrac
    FROM TMP_VTA_INSTITUCIONAL_DET INS
    WHERE INS.COD_GRUPO_CIA = cCodGrupoCia_in
    AND   INS.COD_LOCAL= cCodLocalAtencion_in
    AND   INS.NUM_PED_VTA = cNumPedVtaDel_in
    AND   NVL(INS.VAL_FRAC,0) = 0;*/
    
    IF vCantSinValFrac > 0 THEN
      -- KMONCADA 27.06.2016 CARGA DE VAL FRAC EN EL CASO DE LOS ERRORES REPORTADOS
      FOR LISTA IN(
          SELECT X.*,
                 CASE
                   WHEN X.VAL_FRAC = 1 THEN 1
                   WHEN X.CANT_ATENDIDA != X.CANT_REG THEN X.VAL_FRAC_LOCAL
                   ELSE X.VAL_FRAC
                 END VAL_FRAC_LOTE
            FROM (SELECT DET.COD_PROD, DET.CANT_ATENDIDA, DET.VAL_FRAC, L.VAL_FRAC_LOCAL,
                         (SELECT SUM(CANT_ATENDIDA)
                            FROM TMP_VTA_INSTITUCIONAL_DET INS
                           WHERE DET.COD_GRUPO_CIA = INS.COD_GRUPO_CIA
                             AND DET.COD_LOCAL = INS.COD_LOCAL
                             AND DET.NUM_PED_VTA = INS.NUM_PED_VTA
                             AND DET.COD_PROD = INS.COD_PROD) CANT_REG
                    FROM TMP_VTA_PEDIDO_VTA_DET DET, 
                         LGT_PROD_LOCAL L
                   WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
                     AND DET.COD_LOCAL = cCodLocalAtencion_in
                     AND DET.NUM_PED_VTA = cNumPedVtaDel_in
                     AND L.COD_GRUPO_CIA = DET.COD_GRUPO_CIA
                     AND L.COD_LOCAL = DET.COD_LOCAL
                     AND L.COD_PROD = DET.COD_PROD
                   ORDER BY DET.SEC_PED_VTA_DET) X) LOOP
         
         UPDATE TMP_VTA_INSTITUCIONAL_DET INS
          SET INS.VAL_FRAC = LISTA.VAL_FRAC_LOTE
          WHERE INS.COD_GRUPO_CIA = cCodGrupoCia_in
          AND   INS.COD_LOCAL= cCodLocalAtencion_in
          AND   INS.NUM_PED_VTA = cNumPedVtaDel_in
          AND   INS.COD_PROD = LISTA.COD_PROD;
         END LOOP
         ;
/*         
      UPDATE TMP_VTA_INSTITUCIONAL_DET INS
      SET INS.VAL_FRAC = (SELECT D.VAL_FRAC
                          FROM TMP_VTA_PEDIDO_VTA_DET D
                          WHERE D.COD_GRUPO_CIA = INS.COD_GRUPO_CIA
                          AND D.COD_LOCAL = INS.COD_LOCAL
                          AND D.NUM_PED_VTA = INS.NUM_PED_VTA
                          AND D.COD_PROD = INS.COD_PROD)
      WHERE INS.COD_GRUPO_CIA = cCodGrupoCia_in
      AND   INS.COD_LOCAL= cCodLocalAtencion_in
      AND   INS.NUM_PED_VTA = cNumPedVtaDel_in
      AND   NVL(INS.VAL_FRAC,0) = 0;*/
    END IF;
       ...
       ---@@-------------------------------------------------@@---
		...
		WITH
		PRO AS (SELECT COUNT(1) CANT, SUM((TPD.CANT_ATENDIDA*LPL.VAL_FRAC_LOCAL)/TPD.VAL_FRAC) SUMA
		...
		INS AS (SELECT COUNT(1) CANT, SUM((TPD.CANT_ATENDIDA*LPL.VAL_FRAC_LOCAL)/TPD.VAL_FRAC) SUMA
    --select COUNT(1) CANT, SUM(CANT_ATENDIDA) SUMA
				from ptoventa.TMP_VTA_INSTITUCIONAL_DET TPD JOIN 
						PTOVENTA.LGT_PROD_LOCAL LPL ON (LPL.COD_GRUPO_CIA = TPD.COD_GRUPO_CIA
														AND LPL.COD_LOCAL = TPD.COD_LOCAL
														AND LPL.COD_PROD = TPD.COD_PROD)
				WHERE 
  ...
  BEGIN
       OPEN curCli FOR
            ...
            FROM (
              ...
              (SELECT --SUM(INS.CANT_ATENDIDA) -- KMONCADA 28.06.2016 FRACCION POR LOTE
                      SUM(CASE
                           WHEN INS.VAL_FRAC = 1 THEN
                            (INS.CANT_ATENDIDA / INS.VAL_FRAC)
                           ELSE
                ...
                FROM TMP_VTA_INSTITUCIONAL_DET INS, LGT_PROD_LOCAL PL
               WHERE INS.COD_GRUPO_CIA = DET.COD_GRUPO_CIA
                 AND INS.COD_LOCAL = DET.COD_LOCAL
                 AND INS.NUM_PED_VTA = DET.NUM_PED_VTA
                 AND INS.COD_PROD = DET.COD_PROD
...
FUNCTION CLI_LISTA_TRANSFERENCIA(
 ...
 AS
  ...
  (COD_GRUPO_CIA, COD_LOCAL, NUM_PED_VTA, SEC_VTA_INST_DET, COD_PROD, NUM_LOTE_PROD, CANT_ATENDIDA, 
  USU_CREA_VTA_INST_DET, FEC_CREA_VTA_INST_DET, FEC_VENCIMIENTO_LOTE, VAL_FRAC)
