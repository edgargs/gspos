...
public class DlgPedidoDiario extends JDialog {
    ...
    private void initTableListaDiferenciasConsolidado() {
        tableModelListaProductos =
                new FarmaTableModel(ConstantsInvDiario.columnsListaDetTemporal, ConstantsInvDiario.defaultValuesListaDetTemporal,
                                    0);
        FarmaUtility.initSimpleList(tblListaProductosPedido, tableModelListaProductos,
    ...
    private void txtProductos_keyPressed(KeyEvent e) {
        FarmaGridUtils.aceptarTeclaPresionada(e, tblListaProductosPedido, txtProductos, 1);
        chkKeyPressed(e);
    }
    ...
    private void chkKeyReleased(KeyEvent e) {
        operaTotales();
        FarmaGridUtils.buscarDescripcion(e, tblListaProductosPedido, txtProductos, 1);
    ...
    private void cargaListaDetTemporal(String pNumPedidoTemporal_in) {
        try {
            DBInvDiario.cargaDetallePedTemporal(tableModelListaProductos, pNumPedidoTemporal_in);
            if (tblListaProductosPedido.getRowCount() > 0)
                FarmaUtility.ordenar(tblListaProductosPedido, tableModelListaProductos, 1,
                                     FarmaConstants.ORDEN_ASCENDENTE);
            /* lblMontoTotal.setText(FarmaUtility.formatNumber(FarmaUtility.sumColumTable(tblListaProductosPedido,
                                                                                  5)));*/
        } catch (SQLException sql) {
            log.error("", sql);
            FarmaUtility.showMessage(this, "Error al obtener el detalle de Pedido Temporal :\n" +
    ...
    private void operaTotales() {
        ...
        for (int i = 0; i < tableModelListaProductos.data.size(); i++) {
            dTotalVenta +=
                    FarmaUtility.getDecimalNumber(FarmaUtility.getValueFieldArrayList(tableModelListaProductos.data, i,
                                                                                      5));
        }
        ...
        for (int i = 0; i < VariablesInvDiario.vListaTrabParaDescuento.size(); i++) {
            ...
            dTotalDcto +=
                    FarmaUtility.getDecimalNumber(FarmaUtility.getValueFieldArrayList(VariablesInvDiario.vListaTrabParaDescuento,
                                                                                      i, 3));
        }
        ...
        totalNetoRedondeado =
                UtilityVentas.Redondear(dTotalVenta, 2); //redondeo a 2 cifras pero no a ajustado a .05 o 0.00
    ...
    public void eliminaProducto() {
        if (tblListaProductosPedido.getRowCount() > 0) {
            ...
            if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                                                                            "¿Seguro de quitar el producto del pedido?")) {
                ...
                try {
                    ...
                    if (tableModelListaProductos.getRowCount() > 0) {
                        FarmaGridUtils.showCell(tblListaProductosPedido, 0, 0);
                    }
                    operaTotales();
    ...
    public void eliminaPedidoTemporal() {
        if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this, "¿Seguro que desea salir del pedido?")) {
            try {
    ...
    public void generarPedido() {
        if (tableModelListaProductos.getRowCount() > 0) {
            ...
            if (DiferenciaGlobal == 0) {
                try {
                    DBInvDiario.generaPedidoInvDiario(VariablesInvDiario.vSecPedidoTemporal,
                    ...
                    if (e.getErrorCode() == 20003) {
                        FarmaUtility.showMessage(this, "El monto asignado a los Trabajadores\n es menor que el pedido",
                                                 txtProductos);
                    } else {
                        FarmaUtility.showMessage(this, "Ocurrió un error al generar el Pedido.\n" +
                                e.getMessage(), txtProductos);
                    }
                }
            } else {
