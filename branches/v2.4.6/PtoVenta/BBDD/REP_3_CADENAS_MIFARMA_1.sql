--------------------------------------------------------
--  DDL for Package Body REP_3_CADENAS_MIFARMA
--------------------------------------------------------
...
--       AND A.FEC_DIA_VTA BETWEEN TRUNC(v_dFecha - 90) AND TRUNC(v_dFecha)
  ...
  BEGIN
    ...
    ---JQUISPE 23/03/2012 , SE PIDIO QUE NO TOME EN CUENTA LOS PRODUCTOS QUE ESTEN EN CAMPA√`A IMBATIBLES MARZO
...
*/    END;
  ...
  procedure P_ACTUALIZA_PROD_LOCAL_REP(cCodLocal_in in char) is
    begin
        ...
        -- Author    : JOLIVA
        -- Fecha     : 2014-07-18 
        -- PropÛsito : Se usa la columna UN_VTA_M5 para almanenar la m·xima venta por mes que se usar· en el calculo del masterpack
        UN_VTA_M5)
                                         = (
...
BEGIN
  ...
  -- 2009-05-20 DOCUMENTADO POR JOLIVA
  -- Saca backup a la √∫ltima cantidad SUGERIDA y obtiene el sugerido convertido a la unidad m√≠nima del producto
...
*/
  ...
  -- 2009-11-24 JOLIVA: SE MODIFICA PARA QUE S√øLO TRABAJE CON ENTEROS
        SELECT CEIL(NVL(MAX(
                       SUM(
...
-- 2009-06-26 FALTA AGREGAR ESTA LOGICA PARA LA ROTACION REAL
    ...
    -- 2009-05-20 JOLIVA: Obtener el c√≥digo de producto que se usar√° para reemplazar a los SOLICITADOS que no tengan stock en almacen
      BEGIN
...
--        SELECT SUM(T.UNID_MIN_SUG)
    ...
    -- Asumo que
    -- A partir de aqui no se debe superar el m√°ximo del grupo
      ...
      FOR v_cur_ProdGrupo IN curProdGrupo(v_cur_Grupos.COD_GRUPO) LOOP
         ...
         END IF;
          ...
          ELSE
              ...
              ELSE
                 IF vCodGrupo IS NOT NULL THEN
                    DBMS_OUTPUT.PUT_LINE('Ya se super√≥ el stock m√°ximo del grupo=' || (vStockAdicionalProdSugerido + vStockTotalGrupoUnidMinimas));
                 END IF;
              END IF;
          END IF;
