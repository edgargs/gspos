        ...
        if(UtilityPuntos.isActivoFuncionalidad()){
            ...
            if(tarjetaPuntos != null){
                if(tarjetaPuntos.getNumeroTarjeta().trim().length()>0){
                    ...
                    if(tarjetaPuntos.getIdTransaccion()!=null &&
                       !tarjetaPuntos.getIdTransaccion().equals("0")){//LTAVARA 20.04.2015 cabecera anterior - QUOTE no tiene conexion
                        parametros.add(tarjetaPuntos.getNumeroTarjeta().trim());
                        parametros.add(tarjetaPuntos.getIdTransaccion().trim());
