...
-------------------------------------------------------------------
  ...
  --Descripcion:  Actualizar el stock de los productos tipo componente
  --Fecha                 Usuario        Comentario.
  --17/10/2014      ASOSA          Creacion
   PROCEDURE BTLMF_P_ACT_STK_PROD_COMP(cCodGrupoCia_in      IN CHAR,
                                       cCodLocal_in         IN CHAR,
                                      cNumPedVta_in        IN CHAR,
                                     cCodMotKardex_in    IN CHAR,
                                      cTipDocKardex_in    IN CHAR,
...
*/
  ...
  BEGIN
    OPEN curDoc FOR
      ...
      ORDER BY F.COD_DOCUMENTO_VERIFICACION;
    RETURN curDoc;
  END;
   ...
   BEGIN
       SELECT r.flg_imprime
        ...
        WHEN NO_DATA_FOUND THEN
              vFlagImprime := NULL;
    END;
    -- KMONCADA 22.01.2016 GENERAR EN TABLAS TEMPORALES PARA PROFORMA
    ...
    ELSE
      insert into TMP_CON_BTL_MF_PED_VTA
        (COD_GRUPO_CIA,
         COD_LOCAL,
         NUM_PED_VTA,
         COD_CAMPO,
...
RETURN CHAR IS
 ...
 -- KMONCADA 14.06.2016 SE ASIGNA VALOR POR DEFECTO
  ...
  vNumPedVtaProforma TMP_VTA_PEDIDO_VTA_CAB.NUM_PED_VTA%TYPE;
 BEGIN
           ...
           ELSIF cTipoComp = COD_TIP_COMP_TICKET THEN
            IF(vTipProcPago='1') THEN
                ...
                ELSE
                 desComprobantePago := 'TICKET';
                END IF;
		   ELSIF cTipoComp = COD_TIP_COMP_NOTA_CRED THEN
...
AND A.COD_GRUPO_CIA = cCodGrupoCia_in;
  ...
  BEGIN
    ...
    LOOP
          ...
          SELECT USU_MOD_PROD_LOCAL, FEC_MOD_PROD_LOCAL, STK_FISICO, 0
          INTO usuMod, fecMod, stkFis, stkComp
          FROM LGT_PROD_LOCAL
          WHERE COD_GRUPO_CIA = cCodGrupoCia_in
          AND     COD_LOCAL = cCodLocal_in
