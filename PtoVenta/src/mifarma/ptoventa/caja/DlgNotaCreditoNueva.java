        ...
        try{
            ...
            if(FarmaVariables.vAceptar){
                ...
                for(int i=0;i<pListaPedidos.length;i++ ){
                    ...
                    String nroNC = DBCaja.agregarDetalleNotaCredito(VariablesCaja.vNumPedVta_Anul,numera,"0","0","0.0",
                                                      i+"" //ERIOS 2.4.3 Indica el secuencial de NC
                                                      );
                    
                    if("N".equalsIgnoreCase(nroNC)){
                        nroCorrecto = nroCorrecto && false;
                        log.error("ERROR AL GENERAR LA NOTA DE CREDITO DEL PEDIDO "+VariablesCaja.vNumPedVta_Anul);
                    }else{
                        if(!"G".equalsIgnoreCase(nroNC) && conexionRAC){
                ...
                for(int i=0;i<tblListaProductos.getRowCount();i++){
                    ...
                    if(VariablesCaja.vTipComp.equals(ConstantsVentas.TIPO_COMP_BOLETA)||VariablesCaja.vTipComp.equals(ConstantsVentas.TIPO_COMP_TICKET)){
