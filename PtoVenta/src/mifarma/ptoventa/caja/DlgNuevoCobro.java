...
public class DlgNuevoCobro extends JDialog {
    ...
    private boolean bEntregaProforma = false;
    private boolean isCobrandoProforma = false;
    ...
    private void jbInit() throws Exception {
        this.setSize(new Dimension(447, 546));
        ...
        jPanel3.setBounds(new Rectangle(10, 225, 420, 35));
        ...
        jPanel1.setBounds(new Rectangle(10, 290, 420, 125));
        ...
        lblEsc.setBounds(new Rectangle(340, 475, 90, 35));
        lblF11.setText("<HTML><CENTER>[F11]<BR>Aceptar</HTML></CENTER>");
        lblF11.setBounds(new Rectangle(230, 475, 90, 35));
        pnlTotales.setBounds(new Rectangle(10, 425, 420, 40));
        ...
        lblF9.setBounds(new Rectangle(10, 475, 90, 35));
        ...
        lblMontoRedencion_T1.setText("(S/.");
        ...
        lblF10.setBounds(new Rectangle(120, 475, 90, 35));
        lblF10.setText("<HTML><CENTER>[ F10 ]<BR>Nota Cr\u00e9dito</HTML></CENTER>");
        lblAhorro_T.setText("Ud. ha ahorrado : S/.");
        ...
        pnlPuntos2.setBounds(new Rectangle(10, 265, 420, 20));
        ...
        lblPuntosRedimir.setBounds(new Rectangle(125, 0, 85, 20));
        ...
        // KMONCADA 18.04.2016
        lblTotalPercepcion.setText("Total Percepcion : S/.");
        ...
        lblTotalPedido.setText("Total Pedido : S/.");
        ...
        if (e.getKeyCode() == KeyEvent.VK_F9) {
            if(vActivaF9){
                ...
                if(VariablesPuntos.frmPuntos != null && VariablesPuntos.frmPuntos.getTarjetaBean() != null){
                    ...
                    if(FarmaPuntosConstante.EstadoTarjeta.BLOQUEADA_REDIMIR.equalsIgnoreCase(VariablesPuntos.frmPuntos.getTarjetaBean().getEstadoTarjeta()) ||
                       facadePtos.verificaClienteBloqueadoRedime(VariablesPuntos.frmPuntos.getTarjetaBean().getDni())){
                    ...
                    if (JConfirmDialog.rptaConfirmDialog(this, "La venta será anulada. ¿Desea continuar?")) {
                        try { //ERIOS 11.10.2013 Nuevo modelo cobro
                            /*if (VariablesCaja.vIndPedidoCobrado) {
                                ...
                                DBCaja.anularPedidoPendienteSinRespaldo_02(this,VariablesCaja.vNumPedVta);
                            //}
    ...
    private void muestraInfoPedido(ArrayList pArrayList) {
        ...
        if (intTipoCobro == ConstantsCaja.COBRO_PEDIDO) {
            if(isCobrandoProforma){
                String estPedido = ((String)((ArrayList)pArrayList.get(0)).get(22)).trim();
                if (!UtilityCaja.verificaEstadoProforma(this, VariablesCaja.vNumPedVta, ConstantsCaja.ESTADO_PENDIENTE,
                                                        txtMontoPagado, estPedido)) {
                    VariablesCaja.vIndPedidoSeleccionado = FarmaConstants.INDICADOR_N;
                    return;
                }
            }else{
        ...
        if (e.getKeyCode() == KeyEvent.VK_ENTER || Ejecuta) {
            ...
            if (tmpMontoTarjeta > 0) {
                if (tmpMontoTarjeta != tmpMontoSaldo) {
                    FarmaUtility.showMessage(this, "El monto de tarjeta, debe ser el monto del saldo a pagar.",
                                             txtMontoTarjeta);
                } else {
    ...
    private void initVariablesCobro(boolean pEjecOldCobro) {
        if (indPedirLogueo && !pEjecOldCobro) {
            ...
            if (!UtilityCaja.existeCajaUsuarioImpresora(this, null, bEntregaProforma, isCobrandoProforma) ||
                !UtilityCaja.validaFechaMovimientoCaja(this, txtMontoPagado)) {
                if(isCobrandoProforma){
                    FarmaUtility.showMessage(this, "Verifique y Vuelva a generar el cobro.", null);
        ...
        try {
            if (intTipoCobro == ConstantsCaja.COBRO_PEDIDO) {
                VariablesCaja.vFecPedACobrar =
                        ""; //Problemas al generar la venta, cuando la variable tiene contenido.   Autor Luigy Terrazos
                if(isCobrandoProforma){
                    DBProforma.obtieneInfoCobrarPedido(myArray, VariablesCaja.vNumPedPendiente, VariablesCaja.vFecPedACobrar);
                }else{
    ...
    public void cargaFormasPagoSinConvenio(String numPed, String indConvenio, String codConvenio, String codCliente) {
        if(isCobrandoProforma){
            numPed = VariablesDelivery.vNumeroPedido;
        }else if (VariablesVentas.vNum_Ped_Vta.trim().length() > 0) {
        ...
        try {
            DBCaja.obtieneFormasPagoSinConvenio(tableModelFormasPago, indConvenio, codConvenio, codCliente, numPed, isCobrandoProforma);
        } catch (SQLException sql) {
            log.error("", sql);
    ...
    private void muestraInfoPedidoRecau(ArrayList pArrayList) {
        ...
        lblMensaje.setVisible(true);
        lblMensaje.setText("<html> Recaudación: " + strTipoRecau + " <br> Sólo podrá anularse dentro de " +
     ...
     * @author ERIOS
     * @since 11.06.2013
     * @since 28.01.2016 Para local-mayorista, se envia el codigo de convenio.
     */
    private void adicionaDetallePagoCredito() {
        double montoCredito =
            UtilityConvenioBTLMF.obtieneMontoCredito(this, null, new Double(FarmaUtility.getDecimalNumber(VariablesCaja.vValTotalPagar)),
                                                    ...
                                                    "", VariablesConvenioBTLMF.vCodConvenio, vValorSelCopago);
        ...
        if (validarFormaPago()) {
            if (intTipoCobro == ConstantsCaja.COBRO_CAJA_ELECTRONICA) {
                ...
                /*
                  ...
                  * */
                if(!isCobrandoProforma){
                    UtilityEposTrx.vPermiteCobro(this);
                }
                //if (UtilityEposTrx.vPermiteCobro(this)) {
                    ...
                    if (intTipoCobro == ConstantsCaja.COBRO_PEDIDO) {
                        ...
                        if(isCobrandoProforma){
                            grabarFormaPagoProforma();
                            if(VariablesCaja.vIndPedidoCobrado){
                                pedidoCobrado(null);
                                cerrarVentana(true, null);
                            }
                        }else if (VariablesVentas.vEsPedidoDelivery &&
                            ...
                            UtilityCaja.grabarNuevoCobro(this, dlgFormPago, myParentFrame, bEntregaProforma);
    ...
    public void procesarNuevoCobro() {
        ...
        UtilityCaja.procesarPedidoNuevoCobro(myParentFrame, this, lblVuelto, tblDetallePago, txtMontoPagado, bEntregaProforma);
    ...
    private void funcionF10() {
        ...
        if (FarmaVariables.vAceptar) {            
            ...
            adicionaDetallePago(null);
            FarmaUtility.showMessage(this, "Se agregó la NCR al pedido. Monto de uso total S/. "+VariablesCaja.vValTotalPagado,null);
            funcionF11();
        }   
    }
    ...
    public void setEntregaProforma(boolean bEntregaProforma) {
        this.bEntregaProforma = bEntregaProforma;
    }

    public void setCobrandoProforma(boolean isCobrandoProforma) {
        this.isCobrandoProforma = isCobrandoProforma;
    }
    
    public void grabarFormaPagoProforma() {
        try {
            //obtiene el monto del vuelto
            String vueltoPedido = lblVuelto.getText().trim();
            log.debug("vuelto del cobro: " + vueltoPedido);
            ...
            for (int i = 0; i < tblDetallePago.getRowCount(); i++) {
                //grabar forma de pago del pedido
                //descripcion de la forma de pago en el detalle
                DBProforma.grabaFormaPagoPedido(((String)tblDetallePago.getValueAt(i, 0)).trim(),
                                            ((String)tblDetallePago.getValueAt(i, 4)).trim(),
                                            ((String)tblDetallePago.getValueAt(i, 6)).trim(),
                                            VariablesCaja.vValTipoCambioPedido,
                                            ((String)tblDetallePago.getValueAt(i, 7)).trim(),
            ...
            //2.1 Valida formas de pago campana
            String vResValidaCampConFormaPAgo = DBProforma.verificaPagoUsoCampana(VariablesDelivery.vNumeroPedido);
            log.warn("verificaPagoUsoCampana ERROR:   " + vResValidaCampConFormaPAgo);
            if (vResValidaCampConFormaPAgo.trim().equalsIgnoreCase("ERROR")) {
                log.warn("verificaPagoUsoCampana ERROR");
                FarmaUtility.liberarTransaccion();
            ...
            //2.2 Valida montos de pedido
            String v_Resultado = DBProforma.verificaPedidoFormasPago(VariablesDelivery.vNumeroPedido);
            ...
            log.info("[LOCAL M]: SE CARGARA DETALLE DE LOTES POR PRODUCTO");
            //KMONCADA 08.06.2016 [PROYECTO M] FINALIZANDO EL COBRO DE LA PROFORMA
            //SE REALIZAN LOS PROCESOS PARA ENVIO DE PROFORMA A DESPACHAR
            (new UtilityDespacho()).enviarProformaADespacho(this, VariablesDelivery.vNumeroPedido, ConstantsVentas.ESTADO_PEDIDO_SIN_COMPROBANTE);
            //SE IMPRIME CONSTANCIA DE PAGO
            (new UtilityProforma()).imprimirConstanciaPagoProforma(this, VariablesDelivery.vNumeroPedido);
            
            FarmaUtility.aceptarTransaccion();
            VariablesCaja.vIndPedidoCobrado = true;
            FarmaUtility.showMessage(this, "Formas de pago Registradas con Exito.", tblFormasPago);
        } catch (Exception e) { //No continuar con la transaccion
            FarmaUtility.liberarTransaccion();
            //VariablesCaja.vIndPedidoCobrado = false;
            log.error("", e);
