--------------------------------------------------------
--  DDL for Package Body PTOVENTA_REPORTE_PACK_DU
--------------------------------------------------------
   ...
   INSERT INTO V_RES_VTA_REP_PACK
        ...
        SELECT C.COD_GRUPO_CIA,
         ...
         WHERE C.COD_GRUPO_CIA = '001'
           AND C.COD_LOCAL IN (SELECT DISTINCT COD_LOCAL FROM VTA_IMPR_LOCAL I)
...
/* ********************************************************* */
 ...
 PROCEDURE CREA_RESUMEN(fecInicio in char, fecFin    in char)
 IS
 BEGIN
  ...
  carga_detalle_cabecera(fecInicio, fecFin);
  COMMIT;
