...
import mifarma.ptoventa.reportes.*;
...
public class DlgListaDiferenciasConsolidadoDiario extends JDialog {
  ...
  private JLabelFunction lblF8 = new JLabelFunction();
	...
	private void initTableListaDiferenciasConsolidado() {
		tableModelDiferenciasConsolidadoDiario = new FarmaTableModel(ConstantsInvDiario.columnsListaDiferenciasConsolidado,ConstantsInvDiario.defaultValuesListaDiferenciasConsolidado, 0);
		FarmaUtility.initSimpleList(tblRelacionDiferenciasProductos,tableModelDiferenciasConsolidadoDiario,ConstantsInvDiario.columnsListaDiferenciasConsolidado);
    ...
    try {
      DBInvDiario.listaDiferenciasConsolidado(tableModelDiferenciasConsolidadoDiario,VariablesInvDiario.vSecToma);
      if (tblRelacionDiferenciasProductos.getRowCount() > 0)
        FarmaUtility.ordenar(tblRelacionDiferenciasProductos,tableModelDiferenciasConsolidadoDiario, 1,FarmaConstants.ORDEN_ASCENDENTE);
  ...
  private void cargaListaFiltro() {
	...
	private void filtrarTablaProductos() {
      ...
      {
        DBInvDiario.listaDiferenciasConsolidadoFiltro(tableModelDiferenciasConsolidadoDiario,VariablesInvDiario.vSecToma);
        FarmaUtility.ordenar(tblRelacionDiferenciasProductos, tableModelDiferenciasConsolidadoDiario, 1,FarmaConstants.ORDEN_ASCENDENTE);
        lblTotal.setText(FarmaUtility.formatNumber(FarmaUtility.sumColumTable(tblRelacionDiferenciasProductos,5)));  
      }
      if(tblRelacionDiferenciasProductos.getRowCount() <= 0)
  ...
  private void ordenarFiltro()
  {
    if(tblRelacionDiferenciasProductos.getRowCount()>0)
    {
      ...
      FarmaLoadCVL.loadCVLfromArrays(dlgOrdenar.getCmbCampo(),
                                     ...
                                     ConstantsInvDiario.vCodDiferencia,
                                     ConstantsInvDiario.vDescCampoDiferencia,
                                     true);
      dlgOrdenar.setVisible(true);
      if(FarmaVariables.vAceptar)
      {
        FarmaUtility.ordenar(tblRelacionDiferenciasProductos, tableModelDiferenciasConsolidadoDiario,VariablesReporte.vCampo, VariablesReporte.vOrden);
