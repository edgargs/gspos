...
public class DlgIngresoCantLote extends JDialog {
    ...
    private ContextProforma contextProforma;
    private String pValFracVta;
    private String pSecDetPedVta;
    private String pValFracLocal;
    private double stkActualLote;
    ...
    private void cargaListaDetalleProdLote(String pNumPedido, String pCodProd) {
        try {
            contextProforma.cargaListaDetProductoLote(tableModelDetalleIngreso, pNumPedido, pCodProd, pSecDetPedVta);
            lblCantTmp.setText("" +
    ...
    private void validaCodigo(JTextField pJTextField_Cod, String pTipoMaestro) {
        if (pJTextField_Cod.getText().trim().length() > 0) {
            ...
            if (existeLoteProducto(VariablesDelivery.vCodProducto, numLote)) {
                ...
                //KMONCADA 04.08.2016 [PROYECTO M] NO SE PERMITE REGISTRAR LOTES SOLO POR MEDIDO DE AJUSTES O RECEPCION
            ...
            //KMONCADA 04.08.2016 [PROYECTO M] SE DEBE DE REGISTRAR UN VALOR EN EL LOTE
            if(UtilityPtoVenta.isLocalVentaMayorista()){
                FarmaUtility.showMessage(this, "No ha ingresado dato!!!", txtLote);
                return;
            }
            cargaListaMaestros(pTipoMaestro, pJTextField_Cod);
        }
    ...
    private void cargaListaMaestros(String pTipoMaestro, JTextField pJTextField_Cod) {
        ...
        //ERIOS 29.01.2016 Para local-mayorista es obligatorio el control de lote.
        if(UtilityPtoVenta.isLocalVentaMayorista()){
            isControlLote = false;
        }
        // KMONCADA 03.07.2014
        ...
        if (!loteVacio) {
            ...
            int pCant = FarmaUtility.trunc(VariablesDelivery.vCantidadLista);
            int valFrac = FarmaUtility.trunc(pValFracVta);
            int valFracLocal = FarmaUtility.trunc(pValFracLocal);
            dlgListaMaestros.setLocalMayorista(UtilityPtoVenta.isLocalVentaMayorista());
            dlgListaMaestros.setStockLocalLote((pCant*valFracLocal)/valFrac);
    ...
    private void adicionaDetalleIngreso() {
        ...
        // KMONCADA 04.08.2016 [PROYECTO M] VALIDA STOCK DEL LOTE A UTILIZAR
        if(!validaStockLote()){
            FarmaUtility.showMessage(this, "Lote ingresado no cuenta con Stock suficiente.", txtCantidad);
            return;
        }
    ...
    /**
     * 
     * @since 04.08.2016 [PROYECTO M] SE MODIFICO PARA OBTENER STOCK DEL LOTE.
     * @param pCodProd
     * @param pNumLote
     * @return
     */
    private boolean existeLoteProducto(String pCodProd, String pNumLote) {
        ...
        boolean valor = false;
        try {
            stkLote = DBDelivery.obtieneIndicadorExisteLote(pCodProd, pNumLote, VariablesDelivery.vNumeroPedido);
        } catch (Exception sql) {
            log.error("", sql);
            //indicador = FarmaConstants.INDICADOR_N;
            stkLote = "-1";
        } finally {
            stkActualLote = FarmaUtility.getDecimalNumber(stkLote);
            if (stkActualLote != -1 )
                valor = true;
            else
                valor = false;
    ...
    private boolean eliminaBDProdLote(int pRow) {
        try {
            contextProforma.eliminaDetalleProducto(FarmaUtility.getValueFieldJTable(tblListaIngresoLote, pRow, 2),
    ...
    private void grabaDetalleProductoLote() {
        try {
            ...
            contextProforma.agregaDetalleProductoLote(tblListaIngresoLote,pSecDetPedVta,pValFracVta);
    ...
    void setContext(ContextProforma contextProforma) {
        this.contextProforma = contextProforma;
    }

    void setValFracVta(String pValFracVta, String pValFracLocal) {
        this.pValFracVta = pValFracVta;
        this.pValFracLocal = pValFracLocal;
    }

    void setSecPedVta(String pSecDetPedVta) {
        this.pSecDetPedVta = pSecDetPedVta;
    }
    
    /**
     * VALIDA STOCK DEL LOTE A UTILIZAR.
     * 
     * @author KMONCADA
     * @since 04.08.2016
     * @return
     */
    private boolean validaStockLote(){
        if(UtilityPtoVenta.isLocalVentaMayorista()){
            double valFracLocal = FarmaUtility.getDecimalNumber(pValFracLocal);
            double valFracVta = FarmaUtility.getDecimalNumber(pValFracVta);
            double stkFracVta = ((stkActualLote * valFracVta) / valFracLocal);
            double cantIngresada = FarmaUtility.getDecimalNumber(txtCantidad.getText().trim());
            return (cantIngresada<=stkFracVta);
        }else{
            return true;
        }
    }
