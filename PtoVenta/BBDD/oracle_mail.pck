CREATE OR REPLACE PACKAGE PTOVENTA."ORACLE_MAIL" AS

  ----------------------- Customizable Section -----------------------

  -- Customize the SMTP host, port and your domain name below.
  smtp_host   VARCHAR2(256) := '192.168.0.206';
  smtp_port   PLS_INTEGER   := 25;
  smtp_domain VARCHAR2(256) := 'mifarma.com.pe';
  ...
  --Descripcion: begin_mail.
  --Fecha       Usuario		Comentario
  --16/06/2006  ERIOS    	Modificación:  CCrecipients
  ...
  --Descripcion: begin_session.
  --Fecha       Usuario		Comentario
  --15/06/2006  ERIOS    	Creación
  FUNCTION begin_session(v_smtp_host IN VARCHAR2) RETURN utl_smtp.connection;
...
CREATE OR REPLACE PACKAGE BODY PTOVENTA."ORACLE_MAIL" AS
  ...
  BEGIN
	...
	addr := substr(addr, 1, i - 1);
      END IF;
    END IF;
   if (instr(addr,chr(64))=0) then
      addr:=addr||chr(64)||smtp_domain;
   end if;
--   dbms_output.put_line(addr);
  ...
  PROCEDURE begin_mail_in_session(conn       IN OUT NOCOPY utl_smtp.connection,
    ...
    my_recipients2 VARCHAR2(32767) := recipients;
    my_recipients3 VARCHAR2(32767) ;
    my_sender     VARCHAR2(32767) := sender;
    my_CCrecipients VARCHAR2(32767) := CCrecipients;
    my_CCrecipients2 VARCHAR2(32767) := CCrecipients;
    my_CCrecipients3 VARCHAR2(32767) :='';
  BEGIN
    ...
    END LOOP;
    --**I
    WHILE (my_recipients2 IS NOT NULL) LOOP
       my_recipients3:=my_recipients3||get_address(my_recipients2)||';';
    END LOOP;
    --**F
    IF CCrecipients IS NOT NULL THEN
      ...
      END LOOP;
      --*I
      WHILE (my_CCrecipients2 IS NOT NULL) LOOP
        my_CCrecipients3:=my_CCrecipients2||get_address(my_CCrecipients2)||';';
      END LOOP;
      --*F
    END IF;
    -- Start body of email
...
--    write_mime_header(conn, 'To', recipients);
    write_mime_header(conn, 'To', my_recipients3);

    IF CCrecipients IS NOT NULL THEN
      -- Set "CC" MIME header
--       write_mime_header(conn, 'CC',CCrecipients);
    write_mime_header(conn, 'CC', my_CCrecipients3);
    END IF;
