...
// **************************************************************************
  ...
  {
    tableModelListaPrecioProductosGratuitos = new FarmaTableModel(ConstantsVentas.columnsListaProductos,ConstantsVentas.defaultValuesListaProductos,0);
    tableModelListaPrecioProductosGratuitos.data = VariablesVentas.tableModelListaGlobalProductos.data;
    FarmaUtility.initSelectList(tblProductos,tableModelListaPrecioProductosGratuitos,ConstantsVentas.columnsListaProductos);
  ...
  private void initTableProductosAlternativos()
  {
    ...
    tableModelListaProductosAlternativosGratuitos = new FarmaTableModel(ConstantsVentas.columnsListaProductos,ConstantsVentas.defaultValuesListaProductos,0);
    FarmaUtility.initSelectList(tblListaProductosAlternativos,tableModelListaProductosAlternativosGratuitos,ConstantsVentas.columnsListaProductos);
    tblListaProductosAlternativos.setName(ConstantsVentas.NAME_TABLA_SUSTITUTOS);
  ...
  public void iniciaProceso(boolean pInicializar) 
  {
    ...
    System.out.println("VariablesVentas.vArrayList_PedidoVenta : " + VariablesVentas.vArrayList_PedidoVenta.size());
...
// **************************************************************************
  ...
  private void muestraIngresoCantidad()
  {
    ...
    VariablesVentas.vDesc_Prod = ((String)(myJTable.getValueAt(myJTable.getSelectedRow(),2))).trim();
    VariablesVentas.vNom_Lab = ((String)(myJTable.getValueAt(myJTable.getSelectedRow(),4))).trim();
    VariablesVentas.vPorc_Igv_Prod = ((String)(myJTable.getValueAt(myJTable.getSelectedRow(),11))).trim();
  ...
  private void colocaTotalesPedido()
  {
    ...
    totalItems = VariablesVentas.vArrayList_PedidoVenta.size();
  ...
  private void calculaTotalVentaPedido() {
    totalVenta = 0;
    for(int i=0; i<VariablesVentas.vArrayList_PedidoVenta.size(); i++)
      totalVenta += FarmaUtility.getDecimalNumber(((String)((ArrayList)VariablesVentas.vArrayList_PedidoVenta.get(i)).get(7)).trim());
  }
  ...
  private void seleccionaProducto()
  {
    ...
    if(!actualizaStkComprometidoProd(VariablesVentas.vCod_Prod,Integer.parseInt(VariablesVentas.vCant_Ingresada),ConstantsVentas.INDICADOR_A, ConstantsPtoVenta.TIP_OPERACION_RESPALDO_SUMAR))
      return;
  ...
  private void deseleccionaProducto()
  {
    ...
    Boolean valorTmp = (Boolean)(myJTable.getValueAt(myJTable.getSelectedRow(),0));
    if(!actualizaStkComprometidoProd(VariablesVentas.vCod_Prod,Integer.parseInt(cantidad),ConstantsVentas.INDICADOR_D, ConstantsPtoVenta.TIP_OPERACION_RESPALDO_BORRAR))
      return;
    FarmaUtility.setCheckValue(myJTable,false);
    Boolean valor = (Boolean)(myJTable.getValueAt(myJTable.getSelectedRow(),0));
    operaProductoSeleccionadoEnArrayList(valor);
  ...
  {
    ArrayList myArray = new ArrayList();
    myArray.add(VariablesVentas.vCod_Prod);
    ...
    myArray.add(VariablesVentas.vPorc_Dcto_1);
    ...
    myArray.add(VariablesVentas.vNom_Lab);
  ...
  private boolean actualizaStkComprometidoProd(String pCodigoProducto, int pCantidad, String pTipoStkComprometido, String pTipoRespaldoStock) {
    try {
      DBVentas.actualizaStkComprometidoProd(pCodigoProducto,pCantidad,pTipoStkComprometido);
      DBPtoVenta.ejecutaRespaldoStock(pCodigoProducto, "", pTipoRespaldoStock, pCantidad, Integer.parseInt(VariablesVentas.vVal_Frac),ConstantsPtoVenta.MODULO_VENTAS);
      FarmaUtility.aceptarTransaccion();
      return true;
    } catch (SQLException sql) {
  ...
  private void cancelaOperacion()
  {
    ...
    for(int i=0; i<VariablesVentas.vArrayList_PedidoVenta.size(); i++) {
      ...
      if(!existe) 
        if(!actualizaStkComprometidoProd(codProd, FarmaUtility.trunc(FarmaUtility.getDecimalNumber(cantidad)),ConstantsVentas.INDICADOR_D, ConstantsPtoVenta.TIP_OPERACION_RESPALDO_BORRAR))
          return;
      existe = false;
    }
    for(int i=0; i<VariablesVentas.vArrayList_ResumenPedido.size(); i++) {
      ...
      if(!existe) 
        if(!actualizaStkComprometidoProd(codProd, FarmaUtility.trunc(FarmaUtility.getDecimalNumber(cantidad)),ConstantsVentas.INDICADOR_A, ConstantsPtoVenta.TIP_OPERACION_RESPALDO_SUMAR))
          return;
      else{
        if(Integer.parseInt(cantidad) < Integer.parseInt(cantidadTmp))
          if(!actualizaStkComprometidoProd(codProd, (Integer.parseInt(cantidadTmp) - Integer.parseInt(cantidad)),ConstantsVentas.INDICADOR_D, ConstantsPtoVenta.TIP_OPERACION_RESPALDO_BORRAR))
            return;
        else if(Integer.parseInt(cantidad) > Integer.parseInt(cantidadTmp))
          if(!actualizaStkComprometidoProd(codProd, (Integer.parseInt(cantidad) - Integer.parseInt(cantidadTmp)),ConstantsVentas.INDICADOR_A, ConstantsPtoVenta.TIP_OPERACION_RESPALDO_SUMAR))
            return;
      }
      existe = false;
    }
  ...
  private void filtrarTablaProductos(String pTipoFiltro,
    ...
    try {
      tableModelListaPrecioProductosGratuitos.clearTable();
      tableModelListaPrecioProductosGratuitos.fireTableDataChanged();
      DBVentas.cargaListaProductosVenta_Filtro(tableModelListaPrecioProductosGratuitos,
                                               pTipoFiltro,
                                               pCodFiltro);
  ...
  private void muestraProductosAlternativos() {
    try {
      if ( tblProductos.getSelectedRow() >= 0 ) {
        ...
        DBVentas.cargaListaProductosAlternativos(tableModelListaProductosAlternativosGratuitos,codigoProducto);
  ...
  private void pintaCheckOtroJTable(JTable pActualJTable, Boolean pValor) {
    ...
    if(pActualJTable.getName().equalsIgnoreCase(ConstantsVentas.NAME_TABLA_PRODUCTOS)) {
      FarmaUtility.setearCheckInRow(tblListaProductosAlternativos,pValor,true,true,VariablesVentas.vCod_Prod,1);
      tblListaProductosAlternativos.repaint();
    } else if(pActualJTable.getName().equalsIgnoreCase(ConstantsVentas.NAME_TABLA_SUSTITUTOS)) {
      FarmaUtility.setearCheckInRow(tblProductos,pValor,true,true,VariablesVentas.vCod_Prod,1);
  ...
  {
    if(myJTable.getRowCount() == 0) return;
    VariablesVentas.vCodProdOrigen_Comple = ((String)(myJTable.getValueAt(myJTable.getSelectedRow(),1))).trim();
    VariablesVentas.vDescProdOrigen_Comple = ((String)(myJTable.getValueAt(myJTable.getSelectedRow(),2))).trim();
    ...
    if(FarmaVariables.vAceptar)
    {
      System.out.println("VariablesVentas.vCodProdComplementario : " + VariablesVentas.vCodProdComplementario);
