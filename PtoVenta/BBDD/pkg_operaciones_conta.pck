...
/
create or replace package body PKG_OPERACIONES_CONTA as
...
-- EMAQUERA -- Se valida que el sobre no este asociado a un remito
  ...
  WHERE A.COD_SOBRE=cSobre;
    ...
    IF Remito IS NULL THEN
      SELECT COUNT(*) INTO v_Sobre FROM CE_SOBRE 
      WHERE COD_GRUPO_CIA='001'
      AND COD_LOCAL= v_Cod_local
      AND COD_SOBRE= cSobre;
      ...
      SELECT A.FEC_DIA_VTA, A.COD_SOBRE ,
        ...
        FROM CE_SOBRE A, CE_FORMA_PAGO_ENTREGA B
         ...
         AND A.SEC_FORMA_PAGO_ENTREGA = B.SEC_FORMA_PAGO_ENTREGA) N
         WHERE N.COD_SOBRE=cSobre;
       ELSE
        SELECT TO_CHAR(N.FEC_DIA_VTA,'DD/MM/YYYY'), N.COD_SOBRE, N.FORMA_PAGO, N.MONTO, N.USU_CREA_SOBRE
        INTO v_Fecha, v_Cod_sobre, v_Forma_Pago, v_Monto, v_Usu  FROM (
           SELECT FEC_DIA_VTA, COD_SOBRE, DECODE(TIP_MONEDA, '01', 'SOLES', '02', 'DOLARES') FORMA_PAGO,
                  TRIM(TO_CHAR(NVL(CASE
           ...
           WHERE COD_GRUPO_CIA = '001'
           AND COD_LOCAL = v_Cod_local
           AND FEC_DIA_VTA >=TRUNC(SYSDATE-10)) N
           WHERE N.COD_SOBRE=cSobre;         
       END IF;
...
Monto_Soles varchar2(20);
Monto_Dolares varchar2(20);
Monto_Total varchar2(25);
...
-- EMAQUERA -- Se verifica la fecha este dentro del rango
    SELECT COUNT(DISTINCT FEC_DIA_VTA) INTO Ind_Fecha FROM CE_MOV_CAJA
    WHERE COD_GRUPO_CIA='001' AND COD_LOCAL = v_Cod_local
    AND FEC_DIA_VTA=v_Fecha AND FEC_DIA_VTA>=SYSDATE-10;
...
-- EMAQUERA -- Se valida que exista el remito en el dia indicado
  ...
  AND FEC_CREA_REMITO>=SYSDATE-10;
    ...
    IF Ind_Remito > 0 THEN
      ...
      WHERE COD_GRUPO_CIA='001' AND COD_LOCAL = v_Cod_local
      AND TRUNC(FEC_CREA_REMITO)=v_Fecha AND FEC_CREA_REMITO>=SYSDATE-10;
       ...
       IF Max_Remito = Remito THEN
        ...
        SELECT COUNT(*) INTO Remito_Apr FROM CE_REMITO
        WHERE COD_GRUPO_CIA='001' AND COD_LOCAL = v_Cod_local
        AND COD_REMITO = v_Codigo_Remito AND FEC_PROCESO_INT_CE IS NULL
        AND FEC_PROCESO_ARCHIVO IS NULL;

        IF Remito_Apr > 0 THEN
          -----
          ...
          ELSE
            message:='<B>NO SE PUEDE ANULAR EL REMITO DE LA FECHA '||v_Fecha||' PORQUE YA FUE PROCESADO EN SAP.</B>';
          END IF;
       ELSE
