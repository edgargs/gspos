...
DBFidelizacion.isValidaPedidoFidelizadoMatriz(pNumPed, pRuc, VariablesFidelizacion.vDniCliente);
    ...
    public static boolean validarRUC(String sNumeroRuc){
        ...
        sNumeroRuc = sNumeroRuc.trim();
        if (sNumeroRuc.length() == 11){
            try{
                sSerie = Integer.parseInt(sNumeroRuc.substring(0, 2));
                if (sSerie == 10 || sSerie == 15 || sSerie == 17 || sSerie == 20){
                    nSuma = 0;
                    nValor = 6;
                    for (int i = 0; i <= 9; i++){
                        if (i == 4){
                        ...
                        sNumero = sNumeroRuc.substring(i, (i + 1));
                        nDigito = Integer.parseInt(sNumero);
                        nValor = nValor - 1;
                        nSuma = nSuma + (nDigito * nValor);
                    }
                    nResto = nSuma%11;
                    nResto = 11 - nResto;
                    if (nResto >= 10){
                    ...
                    if (nResto == Integer.parseInt(sNumeroRuc.substring(10, 11))){
                        bValido = true;
                    }
                }
            }catch (Exception e){
