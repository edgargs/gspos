CREATE OR REPLACE PACKAGE "PTOVENTA_CONSUMO_AUTOMATICO" AS
  ...
  v_correo    varchar2(1000) := 'pyovera@mifarma.com.pe,loc_despachos@mifarma.com.pe';
...
END; --FIN DEL PACKAGE
/
CREATE OR REPLACE PACKAGE BODY "PTOVENTA_CONSUMO_AUTOMATICO" AS

  -- Creación           :  desarrollo4
  -- Motivo             : Lógica del Ajuste automático.
  -- Fecha              : 25.09.2014
  -- Requerimiento      : Ajuste Autoático de Insumos
  -- Jefe Requriemiento : JOLIVA
  -- Usuario interesado : PYOVERA
  ------------------------------------------------
  -- Usu y Fecha Modific.       : 15.12.2014
  -- Motivo                     : Se valida cuando no exista Stock, no se acumle las cantidad a ajustar.
  ------------------------------------------------
  -- Usu y Fecha Modific.       :
  -- Motivo                     :
  PROCEDURE P_PROCESAR_AJUSTE IS
    ...
    q          integer:=0;
    v_dias     number(3)  ; --24.02.2015

    --- Valida productos Inactivos y NO reponer 24.02.2015
    CURSOR cListaActivos IS
     select LPL.COD_LOCAL, LPL.COD_PROD, P.EST_PROD, LPL.EST_PROD_LOC, LPL.IND_REPONER
       from lgt_ajuste_auto a, LGT_PROD P, LGT_PROD_LOCAL LPL
      where P.COD_PROD = A.COD_PROD
        and P.COD_PROD = LPL.COD_PROD
        and P.COD_GRUPO_CIA = LPL.COD_GRUPO_CIA
        and A.COD_LOCAL = LPL.COD_LOCAL
        and LPL.COD_GRUPO_CIA   = cCodGrupoCia_in
    ...
    CURSOR cLista IS
      select a.cod_local,
       ...
       where P.COD_PROD = A.COD_PROD
         ...
         and P.COD_GRUPO_CIA = LPL.COD_GRUPO_CIA
         and A.COD_LOCAL     = LPL.COD_LOCAL
         /*** INICIO CCASTILLO 16/05/2016 ***/
         --Se excluye del ajuste a las bolsas, papel termico e insumos para productos finales 
         /*AND a.COD_PROD NOT IN  (SELECT COD_PROD FROM VTA_INSUMO X 
                                 WHERE X.COD_GRUPO_CIA=LPL.COD_GRUPO_CIA AND X.COD_LOCAL=LPL.COD_LOCAL)*/ 
         AND A.COD_PROD NOT IN  (SELECT COD_PROD FROM VTA_INSUMO X 
                                 WHERE X.COD_GRUPO_CIA=LPL.COD_GRUPO_CIA AND X.COD_LOCAL=LPL.COD_LOCAL AND X.TIP_PROD IN ('01','02')
                                 UNION
                                 SELECT A.COD_PROD_HIJO FROM LGT_REL_PROD_TICO A WHERE A.COD_GRUPO_CIA=LPL.COD_GRUPO_CIA 
                                 GROUP BY A.COD_PROD_HIJO
                                 )                       
         /*** FIN CCASTILLO 16/05/2016 ***/
         and LPL.COD_GRUPO_CIA   = cCodGrupoCia_in
         and LPL.IND_REPONER     = VALOR_SI
    ...
    CURSOR cListaMaestros IS
      select lp.cod_local,
       ...
       WHERE --P.COD_PROD = MFP.COD_PROD_MF
                  P.COD_GRUPO_CIA  = LP.COD_GRUPO_CIA
       AND P.COD_PROD       = LP.COD_PROD
       AND P.COD_GRUPO_CIA  = LPL.COD_GRUPO_CIA
       AND P.COD_PROD       = LPL.COD_PROD
       AND LP.COD_LOCAL     = LPL.COD_LOCAL
       AND LP.COD_GRUPO_CIA = cCodGrupoCia_in
       AND LPL.IND_REPONER  = VALOR_SI
       AND P.COD_GRUPO_REP = cCodGrupoInsumo
       AND P.EST_PROD = ESTADO_ACTIVO
  ...
  BEGIN
    ----- ACTUALIZA LA TABLA MAE_PROD_AJUSTAR_AUTO
          MERGE INTO MAE_PROD_AJUSTAR_AUTO A
          USING (SELECT P.COD_PROD , P.DESC_PROD
                 FROM   LGT_PROD P
          ...
          WHEN NOT MATCHED THEN
            INSERT
              (A.COD_PROD, A.DES_PROD, A.IND_AJUSTAR, A.USU_CREA, A.NUM_DIAS)
            VALUES
              (B.COD_PROD, B.DESC_PROD, 'S', 'SYSTEM', 30);

    ----- DESHABILITA PRODUCTO INACTIVOS y NO REPONER para EL AJUSTE
  FOR A in cListaActivos LOOP
      if A.EST_PROD    =  ESTADO_ACTIVO_NO OR
         A.EST_PROD_LOC = ESTADO_ACTIVO_NO OR
         A.IND_REPONER  = VALOR_NO
        then
       ...
       UPDATE LGT_AJUSTE_AUTO L
        ...
        WHERE l.cod_local = A.COD_LOCAL
          AND l.cod_prod  = A.COD_PROD
          AND L.EST_PROD = ESTADO_ACTIVO;

       UPDATE TMP_LGT_AJUSTE_AUTO T
          SET T.FEC_PROCESADO = SYSDATE, T.IND_PROCESADO = VALOR_SI
        WHERE T.cod_local = A.COD_LOCAL
          AND T.cod_prod = A.COD_PROD
          AND T.IND_PROCESADO = VALOR_NO;
    ...
    FOR i in cLista LOOP
      ...
      SELECT count(*)
                     into  q
              FROM   TMP_LGT_AJUSTE_AUTO z
                     WHERE z.fec_ajuste_dia = TO_CHAR(v_SYSDATE, 'DD/MM/YYYY')
                     and   z.cod_local      = i.cod_local
                     and   z.cod_prod       = i.cod_prod;

    IF (q = 0) then

      -- Entro al procedimiento para actualizar la cantidad maxima de movimientos
      -- del producto.
      -- colocado el 16.01.2015
      ...
      --- Carga días a ajustar
      SELECT T.NUM_DIAS
             into v_dias
      FROM   MAE_PROD_AJUSTAR_AUTO T
      WHERE  T.COD_PROD  = i.cod_prod ;

      -- obtenga el valor del ajuste diario.
      --** valore de días ingresados por interfaz
      c_ajuste_dia := round((i.cant_mov_mes / v_dias--v_num_dia
                      ) * v_nValFrac, 3);
      ...
      if cant_x_aj >= 1 then
        ...
        if diferencia >= 0 then
          ...
          --sobrante := cant_x_aj + diferencia;
            sobrante := cant_x_aj - v_nStkFisico ;
        end if;
      
      else
        -- SI  la cantidad final a ajustar final es menor a 1
        --sobrante := c_ajuste_dia;
        --sobrante := cant_x_aj; -- sobrante ===> 0 (cero)
                 --16.01.2015 cambio urgente
                 if v_nStkFisico = 0 then
                         sobrante := 0; -- se reemplazo cant_x_aj por 0 24.02.2015 ; -- sobrante ===> 0 (cero)
                 else
        sobrante := c_ajuste_dia;
      end if;
  ...
  PROCEDURE P_INSERT_AJUSTE_AUTO(cCodLocal    CHAR,
    ...
    valor integer := 0;
    v_secuncia integer :=0;
    v_ind_repone char(1);
    v_ind_ajustar char(1);
  ...
  BEGIN
    ...
    select IND_REPONER
     ...
     --- indicador ajustar
     Begin
       select NVL(t.ind_ajustar, 'N')
         into v_ind_ajustar
         from MAE_PROD_AJUSTAR_AUTO T
        where T.cod_prod = cCodProd;
     exception
       WHEN OTHERS THEN
         v_ind_ajustar := 'N';
     end;
...
--       if  v_ind_repone = 'N'and valor > vcero then
   if ( ( v_ind_repone = 'N' OR
            v_ind_ajustar = 'N'
          )
        AND valor > vcero)
     then --24.02.2015
           --//deshabilita producto que sin authorizacion a reponer
           UPDATE LGT_AJUSTE_AUTO L
              SET l.fec_mod  = v_SYSDATE,
                  l.usu_mod  = USU_CONSUMO_AUTO,
                  l.est_prod = ESTADO_ACTIVO_NO
            WHERE l.cod_local = cCodLocal
              AND l.cod_prod = cCodProd
              AND L.EST_PROD = ESTADO_ACTIVO;
              --24.02.2015 actualizas los acumulados para no utilizarse
            UPDATE TMP_LGT_AJUSTE_AUTO T
                   SET T.FEC_PROCESADO = SYSDATE ,
                       T.IND_PROCESADO = VALOR_SI
            WHERE t.cod_local = cCodLocal
              AND t.cod_prod = cCodProd
              AND t.ind_procesado = VALOR_NO;

              commit;
--       elsif v_ind_repone = 'S' then
   elsif (v_ind_repone = 'S'AND
            v_ind_ajustar = 'S'
                     )then  --24.05.2015
    --// PARA PRODUCTOS CON AUTORIZADOS A REALIZAR REPOSICION
