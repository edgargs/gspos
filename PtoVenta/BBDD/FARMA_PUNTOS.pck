CREATE OR REPLACE PACKAGE FARMA_PUNTOS is
  ...
  COD_PROD_TARJ_PUNTOS           CHAR(6) := '554595';
  ...
  TAB_IND_MSJ_BLOQUEO_REDIME     INTEGER := 541;
  TAB_IND_COD_MOT_KARDEX_TARJ    INTEGER := 545;
  TAB_IND_SOLICITA_DIGITA_DNI    INTEGER := 566;
...
PROCEDURE  P_DESCARTA_TRX_ANULA_ORBIS(cCodGrupoCia_in   IN CHAR,
  ...
  MODIFICACION         01.10.2015 KMONCADA   SE AGREGA LOGICA PARA DETERMINAR ORDEN DE BUSQUEDA 
                                             EN LAS TABLAS DE CALCULO DE PUNTOS MEDIANTE TAB_GRAL
  */
  ...
  */
  ...
  FUNCTION F_LST_MONTOS_REDENCION(cCodGrupoCia_in IN VTA_PEDIDO_VTA_DET.COD_GRUPO_CIA%TYPE,
                                  cCodLocal_in    IN VTA_PEDIDO_VTA_DET.COD_LOCAL%TYPE,
                                  cNumPedVta_in   IN VTA_PEDIDO_VTA_DET.NUM_PED_VTA%TYPE,
                                  cValidaMonto_in IN CHAR DEFAULT 'N')
    RETURN FARMACURSOR;
  ...
  */
  ...
  /**
  KMONCADA 24.07.2015 PARA IMPRESION DE PUNTOS EN TICKETS BOLETA
  */
  FUNCTION F_GET_VOUCHER_PTOS_NE(cCodGrupoCia_in       IN CHAR,
                                  cCodLocal_in          IN CHAR,
                                  cNumPedVta_in         IN CHAR,
                                  cSecCompPago_in       IN CHAR DEFAULT 'N',
  ...
  PROCEDURE P_CTRL_ENTREGA_TARJ_PTOS_NUEVA(cCodGrupoCia_in    IN CHAR,
                                          cCodLocal_in       IN CHAR,
                                          cNumTarjeta_in     IN VARCHAR2,
                                          cNumDocumento_in   IN VARCHAR2,
                                          cUsuCrea_in        IN VARCHAR2)
  ;
  
  FUNCTION F_VAR_MSJ_BLOQUEO_REDIME
    RETURN VARCHAR2;
  
  FUNCTION F_CLIENTE_BLOQUEADO_REDIME(cNumDocumento_in IN VTA_PEDIDO_VTA_DET.COD_GRUPO_CIA%TYPE)
    RETURN CHAR;
    
  PROCEDURE P_TOTALIZA_AHORRO(cCodGrupoCia_in IN CHAR,
                              cCodLocal_in    IN CHAR,
                              cNumPedVta_in   IN CHAR);
  FUNCTION F_GET_DNI_TARJETA(cCodLocal_in      IN CHAR,
                             cNroDocumento_in  IN CHAR, 
                             cIsIndMonedero_in IN CHAR)
    RETURN VARCHAR2;
  
  /**  
  DEVUELVE LOS PRODUCTOS QUE SE PODRA REDIMIR O APLICARA LA REDENCION.
  ----------------------------------------------------------------------------------------------
  ACCION               FECHA      USUARIO    DETALLE
  ----------------------------------------------------------------------------------------------
  CREACION             01.10.2015 KMONCADA   CREACION
  */
  FUNCTION F_CUR_PRODUCTO_APLICA_REDIME(cCodGrupoCia_in IN CHAR,
                                        cCodLocal_in    IN CHAR,
                                        cNumPedVta_in   IN CHAR)
  RETURN FARMACURSOR;

  /**  
  VALIDA SI PEDIDO FUE REDIMIDO CON TODOS LOS PUNTOS PERMITIDOS SEGUN EL DETALLE DEL PEDIDO
  ----------------------------------------------------------------------------------------------
  ACCION           FECHA      USUARIO    DETALLE
  ----------------------------------------------------------------------------------------------
  CREACION         01.10.2015 KMONCADA   CREACION
  */
  FUNCTION F_VAR_VALIDA_TODO_REDIMIDO(cCodGrupoCia_in IN CHAR,
                                      cCodLocal_in    IN CHAR,
                                      cNumPedVta_in   IN CHAR,
                                      cMontoRedimido  IN NUMBER)
    RETURN VARCHAR2;

  /**  
  REALIZARA VALIDACION PARA SOLICITAR DIGITAR DNI
  ----------------------------------------------------------------------------------------------
  ACCION           FECHA      USUARIO    DETALLE
  ----------------------------------------------------------------------------------------------
  CREACION         06.10.2015 KMONCADA   CREACION
  */    
  FUNCTION F_VAR_SOLICITA_DIGITA_DNI
    RETURN VARCHAR2;
 
  --****************************************************************
  --** OBJETIVO             : COMPRUEBA SI LA TARJETA ESTA REGISTRADA LOCALMENTE
  --** NOMBRE               : F_COMPRUEBA_TARJETA
  --** AUTOR                : DESARROLLO5 - ALEJANDRO NUÑEZ
  --** FECHA CREACION       : 21/10/2015
  --****************************************************************
  FUNCTION F_COMPRUEBA_TARJETA(pNumTarjeta VARCHAR2)
  RETURN INT;
END FARMA_PUNTOS;
/
CREATE OR REPLACE PACKAGE BODY FARMA_PUNTOS is
 ...
 /* *********************************************************************** */
  ...
  BEGIN
      ...
      -- KMONCADA 22.09.2015 SE VALIDAR QUE NRO DE TARJETA FORMATO EAN13
      IF nRetorno = 'S' AND TRIM(cNumTarj_in)=13 THEN
         nRetorno := PTOVENTA_CUPON.CUP_F_CHAR_VALIDA_EAN13(cNumTarj_in);
      END IF;
...
/*FUNCTION IMP_RECUPERA_PUNTOS(cCodGrupoCia_in 	IN CHAR,
  ...
  --[Desarrollo5] 16.10.2015 Es para el numero de tarjeta CMR o financiero Encriptado
  --Esta variable se usa para recoger el numero de tarjeta encriptada
  vTarjetaEncriptada VARCHAR2(30) := '';
  vTarjetaEncontrada INT := 0;
  ...
  IF vRpta IS NOT NULL AND LENGTH(vRpta) = 10 THEN
    --[Desarrollo5] 16.10.2015 Es para el numero de tarjeta CMR o financiero Encriptado
	--Si la tarjeta obtenida no esta esta encriptada entonces hara el flujo normal
    SELECT COUNT(A.COD_TARJETA)
      into vTarjetaEncontrada
      FROM FID_TARJETA_PEDIDO A
     WHERE NUM_PEDIDO = vRpta;
     
    IF vTarjetaEncontrada > 0 THEN
      SELECT A.COD_TARJETA
        into vTarjetaEncriptada
        FROM FID_TARJETA_PEDIDO A
       WHERE NUM_PEDIDO = vRpta;
    END IF;
 
    IF LENGTH(vTarjetaEncriptada) < 30 OR vTarjetaEncontrada = 0 THEN
 ...
 /* ********************************************************************** */  
  ...
  begin
    ...
    SET    C.EST_TRX_ORBIS = 'N',
           C.PT_ACUMULADO = 0,--[Desarrollo5] 21.10.2015
           C.PT_TOTAL = 0 ,--[Desarrollo5] 21.10.2015
           C.NUM_TARJ_PUNTOS = ''--[Desarrollo5] 21.10.2015
    WHERE  C.COD_GRUPO_CIA = cCodGrupoCia_in
    AND    C.COD_LOCAL = cCod_Local_in
    AND    C.NUM_PED_VTA = cNumPedido_in;
...
PROCEDURE P_DESCARTA_TRX_ANULA_ORBIS(cCodGrupoCia_in   IN CHAR,
  ...
  PROCEDURE F_I_CALCULA_PUNTOS(cCodGrupoCia_in   IN VTA_PEDIDO_VTA_DET.COD_GRUPO_CIA%TYPE,
    ...
    CURSOR curDetallePtos (cCodGrupoCia CHAR, cCodLocal CHAR, cNumPedVta CHAR, cOrdenCategoria NUMBER, cOrdenProducto NUMBER, cOrdenDefault NUMBER) IS
      SELECT DETALLE.*
      FROM (
            SELECT 
              PRODUCTOS.*,
              ...
              RANK() OVER(PARTITION BY PRODUCTOS.COD_PROD ORDER BY DECODE(PRODUCTOS.ORIGEN, 'CAT', cOrdenCategoria, 'PRO', cOrdenProducto, 'DEF', cOrdenDefault)) INDICADOR
            FROM (
                  ...
                  SELECT 'CAT' ORIGEN,
                         ...
                         ROUND((CASE 
                           WHEN TAB_AHORRO.MONEDA_AHORRO = 0 THEN 
                             0
                           ELSE 
                             ((NVL(DET.AHORRO_CAMP,0)/TAB_AHORRO.MONEDA_AHORRO)*TAB_AHORRO.PT_AHORRO) 
                         END),0) PTO_AHORRO,
                         -- KMONCADA 11.08.2015 CALCULO DE PUNTOS POR EL AHORRO DE PACK 
                         ROUND((CASE 
                           WHEN TAB_AHORRO.MONEDA_AHORRO = 0 THEN 
                             0
                           ELSE 
                             ((NVL(DET.AHORRO_PACK,0)/TAB_AHORRO.MONEDA_AHORRO)*TAB_AHORRO.PT_AHORRO) 
                         END),0) PTO_AHORRO_PACK,
                         CASE 
    ...
    vPrimerSecPermitida VTA_PEDIDO_VTA_DET.SEC_PED_VTA_DET%TYPE;
    vPrioridadCategoria INTEGER;
    vPrioridadProducto  INTEGER;
    vPrioridadDefault   INTEGER;
  BEGIN
    -- KMONCADA 01.10.2015 SE OBTENDRA PRIORIDAD DE BUSQUEDA DE LAS TABLAS DE ACUMULACION DE PTOS
    ...
    OPEN curDetallePtos(cCodGrupoCia_in, cCodLocal_in, cNumPedVta_in, vPrioridadCategoria, vPrioridadProducto, vPrioridadDefault);
      LOOP
        ...
        UPDATE VTA_PEDIDO_VTA_DET A
           ...
           -- KMONCADA 11.08.2015 GRABA EL PTO POR AHORRO PACK
               A.PTOS_AHORRO_PACK = FILA.PTO_AHORRO_PACK,
    ...
    IF isContinua = IND_SI THEN
      ...
      -- KMONCADA 01.10.2015 SE OBTENDRA LA SECUENCIA DEL PRIMER PRODUCTO QUE APLICO EL CALCULO DE PUNTOS
      BEGIN
        SELECT DET.SEC_PED_VTA_DET
        INTO vPrimerSecPermitida
        FROM VTA_PEDIDO_VTA_DET DET
        WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
        AND DET.COD_LOCAL = cCodLocal_in
        AND DET.NUM_PED_VTA = cNumPedVta_in
        AND NVL(DET.CTD_PUNTOS_ACUM,0)>0
        AND ROWNUM <2;
      EXCEPTION 
        WHEN OTHERS THEN
          DBMS_OUTPUT.PUT_LINE(SQLERRM);
          vPrimerSecPermitida := 1;
      END;
      ...
      UPDATE VTA_PEDIDO_VTA_DET A
       ...
       WHERE A.COD_GRUPO_CIA = cCodGrupoCia_in
         AND A.COD_LOCAL = cCodLocal_in
         AND A.NUM_PED_VTA = cNumPedVta_in
         AND A.SEC_PED_VTA_DET = vPrimerSecPermitida;
  ...
  FUNCTION F_CUR_V_LISTA_PRODUCTO(cCodGrupoCia_in   IN VTA_PEDIDO_VTA_DET.COD_GRUPO_CIA%TYPE,
    ...
    CURSOR curDetallePed IS
      SELECT DET.COD_PROD,
             ...
             B.VAL_MAX_FRAC,
             DET.VAL_FRAC_LOCAL,
             -- KMONCADA 11.08.2015 SE AGREGA EL AHORRO POR PACK PARA LOS PTOS EXTRAS
             SUM(ABS(NVL(DET.PTOS_AHORRO,0)) + 
                 ABS(NVL(DET.PTOS_AHORRO_PACK,0))
                ) PTOS_EXTRA
        FROM VTA_PEDIDO_VTA_DET DET, 
             LGT_PROD B
    ...
    OPEN curDetallePed;
      LOOP
        ...
        DBMS_OUTPUT.put_line('*********************'||CHR(13)||
                           ...
                           'PTOS_EXTRA -->'||filaCursor.PTOS_EXTRA);
          ...
          IF cIndAnulaPed_in = 'N' THEN
            ...
            */
            ...
            pCadena := pCadena || 
                       ...
                       TRIM(TO_CHAR(NVL(filaCursor.CTD_PUNTOS_ACUM, 0)+NVL(filaCursor.PTOS_EXTRA, 0), '99999990.00')) || ',' || -- PUNTOS ACUMULADO + PTOS EXTRA
                       TRIM(TO_CHAR(NVL(filaCursor.PTOS_EXTRA, 0), '99999990.00')); -- PUNTOS EXTRA
          ELSE
  ...
  FUNCTION F_LST_MONTOS_REDENCION(cCodGrupoCia_in IN VTA_PEDIDO_VTA_DET.COD_GRUPO_CIA%TYPE,
    ...
    vPrioridadProducto  INTEGER;
    vPrioridadDefault   INTEGER;
    vDocCliente         PBL_CLIENTE.DNI_CLI%TYPE;
    vBloqueoRedime      CHAR(1);
  BEGIN
    ...
    BEGIN
      SELECT FTP.DNI_CLI
      INTO vDocCliente
      FROM FID_TARJETA_PEDIDO FTP
      WHERE FTP.COD_GRUPO_CIA = cCodGrupoCia_in
      AND FTP.COD_LOCAL = cCodLocal_in
      AND FTP.NUM_PEDIDO = cNumPedVta_in;
      
      vBloqueoRedime := F_CLIENTE_BLOQUEADO_REDIME(vDocCliente);
    EXCEPTION 
      WHEN OTHERS THEN
        vBloqueoRedime := 'N';
    END;
    
    IF vBloqueoRedime = 'S' THEN
      OPEN vCursor FOR
      SELECT '0.00' PTOS_MAXIMO,
             '0.00' MONTO_MAXIMO
      FROM DUAL;
    ELSE
              ...
              FROM (SELECT F_CHAR_REDONDEO_CALCULO_PTOS INDICA FROM DUAL) A,
                   (SELECT CASE 
                            ...
                            CASE
                              WHEN VTA_MANUAL = 'S' THEN
                                0
                              WHEN vNuevoAlgoritmo = '0' AND NVL(DETALLE.PT_REDIME, 0) > 0 THEN
...
--      vMontoNeto := TO_NUMBER(montoTotalPedido,'999999.99');
  ...
  BEGIN
     --select SUM(NVL(C.PTOS_AHORRO,0))
     select SUM(NVL(C.PTOS_AHORRO,0) + NVL(C.PTOS_AHORRO_PACK,0))
     into   vSumnotnull
     from   vta_pedido_vta_DET C
     where  C.cod_grupo_cia =  cCodGrupoCia_in
     and    C.cod_local = cCodLocal_in
  ...
  RETURN FARMACURSOR IS  
    ...
    CURSOR curCabecera IS
     SELECT REPLACE((REPLACE((EXTRACTVALUE(xt.column_value, 'e')),'Ã','&')),'Ë','<') VAL
      ...
      SELECT 
        'MONEDERO MIFARMA'||'@'||
        ' '||'@'||
        CLI.NOM_CLI || ' ' || CLI.APE_PAT_CLI || ' ' || CLI.APE_MAT_CLI ||'@'||
        'Tarjeta : ' || (SELECT '****' || SUBSTR((SUBSTR(CAB.NUM_TARJ_PUNTOS,1,4) || 
              ...
              FROM dual) ||'@'|| 
        'DNI/CE : ' || NVL(cDocTarjetaPtos_in, CLI.DNI_CLI)
      FROM VTA_PEDIDO_VTA_CAB CAB,
           PBL_CLIENTE CLI
      WHERE CAB.COD_GRUPO_CIA = cCodGrupoCia_in
      AND CAB.COD_LOCAL = cCodLocal_in
    ...
    vIdDoc_in VARCHAR2(100);
    vIpPc_in  VARCHAR2(100);
  BEGIN
    vIdDoc_in := FARMA_PRINTER.F_GENERA_ID_DOC;
    vIpPc_in := FARMA_PRINTER.F_GET_IP_SESS;  
    
    -- CALCULO DE UN MONTON DE CONCEPTOS DE PUNTOS
    IF cSecCompPago_in = 'N' THEN
    ...
    FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in,
                                       vIpPc_in => vIpPc_in,
                                       vValor_in => 'EN LA COMPRA USTED AHORRO S/. '|| PTOVENTA_FIDELIZACION.FID_F_GET_NUM_FORMATED_02(ptosAhorro),
                                       vTamanio_in => FARMA_PRINTER.TAMANIO_1,
                                       vAlineado_in => FARMA_PRINTER.ALING_CEN);
    ...
    IF ptosAhorro > 0.5 THEN
      FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in,
                                        vIpPc_in => vIpPc_in,
                                        vValor_in => RPAD('AHORRASTE', 30,' ')||'S/.' || LPAD(PTOVENTA_FIDELIZACION.FID_F_GET_NUM_FORMATED_02(ptosAhorro),9,' '),
    ...
    IF vPtoRedimido > 0 THEN
      FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in,
                                                vIpPc_in => vIpPc_in,
                                                vValor_in => RPAD('HAS CANJEADO (PTOS DINERO)',30,' ')||LPAD(PTOVENTA_FIDELIZACION.FID_F_GET_NUM_FORMATED(vPtoRedimido),12,' '),
                                                vTamanio_in => FARMA_PRINTER.TAMANIO_0,
                                                vAlineado_in => FARMA_PRINTER.ALING_IZQ);
    END IF;
    ...
    IF cCantidadCupon < 10 THEN
       textoCantCupon := RPAD('HAS GANADO (CUPON)',30,' ') || LPAD(('0'|| cCantidadCupon),12,' ');
    ELSE
       textoCantCupon := RPAD('HAS GANADO (CUPONES)',30,' ') || LPAD(cCantidadCupon,12,' ');
    END IF;
    
    IF cCantidadCupon > 0 THEN
      FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in,
                                  vIpPc_in => vIpPc_in,
                                  vValor_in => textoCantCupon,
                                  vTamanio_in => FARMA_PRINTER.TAMANIO_0,
                                  vAlineado_in => FARMA_PRINTER.ALING_IZQ);
    END IF;
    ...
    IF vPtoAcumulado > 0 THEN
      FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in,
                                                vIpPc_in => vIpPc_in,
                                                vValor_in => RPAD('HAS GANADO (PTOS DINERO)',30,' ')||LPAD(PTOVENTA_FIDELIZACION.FID_F_GET_NUM_FORMATED(vPtoAcumulado),12,' '),
                                                vTamanio_in => FARMA_PRINTER.TAMANIO_0,
                                                vAlineado_in => FARMA_PRINTER.ALING_IZQ);
    ...
    IF ABS(NVL(vPtoTotal,0)) > 0 THEN
      IF NVL(vPtoTotal,0) < 0 THEN
        FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in,
                                             vIpPc_in => vIpPc_in,
                                             vValor_in => RPAD('ACUMULADOS (PTOS DINERO)',30,' ')||LPAD(PTOVENTA_FIDELIZACION.FID_F_GET_NUM_FORMATED(vPtoTotal)||'*',11,' '),
                                             vTamanio_in => FARMA_PRINTER.TAMANIO_0,
                                             vAlineado_in => FARMA_PRINTER.ALING_IZQ);
      ELSE
        FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in,
                                             vIpPc_in => vIpPc_in,
                                             vValor_in => RPAD('ACUMULADOS (PTOS DINERO)',30,' ')||LPAD(PTOVENTA_FIDELIZACION.FID_F_GET_NUM_FORMATED(vPtoTotal),12,' '),
                                             vTamanio_in => FARMA_PRINTER.TAMANIO_0,
                                             vAlineado_in => FARMA_PRINTER.ALING_IZQ);
    ...
    valorImpr := PTOVENTA_FIDELIZACION.FID_F_GET_NUM_FORMATED_02(nvl(vValorAhorro,0));
    
    IF valorImpr <> '0.00' THEN
      FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in,
                                        vIpPc_in => vIpPc_in,
                                        vValor_in => RPAD(UPPER(FARMA_PUNTOS.F_CHAR_GET_MSJ_TIEMPO_AHORRO|| ' AHORRO'),30,' ')||'S/.' || LPAD(valorImpr,9,' '),
                                        vTamanio_in => FARMA_PRINTER.TAMANIO_0,
                                        vAlineado_in => FARMA_PRINTER.ALING_IZQ,
                                        vNegrita_in => FARMA_PRINTER.BOLD_ACT);
    ...
    FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in,
                                      vIpPc_in => vIpPc_in,
                                      vValor_in => 'Sigue usando tu Monedero del Ahorro',
                                      vTamanio_in => FARMA_PRINTER.TAMANIO_1,
                                      vAlineado_in => FARMA_PRINTER.ALING_IZQ,
                                      vNegrita_in => FARMA_PRINTER.BOLD_ACT, 
                                      vJustifica_in => 'N');
    FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in,
                                      ...
                                      vNegrita_in => FARMA_PRINTER.BOLD_ACT, 
                                      vJustifica_in => 'N');
    curTicket := FARMA_PRINTER.F_CUR_OBTIENE_DOC_IMPRIMIR(vIdDoc_in => vIdDoc_in, vIpPc_in => vIpPc_in);
    RETURN curTicket;                                      
  END;
  ...
  PROCEDURE P_CTRL_ENTREGA_TARJ_PTOS_NUEVA(cCodGrupoCia_in    IN CHAR,
                                          ...
                                          cUsuCrea_in        IN VARCHAR2)
  IS
    vCodMotivoKardex LGT_KARDEX.COD_MOT_KARDEX%TYPE;
    vIsTarjPuntos INTEGER := 0;
    vSecKardex LGT_KARDEX.SEC_KARDEX%TYPE;
    vNumDocumento FID_TARJETA.DNI_CLI%TYPE;
    vStockActual LGT_PROD_LOCAL.STK_FISICO%TYPE;
    vValFrac LGT_PROD_LOCAL.VAL_FRAC_LOCAL%TYPE;
    vCodMotKardex LGT_KARDEX.COD_MOT_KARDEX%TYPE;
  BEGIN
    SELECT COUNT(1) 
    INTO vIsTarjPuntos
    FROM VTA_RANGO_TARJETA A 
    WHERE A.COD_RANGO_TARJETA = 2
    AND cNumTarjeta_in BETWEEN DESDE AND HASTA;
    
    vNumDocumento := cNumDocumento_in;
    IF LENGTH(NVL(TRIM(vNumDocumento),'')) > 10 THEN
      vNumDocumento := SUBSTR(TRIM(vNumDocumento),1,10);
    END IF;
    
    IF vIsTarjPuntos > 0 THEN
      ...
      AND PROD.COD_PROD = COD_PROD_TARJ_PUNTOS;
      
      SELECT A.LLAVE_TAB_GRAL
      INTO vCodMotKardex 
      FROM PBL_TAB_GRAL A
      WHERE A.ID_TAB_GRAL = TAB_IND_COD_MOT_KARDEX_TARJ;
      
      PTOVENTA_INV.INV_GRABAR_KARDEX(cCodGrupoCia_in => cCodGrupoCia_in,
                                     cCodLocal_in => cCodLocal_in,
                                     cCodProd_in => COD_PROD_TARJ_PUNTOS,
                                     cCodMotKardex_in => vCodMotKardex,
                                     cTipDocKardex_in => '01',
                                     cNumTipDoc_in => vNumDocumento,
                                     nStkAnteriorProd_in => vStockActual,
                                     nCantMovProd_in => -1,
                                     nValFrac_in => vValFrac,
                                     cDescUnidVta_in => 'UNIDAD',
                                     cUsuCreaKardex_in => cUsuCrea_in,
                                     cCodNumera_in => '016',
                                     cGlosa_in => 'TARJETA MONEDERO DEL AHORRO',
                                     cTipDoc_in => 'CP',
                                     cNumDoc_in => cNumTarjeta_in);
      UPDATE LGT_PROD_LOCAL
      SET USU_MOD_PROD_LOCAL = cUsuCrea_in,
              FEC_MOD_PROD_LOCAL = SYSDATE,
              STK_FISICO = STK_FISICO - 1
      WHERE  COD_GRUPO_CIA = cCodGrupoCia_in
      AND	   COD_LOCAL = cCodLocal_in
      AND	   COD_PROD = COD_PROD_TARJ_PUNTOS;
    END IF;
  END;
  ...
  BEGIN
    ...
    INTO vMensaje
    FROM PBL_TAB_GRAL A
    WHERE A.ID_TAB_GRAL = TAB_IND_MSJ_BLOQUEO_REDIME;
    RETURN vMensaje;
  EXCEPTION
    WHEN OTHERS THEN
      RETURN 'TARJETA BLOQUEADA PARA REDIMIR PUNTOS';
  END;
  ...
  BEGIN
    SELECT COUNT(1)
    INTO vExiste
    FROM DNI_BLOQ_PTOS A
    WHERE A.DNI_CLI = cNumDocumento_in
    AND A.BLOQ_REDIME = 'S'
    AND A.ESTADO = 'A';
  ...
  IS
  BEGIN
    IF 'N' = FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in => cCodGrupoCia_in,
                                           cCodLocal_in => cCodLocal_in) THEN
      UPDATE VTA_PEDIDO_VTA_DET DET
      SET DET.AHORRO = NVL(DET.AHORRO_CAMP,0)+NVL(DET.AHORRO_PACK,0)
      WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
      AND DET.COD_LOCAL = cCodLocal_in
      AND DET.NUM_PED_VTA = cNumPedVta_in;
    ELSE
      UPDATE TMP_VTA_PEDIDO_VTA_DET DET
      SET DET.AHORRO = NVL(DET.AHORRO_CAMP,0)+NVL(DET.AHORRO_PACK,0)
      WHERE DET.COD_GRUPO_CIA = cCodGrupoCia_in
      AND DET.COD_LOCAL = cCodLocal_in
      AND DET.NUM_PED_VTA = cNumPedVta_in;
  ...
  FUNCTION F_GET_DNI_TARJETA(cCodLocal_in      IN CHAR,
                             ...
                             cIsIndMonedero_in IN CHAR)
    RETURN VARCHAR2 IS
    vNroTarjeta FID_TARJETA.COD_TARJETA%TYPE;
  BEGIN
    BEGIN
      SELECT X.COD_TARJETA 
      INTO vNroTarjeta
      FROM (
        SELECT A.COD_TARJETA,
               NVL(A.FEC_MOD_TARJETA,A.FEC_CREA_TARJETA) FECHA 
        FROM FID_TARJETA A 
        WHERE A.DNI_CLI = cNroDocumento_in
        AND NVL(A.IND_PUNTOS,'N') = cIsIndMonedero_in
        AND A.COD_TARJETA LIKE CASE WHEN cIsIndMonedero_in = 'N' THEN '999'||cCodLocal_in||'%'
                                    WHEN cIsIndMonedero_in = 'S' THEN '%'
                                END
        ORDER BY NVL(A.FEC_MOD_TARJETA,A.FEC_CREA_TARJETA) DESC
      ) X 
      WHERE ROWNUM=1;
    EXCEPTION 
      WHEN OTHERS THEN
  ...
  BEGIN
    ...
    --BEGIN
      OPEN cursorProducto FOR
      SELECT SEC_PED_VTA_DET, COD_PROD 
      FROM (
        SELECT DETALLE.SEC_PED_VTA_DET, DETALLE.COD_PROD, CASE 
                               WHEN VTA_MANUAL = 'S' THEN
                                 0
                               WHEN vNuevoAlgoritmo = '0' THEN
                                 (NVL(DETALLE.VAL_PREC_TOTAL,0) * DETALLE.PT_REDIME) / NVL(DETALLE.MONEDA_REDIME,1)
       ...
       WHERE B.PTOS_MAXIMO>0

       ;
     RETURN cursorProducto;
  END;
  
  FUNCTION F_VAR_PRODUCTO_APLICA_ACUMULA(cCodGrupoCia_in IN CHAR,
                                        cCodLocal_in    IN CHAR,
                                        cCodProd_in     IN CHAR)
  RETURN VARCHAR2 IS
  BEGIN
  ...
  FUNCTION F_VAR_VALIDA_TODO_REDIMIDO(cCodGrupoCia_in IN CHAR,
                                      ...
                                      cMontoRedimido  IN NUMBER)
    RETURN VARCHAR2 IS
    curRedencion FARMACURSOR;
    vMontoRedimido NUMBER(12,3);
    vPuntos VARCHAR2(100);
    vMontoPto VARCHAR2(100);
    vMontoTotalPedido VTA_PEDIDO_VTA_CAB.VAL_NETO_PED_VTA%TYPE;
  BEGIN
    curRedencion := FARMA_PUNTOS.F_LST_MONTOS_REDENCION(cCodGrupoCia_in,cCodLocal_in,cNumPedVta_in,'S');
    SELECT CAB.VAL_NETO_PED_VTA
    INTO vMontoTotalPedido
    FROM VTA_PEDIDO_VTA_CAB CAB
    WHERE CAB.COD_GRUPO_CIA = cCodGrupoCia_in
    AND CAB.COD_LOCAL = cCodLocal_in
    AND CAB.NUM_PED_VTA = cNumPedVta_in;
    ...
    LOOP
      FETCH curRedencion INTO vPuntos, vMontoPto;
      EXIT WHEN curRedencion%NOTFOUND;
        vMontoRedimido := TO_NUMBER(vMontoPto,'9999999990.000');
    END LOOP;
    CLOSE curRedencion;
    IF cMontoRedimido = vMontoRedimido AND vMontoTotalPedido = cMontoRedimido THEN
      RETURN 'S';
    ELSE
      RETURN 'N';
    END IF;
  ...
  BEGIN
    BEGIN 
      ...
      INTO vResultado
      FROM PBL_TAB_GRAL A
      WHERE A.ID_TAB_GRAL = TAB_IND_SOLICITA_DIGITA_DNI;
    EXCEPTION 
      WHEN OTHERS THEN
        vResultado := 'N';
    END;
  ...
  FUNCTION F_COMPRUEBA_TARJETA(pNumTarjeta VARCHAR2)
  RETURN INT IS
  vValorEncontrado NUMBER;
  BEGIN
     SELECT COUNT(A.DNI_CLI)
     INTO vValorEncontrado
     FROM FID_TARJETA A
     WHERE A.COD_TARJETA = pNumTarjeta;
     
     IF vValorEncontrado > 0 THEN
         RETURN 1;
     ELSE
         RETURN 0;
     END IF;
