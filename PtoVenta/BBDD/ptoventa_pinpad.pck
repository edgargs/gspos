CREATE OR REPLACE PACKAGE PTOVENTA."PTOVENTA_PINPAD" is
  ...
  TYPE FarmaCursor IS REF CURSOR;
	...
	TIP_COMPRA CHAR(3) := 'COM';
	TIP_ANULACION CHAR(3) := 'ANU';
  ...
  FUNCTION CONS_INF_TRANS_PINPAD(cCodGrupoCia_in          IN CHAR,
								  ...
								  cTipoRegistro IN CHAR DEFAULT TIP_COMPRA)
  RETURN CHAR;
...
FUNCTION GRAB_TRAMA_PINPAD_MC(cCodGrupoCia_in          IN CHAR,
  ...
  FUNCTION LIST_IMP_VOUCHER_TRANSAC(cCodGrupoCia_in IN CHAR,
                               cCod_Local_in   IN CHAR,
                               cOperador_in    IN CHAR,
                               cNum_Pedido_in  IN CHAR,
                               cTip_copia_in   IN CHAR)
  ...
  PROCEDURE OBTENER_CABECERA_VOUCHERS_2(cCodGrupoCia_in       IN CHAR,
                                        cCod_Local_in         IN CHAR,
									                      nTipoOperador_in      IN NUMBER,
  ...
  FUNCTION LST_OBTENER_CAB_VOUCHERS_2(cCodGrupoCia_in       IN CHAR,
                                     cCod_Local_in         IN CHAR,
									                   nTipoOperador_in      IN NUMBER)
  RETURN FARMACURSOR;
...
--       IF(CFP='00083') THEN
             ...
             where PPV.NUM_PED_VTA = cNumPedido_in and
                   PPV.TIP_REGISTRO = cTipoRegistro and
       ...
       --ELSIF (CFP='00084' or CFP='00087' or CFP='00088') THEN
             ...
             where PPM.NUM_PED_VTA=cNumPedido_in and
				   PPM.TIP_REGISTRO = cTipoRegistro and
                   FPP.COD_FORMA_PAGO = CFP;
--       ELSE
           EXCEPTION
             WHEN NO_DATA_FOUND THEN
  ...
  BEGIN
	...
	--ERIOS 12.05.2015 Considera como anulacion, las NCR generadas.
       IF (cTipTarjeta_in = 'M') THEN
            select trim(to_char((TO_NUMBER(MC.DSC_AMOUNT)/100),'9990.00')) ||'Ã'||
                   (select EST_PED_VTA||decode(ind_pedido_anul,'N','',ind_pedido_anul)
                    ...
                    where NUM_PED_VTA = MC.NUM_PED_VTA) ||''||
                    (select decode(EST_PED_VTA,'C','S','')
                      from VTA_PEDIDO_VTA_CAB
                      where COD_GRUPO_CIA = cCodGrupoCia_in
                      AND COD_LOCAL = cCodLocal_in
                      AND NUM_PED_VTA_ORIGEN = MC.NUM_PED_VTA) ||'Ã'||
					MC.NUM_PED_VTA ||'Ã'||
					MC.COD_FORMA_PAGO
					into MONTO
       ...
       ELSIF (cTipTarjeta_in = 'V') THEN
            select ' ' ||'Ã'||
                   (select EST_PED_VTA||decode(ind_pedido_anul,'N','',ind_pedido_anul)
                    from VTA_PEDIDO_VTA_CAB
                    where NUM_PED_VTA = VI.NUM_PED_VTA) ||''||
                    (select decode(EST_PED_VTA,'C','S','')
                      from VTA_PEDIDO_VTA_CAB
                      where COD_GRUPO_CIA = cCodGrupoCia_in
                      AND COD_LOCAL = cCodLocal_in
                      AND NUM_PED_VTA_ORIGEN = VI.NUM_PED_VTA) ||'Ã'||
					VI.NUM_PED_VTA ||'Ã'||
					VI.COD_FORMA_PAGO
					into MONTO
  ...
  BEGIN
          ...
          IF vCodMarca <> COD_CIA_MARKET_01 THEN
                     vTextoMarca := 'BOTICAS ';
          END IF;
          --FIN ASOSA - 03/09/2014
...
<br/>
<div{ESP}align="center"{ESP}class="tipoLetraA">' || vTextoMarca || ' {MARCA}</div>
...
<hr/>';
  ...
  BEGIN
       ...
       RESULTADO := CONS_INF_TRANS_PINPAD(cCodGrupoCia_in,cCod_Cia_in,cCod_Local_in,cNumPedido_in);
  ...
  RETURN FarmaCursor
  IS
    cursorImp FarmaCursor;
    cursorCab FarmaCursor;
    impr1 VARCHAR(1000);
    impr2 CLOB;
    ...
    CURSOR curMensaje(TEXTO VARCHAR2) IS
  ...
  BEGIN
    ...
    IF (cOperador_in ='M') THEN
      select DSC_PRINT_DATA
      INTO msjVoucher
      from VTA_PROCESO_PINPAD_MC
      where NUM_PED_VTA = cNum_Pedido_in;
      
      SELECT REPLACE (msjVoucher, CHR(10), '@') 
      INTO msjVoucher 
      FROM dual;
              
      OPEN curMensaje(msjVoucher);
        LOOP
         ...
         END LOOP;
      CLOSE curMensaje;
          /*INSERT INTO TMP_VOUCHER_TARJETA
              SELECT REPLACE((REPLACE((EXTRACTVALUE(xt.column_value, 'e')),'Ã','&')),'Ë','<') VAL ,'0','I','N','S'
              FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' ||
                                 REPLACE( REPLACE((REPLACE((
                msjVoucher
              ),'&','Ã')),'<','Ë'),'@','</e><e>') ||'</e></coll>'),'/coll/e'))) xt;  */
             ...
             /* OPEN cursorImp FOR
                ...
                FROM TMP_VOUCHER_TARJETA A; */
          cursorImp := FARMA_PRINTER.F_CUR_OBTIENE_DOC_IMPRIMIR(vIdDoc_in => vIdDoc, vIpPc_in => vIpPc);
          return cursorImp;
    ELSIF (cOperador_in ='V') THEN
      --si el registro existe devolver el texto en HTML para la impresión
      IF (cTip_copia_in = 'C') THEN
        --se consulta el mensaje del cuerpo del voucher para la impresión
        select DSC_MSJ_FIN_OPE_TRAMA
        ...
        --se arma la cabecera del voucher para la impresión
        OBTENER_CABECERA_VOUCHERS_2(cCodGrupoCia_in,cCod_Local_in,OPERADOR_VISA, vIdDoc_in => vIdDoc, vIpPc_in => vIpPc);
...
/*              OPEN cursorImp FOR
      ...
      ELSE
              ...
              --reemplazamos saltos de linea por <br/>
              select REPLACE (msjVoucher, CHR(10), '@') into msjVoucher from dual;
  ...
  BEGIN
   ...
   SELECT MG.NOM_MARCA,
     ...
     INTO nomMarca, 
          ...
          vDirecLocal
     FROM PBL_LOCAL PL
     JOIN PBL_MARCA_CIA MC
       ON (PL.COD_GRUPO_CIA = MC.COD_GRUPO_CIA AND
          PL.COD_MARCA = MC.COD_MARCA AND PL.COD_CIA = MC.COD_CIA)
     JOIN PBL_MARCA_GRUPO_CIA MG
       ON (MG.COD_GRUPO_CIA = MC.COD_GRUPO_CIA AND
          MG.COD_MARCA = MC.COD_MARCA)
     JOIN PBL_CIA PC
       ON (PC.COD_CIA = PL.COD_CIA)
    WHERE PL.COD_GRUPO_CIA = cCodGrupoCia_in
      AND PL.COD_LOCAL = cCod_Local_in;

    --se verifica longitud para que no se descuadre el ticket
    select LENGTH(direccion) 
    into long_direccion 
    from dual;

    IF (long_direccion>50) THEN
       SELECT SUBSTR(direccion,0,50)||'@'||SUBSTR(direccion,51,long_direccion)
       INTO direccion
       FROM dual;
    END IF;

    info_local := cCod_Local_in || ' ' || vDirecLocal;
    
    SELECT LENGTH(info_local) 
    INTO long_info_local 
    FROM dual;

    IF (long_info_local>42) THEN
       SELECT SUBSTR(info_local,0,42)||'@'||SUBSTR(info_local,43,long_info_local)
       INTO info_local
       FROM dual;
    END IF;

    --DELETE FROM TMP_VOUCHER_TARJETA;
    -- CABECERA
    SELECT DECODE(nTipoOperador_in,OPERADOR_VISA,'VISANET', OPERADOR_MC,'PROCESOS DE MEDIOS DE PAGO S.A.')
    INTO vValor
    FROM DUAL; 
    ...
    FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in, 
                                 vIpPc_in => vIpPc_in, 
                                 vValor_in => vTextoMarca||' '||nomMarca, 
                                 vTamanio_in => FARMA_PRINTER.TAMANIO_1, 
                                 vAlineado_in => FARMA_PRINTER.ALING_CEN, 
                                 vInterlineado_in => FARMA_PRINTER.LINEADO_DOBLE);
    /*INSERT INTO TMP_VOUCHER_TARJETA
    SELECT vTextoMarca||' '||nomMarca,'0','C','N','S'
    FROM DUAL;*/
    
    FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in, 
                                 vIpPc_in => vIpPc_in, 
                                 vValor_in => razonSocial||' RUC: '||numRuc, 
                                 vTamanio_in => FARMA_PRINTER.TAMANIO_1, 
                                 vAlineado_in => FARMA_PRINTER.ALING_IZQ, 
                                 vInterlineado_in => FARMA_PRINTER.LINEADO_DOBLE);
    /*INSERT INTO TMP_VOUCHER_TARJETA
    SELECT razonSocial||' RUC: '||numRuc,'0','I','N','S'
    FROM DUAL;*/
    OPEN curMensaje('Dom Fiscal: '||direccion);
      LOOP
        FETCH curMensaje INTO filaCurMensaje;
        EXIT WHEN curMensaje%NOTFOUND;
    ...
    FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in, 
                                 vIpPc_in => vIpPc_in, 
                                 vValor_in => 'Telf:'||vTelefCia, 
                                 vTamanio_in => FARMA_PRINTER.TAMANIO_1, 
                                 vAlineado_in => FARMA_PRINTER.ALING_IZQ, 
                                 vInterlineado_in => FARMA_PRINTER.LINEADO_DOBLE);
/*    INSERT INTO TMP_VOUCHER_TARJETA
    SELECT 'Telf:'||vTelefCia,'0','I','N','N'
    FROM DUAL;*/
    
    OPEN curMensaje('T'||info_local);
      LOOP
        FETCH curMensaje INTO filaCurMensaje;
        EXIT WHEN curMensaje%NOTFOUND;
...
--       RETURN NULL;
  ...
  BEGIN
    vIdDoc := FARMA_PRINTER.F_GENERA_ID_DOC;
    vIpPc := FARMA_PRINTER.F_GET_IP_SESS;
    OBTENER_CABECERA_VOUCHERS_2(cCodGrupoCia_in       ,
                                     cCod_Local_in         ,
									                   nTipoOperador_in      ,
                                     vIdDoc,
                                     vIpPc);
