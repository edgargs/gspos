...
public class UtilityImpCompElectronico {
    ...
    public boolean printDocumento(String pNumPedidoVta, String pSecCompPago, boolean isReimpresion,
                                  boolean isCobro) throws Exception {
        ...
        rutaFile =
                rutaFile + UtilityCaja.getNombreElectronico(FarmaVariables.vCodGrupoCia, FarmaVariables.vCodLocal, pNumPedidoVta,
                                                            pSecCompPago);

        List listComanda = new ArrayList();
        listComanda =
                DBComprobanteElectronico.getDatosImpresion(VariablesPtoVenta.vVersion, pNumPedidoVta, pSecCompPago,
                                                           isReimpresion);
        if (listComanda.size() > 0) {
            boolean rest =
                impresionDocumentoEnTermica(listComanda, VariablesPtoVenta.vTipoImpTermicaxIp, VariablesPtoVenta.vImpresoraActual.getName(),
                                            true, rutaFile, isCobro);
            if (!rest) {
    ...
    public boolean impresionDocumentoEnTermica(List listComanda, String tipoImpresora, String ruta,
                                               boolean isPonerLogo, String pRutaFile,
                                               boolean isCobro) throws Exception {
        String impresora = "";
        ...
        FarmaPrinterFacade printer =
            new FarmaPrinterFacade(tipoImpresora, ruta); //manda imprimir segun el modelo de impresora
