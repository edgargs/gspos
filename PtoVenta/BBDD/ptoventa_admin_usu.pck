...
--Descripcion: Obtiene los datos de un trabajador del maestro de trabajadores
  ...
  --23.06.2016  KMONCADA  SE CAMBIA TIPO DE DATO DE LA VARIABLE CODTRAB
FUNCTION USU_OBTIENE_DATA_TRABAJADOR(cCodCia_in  IN CHAR,
		 					                       cCodTrab_in IN CE_MAE_TRAB.COD_TRAB_RRHH%TYPE)
  RETURN FarmaCursor;
 /*************************************************************/
...
PROCEDURE USU_INGRESA_USUARIO(cCodGrupoCia_in IN CHAR,
  ...
  --FECHA       USUARIO  COMENTARIO
  --27/11/2007  DUBILLUZ  CREACION
  --23.06.2016  KMONCADA SE CAMBIO TIPO DE DATO DE LA VARIABLE CODTRAB
  FUNCTION USU_EXISTE_USUARIO(cCodGrupoCia_in   IN CHAR,
                              cCodTrabRR_HH_in  IN CE_MAE_TRAB.COD_TRAB_RRHH%TYPE)
  RETURN CHAR;
 ...
 END;
  ...
  BEGIN
    ...
    WHERE  /*COD_CIA  = cCodCia_in
           ...
           AND*/ C.COD_TRAB_RRHH = cCodTrab_in
    ;
...
-- 01.12.2015 KMONCADA  
  FUNCTION F_REGISTRAR_HUELLA(pCodGrupoCia_in    IN PBL_USU_LOCAL.COD_GRUPO_CIA%TYPE,
    ...
    vLogin   PTOVENTA.PBL_USU_LOCAL.LOGIN_USU%TYPE;
    vDni     PTOVENTA.PBL_USU_LOCAL.DNI_USU%TYPE;
    vCant    NUMBER(5) := 0;
    v_vDescLocal varchar2(200);
    vCorreoCopia VTA_ZONA_VTA.EMAIL_JEFE_ZONA%TYPE;
    vMensajeFinal varchar2(5000);
    cFechaMensaje  varchar (150);
  BEGIN
    BEGIN
      SELECT LOC.COD_CIA, LOC.COD_TRAB, loc.login_usu, loc.dni_usu
      INTO vCodCia, vCodTrab, vLogin, vDni
    ...
    IF pPosicionHuella_in = 0 THEN
        ...
        INTO vCant
        FROM CE_MAE_TRAB A
        WHERE length(A.HUELLA_MENIQUE_IZQ) > 0
        AND A.COD_CIA = vCodCia 
        AND A.COD_TRAB = vCodTrab
        AND A.EST_TRAB = ESTADO_ACTIVO;
    ...
    ELSIF pPosicionHuella_in = 1 THEN
        ...
        INTO vCant
        FROM CE_MAE_TRAB A
        WHERE length(A.HUELLA_ANULAR_IZQ) > 0
        AND A.COD_CIA = vCodCia 
        AND A.COD_TRAB = vCodTrab
        AND A.EST_TRAB = ESTADO_ACTIVO;
    ...
    ELSIF pPosicionHuella_in = 2 THEN
        ...
        INTO vCant
        FROM CE_MAE_TRAB A
        WHERE length(A.HUELLA_MEDIO_IZQ) > 0
        AND A.COD_CIA = vCodCia 
        AND A.COD_TRAB = vCodTrab
        AND A.EST_TRAB = ESTADO_ACTIVO;
    ...
    ELSIF pPosicionHuella_in = 3 THEN
        ...
        INTO vCant
        FROM CE_MAE_TRAB A
        WHERE length(A.HUELLA_INDICE_IZQ) > 0
        AND A.COD_CIA = vCodCia 
        AND A.COD_TRAB = vCodTrab
        AND A.EST_TRAB = ESTADO_ACTIVO;
    ...
    ELSIF pPosicionHuella_in = 4 THEN
        ...
        INTO vCant
        FROM CE_MAE_TRAB A
        WHERE length(A.HUELLA_PULGAR_IZQ) > 0
        AND A.COD_CIA = vCodCia 
        AND A.COD_TRAB = vCodTrab
        AND A.EST_TRAB = ESTADO_ACTIVO;
    ...
    ELSIF pPosicionHuella_in = 5 THEN
        ...
        INTO vCant
        FROM CE_MAE_TRAB A
        WHERE length(A.HUELLA_PULGAR_DER) > 0
        AND A.COD_CIA = vCodCia 
        AND A.COD_TRAB = vCodTrab
        AND A.EST_TRAB = ESTADO_ACTIVO;
    ...
    ELSIF pPosicionHuella_in = 6 THEN
        ...
        INTO vCant
        FROM CE_MAE_TRAB A
        WHERE length(A.HUELLA_INDICE_DER) > 0
        AND A.COD_CIA = vCodCia 
        AND A.COD_TRAB = vCodTrab
        AND A.EST_TRAB = ESTADO_ACTIVO;
    ...
    ELSIF pPosicionHuella_in = 7 THEN
        ...
        INTO vCant
        FROM CE_MAE_TRAB A
        WHERE length(A.HUELLA_MEDIO_DER) > 0
        AND A.COD_CIA = vCodCia 
        AND A.COD_TRAB = vCodTrab
        AND A.EST_TRAB = ESTADO_ACTIVO;
    ...
    ELSIF pPosicionHuella_in = 8 THEN
        ...
        INTO vCant
        FROM CE_MAE_TRAB A
        WHERE length(A.HUELLA_ANULAR_DER) > 0
        AND A.COD_CIA = vCodCia 
        AND A.COD_TRAB = vCodTrab
        AND A.EST_TRAB = ESTADO_ACTIVO;
    ...
    ELSIF pPosicionHuella_in = 9 THEN
        ...
        INTO vCant
        FROM CE_MAE_TRAB A
        WHERE length(A.HUELLA_MENIQUE_DER) > 0
        AND A.COD_CIA = vCodCia 
        AND A.COD_TRAB = vCodTrab
        AND A.EST_TRAB = ESTADO_ACTIVO;
    ...
    IF vCant > 0 THEN
        ...
        SELECT B.EMAIL_JEFE_ZONA
        INTO vCorreoCopia
        FROM vta_local_x_zona A,
             vta_zona_vta B
        WHERE A.COD_GRUPO_CIA = B.COD_GRUPO_CIA
        AND A.COD_ZONA_VTA = B.COD_ZONA_VTA
        AND A.COD_LOCAL = pCodLocal_in
        AND A.COD_GRUPO_CIA = pCodGrupoCia_in;

        vMensajeFinal :='<BR> <BR>' || 
                        '<TABLE  BORDER=0 align="left">' ||
                        '<TR><TD colspan=2 >El trabajador con:</TD></TR>' ||
                        '<TR><TD>DNI : </TD><TD>' || vDni || '</TD></TR>' ||
                        '<TR><TD>Usuario: </TD><TD>' || vLogin || '</TD></TR>' ||
                        '<TR><TD>Código Trabajador: </TD><TD>' || vCodTrab || '</TD></TR>' ||
                        '<TR><TD colspan=2 >ha modificado su huella digital con ayuda de operador</TD></TR>' ||
                        '</TABLE>';
        cFechaMensaje := TO_CHAR (TRUNC (SYSDATE), 'DD/MM/YYYY '); 
     
       ENVIA_CORREO_INF_MAIL_LOCAL(pCodGrupoCia_in,
                                pCodLocal_in,
                                'ALERTA MODIFICACIÓN HUELLA DIGITAL',
                                'HUELLA DIGITAL',
                                vMensajeFinal,
                                vCorreoCopia);
    END IF;
