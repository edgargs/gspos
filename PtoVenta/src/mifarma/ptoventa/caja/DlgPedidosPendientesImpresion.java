...
//**************************************************************************
  private void cargaListaPendientes()
  {
   	...
   	try {
			...
			if (tblListaPendientes.getRowCount() > 0)
				FarmaUtility.ordenar(tblListaPendientes, tableModelListaPendientesImpresion, 3, FarmaConstants.ORDEN_DESCENDENTE);
        ...
        try
        {
            ...
            try
            {
                ...
                if("S".equalsIgnoreCase(DBCaja.getIndPedConvenio(VariablesCaja.vNumPedVta))){
                    ...
                    flag=electronico.printDocumento(VariablesCaja.vNumPedVta, VariablesCaja.vSecComprobante);
                    if(flag){
                        UtilityCaja.actualizarFechaImpresion(VariablesCaja.vSecComprobante,VariablesVentas.vTip_Comp_Ped);   
