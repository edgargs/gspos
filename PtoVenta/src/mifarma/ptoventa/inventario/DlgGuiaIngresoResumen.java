...
public class DlgGuiaIngresoResumen extends JDialog {
    ...
    private void jbInit() throws Exception {
        ...
        pnlHeader1.setBounds(new Rectangle(10, 10, 660, 65));
    ...
    private void initTableResumenPedido() {
        // KMONCADA 17.08.2016 [PROYECTO M] PERMITE VARIOS LOTES
        int longColLote = 0;
        if(UtilityPtoVenta.isLocalVentaMayorista()){
            longColLote = 60;
        }
        FarmaColumnData[] columnsListaResumenPedido ={ new FarmaColumnData("Codigo", 50, JLabel.LEFT), 
                                                       new FarmaColumnData("Descripcion", (200 - (longColLote/2)), JLabel.LEFT),
                                                       new FarmaColumnData("Unidad", 80, JLabel.LEFT), 
                                                       new FarmaColumnData("Laboratorio", (150 - (longColLote/2)), JLabel.LEFT),
                                                       ...
                                                       new FarmaColumnData("Lote", longColLote, JLabel.RIGHT),
                                                       new FarmaColumnData("Fec Vec", 0, JLabel.RIGHT), 
                                                       new FarmaColumnData("Val Frac", 0, JLabel.RIGHT) };
        
        tableModelResumenPedido = new FarmaTableModel(columnsListaResumenPedido, 
                                                      UtilityPtoVenta.obtenerDefaultValuesTabla(columnsListaResumenPedido.length),
                                                      0);
        FarmaUtility.initSimpleList(tblResumenPedido, 
                                    tableModelResumenPedido,
                                    columnsListaResumenPedido);
    }
    ...
    private void chkKeyPressed(KeyEvent e) {
        ...
        if (mifarma.ptoventa.reference.UtilityPtoVenta.verificaVK_F12(e)) {
            imprimir();
        } else if (mifarma.ptoventa.reference.UtilityPtoVenta.verificaVK_F2(e) && !esGuiaRecepcion) {
            funcion_F2();
    ...
    private void borrarProducto() {
        // KMONCADA 17.08.2016 [PROYECTO M]
        if(UtilityPtoVenta.isLocalVentaMayorista()){
            VariablesInventario.vArrayGuiaIngresoProductos.remove(tblResumenPedido.getSelectedRow());
        }else{
    ...
    private void actualizarProducto() {
        ((ArrayList)(VariablesInventario.vArrayGuiaIngresoProductos.get(tblResumenPedido.getSelectedRow()))).set(4,
                                                                                                                 VariablesInventario.vCant);
        ((ArrayList)(VariablesInventario.vArrayGuiaIngresoProductos.get(tblResumenPedido.getSelectedRow()))).set(7,
                                                                                                                 VariablesInventario.vLote);
        ((ArrayList)(VariablesInventario.vArrayGuiaIngresoProductos.get(tblResumenPedido.getSelectedRow()))).set(8,
                                                                                                                 VariablesInventario.vFechaVec);
        ((ArrayList)(VariablesInventario.vArrayGuiaIngresoProductos.get(tblResumenPedido.getSelectedRow()))).set(5,
                                                                                                                 VariablesInventario.vPrecUnit);
        ((ArrayList)(VariablesInventario.vArrayGuiaIngresoProductos.get(tblResumenPedido.getSelectedRow()))).set(6,
    ...
    private boolean grabar() {
        ...
        try {
            String numera =
                DBInventario.agregarCabeceraGuiaIngreso(VariablesInventario.vFecGuia, VariablesInventario.vTipoDoc,
                                                        VariablesInventario.vNumDoc, VariablesInventario.vTipoOrigen,
                                                        VariablesInventario.vCodOrigen,
                                                        tblResumenPedido.getRowCount() + "",
                                                        FarmaUtility.sumColumTable(tblResumenPedido, 6) + "",
            ...
            for (int i = 0; i < tblResumenPedido.getRowCount(); i++) {
                DBInventario.agregarDetalleGuiaIngreso(numera, VariablesInventario.vTipoOrigen,
                                                       ...
                                                       tblResumenPedido.getValueAt(i, 4).toString(),
    ...
    private void cargarDetalle() {
        try {
            ...
            DBInventario.cargaDetalleGuiaIngreso(VariablesInventario.vNumNota, VariablesInventario.vNumDoc,
        ...
        if (!vEstadoNota.equals(FarmaConstants.INDICADOR_N)) {
            try {
                ...
                if (estadoProcesado.equals(FarmaConstants.INDICADOR_S)) {
                    if (VariablesInventario.vTipoNota.equals(ConstantsInventario.TIP_NOTA_GUIA) &&
                        VariablesInventario.vTipoNotaOrigen.equals(ConstantsInventario.TIP_NOTA_ORIGEN_GUIA)) {
                        if (cargaLoginOper())
                            retorno = true;
                        else {
                            retorno = false;
