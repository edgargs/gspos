--------------------------------------------------------
--  DDL for Package Body PTOVENTA_TOMA_INV
--------------------------------------------------------
...
WHERE   PL.COD_GRUPO_CIA = cCodGrupoCia_in   AND
 ...
 END;
  ...
  OPEN curLabs FOR
       ...
       FROM LGT_PROD P,
            LGT_LAB L,
            /*(SELECT DISTINCT V1.COD_LOCAL,
                    V1.COD_PROD
             FROM (SELECT DISTINCT COD_LOCAL, COD_PROD
                   FROM LGT_NOTA_ES_DET D
                   WHERE D.COD_LOCAL = cCodLocal_in
                   UNION
                   SELECT DISTINCT COD_LOCAL, COD_PROD
                   FROM Int_Recep_Prod_Qs
                   WHERE COD_LOCAL = cCodLocal_in) V1) V2*/
           (
             SELECT DISTINCT COD_LOCAL, COD_PROD
             FROM LGT_KARDEX K
       ...
       OPEN curTi FOR
        /* SELECT DISTINCT PL.COD_PROD || 'Ã' ||
         ...
         WHERE  PL.COD_GRUPO_CIA = ccodgrupocia_in
  	     AND    PL.COD_LOCAL     = ccodlocal_in
  	     AND    P.COD_LAB        = ccodlab_in
         AND    K.FEC_KARDEX BETWEEN fecha_anterior
         AND    SYSDATE
  	     AND    PL.COD_GRUPO_CIA = P.COD_GRUPO_CIA
  	     AND    PL.COD_PROD      = P.COD_PROD
         AND    PL.COD_GRUPO_CIA = K.COD_GRUPO_CIA
         AND    PL.COD_LOCAL = K.COD_LOCAL
         AND    PL.COD_PROD = K.COD_PROD
         AND    K.COD_GRUPO_CIA = P.COD_GRUPO_CIA
         AND    K.COD_PROD = P.COD_PROD;
*/
        SELECT DISTINCT PL.COD_PROD || 'Ã' ||    --ASOSA, 20.01.2011 - cambio de query hecho por RCASTRO
              NVL(P.DESC_PROD,' ') || 'Ã' ||
              P.DESC_UNID_PRESENT || 'Ã' ||
              PL.UNID_VTA || 'Ã' ||
               '0'        || 'Ã' ||
