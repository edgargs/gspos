...
public class DlgCambioClave extends JDialog {
  ...
  private JButtonLabel btnConfirmeContrasena = new JButtonLabel();
		...
		if (e.getKeyCode() == KeyEvent.VK_F1) // Reservado para ayuda
		{
			...
			if (datosValidos()) {
				...
				if (FarmaUtility.rptaConfirmDialog(this,
					...
					String clave = "";
					String neoClave = "";
					clave = txtContActual.getText();
					neoClave = txtContNueva1.getText();
	...
	private boolean datosValidos() {
		...
		if (txtContActual.getText().trim().length() == 0) {
			FarmaUtility.showMessage(this, "Ingrese la contraseña actual",
					txtContActual);
			return false;
		}
		if (txtContNueva1.getText().trim().length() == 0) {
			FarmaUtility.showMessage(this, "Ingrese la nueva contraseña",
					txtContNueva1);
			return false;
		}

		if (txtContNueva2.getText().trim().length() == 0) {
			FarmaUtility.showMessage(this, "Confirme la nueva contraseña",
					txtContNueva2);
			return false;
		}

		if (!txtContNueva1.getText().trim().equals(
				txtContNueva2.getText().trim())) {
			FarmaUtility
					.showMessage(
							this,
