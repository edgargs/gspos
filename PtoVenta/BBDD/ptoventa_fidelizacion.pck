...
--SI EL PEDIDO SOLO EMITE GUIA ES XQ ES CONVENIO AL 100% Y X LO TANTO NO DEBE TENER PUNTOS
  ...
  IS  
	...
	vPtoInicial VTA_PEDIDO_VTA_CAB.PT_INICIAL%TYPE;
	vEstTrx VTA_PEDIDO_VTA_CAB.EST_TRX_ORBIS%TYPE;
  BEGIN
    IF cIndVarios_in != 'S' THEN     
    ...
    SELECT NVL(CAB.PT_ACUMULADO, 0),
           ...
           CASE
             ...
             ELSE
              NVL(CAB.PT_TOTAL, 0)
           END,
		   NVL(CAB.PT_INICIAL, 0),
		   NVL(CAB.EST_TRX_ORBIS, 'P')
    INTO vPtoAcumulado, vPtoRedimido, vPtoTotal,
	       vPtoInicial,vEstTrx
    FROM VTA_PEDIDO_VTA_CAB CAB
    WHERE CAB.COD_GRUPO_CIA = cCodGrupoCia_in
	...
	--ERIOS 16.09.2016 Discrimina offline: se imprime si esta enviado y tiene puntos inicial
	IF vEstTrx = 'E' AND vPtoInicial > 0 THEN
