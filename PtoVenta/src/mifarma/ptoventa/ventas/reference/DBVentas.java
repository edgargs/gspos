...
public class DBVentas {
  ...
  */
    ...
    public static void obtieneInfoProducto(ArrayList pArrayList, String pCodProd) throws SQLException {
        BeanClienteJuridico beanCliente = DlgResumenPedido.beanClientePedido;
        ArrayList parametros = new ArrayList();
        parametros.add(FarmaVariables.vCodGrupoCia);
        parametros.add(FarmaVariables.vCodLocal);
        parametros.add(pCodProd);
        parametros.add(FarmaConstants.INDICADOR_N);
        //if(VariablesCliente.vTipoCliente!=null && VariablesCliente.vTipoCliente.equalsIgnoreCase(ConstantsCliente.CLIENTE_MAYORISTA)){
        if(beanCliente!=null && beanCliente.isIsClienteMayorista()){
            parametros.add(ConstantsCliente.CLIENTE_MAYORISTA);
        }else{
            parametros.add(ConstantsCliente.CLIENTE_MINORISTA);
        }
    ...
    public static void obtieneInfoDetalleProducto(ArrayList pArrayList, String pCodProd) throws Exception {
        BeanClienteJuridico beanClientePedido = DlgResumenPedido.beanClientePedido;
        ArrayList parametros = new ArrayList();
        parametros.add(FarmaVariables.vCodGrupoCia);
        parametros.add(FarmaVariables.vCodLocal);
        parametros.add(pCodProd);
     ...
     * */
	    ...
	    **/
        ...
        //KMONCADA 2016.01.21 INDICADOR PARA GENERAR PROFORMA [LOCAL M]
    ...
    public static void cargaListaProductosSustitutos(FarmaTableModel pTableModel,
                                                     String pCodProd, BeanClienteJuridico beanClientePedido) throws SQLException {
        pTableModel.clearTable();
        ArrayList parametros = new ArrayList();
        parametros.add(FarmaVariables.vCodGrupoCia);
        parametros.add(FarmaVariables.vCodLocal);
...
*/
    ...
    public static List prodsCampaniasAplicables(List listProds, List listCamps) throws SQLException {
        ...
        String codProds = "";
        String p1 =
            "select cc.COD_PROD,cc.COD_CAMP_CUPON,cc.IND_EXCLUYE_ACUM_AHORRO, CC.PT_MULTIPLICA " + "from (SELECT U.IND_EXCLUYE_ACUM_AHORRO,COD_PROD, v.COD_REAL as \"COD_CAMP_CUPON\" ,V.MONT_MIN_USO, V.PT_MULTIPLICA, " +
 ...
 p1 + ")a," + "vta_campana_prod_uso cu where a.prod =  cu.cod_prod " + "and   cu.cod_grupo_cia  = u.cod_grupo_cia " +
    "and   cu.cod_camp_cupon = u.cod_camp_cupon " +
        ...
        String p4 =
            " ( SELECT NVL(CP.COD_CAMP_CUPON_PROD_USO,CP.COD_CAMP_CUPON) COD_CAMPANA,CP.COD_CAMP_CUPON COD_REAL," +
            //agregado 04.03.2010
            "cp.prioridad,cp.ranking,CP.MONT_MIN_USO, NVL(CP.PT_MULTIPLICA,-1) PT_MULTIPLICA " + " FROM PTOVENTA.VTA_CAMPANA_CUPON CP " +
    ...
    public static void grabarDetallePedido_02(String secrespaldo) throws SQLException {
      ...
      if (!VariablesVentas.existenValoresAdicionales) {          //ASOSA - 08/08/2014
        ...
        */
        ...
        //KMONCADA 22.01.2016 REGISTRO DE PEDIDO COMO PROFORMA [LOCAL M]
        if(UtilityPtoVenta.isLocalVentaMayorista()){
            parametros.add("S");
        }else{
            parametros.add("N");
    ...
    */
     ...
     * @throws Exception
     */
    public static ArrayList obtieneListaDsctoPrecios(String pCodProd, String nroRuc)throws Exception{
        ArrayList array = new ArrayList();
        ArrayList parametros = new ArrayList();
        parametros.add(FarmaVariables.vCodGrupoCia);
        parametros.add(FarmaVariables.vCodLocal);
        parametros.add(pCodProd);
        if(nroRuc == null || (nroRuc != null && nroRuc.trim().length()==0)){
            parametros.add("N");
        }else{
            parametros.add(nroRuc);
        }
        log.debug("PTOVENTA_VTA_MAYORISTA.F_CUR_DESC_VOLUMEN_MAYORISTA(?,?,?,?):" + parametros);
        FarmaDBUtility.executeSQLStoredProcedureArrayList(array, "PTOVENTA_VTA_MAYORISTA.F_CUR_DESC_VOLUMEN_MAYORISTA(?,?,?,?)", parametros);
        return array;
    }
    
    public static String obtieneFecModNumeraPed(String codNumera) throws SQLException {
        ArrayList parametros = new ArrayList();
        parametros.add(FarmaVariables.vCodGrupoCia);
        parametros.add(FarmaVariables.vCodLocal);
        parametros.add(codNumera);
        ArrayList myArray = new ArrayList();
    ...
    /**
     * Valida si es un producto controlado y si se le puede vender al cliente
     * @author KMONCADA
     * @since 07.06.2016
     * @param pCodGrupoCia
     * @param pCodLocal
     ...
     * @throws Exception
     */
    public static boolean isProductoControlado(String pCodGrupoCia, String pCodLocal, String pCodProd) {
        boolean isControlado = false;
        try{
            ArrayList parametros = new ArrayList();
            parametros.add(pCodGrupoCia);
            parametros.add(pCodLocal);
            parametros.add(pCodProd);
            log.info("PTOVENTA_VTA_MAYORISTA.F_IS_PROD_CONTROLADO(?,?,?): " + parametros);
            String rpsta = FarmaDBUtility.executeSQLStoredProcedureStr("PTOVENTA_VTA_MAYORISTA.F_IS_PROD_CONTROLADO(?,?,?)", parametros);
            if("S".equalsIgnoreCase(rpsta)){
                isControlado = true;
            }
        }catch(Exception ex){
            log.error("", ex);
