...
--Fecha       Usuario    Comentario
--20.01.2015  RHERRERA      Creación: Obtiene si tico cuenta con padre
FUNCTION GET_IND_PADRE (cCodLocal CHAR) RETURN CHAR ;
 ...
 BEGIN
  ...
  FUNCTION P_IND_CIERRE_MARKET (cFecDiaCierre     IN CHAR,
    ...
    BEGIN
      ...
      ind_padre :=GET_IND_PADRE(cCodMarket);
           if         ind_market = 'N' then
              return 'S';
           elsif     ind_padre   = 'N' then
              return 'S';
             end if;
...
FUNCTION GET_IND_PADRE (cCodLocal CHAR) RETURN CHAR IS
