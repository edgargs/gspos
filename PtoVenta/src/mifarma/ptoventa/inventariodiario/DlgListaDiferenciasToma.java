...
public class DlgListaDiferenciasToma extends JDialog {
    ...
    private void initTableListaDiferenciasConsolidado() {
        tableModelDiferenciasConsolidado =
                new FarmaTableModel(ConstantsInvDiario.columnsListaDiferenciasConsolidado, ConstantsInvDiario.defaultValuesListaDiferenciasConsolidado,
                                    0);

        FarmaUtility.initSelectList(tblRelacionDiferenciasProductos, tableModelDiferenciasConsolidado,
        ...
        if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
            ...
            for (int i = 0; i < tblRelacionDiferenciasProductos.getRowCount(); i++) {
                ...
                */
                ...
                vProductoSeleccionado.add(FarmaUtility.getValueFieldArrayList(tableModelDiferenciasConsolidado.data, i,
                                                                              1)); //COD_PROD
                vProductoSeleccionado.add(FarmaUtility.getValueFieldArrayList(tableModelDiferenciasConsolidado.data, i,
                                                                              10)); //numero toma
                vProductoSeleccionado.add(FarmaUtility.getValueFieldArrayList(tableModelDiferenciasConsolidado.data, i,
                ...
                FarmaUtility.operaListaProd(VariablesInvDiario.vListadoProductosDiferenciaSeleccionadas,
                                            vProductoSeleccionado, valor, 0);
    ...
    private void cargaListaDiferenciasConsolidado() {
        try {
            ...
            if (tblRelacionDiferenciasProductos.getRowCount() > 0)
                FarmaUtility.ordenar(tblRelacionDiferenciasProductos, tableModelDiferenciasConsolidado, 2,
    ...
    private void imprimir() {
        log.debug("Imprimir");
        if (!com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this, "¿Está seguro que desea imprimir?"))
            return;
        ...
        for (int i = 0; i < tblRelacionDiferenciasProductos.getRowCount(); i++) {
            ...
            vUnidPres = (String)tblRelacionDiferenciasProductos.getValueAt(i, 3);
            vStkActual = (String)tblRelacionDiferenciasProductos.getValueAt(i, 4);
            ...
            if (FarmaVariables.vAceptar) {
                ...
                try {
                    for (int i = 0; i < VariablesInvDiario.vListadoProductosDiferenciaSeleccionadas.size(); i++) {
                        ...
                        DBInvDiario.pRevertirProducto(pCodigoProd, pCodigoToma, VariablesInvDiario.vCodMotivoRevertir,
                    ...
                    cargaListaDiferenciasConsolidado();
                    FarmaUtility.showMessage(this, "Se ha revertido correctamente.", txtProductos);
                } catch (SQLException sql) {
    ...
    private void seleccionaProducto() {
      ...
      */
        ...
        vProductoSeleccionado.add(FarmaUtility.getValueFieldArrayList(tableModelDiferenciasConsolidado.data, vFila,
                                                                      1)); //COD_PROD
        vProductoSeleccionado.add(FarmaUtility.getValueFieldArrayList(tableModelDiferenciasConsolidado.data, vFila,
                                                                      10)); //numero toma
        vProductoSeleccionado.add(FarmaUtility.getValueFieldArrayList(tableModelDiferenciasConsolidado.data, vFila,
        ...
        FarmaUtility.operaListaProd(VariablesInvDiario.vListadoProductosDiferenciaSeleccionadas, vProductoSeleccionado,
                                    valor, 0);
        log.debug("Lista Dif Seleccionada: " + VariablesInvDiario.vListadoProductosDiferenciaSeleccionadas);
    }
    ...
    public void fGeneraPedidoDescuentoPersonal() {
        ...
        if (validacionBoleta()) {
            if (generaPedidoTemporal()) {
                DlgPedidoDiario dlgPedido = new DlgPedidoDiario(myParentFrame, "", true);
                ...
                if (FarmaVariables.vAceptar) {
                    ...
                    FarmaUtility.showMessage(this,
                                             "Se generó el pedido con éxito.\nVerificar que existe papel en la impresora.!!!",
                                             txtProductos);
                    try {
    ...
    public boolean generaPedidoTemporal() {
        ...
        try {
            ...
            String pSecDetalle = "", pCodProd = "", pCantMov = "", pValFrac = "", pSecToma = "", pSecKardex =
                "", pCodMotivo = "";
            for (int i = 0; i < VariablesInvDiario.vListadoProductosDiferenciaSeleccionadas.size(); i++) {
                pSecDetalle = "" + (i + 1);
                pCodProd =
                ...
                DBInvDiario.agregaProdDetalleTemporal(pNumPedido, pSecDetalle, pCodProd, pCantMov, pValFrac, pSecToma,
                                                      pSecKardex, pCodMotivo);
    ...
    */
    ...
    public boolean validacionBoleta() {
        ...
        if (VariablesInvDiario.vListadoProductosDiferenciaSeleccionadas.size() > 0) {
            ...
            for (int i = 0; i < VariablesInvDiario.vListadoProductosDiferenciaSeleccionadas.size(); i++) {
                pCant =
                        FarmaUtility.getDecimalNumber(FarmaUtility.getValueFieldArrayList(VariablesInvDiario.vListadoProductosDiferenciaSeleccionadas,
                                                                                          i, 3));
                if (pCant > 0) {
                    pCodProd =
                            FarmaUtility.getValueFieldArrayList(VariablesInvDiario.vListadoProductosDiferenciaSeleccionadas,
            ...
            if (!pValidacion) {
                for (int i = 0; i < tableModelDiferenciasConsolidado.data.size(); i++) {
                    if (pCodProd.trim().equalsIgnoreCase(FarmaUtility.getValueFieldArrayList(tableModelDiferenciasConsolidado.data,
                                                                                             i, 1).trim())) {
                ...
                if (VariablesInvDiario.dTotalProds < 0) {
                    ...
                    if (FarmaVariables.vAceptar) {
                        ...
                        if (agregarAjuste()) {
                            ...
                            FarmaUtility.showMessage(this, "Verificar que existe papel en la impresora.!!!",
                                                     txtProductos);
                            try {
                                imprimirAjuste();
                                limipiarVariables();
    ...
    private void realizarAjuste() {
        ...
        try {
            for (int i = 0; i < VariablesInvDiario.vListadoProductosDiferenciaSeleccionadas.size(); i++) {
                ...
                DBInvDiario.ajustarProd(pCodigoProd, pCodigoToma, VariablesInvDiario.vCodMotivoAjuste, pSecKardex,
                                        pCantMov, pValFracProd, pCodMotKardex, VariablesInvDiario.vCodAjuste);
            }
            VariablesInvDiario.vListadoProductosDiferenciaSeleccionadas = new ArrayList();
    ...
    private void obetenerTotal() {
        ...
        if (VariablesInvDiario.vListadoProductosDiferenciaSeleccionadas.size() > 0) {
            ...
            VariablesInvDiario.dTotalProds =
                    FarmaUtility.getDecimalNumber(FarmaUtility.formatNumber(VariablesInvDiario.dTotalProds, 2));
    ...
    private void imprimirAjuste() throws Exception {
        ...
        if (!vPrint.startPrintService()) {
            ...
            try {
                ...
                //vPrint.printBlankLine(1);
                vPrint.printLine(FarmaPRNUtility.alinearIzquierda("Compañia: ", 10) +
                                 FarmaPRNUtility.alinearIzquierda(FarmaVariables.vNomCia, 30), true);
                vPrint.printLine(FarmaPRNUtility.alinearIzquierda("Local:   ", 10) +
                                 FarmaPRNUtility.alinearIzquierda(FarmaVariables.vCodLocal + " - " +
                                                                  FarmaVariables.vDescLocal, 40), true);
                vPrint.printLine(FarmaPRNUtility.alinearIzquierda("Fecha: ", 10) +
                                 FarmaPRNUtility.alinearIzquierda(fechaActual, 10) + "       " + "  Motivo : " +
                                 VariablesInvDiario.vDescMotivoAjuste.trim() + "      " + " Importe : " +
                                 VariablesInvDiario.dTotalProds, true);
                //vPrint.printBlankLine(1);
                vPrint.printLine(" ", true);
                ...
                vPrint.printLine("========================================================================================================================",
                                 true);
                vPrint.printLine(FarmaPRNUtility.alinearIzquierda(FarmaPRNUtility.llenarBlancos(51) + " Total:  " +
                                                                  VariablesInvDiario.dTotalProds, 100), true);
                vPrint.printLine("========================================================================================================================",
                                 true);
                vPrint.printLine(" ", true);
                vPrint.printLine(" ", true);
                ...
                vPrint.printLine(FarmaPRNUtility.alinearIzquierda("      Firma Auditor " +
                                                                  "                                                " +
                                                                  "        Firma Sello del Jefe Local", 160), true);
                vPrint.printLine(" ", true);
                vPrint.printLine(" ", true);
                vPrint.printLine(" ", true);
                vPrint.printLine(" ", true);
    ...
    private void imprimirBoleteo(String pSecPedidoTemporal) throws Exception {
        ...
        if (!vPrint.startPrintService()) {
            ...
            try {
                ...
                vPrint.printLine(FarmaPRNUtility.alinearIzquierda("Fecha: ", 10) +
                                 FarmaPRNUtility.alinearIzquierda(fechaActual, 10) + "       " + "  Motivo : " +
                                 pDescMotivo.trim() + "      " + " Importe : " + totalPedido, true);
                //vPrint.printBlankLine(1);
                vPrint.printLine(" ", true);
                ...
                for (int i = 0; i < vListaTrabDcto.size(); i++) {
                    ...
                    CodTrab = FarmaUtility.getValueFieldArrayList(vListaTrabDcto, i, 0);
                    DescTrab = FarmaUtility.getValueFieldArrayList(vListaTrabDcto, i, 1);
                    Importe = FarmaUtility.getValueFieldArrayList(vListaTrabDcto, i, 2);
                    vPrint.printLine(FarmaPRNUtility.alinearIzquierda(CodTrab.trim(), 9) + " " +
                                     FarmaPRNUtility.alinearIzquierda(DescTrab.trim(), 50) + " " +
                                     FarmaPRNUtility.alinearIzquierda(Importe.trim(), 15) + " " +
                                     FarmaPRNUtility.alinearDerecha("           ", 15) + " " +
