...
public class DlgIngresoSobre extends JDialog {
    ...
    private void initTableListaVentasVendedor() {
        ...
        tableModelLista =
                new FarmaTableModel(ConstantsCaja.columnsListaSobresTmp, ConstantsCaja.defaultValuesListaSobresTmp, 0);
    ...
    private void cargaCombo() {
        FarmaLoadCVL.loadCVLfromArrays(cmbMoneda, ConstantsCaja.HASHTABLE_MONEDASOBRES, ConstantsCaja.MONEDAS_COD,
    ...
    private void ordenar() {
        ...
        if (FarmaVariables.vAceptar) {
            FarmaUtility.ordenar(tblSobres, tableModelLista, Integer.parseInt(VariablesCaja.vColumna),
    ...
    private void listarSobres() {
        ...
        try { // if(FechaIni.length()>0&&FechaFin.length()>0)
    ...
    private void cmbMoneda_keyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_ENTER) {
            VariablesCaja.vCodTipoMon =
                    FarmaLoadCVL.getCVLCode(ConstantsCaja.HASHTABLE_MONEDASOBRES, cmbMoneda.getSelectedIndex());
            VariablesCaja.vDescTipoMon =
                    FarmaLoadCVL.getCVLDescription(ConstantsCaja.HASHTABLE_MONEDASOBRES, VariablesCaja.vCodTipoMon);
            FarmaUtility.moveFocus(txtMonto);
    ...
    private void adicionaDetallePago() {
        ...
        lblTotal.setText(FarmaUtility.formatNumber(FarmaUtility.sumColumTable(tblSobres, 4)));
        FarmaLoadCVL.setSelectedValueInComboBox(cmbMoneda, ConstantsCaja.HASHTABLE_MONEDASOBRES,
                                                ConstantsCaja.EFECTIVO_SOLES);
    ...
    private boolean obtieneDatosFormaPago() {
        ...
        VariablesCaja.vCodTipoMon =
                FarmaLoadCVL.getCVLCode(ConstantsSobres.HASHTABLE_MONEDASOBRES, cmbMoneda.getSelectedIndex());
        ...
        VariablesCaja.vDescMonedaPagoTmp =
                FarmaLoadCVL.getCVLDescription(ConstantsCaja.HASHTABLE_MONEDASOBRES, codMoneda);
        ...
        if (codMoneda.equalsIgnoreCase(FarmaConstants.CODIGO_MONEDA_SOLES))
            ...
            VariablesCaja.vValTotalPagadoTmp =
                    FarmaUtility.formatNumber(FarmaUtility.getDecimalNumber(VariablesCaja.vValMontoPagadoTmp) *
    ...
    private void eliminaFormaPagoEntrega() {
        ...
        if (tieneRegistroSeleccionado(tblSobres)) {
            ...
            if (VariablesCaja.vIndSobreTmp.trim().equalsIgnoreCase(FarmaConstants.INDICADOR_N)) {
                ...
                try {
                    //  if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,"¿El sistema eliminará el registro físicamente. Desea Continuar?"))
                    // {
                    //Inicio
                    if (VariablesCaja.vIndSobreTmp.trim().equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                        ...
                        VariablesCaja.vSecMovCajaTmp =
                                FarmaUtility.getValueFieldArrayList(tableModelLista.data, tblSobres.getSelectedRow(),
                                                                    7);
                        VariablesCaja.vCodFormaPagoTmp =
                                ((String)tblSobres.getValueAt(tblSobres.getSelectedRow(), 0)).trim();
                        //VariablesCaja.vCodTipoMon = ((String)tblSobres.getValueAt(tblSobres.getSelectedRow(),8)).trim();
                        VariablesCaja.vCodTipoMon =
                                FarmaUtility.getValueFieldArrayList(tableModelLista.data, tblSobres.getSelectedRow(),
                                                                    8);

                        VariablesCaja.vIndSobreTmp =
                                ((String)tblSobres.getValueAt(tblSobres.getSelectedRow(), 5)).trim();
                        VariablesCaja.vCodigoSobreTmp =
                                ((String)tblSobres.getValueAt(tblSobres.getSelectedRow(), 6)).trim();
                        //VariablesCaja.vSecTmp = ((String)tblSobres.getValueAt(tblSobres.getSelectedRow(),9)).trim();
                        VariablesCaja.vSecTmp =
                                FarmaUtility.getValueFieldArrayList(tableModelLista.data, tblSobres.getSelectedRow(),
                                                                    9);
                        ...
                        DBCaja.eliminaSobreRegistrado(VariablesCaja.vSecMovCajaTmp, VariablesCaja.vCodFormaPagoTmp,
                                                      VariablesCaja.vCodTipoMon, VariablesCaja.vCodigoSobreTmp);
                    }
    ...
    private void guardaDatosDetalle() {
        //VariablesCaja.vSecMovCajaTmp = ((String)tblSobres.getValueAt(tblSobres.getSelectedRow(),7)).trim();
        VariablesCaja.vSecMovCajaTmp =
                FarmaUtility.getValueFieldArrayList(tableModelLista.data, tblSobres.getSelectedRow(), 7);

        VariablesCaja.vCodFormaPagoTmp = ((String)tblSobres.getValueAt(tblSobres.getSelectedRow(), 0)).trim();
        //VariablesCaja.vCodTipoMon = ((String)tblSobres.getValueAt(tblSobres.getSelectedRow(),8)).trim();
        VariablesCaja.vCodTipoMon =
                FarmaUtility.getValueFieldArrayList(tableModelLista.data, tblSobres.getSelectedRow(), 8);
        ...
        //VariablesCaja.vSecTmp = ((String)tblSobres.getValueAt(tblSobres.getSelectedRow(),9)).trim();
        VariablesCaja.vSecTmp =
                FarmaUtility.getValueFieldArrayList(tableModelLista.data, tblSobres.getSelectedRow(), 9);
    ...
    private void guardarCreados() {
        ...
        try {
            for (int i = 0; i < tblSobres.getRowCount(); i++) {
                ...
                if (((String)tblSobres.getValueAt(i,
                                                  5)).trim().equalsIgnoreCase(FarmaConstants.INDICADOR_N)) { //INICIO
              ...
              */
                    pSecSobre =
                            //DBCaja.agregaSobre(FarmaUtility.getValueFieldArrayList(tableModelLista.data,tblSobres.getSelectedRow(),7),
                            ...
                            DBCaja.getRealizaAccionSobreTMP_02(ConstantsSobres.ACC_INGRESO,
                                                               //ASOSA, 11.06.2010 usando metodo mejorado
                                " ", " ", pSecMovCaja, ((String)tblSobres.getValueAt(i, 0)).trim(), tip,
                                ((String)tblSobres.getValueAt(i, 3)).trim(),
                                ((String)tblSobres.getValueAt(i, 4)).trim(),
