...
-- Descripcion:ACTUALIZA EL CAMPO COD_PBL_ETV DE LA TABLA CE_REMITO
--Fecha       Usuario		 Comentario
--14.02.2014  CHUANES      Creación
...
CREATE OR REPLACE PACKAGE BODY PTOVENTA."PTOVENTA_CE_REMITO" AS
 ...
 RETURN FarmaCursor IS
    curDet FarmaCursor;
    clocal_tico CHAR(3):= GET_LOCAL_TICO(cCodLocal_in); -- local tico
 BEGIN
...
SELECT to_char(A.FEC_DIA_VTA,'dd/mm/yyyy') || 'Ã' ||
       ...
       WHERE R.COD_REMITO = cCodigoRemito
         ...
         */
         ...
         UNION ALL--- UNION TABLA SOBRES MARKET rherrera 27.08.2014 -- remito
              SELECT TI.FEC_VTA            || 'Ã' ||
                        TI.COD_SOBRE       || 'Ã' ||
                        TI.MONEDA          || 'Ã' ||
                        ...
                        TI.TIP_MOND_SOL                                || 'Ã' ||
                        TI.TIP_MOND_DOL
                 FROM CE_SOBRE_TICO TI , CE_REMITO CR
                 WHERE  TI.COD_REMITO = CR.COD_REMITO
                 AND    TI.EST_SOBRE = 'A'
                 AND    TI.IND_LOCAL = 'S'
                 --AND    TI.USU_LOGIN = 'TICO'
                 AND    TI.COD_REMITO = cCodigoRemito
                 ---- 28.10.2014
                 AND    TI.COD_LOCAL  = clocal_tico --28.10.2014 local tico
                        ;
    RETURN curDet;
          ...
          FROM (SELECT V1.FECHA,
                  ...
                  FROM (SELECT B.fec_crea_remito FECHA,
                         ...
                         GROUP BY B.fec_crea_remito,
                                  B.COD_REMITO,
                                  B.USU_CREA_REMITO
                  UNION ALL -- rherrera : remito tico 27.08.2014
                        --@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
                         SELECT B.fec_crea_remito FECHA,
                               NVL(B.COD_REMITO, ' ') CODIGO,
                               B.USU_CREA_REMITO USU,
                               SUM(SOBRES.MON_ENTRE_SOL) SOLES,
                               SUM(SOBRES.MON_ENTRE_DOL) DOLARES,
                               NVL(SUM(SOBRES.MON_ENTREGA_TOTAL), 0) TOTAL,
                               SUM(CASE
                                     WHEN SOBRES.COD_SOBRE IS NOT NULL THEN
                                      1
                               ...
                               TO_CHAR(SYSDATE, 'DD/MM/YYYY') RANGO
                          FROM CE_REMITO             B,
                               CE_SOBRE_TICO         SOBRES
                         WHERE B.COD_GRUPO_CIA = cCodGrupoCia_in
                           AND B.COD_LOCAL = cCodLocal_in
                           --local tico 28.10.2014 inicio
                           AND SOBRES.COD_LOCAL = clocal_tico
                           ----- fin
                           AND B.fec_crea_remito -- BETWEEN SYSDATE-30 AND SYSDATE
                               BETWEEN
                               TO_DATE(to_char(trunc(SYSDATE - 30),
  ...
  BEGIN
    ...
    UPDATE Ce_Sobre_Tmp SO
       ...
       WHERE SO.COD_SOBRE = cCodSobre
         ...
         AND SO.COD_LOCAL = cCodLocal;
    --RHERRERA 29.08.2014
    UPDATE   CE_SOBRE_TICO S
         SET S.COD_REMITO = cNumRemito,
             S.IND_LOCAL  = 'S' ,
             S.PRECINTO   = cPrecinto
       WHERE S.FEC_VTA    = cFecha
       AND   S.COD_SOBRE  = cCodSobre
       AND   S.COD_LOCAL  = clocal_tico; -- 28.10.2014 local tico
...
--@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
-- Rherrera: Se modifico para poder generar el remito para mas de un local
-- 23.09.2014
PROCEDURE CE_P_SAVE_HIST_REMI(cCodGrupoCia_in IN CHAR,
                             cCodLocal_in    IN CHAR,
                             cCodRemito      IN CHAR,
...
cursor04 FarmaCursor := CE_F_LISTA_TOTALES_SD(cCodGrupoCia_in,
                                              cCodLocal_in,
                                              vUsu_in);
ccod_local char(3);--rherrera 22.09.2014
fecha VARCHAR2(200);
cants VARCHAR2(200);
montos VARCHAR2(200);
cantd VARCHAR2(200);
...
BEGIN
     LOOP --cursor01
     FETCH cursor01 INTO fecha, cants, montos, cantd, montod,ccod_local/**/;
     EXIT WHEN cursor01%NOTFOUND;
          ...
          VALUES(cCodGrupoCia_in,ccod_local/*cCodLocal_in*/,cCodRemito,'S',TO_DATE(fecha,'dd/MM/yyyy'),
     ...
     END LOOP;
     LOOP --cursor02
     FETCH cursor02 INTO usuario, montototals, montototald,ccod_local/**/;
     EXIT WHEN cursor02%NOTFOUND;
          ...
          VALUES(cCodGrupoCia_in,ccod_local/**/,cCodRemito,'S',usuario,
...
/* *********************************************************************** */
--@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
-- Rherrera: Se modifico para listar la suma de montos de mas de un local y
--           por remito
-- 23.09.2014
FUNCTION SEG_F_CUR_GET_DATA4(cCodGrupoCia_in IN CHAR,
                               cCodLocal_in    IN CHAR,
...
--------------------------------------
SELECT --distinct  --sacado rherrera 22.09.2014
             V2.FECHA,
             V2.CANT_S,
             TO_CHAR(V2.SOLES, '999,999,990.00'),
             V2.CANT_D,
        ...
        FROM (SELECT TO_CHAR(sobres.FEC_DIA_VTA, 'DD/MM/YYYY') FECHA,
               ...
               UNION ALL
                ...
                SELECT T.FEC_VTA FECHA,
                       NVL(SUM(T.MON_ENTRE_SOL), 0) SOLES,
                       NVL(SUM(CASE
                                 WHEN T.TIP_MOND_SOL = '1' THEN
                                  1
                               END),
                           0) CANT_S,
                       NVL(SUM(T.MON_ENTRE_DOL), 0) DOLARES,
                       NVL(SUM(CASE
                                 WHEN T.TIP_MOND_DOL = '1' THEN
                                  1
                               END),
                           0) CANT_D,
                           T.COD_LOCAL
                  FROM CE_SOBRE_TICO T
                 WHERE T.COD_REMITO = TRIM(cCodRemito)
                 AND   T.COD_LOCAL  = clocal_tico --28.10.2014 local tico
                 GROUP BY T.FEC_VTA,T.COD_LOCAL
              --@@@@@@@@@@@@@@@@@@@@@@@@@--
               ) V2
               ORDER BY 1 ASC ;--RHERRERA AGREGADO 22/09/2014
    RETURN curVta;
  END;
...
*/
SELECT --distinct   rherrera 22.09.2014 sacado
               V1.USU,
             TO_CHAR(V1.SOLES, '999,999,990.00'),
             TO_CHAR(V1.DOLARES, '999,999,990.00'),
             V1.COD_LOCAL --***AGREGADO
        FROM (
             ...
             from (
               ...
               GROUP BY vt.usu_crea,/**/vt.cod_local
               --rherrera 22.09.2014
               UNION ALL
               SELECT R.USU_CREA_REMITO usu_crea,
                      NVL(SUM(T.MON_ENTRE_SOL), 0) SOLES,
                      NVL(SUM(T.MON_ENTRE_DOL), 0) DOLARES,
                      T.COD_LOCAL
                 FROM CE_SOBRE_TICO T,
                      CE_REMITO     R
                WHERE T.COD_REMITO = R.COD_REMITO
                AND   T.COD_LOCAL  = clocal_tico --28.10.2014 local tico
                AND   T.COD_REMITO = TRIM(cCodRemito)
                GROUP BY R.USU_CREA_REMITO,/**/t.cod_local
...
/* ******************************************************************* */
--@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
-- Rherrera: Se sumo las cantidades para genera el voucher de remito para
--           más de un local
-- 23.09.2014
  ...
  FUNCTION CE_F_HTML_VOUCHER_REMITO_DU(cCodGrupoCia_in IN CHAR,
    ...
    --RHERRERA : 23.09.2014 : Se agrega SUM  a las cantidades y se agrupan
    CURSOR cursor3 IS SELECT xx.usuario,
                             to_char(SUM(xx.total_s),'999,999,990.00') total_s,
                             to_char(SUM(xx.Total_d),'999,999,990.00') Total_d
                      FROM CE_TOTS_REMI_SD xx
                      WHERE xx.cod_grupo_cia=cCodGrupoCia_in
                      --AND xx.cod_local=cCodLocal_in --designa local
                      AND xx.cod_remito=cCodRemito
                      AND xx.Ind_Remito='S'
                      GROUP BY xx.usuario;
    CURSOR cursor4 IS SELECT to_char(yy.fec_dia_vta_rem,'dd/MM/yyyy') fec_dia_vta_rem,
                             SUM(yy.cant_s) cant_s,
                             to_char(SUM(yy.mont_s),'999,999,990.00') mont_s,
                             SUM(yy.cant_d) cant_d,
                             to_char(SUM(yy.mont_d),'999,999,990.00') mont_d
                      FROM CE_SOBRE_REMI_SD yy
                      WHERE yy.cod_grupo_cia=cCodGrupoCia_in
                      --AND yy.cod_local=cCodLocal_in --designa local
                      AND yy.cod_remito=cCodRemito
                      AND yy.Ind_Remito='S'
                      GROUP BY to_char(yy.fec_dia_vta_rem,'dd/MM/yyyy');
    CURSOR curSobSol IS SELECT to_char(zz.fec_dia_vta_rem,'dd/MM/yyyy') fec_dia_vta_rem,
                               SUM(zz.cant_s) cant_s,
                               to_char(SUM(zz.mont_s),'999,999,990.00') mont_s,
                               SUM(zz.cant_d) cant_d,
                               to_char(SUM(zz.mont_d),'999,999,990.00') mont_d
                        FROM CE_SOBRE_REMI_SD zz
                        WHERE zz.cod_grupo_cia=cCodGrupoCia_in
                        --AND zz.cod_local=cCodLocal_in --designa local
                        AND zz.cod_remito=cCodRemito
                        AND zz.Ind_Remito='N'
                        GROUP BY to_char(zz.fec_dia_vta_rem,'dd/MM/yyyy');
    CURSOR curSobDol IS SELECT ww.usuario,
                               to_char(SUM(ww.total_s),'999,999,990.00') total_s,
                               to_char(SUM(ww.Total_d),'999,999,990.00') Total_d
                        FROM CE_TOTS_REMI_SD ww
                        WHERE ww.cod_grupo_cia=cCodGrupoCia_in
                        --AND ww.cod_local=cCodLocal_in --designa local
...
dbms_output.put_line('hito05');
  ...
  select g.raz_soc_cia
   ...
   AND    E.COD_CIA=G.COD_CIA;
	...
	select
		case when E.cod_cia = '003' THEN g.nom_cia||' S.A.C.'||' - '||SUBSTR(e.desc_corta_local,1,6)
                   ...
                   nvl(e.DIREC_LOCAL_CORTA, e.DIREC_LOCAL_CORTA),
				   G.COD_CIA
   ...
   --Para
    ...
    SELECT to_char(SUM(xx.total_s),'999,999,990.00') total_s,
           to_char(SUM(xx.Total_d),'999,999,990.00') Total_d
    ...
    */
    ...
    and    s.cod_grupo_cia = cCodGrupoCia_in
    and    s.cod_local = cCodLocal_in
    union all--rherrera 22.09.2014
    select t.cod_sobre
    from   ce_sobre_tico t
    where  t.cod_remito = cCodRemito
    and    t.cod_local  = clocal_tico -- local tico 28.10.2014
...
-- 23.09.2014
  ...
  BEGIN
   ...
   --Para
	...
	AND COD_CIA = vCodCia
	;
--rherrera:23.09.2014 se agrego union all de ce_sobre_tico y sumo las cantidades
    select
		vLocal|| '@' ||
		vDireccion || '@' ||
           vPara || '@' ||
           trim(to_char((select count(1)
                          from (select s.cod_sobre
                                ...
                                --rherrrera
                                 union all
                                 select c.cod_sobre
                                 from   ce_sobre_tico c
                                 where  c.cod_remito = cCodRemito
                                 and    c.cod_local =  clocal_tico -- 28.10.2014 local tico
                                   )),
                        '999999990')) || '@' ||
                                      -- agrega SUM()
                                      -- V1.SOLES
                                      -- V1.DOLARES
           (SELECT distinct trim(TO_CHAR(SUM(V1.SOLES), '999,999,990.00')) || '@' ||
                            trim(TO_CHAR(SUM(V1.DOLARES), '999,999,990.00'))
              FROM (SELECT vt.usu_crea USU,
                     ...
                     GROUP BY vt.usu_crea

                     --rherrera 23.09.2014
                     union all
                     select r.USU_CREA_REMITO USU,
                            SUM(x.mon_entre_sol)   SOLES,
                            SUM(x.mon_entre_dol)   DOLARES
                     from   ce_sobre_tico x,
                            ce_remito     r
                     where  x.cod_remito = r.cod_remito
                     AND    x.cod_remito = TRIM(cCodRemito)
                     and    x.cod_local  = clocal_tico
                     GROUP BY r.USU_CREA_REMITO
