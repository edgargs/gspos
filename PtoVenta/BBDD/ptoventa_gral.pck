CREATE OR REPLACE PACKAGE PTOVENTA."PTOVENTA_GRAL" AS
  ...
  --Descripcion: Indicador Gestor de Transacciones
  --Fecha       Usuario     Comentario
  --10/08/2015  ERIOS       Creacion
  FUNCTION GET_IND_GESTOR_TX RETURN CHAR;

  --Descripcion: Indicador de impresion de kardex
  --Fecha       Usuario     Comentario
  --06/10/2015  ERIOS       Creacion
  FUNCTION GET_IND_IMPR_KARDEX RETURN CHAR;

  --Descripcion: Indicador de impresion del detalle pedido
  --Fecha       Usuario     Comentario
  --06/10/2015  ERIOS       Creacion
  FUNCTION GET_IND_IMPR_DET_PEDIDO RETURN CHAR;

  --Descripcion: Obtener codigo de marca del local
  --Fecha       Usuario     Comentario
  --12/08/2016  ASOSA       Creacion  
  FUNCTION GET_COD_MARCA_LOCAL(cCodGrupoCia_in IN CHAR,
                               cCodCia_in      IN CHAR,
                               cCodLocal_in    IN CHAR)
  RETURN CHAR;

  --Descripcion: Obtener tipo de almacen del local
  --Fecha       Usuario     Comentario
  --29/08/2016  EMAQUERA    Creacion
  FUNCTION FN_VALIDA_TIPO_ALM (cCodLocal CHAR)
  RETURN CHAR;
...
CREATE OR REPLACE PACKAGE BODY PTOVENTA."PTOVENTA_GRAL" AS
  ...
  IS
    curGral FarmaCursor;
	v_cCodCia CHAR(3);
  v_TpAlm  REL_LOCAL_ALMACEN.ALM_QS%TYPE;
  BEGIN
        ...
        ELSIF(cTipoMaestro_in = '2') THEN --MATRIZ
           --ERIOS 05.05.2016 Determina almacen por CIA
       --EMAQUERA 29.08.2016 Determina almacen por tabla relacional           
       v_TpAlm:=FN_VALIDA_TIPO_ALM(cCodBusqueda_in);
/*
		   SELECT CASE WHEN count(1) > 0 THEN cCodCia_in
				       ELSE '001' --Mifarma
				   END INTO v_cCodCia
			FROM PBL_LOCAL
			WHERE COD_GRUPO_CIA = cCodGrupoCia
...
*/				  
		   OPEN curGral FOR
            ...
            FROM PBL_LOCAL
            WHERE COD_GRUPO_CIA = cCodGrupoCia
                  AND COD_CIA = DECODE(v_TpAlm,'1','001','2','007','3','008','X')--v_cCodCia
                  AND TIP_LOCAL = 'A'
                  AND EST_LOCAL = 'A'
                  AND COD_LOCAL = cCodBusqueda_in;
 ...
 --SELECT NVL(COD_TRANSPORTISTA,' ')|| 'Ã' || NVL(RUC_TRANSPORTISTA,' ')|| 'Ã' ||
  ...
  IS
	...
	v_cCodCia CHAR(3);
  v_TpAlm     REL_LOCAL_ALMACEN.ALM_QS%TYPE;
  v_cCodLocal PBL_LOCAL.COD_LOCAL%TYPE;
  BEGIN
    IF(cTipoMaestro_in = '1') THEN --LOCAL
           OPEN curGral FOR
            SELECT NVL(COD_LOCAL,' ') || 'Ã' ||
       ...
       v_TpAlm:=FN_VALIDA_TIPO_ALM(v_cCodLocal);
/*             
		   SELECT CASE WHEN count(1) > 0 THEN cCodCia_in
				       ELSE '001' --Mifarma
				   END INTO v_cCodCia
...
*/				  
  ...
  END;
    ...
    IS
       cur FarmaCursor;
    BEGIN
	   --ERIOS 20.10.2015 Se agrega opciones denegadas por cadena.
       OPEN cur FOR
 ...
 FUNCTION LISTA_FILTROS_CANT(cTipoFiltro_in IN CHAR,
  ...
  BEGIN
    SELECT TRIM(a.DESC_CORTA) INTO v_cIndicador
     FROM pbl_tab_gral a
     WHERE
     a.cod_apl='FARMA_VENTA'
     AND a.cod_tab_gral = 'ID_GESTOR_TX'
	 AND LLAVE_TAB_GRAL = '01'
     AND a.est_tab_gral='A';
  ...
  BEGIN
    SELECT TRIM(a.DESC_CORTA) INTO v_cIndicador
     FROM PTOVENTA.pbl_tab_gral a
     WHERE
     a.cod_apl='FARMA_VENTA'
     AND a.cod_tab_gral = 'IND_IMPR_KARDEX'
	 AND LLAVE_TAB_GRAL = '01'
     AND a.est_tab_gral='A';
  ...
  BEGIN
    SELECT TRIM(a.DESC_CORTA) INTO v_cIndicador
     ...
     WHERE
     a.cod_apl='FARMA_VENTA'
     AND a.cod_tab_gral = 'IND_IMPR_DET_PEDIDO'
	 AND LLAVE_TAB_GRAL = '01'
     AND a.est_tab_gral='A';
...
FUNCTION FN_VALIDA_TIPO_ALM (cCodLocal CHAR)
  RETURN CHAR IS
  v_TpAlm REL_LOCAL_ALMACEN.ALM_QS%TYPE:= 'X';
BEGIN
    select valor into v_TpAlm
    from (
        (select ALM_QS,
                ALM_PH,
                ALM_JORSA
         from REL_LOCAL_ALMACEN
         where cod_local = cCodLocal)
        unpivot(valor
            for COL_NAME in
                (ALM_QS,ALM_PH,ALM_JORSA)
        )
    );
   RETURN v_TpAlm;
EXCEPTION
