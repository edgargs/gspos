...
public class DlgDetalleAnularPedido extends JDialog 
{
            ...
            else
            {
                ...
                if (FarmaVariables.vAceptar)
                {
                    ...
                    if(VariablesCaja.vIndPedidoConRecarVirtual){
                        ...
                        String codigo = "";
                        String descripcion = "";
                        if (util.validarConexionRecarga())
                        {
                            obj = util.respuestaSolicitudRecargaDAO(FarmaVariables.vCodGrupoCia, 
                                                                                    FarmaVariables.vCodLocal, 
                                                                                    VariablesCaja.vNumPedVta_Anul);
                            if (obj != null) {
                                
                                if (obj.getCodigoRespuesta() == null) {
                                    indIntenta = true;
                                    codigo = (obj.getCodigoRespuesta() == null) ? "666" : obj.getCodigoRespuesta();
                                    descripcion = (obj.getDescripcion() == null) ? "La recarga aún no tiene respuesta \n Si no puede anular intente en unos minutos más" : obj.getDescripcion();                                    
                                    FarmaUtility.showMessage(this, descripcion, null);
                                } else {                                    
                                    codigo = (obj.getCodigoRespuesta() == null) ? "666" : obj.getCodigoRespuesta();
                                    descripcion = (obj.getDescripcion() == null) ? "La recarga no se realizo \n Se procedera a continuar con la anulación" : obj.getDescripcion();
            ...
            if(anularPedidofidelizado(FarmaConstants.INDICADOR_S)){
                ...
                if(isConvenioBTLMF && indConvenioBTLMF != null && indConvenioBTLMF.equals("S")){
                    ...
                    if (esCompCredito){
                        ...
                        if (indConexionRac.equals("S")){
                            ...
                            String pedidoAnuladoRac =  DBConvenioBTLMF.anularPedidoRac(FarmaConstants.INDICADOR_N, FarmaConstants.INDICADOR_N);
                ...
                if (!vConvenioCliAnula.equalsIgnoreCase("N")){ 
                    ...
                    if( VariablesCaja.vIndLineaPtoventaMatriz.equals(FarmaConstants.INDICADOR_S)){
                        if (!isNuevoConvenioBTLMF){
                            DBConvenio.actualizaConsumoClienteEnMatriz(val[0], 
                                                                        val[1], 
                                                                        val[2],
                                                                        FarmaConstants.INDICADOR_N,
                                                                        val[4],
                            ...
                            log.debug("vConvenioCliAnula: "+ val[0]+" "+ val[1]+" "+
                                    FarmaUtility.getDecimalNumber(val[2])+" "+val[4]+" "+
                                    val[5]+"  val3: "+val[3]);
                        }else{
                        ...
                        FarmaConnectionRemoto.closeConnection();
                        FarmaUtility.showMessage(this,"No se puede anular el pedido, no existe conexion con Matriz",tblUsuariosCaja);
                        return false;
                    }    
  ...
  /**
     ...
     * @param pNumPedido
     */
    private void evaluaPedidoRecarVirtual(String pNumPedido)
    {
      int cantProdVirtualesPed = 0;
  ...
  */ 
     ...
     * @return
     */
    private boolean cargarLogeoOperador()
    {
