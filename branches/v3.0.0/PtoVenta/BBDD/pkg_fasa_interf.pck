CREATE OR REPLACE PACKAGE PTOVENTA.PKG_FASA_INTERF AS
  ...
  v_cia_MiFarma VARCHAR2(3) := '001';
  v_cia_FASA VARCHAR2(3) := '002';
  v_cia_BTL VARCHAR2(3) := '003';
  G_S_COD_PROV_MF  VARCHAR2(5) := '10029';
...
-- 10.- CURSOR CON GUIA Y LOCAL
  CURSOR curSolicitud( v_Cod_Loc_Sap IN VARCHAR2, v_Num_Orden IN VARCHAR2,dFecha IN VARCHAR2, v_Cia IN VARCHAR2) IS
      SELECT DISTINCT 'NB'   AS CLAS_DOC_COMP,
                      I.COD_PROV AS NUM_CTA_PROV,
                      CASE v_Cia
                      WHEN v_cia_FASA THEN '1176'
                      WHEN v_cia_MiFarma THEN '1171'
                      ELSE ''
                      END
                      AS ORG_COMP,
                      '171'      AS GPO_COMP,
                      CASE v_Cia
                      WHEN v_cia_FASA THEN '1177'
                      WHEN v_cia_MiFarma THEN '1170'
                      ELSE ''
                      END
                      AS SOCIEDAD,
                      I.FEC_AJUSTE AS FEC_DOC_COMP
   ...
   CURSOR curResumen(v_Num_Guia IN varchar2,  v_Cod_Loc_Sap IN VARCHAR2,dFecha IN VARCHAR2,v_CiaR IN VARCHAR2) IS
    ...
    (
         SELECT '2|' || TRIM(RPAD(TA.MAT_SAP, 18, ' ')) || '|' ||
             ...
             TRIM(TA.ALMACEN) A
              ...
              (
                          SELECT I.NUM_DOC,
                                 I.COD_PROD AS MAT_SAP,
                                 I.CANT_SOL AS CANTIDAD,
                                 'UN' AS UNI_MED,
                                 I.IMP_AJUSTE AS PREC_NETO,
                                 RANK() OVER(PARTITION BY I.NUM_DOC ORDER BY I.COD_PROD,ROWNUM)*10 AS NUM_POS,
                                 CASE v_CiaR
                                 WHEN v_cia_FASA THEN '1176'
                                 WHEN v_cia_MiFarma THEN '1172'
                                 ELSE ''
                                 END
                                 AS CENTRO,
                                 '0001' AS ALMACEN
                          FROM INT_AJUSTE I
                                 WHERE I.COD_LOCAL      = v_Cod_Loc_Sap
                                  ...
                                  AND I.FEC_AJUSTE BETWEEN TO_DATE(dFecha,'DD/MM/YYYY')
                                                       AND TO_DATE(dFecha,'DD/MM/YYYY')+1-1/24/60/60
                          ORDER BY I.COD_LOCAL,I.NUM_DOC,I.COD_PROD
              ) TA
 ...
 BEGIN
   ...
   vd_Fec_Ini_Gen_Arch:=SYSDATE;
    FOR listGen in (
                    select TA.*
                     from   (
                                 SELECT DISTINCT i.cod_local, I.NUM_DOC,to_char(i.fec_ajuste ,'dd/mm/yyyy') fec_ajuste
                                  FROM int_ajuste i
                                  WHERE i.cod_grupo_cia = cCodGrupoCia_in
                                    AND i.cod_local     = cCodLocal_in
                                    AND i.tip_ajuste = 'CG'
        ...
        */
        ...
        FOR c_Sol IN curSolicitud(listGen.Cod_Local,listgen.NUM_DOC,listgen.fec_ajuste,V_CIA) LOOP
         ...
         BEGIN
          ...
          UTL_FILE.PUT_LINE(ARCHIVO_TEXTO, v_Cabecera);
           ...
           FOR v_rCurResumen IN curResumen(listGen.NUM_DOC, listGen.Cod_Local,listgen.fec_ajuste,V_CIA) LOOP
            UTL_FILE.PUT_LINE(ARCHIVO_TEXTO, v_rCurResumen.RESUMEN);
          ...
          SET a.dir_ruta_save     = v_gNombreDiretorio,
           ...
           WHERE a.cod_local      = listgen.cod_local
             ...
             AND FEC_AJUSTE=listGen.Fec_Ajuste
             AND A.TIP_AJUSTE     = 'CG';
    ...
    v_vNombreArchivo := 'END_IN_OC'||cCodLocal_in||TO_CHAR(SYSDATE,'yyyyMMdd')||TO_CHAR(SYSDATE,'HH24MISS')||'.TXT';
...
PROCEDURE SP_DEVPRO_PROC_ARCH(cCodGrupoCia_in IN CHAR,cCodLocal_in IN CHAR) IS
 ...
 -- 10.- CURSOR CON GUIA Y LOCAL
  CURSOR curSolicitud( v_Cod_Loc_Sap IN VARCHAR2, v_Num_Orden IN VARCHAR2,v_Cia IN VARCHAR2) IS
   ...
   -- 15.-  Cursor con resumen de datos
   CURSOR curResumen(v_Num_Guia IN varchar2,  v_Cod_Loc_Sap IN VARCHAR2,v_CiaR IN VARCHAR2) IS
    select va.a as resumen
    from
    (
         SELECT '2|' || TRIM(RPAD(TA.MAT_SAP, 18, ' ')) || '|' ||
  ...
  SELECT COD_CIA INTO V_CIA FROM PBL_LOCAL WHERE COD_LOCAL=cCodLocal_in;
   ...
   vd_Fec_Ini_Gen_Arch:=SYSDATE;
    FOR listGen in (
        ...
        */
        -- 60 Begin .- Procesar Cabecera
        FOR c_Sol IN curSolicitud(listGen.Cod_Local,listgen.Num_Doc,V_CIA) LOOP

         BEGIN

         --DBMS_OUTPUT.put_line(listgen.Num_Doc);
            ...
            select count(1) into v_num from int_movimiento where nombre_archivo=  v_vNombreArchivo;
          ...
          UTL_FILE.PUT_LINE(ARCHIVO_TEXTO, v_Cabecera);
           ...
           -- 70 Begin.- Procesar Detalle
           FOR v_rCurResumen IN curResumen(listGen.Num_Doc, listGen.Cod_Local,V_CIA) LOOP
            UTL_FILE.PUT_LINE(ARCHIVO_TEXTO, v_rCurResumen.RESUMEN);
            --dbms_output.put_line('In File: '||v_rCurResumen.RESUMEN); -- debug
           END LOOP;
          -- 70 End.- Procesar Detalle
          ...
          if utl_file.is_open(ARCHIVO_TEXTO) = true then
           ...
           begin
                 ...
                 exception
                  when  others then
                    null;
                    --dbms_output.PUT_LINE('ERROR, AL GENERAR ARCHIVO OC Fasa - COMP. DIR. GENERAL INT,Fecha - Local:'||' - '||
             --SQLERRM);
           end;
             --dbms_output.put_line('cierra');
           end if;
...
AS
    ...
    SELECT K.COD_GRUPO_CIA,
            ...
            DECODE(M.TIP_MOV, 'CG',(SELECT VAL_PREC_UNIT
                                    FROM LGT_NOTA_ES_DET
                                    WHERE COD_GRUPO_CIA   = K.COD_GRUPO_CIA
                                          AND COD_LOCAL   = K.COD_LOCAL
                                          AND NUM_NOTA_ES = K.NUM_TIP_DOC
           ...
           /* CASE
            ...
            DECODE(M.TIP_MOV,'CG',/*(SELECT c.num_nota_es                  --- Numero Interno para Comp. Merc. Gral
                                    ...
                                    WHERE COD_GRUPO_CIA = K.COD_GRUPO_CIA
                                          AND COD_LOCAL = K.COD_LOCAL
                                          AND NUM_NOTA_ES = K.NUM_TIP_DOC)*/
                                 (SELECT DISTINCT COD_OC
                                  FROM PTOVENTA.LGT_OC_CAB_RECEP
                                  where (num_doc_cab,COD_PROV) IN (
                                        select num_doc,COD_ORIGEN_NOTA_ES
                                        from ptoventa.lgt_nota_es_cab
                                        where num_nota_es=K.NUM_TIP_DOC))
                                          ,
    ...
    AND M.TIP_MOV         = cTipMov_in
    AND K.CANT_MOV_PROD   <> 0
    AND ( K.DESC_GLOSA_AJUSTE IS NULL
    OR
    (K.DESC_GLOSA_AJUSTE IS NOT NULL AND UPPER(TRIM(K.DESC_GLOSA_AJUSTE)) <>UPPER(TRIM('Regularizacion de Mercaderia Directa')))
    )
    AND (K.TIP_DOC_KARDEX,K.NUM_TIP_DOC) NOT IN (SELECT '02',NUM_NOTA_ES
                                                  FROM LGT_NOTA_ES_CAB
                                                  WHERE COD_GRUPO_CIA          = cCodGrupoCia_in
...
for listGen in (
   ...
   --NULL;
    ...
    v_vNombreArchivo := 'END_IN_DL'||cCodLocal_in|| TO_CHAR(SYSDATE,'yyyyMMdd')||TO_CHAR(SYSDATE,'HH24MISS')||'.TXT';
           ...
           */
            ...
            WHEN M.TIP_MOV='RD' THEN
                     ...
                     WHEN (
                          ...
                          DECODE(TRIM(K.COD_PROD),'510559','120767','10035')
                     ELSE
                      --- MFA
                      (SELECT COD_ORIGEN_NOTA_ES
                                  FROM LGT_NOTA_ES_CAB
...
-- 2009-03-18 JOLIVA - Se agregan código, indicador de fraccion y factor de conversión del producto destino
  ...
  BEGIN
    ...
    IF v_nCant > 0 THEN
      ...
      IF V_CIA='003' THEN
         v_vNombreArchivo := 'MC'||'1174'||cCodLocal_in||TO_CHAR(SYSDATE,'yyyyMMdd')||TO_CHAR(SYSDATE,'HH24MISS')||'.TXT';
      END IF;
...
-------------------------------------------------------------------------------------------------------------------------*/
 PROCEDURE SP_MOV2_INT_FASA(cCodGrupoCia_in IN CHAR, cCodLocal_in IN CHAR, nDiasAtras_in IN INTEGER DEFAULT 2)
  ...
  -- 10.- Cursor Tipo
    ...
    -- 20.- Cursor Detalle
    CURSOR curMov2(cTipMov_in IN CHAR,cCia IN CHAR) IS
    SELECT K.COD_GRUPO_CIA,
            K.COD_LOCAL,
            K.COD_PROD,
            M.TIP_MOV AS TIP_AJUSTE,--4
            ...
            DECODE(M.TIP_MOV,'RD',(SELECT COD_ORIGEN_NOTA_ES  --AS COD_PROVEEDOR
                                  ...
                                  WHERE COD_GRUPO_CIA = K.COD_GRUPO_CIA
                                        AND COD_LOCAL = K.COD_LOCAL
                                        AND NUM_NOTA_ES = K.NUM_TIP_DOC) ,DECODE(cCia,v_cia_BTL,G_S_COD_PROV_MF,'')) AS COD_PROV,--6
            DECODE(M.TIP_MOV,'AD',ABS(K.CANT_MOV_PROD/K.VAL_FRACC_PROD),
                            (ABS((K.CANT_MOV_PROD*INT_GET_CANT_PRESENT(cCodGrupoCia_in,K.COD_PROD,DECODE(K.VAL_FRACC_PROD,1,' ','X'),K.VAL_FRACC_PROD))/K.VAL_FRACC_PROD))) AS CANT_SOL,--7
            --ABS(K.CANT_MOV_PROD) AS CANT_SOL,
            DECODE(M.TIP_MOV,'CC',(SELECT VAL_PREC_TOTAL
...
-- 2009-01-15 Se agrega código, Indicador de Fracción y el factor de conversión del producto cambio de código
  ...
  BEGIN
      ...
      LOOP
        ...
        FOR v_rCurMov2 IN curMov2(v_rCurTipMov.TIP_MOV,V_CIA)
        LOOP
...
PROCEDURE SP_MOV2_PROC_ARCH(cCodGrupoCia_in IN CHAR, cCodLocal_in IN CHAR)
  AS
     ...
     v_nCuenta       NUMBER      := 0;
     c_vcNUM_SOC     VARCHAR2(4) := '1177';
     c_vcNUM_SOCBTL  VARCHAR2(4) := '1174';
...
-- 2009-03-18 JOLIVA - Se agregan código, indicador de fraccion y factor de conversión del producto destino
  ...
  BEGIN
   ...
   select cod_cia into v_cia from pbl_local where cod_local=cCodLocal_in;
    ...
    IF v_nCant > 0 THEN
      IF v_cia=v_cia_BTL THEN
         v_vNombreArchivo := 'MC'||c_vcNUM_SOCBTL||cCodLocal_in||TO_CHAR(SYSDATE,'yyyyMMdd')||TO_CHAR(SYSDATE,'HH24MISS')||'.TXT';
      END IF;

      IF v_cia=v_cia_FASA THEN
         v_vNombreArchivo := 'MC'||c_vcNUM_SOC||cCodLocal_in||TO_CHAR(SYSDATE,'yyyyMMdd')||TO_CHAR(SYSDATE,'HH24MISS')||'.TXT';
      END IF;
