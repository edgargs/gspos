...
public class DlgListaMaeLote extends JDialog {
    ...
    private void initTable() {
        ...
        if (VariablesPtoVenta.vTipListaMaestros.equals(ConstantsPtoVenta.TIP_LIST_MAESTRO_ORD)) {
            ...
            tableModelListaMaestro =new FarmaTableModel(ConstantsPtoVenta.columnsListaMaestrosTransf, 
                                                        UtilityPtoVenta.obtenerDefaultValuesTabla(ConstantsPtoVenta.columnsListaMaestrosTransf.length),
                                                        0);
            ...
            if (pExisteLote && pExisteFecha) {
                ...
                if (pExisteLote && !pExisteFecha) {
                    ...
                    FarmaUtility.showMessage(this, "VERIFIQUE LA FECHA DE VENCIMIENTO.\n" +
                            "No coincide con el LOTE INGRESADO", txtFechVenc);
                } else {
                    if (!pExisteLote && !pExisteFecha) {
                        //KMONCADA 04.08.2016 [PROYECTO M] VALIDARA SI PERMITE REGISTRO DE AJUSTE AUTOMATICO PARA EL KARDEX
                        if(pLocalMayorista && !(new UtilityDespacho()).isPermiteAjusteAutomaticoDespacho()){
                            FarmaUtility.showMessage(this, "Local Mayorista:\nLote no registrado en el Sistema, Verifique Lote ingresado.\n"+
                                                           "Si lote es correcto, coordine con Jefe de Local para ingresarlo\n mediante un ajuste de kardex.", this);
                            return;
                        }
                        //NO EXISTE EL LOTE OBLIGA A CREAR
                        DlgIngresarLote objx =
                            new DlgIngresarLote(myParentFrame, "", true, txtDescripcion.getText().trim(),
                                                txtFechVenc.getText().trim());
                        objx.codprodxx = codprodx;
                        objx.tableModelListaMaestroX = tableModelListaMaestro;
                        objx.setLocalMayorista(pLocalMayorista);
                        objx.setStockLocalLote(pStockLocalLote);
                        objx.setVisible(true);
                        if (FarmaVariables.vAceptar) {
                            VariablesPtoVenta.vCodMaestro = VariablesInventario.nroLoteX;
                            VariablesPtoVenta.vDescMaestro = VariablesInventario.fechaVencLoteX;
    ...
    public void setStockLocalLote(int pStockLocalLote) {
        this.pStockLocalLote = pStockLocalLote;
    }
    
    public void setLocalMayorista(boolean pLocalMayorista){
