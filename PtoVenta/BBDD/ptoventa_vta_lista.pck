...
CREATE OR REPLACE PACKAGE BODY PTOVENTA."PTOVENTA_VTA_LISTA" is
  ...
  BEGIN
    ...
    EXCEPTION 
      WHEN NO_DATA_FOUND THEN
        vCodTipoLocalVenta := NULL;
    END;
    
    IF vCodTipoLocalVenta IS NULL THEN
      SELECT A.COD_TIP_LOCAL_VENTA
      INTO vCodTipoLocalVenta
      FROM PBL_LOCAL A
      WHERE A.COD_GRUPO_CIA = cCodGrupoCia_in
  ...
  -- 07.01.2015 ERIOS Garantizado por local
    OPEN curVta FOR
	SELECT 
     distinct(PROD.COD_PROD) || 'Ã' ||
     CASE 
       WHEN LOC.COD_TIP_LOCAL_VENTA = PKG_CONSTANTES.LOCAL_TIPO_VTA_M AND 1=2 THEN
           
           PROD.DESC_PROD || 'Ã' ||
           -- KMONCADA 11.01.2015 LOCALES TIPO M, MOSTRARA SIEMPRE EN ENTEROS
           PROD.DESC_UNID_PRESENT || 'Ã' ||
           -- DECODE(PROD_LOCAL.IND_PROD_FRACCIONADO,'N',PROD.DESC_UNID_PRESENT,PROD_LOCAL.UNID_VTA) || 'Ã' ||
           LAB.NOM_LAB || 'Ã' ||
           -- KMONCADA 11.01.2015 LOCALES TIPO M, MOSTRARA SIEMPRE EN ENTEROS
           TRUNC(PTOVENTA_VTA.F_GET_STOCK_PRODUCTO(cCodGrupoCia_in, cCodLocal_in, PROD.COD_PROD, 'S')) || 'Ã' ||
--           TRUNC((PROD_LOCAL.STK_FISICO/PROD_LOCAL.VAL_FRAC_LOCAL)) || 'Ã' ||
           --(PROD_LOCAL.STK_FISICO) || 'Ã' ||
           TO_CHAR(ptoventa_vta.VTA_F_CHAR_PREC_REDONDEADO(PROD_LOCAL.VAL_PREC_VTA*PROD_LOCAL.VAL_FRAC_LOCAL),'999,990.000') || 'Ã' || --JCHAVEZ 29102009  precio redondeado
           ...
           DECODE(NVL(PROD_LOCAL.IND_ZAN,'NG'),'NG',' ',PROD_LOCAL.IND_ZAN) || 'Ã' ||--ASOSA - 22/05/2015 - NG - NG(NO GARANTIZADO)
           PROD_LOCAL.IND_PROD_CONG || 'Ã' ||
           -- KMONCADA 11.01.2015 LOCALES TIPO M, MOSTRARA SIEMPRE EN ENTEROS
           1 || 'Ã' ||
           -- PROD_LOCAL.VAL_FRAC_LOCAL || 'Ã' ||
           TO_CHAR((PROD_LOCAL.VAL_PREC_LISTA*PROD_LOCAL.VAL_FRAC_LOCAL),'999,990.000') || 'Ã' ||
...
--     distinct(PROD.COD_PROD) || 'Ã' ||
		...
		AND	   PROD.EST_PROD = 'A'
    AND    (PROD.IND_TIPO_CONSUMO = 'N' OR PROD.IND_TIPO_CONSUMO = 'F')       --ASOSA - 01/10/2014 - PANHD
    ...
    AND    LOC.COD_GRUPO_CIA = PROD_LOCAL.COD_GRUPO_CIA
    AND    LOC.COD_LOCAL = PROD_LOCAL.COD_LOCAL
    -- KMONCADA 26.04.2016 ARCANGEL - CENTRO MEDICO
    AND   PROD_LOCAL.COD_PROD IN (
                                    SELECT COD_PROD 
                                    FROM REL_TIP_LOCAL_VENTA_PROD 
                                    WHERE COD_TIP_LOCAL_VENTA = vCodTipoLocalVenta
                                  )
    ;
...
-- 07.01.2015 ERIOS Garantizado por local
 ...
 RETURN FarmaCursor
  IS
    curVta FarmaCursor;
    isLocalM CHAR(1) := 'N';
    vPrecioProd LGT_PROD_LOCAL.VAL_PREC_VTA%TYPE;
  BEGIN
    isLocalM := FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in => cCodGrupoCia_in, 
    ...
    -- KMONCADA 11.01.2015 LOCALES TIPO M, MOSTRARA SIEMPRE EN ENTEROS
    CASE 
      WHEN isLocalM = 'S' THEN
        PROD.DESC_UNID_PRESENT
      ELSE
        DECODE(PROD_LOCAL.IND_PROD_FRACCIONADO,'N',PROD.DESC_UNID_PRESENT,PROD_LOCAL.UNID_VTA) 
    END || 'Ã' ||
		LAB.NOM_LAB || 'Ã' ||
    -- KMONCADA 11.01.2015 LOGICA DE LOCAL M
    CASE 
      WHEN isLocalM = 'S' THEN
        PTOVENTA_VTA.F_GET_STOCK_PRODUCTO(cCodGrupoCia_in, cCodLocal_in, PROD.COD_PROD, 'S')
    ...
    CASE 
      WHEN isLocalM = 'S' THEN
        TO_CHAR( ptoventa_vta.VTA_F_CHAR_PREC_REDONDEADO(PTOVENTA_VTA_MAYORISTA.VTA_OBTIENE_VAL_PREC_VTA(PROD_LOCAL.COD_GRUPO_CIA,
                                                                                                         PROD_LOCAL.COD_LOCAL,
                                                                                                         PROD_LOCAL.COD_PROD,
                                                                                                         cTipoCliente_in) * PROD_LOCAL.VAL_FRAC_LOCAL),'999,990.000')
      ELSE 
        TO_CHAR( ptoventa_vta.VTA_F_CHAR_PREC_REDONDEADO(PROD_LOCAL.VAL_PREC_VTA),'999,990.000') 
    END || 'Ã' ||
   
		NVL(PROD_LOCAL.IND_ZAN,' ') || 'Ã' ||
	  PROD_LOCAL.IND_PROD_CONG || 'Ã' ||
    
	  DECODE(isLocalM, 'S', PKG_CONSTANTES.VAL_FRAC_ENTERO, PROD_LOCAL.VAL_FRAC_LOCAL) || 'Ã' ||
    
    CASE 
      WHEN isLocalM = 'S' THEN
        TO_CHAR( ptoventa_vta.VTA_F_CHAR_PREC_REDONDEADO(PROD_LOCAL.VAL_PREC_LISTA * PROD_LOCAL.VAL_FRAC_LOCAL),'999,990.000')
      ELSE 
        TO_CHAR( ptoventa_vta.VTA_F_CHAR_PREC_REDONDEADO(PROD_LOCAL.VAL_PREC_LISTA),'999,990.000') 
    END || 'Ã' ||
		TO_CHAR(IGV.PORC_IGV,'990.00') || 'Ã' ||
    PROD.IND_PROD_FARMA || 'Ã' ||
    DECODE(NVL(PR_VRT.COD_PROD,'N'),'N','N','S') || 'Ã' ||
    ...
    IND_ORIGEN_SUST
      ...
      WHERE  PROD_LOCAL.COD_GRUPO_CIA = cCodGrupoCia_in
          AND	   PROD_LOCAL.COD_LOCAL = cCodLocal_in
          AND PROD_LOCAL.COD_GRUPO_CIA = LOC.COD_GRUPO_CIA
          AND PROD_LOCAL.COD_LOCAL = LOC.COD_LOCAL
    ...
    OPEN curVta FOR
      ...
      CASE
        WHEN LOCAL.COD_TIP_LOCAL_VENTA = PKG_CONSTANTES.LOCAL_TIPO_VTA_M THEN
           ...
           TO_CHAR((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL - ((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL) * V3.PORC_DCTO_OFERTA/100)),'999,990.000') || 'Ã' ||
           PROD_LOCAL.IND_PROD_CONG || 'Ã' ||
           TO_CHAR((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL) - ( (vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL) * V3.PORC_DCTO_OFERTA/100),'999,990.000')|| 'Ã' ||
           ...
           TO_CHAR((vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL),'999,990.000')|| 'Ã' ||
           TO_CHAR( (vValPrecioVta * PROD_LOCAL.VAL_FRAC_LOCAL) * V3.PORC_DCTO_OFERTA/100,'999,990.000')
        ELSE
         (PROD_LOCAL.STK_FISICO) || 'Ã' ||
			   PROD.DESC_UNID_PRESENT || 'Ã' ||
			   PROD_LOCAL.VAL_FRAC_LOCAL || 'Ã' ||
