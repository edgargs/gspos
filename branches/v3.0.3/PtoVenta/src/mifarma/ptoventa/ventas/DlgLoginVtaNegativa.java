...
public class DlgLoginVtaNegativa extends JDialog {
    ...
    public DlgLoginVtaNegativa(Frame parent, String title, boolean modal, String pCodProd, String pDescripcionProd,
                               int pCantVeces) {
    ...
    private void txtUsuario_keyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_ENTER) {
            ...
            if (afirma) {
                txtUsuario.setText(FarmaUtility.caracterIzquierda(txtUsuario.getText().trim(), 3, "0"));
            } else
...
*/
    ...
    private void btnAceptar_actionPerformed(ActionEvent e) {
        ...
        if ((usuario.trim().length() == 0) || (clave.trim().length() == 0)) {
            FarmaUtility.showMessage(this, "Datos invalidos. Verifique !!!", null);
            clearData();
            return;
        }
        ...
        if (!vSecurityLogin.getLoginStatus().equalsIgnoreCase(FarmaConstants.LOGIN_USUARIO_OK)) {
            if (vSecurityLogin.getLoginStatus().equalsIgnoreCase(FarmaConstants.LOGIN_USUARIO_INACTIVO)) {
                FarmaUtility.showMessage(this, "El usuario se encuentra Inactivo !!!", null);
            } else if (vSecurityLogin.getLoginStatus().equalsIgnoreCase(FarmaConstants.LOGIN_NO_REGISTRADO_LOCAL)) {
                FarmaUtility.showMessage(this, "El usuario no se encuentra registrado en el Local !!!", null);
            } else if (vSecurityLogin.getLoginStatus().equalsIgnoreCase(FarmaConstants.LOGIN_CLAVE_ERRADA)) {
                FarmaUtility.showMessage(this, "La clave ingresada no es correcta !!!", null);
            } else if (vSecurityLogin.getLoginStatus().equalsIgnoreCase(FarmaConstants.LOGIN_USUARIO_NO_EXISTE)) {
                FarmaUtility.showMessage(this, "El usuario ingresado no se encuentra registrado !!!", null);
            } else if (vSecurityLogin.getLoginStatus().equalsIgnoreCase(FarmaConstants.ERROR_CONEXION_BD)) {
                FarmaUtility.showMessage(this, "Error de conexión a la Base de Datos !!!", null);
            }
            clearData();
            return;
        }
    ...
    private void this_windowOpened(WindowEvent e) {
        ...
        FarmaUtility.moveFocus(txtUsuario);
        if (FarmaVariables.vEconoFar_Matriz && FarmaVariables.vClaveMatriz.trim().length() > 0) {
    ...
    private String getMsgRol(String pCodRol) throws SQLException {
        ...
        return FarmaDBUtility.executeSQLStoredProcedureStr("Farma_Gral.GET_MSG_ROL_LOGIN(?,?)", vParameters);
    }
    ...
    private String getMsgRoleEspecial(String pCodRol) throws SQLException {
        ...
        return FarmaDBUtility.executeSQLStoredProcedureStr("PTOVENTA_MENSAJES.F_VAR2_GET_MENSAJE(?,?)", vParameters);
    }
