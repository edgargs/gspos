            ...
            val_vta_item_e) =
                             ...
                             -- LTAVARA 26.12.2014 PROMOCIONES
                              SELECT DECODE(D.COD_TIP_AFEC_IGV_E, '31', 
                                            ...
                                            END)
                                            ,GG.UNIT_NUEVO),GG.AHORRO_NUEVO,GG.TOTAL_NUEVO
                                  FROM   (
    ...
    ELSE
      ...
      SELECT 
        ...
        RPAD(SUBSTR(DECODE(DET.COD_TIP_AFEC_IGV_E,'31',
                    ...
                    ,0,34)||'',34,' ') ||
        -- RPAD(SUBSTR(prod.desc_unid_present,0,20)||'',22,' ') || entero
        RPAD(SUBSTR(
        ...
        --LPAD(TRIM(TO_CHAR(NVL(DET.VAL_VTA_UNIT_ITEM_E,0) ,'999,999,990.000')) ,9)||
       ...
       \* INSERT INTO TMP_DOCUMENTO_ELECTRONICOS 
          ...
          ORDER BY DET.SEC_PED_VTA_DET ASC;*\
           ...
           SELECT 
            ...
            RPAD(NVL(DET.COD_PROD,' '),8)  ||
            --27/10/2014 CUANDO ES UNA PROMOCION DEBE IR ADELANTE LA PALABRA PROMOCION
            RPAD(SUBSTR(DECODE(DET.COD_TIP_AFEC_IGV_E,'31','PROM-'||PROD.DESC_PROD,NVL(PROD.DESC_PROD,' ')),0,34)||'',34,' ') ||
            -- RPAD(SUBSTR(prod.desc_unid_present,0,20)||'',22,' ') || entero
            RPAD(SUBSTR(DECODE(DET.VAL_FRAC,1,prod.desc_unid_present,DET.UNID_VTA),0,20)||'',22,' ') ||
            '       '||
            rpad(''||trim(SUBSTR(la.nom_lab,0,20))||'',19,' ') ||
            --Cantidad vendida en fracciones
            LPAD(DECODE(vTip_Ped_vta, TIPO_VTA_INSTITUCIONAL,--Si el Tipo de venta es Mayorista
               ...
               DECODE(MOD(ABS(DET.CANT_ATENDIDA),ABS(DET.VAL_FRAC)),0,
                              (ABS(DET.CANT_ATENDIDA)/ABS(DET.VAL_FRAC))||'',--Caso contrario las imprime en fracciones
                              ABS(DET.CANT_ATENDIDA*prod.val_max_frac/det.val_frac)||
                              DECODE(ABS(DET.VAL_FRAC),1,'','/'||ABS(PROD.Val_Max_Frac))))*\
               ,10) || 
            -- Valor de venta unitario por ítem(no incluye igv)
            LPAD(NVL(TRIM(TO_CHAR(NVL(ABS(DET.VAL_VTA_UNIT_ITEM_E),0),'999,999,990.000')),0),9) ||
            -- Descuento del Item(no incluye igv)
            LPAD(NVL(TRIM(TO_CHAR(NVL(ABS(DET.VAL_TOTAL_DESC_ITEM_E),0),'999,999,990.000')),0),8) ||
            LPAD(NVL(TRIM(TO_CHAR(NVL(ABS(DET.VAL_VTA_ITEM_E),0),'999,999,990.00')),0),11)
            ,'1','D','N','N'
          FROM  VTA_PEDIDO_VTA_DET DET, 
              LGT_PROD PROD,
          ...
          AND   DET.NUM_PED_VTA   = vNumPedVta_in
          AND  (CASE
              WHEN vTipoClienteConvenio = '1' AND vTipoDocumento != COD_TIP_COMP_NOTA_CRED THEN DET.SEC_COMP_PAGO_BENEF
              WHEN vTipoClienteConvenio = '2' AND vTipoDocumento != COD_TIP_COMP_NOTA_CRED THEN DET.SEC_COMP_PAGO_EMPRE
              ELSE DET.SEC_COMP_PAGO
              END) = vSecCompPago_in
          AND   DET.COD_GRUPO_CIA = PROD.COD_GRUPO_CIA
          AND   DET.COD_PROD     = PROD.COD_PROD
    ...
    /*** MONTO EN LETRAS ***/
      ...
      IF vTipoDocumento != COD_TIP_COMP_NOTA_CRED THEN
        IF vMontoAhorro != '0.00' THEN
          INSERT INTO TMP_DOCUMENTO_ELECTRONICOS VALUES ('EN LA COMPRA USTED AHORRO: S/.'||vMontoAhorro,'9','I','N','N');
        END IF;
      END IF;
