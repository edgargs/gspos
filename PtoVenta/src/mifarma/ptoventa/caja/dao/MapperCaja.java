...
public interface MapperCaja {
    ...
    @Select(value =
            "{call #{listado, mode=OUT, jdbcType=CURSOR, resultMap=impresion} := " + "PTOVENTA.PTOVENTA_DEVOL_DIF.F_GET_VOUCHER(" +
            "#{cCodGrupoCia_in}," + "#{cCodCia_in}," + "#{cCodLocal_in}," + "#{cNroPedido_in}"+ ")}")
    @Options(statementType = StatementType.CALLABLE)
    public void getVoucherDevolDif(HashMap<String, Object> object);
