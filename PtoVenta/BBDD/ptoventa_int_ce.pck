...
--           NVL(TRAB.COD_TRAB_RRHH,' ') ASIG_1,
  ...
  AS
    ...
    SELECT CCD.COD_GRUPO_CIA COD_GRUPO_CIA,
           ...
           ' ' IMPUESTO,
           ' ' ASIG_1,
           C_C_RT || '-' || CCD.COD_LOCAL || '-' || TO_CHAR(MOV.FEC_DIA_VTA,'DDMMYYYY') || '/' ||
           CU.DESC_CORTA_CUADRATURA TEX_DET_1,
           CU.DESC_CLAVE_CUENTA_2 CLA_CUE_2,
           CU.DESC_CUENTA_2 CUE_2,
           ' ' CEN_COSTO,
           ...
           C_C_RT || '-' || CCD.COD_LOCAL || '-' || TO_CHAR(MOV.FEC_DIA_VTA,'DDMMYYYY') || '/' ||
           CU.DESC_CORTA_CUADRATURA TEX_DET_2
    FROM   CE_CUADRATURA_CAJA CCD,
           CE_CUADRATURA CU,
           PBL_LOCAL LOC,
           CE_MOV_CAJA MOV
    WHERE  CCD.COD_GRUPO_CIA = cCodGrupoCia_in
    AND    CCD.EST_CUADRATURA_CAJA = 'A' -- RHERRERA: FILTRAR LAS CUADRATURAS PARA ESTADO ACTIVO
    AND    CCD.COD_LOCAL = cCodLocal_in
    AND    MOV.FEC_DIA_VTA = vFecCierreDia_in
    AND    CCD.COD_CUADRATURA = C_C_DELIVERY_PERDIDO
    AND    CCD.COD_GRUPO_CIA = CU.COD_GRUPO_CIA
    ...
    AND    CCD.COD_GRUPO_CIA = LOC.COD_GRUPO_CIA
    AND    CCD.COD_LOCAL = LOC.COD_LOCAL
    AND    CCD.COD_GRUPO_CIA = MOV.COD_GRUPO_CIA
    AND    CCD.COD_LOCAL     = MOV.COD_LOCAL
    AND    CCD.SEC_MOV_CAJA  = MOV.SEC_MOV_CAJA;
  ...
  BEGIN
    ...
    FOR movCuadratura_rec2 IN movCuadraturaNuevo
     LOOP
         v_cSecIntCe:= CE_GET_SECUENCIAL_INT(cCodGrupoCia_in, cCodLocal_in, vFecCierreDia_in);

         INT_GRABA_INTERFACE_CE(movCuadratura_rec2.COD_GRUPO_CIA,
                                movCuadratura_rec2.COD_LOCAL,
                                movCuadratura_rec2.FECHA,
                                movCuadratura_rec2.TIPO_OPERACION,
                                v_cSecIntCe,
                                movCuadratura_rec2.CLASE_DOC,
                                movCuadratura_rec2.FECHA_DOC,
                                movCuadratura_rec2.FECHA_CON,
                                movCuadratura_rec2.REFERENCIA,
                                movCuadratura_rec2.TEX_CAB,
                                movCuadratura_rec2.MONEDA,
                                movCuadratura_rec2.CLA_CUE_1,
                                movCuadratura_rec2.CUE_1,
                                movCuadratura_rec2.IMPUESTO,
                                movCuadratura_rec2.IMPORTE,
                                movCuadratura_rec2.ASIG_1,
                                movCuadratura_rec2.TEX_DET_1,
                                movCuadratura_rec2.CLA_CUE_2,
                                movCuadratura_rec2.CUE_2,
                                movCuadratura_rec2.CEN_COSTO,
                                movCuadratura_rec2.IND_IMP,
                                movCuadratura_rec2.TEX_DET_2,
                                C_C_USU_CREA_INT_CE,
                                movCuadratura_rec2.CME);
