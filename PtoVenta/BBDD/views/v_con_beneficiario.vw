CREATE OR REPLACE FORCE VIEW PTOVENTA.V_CON_BENEFICIARIO AS
SELECT
...
FROM ptoventa.et_listado2
    where  (COD_CONVENIO,COD_CLIENTE)not in (select COD_CONVENIO,COD_CLIENTE from ptoventa.et_tmp_listado2)
    union
    select
