--------------------------------------------------------
--  DDL for Package Body PTOVENTA_CARGA_INICIAL
--------------------------------------------------------
...
BEGIN
 ...
 --Limpiando las tablas
  ...
  EXECUTE IMMEDIATE 'TRUNCATE TABLE  CE_HIS_CIERRE_DIA_VENTA DROP STORAGE';
  EXECUTE IMMEDIATE 'TRUNCATE TABLE  CE_HIS_MOV_CAJA DROP STORAGE';
  ...
  EXECUTE IMMEDIATE 'TRUNCATE TABLE  INT_RECEP_PROD_QS_T DROP STORAGE';
  ...
  --EXECUTE IMMEDIATE 'TRUNCATE TABLE  LGT_PROD_LOCAL_REP_20080902 DROP STORAGE';
...
BEGIN
  ...
  IF (cTipoRevertir_in IS NULL) THEN --solo en carga inicial
    ...
    UPDATE VTA_CAMP_X_LOCAL A SET A.COD_LOCAL = cCodLocalNuevo_in;
    ...
    DELETE PBL_ROL_USU A WHERE A.COD_LOCAL NOT IN  (cCodLocalOrigen_in);
    UPDATE PBL_ROL_USU A SET A.COD_LOCAL=cCodLocalNuevo_in;
  ...
  DELETE PBL_ROL_USU A WHERE A.SEC_USU_LOCAL NOT IN ('000') AND A.SEC_USU_LOCAL<'900';
...
--  UPDATE LGT_PROD_LOCAL A SET A.COD_LOCAL=cCodLocalNuevo_in, STK_FISICO = 0  , VAL_FRAC_LOCAL = 1;
  ...
  UPDATE VTA_NUMERA_CUPON A  SET A.COD_LOCAL = cCodLocalNuevo_in, A.SEC_CUPON=1;
...
AS
  ...
  from all_constraints    a
  where a.constraint_type='R' and a.owner='PTOVENTA'
  and a.constraint_name not in ('FK_CA_CLI_CAMPANA','FK_CA_PED_ACUM_PROD_LOCAL','FK_CA_CANJ_CLI_CAMP','FK_CA_CANJ_PED_ACUM','FK_CA_PED_OR_CAJ','FK_VTA_ENCARTE_LGT_PROD');
  x     cr1%rowtype;
BEGIN
