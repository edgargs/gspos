...
public class DlgIngresarLote extends JDialog {
    ...
    private boolean pLocalMayorista= false;
    private boolean indTomaInv=false;
    
    private int pStockLocalLote = -1;
    ...
    public DlgIngresarLote(Frame parent, String title, boolean modal,String lote , boolean pLocalMayorista) {
        super(parent, title, modal);
        
        this.pLocalMayorista=pLocalMayorista;
        indTomaInv= true;
        myParentFrame = parent;
        try {
            jbInit();
            this.setTitle(title);
            txtnrolote.setText(lote); 
        } catch (Exception e) {
            log.error("", e);
        }
    }
    ...
    private void chkKeyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
            ...
            if(indTomaInv){ // cuando es llamado por TOMA DE INVENTARIO
                    pLoteOld=txtnrolote.getText().trim();
                    pFechaVencold=txtfechvenc.getText().trim();
                }
            
            if (isConfirmacionDatos(pLoteOld, pFechaVencold)){
                if(pLocalMayorista){
                    
                    if(pStockLocalLote>0){
                        if(JConfirmDialog.rptaConfirmDialogDefaultNo(this, "Se va agregar unidades al stock. ¿Está seguro?")){
                            agregarStockLocalEntrega();
                        }
                    }else{
                        registrarLoteMayorista();
                    }
                }else{
                    aceptarFechaVenc();
                }
    ...
    private void registrarLoteMayorista() {
        if (validaCampoLote()) {
                if (validarFecha()) {
                    if(JConfirmDialog.rptaConfirmDialog(this, "Desea registrar el lote?")){
                    try {
                        DBInventario.insertarLoteMayorista(codprodxx.trim(), txtnrolote.getText().trim(),
                                                  txtfechvenc.getText().trim());
                        FarmaUtility.aceptarTransaccion();
                        FarmaUtility.showMessage(this, "Lote ingresado exitosamente", null);
                        cerrarVentana(true);
                        ...
                        FarmaUtility.liberarTransaccion();
                        FarmaUtility.showMessage(this,
                                                 "Error en insertar lote: " + e.getMessage(),
                                                 null);
                        log.error("", e);
                    }
                }
    ...
    private boolean validarFecha() {
        ...
        if (txtfechvenc.getText().trim().equals("")){
                txtfechvenc.setText("");
                FarmaUtility.showMessage(this, "Ingresar fecha de vencimiento", txtfechvenc);
                flag = false;
            }else if(!FarmaUtility.validaFecha(txtfechvenc.getText(), "dd/MM/yyyy")) {
            flag = false;
            txtfechvenc.setText("");
    ...
    /**
     * Se registra el nuevo stock.
     * @author ERIOS
     * @since 29.01.2016
     */
    private void agregarStockLocalEntrega() {
        if (validaCampoLote() && validarFecha()) {
            try {
                DBInventario.agregaStockLocalEntrega(codprodxx.trim(),pStockLocalLote, txtnrolote.getText().trim(),
                                          txtfechvenc.getText().trim());
                VariablesInventario.fechaVencLoteX = txtfechvenc.getText().trim();
                VariablesInventario.nroLoteX = txtnrolote.getText().trim();
                FarmaUtility.aceptarTransaccion();
                ...
                log.error("", e);
                FarmaUtility.showMessage(this,
                                         "Error en insertar lote: \n" + e.getMessage(),
                                         null);
            }
        }
    }

    void setStockLocalLote(int pStockLocalLote) {
        this.pStockLocalLote = pStockLocalLote;
    }
    
    void setLocalMayorista(boolean pLocalMayorista){
        this.pLocalMayorista = pLocalMayorista;
    }
}
