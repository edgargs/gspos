...
public class DlgMantClienteJuridico extends JDialog {
    ...
    private void guardaValoresCliente() {
        ...
        myArray.add(beanCliente.getCodCliente());
        myArray.add(beanCliente.getNumDocumento());
        myArray.add(beanCliente.getRazonSocial());
        myArray.add(beanCliente.getDireccion());
        myArray.add(beanCliente);
    ...
    private void guardarDatosBean(){
        ...
        beanCliente.setCodCliente(txtCodigo.getText().toUpperCase());
        beanCliente.setNumDocumento(txtRuc.getText().trim().toUpperCase());
        beanCliente.setRazonSocial(txtRazonSocial.getText().trim().toUpperCase());
        beanCliente.setDireccion(txtDireccion.getText().trim().toUpperCase());
    ...
    private boolean validaDatosClientes() {
        ...
        String nombreCliente = txtRazonSocial.getText();
        String direccionCliente = txtDireccion.getText();
        
        if (nroRuc == null || (nroRuc != null && nroRuc.trim().length()==0) || 
            verificaRucValido(nroRuc).equalsIgnoreCase(ConstantsCliente.RESULTADO_RUC_INVALIDO)) {
            FarmaUtility.showMessage(this, "Ingrese un RUC correcto", txtRuc);
            return false;
        } else if (nombreCliente == null || (nombreCliente != null && nombreCliente.trim().length()==0)) {
            FarmaUtility.showMessage(this, "Ingrese Razon Social correcta", txtRazonSocial);
            return false;
        } else if (direccionCliente == null || (direccionCliente != null && direccionCliente.trim().length()==0)) {
    ...
    public void setBeanClienteJuridico(BeanClienteJuridico beanClienteJuridico){
        this.beanCliente = beanClienteJuridico;
    }
