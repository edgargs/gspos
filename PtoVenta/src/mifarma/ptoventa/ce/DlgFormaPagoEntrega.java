...
import mifarma.ptoventa.administracion.fondoSencillo.reference.ConstantsFondoSencillo;
...
public class DlgFormaPagoEntrega extends JDialog {
    ...
    private int COL_COD_FPAGO = 0, COL_DESC_FPAGO = 1, COL_CANT = 2, COL_MONEDA = 3, COL_MONTO = 4, COL_TOTAL =
        5, COL_LOTE = 6,
    //COL_IND_SOBRE_BD = 7,
    COL_IND_SOBRE = 7, COL_SOBRE = 8, COL_SEC_CAJA = 9, COL_MON_2 = 10, COL_IND = 11, COL_IND_AUTOMATICO =
        12, COL_SEC_FPAGO = 13; //,COL_IND_SOBRE = 14;
    ...
    private void jbInit() throws Exception {
        ...
        lblmensaje.setText("LOS SOBRES APROBADOS POR EL QUIMICO FAMACEUTICO SE MUESTRAN DE COLOR ROJO");
        lblmensaje.setBounds(new Rectangle(15, 275, 655, 15));
    ...
    private void initialize() {
        ...
        try {
            VariablesCajaElectronica.vCantModSobres =
                    Integer.parseInt(DBCajaElectronica.getCantModificacionesSobre().trim());
        } catch (Exception e) {
    ...
    private void initTableDetallePago() {
        ...
        tableModelDetallePago =
                new FarmaTableModel(ConstantsCajaElectronica.columsListaDetallePago, ConstantsCajaElectronica.defaultListaDetallePago,
                                    0);
        FarmaUtility.initSimpleList(tblDetallePago, tableModelDetallePago,
                                    ConstantsCajaElectronica.columsListaDetallePago);
    }
    ...
    private void initTableFormasPago() {
        ...
        tableModelFormasPago =
                new FarmaTableModel(ConstantsCajaElectronica.columsListaFormasPagoEntrega, ConstantsCajaElectronica.defaultListaFormasPagoEntrega,
                                    0);
        FarmaUtility.initSimpleList(tblFormasPago, tableModelFormasPago,
                                    ConstantsCajaElectronica.columsListaFormasPagoEntrega);
    }
            ...
            if (VariablesCajaElectronica.vUsuarioCajero &&
                ...
                if (buscarSobres()) { //INI ASOSA, 09.08.2010 - otro cambio imprevisto
                    if (cargaValidaLogin()) {
                        ...
                        FarmaUtility.showMessage(this, "Necesita autorización del Jefe del Local.", tblFormasPago);
                    }
                } else {
                    agregaFormaPagoEntrega();
    ...
    private void validaFormaPagoSeleccionada() {
        ...
        if (codFormaPago.equalsIgnoreCase(ConstantsCaja.FORMA_PAGO_EFECTIVO_SOLES)) {
            ...
            FarmaUtility.moveFocus(txtMontoPagado);
                   ...
                   VariablesCajaElectronica.vIndDebito.equalsIgnoreCase(ConstantsCajaElectronica.IND_DEBITO)) {
    ...
    private void adicionaDetallePago() {
        ...
        if (!isLocalProsegur(VariablesCaja.vCodFormaPago) &&
            !obtenerIndSobresXFPago(VariablesCaja.vCodFormaPago)) { //ASOSA, 31.05.2010
            if (VariablesCajaElectronica.vIndSobre.equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                FarmaUtility.showMessage(this, "No puede registrar la forma de pago como Sobre. Verifique!!!",
                                         tblFormasPago);
                return;
            }
            if (!validaCodigoFormaPago()) {
                FarmaUtility.showMessage(this, "La forma de pago ya existe para el Cierre de Turno. Verifique!!!",
                                         tblFormasPago);
                return;
            }
    ...
    private void obtieneDatosFormaPago() {
        log.debug("VariablesCaja.vIndTarjetaSeleccionada obtiene fromas de pago inicio : " +
                  VariablesCaja.vIndTarjetaSeleccionada);
        if (tblFormasPago.getRowCount() <= 0)
            return;
        int fila = tblFormasPago.getSelectedRow();
        ...
        if (VariablesCaja.vCodFormaPago.trim().equalsIgnoreCase(ConstantsCaja.FORMA_PAGO_FONDO_SENCILLO)) {
            VariablesCaja.vValMontoPagado =
                    FarmaUtility.formatNumber(FarmaUtility.getDecimalNumber(VariablesFondoSencillo.vMonto));
            ...
            log.debug("VariablesCaja.vIndTarjetaSeleccionada obtiene fromas de pago fin : " +
                      VariablesCaja.vIndTarjetaSeleccionada);
    ...
    private boolean validaMontoIngresado() {
        ...
        if (FarmaUtility.getDecimalNumber(monto) < 0) {
            if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                                                                            "El monto ingresado es negativo. Está seguro de adicionar?"))
                return true;
            else
                return false;
        }
    ...
    private boolean validaCodigoFormaPago() {
        ...
        for (int i = 0; i < tblDetallePago.getRowCount(); i++) {
            ...
            if (indTarjeta.equalsIgnoreCase(FarmaConstants.INDICADOR_N)) {
                if (codFormaPago.equalsIgnoreCase(codTmp) && codMoneda.equalsIgnoreCase(codMonedaTmp))
                    return false;
            } else if (indTarjeta.equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                if (codFormaPago.equalsIgnoreCase(codTmp) && codMoneda.equalsIgnoreCase(codMonedaTmp) &&
                    numLote.equalsIgnoreCase(numLoteTmp))
                    return false;
            }
        }
        return true;
        ...
        try {
            for (int i = 0; i < tblDetallePago.getRowCount(); i++) {
                if (((String)tblDetallePago.getValueAt(i,
                                                       COL_IND)).trim().equalsIgnoreCase(FarmaConstants.INDICADOR_N)) {
                    pSecFormaPagoEntrega =
                            DBCajaElectronica.agregaFormaPagoEntrega(((String)tblDetallePago.getValueAt(i,
                                                                                                        COL_SEC_CAJA)).trim(),
                    ...
                    ArrayList listaSobres02 =
                        colectarSobresRecienDeclarados(VariablesCajaElectronica.vSecMovCaja); //ASOSA, 13.08.2010
                    ...
                    if (CodFormPago.trim().equalsIgnoreCase(ConstantsCaja.FORMA_PAGO_FONDO_SENCILLO) &&
                        ...
                        log.info("antes de ind Tiene Monto Devolver VariablesFondoSencillo.vIndTieneMontoDevolver:" +
                                 VariablesFondoSencillo.vIndTieneMontoDevolver);
                        VariablesFondoSencillo.vIndTieneMontoDevolver =
                                UtilityFondoSencillo.getIndTieneDevFondo(FarmaVariables.vNuSecUsu,
                                                                         VariablesCajaElectronica.vSecMovCaja, this,
                                                                         tblFormasPago).trim();
                        log.info("VariablesFondoSencillo.vIndTieneMontoDevolver:" +
                                 VariablesFondoSencillo.vIndTieneMontoDevolver);
                        if (VariablesFondoSencillo.vIndTieneMontoDevolver.equalsIgnoreCase("D")) {
                            ...
                            DBFondoSencillo.insDevolucionSencillo(VariablesFondoSencillo.vMonto,
                                    ...
                                    ConstantsFondoSencillo.EstadoEmitido, ConstantsFondoSencillo.TipoFondoDevuelve,
                                    FarmaVariables.vIdUsu, FarmaVariables.vIpPc, VariablesCajaElectronica.vSecMovCaja);
                            UtilityFondoSencillo.imprimeVoucherDevoluciones(this, VariablesCajaElectronica.vSecMovCaja,
                                                                            tblFormasPago);
                        }
                    }
                    //--
                    ...
                    if (VariablesPtoVenta.vIndTico.equals("S") &&
                        VariablesPtoVenta.vIndConFarma.equals("S")) {
                        UtilitySobres.obtenerSobres(); ////rherrera 28.08.2014
                        //-----@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@-----//
                    }
            ...
            if (sql.getErrorCode() == 20001) {
                ...
                FarmaUtility.showMessage(this, "No se puede agregar el sobre.\n" +
                        "Porque el dia ya se asoció a un Remito.", null);
            } else {
    ...
    private void eliminaFormaPagoEntrega() {
        ...
        if (indSobre.trim().equalsIgnoreCase("S") && CodSobre.trim().length() > 0 &&
            ...
            validarEstadoSobre(CodSobre, CodFormPago).equalsIgnoreCase("A")) {

            //enviar correo de intento
            TipMensaje = "01";
            try {
                DBCajaElectronica.enviarCorreoSobresModificados(VariablesCajaElectronica.vFecCierreCajaAux,
                                                                VariablesCajaElectronica.vSecMovCaja, TipMensaje,
                                                                CodSobre);
            } catch (Exception e) {
            ...
            if (!validaSobreRemito(VariablesCajaElectronica.vFecCierreCajaAux, VariablesCajaElectronica.vSecMovCaja,
                                   CodSobre)) {
        ...
        if (tieneRegistroSeleccionado(tblDetallePago)) {
            ...
            if (VariablesCajaElectronica.vIndEliminacion.equalsIgnoreCase(FarmaConstants.INDICADOR_N)) {
                ...
                try {
                    ...
                    if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                                                                                    "¿El sistema eliminará la forma de pago físicamente. Desea Continuar?")) {
                        if (VariablesCajaElectronica.vIndAutomatico.equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                            if (!cargaLoginOper()) {
                                FarmaUtility.liberarTransaccion();
                                FarmaUtility.showMessage(this,
                                                         "No se realizó la operación. Solo un usuario con Rol de\nOperador de Sistemas puede eliminar esta forma de pago.",
                                                         tblDetallePago);
                                return;
                            }
                        }
                        ...
                        if (VariablesCajaElectronica.vIndSobre.trim().equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                            ...
                            try {
                                ...
                                if (VariablesPtoVenta.vIndTico.equals("S") &&
                                    ...
                                    res = UtilitySobres.eliminarSobre(CodSobre);

                                    if (res.equals("N")) {
                                        FarmaUtility.showMessage(this, "No se puede eliminar el sobre.\n" +
                                                "Porque el local FARMA lo esta visualizando", null);
                                        FarmaUtility.liberarTransaccion();
                                        return;
                                    }
                                }
                        ...
                        if (!VariablesCajaElectronica.vLote.equalsIgnoreCase("")) {
                            ...
                            if (CodFormPago.trim().equalsIgnoreCase(ConstantsCaja.FORMA_PAGO_FONDO_SENCILLO) &&
                                ...
                                String indFondoAceptado =
                                    DBFondoSencillo.getIndDevolucionAceptada(VariablesCajaElectronica.vSecMovCaja).trim();
                                if (indFondoAceptado.equalsIgnoreCase("S")) {
                                    FarmaUtility.showMessage(this,
                                                             "No puede Eliminar una Forma de Sencillo Aceptada por el Administrador del Local.",
                                                             tblFormasPago);
                                    indSigueProce = false;
                                } else {
                                    if (cargaLoginOper()) {
                                        ...
                                        indSigueProce = false;
                                        FarmaUtility.showMessage(this, "Necesita validar con clave operador.",
                                                                 tblFormasPago);
                                    }
                                }
                            }
    ...
    private void guardaDatosDetalle() {
        VariablesCajaElectronica.vIndEliminacion =
                ((String)tblDetallePago.getValueAt(tblDetallePago.getSelectedRow(), COL_IND)).trim();
        VariablesCajaElectronica.vSecMovCaja =
                ((String)tblDetallePago.getValueAt(tblDetallePago.getSelectedRow(), COL_SEC_CAJA)).trim();
        VariablesCajaElectronica.vCodFormaPago =
                ((String)tblDetallePago.getValueAt(tblDetallePago.getSelectedRow(), COL_COD_FPAGO)).trim();
        VariablesCajaElectronica.vTipMoneda =
                ((String)tblDetallePago.getValueAt(tblDetallePago.getSelectedRow(), COL_MON_2)).trim();
        VariablesCajaElectronica.vLote =
                ((String)tblDetallePago.getValueAt(tblDetallePago.getSelectedRow(), COL_LOTE)).trim();
        VariablesCajaElectronica.vIndAutomatico =
                ((String)tblDetallePago.getValueAt(tblDetallePago.getSelectedRow(), COL_IND_AUTOMATICO)).trim();
        VariablesCajaElectronica.vSecFPEntrega =
                ((String)tblDetallePago.getValueAt(tblDetallePago.getSelectedRow(), COL_SEC_FPAGO)).trim();

        VariablesCajaElectronica.vIndSobre =
                ((String)tblDetallePago.getValueAt(tblDetallePago.getSelectedRow(), COL_IND_SOBRE)).trim();
        VariablesCajaElectronica.vCodigoSobre =
                ((String)tblDetallePago.getValueAt(tblDetallePago.getSelectedRow(), COL_SOBRE)).trim();
    ...
    private void listaFormasPagoEntrega(boolean pIndElimina) {
        ...
        try {
            DBCajaElectronica.cargaListaDetalleFormasPago(tableModelDetallePago, VariablesCajaElectronica.vSecMovCaja);
            if (pIndElimina) {
            ...
            log.error("", sql);
            FarmaUtility.showMessage(this, "Error al listas las formas de pago Entrega Realizadas \n" +
                    sql.getMessage(), null);
        }
    }
    ...
    private void cargarSobres() {
        ...
        try {
            ...
            FarmaUtility.initSimpleListCleanColumns(tblDetallePago, tableModelDetallePago,
                                                    ConstantsCajaElectronica.columsListaDetallePago, aux, Color.white,
                                                    Color.red, false);
    ...
    private boolean ingresaDevMonto(String pMonto) {
        boolean flag = false;
        try {
            DBFondoSencillo.insDevolucionSencillo(pMonto, VariablesFondoSencillo.vSecAdmLocal, // DESTINO
                    FarmaVariables.vNuSecUsu, // ORIGEN
                    ConstantsFondoSencillo.EstadoEmitido, ConstantsFondoSencillo.TipoFondoDevuelve,
                    FarmaVariables.vIdUsu, FarmaVariables.vIpPc, VariablesCajaElectronica.vSecMovCaja);
                ...
                if (validaFondoSencilloTable()) {
                    ...
                    if (FarmaVariables.vAceptar) {
                        ...
                        if (!isLocalProsegur(VariablesCaja.vCodFormaPago)) {
                            if (VariablesCajaElectronica.vIndSobre.equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                                FarmaUtility.showMessage(this,
                                                         "No puede registrar la forma de pago como Sobre. Verifique!!!",
                                                         tblFormasPago);
                                return;
                            }
                            if (!validaCodigoFormaPago()) {
                                FarmaUtility.showMessage(this,
                                                         "La forma de pago ya existe para el Cierre de Turno. Verifique!!!",
                                                         tblFormasPago);
                                return;
                            }
                        }
    ...
    private String validarEstadoSobre(String codsobre, String codFormaPago) {
        ...
        try {
            ...
            isAprobado =
                    DBCajaElectronica.getSobreAprobado(codsobre, codFormaPago, VariablesCajaElectronica.vFecCierreCajaAux).trim();
    ...
    private void determinarVisibilidadMsgRed() {
        boolean flag = false;
        flag = obtenerIndSobresXFPago("00001");
        if (flag)
    ...
    private void reimprimir() {
        String indSobre =
            FarmaUtility.getValueFieldArrayList(tableModelDetallePago.data, tblDetallePago.getSelectedRow(), 7).trim();
        String secSobre =
            FarmaUtility.getValueFieldArrayList(tableModelDetallePago.data, tblDetallePago.getSelectedRow(), 8).trim();
        String pSecMovCaja =
            FarmaUtility.getValueFieldArrayList(tableModelDetallePago.data, tblDetallePago.getSelectedRow(), 9).trim();
    ...
    private ArrayList colectarSobresRecienDeclarados(String pSecMovCaja) {
        ...
        try {
            ...
            // KMONCADA 22.06.2016 IMPRESION DE ENTREGA DE SOBRES
            ...
            FarmaUtility.showMessage(this, "ERROR al colectar Sobres Recien Declarados.", null);
        }
        return pLista;
    }
    ...
    private void imprimeSobresDeclarados_0X(ArrayList listaS, String pSecMovCaja) {
        ...
        try {
            //pIndProsegur = DBCajaElectronica.getIndProsegur().trim();
            // KMONCADA 22.06.2016 IMPRESION DE COMPROBANTE DE PAGO
            ...
            if (pIndProsegur.trim().equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                ...
                if (!vIndImpre.equals("N")) {
                    ...
                    if (indImp)
                        FarmaUtility.showMessage(this, "Recoger Voucher de sobres declarados.", null);
                }
            }
            ...
            //log.error("",sqlException);
            log.error(null, sqlException);
            FarmaUtility.showMessage(this, "Error al obtener los datos de los sobres a imprimir.", null);
    ...
    public boolean obtMsjValidarTarj() {
        ...
        try {
            varpru =
                    DBCajaElectronica.validarTarjeta(VariablesCajaElectronica.vSecMovCaja, VariablesCaja.vCodFormaPago,
                                                     VariablesCajaElectronica.vLote, VariablesCaja.vCodMonedaPago,
                                                     VariablesCajaElectronica.vCantidad, VariablesCaja.vValMontoPagado,
                                                     VariablesCaja.vValTotalPagado);
        } catch (Exception e) {
            log.error("", e);
            varpru = "";
