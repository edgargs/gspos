--------------------------------------------------------
--  DDL for Package Body PTOVENTA_CAJ_ANUL
--------------------------------------------------------
...
--                   IF v_rCurProdVirtual.FECHA > 0.007 THEN --0.007 INDICA 10 MINUTOS
  ...
  FUNCTION CAJ_LISTA_CABECERA_PEDIDO(cCodGrupoCia_in IN CHAR,cCodLocal_in IN CHAR,cNumPedVta_in IN CHAR ,cNumCompPag IN CHAR)
  RETURN FarmaCursor
  IS
    curCab FarmaCursor;
  BEGIN
    ...
    WHERE A.COD_GRUPO_CIA=C.COD_GRUPO_CIA
          ...
          AND C.NUM_COMP_PAGO=cNumCompPag
  ...
  BEGIN
    ...
    INSERT INTO VTA_PEDIDO_VTA_CAB(COD_GRUPO_CIA,
                                    ...
                                    NUM_PED_VTA_ORIGEN,
                                    IND_PED_CONVENIO,
                                    COD_CONVENIO,IND_FID,
                                    --Agregado por dveliz 24.02.2009
                                    NUM_PEDIDO_DELIVERY,
                                    --Agregado por FRAMIREZ 11.06.2012
                                    ind_conv_btl_mf,
                                    ip_cob_ped,
                                    name_pc_cob_ped,
                                    dni_usu_local,
...
/*    UPDATE PBL_RESPALDO_STK SET FEC_MOD_RESPALDO_STK=SYSDATE ,USU_MOD_RESPALDO_STK =vIdUsu_in,
  ...
  /* ********************************************************************************************** */
  FUNCTION CAJ_AGREGAR_CAB_NOTA_CREDITO(cCodGrupoCia_in IN CHAR,cCodLocal_in IN CHAR,cNumVtaAnt_in IN CHAR,nTipoCam_in IN NUMBER,vIdUsu_in IN VARCHAR2,nNumCajaPago_in IN NUMBER,cMotivoAnulacion IN VARCHAR2)
  RETURN CHAR
  IS
    --v_nNumPed VTA_PEDIDO_VTA_CAB.NUM_PED_VTA%TYPE;
    ...
    WHERE COD_GRUPO_CIA = cCodGrupoCia_in
          AND COD_LOCAL = cCodLocal_in
          AND NUM_PED_VTA = cNumVtaAnt_in FOR UPDATE;*/
-- dubilluz 27.09.2013
SELECT      CA.COD_CLI_LOCAL,
            ...
            CA.fec_proceso_rac,
            -- DUBILLUZ 27.09.2013
            CP.SEC_COMP_PAGO,CP.TIP_CLIEN_CONVENIO,
  ...
  BEGIN
    ...
    */
    ...
    WHERE COD_GRUPO_CIA = cCodGrupoCia_in
      ...
      UPDATE VTA_PEDIDO_VTA_CAB SET USU_MOD_PED_VTA_CAB = vIdUsu_in,FEC_MOD_PED_VTA_CAB = SYSDATE,
        MOTIVO_ANULACION= cMotivoAnulacion
      WHERE COD_GRUPO_CIA = cCodGrupoCia_in
            AND COD_LOCAL = cCodLocal_in
...
-- dubilluz 27.09.2013
   if vCtdComprobantes > 1 then
    ...
    VALUES( cCodGrupoCia_in,
            ...
            v_rCurCab.SEC_COMP_PAGO,v_rCurCab.TIP_CLIEN_CONVENIO,
...
-- dubilluz 27.09.2013
  ...
  /* *********************************************************************** */
     FUNCTION CAJ_AGREGAR_DET_NC(cCodGrupoCia_in IN CHAR,
                                       ...
                                       cCodProd_in IN CHAR,nCantProd_in IN NUMBER,nTotal_in IN NUMBER,vIdUsu_in IN VARCHAR2,
    ...
    IF vTipoComprobantePago <> '03' THEN
      ...
      INSERT INTO VTA_COMP_PAGO(COD_GRUPO_CIA, COD_LOCAL, NUM_PED_VTA, SEC_COMP_PAGO,
                      ...
                      /* INICIO LTAVARA SE AGREGO PARA COMPROBANTE ELECTRONICO 30-07-2014   */
                        ,
                        COD_TIP_PROC_PAGO,
                        --NUM_COMP_PAGO_E,--NUEVO COMPROBANTE
                        COD_TIP_IDENT_RECEP_E,
                        NUM_COMP_PAGO_EREF,-- NUMERO DE COMPROBANTE ELECTRONICO DE REFERENCIA
      ...
      select  cCodGrupoCia_in, cCodLocal_in, cNumVta_in, v_nSecCompPago,
              '04',vNumCompPago_in,
              v_cSecMovCaja_in,CANT_ITEM, cp.COD_CLI_LOCAL, NOM_IMPR_COMP,
              DIREC_IMPR_COMP, NUM_DOC_IMPR, VAL_BRUTO_COMP_PAGO*-1,
              VAL_NETO_COMP_PAGO*-1, VAL_DCTO_COMP_PAGO*-1,
              VAL_AFECTO_COMP_PAGO*-1, VAL_IGV_COMP_PAGO*-1,
              VAL_REDONDEO_COMP_PAGO*-1, USU_CREA_COMP_PAGO, PORC_IGV_COMP_PAGO,
              IND_AFECTA_KARDEX
                 /* INICIO LTAVARA SE AGREGO PARA COMPROBANTE ELECTRONICO 30-07-2014  */
                        ,
                        cp.COD_TIP_PROC_PAGO,
                       cp.COD_TIP_IDENT_RECEP_E,
                         cp.NUM_COMP_PAGO,--POR AHORA, LUEGO CAMBIAR A NUM_COMP_PAGO_E
                        ...
                        DECODE(cp.TIP_COMP_PAGO,
                                      ...
                                      '05','03')
                                             ,
                       '06',--TIPO DE NC POR DEVOLUCION TOTAL
                        cp.VAL_TOTAL_E,
                        cp.TOTAL_GRAV_E,
                        cp.TOTAL_INAF_E,
                        cp.TOTAL_GRATU_E,
 ...
 /* ****************************************************************** */
            ...
            --24/07/2014  LTAVARA         Creacion
/*
            ptoventa_upd_comp_pago_e(cCodGrupoCia_in,
                                                     cCodLocal_in,
                                                     cNumVta_in,
                                                     v_nSecCompPago,
                                                     (case vTipClieConv_in WHEN 1 THEN '1'
                                                       WHEN 2 THEN '2'END)
                                                     );*/
      ...
      SET    FEC_MOD_IMPR_LOCAL = SYSDATE,
             ...
             NUM_COMP           = TRIM(TO_CHAR((TO_NUMBER(NUM_COMP) + 1),
                                              '0000000'))
      WHERE COD_GRUPO_CIA = cCodGrupoCia_in
      AND   COD_LOCAL = cCodLocal_in
      AND   Tip_Comp = '04';
    ...
    END IF;
      ...
      if vIndAfectaKardex_in = 'S' then
       ...
       and     ca.num_ped_vta = cNumVtaAnt_in;
		...
		BEGIN
			select c.im_total_pago*-1
			into   vImptotalVtaCredito
			from   vta_forma_pago_pedido c
			where  c.cod_grupo_cia =cCodGrupoCia_in
			and    c.cod_local = cCodLocal_in
			and    c.num_ped_vta = cNumVtaAnt_in
			and    c.cod_forma_pago = '00050';-- CREDITO BTL MF

        EXCEPTION
        WHEN NO_DATA_FOUND THEN
            vImptotalVtaCredito:=0.000; --RHEREREA : si no encuentra forma de pago 00080,
                                     -- resta valor 0.000
        END;
        vImptotalSoles := vImptotalSoles - vImptotalVtaCredito;
      ...
      if nSecDetPed_in = 0 then
			 ...
			 --ERIOS 2.4.7 NC de Vtas al Credito 
			 if abs(vImptotalVtaCredito) > 0 then
                 INSERT INTO VTA_FORMA_PAGO_PEDIDO(COD_GRUPO_CIA,COD_LOCAL,
                COD_FORMA_PAGO,NUM_PED_VTA,IM_PAGO,TIP_MONEDA,
                VAL_TIP_CAMBIO,VAL_VUELTO,IM_TOTAL_PAGO,
                USU_CREA_FORMA_PAGO_PED  )
                VALUES(cCodGrupoCia_in,cCodLocal_in,
                '00050',cNumVta_in,vImptotalVtaCredito,'01',--SOLES
                0,0,vImptotalVtaCredito,vIdUsu_in);
             end if;
