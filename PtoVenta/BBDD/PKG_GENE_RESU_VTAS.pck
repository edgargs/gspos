...
PROCEDURE SP_GIG_LOAD_VENTAS_VEN_LOC(
 ...
 BEGIN
   ...
   SELECT to_char(to_date(to_char(an_Mes_Id),'yyyymm'),'dd/MM/yyyy') fec_ini,
          to_char(last_day(to_date(to_char(an_Mes_Id),'yyyymm')),'dd/mm/yyyy')fec_fin
  ...
  -- KMONCADA 13.10.2015 SE CAMBIA LOGICA DE PARA DETERMINAR LOS PRODUCTOS QUE PARTICIPAN EN CADA CATEGORIA
  -- SOLICITADO POR JOLIVA
   INSERT INTO GIG_VTA_RES_VTA_VEND_LOCAL(
          cod_grupo_cia, cod_local, sec_usu_local, fec_dia_venta, comision_real,
          comis_gigant_a, comis_gigant_b, mon_tot_s_igv, mon_tot_c_igv)
    SELECT
        VTA.COD_GRUPO_CIA COD_GRUPO_CIA, 
        VTA.COD_LOCAL COD_LOCAL, 
        VTA.SEC_USU_LOCAL SEC_USU_LOCAL, 
        VTA.FECHA FEC_DIA_VENTA,
        NVL(TOT_COM, 0) COMISION_REAL,
    ...
    FROM(
      ...
      SELECT A.COD_GRUPO_CIA, 
             ...
             A.FECHA, 
             SUM(A.COMIS) TOT_COM,
             SUM(A.MON_CIGV) MON_TOT_CIGV,
             SUM(A.MON_SIGV) MON_TOT_SIGV
      FROM (
        SELECT CAB.COD_GRUPO_CIA,
               ...
               (NVL(DET.PORC_ZAN, 0.4) * DET.VAL_PREC_TOTAL / (1 + VAL_IGV / 100) / 100) COMIS,
               --SUM(DET.VAL_PREC_TOTAL) MON_TOT_CIGV,
               DET.VAL_PREC_TOTAL MON_CIGV,
               --SUM(DET.VAL_PREC_TOTAL / (1 + VAL_IGV / 100)) MON_TOT_SIGV,
               (DET.VAL_PREC_TOTAL / (1 + VAL_IGV / 100)) MON_SIGV,
               CASE 
                 -- TODO GIGANTES
                 WHEN vCategoriaLocal = '001' AND TRIM(DET.IND_ZAN) IN ('GG','3G') THEN
                    'X'
                 -- VITAMINAS Y SUPLEMENTOS
                 WHEN vCategoriaLocal = '002' AND TRIM(DET.IND_ZAN) IN ('GG','3G') AND PROD.COD_LAB IN ('00768','00781','01104','00706','00945') THEN
                   'X'
                 -- FARMACIA
                 WHEN vCategoriaLocal = '003' AND TRIM(DET.IND_ZAN) IN ('GG','3G') AND DET.COD_GRUPO_REP_EDMUNDO = '001' THEN
                   'X'
                 ELSE
                   NULL
               END APLICA
         ...
         WHERE CAB.COD_GRUPO_CIA = cCodGrupoCia_in
           ...
           AND CAB.FEC_PED_VTA BETWEEN TO_DATE(cFecha_Ini_in, 'dd/MM/yyyy HH24:mi:ss') AND
                                       TO_DATE(cFecha_Fin_in, 'dd/MM/yyyy HH24:mi:ss')
      ...
      WHERE A.APLICA = 'X'
      GROUP BY A.COD_GRUPO_CIA, A.COD_LOCAL, A.SEC_USU_LOCAL, A.FECHA
    ) VTA;
