...
/*  PROCEDURE INS_AHORRO_INMED(cCodGrupoCia_in  IN CHAR,
  ...
  FUNCTION F_GET_ENVIO_RECONFIRMA_EPOS
  RETURN CHAR;
  
  FUNCTION F_CHAR_TRAMA_ALBIS
  RETURN CHAR;
...
CREATE OR REPLACE PACKAGE BODY "FARMA_EPOS" as
  ...
  FUNCTION F_VAR_TRM_GET_CAB(cGrupoCia      VARCHAR2,
    ...
    vTramaAlbis CHAR(1);
  BEGIN
    vTramaAlbis := FARMA_EPOS.F_CHAR_TRAMA_ALBIS;
    BEGIN
      ...
      SELECT 'EN|' || -- ENCABEZADO
              ...
              -- KMONCADA 27.04.2016 ALBIS-ARCANGEL
              CASE 
                WHEN vTramaAlbis = 'S' AND COMP.COD_TIP_IDENT_RECEP_E = '0' THEN '1'
                ELSE COMP.COD_TIP_IDENT_RECEP_E 
              END || '|' || -- 18 Tipo de Identificacion Receptor
              -- KMONCADA 27.04.2016 ALBIS-ARCANGEL
              CASE 
                WHEN vTramaAlbis = 'S' THEN 
                  REEMPLAZA_CARACTERES(TRIM(NVL(TRIM(COMP.NOM_IMPR_COMP), 'CLIENTE CONSUMIDOR FINAL')))
                ELSE 
                  REEMPLAZA_CARACTERES(TRIM(NVL(TRIM(COMP.NOM_IMPR_COMP), 'SIN NOMBRE'))) 
              END || '|' || -- 19 Razon Social Receptor
             ...
             --DECODE(COMP.TIP_COMP_PAGO,'04',TRIM(TO_CHAR(NVL(COMP.VAL_NETO_COMP_PAGO*-1,0),'999999990.00')),TRIM(TO_CHAR(NVL(COMP.VAL_NETO_COMP_PAGO,0),'999999990.00')))||'|'||-- 21 Monto Neto
              ...
              DECODE(COMP.TIP_COMP_PAGO,
                     ...
                     TRIM(TO_CHAR(NVL((COMP.VAL_NETO_COMP_PAGO +COMP.VAL_REDONDEO_COMP_PAGO) * -1, 0),
                                  '999999990.00')),
                     TRIM(TO_CHAR(NVL(trunc((COMP.VAL_NETO_COMP_PAGO - NVL(COMP.VAL_PERCEP_COMP_PAGO,0)) + COMP.VAL_REDONDEO_COMP_PAGO
                     /*+ (CASE -- SOLO PARA CONVENIO COPAGO PARA LA EMPRESA EL MONTO TOTAL AL 100% -- LTAVARA 17.10.2014
              ...
              CASE 
                WHEN vTramaAlbis = 'S' THEN
                  ...
                  CASE
                    WHEN NVL(ABS(COMP.TOTAL_GRAV_E), 0) != 0 THEN '1001'
                    WHEN NVL(ABS(COMP.TOTAL_INAF_E), 0) != 0 THEN '1002'
                    WHEN NVL(ABS(COMP.TOTAL_EXON_E), 0) != 0 THEN '1003'
                    ELSE ''
                  END || '|' ||
                  -- 27 Total de Valor Venta Neto
                  TRIM(TO_CHAR((PED.VAL_NETO_PED_VTA),'99999990.00'))
                ELSE
                  '' || '|' || -- 26 Códigos de otros conceptos tributarios o comerciales recomendados
                  ''           -- 27 Total de Valor Venta Neto
              END || '|' ||
              ...
              CASE 
                WHEN vTramaAlbis = 'S' AND COMP.COD_TIP_IDENT_RECEP_E = '0' THEN '1'
                ELSE COMP.COD_TIP_IDENT_RECEP_E 
              END || '|' || -- 29 Tipo de documento de identidad del adquirente o usuario
             ...
             ELSE
              ...
              CASE PED.TIP_PED_VTA
                WHEN '02' --PEDIDO DELIVERY
                 THEN
                 LOCAL.UBDEP || LOCAL.UBPRV || LOCAL.UBDIS || '|' || -- 32 Dirección del Punto de Partida, Código de Ubigeo
                 REEMPLAZA_CARACTERES(LOCAL.DIREC_LOCAL_CORTA) || '|' || -- 33 Dirección del Punto de Partida, Dirección completa y detallada
                 '' || '|' || -- 34 Dirección del Punto de Partida, Urbanización
                 UBI_LOCAL.NOPRV || '|' || -- 35 Dirección del Punto de Partida, Provincia
                 UBI_LOCAL.NODEP || '|' || -- 36 Dirección del Punto de Partida, Departamento
                 UBI_LOCAL.NODIS || '|' || -- 37 Dirección del Punto de Partida, Distrito
                 ...
                 (SELECT
                   ...
                   REEMPLAZA_CARACTERES(COMA.DIRECCION) || '|' || -- 40 Dirección del Punto de Llegada, Dirección completa y detallada
  ...
  FUNCTION F_VAR_TRM_GET_DOC(cGrupoCia      VARCHAR2,
   ...
   RETURN VARCHAR2 AS
    ...
    vUsaTramaAlbis CHAR(1);
  BEGIN
    vUsaTramaAlbis := FARMA_EPOS.F_CHAR_TRAMA_ALBIS;
    BEGIN
      ...
      FOR REG IN (
                  ...
                  -- 2005 Total descuentos
                          ...
                          --TRIM(TO_CHAR(NVL(ABS( DECODE(cTipoClienteConvenio,DOC_EMPRESA,(COMP.VAL_COPAGO_E+COMP.TOTAL_DESC_E),COMP.TOTAL_DESC_E)), 0),
                            TRIM(TO_CHAR(CASE WHEN NVL(COMP.PCT_BENEFICIARIO,0)>0 
                                                       ...
                                                       AND COMP.TIP_CLIEN_CONVENIO=DOC_EMPRESA  THEN
                                                           NVL(COMP.VAL_COPAGO_E,0) + NVL(COMP.TOTAL_DESC_E,0)
                                                       ELSE 
                                                         --KMONCADA 10.08.2015 EN CASO SE REGISTRE NULO PARA ENVIO DE TRAMA
                                                         NVL(COMP.TOTAL_DESC_E,0) --23.12.2014 LTAVARA
                                                       END,'999999990.00')) || '|'||
                                       ''||'|'|| -- 3.* Monto Total Documento Incluida la percepcion
                                       ''|| -- 4.* Este campo puede informar:
                                                  --   -. Base imponible percepcion.
                   ...
                   -- 2001 Total PERCEPCIONES
                   UNION ALL
                   select 'DOC|' || '2001|'
                          || TRIM(TO_CHAR(NVL(COMP.VAL_PERCEP_COMP_PAGO,0),'999999990.00')) || '|' ||
                          -- KMONCADA 27.04.2016 ALBIS - ARCANGEL
                          CASE 
                            WHEN vUsaTramaAlbis = 'S' THEN
                              TRIM(TO_CHAR(COMP.VAL_NETO_COMP_PAGO,'9999999990.00'))||'|0' 
                            ELSE
                              ''||'|' 
                          END || CHR(13)
                          VALOR
    ...
    BEGIN
      SELECT 'DN|'||
           ...
           'SON: ' ||  TRIM(UPPER(TRIM(FARMA_UTILITY.LETRAS(ABS(COMP.VAL_NETO_COMP_PAGO +COMP.VAL_REDONDEO_COMP_PAGO)))) || ' ' ||
             ...
             -- KMONCADA 20.04.2016 MENSAJE PERSONALIZADO PARA AMAZONIA
             -- SOLICITADO POR DJARA
             CASE 
               WHEN NVL(C.FLG_EXONERADO,'N')='S' THEN
                 CHR(13)||'DN|'||
                 '1|'||--Número de Línea de Nota	1
                 '2001|' ||--Código de la leyenda	2
                 --- Glosa de la leyenda	3
                 (SELECT LLAVE_TAB_GRAL
                  FROM PBL_TAB_GRAL 
                  WHERE ID_TAB_GRAL = 608)
                 ||'|'||
                 -------------------------------
                 ''||
                 CHR(13)
        ...
        FROM VTA_COMP_PAGO COMP,
             PBL_LOCAL L,
             PBL_CIA C
       WHERE COMP.COD_GRUPO_CIA = L.COD_GRUPO_CIA
       AND COMP.COD_LOCAL = L.COD_LOCAL 
       AND L.COD_CIA = C.COD_CIA
       AND COMP.COD_GRUPO_CIA = cGrupoCia
         AND COMP.COD_LOCAL = cCodLocal
         AND COMP.NUM_PED_VTA = cNumPedidoVta
         AND COMP.SEC_COMP_PAGO = cSecCompPago;
...
--   RETURN VARCHAR2 AS
  RETURN FARMACURSOR AS
    ...
    vUsaTramaAlbis  CHAR(1);
  BEGIN
    -- KMONCADA 27.04.2016 ALBIS
    vUsaTramaAlbis := F_CHAR_TRAMA_ALBIS;
                    ...
                    --DETALLE DE DOCUMENTO
                     ...
                     -- TRIM(TO_CHAR(NVL(ABS(DET.VAL_VTA_UNIT_ITEM_E),0),'999999990.00'))||'|'||-- Valor de venta unitario por ítem
                      TRIM(TO_CHAR(NVL(ABS(DET.VAL_VTA_UNIT_ITEM_E),
                                       0),
                                   '999999990.000')) || '|' || -- 8 Valor de venta unitario por ítem
                      TRIM(TO_CHAR(NVL(ABS(DET.VAL_VTA_ITEM_E), 0),
                     ...
                     --REPLACE(REPLACE((ABS(DET.CANT_ATENDIDA)||'/'||ABS(DET.VAL_FRAC)
                      REPLACE(REPLACE((
                                      ...
                                      -- dubilluz 06.11.2014
                                       ...
                                       CASE 
                                         WHEN vUsaTramaAlbis = 'S' THEN
                                           ';'||TRIM(TO_CHAR(NVL(DET.VAL_PREC_TOTAL, 0),'999999990.00'))
                                         ELSE ''
                                       END 
      ...
      OPEN cursorDetalle FOR
        ...
        --DETALLE DE DOCUMENTO
         ...
         -- DETALLE DEIM
          ...
          DECODE(PAGO.PORC_IGV_COMP_PAGO, FARMA_GRAL.F_GET_ESPECIF_IGV_LOCAL(FARMA_GRAL.COD_IGV_18), 'VAT', '') || '|' || -- 5 Tipo de Impuesto
          DECODE(PAGO.PORC_IGV_COMP_PAGO, FARMA_GRAL.F_GET_ESPECIF_IGV_LOCAL(FARMA_GRAL.COD_IGV_18), '10', '30') || '|' || -- 6 Afectación del IGV
...
/* ************************************************************* */                      
  ...
  BEGIN
    ...
    SELECT   
          ...
          END
    INTO  vPedidoConvenio
    FROM  VTA_PEDIDO_VTA_CAB CAB
    WHERE CAB.COD_GRUPO_CIA = cGrupoCia
    AND   CAB.COD_LOCAL     = cCodLocal
    ...
    SELECT nvl(COMP.TOTAL_DESC_E, 0),
           ...
           CASE
             WHEN NVL(COMP.VAL_PERCEP_COMP_PAGO,0) != 0 THEN 'S'
             ELSE 'N'
           END
      INTO v_vAhorroTotal, 
           v_vREDONDEO, 
           vCPContienePercepcion
      FROM VTA_COMP_PAGO COMP
     WHERE COMP.COD_GRUPO_CIA = cGrupoCia
       AND COMP.COD_LOCAL = cCodLocal
       AND COMP.NUM_PED_VTA = cNumPedidoVta
    ...
    IF v_vAhorroTotal > 0 AND cTipoDocumento !='04' THEN
      v_vPES_AT := v_vPES_AT || 'PESD|' || v_vCONTADOR ||
                   ...
                   TO_CHAR(v_vAhorroTotal * (1 + (FARMA_GRAL.F_GET_IGV_LOCAL/100)), '999999990.00') || CHR(13);        --ASOSA - 24/06/2015 - IGVAZNIA
    END IF;
    ...
    IF vPedidoConvenio = 'S' AND (cTipoClienteConvenio = DOC_BENEFICIARIO or
                                  cTipoClienteConvenio = DOC_EMPRESA) THEN
    ...
    IF vCPContienePercepcion = 'S' THEN
      BEGIN 
        SELECT v_vPES_AT || 'PESD|' || v_vCONTADOR || '|' ||NVL(TRIM(L.LLAVE_TAB_GRAL),'')||CHR(13)
        INTO v_vPES_AT
        FROM PBL_TAB_GRAL L WHERE L.ID_TAB_GRAL = 610;
      EXCEPTION 
        WHEN OTHERS THEN
          v_vPES_AT := v_vPES_AT || 'PESD|' || v_vCONTADOR || '|COMPROBANTE AFECTO PERCEPCION' ||CHR(13);
      END;
      v_vCONTADOR := v_vCONTADOR +1;
    END IF;
    
    IF 'S' = FARMA_UTILITY.F_GET_LOCAL_APLICA_PERCEPCION(cCodGrupoCia_in => cGrupoCia,
                                                         cCodLocal_in => cCodLocal) THEN
      
      BEGIN 
        SELECT v_vPES_AT || 'PESD|' || v_vCONTADOR || '|' ||NVL(TRIM(L.LLAVE_TAB_GRAL),'')||CHR(13)
        INTO v_vPES_AT
        FROM PBL_TAB_GRAL L WHERE L.ID_TAB_GRAL = 611;
      EXCEPTION 
        WHEN OTHERS THEN
          v_vPES_AT := v_vPES_AT || 'PESD|' || v_vCONTADOR || '|COMPROBANTE AFECTO PERCEPCION' ||CHR(13);
      END;
  ...
  BEGIN
    ...
    v_vPES_DT := v_vPES_DT || 'PESD|5|DELIVERY 612-5000 LAS 24 HORAS' ||
                 CHR(13);
    BEGIN
      SELECT v_vPES_DT || 'PESD|6|'||'USUARIO: ' ||USU.LOGIN_USU|| ' - CAJA: ' || MOV.NUM_CAJA_PAGO||CHR(13)
      INTO v_vPES_DT
      FROM VTA_COMP_PAGO CP,
           CE_MOV_CAJA MOV,
           PBL_USU_LOCAL USU
      ...
      AND CP.NUM_PED_VTA = cNumPedidoVta
      AND CP.SEC_COMP_PAGO = cSecCompPago
      AND CP.COD_GRUPO_CIA = MOV.COD_GRUPO_CIA
      AND CP.COD_LOCAL = MOV.COD_LOCAL
      AND CP.SEC_MOV_CAJA = MOV.SEC_MOV_CAJA
      AND USU.COD_GRUPO_CIA = MOV.COD_GRUPO_CIA
...
/* **********************************************************************/
  ...
  FUNCTION  F_VAR_GET_TIP_PROC_CPAGO(cGrupoCia VARCHAR2,
   ...
   --INI ASOSA -15/07/2015 - IGVELECTRAZNIA
   valorIgvComp NUMBER(9,3) := 0;   
   cantProdExon number(3) := 0;
   cantProdInaf number(3) := 0;
   v_cod_cia char(3) := '001';
   v_cant_cod_cia number(2) := 0;
   --FIN ASOSA -15/07/2015 - IGVELECTRAZNIA
  ...
  begin
      ...
      -- KMONCADA 22.09.2015 CUADRE DE VALOR DE PRECIO UNITARIO PARA COMPROBANTE ELECTRONICO
      UPDATE VTA_PEDIDO_VTA_DET D
      SET D.val_vta_unit_item_e = case
                                     when d.val_vta_item_e > ((d.val_vta_unit_item_e * d.cant_atendida) - d.val_total_desc_item_e) then
                                      to_number(to_char((val_vta_unit_item_e + 0.0005), '9999999990.000'),
                                                '9999999990.000')
                                     else
    ...
    update vta_comp_pago
     ...
     where cod_grupo_cia = p_cod_grupo_cia
       ...
       */
       ...
       --INI ASOSA - 15/07/2015 - IGVELECTRAZNIA
       --HALLAR VALOR IGV
       SELECT NVL(VCP.VAL_IGV_COMP_PAGO,0)
       INTO valorIgvComp
       FROM VTA_COMP_PAGO VCP
       WHERE VCP.COD_GRUPO_CIA = p_cod_grupo_cia
       AND VCP.COD_LOCAL = p_cod_local
       AND VCP.NUM_PED_VTA = p_num_ped_vta
       AND VCP.SEC_COMP_PAGO = p_sec_com_pago;
       --HALLAR VALOR CIA
       select count(1)
       into v_cant_cod_cia
       ...
       IF valorIgvComp = 0 THEN
		...
		if v_flg_exonerado = 'S' then
          ...
          --CANTIDAD DE EXONERADOS
          SELECT COUNT(1)
          INTO cantProdExon
          FROM VTA_COMP_PAGO C,
               VTA_PEDIDO_VTA_DET D
          WHERE C.COD_GRUPO_CIA = p_cod_grupo_cia
          AND C.COD_LOCAL = p_cod_local
          AND C.NUM_PED_VTA = p_num_ped_vta
          AND C.SEC_COMP_PAGO = p_sec_com_pago
          AND C.COD_GRUPO_CIA = D.COD_GRUPO_CIA
          AND C.COD_LOCAL = D.COD_LOCAL
          AND C.NUM_PED_VTA = D.NUM_PED_VTA
          AND C.SEC_COMP_PAGO = CASE
                                  WHEN D.SEC_COMP_PAGO_BENEF  IS NOT NULL
                                                               AND C.TIP_COMP_PAGO <> '04' THEN
                                        D.SEC_COMP_PAGO_BENEF
                                  ELSE
                                       D.SEC_COMP_PAGO
                                  END
          AND D.COD_PROD IN (SELECT R.COD_PROD
                             FROM REL_PROD_CIA R
                             WHERE R.COD_GRUPO_CIA = p_cod_grupo_cia
                             AND R.COD_CIA = v_cod_cia);
          
          --CANTIDAD DE INAFECTOS
          SELECT COUNT(1)
          INTO cantProdInaf
          FROM VTA_COMP_PAGO C,
               VTA_PEDIDO_VTA_DET D
          ...
          AND C.SEC_COMP_PAGO = CASE
                                  ...
                                  ELSE
                                       D.SEC_COMP_PAGO
                                  END
          AND D.COD_PROD NOT IN (SELECT R.COD_PROD
                             FROM REL_PROD_CIA R
                             WHERE R.COD_GRUPO_CIA = p_cod_grupo_cia
                             AND R.COD_CIA = v_cod_cia);
        end if;
          --SI SOLO HAY EXONERADOS
          IF cantProdExon > 0 AND cantProdInaf = 0 THEN
              
              update vta_comp_pago c
              set c.total_exon_e = c.val_neto_comp_pago,
                  c.total_inaf_e = 0
              where c.cod_grupo_cia = p_cod_grupo_cia
              and c.cod_local = p_cod_local
              and c.num_ped_vta = p_num_ped_vta
          ...
          END IF;
          --SI SOLO HAY INAFECTOS
          IF cantProdExon = 0 AND cantProdInaf > 0 THEN
              
              update vta_comp_pago c
              set c.total_exon_e = 0,
                  c.total_inaf_e = c.val_neto_comp_pago
              where c.cod_grupo_cia = p_cod_grupo_cia
              and c.cod_local = p_cod_local
              and c.num_ped_vta = p_num_ped_vta
              and c.sec_comp_pago = p_sec_com_pago;
          ...
          END IF;
          --SI HAY EXONERADOS E INAFECTOS
          IF cantProdExon > 0 AND cantProdInaf > 0 THEN
              
              update vta_comp_pago c
              set c.total_exon_e = c.val_neto_comp_pago - C.TOTAL_INAF_E
              where c.cod_grupo_cia = p_cod_grupo_cia
              and c.cod_local = p_cod_local
              and c.num_ped_vta = p_num_ped_vta
              and c.sec_comp_pago = p_sec_com_pago;
       ...
       ELSE
          --COMPROBANTE DE PRODUCTOS AFECTOS
          update vta_comp_pago c
          set c.Total_Grav_e = (C.VAL_NETO_COMP_PAGO - C.VAL_PERCEP_COMP_PAGO)  - C.VAL_IGV_COMP_PAGO,
              C.TOTAL_IGV_E = C.VAL_IGV_COMP_PAGO,
              C.TOTAL_INAF_E = 0,
              C.TOTAL_EXON_E = 0
          where c.cod_grupo_cia = p_cod_grupo_cia
          and c.cod_local = p_cod_local
  ...
  BEGIN 
   ...
   -- dubilluz 13.01.2015    
    ...
    INSERT INTO TMP_DOCUMENTO_ELECTRONICOS 
      SELECT REPLACE((REPLACE((EXTRACTVALUE(xt.column_value, 'e')),'Ã','&')),'Ë','<') VAL ,'9','C','N','N'
        FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' ||
                           REPLACE( REPLACE((REPLACE((
        SELECT 
    ...
    INSERT INTO TMP_DOCUMENTO_ELECTRONICOS 
      SELECT REPLACE((REPLACE((EXTRACTVALUE(xt.column_value, 'e')),'Ã','&')),'Ë','<') VAL ,'9','C','S','N'
        FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' ||
                           REPLACE( REPLACE((REPLACE((      
        SELECT 
          CASE 
    ...
    \*** MONTOS TOTALES **\
    INSERT INTO TMP_DOCUMENTO_ELECTRONICOS 
      SELECT REPLACE((REPLACE((EXTRACTVALUE(xt.column_value, 'e')),'Ã','&')),'Ë','<') VAL ,'9','D','N','N'
...
END;
  ...
  FUNCTION F_VAR_TRM_GET_MSJ_PERSONALIZA(cGrupoCia      VARCHAR2,
    ...
    v_vPES_DT VARCHAR2(2000) ;
    
    vCodTipoLocal MAE_TIP_LOCAL_VENTA.COD_TIP_LOCAL_VENTA%TYPE;
    vDireccionCP VARCHAR2(500);
  BEGIN
    -- KMONCADA 27.04.2016 ARCANGEL - CENTRO MEDICO
    SELECT I.COD_TIP_LOCAL_VENTA
    INTO vCodTipoLocal
    FROM VTA_PEDIDO_VTA_CAB CAB,
         VTA_IMPR_IP I
    WHERE CAB.COD_GRUPO_CIA = cGrupoCia
    AND CAB.COD_LOCAL = cCodLocal
    AND CAB.NUM_PED_VTA = cNumPedidoVta
    AND I.COD_GRUPO_CIA = CAB.COD_GRUPO_CIA
    AND I.COD_LOCAL = CAB.COD_LOCAL
    AND I.IP = CAB.IP_PC;
      
    
    BEGIN
      SELECT A.DIRECCION_LOCAL_VENTA
      INTO vDireccionCP
      FROM AUX_PBL_LOCAL_TIP_VENTA A
      WHERE A.COD_GRUPO_CIA = cGrupoCia
      AND A.COD_LOCAL = cCodLocal
      AND A.COD_TIP_LOCAL_VENTA = vCodTipoLocal;
    EXCEPTION 
      WHEN OTHERS THEN
        SELECT L.DIREC_LOCAL_CORTA
        INTO vDireccionCP
        FROM PBL_LOCAL L
        WHERE L.COD_GRUPO_CIA = cGrupoCia
        AND L.COD_LOCAL = cCodLocal;
    END;
    --vDireccionCP := NVL(cCodLocal,' ')||' - '||NVL(vDireccionCP, ' ');
  
    v_vPES_DT := v_vPES_DT || 'PE|CODLOCAL|'||cCodLocal || CHR(13);
    
     SELECT --v_vPES_DT || 'PE|DIRLOCAL|' || TRIM(A.DIREC_LOCAL_CORTA) || CHR(13)
            v_vPES_DT || 'PE|DIRLOCAL|' || TRIM(vDireccionCP) || CHR(13)
 ...
 -- 23.12.2014 LTAVARA 
   ...
   -- FIN 23.12.2014 LTAVARA
    -- KMONCADA 27.04.2016 ARCANGEL - CENTRO MEDICO
    SELECT  v_vPES_DT 
            || 'PE|Hora|' || TO_CHAR(CAB.FEC_CREA_COMP_PAGO,'HH24:MI:SS') || CHR(13)
            || 'PE|Caja|' || CAJ.NUM_CAJA_PAGO || CHR(13)
            || 'PE|Cajero|' || CAB.USU_CREA_COMP_PAGO || CHR(13)
            || 'PE|MontoPercepcion|' || TRIM(TO_CHAR((NVL(CAB.VAL_PERCEP_COMP_PAGO,0)),'99990.00')) || CHR(13)
    INTO v_vPES_DT
    FROM VTA_COMP_PAGO CAB,
         CE_MOV_CAJA CAJ 
    WHERE CAB.COD_GRUPO_CIA = cGrupoCia
    AND CAB.COD_LOCAL = cCodLocal
    AND CAB.NUM_PED_VTA = cNumPedidoVta
    AND CAB.SEC_COMP_PAGO = cSecCompPago
    AND CAJ.COD_GRUPO_CIA = CAB.COD_GRUPO_CIA
    AND CAJ.COD_LOCAL = CAB.COD_LOCAL
    AND CAJ.SEC_MOV_CAJA = CAB.SEC_MOV_CAJA
    AND CAJ.TIP_MOV_CAJA = 'A';
...
--   vTextoPtoInicial     VARCHAR2(200);  --ASOSA - 08/05/2015 - PTOSYAYAYAYA
  ...
  CURSOR curCabecera(vDireccionCP IN VARCHAR2) is
       ...
       SELECT REPLACE((REPLACE((EXTRACTVALUE(xt.column_value, 'e')),'Ã','&')),'Ë','<') VAL --, '9','I','N','N'
        FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' ||
           ...
           SELECT
              ...
              -- KMONCADA 26.04.2016 ARCANGEL - CENTROS MEDICOS 
              -- NVL(PED.COD_LOCAL,' ')||' - '||NVL(LOC.DIREC_LOCAL_CORTA, ' ') ||'@' ||
              vDireccionCP ||'@' ||
              CASE 
               WHEN COMP.TIP_COMP_PAGO IN (COD_TIP_COMP_BOLETA, COD_TIP_COMP_TICKET) THEN 'BOLETA ELECTRONICA'
              ...
              FARMA_UTILITY.GET_T_COMPROBANTE_2(COMP.COD_TIP_PROC_PAGO, COMP.NUM_COMP_PAGO_E, COMP.NUM_COMP_PAGO) ||
              CASE 
               WHEN vReimprime = 'S' OR vReimprime = 'S' THEN '@'||'COPIA DE COMPROBANTE'
              END||'@'||
              'FECHA DE EMISION: '||NVL(TO_CHAR(COMP.FEC_CREA_COMP_PAGO, 'dd/mm/yyyy HH24:MI:SS'), ' ') ||
              -- CONVENIOS CON GUIA
              CASE 
...
-- KMONCADA 18.08.2015 REDONDEO AL TERCER DECIMAL PARA PRECION DE PRECIO UNITARIO
  CURSOR curDetalleComprobante IS
    SELECT
       ...
       LPAD(DETALLE.PREC_UNIT, 9, ' ') ||
       -- DSCTO
       LPAD(DETALLE.DSCTO, 10, ' ') ||
       -- IMPORTE
       LPAD(DETALLE.SUBTOTAL, 9, ' ') ITEM
    ...
    FROM (
      SELECT 
        ...
        DET_CP.CANTIDAD,
        CASE 
          WHEN DET_CP.CAMBIA='S' AND NVL(TAB.LLAVE_TAB_GRAL,'S')='S' THEN
            TRIM(TO_CHAR(ROUND((((DET_CP.STOT_NRO  - DET_CP.DCTO_NRO)/DET_CP.CANT_NRO)+0.0005),3),'999,999,990.000'))
          ELSE
            DET_CP.PREC_UNIT
        END PREC_UNIT,
        DET_CP.DSCTO,
      ...
      FROM (
          SELECT
              DET_PREV.COD_PROD,
              DET_PREV.DESCRIPCION,
              DET_PREV.UNID_PRESENT,
              DET_PREV.LAB,
              DET_PREV.CANTIDAD,
              DET_PREV.PREC_UNIT,
              DET_PREV.DSCTO,
              DET_PREV.SUBTOTAL,
              TO_NUMBER(DET_PREV.CANTIDAD,'999990.000') CANT_NRO,
              TO_NUMBER(DET_PREV.PREC_UNIT,'999990.000') PU_NRO,
              TO_NUMBER(NVL(TRIM(DET_PREV.DSCTO),'0'),'999990.000') DCTO_NRO,
              TO_NUMBER(DET_PREV.SUBTOTAL,'999990.000') STOT_NRO,
              CASE 
                WHEN TO_NUMBER(DET_PREV.PREC_UNIT,'999990.000') > 0 AND ((TO_NUMBER(DET_PREV.CANTIDAD,'999990.000')  * TO_NUMBER(DET_PREV.PREC_UNIT,'999990.000')) + TO_NUMBER(NVL(TRIM(DET_PREV.DSCTO),'0'),'999990.000') )<TO_NUMBER(DET_PREV.SUBTOTAL,'999990.000') THEN
                  'S'
                ELSE
                  'N' 
              END CAMBIA
                       ...
                       TRIM(TO_CHAR(NVL(DET.VAL_PREC_VTA_UNIT_E, 0),'999,999,990.000')) PREC_UNIT,
                        CASE
                          WHEN FARMA_LEALTAD.IND_IMPR_PUNTOS_REDI(vCodGrupoCia_in, vCodLocal_in, vAuxNumPedVta) = 'S' THEN
                            CASE
                              --WHEN ABS(NVL(DET.AHORRO_CAMP, 0)) = 0 THEN
                              WHEN ABS(NVL(DET.AHORRO, 0) - NVL(DET.AHORRO_PUNTOS, 0)) = 0 THEN
                                ' '
                              --WHEN ABS(NVL(DET.AHORRO_CAMP, 0)) > 0 THEN
                              WHEN ABS(NVL(DET.AHORRO, 0) - NVL(DET.AHORRO_PUNTOS, 0)) > 0 THEN
                                --TRIM(TO_CHAR((ABS(NVL(DET.AHORRO_CAMP, 0)) * (-1)), '999,999,990.000'))
                                -- KMONCADA 11.08.2015 NUEVO CALCULO PARA OBTENER PTOS EXTRAS
                                TRIM(TO_CHAR((ABS(NVL(DET.AHORRO, 0) - NVL(DET.AHORRO_PUNTOS, 0)) * (-1)), '999,999,990.000'))
                            END
                          ELSE
                            CASE
                              WHEN ABS(NVL(DET.AHORRO, 0)) = 0 THEN
            ...
            pbl_tab_gral TAB
            where id_tab_gral=547
      )DETALLE;
  
  filaCurDetalle curDetalleComprobante%ROWTYPE;
  
  CURSOR curMontoTotal IS
    SELECT REPLACE((REPLACE((EXTRACTVALUE(MONTOS_TOTAL.column_value, 'e')),'Ã','&')),'Ë','<') VAL 
      FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' || REPLACE( REPLACE((REPLACE((
        ...
        SELECT  
          -- MONTO TOTALES INFO CLIENTE
            ...
            CASE 
              WHEN NVL(PAGO.VAL_REDONDEO_COMP_PAGO,0) + NVL(PAGO.VAL_REDONDEO_PERCEPCION,0)!=0 THEN
                 LPAD('REDONDEO: S/.',13)||LPAD(TRIM(TO_CHAR((ABS(NVL(PAGO.VAL_REDONDEO_COMP_PAGO,0)+ NVL(PAGO.VAL_REDONDEO_PERCEPCION,0) )*(-1)),'999,999,990.00')),7)
              ELSE
                ' '   
            END ||
            
            LPAD('TOTAL A PAGAR: S/.',25)||LPAD(TRIM(TO_CHAR(ABS(NVL(PAGO.VAL_NETO_COMP_PAGO,0)+NVL(PAGO.VAL_REDONDEO_COMP_PAGO,0)+ NVL(PAGO.VAL_REDONDEO_PERCEPCION,0)),'999,999,990.00')),15) || '@' ||
          ...
          CASE 
            WHEN NVL(ABS(PAGO.VAL_NETO_COMP_PAGO) + (CASE 
              ...
              LPAD('IMPORTE TOTAL: S/.',20)|| 
                   LPAD(TRIM(TO_CHAR(NVL(ABS(
                     (PAGO.VAL_NETO_COMP_PAGO - NVL(PAGO.VAL_PERCEP_COMP_PAGO,0))
                     +PAGO.VAL_REDONDEO_COMP_PAGO
                   ),0),'999,999,990.00')),15)|| '@'
              -- KMONCADA 22.12.2014 MONTOS YA NO SE MOSTRARAN AL 100%
              /*CASE WHEN vCopagoEmpresa = 'S' THEN
                              LPAD(TRIM(TO_CHAR(NVL(ABS(PAGO.VAL_NETO_COMP_PAGO) + (CASE 
           ...
           CASE WHEN vCopagoEmpresa = 'S' THEN 
                         ...
                         */
         -- END 
          CASE WHEN NVL(PAGO.VAL_PERCEP_COMP_PAGO,0)!=0 AND PAGO.TIP_COMP_PAGO != COD_TIP_COMP_NOTA_CRED THEN
            LPAD('PERCEPCION-'||PAGO.VAL_PCT_PERCEPCION||'%: S/.',25)||LPAD(TRIM(TO_CHAR(NVL(ABS(PAGO.VAL_PERCEP_COMP_PAGO),0),'999,999,990.00')),15) || '@' 
          END || '-'
              FROM VTA_COMP_PAGO PAGO
              WHERE PAGO.COD_GRUPO_CIA = vCodGrupoCia_in
              AND PAGO.COD_LOCAL = vCodLocal_in
              AND PAGO.NUM_PED_VTA= vNumPedVta_in
              AND PAGO.SEC_COMP_PAGO = vSecCompPago_in
      ),'&','Ã')),'<','Ë'),'@','</e><e>') ||'</e></coll>'),'/coll/e'))) MONTOS_TOTAL;  
          ...
          SELECT 
            ...
            CASE 
              WHEN NVL(A.LLAVE_TAB_GRAL,'A') = 'A' THEN
                'Ahorraste S/.' || trim(TO_CHAR(ROUND(valAhorroTotal + valDsctoPedido,2),'999,990.00')) ||' ' || FARMA_PUNTOS.F_CHAR_GET_MSJ_TIEMPO_AHORRO
              ELSE
  ...
  CURSOR curMensaje(vTexto VARCHAR2,vSeparacion VARCHAR2) IS
     SELECT REPLACE((REPLACE((EXTRACTVALUE(xt.column_value, 'e')),'Ã','&')),'Ë','<') VAL 
            FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' || REPLACE( REPLACE((REPLACE((
            vTexto
        ),'&','Ã')),'<','Ë'),vSeparacion,'</e><e>') ||'</e></coll>'),'/coll/e'))) xt;
  ...
  vImporteSinRedimir VTA_PEDIDO_VTA_DET.VAL_PREC_TOTAL%TYPE;
   ...
   v_cIndReimp CHAR(1);
   vDsctoTotalPedido VTA_PEDIDO_VTA_DET.AHORRO%TYPE;
   -- KMONCADA 20.04.2016 INDICADOR DE CP AFECTO A PERCEPCION
   vCompPagoIsConPercepcion CHAR(1);
   
   -- KMONCADA 26.04.2016 ARCANGEL CENTRO MEDICO
   vCodTipoLocal PBL_LOCAL.COD_TIP_LOCAL_VENTA%TYPE;
   vDireccionCP VARCHAR2(200);
  BEGIN 
    -- KMONCADA 27.04.2016 ALBIS - ARCANGEL
    SELECT I.COD_TIP_LOCAL_VENTA
    INTO vCodTipoLocal
    ...
    EXCEPTION 
      WHEN OTHERS THEN
        ...
        INTO vDireccionCP
        FROM PBL_LOCAL L
        WHERE L.COD_GRUPO_CIA = vCodGrupoCia_in
        AND L.COD_LOCAL = vCodLocal_in;
    END;
    vDireccionCP := NVL(vCodLocal_in,' ')||' - '||NVL(vDireccionCP, ' ');
    ...
    select case
           ...
           CASE 
             WHEN NVL(C.VAL_PERCEP_COMP_PAGO,0)!=0 THEN 'S'
             ELSE 'N'
           END
    into   nPermiteImp,
           vCompPagoIsConPercepcion
    from   vta_comp_pago c
    where c.cod_grupo_cia = vCodGrupoCia_in
    and   c.cod_local = vCodLocal_in
    and   c.num_ped_vta = vNumPedVta_in
...
--    delete TMP_DOCUMENTO_ELECTRONICOS;  
    ...
    -- KMONCADA AHORRO TOTAL DEL PEDIDO SE CONSIDERA AHORRO POR PTOS AHORRO CAMPAÑA Y AHORRO PROMOCIONES
    -- KMONCADA AHORRO EXTRA ES EL AHORRO TOTAL MENOS EL AHORRO POR PTOS
    SELECT SUM(NVL(DET.AHORRO, 0)), 
           --SUM(NVL(DET.AHORRO_CAMP, 0)),
           SUM(NVL(DET.AHORRO, 0) - NVL(DET.AHORRO_PUNTOS,0)),
...
/** INICIO DE COMPROBANTE **/    
    ...
    -- KMONCADA 03.07.2015 OBTIENE AHORRO TOTAL DEL PEDIDO
    SELECT SUM(NVL(DET.AHORRO, 0))
      INTO vDsctoTotalPedido
     FROM VTA_PEDIDO_VTA_DET DET
     WHERE DET.COD_GRUPO_CIA = vCodGrupoCia_in
       AND DET.COD_LOCAL = vCodLocal_in
       AND DET.NUM_PED_VTA = vNumPedVta_in;
    
      OPEN curEncabezadoPtos(vValorAhorroSoles, vDsctoTotalPedido);
        LOOP
...
\*            WHEN vCopagoEmpresa = 'S' AND vDescuentoNC > 0 THEN
    ...
    -- NOTA DE CREDITO NO MOSTRARA FORMA DE PAGO
      ...
      IF vValor = 'S' THEN
        BEGIN
          SELECT NVL(LLAVE_TAB_GRAL,NULL)
          INTO vValor
          FROM PBL_TAB_GRAL
          WHERE ID_TAB_GRAL = 608;
        EXCEPTION 
...
--         INSERT INTO TMP_DOCUMENTO_ELECTRONICOS VALUES (vPDF417, '9','P','N','N');
      ...
      -- KMONCADA 20.04.2016 MENSAJE DE COMPROBANTE AFECTO A PERCEPCION
      IF vCompPagoIsConPercepcion = 'S' THEN
        SELECT NVL(TRIM(L.LLAVE_TAB_GRAL),NULL)
        INTO vValor
        FROM PBL_TAB_GRAL L
        WHERE L.ID_TAB_GRAL = 610;
      ...
      -- KMONCADA 20.04.2016 MENSAJE DE AGENTE DE PERCEPCION
      SELECT NVL(C.IND_AGENTE_PERCEPCION,'N')
      INTO vValor
      FROM PBL_LOCAL L,
           PBL_CIA C
      WHERE C.COD_CIA = L.COD_CIA
      AND L.COD_GRUPO_CIA = vCodGrupoCia_in
      AND L.COD_LOCAL = vCodLocal_in;
      ...
      SELECT 'Guarda tu voucher. Es el sustento para validar tu compra. '||
             'Representación impresa de la '||
                   (CASE 
                     ...
                     WHEN vTipoDocumento IN (COD_TIP_COMP_FACTURA, COD_TIP_COMP_TICKET_FA) THEN 'FACTURA ELECTRONICA '
                     WHEN vTipoDocumento = COD_TIP_COMP_NOTA_CRED THEN 'NOTA DE CREDITO ELECTRONICA '
                    END)||'puede ser consultado en '||
             DESC_LARGA ||'. '||
             'No se aceptan devoluciones de dinero. '||
             'Cambio de mercadería únicamente dentro de las 48 horas siguientes a la compra. Indispensable presentar comprobante.'
      INTO vValor
      ...
      SELECT REPLACE((REPLACE((EXTRACTVALUE(xt.column_value, 'e')),'Ã','&')),'Ë','<') VAL ,'9','I','N','N'
       ...
       (SELECT  DESC_LARGA FROM PTOVENTA.PBL_TAB_GRAL  WHERE ID_TAB_GRAl='604')||--'@'||
       ' No se aceptan devoluciones de dinero. '||--'@'||
       'Cambio de mercadería únicamente dentro '||--'@'||
       'de las 48 horas siguientes a la compra. '||--'@'||
       'Indispensable presentar comprobante.' ||'@'\*||
       '-'||'@'*\
      FROM DUAL
...
--        INSERT INTO TMP_DOCUMENTO_ELECTRONICOS VALUES ('-', '9','I','N','N');
  ...
  FUNCTION F_CHAR_TRAMA_ALBIS
    ...
    vRspta CHAR(1);
  BEGIN
    SELECT NVL(L.LLAVE_TAB_GRAL,'N')
    INTO vRspta
    FROM PBL_TAB_GRAL L
    WHERE L.ID_TAB_GRAL = 625;
