create or replace package PTOVENTA_CONTROL_ASISTENCIA is
  ...
  g_COD_ROL_TECNICO CHAR(3) := '010';
  g_COD_ROL_ADM_LOCAL CHAR(3) := '011';
  g_COD_ROL_CTRL_INTERNO CHAR(3) := '058';
  g_COD_ROL_GNC CHAR(3) := '059';
  g_COD_ROL_DERMO CHAR(3) := '060';
  ...
  /* ****************************************************************** */
  --Descripcion: Indicar si esta activa la funcionalidad de
--                 mostrar la necesidad de horario.
  --Fecha       Usuario        Comentario
  --27/10/2015  DUBILLUZ          Creacion 
  --03/02/2015  ASOSA             Modificación 
  /* ****************************************************************** */          
  ...
  procedure CTRL_P_VALIDA_APROBACION(cCodGrupoCia_in   IN CHAR,
                                     ...
                                     cSecUsu_in        IN CHAR);                                        
  /* ****************************************************************** */
  --Descripcion: Activar nueva marcacion
  --Fecha       Usuario        Comentario
  --19/01/2016  ASOSA          Creacion 
  /* ****************************************************************** */            
  procedure CTRL_P_ACTIVA_NUEVA_MARC(cCodGrupoCia_in   IN CHAR default '001',
                                   cCodCia_in        IN CHAR default '001',
                                   cCodLocal_in      IN CHAR default '001',
                                   cIndicador_in     IN CHAR default 'S');
                                   
  /* ****************************************************************** */
  --Descripcion: listar personas que tienen aprobadas para trabajar cierta cantidad de horas superiores de 48hrs durante una semana especifica.
  --Fecha       Usuario        Comentario
  --05/02/2016  ASOSA          Creacion 
  /* ****************************************************************** */            
  FUNCTION CTRL_F_GET_LIST_HRS_PAPROB(cCodGrupoCia_in IN CHAR default '001',
                                    cCodCia_in      IN CHAR default '001',
                                    cCodLocal_in    IN CHAR default '001',
                                    vFechIni_in     IN VARCHAR2,
                                    vFechFin_in     IN VARCHAR2)
  RETURN FarmaCursor;

  /* ****************************************************************** */
  --Descripcion: Listar colaboradores del local, solo vendedores, QF, GNC, dermoconsultores y control interno.
  --Fecha       Usuario        Comentario
  --15/02/2016  ASOSA          Creacion 
  /* ****************************************************************** */              
  FUNCTION CTRL_F_GET_LIST_COLABORADORES(cCodGrupoCia_in IN CHAR default '001',
                                      cCodCia_in      IN CHAR default '001',
                                      cCodLocal_in    IN CHAR default '001') 
  RETURN FarmaCursor;
      ...
      ELSE
        IF (v_ENTRADA IS NOT NULL AND v_SALIDA IS NOT NULL AND
          ...
          IF TRUNC(SYSDATE) = TRUNC(v_dDiaTrabajo) THEN
            ...
            v_Flag := g_IND_SALIDA;
          ELSE
            SELECT TO_NUMBER(to_char((TO_DATE(TO_CHAR(SYSDATE,
                                                     'dd/MM/YYYY HH24:MI'),
                                             'dd/MM/YYYY HH24:MI') -
                                    TO_DATE(TO_CHAR(v_ENTRADA2,
                                                     'dd/MM/YYYY HH24:MI'),
                                             'dd/MM/YYYY HH24:MI')) * 24 - 1,
                                    '9990.00'),
                            '9990.00')
            INTO v_nCantHrs
           FROM DUAL;
...
--********************************************************************************
  ...
  BEGIN
    ...
    v_MaxHoraSemana := nCantHrs_in;-- CTRL_F_GET_MAX_HORA_SEM;
...
--********************************************************************************
  ...
  BEGIN
    ...
    --ANTERIORMENTE ESTA VALIDACION DEBIA REALIZARSE PERO CON 
    --              LA GENERACION DE SOLICITUDES ESTO SE ACTUALIZA 
    --              AUTOMATICAMENTE.
    --COSAS QUE PASAN CUANDO SE CAMBIA Y CAMBIA
    v_DebeValidar := 'N';
...
--********************************************************************************
  ...
  BEGIN
    ...
    IF v_fecha IS NULL THEN
      ...
      select TO_NUMBER(to_char(sysdate,'d'),'9')
      into v_nDiaSemana
      from dual;

      select to_char(trunc(sysdate - v_nDiaSemana + 2),'dd/MM/yyyy')
      into v_cFechaIni
      from dual;

    ELSE

      select to_char(trunc(v_fecha) + 7, 'dd/MM/yyyy')
      into v_cFechaIni
      from dual;

    END IF;

    select to_char(to_Date(v_cFechaIni, 'dd/MM/yyyy') + 6, 'dd/MM/yyyy')
    into v_cFechaFin
    from dual;

    OPEN v_curHorario FOR
      SELECT v_cFechaIni || 'Ã' ||
             v_cFechaFin
             /*'08/02/2016' || 'Ã' ||
             '14/02/2016'*/
      FROM DUAL;
...
--********************************************************************************
  FUNCTION CTRL_P_AVISO_FIN_HORARIO(cCodGrupoCia_in     IN CHAR,
     ...
     nDiasAntesAviso number(3);
     vv_filaHorario pbl_cab_horario%rowtype;
     vExiste number(3) := 0;
     nCantHorarioActual number(3) := 0;
  ...
  BEGIN
    ...
    END;
      ...
      begin
        ...
        select count(1)
        INTO   nCantHorarioActual
        from   pbl_cab_horario a
        where  trunc(sysdate) between fec_inicio and fec_fin;

        IF nCantHorarioActual = 0 THEN

            vResultado := 'Recuerde que está retrasado(a) con la planificación@ de sus horarios.'||
        ...
        ELSE
            ...
            if vExiste = 0 then
              if vv_filaHorario.Fec_Fin-trunc(sysdate) <=
                nDiasAntesAviso then
                 vResultado := 'Recuerda que se solo faltan '||
                 (vv_filaHorario.Fec_Fin-trunc(sysdate)) || ' días ' ||
                 '@ para finalizar el horario registrado de la semana.@'||
                 'Por favor planificar el horario de la próxima semana.';
              else
                 vResultado := 'S';
              end if;
            else
...
procedure CTRL_P_ACTIVA_NUEVA_MARC(cCodGrupoCia_in   IN CHAR default '001',
                                   cCodCia_in        IN CHAR default '001',
                                   cCodLocal_in      IN CHAR default '001',
                                   cIndicador_in     IN CHAR default 'S') is
 v_nId01 number(3) := 724;
 v_nId02 number(3) := 737;
 v_nId03 number(3) := 573;
BEGIN

  UPDATE PBL_TAB_GRAL A
  SET A.LLAVE_TAB_GRAL = upper(cIndicador_in)
  WHERE  A.ID_TAB_GRAL = v_nId01;

  UPDATE PBL_TAB_GRAL A
  SET A.LLAVE_TAB_GRAL = upper(cIndicador_in)
  WHERE  A.ID_TAB_GRAL = v_nId02;

  UPDATE PBL_TAB_GRAL A
  SET A.LLAVE_TAB_GRAL = upper(cIndicador_in)
  WHERE  A.ID_TAB_GRAL = v_nId03;
  ...
  BEGIN
    OPEN v_curHorario FOR
      SELECT C.SEC_USU_LOCAL || 'Ã' ||
             A.CANT_HORAS
        FROM PBL_HE_PRE_APROB A,
             PBL_SEMANA B,
             PBL_USU_LOCAL C
       WHERE A.ID_SEMANA = B.ID_SEMANA
       AND A.COD_GRUPO_CIA = B.COD_GRUPO_CIA
       ...
       AND A.cod_local = cCodLocal_in
       AND B.FEC_INI = vFechIni_in
       AND B.FEC_FIN = vFechFin_in
       AND A.ESTADO = g_IND_ACTIVO;
    RETURN v_curHorario;
  END;
...
FUNCTION CTRL_F_GET_LIST_COLABORADORES(cCodGrupoCia_in IN CHAR default '001',
  ...
  BEGIN
    OPEN v_curHorario FOR
      select T.SEC_USU_LOCAL || 'Ã' ||
             T.login_usu || 'Ã' ||
             T.DNI_USU
      from PBL_USU_LOCAL t,
           Pbl_Rol_Usu G
      WHERE T.COD_GRUPO_CIA = G.COD_GRUPO_CIA
      AND T.COD_LOCAL = G.COD_LOCAL
      AND T.SEC_USU_LOCAL = G.SEC_USU_LOCAL
      AND G.COD_ROL IN (g_COD_ROL_TECNICO,
                       g_COD_ROL_ADM_LOCAL,
                       g_COD_ROL_CTRL_INTERNO,
                       g_COD_ROL_GNC,
                       g_COD_ROL_DERMO)
      AND G.SEC_USU_LOCAL BETWEEN '001' AND '899'
      AND T.EST_USU = g_IND_ACTIVO
      GROUP BY T.SEC_USU_LOCAL,
               T.login_usu,
               T.DNI_USU;
    RETURN v_curHorario;
  END;
