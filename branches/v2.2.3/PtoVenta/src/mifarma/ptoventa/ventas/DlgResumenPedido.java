...
public class DlgResumenPedido extends JDialog {
        ...
        try {
           ...
           FarmaUtility.moveFocus(txtDescProdOculto);*/
                    ...
                    */
                    ...
                    //vEjecutaAccionTeclaResumen = false;
                        if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,"Está seguro que Desea salir del pedido?"))
                        {
    ...
    private void calculaTotalesPedido() {
        ...
        log.info("ajustando monto a 2 cifras totalNetoRedondeado:" + 
                           totalNetoRedondeado);
        log.info("ajustando monto a 2 cifras totalNetoRedondeado 2:" + 
    ...
    private synchronized void grabarPedidoVenta(String pTipComp) {
        ...
        if (VariablesVentas.vArrayList_Cupones.size() > 0) {
            for (int i = 0; i < VariablesVentas.vArrayList_Cupones.size(); i++) {
                ...
                flagCampAutomatico =
                        (mapaCupon.get("COD_CAMP_CUPON").toString().indexOf("A") != -1 || 
        ...
        if (aceptaCupones) {
            ...
            {
                try {
                    //Se obtiene tipo de comrpobante de la relacion maquina - impresora
                    ...
                    double vValorMaximoCompra = 
                        Double.parseDouble(DBVentas.getMontoMinimoDatosCliente()); 
                        ...
                        VariablesVentas.vIndObligaDatosCliente) // Pedir datos si pase del monto indicado
                    {
                        ...
                        if (!FarmaVariables.vAceptar) {
                            ...
                            if (!FarmaVariables.vTipCaja.equalsIgnoreCase(ConstantsPtoVenta.TIP_CAJA_MULTIFUNCIONAL))
                                if (!com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this, "Esta seguro de realizar el pedido?")) {
                                    FarmaUtility.liberarTransaccion();
                                    return;
                                }
                        }
...
(ArrayList)(VariablesVentas.vArrayList_Prod_Promociones.get(i));
    ...
    private void agregarComplementarios(String vkF) {
        ...
        for (int i = 0; i <= tblProductos.getRowCount(); i++) {
            ...
            log.debug("FarmaVariables.vAceptar : " + 
                               FarmaVariables.vAceptar);
...
//                                               precioVtaOrig) * 100.0, 2);
       ...
       if(!VariablesConvenioBTLMF.vHayDatosIngresadosConvenioBTLMF){                                                                                              
        for (int i = 0; i < VariablesVentas.vArrayList_ResumenPedido.size(); i++) {
            ...
            if (mapCampPrec.size()> 0) {
              ...
              vd_Val_Prec_Prom = FarmaUtility.getDecimalNumber(vs_Val_Prec_Prom)/fraccionPed;
              log.debug("###### TCT 15 : vd_Val_Prec_Prom: "+vd_Val_Prec_Prom);
              
              //&&&&&&&&&&&&&&&& CAMBIAR DATOS DE LINEA DE PRODUCTO X CAMPAÑA DE PRECIO &&&&&&&&&&&&&&&&&&&&&&&&
              if (precioVtaTotal > (vd_Val_Prec_Prom*cantidad) ){
                  
                  dctoVtaMayorAhorro = UtilityVentas.Redondear((1-(vd_Val_Prec_Prom*cantidad)/precioVtaTotal)*100,2);
                  precioVtaTotal = UtilityVentas.Redondear(vd_Val_Prec_Prom* cantidad,3);
    ...
    public void continuarCobroPedido(KeyEvent e) {
        ...
        } else if (mifarma.ptoventa.reference.UtilityPtoVenta.verificaVK_F1(e) || e.getKeyChar() == '+') {                  
