...
--------------------------------------------------------
 ...
 function fn_convenio_planilla(p_cod_convenio varchar2) return varchar2 as
     v_count varchar2(2);
 begin
     select count(1)
       into v_count
       from nuevo.mae_parametro
      where cod_parametro in ('CODCONVBTL', 'CODCONVMF', 'CODCONVFS','CODCONVBZ')
        and trim(val_parametro) = p_cod_convenio;
     return v_count;
 end;
...
BEGIN
  ...
  IF V_CIA = CONS_CIA_MIFARMA THEN
    IF TRIM(pIndNotaCred) ='S' THEN
         ...
         ind_fp_fid_tarjeta,   cod_fp_fid_tarjeta,   cod_cli_conv,   cod_barra_conv,   ind_conv_btl_mf,   name_pc_cob_ped,   ip_cob_ped,
         dni_usu_local,   fec_proceso_rac,   fecha_proceso_anula_rac
         FROM PTOVENTA.VTA_PEDIDO_VTA_CAB@XE_000_2 C
         ...
         ahorro,   porc_dcto_calc,   porc_zan,   ind_prom_automatico,   ahorro_pack,   porc_dcto_calc_pack,   cod_grupo_rep,
         cod_grupo_rep_edmundo,   sec_respaldo_stk,   num_comp_pago,   sec_comp_pago_benef,   sec_comp_pago_empre
         FROM PTOVENTA.VTA_PEDIDO_VTA_DET@XE_000_2
         ...
         (COD_GRUPO_CIA,COD_LOCAL,NUM_PED_VTA,SEC_COMP_PAGO,TIP_COMP_PAGO,NUM_COMP_PAGO,SEC_MOV_CAJA,SEC_MOV_CAJA_ANUL,CANT_ITEM,COD_CLI_LOCAL,NOM_IMPR_COMP,
         DIREC_IMPR_COMP,NUM_DOC_IMPR,VAL_BRUTO_COMP_PAGO,VAL_NETO_COMP_PAGO,VAL_DCTO_COMP_PAGO,VAL_AFECTO_COMP_PAGO,VAL_IGV_COMP_PAGO,VAL_REDONDEO_COMP_PAGO,
         PORC_IGV_COMP_PAGO,USU_CREA_COMP_PAGO,FEC_CREA_COMP_PAGO,USU_MOD_COMP_PAGO,FEC_MOD_COMP_PAGO,FEC_ANUL_COMP_PAGO,IND_COMP_ANUL,NUM_PEDIDO_ANUL,NUM_SEC_DOC_SAP,
         FEC_PROCESO_SAP,NUM_SEC_DOC_SAP_ANUL,FEC_PROCESO_SAP_ANUL,IND_RECLAMO_NAVSAT,VAL_DCTO_COMP,MOTIVO_ANULACION,FECHA_COBRO,FECHA_ANULACION,FECH_IMP_COBRO,
         FECH_IMP_ANUL,TIP_CLIEN_CONVENIO,VAL_COPAGO_COMP_PAGO,VAL_IGV_COMP_COPAGO,NUM_COMP_COPAGO_REF,IND_AFECTA_KARDEX,PCT_BENEFICIARIO,PCT_EMPRESA,IND_COMP_CREDITO,
         TIP_COMP_PAGO_REF, NUM_COMP_PAGO_E, COD_TIP_PROC_PAGO
         )
         SELECT COD_GRUPO_CIA,COD_LOCAL,NUM_PED_VTA,SEC_COMP_PAGO,TIP_COMP_PAGO,NUM_COMP_PAGO,SEC_MOV_CAJA,SEC_MOV_CAJA_ANUL,CANT_ITEM,COD_CLI_LOCAL,
                ...
                PCT_EMPRESA,IND_COMP_CREDITO,TIP_COMP_PAGO_REF, NUM_COMP_PAGO_E, COD_TIP_PROC_PAGO
         FROM PTOVENTA.VTA_COMP_PAGO@XE_000_2
         WHERE NUM_PED_VTA = pNum_Ped_Vta_in
         AND COD_GRUPO_CIA = pCodGrupo_Cia_in
         AND COD_LOCAL = pCod_local_in;
         ...
         DNI_CLI_TARJ
         FROM PTOVENTA.VTA_FORMA_PAGO_PEDIDO@XE_000_2
         WHERE NUM_PED_VTA = pNum_Ped_Vta_in
         AND COD_GRUPO_CIA = pCodGrupo_Cia_in
         AND COD_LOCAL = pCod_local_in;
         ...
         FROM PTOVENTA.CON_BTL_MF_PED_VTA@XE_000_2
         WHERE NUM_PED_VTA = pNum_Ped_Vta_in
         AND COD_GRUPO_CIA = pCodGrupo_Cia_in
         AND COD_LOCAL = pCod_local_in;
    ...
    ELSE
         ...
         ind_fp_fid_tarjeta, cod_fp_fid_tarjeta, cod_cli_conv, cod_barra_conv, ind_conv_btl_mf, name_pc_cob_ped, ip_cob_ped, dni_usu_local,
         fec_proceso_rac, fecha_proceso_anula_rac
         FROM PTOVENTA.RAC_VTA_PEDIDO_VTA_CAB@XE_000_2
         WHERE NUM_PED_VTA = pNum_Ped_Vta_in
         AND COD_GRUPO_CIA = pCodGrupo_Cia_in
         AND COD_LOCAL = pCod_local_in;
         ...
         porc_zan,   porc_dcto_calc_pack,   cod_grupo_rep,   cod_grupo_rep_edmundo,   ahorro_pack,   ind_prom_automatico,
         sec_respaldo_stk,   num_comp_pago,   sec_comp_pago_benef,   sec_comp_pago_empre
         FROM PTOVENTA.RAC_VTA_PEDIDO_VTA_DET@XE_000_2
         WHERE NUM_PED_VTA = pNum_Ped_Vta_in
         AND COD_GRUPO_CIA = pCodGrupo_Cia_in
         AND COD_LOCAL = pCod_local_in;
         ...
         SELECT COD_GRUPO_CIA,COD_LOCAL,NUM_PED_VTA,SEC_COMP_PAGO,TIP_COMP_PAGO,NUM_COMP_PAGO,SEC_MOV_CAJA,SEC_MOV_CAJA_ANUL,CANT_ITEM,COD_CLI_LOCAL,NOM_IMPR_COMP,DIREC_IMPR_COMP,
                ...
                IND_AFECTA_KARDEX,PCT_BENEFICIARIO,PCT_EMPRESA,IND_COMP_CREDITO,TIP_COMP_PAGO_REF, NUM_COMP_PAGO_E, COD_TIP_PROC_PAGO
         FROM PTOVENTA.RAC_VTA_COMP_PAGO@XE_000_2
         WHERE NUM_PED_VTA = pNum_Ped_Vta_in
         AND COD_GRUPO_CIA = pCodGrupo_Cia_in
         AND COD_LOCAL = pCod_local_in;
         ...
         FROM PTOVENTA.RAC_VTA_FORMA_PAGO_PEDIDO@XE_000_2
         WHERE NUM_PED_VTA = pNum_Ped_Vta_in
         AND COD_GRUPO_CIA = pCodGrupo_Cia_in
         AND COD_LOCAL = pCod_local_in;
         ...
         SELECT COD_GRUPO_CIA,COD_LOCAL,NUM_PED_VTA,COD_CAMPO,COD_CONVENIO,COD_CLIENTE,FEC_CREA_PED_VTA_CLI,USU_CREA_PED_VTA_CLI,FEC_MOD_PED_VTA_CLI,USU_MOD_PED_VTA_CLI,DESCRIPCION_CAMPO,NOMBRE_CAMPO,FLG_IMPRIME,
                COD_VALOR_IN
         FROM PTOVENTA.RAC_CON_BTL_MF_PED_VTA@XE_000_2
         WHERE NUM_PED_VTA = pNum_Ped_Vta_in
         AND COD_GRUPO_CIA = pCodGrupo_Cia_in
         AND COD_LOCAL = pCod_local_in;
...
IF V_CIA=CONS_CIA_FASA or V_CIA=CONS_CIA_BTL THEN
    ...
    -- 1 --
         ...
         ind_fp_fid_tarjeta, cod_fp_fid_tarjeta, cod_cli_conv, cod_barra_conv, ind_conv_btl_mf, name_pc_cob_ped, ip_cob_ped, dni_usu_local,
         fec_proceso_rac, fecha_proceso_anula_rac
         FROM PTOVENTA.VTA_PEDIDO_VTA_CAB
         WHERE NUM_PED_VTA = pNum_Ped_Vta_in
         AND COD_GRUPO_CIA = pCodGrupo_Cia_in
         AND COD_LOCAL = pCod_local_in;
...
EXCEPTION
   /* -- Comentado por DJARA, no debe hacer esto -- */
                    ...
                    if (reg.cod_tipodoc = reg_cab.cod_tipo_documento) then
                      if (reg.num_comp_pago != reg_cab.num_documento) then
                        hmeza1.pkg_sincro_vtas_conv.sp_correlativo(a_cia                => reg_cab.cia,
                                                                   ...
                                                                   a_num_documento_e_debe => reg.num_comp_pago);
                      end if;
                    else
...
function SP_GRABA_DOCUMENTOS( pCodGrupo_Cia_in CHAR,
                              ...
                              pIndCreditoSeparado CHAR DEFAULT 'N'
-- KMONCADA 02.10.2014 INDICA SI TIENE QUE DEVOLVER EL CREDITO PARA REALIZAR EL REGISTRO DE LOS DOCUMENTOS
                            ) RETURN CHAR
  AS
    ...
    CURSOR C_CABECERA IS
        ...
        FROM (
              SELECT COD_CIA,
                     ...
                     TIP_PED_VTA, -- KMONCADA 25.09.2014 CONVENIO MIXTO
                     REDONDEO,     -- KMONCADA 07.11.2014 VALIDACION DE MONTO TOTAL
                     NUM_DOCUMENTO_E, -- KMONCADA 02.10.2014 NUMERO DE COMPROBANTE ELECTRONICO
                     FLG_DOCUMENTO_E
                FROM (
                       SELECT (SELECT ML.CIA
                                ...
                                WHERE (L.COD_LOCAL_SAP BETWEEN '001' AND '999' or fch_migracion is not null)
                                  AND L.COD_LOCAL_SAP = R.COD_LOCAL) COD_CIA,
                              (SELECT L.COD_LOCAL
                                 FROM NUEVO.AUX_MAE_LOCAL L
                                WHERE /*COD_LOCAL_SAP BETWEEN '001' AND '599'*/
                                (COD_LOCAL_SAP BETWEEN '001' AND '999' or fch_migracion is not null)
                              ...
                              CASE
                                  ...
                                  -- KMONCADA 25.09.2014 CONVENIO MIXTO
                                  WHEN R.TIP_CLIEN_CONVENIO='1' AND R.IND_COMP_CREDITO='S' AND PKG_DOCUMENTO_MF.IS_CONVENIO_MIXTO(P.COD_GRUPO_CIA, P.COD_LOCAL, P.NUM_PED_VTA)='1' THEN '1'
                              ...
                              P.TIP_PED_VTA TIP_PED_VTA,
                              R.VAL_REDONDEO_COMP_PAGO REDONDEO, -- KMONCADA 07.11.2014 VALIDACION DE MONTO TOTAL DEL DOCUMENTO
                              R.NUM_COMP_PAGO_E NUM_DOCUMENTO_E,
                              R.COD_TIP_PROC_PAGO FLG_DOCUMENTO_E
...
--                               DECODE(R.IND_COMP_CREDITO, 'S', '2', '0') COD_TIPO_VTAFIN,
                               ...
                               P.TIP_PED_VTA,
                               R.VAL_REDONDEO_COMP_PAGO REDONDEO, -- KMONCADA 07.11.2014 VALIDACION DE MONTO TOTAL
                               -- KMONCADA 15.10.2014 COMPROBANTES ELECTRONICOS
                               CP.NUM_COMP_PAGO_E NUM_DOCUMENTO_E,
                               CP.COD_TIP_PROC_PAGO FLG_DOCUMENTO_E
                          FROM BTLPROD.RAC_VTA_COMP_PAGO R
                         INNER JOIN BTLPROD.RAC_VTA_PEDIDO_VTA_CAB P
                           ...
                           and r.sec_comp_pago =
                               (select nvl(decode(nvl(rr.tip_clien_convenio, '1'),
                                                  '1',
                                                  d.sec_comp_pago_benef,
                                                  d.sec_comp_pago_empre),
                                           d.sec_comp_pago_origen)
                                  from btlprod.rac_vta_pedido_vta_det d
                                 inner join btlprod.rac_vta_comp_pago rr
                                    on rr.num_ped_vta = d.num_ped_vta
                                   and rr.cod_grupo_cia = d.cod_grupo_cia
                                   and rr.cod_local = d.cod_local
                                 where d.num_ped_vta = p.num_ped_vta
                                   and d.cod_grupo_cia = p.cod_grupo_cia
                                   and d.cod_local = p.cod_local
                                   and d.sec_ped_vta_det = 1)
    ...
    CURSOR C_DETALLE (A_COD_CIA_DOC varchar2, A_COD_TIPO_DOC varchar2, A_NUM_DOCUMENTO_DOC varchar2) IS
        select *
          from (select btlprod.pkg_local.fn_devuelve_cia(btlprod.pkg_local.fn_get_cod_local(cp.cod_local)) cod_cia,
                       btlprod.pkg_local.fn_get_cod_local(cp.cod_local) cod_local,
                       ...
                       cp.num_comp_pago num_documento,
                       p.cod_producto,
                       --cmr.pkg_util.fn_uf(0, d.cant_atendida, val_frac, 'U') ctd_producto,
                       --cmr.pkg_util.fn_uf(0, d.cant_atendida, val_frac, 'F') ctd_producto_frac,
                       d.val_frac ctd_fracciona,
                       decode(d.val_frac, 1, '0', '1') flg_fraccion,
                       round((d.val_prec_total / (decode(d.val_frac, 1, d.cant_atendida, 0) + (decode(d.val_frac, 1, 0, d.cant_atendida) / d.val_frac))), 2) prc_unit_vta,
                       round((d.val_prec_total / (decode(d.val_frac, 1, d.cant_atendida, 0) + (decode(d.val_frac, 1, 0, d.cant_atendida) / d.val_frac))) * (decode(cp.ind_comp_credito, 'S', decode(cp.pct_beneficiario, 0, 100, cp.pct_beneficiario), cp.pct_beneficiario) / 100), 2) prc_unit_neto_vta,
                       round(((d.val_prec_total / (decode(d.val_frac, 1, d.cant_atendida, 0) + (decode(d.val_frac, 1, 0, d.cant_atendida) / d.val_frac))) * (decode(cp.ind_comp_credito, 'S', decode(cp.pct_beneficiario, 0, 100, cp.pct_beneficiario), cp.pct_beneficiario) / 100)) / (1 + (d.val_igv / 100)), 2) imp_unit_base_imp,
                       ...
                       nvl((select nvl((pd.val_prec_total * (pg.pct_empresa / 100)), 0)
                            ...
                            inner join btlprod.rac_vta_pedido_vta_det pd
                               on pg.cod_grupo_cia = pd.cod_grupo_cia
                              and pg.cod_local = pd.cod_local
                              and pg.num_ped_vta = pd.num_ped_vta
                            where pg.cod_grupo_cia = d.cod_grupo_cia
                              and pg.cod_local = d.cod_local
                              and pg.num_ped_vta = d.num_ped_vta
                              and pg.tip_comp_pago = cp.tip_comp_pago_ref
                              and pg.num_comp_pago = cp.num_comp_copago_ref
                              and pd.cod_prod = d.cod_prod
                              and pd.sec_ped_vta_det = d.sec_ped_vta_det),
                           0) imp_copago,
                       ...
                       d.val_frac
                  from btlprod.rac_vta_pedido_vta_det d
                 inner join cmr.aux_mae_producto_com p
                    on d.cod_prod = p.cod_codigo_sap
                 inner join btlprod.rac_vta_comp_pago cp
                    on cp.cod_grupo_cia = d.cod_grupo_cia
                   and cp.cod_local = d.cod_local
                   and cp.num_ped_vta = d.num_ped_vta
                   and d.sec_comp_pago_benef = cp.sec_comp_pago
                  left join ptoventa.lgt_prod l
                    on l.cod_prod = d.cod_prod
                 inner join cmr.mae_producto_com pc
                 ...
                 where cp.tip_clien_convenio = 1 --DOCUMENTO DE BENEFICIARIO
                   ...
                   and l.cod_grupo_cia='001'
                union all
                select btlprod.pkg_local.fn_devuelve_cia(btlprod.pkg_local.fn_get_cod_local(cp.cod_local)) cod_cia,
                       btlprod.pkg_local.fn_get_cod_local(cp.cod_local) cod_local,
                       (select cod_tipodoc from btlprod.aux_tipo_doc_conv where tip_comp_pago = cp.tip_comp_pago and cod_grupo_cia = cp.cod_grupo_cia) cod_tipo_documento,
                       cp.num_comp_pago num_documento,
                       p.cod_producto,
                       ...
                       decode(d.val_frac, 1, '0', '1') flg_fraccion,
                       round((d.val_prec_total / (decode(d.val_frac, 1, d.cant_atendida, 0) + (decode(d.val_frac, 1, 0, d.cant_atendida) / d.val_frac))), 2) prc_unit_vta,
                       round((d.val_prec_total / (decode(d.val_frac, 1, d.cant_atendida, 0) + (decode(d.val_frac, 1, 0, d.cant_atendida) / d.val_frac))) * (decode(cp.ind_comp_credito, 'S', decode(cp.pct_empresa, 0, 100, cp.pct_empresa), cp.pct_empresa) / 100), 2) prc_unit_neto_vta,
                       round(((d.val_prec_total / (decode(d.val_frac, 1, d.cant_atendida, 0) + (decode(d.val_frac, 1, 0, d.cant_atendida) / d.val_frac))) * (decode(cp.ind_comp_credito, 'S', decode(cp.pct_empresa, 0, 100, cp.pct_empresa), cp.pct_empresa) / 100)) / (1 + (d.val_igv / 100)), 2) imp_unit_base_imp,
                       (d.val_prec_total * (decode(cp.ind_comp_credito, 'S', decode(cp.pct_empresa, 0, 100, cp.pct_empresa), cp.pct_empresa) / 100)) mto_subtotal,
                       nvl(d.porc_dcto_calc, 0) pct_descuento,
                       d.val_igv / 100 pct_igv,
                       round(( ((d.val_prec_total ) * (decode(cp.ind_comp_credito, 'S', decode(cp.pct_empresa, 0, 100, cp.pct_empresa), cp.pct_empresa) / 100)) 
					           / (1 + (d.val_igv / 100))) * (d.val_igv / 100), 2) mto_igv,
					   d.sec_ped_vta_det num_item,
                       pc.des_producto,
         ...
         where cod_cia = a_cod_cia_doc
           and cod_tipo_documento = a_cod_tipo_doc
           and num_documento = a_num_documento_doc
        ...
        select *
          from (select btlprod.pkg_local.fn_devuelve_cia(btlprod.pkg_local.fn_get_cod_local(cp.cod_local)) cod_cia,
                 ...
                 inner join btlprod.rac_vta_comp_pago cp
                   ...
                   and cp.cod_local = d.cod_local
                   and cp.num_ped_vta = d.num_ped_vta
                   and nvl(d.sec_comp_pago_benef, d.sec_comp_pago_origen) =
                       cp.sec_comp_pago
                 inner join ptoventa.lgt_prod l
                    on l.cod_prod = d.cod_prod
                 inner join cmr.mae_producto_com pc
                    on pc.cod_producto = p.cod_producto
                 where cp.tip_clien_convenio = 1 --DOCUMENTO DE BENEFICIARIO
                ...
                select btlprod.pkg_local.fn_devuelve_cia(btlprod.pkg_local.fn_get_cod_local(cp.cod_local)) cod_cia,
                       ...
                       cp.num_comp_pago num_documento,
                       p.cod_producto,
                       cmr.pkg_util.fn_uf(0, cant_atendida, val_frac, 'U') ctd_producto,
                       cmr.pkg_util.fn_uf(0, cant_atendida, val_frac, 'F') ctd_producto_frac,
                       d.val_frac ctd_fracciona,
                       decode(d.val_frac, 1, '0', '1') flg_fraccion,
                       round((d.val_prec_total / (decode(d.val_frac, 1, d.cant_atendida, 0) + (decode(d.val_frac, 1, 0, d.cant_atendida) / d.val_frac))), 2) prc_unit_vta,
                       round((d.val_prec_total / (decode(d.val_frac, 1, d.cant_atendida, 0) + (decode(d.val_frac, 1, 0, d.cant_atendida) / d.val_frac))) * (decode(cp.ind_comp_credito, 'S', decode(cp.pct_empresa, 0, 100, cp.pct_empresa), cp.pct_empresa) / 100), 2) prc_unit_neto_vta,
                       ...
                       nvl(d.porc_dcto_calc, 0) pct_descuento,
                       d.val_igv / 100 pct_igv,
                       round((((d.val_prec_total / (decode(d.val_frac, 1, d.cant_atendida, 0) + (decode(d.val_frac, 1, 0, d.cant_atendida) / d.val_frac))) * (decode(cp.ind_comp_credito, 'S', decode(cp.pct_empresa, 0, 100, cp.pct_empresa), cp.pct_empresa) / 100)) / (1 + (d.val_igv / 100))) * (d.val_igv / 100), 2) imp_unit_impuesto,
                       d.sec_ped_vta_det num_item,
                       pc.des_producto,
                       nvl((select nvl((pd.val_prec_total * (decode(pg.ind_comp_credito, 'S', decode(pg.pct_beneficiario, 0, 100, pg.pct_beneficiario), pg.pct_beneficiario) / 100)), 0)
                             from btlprod.rac_vta_comp_pago pg
                 ...
                 inner join btlprod.rac_vta_comp_pago cp
                   ...
                   and cp.cod_local = d.cod_local
                   and cp.num_ped_vta = d.num_ped_vta
                   and nvl(d.sec_comp_pago_empre, d.sec_comp_pago_origen) = cp.sec_comp_pago
                 inner join ptoventa.lgt_prod l
                    on l.cod_prod = d.cod_prod
                 inner join cmr.mae_producto_com pc
                    on pc.cod_producto = p.cod_producto
    ...
    CURSOR C_DETALLE_REGULAR (A_COD_CIA_DOC CHAR, A_COD_TIPO_DOC CHAR,A_NUM_DOCUMENTO_DOC CHAR) IS
                       ...
                       (
                        SELECT
                              (SELECT ML.CIA FROM NUEVO.AUX_MAE_LOCAL L INNER JOIN NUEVO.Mae_Local ML ON L.COD_LOCAL=ML.COD_LOCAL
                               WHERE /*COD_LOCAL_SAP BETWEEN '001' AND '599' */
                               (COD_LOCAL_SAP BETWEEN '001' AND '999' or fch_migracion is not null) AND COD_LOCAL_SAP = CP.COD_LOCAL) COD_CIA,
                              (SELECT L.Cod_Local FROM NUEVO.AUX_MAE_LOCAL l WHERE /*COD_LOCAL_SAP BETWEEN '001' AND '599' */
                              (COD_LOCAL_SAP BETWEEN '001' AND '999' or fch_migracion is not null) AND COD_LOCAL_SAP = CP.COD_LOCAL) COD_LOCAL,
                              (SELECT COD_TIPODOC FROM  BTLPROD.AUX_TIPO_DOC_CONV WHERE TIP_COMP_PAGO=CP.TIP_COMP_PAGO AND COD_GRUPO_CIA=CP.COD_GRUPO_CIA) COD_TIPO_DOCUMENTO,
                              CP.NUM_COMP_PAGO NUM_DOCUMENTO,
                              P.COD_PRODUCTO,
                        ...
                        INNER JOIN PTOVENTA.LGT_PROD L ON L.COD_PROD=D.COD_PROD INNER JOIN CMR.MAE_PRODUCTO_COM PC
                        ON PC.COD_PRODUCTO=P.COD_PRODUCTO
          ...
          -- KMONCADA CAMBIOS PARA FACTURACION ELECTRONICA
          V_NUM_COMP_E           BTLPROD.CAB_DOCUMENTO.NUM_DOCUMENTO_E%TYPE;
          V_FLAG_E               BTLPROD.CAB_DOCUMENTO.FLG_DOCUMENTO_E%TYPE;
          V_MONTO_TOTAL_DETALLE  BTLPROD.DET_DOCUMENTO.MTO_SUBTOTAL%TYPE default 0;

    MSG_ERRM VARCHAR2(500);
BEGIN
  BEGIN
--  RAISE_APPLICATION_ERROR(-20050,'Error al grabar en RAC por Facturacion Electronica - Controlado');
    ...
    from (
      SELECT L.Cod_Local
        FROM NUEVO.AUX_MAE_LOCAL l
       WHERE COD_LOCAL_SAP BETWEEN '001' AND '999'
         AND COD_LOCAL_SAP = pCod_local_in
      Union /*all*/
      SELECT L.COD_LOCAL
      FROM NUEVO.AUX_mae_LOCAL L
 ...
 -- COMENTADO POR DJARA 14/10/2014 --
...
/*
     begin
         for reg in (select *
                       from btlprod.rac_v_lista_maquina_ip r
                      where r.cod_grupo_cia = pCodGrupo_Cia_in
                        and r.cod_local = pCod_local_in
                        and r.num_ped_vta = pNum_Ped_Vta_in) loop
             sp_crea_cajas_liquidaciones2(reg.cod_local,
                                          reg.cod_maquina,
                                          reg.num_ip,
                                          reg.cod_cia,
                                          reg.cod_usuario,
    ...
    if v_tip_comp <> '04' then
        for reg_cab in c_cabecera loop
            ...
            begin
                ...
                exception
                    when others then
                        sp_crea_cajas_liquidaciones2(lpad(reg_cab.cod_local, '3'),
                                                     reg_cab.cod_maquina,
                                                     reg_cab.cod_maquina,
                                                     reg_cab.cod_cia,
                                                     lpad(reg_cab.usu_emision, 5, '0'),
                                                     to_date(substr(reg_cab.fch_emision, 1, 10), 'DD/MM/YYYY'));
/*
...
*/
                ...
                if reg_cab.cod_tipo_documento <> 'GRL' then
                    ...
                    --begin
                    -- KMONCADA 25.09.2014 PARA CONVENIO MIXTO
                    if reg_cab.cod_tipo_vtafin = '1' AND REG_CAB.TIP_PED_VTA='03' then
                    ...
                    -- KMONCADA 02.10.2014 DEVUELVE CREDITOS TOMADOS TEMPORALMENTE
                    IF pIndCreditoSeparado = 'S' THEN
                       -- DEVUELVE CREDITO, VTA EMPRESA Y VTA CONVENIO MIXTO PCT_CLIENTE
                       IF REG_CAB.COD_TIPO_VTAFIN='1' THEN
                          if fn_convenio_planilla(reg_cab.cod_convenio) = '0' then
                             UPDATE MEDCO.CTA_CTE C
                             SET    C.CRED_US = C.CRED_US - reg_cab.mto_total,
                                    C.CRED_DI = C.CRED_DI + reg_cab.mto_total
                             WHERE C.CODIGO = (
                                   SELECT A.NUM_DOCUMENTO_ID
                                   FROM CMR.MAE_CLIENTE A
                                   WHERE A.COD_CLIENTE = reg_cab.cod_cliente);
                          end if;
                       END IF;
                     -- DEVUELVE CREDITO VTA CONVENIO PCT_EMPRESA
                       IF REG_CAB.COD_TIPO_VTAFIN='2' then
                           if fn_convenio_planilla(reg_cab.cod_convenio) = '0' then
                             UPDATE MEDCO.CTA_CTE C
                             SET    C.CRED_US = C.CRED_US - reg_cab.mto_total,
                             ...
                             WHERE C.CODIGO = (
                                   SELECT A.NUM_DOCUMENTO_ID
                                   FROM CMR.MAE_CLIENTE A
                                   WHERE A.COD_CLIENTE = v_cod_empresa_fin) ;
                         end if;
                  -- SE BORRA DOCUMENTO INGRESADO TEMPORALMENTE EN EL CREDITO DEL BENEFECIARIO
                         DELETE FROM BTLPROD.REL_DOCUMENTO_BENEFICIARIO A
                         WHERE A.COD_TIPO_DOCUMENTO='PED'
                         AND A.NUM_DOCUMENTO = pnum_ped_vta_in
                         AND A.COD_CONVENIO = v_cod_convenio
                         AND A.COD_CLIENTE = reg_cab.cod_cliente;
                       END IF;
                    END IF;

                    -- KMONCADA 02.10.2014 COMPROBANTE ELECTRONICO.
                    IF reg_cab.num_documento_e IS NULL OR reg_cab.num_documento_e = reg_cab.num_documento THEN
                      V_NUM_COMP_E := reg_cab.num_documento;
                      V_FLAG_E     := '0';
                    ELSE
                      V_NUM_COMP_E := reg_cab.num_documento_e;
                      V_FLAG_E     := '1';
                    END IF;
                        ...
                        insert into btlprod.cab_documento
                            (cia, --1
                             ...
                             num_documento_e, -- 69 -- KMONCADA 02.10.2014 NUMEROS DE COMPROBANTES ELECTRONICOS
                             flg_documento_e
                             )
                        values
                            (reg_cab.cod_cia, --1
                             ...
                             V_NUM_COMP_E, -- KMONCADA 02.10.2014 NUMEROS DE COMPROBANTES ELECTRONICOS
                             V_FLAG_E
                             );
                    --exception
                    --    when others then
                    ...
                    --begin
                        ...
                        update btlprod.cab_documento
                         ...
                         where cia = reg_cab.cod_cia
                           and cod_tipo_documento = reg_cab.cod_tipo_documento
                           and num_documento = reg_cab.num_documento;

                      V_MONTO_TOTAL_DETALLE := V_MONTO_TOTAL_DETALLE + reg_cab.mto_total - reg_cab.redondeo;
                    --exception
                    --    when others then
                    --        raise_application_error(-20000, sqlerrm);
                    --end;
...
--                      V_MONTO_TOTAL_DETALLE := 0;
                        for reg1 in c_detalle(reg_cab.cod_cia,
                            ...
                            begin
                                ...
                                if reg1.prod_apps is null then
                                    ...
                                    v_cant_producto := reg1.cant_atendida;
                                end if;
                                ...
                                values
                                    (reg1.cod_cia, --1
                                     ...
                                     V_MONTO_TOTAL_DETALLE := V_MONTO_TOTAL_DETALLE - reg1.mto_subtotal;
                            exception
                                when others then
                        ...
                        /*IF (V_MONTO_TOTAL_DETALLE != reg_cab.mto_total) THEN
                          raise_application_error(-20854,
                           'MONTO DEL DETALLE NO COINCIDE CON EL TOTAL DEL DOCUMENTO: '||
                           reg_cab.cod_tipo_documento||' - ' ||reg_cab.num_documento||
                           '; TOTAL DOCUMENTO: '||reg_cab.mto_total||', TOTAL DETALLE: '||
                           V_MONTO_TOTAL_DETALLE||'...');
                        END IF;*/
                   ...
                   -- SE BORRA DOCUMENTO INGRESADO TEMPORALMENTE EN EL CREDITO DEL BENEFECIARIO
                    ...
                    --begin
                        ...
                        values
                            ...
                            -- KMONCADA 07.11.2014 VALIDACION DE MONTO TOTAL
                             V_MONTO_TOTAL_DETALLE := V_MONTO_TOTAL_DETALLE + reg_cab.mto_total - reg_cab.redondeo;
                    --exception
                    --    when others then
                    --        raise_application_error(-20000, sqlerrm);
 ...
 'R4 Faltan Ingresar Datos de la Receta. ' || reg_cab.cod_tipo_documento || '-' || reg_cab.num_documento || '-' || c_fch_receta || '-' || c_cod_medico || '-' || c_cod_local_receta || '-' || a_cad_cod_diagnostico);
        ...
        for reg_cab_nc in c_cabecera_nc loop
            if c_cod_modalidad = btlprod.pkg_constantes.cons_modal_vta_002_convemp then
                ...
                exception
                    when no_data_found then
                        sp_crea_cajas_liquidaciones2(lpad(reg_cab_nc.cod_local, '3'),
                                                     reg_cab_nc.cod_maquina,
                                                     reg_cab_nc.cod_maquina,
                                                     reg_cab_nc.cod_cia,
                                                     lpad(reg_cab_nc.usu_emision, 5, '0'),
                                                     to_date(substr(reg_cab_nc.fch_emision, 1, 10), 'DD/MM/YYYY'));
                    when others then
                        raise_application_error(-20000,
                                                ' cia:' || reg_cab_nc.cod_cia ||
                ...
                --begin
                    ...
                    -- KMONCADA 02.10.2014 CONVENIO MIXTO
                    if reg_cab_nc.cod_tipo_vtafin = '1' AND reg_cab_nc.TIP_PED_VTA='03' then
                        V_COD_MODALIDAD := pkg_constantes.CONS_MODAL_VTA_003_MAYORIS ;
                        v_cod_convenio := null;
...
--                        V_COD_CONVENIO := COD_CONVENIO;
            --            V_COD_TIPO_VTAFIN = '1'
                ...
                -- KMONCADA 02.10.2014 COMPROBANTE ELECTRONICO.
                    IF reg_cab_nc.num_documento_e IS NULL OR reg_cab_nc.num_documento_e = reg_cab_nc.num_documento THEN
                      V_NUM_COMP_E := reg_cab_nc.num_documento;
                      V_FLAG_E     := '0';
                    ELSE
                      V_NUM_COMP_E := reg_cab_nc.num_documento_e;
                      V_FLAG_E     := '1';
                    END IF;
                    ...
                    insert into btlprod.cab_documento
                        (cia, --1
                         ...
                         num_ped_vta, --66
                         sec_comp_pago, --67
                         num_documento_e, -- KMONCADA 02.10.2014 NUM COMP ELECTRONICO
                         flg_documento_e
                         )
                    values
                ...
                --begin
                    ...
                    -- KMONCADA 07.11.2014 VALIDACION DE MONTO TOTAL
                       V_MONTO_TOTAL_DETALLE := V_MONTO_TOTAL_DETALLE + reg_cab_nc.mto_total - reg_cab_nc.redondeo;
                    ...
                    begin
                        ...
                        values
                            (reg_cab_nc.cod_cia, --1
                             ...
                             V_MONTO_TOTAL_DETALLE := V_MONTO_TOTAL_DETALLE - reg1_nc.mto_subtotal;
                    exception
                        when others then
                ...
                /*IF (V_MONTO_TOTAL_DETALLE != reg_cab_nc.mto_total) THEN
                  raise_application_error(-20854,
                   'MONTO DEL DETALLE NO COINCIDE CON EL TOTAL DEL DOCUMENTO: '||
                   reg_cab_nc.cod_tipo_documento||' - ' ||reg_cab_nc.num_documento||
                   '; TOTAL DOCUMENTO: '||reg_cab_nc.mto_total||', TOTAL DETALLE: '||
                   V_MONTO_TOTAL_DETALLE||'...');
                END IF;*/
            end if;
        end loop;
...
/*     IF C_COD_MODALIDAD=BTLPROD.PKG_CONSTANTES.CONS_MODAL_VTA_002_CONVEMP  THEN
     ...
     if V_MONTO_TOTAL_DETALLE >= -0.05 and V_MONTO_TOTAL_DETALLE <= 0.05 THEN
        COMMIT;
     ELSE
        raise_application_error(-20854,
                   chr(13)||'MONTO DEL DETALLE NO COINCIDE CON EL TOTAL DEL DOCUMENTO: '||chr(13));
     END IF;
  ...
  EXCEPTION
      WHEN OTHERS THEN
        --ERIOS 14.08.2014 Control de errores
         ...
         -- KMONCADA 18.09.2014 MODIFICAR EL MENSAJE POR FALTA DE CREDITO DEL CLIENTE
         IF INSTR(UPPER(sqlerrm),UPPER('no cuenta con crédito suficiente'))!=0 THEN
           MSG_ERRM := SUBSTR(sqlerrm,1,500);
           RAISE_APPLICATION_ERROR(-20850,MSG_ERRM);
         ELSE
           -- KMONCADA 18.09.2014 ERROR POR DUPLICIDAD DE DOCUMENTO.
           IF INSTR(UPPER(SQLERRM),'20851')!=0 THEN
             MSG_ERRM := SUBSTR(sqlerrm,1,500);
             RAISE_APPLICATION_ERROR(-20851,'DUPLICIDAD DE DOCUMENTO EN EL RAC.' || MSG_ERRM);
           ELSE
             -- KMONCADA 18.09.2014 CONTROL DE CODIGO SAP DE CLIENTE
             IF INSTR(UPPER(sqlerrm),UPPER('cliente sin codigo sap'))!=0 THEN
               MSG_ERRM := SUBSTR(sqlerrm,1,500);
               RAISE_APPLICATION_ERROR(-20852,MSG_ERRM);
             ELSE
               -- KMONCADA 29.09.2014 CLIENTE NO CUENTA CON CTA_CTE
               IF INSTR(UPPER(sqlerrm),UPPER('NO SE ENCONTRARON LOS DATOS DE CLIENTE (P_GRABA_FACTURA).-->'))!=0 THEN
                 MSG_ERRM := SUBSTR(sqlerrm,1,500);
                 RAISE_APPLICATION_ERROR(-20853,MSG_ERRM);
               ELSE
                 MSG_ERRM := SUBSTR(sqlerrm,1,500);
...
begin
    ...
    begin
        ...
        for reg4 in (select c.cod_campo,
                            --ERIOS 06.11.2014 Los campo tipo lista, se obtiene el codigo.
                            CASE WHEN c.cod_campo IN (SELECT COD_TIPO_CAMPO
                                                        FROM CMR.MAE_TIPO_CAMPO
                                                        WHERE FLG_LISTA = 1) THEN c.cod_valor_in
                                 ELSE c.descripcion_campo
                            END descripcion_campo,
                            (select ml.cia
                               from nuevo.aux_mae_local l
                              inner join nuevo.mae_local ml
                                 on l.cod_local = ml.cod_local
...
procedure sp_graba_det_fpago(pcodgrupo_cia_in char,
    ...
    cursor c_det(pnum_ped_vta_in  char,
                 pcodgrupo_cia_in char,
                 pcod_local_in    char) is
        select (select case -- KMONCADA 23.10.2014 FACTURACION ELECTRONICA
                           when r.cod_tip_proc_pago = '1' and
                                cod_tipodoc = 'TKB' then
                            'BOL'
                           else
                            cod_tipodoc
                       end
    ...
    cursor c_pag(pnum_ped_vta_in  char,
                 pcodgrupo_cia_in char,
                 pcod_local_in    char) is
        select rownum num_item,
          ...
          from btlprod.rac_vta_forma_pago_pedido p
         inner join aux_forma_pago_mf f
...
begin
    ...
    if v_secuencia is null or v_secuencia < 1 then
        raise_application_error(-20000,
                                chr(13) ||
                                'Error al Determinar Secuencia de Pagos ');
    end if;
    ...
    exception
        when others then
            raise_application_error(-20000,
                                    chr(13) ||
                                    'Error de registro en CAB_FORPAG_DOC: ' ||
                                    v_secuencia || sqlerrm);
    end;
    ...
    for reg in c_det(pnum_ped_vta_in, pcodgrupo_cia_in, pcod_local_in) loop
        begin
            insert into btlprod.rel_forpag_doc
                (cia, cod_tipo_documento, num_documento, sec_forpag_doc)
            values
                (c_cia,
        ...
        exception
            when dup_val_on_index then
                raise_application_error(-20000,
                                        chr(13) ||
                                        'Forma de Pago ya Ingresada del documento TIPO:' ||
                                        reg.cod_tipo_documento ||
                                        ' Numero:' || reg.num_documento ||
                                        sqlerrm);
            when others then
                raise_application_error(-20000,
                                        chr(13) ||
                                        'Error de relacion de Documento - Pago_1 ' ||
                                        sqlerrm);
        end;
    ...
    for reg in c_pag(pnum_ped_vta_in, pcodgrupo_cia_in, pcod_local_in) loop
        ...
        begin
            ...
            if reg.cod_hijo is null then
                raise_application_error(-20000,
                                        'No Existe el hijo de la Forma de Pago');
            end if;
        ...
        exception
            when dup_val_on_index then
                raise_application_error(-20000,
                                        chr(13) ||
                                        'La forma de pago esta duplicada Duplicado ');
            when others then
                raise_application_error(-20000,
                                        chr(13) ||
                                        'Error de Registro Formas de Pago ' ||
                                        sqlerrm);
        end;
...
procedure sp_crea_cajas_liquidaciones(pcodgrupo_cia_in char,
    ...
    cursor c_datos_liquidaciones is
        select distinct cod_local,
                        ...
                        cod_usuario,
                        fch_emision
          from (select (select l.cod_local
                           from nuevo.aux_mae_local l
                          where /*COD_LOCAL_SAP BETWEEN '001' AND '599' */
                          (cod_local_sap between '001' and '999' or
                          fch_migracion is not null)
                       and cod_local_sap = r.cod_local) cod_local,
                       --P.IP_PC COD_MAQUINA,
                       (select cod_maquina
                          from btlprod.aux_maquina_x_local
                         where num_ip = p.ip_pc) cod_maquina,
                       p.ip_pc num_ip,
                       (select ml.cia
                           from nuevo.aux_mae_local l
                          inner join nuevo.mae_local ml
...
begin
    dbms_output.put_line('sp_crea_cajas_liquidaciones '||pcodgrupo_cia_in||'|'||pcod_local_in||'|'||pnum_ped_vta_in);
    for reg in c_datos_liquidaciones loop
        dbms_output.put_line('cod_maquina '||reg.cod_maquina);
        --para validar que no habra liquidacion si no ha cerrado o tiene pendientes
        if reg.cod_local is null then
            raise_application_error(-20000,
                                    'EL LOCAL NO ESTA REGISTRADO EN BTL');
        end if;
        ...
        if reg.cod_maquina is null then
            ...
            begin
                dbms_output.put_line('creando maquina aux_maquina_x_local');
        ...
        if reg.cod_usuario is null then
            raise_application_error(-20000,
                                    'EL USUARIO NO ESTA REGISTRADO EN BTL');
        end if;
        ...
        dbms_output.put_line('Existe liquidacion: '||v_conteo);
        ...
        if v_conteo = 0 then
            begin
                ...
                begin
                    ...
                    reg.COD_LOCAL,
                    '0');*/
                    --Arturo Escate 28/11/2014, cambie aca para que haga commit a cada secuencia de caja
                    c_liquida := cmr.fn_dev_prox_sec_commit('BTLPROD',
                                                            'CAB_LIQUIDACION',
                                                            reg.cod_cia,
                                                            reg.cod_local,
                                                            '0');
                ...
                exception
                    when others then
                        raise_application_error(-20000,
                                                'SE PRODUJO UN ERROR AL DETERMINAR LA SECUENCIA DE LIQUIDACION');
                end;

                if c_liquida < 0 then
                    raise_application_error(-20000,
                                            'NO EXISTE CORRELATIVO DE LIQUIDACIONES.');
                end if;
                ...
                exception
                    when others then
                        raise_application_error(-20000,
                                                'SE PRODUJO UN ERROR AL DETERMINAR LA SECUENCIA DE LA CAJA ' ||
                                                v_sec_caja || ';' ||
                                                sqlerrm);
                ...
                begin
                    dbms_output.put_line('creando liquidacion aux_control_caja');
...
begin
    ...
    dbms_output.put_line('=================>>>>> sp_crea_cajas_liquidaciones2');
    dbms_output.put_line(p_cod_local||'|'||p_cod_maquina||'|'||p_num_ip||'|'||p_cod_cia||'|'||
                         p_cod_usuario||'|'||p_fch_emision);
    dbms_output.put_line('sp_crea_cajas_liquidaciones2 <<<<<=================');

    if p_cod_local is null then
        raise_application_error(-20000,
                                'EL LOCAL NO ESTA REGISTRADO EN BTL');
    end if;
    ...
    v_cod_maquina := p_num_ip;

    if p_cod_maquina is null then
        begin
            insert into btlprod.aux_maquina_x_local
                (cia,
                 cod_local,
        ...
        exception
            when dup_val_on_index then
                update btlprod.aux_maquina_x_local
                   set fch_actualiza = sysdate, cod_maquina = p_num_ip
                 where num_ip = p_num_ip;
            when others then
                raise_application_error(-20000,
                                        'Error al registrar la Maquina: ' ||
                                        p_num_ip || ' , ' || sqlerrm);
    ...
    select count(1)
      into v_conteo
      from btlprod.aux_control_caja c
     where c.cia = p_cod_cia
       and c.cod_local = p_cod_local
       and c.fch_inicio = p_fch_emision
       and c.flg_estado_caja = '0'
...
begin
  ...
  if pcodtipodoc <> '04' then
    ...
    if v_entro = 'N' then
      --ERIOS 24.09.2014 Si es por NC y no hay documentos por anular
      IF pIndNotaCred = 'S' THEN
        v_estado := 'S';
      ELSE
        v_estado := 'P';
      END IF;
 ...
 ******************************************************************************/
 ...
 RETURN CHAR IS
  ...
  BEGIN
    BEGIN
    SELECT DECODE(CAB.NUM_PED_VTA,NULL,'0','1')
    INTO   vFlag
    FROM   RAC_VTA_PEDIDO_VTA_CAB CAB,
           RAC_VTA_FORMA_PAGO_PEDIDO FP
    WHERE  CAB.COD_GRUPO_CIA=FP.COD_GRUPO_CIA
    AND    CAB.COD_LOCAL=FP.COD_LOCAL
    AND    CAB.NUM_PED_VTA=FP.NUM_PED_VTA
    AND    CAB.COD_GRUPO_CIA=pCodGrupo_Cia_in
    AND    CAB.COD_LOCAL=pCod_local_in
    AND    CAB.NUM_PED_VTA=pNum_Ped_Vta_in
    AND    FP.COD_FORMA_PAGO='00050';
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        vFlag:='0';
    END;
 ...
 END;
  ...
  RETURN CHAR IS
    ...
    C_COD_LOCAL NUEVO.AUX_MAE_LOCAL.COD_LOCAL%TYPE;
    V_COD_CLIENTE CMR.MAE_CLIENTE.COD_CLIENTE%TYPE ;
    V_NOM_CLIENTE CMR.MAE_CLIENTE.DES_CLIENTE%TYPE;
    V_COD_EMPRESA CMR.MAE_CONVENIO.COD_CLIENTE%TYPE;
    V_NUM_RUC_EMPRESA CMR.MAE_CLIENTE.NUM_DOCUMENTO_ID%TYPE;
    ...
    V_TIPO MEDCO.CTA_CTE.TIPO%TYPE;
    V_CON_VTA MEDCO.CTA_CTE.CONVTA%TYPE;
    V_CIA NUEVO.MAE_LOCAL.CIA%TYPE;
    C_CODIGO_SAP MEDCO.CTA_CTE.CODCLISAP%TYPE;
    C_CRED_DI NUMBER(14,4);

    SIN_COD_SAP EXCEPTION;
    SIN_CREDITO EXCEPTION;
    SIN_TIPO_CLIENTE EXCEPTION;
    SIN_CON_VTA EXCEPTION;
    CON_VTA_CONTADO EXCEPTION;
    RUC_NULO EXCEPTION;
    pResultado CHAR(1) :='N';
    ...
    -- KMONCADA 21.10.2014
    --V_CONVENIO_MIFARMA BTLPROD.CAB_DOCUMENTO.COD_CONVENIO%TYPE := '0000000833';
    --V_CONVENIO_FASA BTLPROD.CAB_DOCUMENTO.COD_CONVENIO%TYPE := '0000001072';
    vContadorExistente INTEGER := 0;

  BEGIN
    BEGIN
      SELECT L.COD_LOCAL, ML.CIA
        INTO C_COD_LOCAL, V_CIA
        FROM NUEVO.AUX_MAE_LOCAL L
       INNER JOIN NUEVO.MAE_LOCAL ML
          ON L.COD_LOCAL = ML.COD_LOCAL
       WHERE L.COD_LOCAL_SAP = pCod_local_in
         AND (L.FCH_MIGRACION IS NOT NULL OR (L.COD_LOCAL_SAP BETWEEN '001' AND '999'));
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20000, 'No se ha encontrado el local: ' || pCod_local_in);
    END;

    -- VERIFICA CLIENTE DEL CONVENIO
    BEGIN
      SELECT C.COD_CLIENTE, C.DES_CLIENTE
        INTO V_COD_CLIENTE, V_NOM_CLIENTE
        FROM CMR.MAE_CLIENTE C
       WHERE C.COD_CLIENTE = pCod_cliente_in; -- cod_cliente_conv
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        BEGIN
          SELECT A.COD_CLIENTE, MC.DES_CLIENTE
           ...
           INNER JOIN CMR.MAE_CLIENTE MC
              ON A.COD_CLIENTE = MC.COD_CLIENTE
           WHERE A.COD_CONVENIO = pCod_convenio_in
             AND RPAD(NVL(A.COD_ASEGURADO, ' '), 5, ' ') || '-' ||
                 RPAD(A.NUM_POLIZA, 6, ' ') || '-' || RPAD(A.NUM_PLAN, 5, ' ') || '-' ||
                 RPAD(A.NUM_ITEM, 2, ' ') = pCod_cliente_in
             AND ROWNUM = 1;
        EXCEPTION
          WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20000, 'No se ha encontrado el Cliente: ' || pCod_cliente_in);
        END;
    END;

    -- VERIFICA CONVENIO
    BEGIN
      SELECT CC.COD_CLIENTE, TRIM(MC.NUM_DOCUMENTO_ID), CC.FLG_RECETA
        INTO V_COD_EMPRESA, V_NUM_RUC_EMPRESA, V_FLG_RECETA
        FROM CMR.MAE_CONVENIO CC
       INNER JOIN CMR.MAE_CLIENTE MC
          ON CC.COD_CLIENTE = MC.COD_CLIENTE
       WHERE CC.COD_CONVENIO = pCod_convenio_in;
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20000, 'No se ha encontrado al Convenio: ' || pCod_convenio_in);
    END;

    IF pCod_Tip_Doc_in <> 'GRL' THEN
      BEGIN
        IF pCod_Tip_Doc_in IN ('BOL', 'FAC', 'TKB', 'TKF') THEN
          IF pVtaFin_in = '1' THEN -- CREDITO
              V_RUC := BTLPROD.PKG_CLIENTE.FN_LISTA_DOCUMENTO(pCod_cliente_in);
          ELSE
            IF pVtaFin_in = '2' THEN -- CONVENIO
              IF V_NUM_RUC_EMPRESA IS NULL THEN
                RAISE RUC_NULO;
              END IF;
              V_RUC := V_NUM_RUC_EMPRESA;
              --2do ver si es convenio BTL, sacar el dni de beneficiario
              IF pCod_convenio_in IN (V_CONVENIO_BTL, V_CONVENIO_AMA) THEN
                -- KMONCADA 21.10.2014 SE AGREGA CONVENIOS DE MIFARMA Y FASA
                -- (V_CONVENIO_BTL, V_CONVENIO_AMA, V_CONVENIO_FASA, V_CONVENIO_MIFARMA) THEN
                V_RUC := BTLPROD.PKG_CLIENTE.FN_LISTA_DOCUMENTO(pCod_cliente_in);
              END IF;
            END IF;
          END IF;
        ELSE
          IF pCod_Tip_Doc_in = 'NCR' THEN
            IF pCod_convenio_in IS NOT NULL THEN
              V_RUC := BTLPROD.PKG_CLIENTE.FN_LISTA_DOCUMENTO(V_COD_EMPRESA);
            ELSE
              V_RUC := BTLPROD.PKG_CLIENTE.FN_LISTA_DOCUMENTO(NVL(pCod_cliente_in,V_COD_EMPRESA));
            END IF;
          END IF;
        END IF;
        ...
        -- VALIDA CLIENTE SI EXISTE EN CTA_CTE Y CONDICION DE VENTA

        SELECT TIPO, CONVTA, CODCLISAP
        INTO   V_TIPO, V_CON_VTA, C_CODIGO_SAP
        FROM   MEDCO.CTA_CTE
        WHERE  CODIGO = V_RUC;

        --tipo de cliente
        IF V_TIPO IS NULL OR LENGTH(V_TIPO)<=0 THEN
          RAISE SIN_TIPO_CLIENTE;
        END IF;

        --la condicion de venta
        IF V_CON_VTA IS NULL OR LENGTH(V_CON_VTA)<=0 THEN
          RAISE SIN_CON_VTA;
        END IF;

        --si la condicion de venta es efectivo no se puede vender al credito
        IF V_CON_VTA = '01' THEN
          RAISE CON_VTA_CONTADO;
        END IF;

        -- VERIFICA CODIGO_SAP
        IF C_CODIGO_SAP IS NULL AND V_CIA <> '11' THEN
          RAISE SIN_COD_SAP;
        END IF;

        -- EVALUA LINEA DE CREDITO DE LA EMPRESA
        if fn_convenio_planilla(pCod_convenio_in) = '0' then
            C_CRED_DI := NVL(MEDCO.PACK_CTA_CTE.JALA_LINEA(V_RUC), 0);
            IF (C_CRED_DI - pMonto_in) <= 0 THEN
              RAISE SIN_CREDITO;
            END IF;
        end if;
      EXCEPTION
        WHEN RUC_NULO THEN
          RAISE_APPLICATION_ERROR(-20000,'NO SE CONTRO LOS DATOS DEL CONVENIO - RUC DEL BENEFICIARIO');
        WHEN CON_VTA_CONTADO THEN
          RAISE_APPLICATION_ERROR (-20000,'LA CONDICIÓN DE VENTA DEL CLIENTE ES CONTADO. NO PUEDE REALIZAR UNA VENTA AL CRÉDITO. COMUNÍQUESE CON CRÉDITOS Y COBRANZAS.');
        WHEN SIN_CON_VTA THEN
          RAISE_APPLICATION_ERROR (-20000,'NO SE HA DEFINIDO LA CONDICIÓN DE VENTA DEL CLIENTE. '||V_RUC);
        WHEN SIN_TIPO_CLIENTE THEN
          RAISE_APPLICATION_ERROR (-20000,'NO SE HA DEFINIDO EL TIPO DE CLIENTE.');
        WHEN SIN_CREDITO THEN
          RAISE_APPLICATION_ERROR(-20000, 'El cliente '||V_RUC||' no cuenta con crédito suficiente para realizar esta venta. Comuníquese con Créditos y Cobranzas'||CHR(13));
        WHEN SIN_COD_SAP THEN
          RAISE_APPLICATION_ERROR(-20001, 'Cliente sin codigo SAP, Revise Cliente'||chr(13)||'RUC/DNI: '||V_RUC);
        WHEN NO_DATA_FOUND THEN
          RAISE_APPLICATION_ERROR(-20000, 'NO SE ENCONTRARON LOS DATOS DE CLIENTE (P_GRABA_FACTURA).-->' || V_RUC);
        WHEN OTHERS THEN
          RAISE_APPLICATION_ERROR(-20000, 'ERROR CARGANDO DATOS DEL CLIENTE.' || CHR(13) || SQLERRM);
      END;
    ...
    BEGIN
      ...
      SELECT COUNT(1)
      INTO   vContadorExistente
      FROM BTLPROD.RAC_VTA_PEDIDO_TMP A
      WHERE A.COD_GRUPO_CIA = '001'
      AND   A.COD_LOCAL     = pCod_local_in
      AND   A.NUM_PED_VTA   = pNumPedVta_in;

      IF vContadorExistente = 0 THEN
        IF pCod_Tip_Doc_in <> 'GRL' THEN
          -- EXITO EN LAS VALIDACIONES
          -- VTA CREDITO Y VTA CONVENIO
          if fn_convenio_planilla(pCod_convenio_in) = '0' then
              UPDATE MEDCO.CTA_CTE A
              SET A.CRED_DI = A.CRED_DI - pMonto_in,
              A.CRED_US = A.CRED_US + pMonto_in
              WHERE CODIGO = V_RUC;
          end if;
        END IF;

        IF (pVtaFin_in ='2') THEN -- VENTA CONVENIO
          INSERT INTO BTLPROD.REL_DOCUMENTO_BENEFICIARIO A (
                 COD_CONVENIO, COD_CLIENTE, CIA, COD_TIPO_DOCUMENTO, NUM_DOCUMENTO, COD_ESTADO,
                 MTO_TOTAL, FCH_EMISION, COD_LOCAL, COD_CLIENTE_SUP, FLG_PAGADO, FCH_PAGADO)
          VALUES(
                 pCod_convenio_in, pCod_cliente_in, V_CIA, 'PED', pNumPedVta_in, 'EMI',
                 pMonto_in, SYSDATE, C_COD_LOCAL, NULL, 0, NULL);
        END IF;

        INSERT INTO BTLPROD.RAC_VTA_PEDIDO_TMP (COD_GRUPO_CIA, COD_LOCAL, NUM_PED_VTA, MONTO)
        VALUES ('001',pCod_local_in,pNumPedVta_in,pMonto_in);
 ...
 /******************************************************************************
 KMONCADA 20.10.2014 FUNCION QUE VALIDA QUE NUMERO NO SE ENCUENTRE REGISTRADO ANTERIORMENTE.
 ...
 FUNCTION FN_EXISTE_DOCUMENTO(pCodGrupo_Cia_in CHAR,
                              ...
                              pIndCompElectronico CHAR DEFAULT 'N') RETURN INTEGER IS

   V_CONTADOR INTEGER DEFAULT 0;
   V_CIA BTLPROD.CAB_DOCUMENTO.CIA%TYPE;
   V_TIPO_DOC BTLPROD.CAB_DOCUMENTO.COD_TIPO_DOCUMENTO%TYPE;
 ...
 BEGIN
   ...
   INTO V_CIA
   FROM   NUEVO.AUX_MAE_LOCAL L
   INNER JOIN NUEVO.MAE_LOCAL ML ON L.COD_LOCAL = ML.COD_LOCAL
   WHERE (L.COD_LOCAL_SAP BETWEEN '001' AND '999' OR L.FCH_MIGRACION IS NOT NULL)
   AND L.COD_LOCAL_SAP = pCodLocal_in;

   SELECT COD_TIPODOC
   INTO   V_TIPO_DOC
   FROM  BTLPROD.AUX_TIPO_DOC_CONV
   WHERE TIP_COMP_PAGO = pTipDoc_in
   AND COD_GRUPO_CIA = pCodGrupo_Cia_in;

   IF pIndCompElectronico='S' THEN
     V_QUERY := ' SELECT COUNT(1) FROM BTLPROD.CAB_DOCUMENTO WHERE CIA = :a AND COD_TIPO_DOCUMENTO = :b AND NUM_DOCUMENTO_E = :c';
   ELSE
     V_QUERY := ' SELECT COUNT(1) FROM BTLPROD.CAB_DOCUMENTO WHERE CIA = :a AND COD_TIPO_DOCUMENTO = :b AND NUM_DOCUMENTO = :c';
   END IF;

   EXECUTE IMMEDIATE V_QUERY INTO V_CONTADOR USING V_CIA, V_TIPO_DOC, pNumCompVerifica_in ;

/*   SELECT COUNT(1)
   INTO V_CONTADOR
   FROM BTLPROD.CAB_DOCUMENTO
   WHERE CIA = V_CIA
   AND COD_TIPO_DOCUMENTO = V_TIPO_DOC
   AND NUM_DOCUMENTO = pNumCompVerifica_in;*/
   RETURN V_CONTADOR;
 END;
