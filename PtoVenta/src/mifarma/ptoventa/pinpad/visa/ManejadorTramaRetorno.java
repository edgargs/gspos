...
public class ManejadorTramaRetorno {
    ...
    public boolean iniciarProceso(String trama) {
        ...
        try { //se configura la comunicación con el pinpad
            loadLibrary();
    ...
    public Integer guardarTramaBD(String numPed, String formaPago) {
        ...
        try {
            if (!DBPinpad.guardarTramaPinpadVisa(codRespuesta, mensFinOperacion, nombreCliente, numAutorizacion,
                                                 numReferencia, nroTarjetaBruto, fechaExpiracionStr,
                                                 fechaTransaccionStr, horaTransaccion, codOperacion, cortePapelStr,
                                                 propinaStr, numMozo, empresa, terminal, cuotas.toString(),
                                                 pagoDiferido, pidePin, idUnico, montoCuotaStr, numPed, "COM",
                                                 formaPago))
    ...
    public Integer guardarTramaAnulacionBD(String numPed, String formaPago) {
        ...
        try {
            if (!DBPinpad.guardarTramaPinpadVisa(codRespuesta, mensFinOperacion, nombreCliente, numAutorizacion,
                                                 numReferencia, numTarjeta, fechaExpiracionStr, fechaTransaccionStr,
                                                 horaTransaccion, codOperacion, cortePapelStr, propinaStr, numMozo,
                                                 empresa, terminal, cuotas.toString(), pagoDiferido, pidePin, idUnico,
                                                 montoCuotaStr, numPed, "ANU", formaPago))
                pRespuesta = VariablesPinpad.RET_NOK;
    ...
    public Integer guardarMsjeImprBD(String msjeImpr, String codRespuesta, String numPed) {
        try {
            if (!DBPinpad.guardarTramaPinpadVisa(codRespuesta, msjeImpr, "", "", "", "", "", "", "", "", cortePapelStr,
                                                 "", "", "", "", "", "", "", "", "", numPed, "I", ""))
                return VariablesPinpad.RET_NOK;
        } catch (Exception e) {
    ...
    public boolean imprVoucher(String trama) { //String[] segmentos = trama.split("0x0D");
        ...
        textoImpr.append("<div class=\"tipoLetraA\">OLD<br/>" + temp + "</div>");
        ...
        hilo.start();
        */
        imprVoucher(lstVoucher);

        return true;
    }

    public boolean imprVoucher(List lstTrama) { //String[] segmentos = trama.split("0x0D");
     ...
     * @since 17-Julio-2013
     */
    private void loadLibrary() { //Cargar la libreria intermedia
