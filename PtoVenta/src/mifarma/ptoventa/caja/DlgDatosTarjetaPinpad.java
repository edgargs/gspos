...
public class DlgDatosTarjetaPinpad extends JDialog
{
            ...
            if((strIndLectorBanda.equals("S") && intCountEnter==2) || (strIndLectorBanda.equals("N") && intCountEnter==1) )
            {
                ...
                {   if(hablitarTipoTarjeta())
                    {   lblTipoTarjeta.setText(desc_prod+"/"+strDescFormaPago);
                        ...
                        else if(strCodFormaPago.equals(ConstantsPtoVenta.FORPAG_MC_PINPAD) ||
                                strCodFormaPago.equals(ConstantsPtoVenta.FORPAG_VISA_PINPAD) || 
                                strCodFormaPago.equals(ConstantsPtoVenta.FORPAG_DINERS) || 
                                strCodFormaPago.equals(ConstantsPtoVenta.FORPAG_AMEX))
    ...
    private boolean buscarInfotarjeta(){
        ...
        {   log.debug("ERROR en buscarInfotarjeta: "+e.getMessage());
            log.error("",e);
            log.debug("ERROR en buscarInfotarjeta: ",e);            
        } 
        if(lista.size()>0)
        ...
        {   //si no se encuentra la información del bin de tarjeta, se lanza una ventan para decidir que forma de pago procesar
            ...
            {   
                ...
                dlgEleccionTarjeta.setValores(txtNroTarjeta.getText());
