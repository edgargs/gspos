 ...
 //Agregado por   19.09.08
  ...
  private void addKeyListenerToTextField(final JTextField pJTextField,
    ...
    pJTextField.addKeyListener(new KeyAdapter() {
      ...
      public void keyPressed(KeyEvent e) {
        //System.out.println(pJTextField.getText());
        if ( e.getKeyCode()==KeyEvent.VK_ENTER ) {
          ...
          if ((tblListaLocales.getSelectedRow()+1)==tblListaLocales.getRowCount() ){
            FarmaUtility.setearRegistro(tblListaLocales,0,null,0);
             System.out.println("Ultima regitro...");
          } else{
 ...
 * */
  ...
  * */
  private void cargaCampos()
  {
    ...
    for (int i=0; i<tblListaLocales.getRowCount(); i++){
    local="";
    System.out.println(""+arrayDescripcion.size());
    System.out.println(""+tblListaLocales.getRowCount());
    if(arrayDescripcion.size()==tblListaLocales.getRowCount()){
      local=((String) ((ArrayList) arrayDescripcion.get(i)).get(0)).trim();
      System.out.println("local seteado :"+local);
      System.out.println("local array :"+FarmaUtility.getValueFieldJTable(tblListaLocales,i,0));
 ...
 * */
  private void funcion_f7()
  {
   ...
   /* }else{
      ...
      System.out.println("condicion :"+(cantidad*i)+"<"+Integer.parseInt(lblCantStock.getText().trim()));
        if((cantidad*i)+cantidad<=Integer.parseInt(lblCantStock.getText().trim())){
        System.out.println("set registro -->"+i);
        tblListaLocales.setValueAt(""+cantidad,i,ConstantsProducto.columnEditable_Cantidad_Distribu);
        row=i;
        }
      }
      ...
      tblListaLocales.repaint();
      setCantidades(getCantidadTotalReponer());
      System.out.println("Total a Reponer :"+getCantidadTotalReponer());
  ...
  private void funcion_f11()
  {
    ...
    /*else if(vCantReponer==0)
    {
      ...
      if(FarmaUtility.rptaConfirmDialog(this,"¿Esta seguro de no realizar ninguna distribucion para este producto?"))
        {
    ...
    if(FarmaUtility.rptaConfirmDialog(this,"¿Esta seguro que desea grabar los nuevos PVM en locales?"))
    {
        FarmaUtility.moveFocus(txtCantStock);
        moveFocusTo(0);
  ...
  private void eliminarDistribucion(){
    try{
    System.out.println("producto enviado: "+VariablesProducto.cCodProd);
