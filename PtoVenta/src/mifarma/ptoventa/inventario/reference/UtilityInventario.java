...
public class UtilityInventario {
    ...
    public static void imprimeComprobanteGuias(JDialog pJDialog, ArrayList pDetalleComprobante,
                                               ArrayList pTotalesComprobante, String pTipCompPago,
                                               String pNumComprobante, int pTipoImpresion,
        ...
        if (VariablesInventario.vTipoFormatoImpresion <= 8) {
            ...
            if (!VariablesInventario.vIndTransfRecepCiega) {
                if (ConstantsPtoVenta.CODCIA_MIFARMA.equalsIgnoreCase(FarmaVariables.vCodCia)||
                // KMONCADA 19.05.2016 IMPRESION FORMATO DE MIFARM
                    ConstantsPtoVenta.CODCIA_ARCANGEL.equalsIgnoreCase(FarmaVariables.vCodCia) ||
                    ConstantsPtoVenta.CODCIA_JORSA.equalsIgnoreCase(FarmaVariables.vCodCia) ||
                // KMONCADA 10.08.2016 [PROYECTO M]
                    UtilityPtoVenta.isLocalVentaMayorista()){
                    //CHUANES-09.04.2014
                    //SE ADICIONA PARAMETRO pGuiaRemision
                    imprimeGuia2(pJDialog, fechaBD, pDetalleComprobante, pTipoImpresion, pGuiaRemision);
                }else if (ConstantsPtoVenta.CODCIA_FASA.equalsIgnoreCase(FarmaVariables.vCodCia))
                    imprimeGuiaFASA(pJDialog, fechaBD, pDetalleComprobante, pGuiaRemision);
                else if (ConstantsPtoVenta.CODCIA_BTL.equalsIgnoreCase(FarmaVariables.vCodCia)|| 
                    FarmaVariables.vCodCia.equals(ConstantsPtoVenta.CODCIA_BTL_AMAZONIA)) { //ASOSA - 30/07/2015 - IGVAZNIA
                    imprimeGuiaBTL(pJDialog, fechaBD, pDetalleComprobante, pGuiaRemision);
                }
            } else {
                if (ConstantsPtoVenta.CODCIA_MIFARMA.equalsIgnoreCase(FarmaVariables.vCodCia) ||
                    // KMONCADA 10.08.2016 [PROYECTO M]
            ...
            if (!VariablesInventario.vIndTransfRecepCiega) {
                ...
                // KMONCADA 10.08.2016 [PROYECTO M]
                    UtilityPtoVenta.isLocalVentaMayorista()){
                    imprimeGuia(pJDialog, fechaBD, pDetalleComprobante, pGuiaRemision);
                }else if (ConstantsPtoVenta.CODCIA_FASA.equalsIgnoreCase(FarmaVariables.vCodCia))
                    imprimeGuiaFASA(pJDialog, fechaBD, pDetalleComprobante, pGuiaRemision);
                else if (ConstantsPtoVenta.CODCIA_BTL.equalsIgnoreCase(FarmaVariables.vCodCia) || 
                    FarmaVariables.vCodCia.equals(ConstantsPtoVenta.CODCIA_BTL_AMAZONIA)) { //ASOSA - 30/07/2015 - IGVAZNIA
    ...
    public static void imprimeComprobantePago(JDialog pJDialog, ArrayList pDetalleComprobante,
        ...
        //validar aqui si es Guia generada en Transf x Recepcion Ciega
        if (!VariablesInventario.vIndTransfRecepCiega) {
            imprimeGuia(pJDialog, fechaBD, pDetalleComprobante, false);
        } else {
            imprimeGuiaRecepCiega(pJDialog, fechaBD, pDetalleComprobante);
            VariablesInventario.vIndTransfRecepCiega = false;
        }
    ...
    public static boolean procesoImpresionComprobante(JDialog pJDialog, Object pObjectFocus) {
        boolean presultado = false;
        try {
            ArrayList secGuia =
                DBInventario.getSecuencialGuia(VariablesInventario.vNumNotaEs, VariablesCaja.vSecImprLocalGuia);
      ...
      {*/
                VariablesCaja.vNumCompImprimir =
                        obtieneNumCompPago_ForUpdate(pJDialog, VariablesCaja.vSecImprLocalGuia, pObjectFocus);
                /*}
      else
      {
      reConfiguraCaja(VariablesCaja.vSecImprLocalGuia,VariablesCaja.vNumCompImprimir.substring(4,VariablesCaja.vNumCompImprimir.length()));
                ...
                if (VariablesCaja.vNumCompImprimir.equalsIgnoreCase("")) {
                    ...
                    FarmaUtility.showMessage(pJDialog, "No se pudo determinar el Numero de Guia. Verifique!!!",
                                             pObjectFocus);
                    return presultado;
                }
                if (!obtieneDetalleImprComp(pJDialog, ((ArrayList)secGuia.get(i)).get(0).toString().trim(),
                                            pObjectFocus))
                    return presultado;
                ...
                actualizaGuiaImpreso(VariablesInventario.vNumNotaEs,
                                     ((ArrayList)secGuia.get(i)).get(0).toString().trim(),
                                     VariablesCaja.vNumCompImprimir);
                //AGREGADO 13/06/2006 ERIOS
                actualizaNumGuiaKardex(VariablesInventario.vNumNotaEs, VariablesCaja.vArrayList_DetalleImpr,
                                       ((ArrayList)secGuia.get(i)).get(0).toString().trim());
                ...
                if (VariablesInventario.vCant.equalsIgnoreCase("") ||
                    VariablesInventario.vCant.equalsIgnoreCase("0")) {
                    continue;
            ...
            if (sql.getErrorCode() == 20061)
                FarmaUtility.showMessage(pJDialog,
                                         "No puede imprimir, porque no cuenta con guías secuenciales disponibles.\nVerifique y corrija el número de guía inicial.",
                                         pObjectFocus);
            else {
                log.error("", sql);
    ...
    public static void procesoImpresionGuias(JDialog pJDialog, Object pObjectFocus, int pTipoFormatoImpresion,
        ...
        reConfiguraCaja(VariablesCaja.vSecImprLocalGuia,VariablesCaja.vNumCompImprimir.substring(4,VariablesCaja.vNumCompImprimir.length()));
                ...
                if (!pGuiaRemision) {
                    ...
                    if (!obtieneDetalleImprGuia(pJDialog, ((ArrayList)secGuia.get(i)).get(0).toString().trim(),
                                                pObjectFocus))
                        return;
                }
                //log.debug("secCompPago : " + String.valueOf(i));
                ...
                imprimeComprobanteGuias(pJDialog, VariablesCaja.vArrayList_DetalleImpr,
                                        VariablesCaja.vArrayList_TotalesComp, VariablesVentas.vTip_Comp_Ped,
                                        VariablesCaja.vNumCompImprimir, pTipoFormatoImpresion, pGuiaRemision);
     ...
     *
     * @param pJDialog
     * @param pFechaBaseDatos
     * @param pDetallePedido
     * @param pGuiaRemision TRUE cuando se trate de una guia de remision
     * @throws Exception
     */
    public static void imprimeGuia(JDialog pJDialog, String pFechaBaseDatos, ArrayList pDetallePedido,
        ...
        vPrint.printLine("     " + "LOCAL: " + FarmaVariables.vDescCortaLocal + "          CORR." +
                         VariablesInventario.vNumNotaEs + "          No. " +
        ...
        if (!VariablesInventario.vDescMotivo_Transf_Larga.trim().equalsIgnoreCase("")) {
            vPrint.printLine("     " + "MOTIVO: " +
                             FarmaPRNUtility.alinearIzquierda(VariablesInventario.vDescMotivo_Transf.trim() + " - " +
                                                              VariablesInventario.vDescMotivo_Transf_Larga.trim(), 64),
                             true);
        } else {
            vPrint.printLine("     " + "MOTIVO: " +
                             FarmaPRNUtility.alinearIzquierda(VariablesInventario.vDescMotivo_Transf.trim(), 64),
                             true);
        }
        vPrint.printLine("                  " + dia + " " + mesLetra + " " + anio + "  " + "       " +
                         "                  " + dia + " " + mesLetra + " " + anio + "  " + "       ", true);
        vPrint.printLine("                  " +
                         FarmaPRNUtility.alinearIzquierda(VariablesInventario.vDestino_Transf.trim(), 64), true);
        vPrint.printLine("                  " +
                         FarmaPRNUtility.alinearIzquierda(VariablesInventario.vRucDestino_Transf.trim(), 64) +
                         "              " + FarmaVariables.vCodLocal.trim() + " - " +
                         FarmaVariables.vDescCortaLocal.trim(), true);
        vPrint.printLine("                  " +
                         FarmaPRNUtility.alinearIzquierda(VariablesInventario.vDirecDestino_Transf.trim(), 64) +
                         "              " + getDireccion(VariablesInventario.vDirecOrigen_Transf), true);
        vPrint.printLine("                  " +
                         FarmaPRNUtility.alinearIzquierda(VariablesInventario.vTransportista_Transf.trim(), 64) +
                         "              " + VariablesInventario.vCodDestino_Transf.trim() + " - " +
                         VariablesInventario.vDestino_Transf, true);
        vPrint.printLine("                  " +
                         FarmaPRNUtility.alinearIzquierda(VariablesInventario.vRucTransportista_Transf.trim(), 64) +
                         "              " + getDireccion(VariablesInventario.vDirecDestino_Transf), true);
        ...
        for (int i = 0; i < pDetallePedido.size(); i++) {
            // KMONCADA 05.11.2014 VALIDA SI ES GUIA DE REMISION A IMPRIMIR
            if (!pGuiaRemision) {
                ...
                vPrint.printCondensed("   " +
                        ...
                        FarmaPRNUtility.alinearIzquierda(((String)((ArrayList)pDetallePedido.get(i)).get(2)).trim(),
                                                         23) + " " +
                        FarmaPRNUtility.alinearDerecha(FarmaPRNUtility.tabular(((String)((ArrayList)pDetallePedido.get(i)).get(3)).trim(),
                                                                               5, "/"), 12), true);
                linea += 1;
            } else {
                ArrayList array = (ArrayList)pDetallePedido.get(i);
                String desc = ((String)array.get(0)).trim();
                vPrint.printCondensed(FarmaPRNUtility.llenarBlancos(6 + 2) +
                                      FarmaPRNUtility.alinearIzquierda(desc, 100), true);
            }
        }
        vPrint.activateCondensed();
    ...
    public static void imprimeGuiaFASA(JDialog pJDialog, String pFechaBaseDatos, ArrayList pDetallePedido,
                                       boolean pGuiaRemision) throws Exception {
        FarmaPrintService vPrint = new FarmaPrintService(66, VariablesCaja.vRutaImpresora, false);
        //si no se puede iniciar el servicio de impresión
        if (!vPrint.startPrintService())
        ...
        if (VariablesInventario.vTipoDestino_Transf.equals(ConstantsPtoVenta.LISTA_MAESTRO_LOCAL) ||
            ...
            vPrint.printLine(FarmaPRNUtility.llenarBlancos(12) +
                             FarmaPRNUtility.alinearIzquierda(dia + "/" + mesLetra + "/" + anio, 25), true);
            //Linea 9 - se imprime el punto de partida
            vPrint.printLine(FarmaPRNUtility.llenarBlancos(12) +
                             FarmaPRNUtility.alinearIzquierda(FarmaVariables.vCodLocal.trim() + " - " +
                                                              FarmaVariables.vDescCortaLocal.trim(), 40), true);
            ...
            vPrint.printLine(FarmaPRNUtility.llenarBlancos(12) +
                             ...
                             'X' + FarmaPRNUtility.llenarBlancos(14) +
                             FarmaPRNUtility.alinearIzquierda(VariablesInventario.vTransportista_Transf.trim(), 30),
                             true);
        }
        ...
        vPrint.printLine(FarmaPRNUtility.llenarBlancos(12) +
                         FarmaPRNUtility.alinearIzquierda(VariablesInventario.vCodDestino_Transf.trim() + " - " +
                                                          VariablesInventario.vDestino_Transf, 70), true);
        ...
        for (int i = 0; i < pDetallePedido.size(); i++) {
            ...
            if (!pGuiaRemision) {
                ...
                vPrint.printLine(FarmaPRNUtility.alinearDerecha(cod, 8) + FarmaPRNUtility.llenarBlancos(5) +
                                 FarmaPRNUtility.alinearIzquierda(desc, 78) + FarmaPRNUtility.llenarBlancos(3) +
                                 FarmaPRNUtility.alinearIzquierda(pres, 8) + FarmaPRNUtility.llenarBlancos(3) +
                                 FarmaPRNUtility.alinearIzquierda(cant, 10), true);
            } else {
                String desc = ((String)temp.get(0)).trim();
                vPrint.printLine(FarmaPRNUtility.llenarBlancos(8 + 5) + FarmaPRNUtility.alinearIzquierda(desc, 100),
                                 true);
            }
        }
    ...
    private static void imprimeGuia2(JDialog pJDialog, String pFechaBaseDatos, ArrayList pDetallePedido,
                                     int pTipoImpresion, boolean pGuiaRemision) throws Exception {
        ...
        vPrint.printLine("                  " + dia + " " + mesLetra + " " + ano + "  " + "       " +
        ...
        for (int i = 0; i < pDetallePedido.size(); i++) {
            ...
            if (!pGuiaRemision) {
                ...
                vPrint.printCondensed("   " + FarmaPRNUtility.alinearDerecha(cod, 6) + "  " +
                                      FarmaPRNUtility.alinearIzquierda(desc, 78 + 10) + " " +
                                      FarmaPRNUtility.alinearIzquierda(pres, 23) + " " +
                                      FarmaPRNUtility.alinearDerecha(FarmaPRNUtility.tabular(cant, 5, "/"), 12), true);
            } else {
                String desc = ((String)array.get(0)).trim();
                // KMONCADA 05.11.2014 ESPACIO DE IMPRESION DE DETALLE DE GUIA
                vPrint.printCondensed(FarmaPRNUtility.llenarBlancos(6 + 2) +
                                      FarmaPRNUtility.alinearIzquierda(desc, 100), true);
            }
            linea += 1;
    ...
    private static boolean obtieneDetalleImprComp(JDialog pJDialog, String pSecGuia, Object pObjectFocus) {
        ...
        try {
            DBInventario.obtieneInfoDetalleImpresionGuia(VariablesCaja.vArrayList_DetalleImpr,
                                                         VariablesInventario.vNumNotaEs, pSecGuia);
    ...
    private static void actualizaNumGuiaKardex(String pNumPedVta, ArrayList pDetallePedido,
                                               String secGuia) throws SQLException {
        for (int i = 0; i < pDetallePedido.size(); i++) {
            DBInventario.actualizaNumGuiaKardex(pNumPedVta, secGuia,
                                                ((String)((ArrayList)pDetallePedido.get(i)).get(0)).trim());
        }
    }
    ...
    public static boolean guardarCantidadIngresadaMatriz(JDialog dialogo, Object objeto) {
        boolean retorno;
        try {
            DBInventario.guardarCantPedRepMatriz(VariablesInventario.vNroPed_PedRep,
    ...
    public static void guardarCantidadAdicional() throws SQLException {
        DBInventario.guardarCantAdicPedRepTemp(VariablesInventario.vCodProd_PedRep,
    ...
    public static void guardarCantidadPedidoAdicionalLocal(String codProd, String cCantSol, String cCantAuto,
        ...
        try {
            DBInventario.guardarCantidadPedidoAdicionalLocal(codProd, cCantSol, cCantAuto, cIndAutori);
        } catch (SQLException sql) {
            log.error("", sql);
            ;
        }
    ...
    public static void guardarCantidadPedidoAdicionalMatriz(String codProd, String cCantAuto,
        ...
        try {
            DBInventario.guardarCantidadPedidoAdicionalMatriz(codProd, cCantAuto, cIndAutori);
        } catch (SQLException sql) {
            log.error("", sql);
            ;
        }
    ...
    private static void imprimeGuiaRecepCiega(JDialog pJDialog, String pFechaBaseDatos,
                                              ArrayList pDetallePedido) throws Exception {
        FarmaPrintService vPrint = new FarmaPrintService(66, VariablesCaja.vRutaImpresora, false);
        if (!vPrint.startPrintService())
            throw new Exception("Error en Impresora. Verifique !!!");
        vPrint.activateCondensed();
        ...
        if (!VariablesRecepCiega.vDescMotivo_Transf_Larga.trim().equalsIgnoreCase("")) {
            vPrint.printLine("     " + "MOTIVO: " +
                             FarmaPRNUtility.alinearIzquierda(VariablesRecepCiega.vDescMotivo_Transf.trim() + " - " +
                                                              VariablesRecepCiega.vDescMotivo_Transf_Larga.trim(), 64),
                             true);
        } else {
            vPrint.printLine("     " + "MOTIVO: " +
                             FarmaPRNUtility.alinearIzquierda(VariablesRecepCiega.vDescMotivo_Transf.trim(), 64),
                             true);
        }
        vPrint.printLine("                  " + dia + " " + mesLetra + " " + ano + "  " + "       " +
                         "                  " + dia + " " + mesLetra + " " + ano + "  " + "       ", true);
        vPrint.printLine("                  " +
                         FarmaPRNUtility.alinearIzquierda(VariablesRecepCiega.vDestino_Transf.trim(), 64), true);
        vPrint.printLine("                  " +
                         FarmaPRNUtility.alinearIzquierda(VariablesRecepCiega.vRucDestino_Transf.trim(), 64) +
                         "              " + FarmaVariables.vCodLocal.trim() + " - " +
                         FarmaVariables.vDescCortaLocal.trim(), true);
        vPrint.printLine("                  " +
                         FarmaPRNUtility.alinearIzquierda(VariablesRecepCiega.vDirecDestino_Transf.trim(), 64) +
                         "              " + getDireccion(VariablesRecepCiega.vDirecOrigen_Transf), true);
        vPrint.printLine("                  " +
                         FarmaPRNUtility.alinearIzquierda(VariablesRecepCiega.vTransportista_Transf.trim(), 64) +
                         "              " + VariablesRecepCiega.vCodDestino_Transf.trim() + " - " +
                         VariablesRecepCiega.vDestino_Transf, true);
        vPrint.printLine("                  " +
                         FarmaPRNUtility.alinearIzquierda(VariablesRecepCiega.vRucTransportista_Transf.trim(), 64) +
                         "              " + getDireccion(VariablesRecepCiega.vDirecDestino_Transf), true);
        //vPrint.printLine("                  " + VariablesInventario.vDirecTransportista_Transf.trim(),true);
        //vPrint.printLine("                  " + VariablesInventario.vPlacaTransportista_Transf.trim(),true);
        //vPrint.printLine("                  " + "",true);
        // vPrint.printLine("                  " + VariablesInventario.vDirecDestino_Transf,true);
        ...
        for (int i = 0; i < pDetallePedido.size(); i++) {
            ...
            vPrint.printCondensed("   " +
                    ...
                    /*FarmaPRNUtility.alinearDerecha(((String) ((ArrayList) pDetallePedido.get(i)).get(3)).trim(),
                                                              5), true);*/
                    FarmaPRNUtility.alinearIzquierda(((String)((ArrayList)pDetallePedido.get(i)).get(2)).trim(), 23) +
                    " " +
                    FarmaPRNUtility.alinearDerecha(FarmaPRNUtility.tabular(((String)((ArrayList)pDetallePedido.get(i)).get(3)).trim(),
                                                                           5, "/"), 12), true);
            linea += 1;
    ...
    private static void imprimeGuiaRecepCiegaFasa(JDialog pJDialog, String pFechaBaseDatos,
                                                  ArrayList pDetallePedido) throws Exception {
        FarmaPrintService vPrint = new FarmaPrintService(66, VariablesCaja.vRutaImpresora, false);
        if (!vPrint.startPrintService())
            throw new Exception("Error en Impresora. Verifique !!!");
        ...
        vPrint.printLine(FarmaPRNUtility.llenarBlancos(7) + dia + " " + mesLetra + " " + ano + "                " +
                         dia + " " + mesLetra + " " + ano, true);
        //se imprime el punto de partida
        vPrint.printLine(FarmaPRNUtility.llenarBlancos(7) +
                         FarmaPRNUtility.alinearIzquierda(FarmaVariables.vCodLocal.trim() + " - " +
                                                          FarmaVariables.vDescCortaLocal.trim(), 85), true);
        ...
        vPrint.printLine(FarmaPRNUtility.llenarBlancos(92) +
                         FarmaPRNUtility.alinearIzquierda(VariablesRecepCiega.vRucTransportista_Transf.trim(), 40),
                         true);
        ...
        vPrint.printLine(FarmaPRNUtility.llenarBlancos(7) +
                         FarmaPRNUtility.alinearIzquierda(VariablesRecepCiega.vCodDestino_Transf.trim() + " - " +
                                                          VariablesRecepCiega.vDirecDestino_Transf, 85), true);
    ...
    private static void imprimeGuiaRecepCiega2(JDialog pJDialog, String pFechaBaseDatos, ArrayList pDetallePedido,
        ...
        try {
            ...
            if (rpta.trim().equalsIgnoreCase("S"))
                ...
                FarmaUtility.showMessage(pDialog,
                                         "No existe dato para validar si es obligatorio ingresar la Fecha Vencimiento.\n" +
                        "Comuníquese con el operador.", pObj);
            }
        } catch (SQLException e) {
            flag = false;
     ...
     * @return
     */
    public static boolean validaGuias(JDialog pJDialog, String vEstadoNota, Object btnRelacionProductos,
                                      String vNumNota) {
        boolean retorno;
        if (!vEstadoNota.equals(FarmaConstants.INDICADOR_N)) {
            try {
                ...
                if (secGuia.size() == 0) {
                    retorno = false;
                    FarmaUtility.showMessage(pJDialog, "No existe guías pendientes de impresión.",
     ...
     * @return
     */
    public static boolean reimprimir(JDialog pJDialog, JTable tblListaProductos, Object btnRelacionProductos,
                                     String vNumNota) {
        boolean retorno = false;
        if (cargaCabecera(pJDialog, btnRelacionProductos, vNumNota))
            try {
                ...
                UtilityInventario.procesoImpresionGuias(pJDialog, tblListaProductos,
                                                        VariablesInventario.vTipoFormatoImpresion);
    ...
    public static void imprimeGuiaBTL(JDialog pJDialog, String pFechaBaseDatos, ArrayList pDetallePedido,
                                      boolean pGuiaRemision) throws Exception {
        FarmaPrintService vPrint = new FarmaPrintService(66, VariablesCaja.vRutaImpresora, false);
        ...
        //se dejan 3 lineas para la cabecera
        for (int i = 0; i < 4; i++) {
            vPrint.printLine(vMargen + "", true); //vNumeroPedido +
        }
        //FECHA DE EMISION
        vPrint.printLine(vMargen + FarmaPRNUtility.llenarBlancos(15) +
                         FarmaPRNUtility.alinearIzquierda(dia + "/" + mesLetra + "/" + anio, 25), true);
        //PUNTO DE PARTIDA EN NUESTRO CASO CODIGO DE LOCAL Y DESCRIPCION CORTA DE LOCAL
        //FarmaPRNUtility.alinearIzquierda(VariablesInventario.vDirecOrigen_Transf.trim()
        vPrint.printLine(vMargen + FarmaPRNUtility.llenarBlancos(20) +
                         FarmaPRNUtility.alinearIzquierda(FarmaVariables.vCodLocal.trim() + " - " +
                                                          FarmaVariables.vDescCortaLocal.trim(), 40), true);
        //DATOS DEL DESTINO(DATO AMBIGUO)
        vPrint.printLine(vMargen + "", true);

        // NOMBRE O RAZON SOCIAL (DESTINATARIO)
        vPrint.printLine(vMargen + FarmaPRNUtility.llenarBlancos(17) +
                         FarmaPRNUtility.alinearIzquierda(VariablesInventario.vCodDestino_Transf.trim() + " - " +
                                                          VariablesInventario.vDestino_Transf, 70), true);


        //DIRECCION DEL PUNTO DE LLEGADA(DESTINATARIO)
        vPrint.printLine(vMargen + FarmaPRNUtility.llenarBlancos(20) +
                         FarmaPRNUtility.alinearIzquierda(VariablesInventario.vDirecDestino_Transf.trim(), 82), true);
        //RUC DESTINO
        vPrint.printLine(vMargen + FarmaPRNUtility.llenarBlancos(15) +
                         FarmaPRNUtility.alinearIzquierda(VariablesInventario.vRucDestino_Transf.trim(), 82), true);
        ...
        for (int i = 0; i < pDetallePedido.size(); i++) {
            ...
            if (!pGuiaRemision) {
                ...
                vPrint.printLine(vMargen + FarmaPRNUtility.alinearDerecha(cod, 8) + FarmaPRNUtility.llenarBlancos(5) +
                                 FarmaPRNUtility.alinearIzquierda(desc, 85) + FarmaPRNUtility.llenarBlancos(5) +
                                 FarmaPRNUtility.alinearIzquierda(pres, 8) + FarmaPRNUtility.llenarBlancos(5) +
                                 FarmaPRNUtility.alinearIzquierda(cant, 10), true);
            } else {
                String desc = ((String)temp.get(0)).trim();
                vPrint.printLine(vMargen + FarmaPRNUtility.llenarBlancos(8 + 5) +
                                 FarmaPRNUtility.alinearIzquierda(desc, 100), true);
            }
        ...
        int numLineas = 19 + pDetallePedido.size();
        if (numLineas < 47) {
            for (int i = 1; i <= (48 - numLineas); i++) {
                vPrint.printLine("", true);
            }
        }


        //PLACA DEL TRANSPORTISTA
        /* vPrint.printLine(FarmaPRNUtility.llenarBlancos(12)+
                        FarmaPRNUtility.alinearIzquierda(VariablesInventario.vPlacaTransportista_Transf.trim(), 15),
                        true);*/

        //NOMBRE O RAZON SOCIAL DE TRANSPORTISTA
        /* vPrint.printLine(FarmaPRNUtility.llenarBlancos(15)+
                        FarmaPRNUtility.alinearIzquierda(VariablesInventario.vTransportista_Transf.trim(), 15),
                        true);*/


        if (VariablesInventario.vTipoDestino_Transf.equals(ConstantsPtoVenta.LISTA_MAESTRO_LOCAL) ||
            VariablesInventario.vTipoDestino_Transf.equals(ConstantsPtoVenta.LISTA_MAESTRO_MATRIZ)) { //PLACA DE TRASNPORTISTA
            vPrint.printLine(vMargen + FarmaPRNUtility.llenarBlancos(15) +
                             FarmaPRNUtility.alinearIzquierda(VariablesInventario.vPlacaTransportista_Transf.trim(),
                                                              15) + FarmaPRNUtility.llenarBlancos(67) + 'X', true);
            ...
            if (!VariablesInventario.vDescMotivo_Transf_Larga.trim().equalsIgnoreCase("")) {
                ...
                vPrint.printLine(vMargen + FarmaPRNUtility.llenarBlancos(15) +
                                 FarmaPRNUtility.alinearIzquierda(VariablesInventario.vTransportista_Transf.trim(),
                                                                  15) + FarmaPRNUtility.llenarBlancos(20) +
                                 "MOTIVO: " +
                                 FarmaPRNUtility.alinearIzquierda(VariablesInventario.vDescMotivo_Transf.trim() +
                                                                  " - " +
                                                                  VariablesInventario.vDescMotivo_Transf_Larga.trim(),
        ...
        } else { //PLACA DEL TRANSPORTISTA
            vPrint.printLine(vMargen + FarmaPRNUtility.llenarBlancos(12) +
                             FarmaPRNUtility.alinearIzquierda(VariablesInventario.vPlacaTransportista_Transf.trim(),
                                                              15), true);
            //NUMERO DE LICENCIA DE CONDUCIR
            vPrint.printLine(vMargen + "", true);
        ...
        vPrint.printLine(vMargen + "", true);
        vPrint.printLine(vMargen + FarmaPRNUtility.llenarBlancos(15) +
                         FarmaPRNUtility.alinearIzquierda(VariablesInventario.vRucTransportista_Transf.trim(), 15),
                         true);
    ...
    /**
     * Utilitario. Obtiene indicador de ingreso manual de la cantidad
     * @author ASOSA
     * @since 28/08/2014
     * @param numeroTabGral
     * @return
     */
    public static boolean isEntradaManual(int numeroTabGral) {
        boolean flag = false;
        String rpta = "";
        try {
            rpta = DBInventario.getValGralString(numeroTabGral);
        } catch (SQLException e) {
            log.info("ERROR en getIndIngresoManualIng: " + e.getMessage());
        }
        if (rpta.trim().equals("S")) {
            flag = true;
        }
        return flag;
    }
    ...
    /**
     * Metodo que devuelve un parametro string de pbl_tab_gral
     * @author ASOSA
     * @since 27/08/2014
     * @param codigo
     * @return
     */
    public static String obtenerParametroString(int codigo) {
        String rpta = "";
        try {
            rpta = DBInventario.getValGralString(codigo);
        } catch (SQLException e) {
            log.info("ERROR en obtenerParametroString: " + e.getMessage());
        }
        return rpta;
    }
     ...
     * @return
     */
    public static String obtenerParamTwoString(int codigo) {
        String rpta = "";
        try {
            rpta = DBInventario.getValGralTwoString(codigo);
        } catch (SQLException e) {
            log.info("ERROR en obtenerParametroString: " + e.getMessage());
        }
        return rpta;
     ...
     * @return
     */
    public static String getRangoProdRimac(String codProd) {
        String rpta = "";
        try {
            rpta = DBInventario.getRangoProdRimac(codProd);
        } catch (SQLException e) {
            log.info("ERROR en obtenerParametroString: " + e.getMessage());
        }
        return rpta;
     ...
     * @return
     */
    public static boolean isExistProdRimac(String codProd){
        boolean flag = false;
        String ind = "N";
        try {
            ind = DBInventario.getIndExistProdRimac(codProd).trim();
            if (ind.equals("S")) {
                flag = true;
            }
        } catch (SQLException e) {
            log.info("ERROR en isExistProdRimac: " + e.getMessage());
        }
        return flag;
    }

    /**
     * Metodo que devuelve un parametro integer de pbl_tab_gral
     * @author ASOSA
     * @since 27/08/2014
     * @param codigo
     * @return
     */
    public static String obtenerParametroInteger(int codigo) {
        String rpta = "";
        try {
            rpta = DBInventario.getValGralInteger(codigo);
        } catch (SQLException e) {
            log.info("ERROR en obtenerParametroString: " + e.getMessage());
        }
        return rpta;
     ...
     * @author ASOSA
     * @since 26/08/2014
     * @param codOrdenCompra
     * @return
     */
    public static boolean isOcRegularizado(String codOrdenCompra) {
        boolean flag = false;
        String rpta = "";
        try {
            rpta = DBInventario.getIndOcRegularizada(codOrdenCompra.trim());
        } catch (SQLException e) {

            log.info("ERROR en isOcRegularizado: " + e.getMessage());
        }
        if (rpta.trim().equals("S")) {
            flag = true;
        }
    ...
    public static boolean confirmarTransferencia(String pNumNota,String pCodLocalDestino,
       ...
       //if(!isEnvioTransWS()) 
         return confirmarTransferenciaJDBC(pNumNota,pCodLocalDestino,pTipoOrigen,vDlg,obj);
       //else
         //return confirmarTransferenciaWS(pNumNota,pCodLocalDestino,pTipoOrigen,vDlg,obj);
    }
    public static boolean confirmarTransferenciaJDBC(String pNumNota,String pCodLocalDestino,
                                              String pTipoOrigen,
                                                JDialog vDlg,Object obj){
        try {
            ...
            DBInventario.grabaInicioFinConfirmacionTransferencia(numTransf, "I",
                                                                 "N"); //JCHAVEZ 10122009 graba inicio de confimacion de transferencia en local origen
            DBInventario.confirmarTransferencia(numTransf);
            DBInventario.grabaInicioFinConfirmacionTransferencia(numTransf, "F",
                                                                 "N"); //JCHAVEZ 10122009 graba fin de confimacion de transferencia en local origen
            FarmaUtility.aceptarTransaccion();
    ...
    public static String getValueFieldArrayList(ArrayList pArrayList, int pRow, 
                                                int pColumn) {
        String value = "";
        if (pArrayList == null || pRow < 0 || pArrayList.size() <= pRow || pColumn < 0 || ((ArrayList)pArrayList.get(pRow)).size() <= pColumn)
            return value;
        value = ((String)((ArrayList)pArrayList.get(pRow)).get(pColumn));
        return value;
    }
    
    public static boolean isEnvioTransWS() {
        boolean flag = false;
        String rpta = "";
        try {
            rpta = DBInventario.isEnvioTransWS();
        } catch (SQLException e) {

            log.info("ERROR en isEnvioTransWS: " + e.getMessage());
        }
        if (rpta.trim().equals("S")) {
            flag = true;
        }
