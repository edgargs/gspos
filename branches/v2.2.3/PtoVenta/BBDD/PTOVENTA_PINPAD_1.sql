...
--------------------------------------------------------
  ...
  BEGIN
       ...
       IF(CFP='00083') THEN
             ...
             select(CFP ||'Ã'||
                    ...
                    to_char(FPP.IM_TOTAL_PAGO,'9990.00')) INTO RESULTADO
  ...
  BEGIN
       IF (cTipTarjeta_in = 'M') THEN
            select DSC_AMOUNT into MONTO
            from vta_proceso_pinpad_mc
            where COD_GRUPO_CIA=cCodGrupoCia_in
                  and COD_CIA = cCodCia_in
                  and COD_LOCAL = cCodLocal_in
                  and DSC_APPROVAL_CODE = cNumAutorizacion_in
                  and TO_CHAR(FEC_CREA,'DD/MM/YYYY')=cFecha_in
                  and TIP_REGISTRO = 'COM'
                  and PTOVENTA_PINPAD.RET_NUM_REFERENCIA_PINPAD_MC(COD_PROCESO_PINPAD_MC)=cNumReferencia_in;
       ELSIF (cTipTarjeta_in = 'V') THEN
            ...
            where COD_GRUPO_CIA = cCodGrupoCia_in and
                  ...
                  DSC_NUM_AUTORIZACION_TRAMA = cNumAutorizacion_in and
                  DSC_NUM_REFERENCIA_TRAMA = cNumReferencia_in and
                  TIP_REGISTRO = 'P' and
                  TO_CHAR(FEC_CREA,'DD/MM/YYYY')=cFecha_in;
       END IF;
       return MONTO;
  EXCEPTION
  WHEN OTHERS THEN
  ...
  IS
      ...
      long_direccion integer;
	    ...
	    vDirecLocal PBL_LOCAL.DIREC_LOCAL_CORTA%TYPE;
  ...
  BEGIN
       --se consulta la cabecera
        ...
        IF (long_info_local>60) THEN
           select SUBSTR(info_local,0,60)||'<br>'||SUBSTR(info_local,61,long_info_local) 
           into info_local
           from dual;
        END IF;
