...
public class DBEpos {
    ...
    public static String getDocumentoElectronico(String cGrupoCia, String cCodLocal, String cNumPedidoVta,
        ...
        String pCUATRO_Det = "";
        List lstDetalle = FarmaDBUtility.executeSQLStoredProcedureListMap("FARMA_EPOS.F_VAR_TRM_GET_DET(?,?,?,?,?,?)", vParameters);
        if(lstDetalle.size() == 0){
            throw new Exception("Trama sin Detalle");
        }else{
        ...
        */
        ...
        String pONCE_MSJ_PERZONALIZADO =
            FarmaDBUtility.executeSQLStoredProcedureStr("FARMA_EPOS.F_VAR_TRM_GET_MSJ_PERSONALIZA(?,?,?,?,?,?)", vParameters);
        ...
        if (pUNO_Cab == null || pUNO_Cab.trim().length() == 0)
            throw new Exception("SIN DATOS trama de Cabecera");
        if (pDOS_Doc == null || pDOS_Doc.trim().length() == 0)
            throw new Exception("SIN DATOS trama de Documento");
        if (pTRES_Not == null || pTRES_Not.trim().length() == 0)
            throw new Exception("SIN DATOS trama de Nota");
        if (pCUATRO_Det == null || pCUATRO_Det.trim().length() == 0)
            throw new Exception("SIN DATOS trama de Detalle");
        ...
        if (pNUEVE_MSJ_BF == null || pNUEVE_MSJ_BF.trim().length() == 0)
            throw new Exception("SIN DATOS trama de Mensaje Antes");
        if (pDIEZ_MSJ_AF == null || pDIEZ_MSJ_AF.trim().length() == 0)
            throw new Exception("SIN DATOS trama de Mensaje Despues");

        return pUNO_Cab + pDOS_Doc + pTRES_Not + pCUATRO_Det + 
               //pCINCO_RG + 
               pSEIS_IG + pSIETE_REF + pOCHO_PP+
               pNUEVE_MSJ_BF +
               pDIEZ_MSJ_AF+pONCE_MSJ_PERZONALIZADO;
    ...
    public static String getTipProcesPago(String vNumCompPago) throws Exception {
        ...
        parametros.add(FarmaVariables.vCodGrupoCia);
        parametros.add(FarmaVariables.vCodLocal);
        parametros.add(VariablesVentas.vNumPedVta_new);
    ...
    public static void updDocE(String cGrupoCia, String cCodLocal, String cNumPedidoVta, String cSecCompPago,
        ...
        FarmaDBUtility.executeSQLStoredProcedure(null,"FARMA_EPOS.FN_MODIFICAR_DOC_NUM_E(?,?,?,?,?,?,?)",
                                                           vParameters,false);
    }
     ...
     * @throws SQLException
     */
    public static Map obtieneMsgContingencia(String pCodGrupoCia, String pCodLocal, String pNumPedVta, String pCodTipoComprobante) throws SQLException {
        ArrayList parametros = new ArrayList();
     ...
     * @throws SQLException
     */
    public static Map validaImpresionPendiente(String pCodGrupoCia, String pCodLocal, String pSecCompPago, String pCodTipoComprobante) throws SQLException {
        ArrayList parametros = new ArrayList();
        parametros.add(pCodGrupoCia);
        parametros.add(pCodLocal);
        ...
        return FarmaDBUtility.executeSQLStoredProcedureMap("FARMA_EPOS.VALIDA_IMPRESION_PENDIENTE(?,?,?,?)", parametros);
    }
     ...
     * @throws Exception
     */
    public static void deshabilitarFlagEComprobante(String pCodGrupoCia, String pCodLocal, String pNumPedVta, String pSecCompPago)throws Exception{
        ArrayList parametros = new ArrayList();
        parametros.add(pCodGrupoCia);
        parametros.add(pCodLocal);
        parametros.add(pNumPedVta);
        ...
        FarmaDBUtility.executeSQLStoredProcedure(null,"FARMA_EPOS.COMP_PAGO_CAMBIA_FLAG_E(?,?,?,?)", parametros,false);
    }
