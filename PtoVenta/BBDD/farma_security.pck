CREATE OR REPLACE PACKAGE PTOVENTA."FARMA_SECURITY" AS
  ...
  */
  ...
  -- dubilluz 13.01.2015              
  FUNCTION VALIDA_MARCA_ASISTENCIA(cCodGrupoCia_in IN CHAR,
                                   cCodLocal_in    IN CHAR,
                                   cSecUsu_in      IN CHAR) RETURN CHAR;    
 	  --Descripcion: Devuelve la cantidad de dias que falta por vencer la clave del usuario.
	  --Fecha       Usuario   Comentario
	  --25.02.2015  CHUANES    Creacion                                  
   FUNCTION VERIFICA_VENCIMIENTO_CLAVE(cCodGrupoCia_in IN CHAR,
                              cCodLocal_in    IN CHAR,
                              cSecUsuLoc_in   IN CHAR)
    RETURN CHAR; 
   --Descripcion: Devuleve  si un usuario debe cambiar su clave o no
	  --Fecha       Usuario   Comentario
	  --11.03.2015  CHUANES    Creacion 
    
    FUNCTION VALIDA_USUARIO_CAMBIO_CLAVE(cSecUsuLoc_in   IN CHAR)
    RETURN CHAR;
    --Descripcion: Recuerda solo una vez el cambio de clave en el dia
	  --Fecha       Usuario   Comentario
	  --12.03.2015  CHUANES    Creacion  
    
    FUNCTION RECORD_CAMBIO_CLAVE(cCodGrupoCia_in IN CHAR,
                                cCodLocal_in    IN CHAR,cSecUsuLoc_in   IN CHAR)
  ...
  BEGIN
    OPEN farcur FOR
      SELECT NVL(DESC_CORTA_LOCAL, ' ') || 'Ã' || NVL(DESC_LOCAL, ' ') || 'Ã' ||
             ...
             NVL(L.DIREC_LOCAL_CORTA, ' ') || 'Ã' ||
       -- 2016-05-09 CCASTILLO: Se agrega columna para saber el tipo de venta del local
             NVL(L.COD_TIP_LOCAL_VENTA , ' ')   
...
-- 2014-12-19 J0LIVA: Se modifica para que en un local TICO se registre SÓLO el primer ingreso de cualquier usuario
...
*/
        ...
        WHERE L.COD_GRUPO_CIA = cCodGrupoCia_in
          AND L.COD_LOCAL = cCodLocal_in
          AND U.COD_GRUPO_CIA = L.COD_GRUPO_CIA
          AND U.COD_LOCAL = L.COD_LOCAL
          AND TO_CHAR(SYSDATE,'HH24:MI:SS') >= '06:00:00'
          ...
          AND
             (
                 ( 
                   (L.COD_TIP_LOCAL_VENTA = '001') -- 2014-12-19 JOLIVA: En local Farma se permite registrar el ingreso de todo cargo permitido
                    AND
                    EXISTS
                    (
                     SELECT 1
                     FROM PBL_TAB_GRAL G
                     WHERE G.ID_TAB_GRAL = 187 
                      AND INSTR(G.LLAVE_TAB_GRAL, TRIM(T.COD_CARGO)) > 0
                      AND T.COD_CARGO IS NOT NULL
                    )
                 )
                 OR
                 ( (L.COD_TIP_LOCAL_VENTA = '002') -- 2014-12-19 JOLIVA: En local TICO se puede registrar cualquier usuario siempre y cuando no exista algún registro en el día
                   AND 
                   NOT EXISTS
                   (
                    SELECT 1
                    FROM PBL_REG_INGRESO_LOCAL
                    WHERE COD_GRUPO_CIA = cCodGrupoCia_in
                      AND COD_LOCAL = cCodLocal_in
...
FUNCTION VALIDA_MARCA_ASISTENCIA(cCodGrupoCia_in IN CHAR,
                                 cCodLocal_in    IN CHAR,
                                 cSecUsu_in      IN CHAR) RETURN CHAR IS
    v_cResultado CHAR(2);
    
   vhora number;
   vDia date;
   vIndValidaIngreso varchar2(5) := 'N';
  BEGIN
    
    if cSecUsu_in = '000' or cSecUsu_in >= '900' then
      v_cResultado := 'N';
          ...
          into  vhora 
          from  pbl_tab_gral t
          where  t.ID_TAB_GRAL = 652;
          

            select  max(it.fecha)
              into  vDia
              from  pbl_ingreso_personal it
              where it.fecha >= trunc(Sysdate - vhora/24) 
              and   it.dni = (
                              select u.dni_usu
                              from   pbl_usu_local u
                              where  u.cod_grupo_cia  = cCodGrupoCia_in
          ...
          if vDia is null then
           ...
           select case
                  when ig.entrada = null then 'S'
                  else 'N'
                  end
           into   v_cResultado
           from   pbl_ingreso_personal ig
           where  ig.fecha = vDia
           and    ig.dni =   (
                              select u.dni_usu
                              from   pbl_usu_local u
    ...
    INTO   vIndValidaIngreso
    FROM   pbl_tab_gral T
    WHERE  ID_TAB_GRAL = 663;
    exception
    when others then
      vIndValidaIngreso:= 'N';
    end;
    
    -- no valida por lo tanto NO OBLIGARA MARCAR ASISTENCIA
    if vIndValidaIngreso = 'N' then
       v_cResultado := 'N';    
    end if;
  ...
  END;
   ...
   /* ************************************************************************ */
    ...
    v_cEstUsu    PBL_USU_LOCAL.EST_USU%TYPE;
    v_cResultado CHAR(5);
    nDiasCamb    NUMBER;--CANTIDAD DE DIAS QUE A CAMBIA SU CLAVE POR ULTIMA VEZ
    nDiasRec     NUMBER;--A PARTIR DE CUANTOS DIAS RECORDAR CAMBIO DE CLAVE
    CANT         NUMBER;--CONSTANTE=90
    BEGIN
    ...
    SELECT TO_NUMBER(TRIM(A.LLAVE_TAB_GRAL)) INTO  nDiasRec
    FROM PBL_TAB_GRAL A
    WHERE A.ID_TAB_GRAL=651;
    ...
    IF(nDiasCamb-CANT<=nDiasRec AND nDiasCamb-CANT>0) THEN
    v_cResultado:=TO_CHAR(ABS(nDiasCamb-CANT));
    ELSE
    v_cResultado:='N';
    END IF;
  ...
  /*****************************************************/
   FUNCTION VALIDA_USUARIO_CAMBIO_CLAVE(cSecUsuLoc_in   IN CHAR)
    ...
    RETURN CHAR IS
    v_vSecUsuLocal PBL_USU_LOCAL.SEC_USU_LOCAL%TYPE;
    v_cResultado CHAR(1);
    BEGIN
    ...
    v_Cantidad NUMBER;
    BEGIN
    SELECT  COUNT(*)INTO v_Cantidad  FROM HIST_RECO_CAMBIO_CLAVE RC
    WHERE RC.COD_GRUPO_CIA=cCodGrupoCia_in
    AND RC.COD_LOCAL=cCodLocal_in
    AND RC.SEC_USU_LOCAL=cSecUsuLoc_in
    AND RC.FEC_RECOR BETWEEN TRUNC(SYSDATE) AND TRUNC(SYSDATE+1)-1/24/60/60 ;--FECMOD:16/04/2015  CHUANES
    
    IF v_Cantidad>0 THEN
    v_cResultado:='N';
    ELSE 
    INSERT INTO HIST_RECO_CAMBIO_CLAVE(COD_GRUPO_CIA,COD_LOCAL,SEC_USU_LOCAL) VALUES(cCodGrupoCia_in,cCodLocal_in,cSecUsuLoc_in);
    COMMIT;
    v_cResultado:='S';
    END IF;
