...
public class PrintConsejo {
    ...
    static void imprimirTermicoCode39(String tipoImpTermica, String nomImpTermica) {
        List<BeanImpresion> listDocumento ;
        listDocumento = new ArrayList<>();
        BeanImpresion bean = new BeanImpresion();
        bean.put("ID_DOC","1");
        bean.put("IP_PC","1");
        
        bean.put("VALOR","C99999"+FarmaVariables.vCodLocal+"12345678");        
        bean.put("TIPO_DATO","5"); //TD_BARCODE_CODE39
        
        bean.put("TAMANIO","2"); //TAMANIO_1
        bean.put("ALINEACION","C"); //ALING_CEN
        bean.put("NEGRITA","N"); //BOLD_DESACT
        bean.put("SUBRAYADO","N"); //SUBRAY_DESACT
        bean.put("INTERLINEADO","1"); //LINEADO_SIMPLE
        bean.put("COLOR_INVERSO","N"); //INVERSO_DESACT
        bean.put("SALTO_LINEA","S"); //SALTO_LINEA_ACT
        
        listDocumento.add(bean);
        
        if(listDocumento!=null){
            try {
                String rutaImpTermica;
                if(!nomImpTermica.equals("")){
                    rutaImpTermica = new UtilityImpCompElectronico(tipoImpTermica,nomImpTermica).getRuta();
                }else{
                    rutaImpTermica = new UtilityImpCompElectronico().getRuta();
                }
                
                impresionTermica(tipoImpTermica, rutaImpTermica, listDocumento);
            } catch (Exception e) {
                log.error("",e);
            }
        }
