CREATE OR REPLACE PACKAGE "PTOVENTA_FIDELIZACION" AS
...
--FIN ASOSA - 23/03/2015 - PTOSYAYAYAYA
  ...
  PROCEDURE FID_P_INSERT_CLIENTE(vDni_cli    IN CHAR,
                                 vNom_cli    IN VARCHAR2,
                                 vApat_cli   IN VARCHAR2 DEFAULT NULL,
                                 vAmat_cli   IN VARCHAR2 DEFAULT NULL,
                                 vEmail_cli  IN VARCHAR2 DEFAULT NULL,
                                 vFono_cli   IN CHAR DEFAULT NULL,
                                 vSexo_cli   IN CHAR DEFAULT NULL,
                                 vDir_cli    IN VARCHAR2,
                                 vFecNac_cli IN CHAR DEFAULT NULL,
                                 pCodTarjeta IN CHAR DEFAULT NULL,
                                 pCodLocal   IN CHAR,
                                 pUser       IN CHAR,
                                 pIndEstado  IN CHAR,
                                 ---agregadas
                                 ...
                                 , -- KMONCADA 14.04.2016
                                 pIndAgenteRetencion_in   IN CHAR DEFAULT NULL,
                                 pIndAgentePercepcion_in  IN CHAR DEFAULT NULL,
                                 pClienteDigemid_in     IN CHAR DEFAULT 'N'
                                 );
  ...
  **/
  ...
  FUNCTION FID_F_CUR_CAMP_X_TARJETA_NEW(cCodGrupoCia_in      IN CHAR,
                                        ...
                                        cCodConvenio_in      IN MAE_CONVENIO.COD_CONVENIO%TYPE DEFAULT NULL)
    RETURN FarmaCursor;
...
/*  -- Author  : ASOSA
  ...
  PROCEDURE FID_P_INS_PROC_PTOS(cCodGrupoCia_in     CHAR,
                                ...
                                vOutput             VARCHAR2,
                                vFecha              VARCHAR2,
                                vNumPedVta          VARCHAR2 default null);
  -- Author  : ASOSA
  -- Created : 06/03/2015
 ...
 FUNCTION FID_F_GET_NUM_FORMATED(nNumber_in in number)
  ...
  PROCEDURE F_ACTUALIZA_ESTADO_AFILIA_PTOS(cNroTarjeta_in   IN VARCHAR2,
                                           cNroDocumento_in IN VARCHAR2,
                                           cEstadoTrsx_in   IN CHAR);
  PROCEDURE P_BLOQUEO_TARJETA(cNroTarjeta_in IN VARCHAR2,
                              cLoginUsu_in   IN PBL_USU_LOCAL.LOGIN_USU%TYPE);

   FUNCTION F_IS_DOC_LOCKED(cNumDocId_in       IN CHAR,
                           cFidTarjeta_in     IN CHAR default 'N',
                           cCodGrupoCia_in    IN CHAR default 'N',
                           cCodLocal_in       IN CHAR default 'N'
                           )
  RETURN CHAR;
  
     FUNCTION F_IS_PROD_LOCKED(cCodProd_in       IN CHAR) 
    RETURN char;
    
    
     FUNCTION F_IS_PROD_LOCKED_DIA(cCodProd_in       IN CHAR) 
    RETURN char ;
    
    
     FUNCTION F_IS_PROD_LOCKED_SEMANA(cCodProd_in       IN CHAR) 
    RETURN char ;
    
         FUNCTION F_PROD_UNID_MAX_DIA(cCodProd_in       IN CHAR) 
    RETURN number;
    
         FUNCTION F_PROD_UNID_MAX_SEMANA(cCodProd_in       IN CHAR) 
    RETURN number;
                              
                              
    FUNCTION F_TOT_UNID_PROD_SEMANA(cCodProd_in       IN CHAR,
                                     cDNi_in in char) 
    RETURN number;
    
    
     FUNCTION F_TOT_UNID_PROD_DIA(cCodProd_in       IN CHAR,
                                     cDNi_in in char) 
    RETURN number;
  FUNCTION FID_IND_REDONDEO_PREC_CAMP(cCodCampana_in in char)
    RETURN char ;
    
  FUNCTION FID_CUR_TIPO_DOCUMENTO 
    RETURN FARMACURSOR;

/*********************************************************************************/
  
  FUNCTION FID_GET_COD_TIPO_DOC_CLIENTE(cNroDocumento_in IN PBL_CLIENTE.DNI_CLI%TYPE)
    RETURN CHAR;
...
/*********************************************************************************/    
  ...
  -- Author  : KMONCDA
  -- Created : 24/09/2015
  -- Descripcion : DETERMINA EL TIPO DE DOCUMENTO
  FUNCTION F_DETERMINA_TIPO_DOC(cNroDocumento_in IN PBL_CLIENTE.DNI_CLI%TYPE)
    RETURN CHAR;

  /**************************************************************************
  ** OBJETIVO             : VALIDAR SI LA TARJETA ESTA O NO ENCRIPTADA
  ** NOMBRE               : F_VERIFICA_NUMTARJETA_ENCRP
  ** AUTOR                : DESARROLLO5 - ALEJANDRO NUÑEZ
  ** FECHA CREACION       : 16/10/2015
  **************************************************************************/
  FUNCTION F_VERIFICA_NUMTARJETA_ENCRP(pCodLocal IN CHAR,
                                       pGrupoCia IN CHAR,
                                       pNumeroTarjeta IN VARCHAR2)
  RETURN VARCHAR2; 
  
  /************************************************************************************/
	--Descripcion: RETORNA LA LISTA DE CLIENTES QUE COINCIDAN CON LAS CONDICIONES DE BUSQUEDA
  --FECHA         USUARIO		   COMENTARIO
  --14.04.2016    KMONCADA     Creación
  FUNCTION F_CUR_BUSCA_CLIENTE(cCodGrupoCia_in     IN CHAR,
                               cCodLocal_in        IN CHAR,
                               cValorBusqueda_in   IN CHAR,
                               cTipoBusqueda_in    IN CHAR)
    RETURN FARMACURSOR;

  /************************************************************************************/
	--Descripcion: REALIZA ACTUALIZACION Y CRECION DE CLIENTES
  --FECHA         USUARIO		   COMENTARIO
  --14.04.2016    KMONCADA     Creación
  --06.06.2016    KMONCADA     [PROYECTO M] INDICADOR DE CLIENTE DIGEMID  
  PROCEDURE P_MANTENIMIENTO_CLIENTE(pCodLocal_in           IN CHAR,
                                    pTipoDocumento_in      IN CHAR,
                                    pNumDocumento_in       IN CHAR,
                                    pRazonSocial_in        IN VARCHAR2,
                                    pDireccion_in          IN VARCHAR2,
                                    pIsAgenteRetencion_in  IN CHAR DEFAULT NULL,
                                    pIsAgentePercepcion_in IN CHAR DEFAULT NULL,
                                    pClienteDigemid_in     IN CHAR DEFAULT 'N',
                                    pUser_in               IN CHAR
                                    );
END PTOVENTA_FIDELIZACION;
/
CREATE OR REPLACE PACKAGE BODY "PTOVENTA_FIDELIZACION" AS
  /****************************************************************************/
  FUNCTION FID_F_VAR_VALIDA_CLIENTE(pCodTarjeta IN CHAR, pCodLocal IN CHAR)
    RETURN VARCHAR2 AS
    vCount1     NUMBER(1);
...
--end if;
  ...
  PROCEDURE FID_P_INSERT_CLIENTE(vDni_cli    IN CHAR,
    ...
    vCount02        NUMBER;
	  iy              INTEGER:=0;
    --pTipoDoc        PBL_CLIENTE.COD_TIP_DOCUMENTO%TYPE;
  BEGIN
  ...
  */  
  ...
  /*IF LENGTH(vDni_cli_2) = 8 THEN
    pTipoDoc := '01 ';
  ELSIF LENGTH(vDni_cli_2) = 9 THEN
    pTipoDoc := '02 ';
  ELSE
    pTipoDoc := '03 ';
  END IF;*/
      ...
      IF (vCount = 0) THEN
        INSERT INTO PBL_CLIENTE
          (DNI_CLI,
           ...
           COD_TIP_DOCUMENTO ,
           IND_AGENTE_RETENCION, 
           IND_AGENTE_PERCEPCION,
           FLG_DIGEMID
           )
        VALUES
          (vDni_cli_2,
           ...
           DECODE(vAmat_cli, 'N', NULL, null, null, vAmat_cli),
           DECODE(vEmail_cli, 'N', NULL, null, null, vEmail_cli),
           decode(vFono_cli, 'N', NULL, null, null,vFono_cli), --TO_NUMBER(vFono_cli, '9999999.000')),
           DECODE(vSexo_cli, 'N', NULL, null, null, vSexo_cli),
           DECODE(vDir_cli, 'N', NULL, null, null, vDir_cli),
           /*           DECODE(vFecNac_cli,
                             'N',
                             ...
                             null,
                             TO_DATE(vFecNac_cli, 'DD/MM/YYYY')),*/
           to_date(DECODE(vFecNac_cli, 'N', NULL, null, null, vFecNac_cli), 'dd/mm/yyyy'),
           SYSDATE,
           pUser,
           NULL,
           NULL,
           ...
           --FIN ASOSA - 06/04/2015 - PTOSYAYAYAYA
           cTipDoc,
           NVL(pIndAgenteRetencion_in,'N'),
           NVL(pIndAgentePercepcion_in,'N'),
           --06.06.2016    KMONCADA     [PROYECTO M] INDICADOR DE CLIENTE DIGEMID
           pClienteDigemid_in
           );
      ELSE
       ...
       /* SELECT COUNT(*)
        ...
        UPDATE PBL_CLIENTE L
           SET NOM_CLI     = DECODE(vNom_cli, 'N', NULL, vNom_cli),
               ...
               FONO_CLI    = decode(vFono_cli, 'N', NULL, vFono_cli), --TO_NUMBER(vFono_cli, '9999999.000')),
               SEXO_CLI    = DECODE(vSexo_cli, 'N', NULL, vSexo_cli),
               DIR_CLI     = DECODE(vDir_cli, 'N', NULL, vDir_cli),
               FEC_NAC_CLI = to_date(decode(vFecNac_cli, 'N', null, vFecNac_cli),'dd/mm/yyyy'),
               /*DECODE(vFecNac_cli,'N', NULL, TO_DATE(vFecNac_cli, 'DD/MM/YYYY'))*/
               ...
               L.EMAIL = vEmail_cli,
               --FIN ASOSA - 06/04/2015 - PTOSYAYAYAYA
               COD_TIP_DOCUMENTO = cTipDoc,
               L.IND_AGENTE_RETENCION = NVL(pIndAgenteRetencion_in, L.IND_AGENTE_RETENCION),
               L.IND_AGENTE_PERCEPCION = NVL(pIndAgentePercepcion_in, L.IND_AGENTE_PERCEPCION),
               --06.06.2016    KMONCADA     [PROYECTO M] INDICADOR DE CLIENTE DIGEMID  
               L.FLG_DIGEMID = pClienteDigemid_in
...
COMMIT;
  ...
  BEGIN
    ...
    -- KMONCADA 27.08.2015 SE REALIZA CAMBIO PARA QUE SE APLIQUE EN EL PAQUETE DE FID_RENIEC
   ...
   /* select count(1)
    ...
    END IF;
    -- KMONCADA 24.09.2015 ACTUALIZA EL TIPO DE DOCUMENTO 
   /* UPDATE PBL_CLIENTE 
    SET COD_TIP_DOCUMENTO = F_DETERMINA_TIPO_DOC(vDni)
    WHERE DNI_CLI=vDni AND COD_TIP_DOCUMENTO IS NULL;*/
    ...
    IF cNroTarjetaLealtad_in IS NULL THEN
      ...
      OPEN curLista FOR
        SELECT COD_TARJETA
         ...
         --AND  COD_TARJETA LIKE '999'||cCodLocal_in||'%';
    ELSE
...
*/
  ...
  PROCEDURE FID_P_INSERT_TARJ_PED(pCodCia   IN CHAR,
   ...
   AS
    ...
    VCANT      number := 0;
    vDni_cli_2 varchar2(30);
    vNumTarjeta FID_TARJETA.COD_TARJETA%TYPE;
  BEGIN
    vDni_cli_2 := pDniCli;
    vDni_cli_2 := fid_valida_dni(vDni_cli_2);
    ...
    ELSE
      ...
      INTO vNumTarjeta
      FROM FID_TARJETA T
      WHERE T.DNI_CLI = pDniCli
      AND NVL(T.IND_PUNTOS,'N') = 'N';
      EXCEPTION
      ...
      IF vNumTarjeta IS NULL THEN
         vNumTarjeta := PTOVENTA_FIDELIZACION.FID_F_CHAR_CREA_NEW_TARJ_FID(vCodGrupoCia_in => pCodCia,
                                                            vCodLocal_in => pCodLocal,
                                                            vConcatenado_in => 'V99999',
                                                            vUsuID_in => pIduUsu);
                                                            
         
      END IF;
      
      INSERT INTO TMP_FID_TARJETA_PEDIDO
        (COD_GRUPO_CIA,
         COD_LOCAL,
         NUM_PEDIDO,
         COD_TARJETA,
  ...
  begin
    ...
    SELECT NVL(V.COD_CAMP_CUPON_PROD_USO, V.COD_CAMP_CUPON)
    INTO vCodCampana
    FROM VTA_CAMPANA_CUPON V
    WHERE V.COD_GRUPO_CIA = cCodGrupoCia_in
    AND   V.COD_CAMP_CUPON = cCodCampana_in;

      select count(1)
        into nCantExiteProd
        from vta_campana_prod_uso c
       where c.cod_grupo_cia = cCodGrupoCia_in
         and c.cod_camp_cupon = vCodCampana/*cCodCampana_in*/
         and c.cod_prod = cCodProducto_in;
      
      if nCantExiteProd > 0 then
        select decode(vIndDocLocked_in,'S',nvl(c.LOCKED_VALOR_CUPON,c.VALOR_CUPON),c.VALOR_CUPON),
               decode(vIndDocLocked_in,'S',nvl(c.LOCKED_MONTO_MAX_DESCT,c.MONTO_MAX_DESCT),c.VALOR_CUPON)
  ...
  FUNCTION FID_F_CHAR_CREA_NEW_TARJ_FID(vCodGrupoCia_in    IN VARCHAR2,
                                        ...
                                        vNroTarjetaLealtad IN CHAR DEFAULT NULL)
    RETURN CHAR AS
    vSecuencialNumera         VARCHAR2(30);
    vConcatenado              VARCHAR2(30);
    vNuevaTarjetaFidelizacion VARCHAR2(30);
    CC_COD_NUMERA             CHAR(3) := 100;
    vPrefijo_tarjeta          CHAR(6) := 'V99999';
  BEGIN
    IF vNroTarjetaLealtad IS NULL THEN 
      vSecuencialNumera := TRIM(Farma_Utility.COMPLETAR_CON_SIMBOLO(
                                                  Farma_Utility.OBTENER_NUMERACION(
                                                                     ...
                                                                     CC_COD_NUMERA),
                                                  8, '0', 'I'));
      vConcatenado := vPrefijo_tarjeta || vCodLocal_in || vSecuencialNumera;
      --vNuevaTarjetaFidelizacion := F_GENERA_EAN13(vConcatenado);
      vNuevaTarjetaFidelizacion := vConcatenado;
  ...
  BEGIN
    ...
    vIndDocLocked_in    := PTOVENTA_FIDELIZACION.F_IS_DOC_LOCKED(dDniCli);
      ...
      OPEN curLista FOR
        SELECT C.COD_CAMP_CUPON AS COD_CAMP_CUPON,
               ...
               (case
                when vIndDocLocked_in = 'N' then
                     nvl(fpc.descuento_personalizado, c.valor_cupon) 
                else
                     nvl(c.LOCKED_valor_cupon,nvl(fpc.descuento_personalizado, c.valor_cupon)) 
             ...
             (case
               ...
               LPAD(C.PRIORIDAD, 8, '0') || LPAD(C.RANKING, 8, '0') ||
               TIP_CUPON || TO_CHAR(100000 - 
               
               decode(vIndDocLocked_in,'S',nvl(c.LOCKED_valor_cupon,c.VALOR_CUPON),
                 c.VALOR_CUPON)
               , '00000.000') AS ORDEN
          FROM VTA_CAMPANA_CUPON C, --, FID_TARJETA T--, FID_CAMP_TARJETA CT
               (
         ...
         WHERE C.COD_GRUPO_CIA = cCodGrupoCia_in
           ...
           and ((exists (select 1
                          ...
                          where q.dni_cli = dDniCli
                            ...
                            and t.cod_camp_cupon = C.COD_CAMP_CUPON
                            -- KMONCADA 17.07.2015 SOLO VALIDA EL NUMERO DE LA TARJETA
                            -- PARA QUE NO DE CAMPAÑAS DE PROPIETARIOS Y CLUB DE DESCUENTOS
                            AND Q.COD_TARJETA = cCodTarjeta_in
                ...
                (SELECT 1
                 ...
                 SELECT 1
                  ...
                  where t.cod_grupo_cia = cCodGrupoCia_in
                    and t.COD_CAMP_CUPON = C.COD_CAMP_CUPON
                    and ROWNUM<2))--KMONCADA 11.09.2015 SOLUCIONA PROBLEMAS DE LENTITUD EN CONSULTA DE CAMPAÑAS
           AND DECODE(C.DIA_SEMANA,
                      NULL,
                      'S',
                      DECODE(C.DIA_SEMANA,
        ...
        SELECT C.COD_CAMP_CUPON AS COD_CAMP_CUPON,
               ...
               (case
                when vIndDocLocked_in = 'N' then
                     ...
                     nvl(c.LOCKED_valor_cupon,c.valor_cupon) 
               end )     AS "VALOR_CUPON", 
               TRIM(to_char(NVL(C.MONT_MIN_USO, 0), '99999999.999')) AS MONT_MIN_USO,
               NVL(C.UNID_MIN_USO, 0) AS UNID_MIN_USO,
             ...
             (case
               ...
               nvl(C.IND_VAL_COSTO_PROM, 'S') AS IND_VAL_COSTO_PROM,
               LPAD(C.PRIORIDAD, 8, '0') || LPAD(C.RANKING, 8, '0') ||
               TIP_CUPON || TO_CHAR(100000 - decode(vIndDocLocked_in,'S',nvl(c.LOCKED_valor_cupon,c.VALOR_CUPON),
                 c.VALOR_CUPON), '00000.000') AS ORDEN
          FROM VTA_CAMPANA_CUPON C --, FID_TARJETA T--, FID_CAMP_TARJETA CT
         WHERE C.COD_GRUPO_CIA = cCodGrupoCia_in
           AND C.ESTADO = 'A'
           AND C.IND_FID = 'N'
        ...
        -- KMONCADA 09.05.2016 CUPONES PARA CONVENIO
        AND C.COD_CAMP_CUPON IN
          (SELECT C.COD_CAMP_CUPON
           FROM DUAL
           WHERE cCodConvenio_in IS NULL
           UNION ALL
           SELECT COD_CAMP_CUPON
           FROM REL_VTA_CAMPANA_CONVENIO_USO
           WHERE COD_GRUPO_CIA = cCodGrupoCia_in
           AND COD_CONVENIO = cCodConvenio_in)
        ;
 ...
 -- DUBILLUZ 07.11.2012
  ...
  BEGIN
    ...
    if vIndDocLocked_in = 'S' then
        ...
        select case
               when nvl(u.max_unid_dcto,c.unid_max_prod)>nvl(c.locked_unid_max_prod,c.unid_max_prod)
                 then nvl(c.locked_unid_max_prod,c.unid_max_prod)
               else
                     nvl(u.max_unid_dcto,c.unid_max_prod)
               end      
          into vResultado
          from vta_campana_cupon c,
               vta_campana_prod_uso u
         where c.cod_grupo_cia = vCodGrupoCia_in
           and c.cod_camp_cupon = vCodCampana_in
           and c.cod_grupo_cia = u.cod_grupo_cia
           and c.cod_camp_cupon = u.cod_camp_cupon
           and u.cod_prod = vCodProd_in;
    else
...
FUNCTION FID_IS_FP_X_USO(cGrupoCia_in    IN CHAR,
  ...
  FUNCTION FID_F_FORMA_PAGO_TARJETA(A_NUM_TARJETA CHAR /*, A_FLG_TIPO CHAR */)
    ...
    vTarjetaDesencriptada VARCHAR2(30);
  BEGIN
	--[Desarrollo5] 21.10.2015 Es para que valide los numeros de la tarjeta encriptada
    IF LENGTH(TRIM(A_NUM_TARJETA)) = 30 THEN
        vTarjetaDesencriptada := farma_utility.F_DESENCRIPTAR_TEXTO_SIMPLE(vCadenaTexto => A_NUM_TARJETA);
    ELSE
        vTarjetaDesencriptada := A_NUM_TARJETA;
    END IF;

    --primero detecta que tarjeta es solo en base al BIN
    if FN_LISTA_TJT_NUMEROM10(vTarjetaDesencriptada, 1) = 0 then
      return '*';
    else
      if FN_LISTA_TJT_NUMEROM10(vTarjetaDesencriptada, 1) = 1 then
        BEGIN
          SELECT COD_FORMA_PAGO,
           ...
           INSTR(COD_BIN_TARJETA, SUBSTR(vTarjetaDesencriptada, 1, 6)) > 0
            ...
            BEGIN
              ...
              SELECT COD_HIJO, DES_HIJO, FLG_RETIRO_EFE, FLG_VALIDA_MOD
                INTO C_COD_TARJETA,
                     ...
                     V_FLG_VALIDA_MOD
                FROM AUX_FORMA_PAGO
               WHERE INSTR(COD_BIN_TARJETA, SUBSTR(vTarjetaDesencriptada, 1, 4)) > 0
                 AND COD_FORMA_PAGO = '002';
              ...
              WHEN NO_DATA_FOUND THEN
                BEGIN
                  ...
                  --WHERE COD_BIN_TARJETA LIKE SUBSTR( A_NUM_TARJETA ,1,2) ;
                   WHERE INSTR(COD_BIN_TARJETA, SUBSTR(vTarjetaDesencriptada, 1, 2)) > 0
                     AND COD_FORMA_PAGO = '002';
                    ...
                    BEGIN
                      SELECT COD_HIJO,
                       ...
                       WHERE COD_BIN_TARJETA LIKE
                             SUBSTR(vTarjetaDesencriptada, 1, 1)
                         AND COD_FORMA_PAGO = '002';
        ...
        IF C_COD_TARJETA <> '*' AND V_FLG_VALIDA_MOD = '1' THEN
          IF INSTR(vTarjetaDesencriptada, '00000000') = 0 THEN
            --PARA CUANDO NO TIENEN LA TARJETA Y LO RELLENAN DE CEROS
            FOR I IN REVERSE 1 .. LENGTH(vTarjetaDesencriptada) LOOP
              V_RESULT := SUBSTR(vTarjetaDesencriptada, I, 1) * X;
              IF V_RESULT >= 10 THEN
  ...
  BEGIN
    ...
    -- KMONCADA 27.08.2015 VALIDARA SEGUN TAB LA LONGITUD DE LA TARJETA A EVALUAR
    BEGIN
      SELECT TO_NUMBER(A.LLAVE_TAB_GRAL,'9990.00') 
      INTO nCantMinimaTarjeta 
      FROM PBL_TAB_GRAL A 
      WHERE A.ID_TAB_GRAL=550;
    EXCEPTION
      WHEN OTHERS THEN
        nCantMinimaTarjeta := 0;
    END;
    
    IF LENGTH(TRIM(A_NUM_TARJETA)) >= nCantMinimaTarjeta THEN
  ...
  FUNCTION FN_LISTA_TJT_NUMEROM10(A_NUM_TARJETA CHAR, A_FLG_VALIDA CHAR)
    ...
    vTarjetaEncriptada VARCHAR2(30);
  BEGIN
	--[Desarrollo5] 21.10.2015 Es para que valide la tarjeta encriptada
    IF LENGTH(A_NUM_TARJETA) = 30 THEN
        vTarjetaEncriptada := farma_utility.F_DESENCRIPTAR_TEXTO_SIMPLE(A_NUM_TARJETA);
    ELSE
        vTarjetaEncriptada := A_NUM_TARJETA;
    END IF;
    
    IF farma_utility.F_IS_NUMBER(pCadena => vTarjetaEncriptada) = 1 THEN
    
      --validamos el mod10 si la tarjeta lo requiere
      IF INSTR(vTarjetaEncriptada, '00000000') = 0 THEN
        --PARA CUANDO NO TIENEN LA TARJETA Y LO RELLENAN DE CEROS
        FOR I IN REVERSE 1 .. LENGTH(vTarjetaEncriptada) LOOP
          V_RESULT := SUBSTR(vTarjetaEncriptada, I, 1) * X;
          IF V_RESULT >= 10 THEN
            V_TOTAL := V_TOTAL + SUBSTR(V_RESULT, 1, 1) +
                       SUBSTR(V_RESULT, 2, 1);
          ELSE
...
/* ************************************************************************** */
  ...
  BEGIN
    ...
    BEGIN
      IF FARMA_PUNTOS.F_VAR_TARJ_VALIDA(cCodGrupoCia_in => cCodGrupoCia_in,
                                      cCodLocal_in => cCodLocal_in,
                                      cNumTarj_in => cNroTarjeta_in)='S' THEN
        SELECT FID.DNI_CLI
          INTO cDniCliente
          FROM FID_TARJETA FID
         WHERE FID.COD_TARJETA = cNroTarjeta_in
         AND NVL(FID.IND_PUNTOS,'N') = 'S'
         AND FID.IND_ENVIADO_ORBIS IN ('E','P');
      ELSE
        SELECT FID.DNI_CLI
          INTO cDniCliente
          FROM FID_TARJETA FID
         WHERE FID.COD_TARJETA = cNroTarjeta_in
         AND NVL(FID.IND_PUNTOS,'N') = 'N';
      END IF;
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        cDniCliente := ' ';
...
END;
  ...
  BEGIN
    ...
    --KMONCADA 16.10.2015 El tipo de doc. ya esta determinado     
...
END;*/
  ...
  FUNCTION FID_F_GET_IND_VARIOS_COMP(cCodGrupoCia_in   IN CHAR,
    ...
    vIndVtaManual CHAR(1) := 'N';
    vCantElectronico INTEGER := 0;
  BEGIN
     ...
     SELECT NVL(CAB.IND_COMP_MANUAL,'N')
     INTO vIndVtaManual
     FROM VTA_PEDIDO_VTA_CAB CAB
     WHERE CAB.COD_GRUPO_CIA = cCodGrupoCia_in
     AND CAB.COD_LOCAL = cCodLocal_in
     ...
     IF vIndVtaManual = 'S' THEN
       RETURN 'S';
     END IF;
     
     -- KMONCADA 24.07.2015 VALIDA QUE NO HAY COMP.ELECTRONICO PARA IMPRIMIR TICKET DE PUNTOS SEPARADO
     SELECT COUNT(1)
     INTO vCantElectronico
     ...
     IF vCantElectronico = 0 THEN
       ...
       AND B.COD_LOCAL = cCodLocal_in
       AND B.NUM_PED_VTA = vNumPedVta_in
       AND B.TIP_COMP_PAGO = '05';
       
       IF cantCompPedido > 0 THEN
         RETURN 'N';
       ELSE
         RETURN 'S';
       END IF;
...
END;
 ...
 EXCEPTION
  ...
  BEGIN
    ...
    IF ABS(nNumero) > 0 then
      IF VALOR = '0' THEN
        nNumero := ROUND(nNumber_in,2);
      ELSE
        nNumero := TRUNC(nNumber_in);
      END IF;
    END IF;
    
    numFormated := trim(TO_CHAR(ROUND(nNumero,2),'999,990.00'));
...
/* ******************************************************** */
  ...
  BEGIN
    ...
    IF nNumber_in <= 0 THEN
      nNumber := 0; --numFormated := '0.00';
    ELSE
      /*if nNumber_in < 1 then
        --numFormated := '0' || trim(TO_CHAR(ROUND(nNumber_in,2),'999,999.99'));
        numFormated := '0' || trim(TO_CHAR(ROUND(nNumber_in,2),'999,990.00'));
      else
        numFormated := trim(TO_CHAR(ROUND(nNumber_in,2),'999,990.00'));
      end if;*/
      nNumber := ROUND(nNumber_in, 2);
    END IF;
    numFormated := TRIM(TO_CHAR(nNumber, '999,990.00'));
...
/* ******************************************************** */
  ...
  BEGIN
     ...
     if valIndTotal = 'A' then
       -- KMONCADA 30.06.2015 SE SUMA EL AHORRO DEL PEDIDO ACTUAL
       SELECT SUM(NVL(DET.AHORRO,0))
       INTO dscto
       FROM VTA_PEDIDO_VTA_DET DET
       WHERE DET.COD_GRUPO_CIA= cCodGrupoCia_in
...
--    delete TMP_EXPERTO;
  ...
  --TOTAL DEL COMPROBANTE
     ...
     dscto := round((dscto * (1 + (FARMA_GRAL.F_GET_IGV_LOCAL / 100))),2);    --ASOSA - 24/06/2015 - IGVAZNIA
...
--SI EL PEDIDO SOLO EMITE GUIA ES XQ ES CONVENIO AL 100% Y X LO TANTO NO DEBE TENER PUNTOS
     ...
     AND f.est_trx_orbis IN (EST_PENDIENTE, EST_ENVIADO);
      ...
      ptosInmed := round((ptosInmed * (1 + (FARMA_GRAL.F_GET_IGV_LOCAL / 100))),2); --ASOSA - 24/06/2015 - IGVAZNIA
  ...
  BEGIN
    ...
    -- KMONCADA 02.07.2015 SI NRO DE DOCUMENTO TIENE ARTIFICIO DE CAMPAÑAS LO MODIFICA
    /*
    IF LENGTH(vDocumento) >9 THEN
      vDocumento := SUBSTR(vDocumento,2, (LENGTH(vDocumento)-2));
    END IF; 
    */
  ...
  BEGIN
    ...
    IF ABS(NVL(vPtoTotal,0)) > 0 THEN
      FARMA_PRINTER.P_AGREGA_TEXTO_SEGUIDO(vIdDoc_in => vIdDoc_in,
                                              vIpPc_in => vIpPc_in,
                                              vValor_in => RPAD(vTextoTotal,30,' '),
                                              vTamanio_in => FARMA_PRINTER.TAMANIO_0);
      IF NVL(vPtoTotal,0) < 0 THEN
        FARMA_PRINTER.P_AGREGA_TEXTO_SEGUIDO(vIdDoc_in => vIdDoc_in,
                                             vIpPc_in => vIpPc_in,
                                             vValor_in => LPAD(FID_F_GET_NUM_FORMATED(vPtoTotal)||'*',13,' ')||' ',
                                             vTamanio_in => FARMA_PRINTER.TAMANIO_0,
                                             vNegrita_in => FARMA_PRINTER.BOLD_ACT);
      ELSE
        FARMA_PRINTER.P_AGREGA_TEXTO_SEGUIDO(vIdDoc_in => vIdDoc_in,
    ...
    IF valorTab491 = 'A' THEN
      SELECT  
        CASE
          WHEN valorAhorro_in < 0 THEN
            0
          ELSE
            (valorAhorro_in * TO_NUMBER(TAB.DESC_CORTA,'99990.00')) / TO_NUMBER(TAB.LLAVE_TAB_GRAL,'99990.00')
        END
      INTO vValorAhorro
      FROM PBL_TAB_GRAL TAB
      WHERE ID_TAB_GRAL = 482;
    ...
    IF valorImpr <> '0.00' THEN
      -- KMONCADA 30.06.2015 SE MOSTRARA TEXTO SE TIEMPO SEGUN TAB_GRAL
      FARMA_PRINTER.P_AGREGA_TEXTO_SEGUIDO(vIdDoc_in => vIdDoc_in,
                                              vIpPc_in => vIpPc_in,
                                              vValor_in => RPAD(UPPER(FARMA_PUNTOS.F_CHAR_GET_MSJ_TIEMPO_AHORRO|| ' HAS AHORRADO'),43,' '),
                                              vTamanio_in => FARMA_PRINTER.TAMANIO_0);
      FARMA_PRINTER.P_AGREGA_TEXTO(vIdDoc_in => vIdDoc_in,
                                        vIpPc_in => vIpPc_in,
  ...
  PROCEDURE F_ACTUALIZA_ESTADO_AFILIA_PTOS(cNroTarjeta_in   IN VARCHAR2,
                                           ...
                                           cEstadoTrsx_in   IN CHAR) 
  IS
    cCodLocal PBL_LOCAL.COD_LOCAL%TYPE;
    cCodGrupoCia VTA_PEDIDO_VTA_CAB.COD_GRUPO_CIA%TYPE;
    
  BEGIN
    SELECT A.COD_LOCAL, A.COD_GRUPO_CIA
    INTO cCodLocal, cCodGrupoCia
    FROM VTA_IMPR_LOCAL A
    GROUP BY A.COD_LOCAL,A.COD_GRUPO_CIA;
    
    
    UPDATE FID_TARJETA A
    SET A.IND_PUNTOS = FARMA_PUNTOS.F_VAR_TARJ_VALIDA(cCodGrupoCia, cCodLocal, cNroTarjeta_in),
        A.IND_ENVIADO_ORBIS = cEstadoTrsx_in,
        A.COD_LOCAL = cCodLocal
    WHERE A.COD_TARJETA = cNroTarjeta_in
    AND A.DNI_CLI = cNroDocumento_in;
    
    UPDATE PBL_CLIENTE A
    SET A.IND_ENVIADO_ORBIS = cEstadoTrsx_in
    WHERE A.DNI_CLI = cNroDocumento_in;
  END;
  ...
  BEGIN
    ...
    SET A.DNI_CLI = NULL,
        A.COD_LOCAL = NULL,
        A.USU_MOD_TARJETA = cLoginUsu_in,
        A.FEC_MOD_TARJETA = SYSDATE,
        A.IND_PUNTOS = 'S',
        A.IND_ENVIADO_ORBIS = 'B'
    WHERE A.COD_TARJETA = cNroTarjeta_in;
...
/* ********************************************************************************** */
  FUNCTION F_IS_DOC_LOCKED(cNumDocId_in       IN CHAR,
    ...
    if cFidTarjeta_in = 'N' then
    
    select decode(count(1),0,'N','S')
    into   vRpt_in
    from   FID_NUM_DOC_LOCKED f
    where  f.num_doc_id = cNumDocId_in;
   ...
   FUNCTION F_IS_PROD_LOCKED(cCodProd_in       IN CHAR) 
    ...
    select decode(count(1),0,'N','S')
    into   vRpt_in
    from   MAE_PROD_LOCKED f
    where  f.cod_prod = cCodProd_in;
    ...
    where  f.cod_prod = cCodProd_in
    and   f.UNID_DIA > 0
    and   f.ctd_dias_hist =1;-- si es por dia
    ...
    where  f.cod_prod = cCodProd_in
    and   f.UNID_DIA > 0
    and   f.ctd_dias_hist >=1;-- si es mas dia    
    ...
    where  f.cod_prod = cCodProd_in
    and   f.UNID_DIA > 0
    and   f.ctd_dias_hist =1;-- si es un dia
    ...
    RETURN number as
     ...
     nDias MAE_PROD_LOCKED.Unid_Dia%type;
    begin
      
      select f.ctd_dias_hist
      into   nDias
      from   MAE_PROD_LOCKED f
      where  f.cod_prod = cCodProd_in
      and   f.UNID_DIA > 0
      and   f.ctd_dias_hist >1;
      
        SELECT SUM(V.CANT_ATENDIDA / V.VAL_FRAC)
          INTO vValor
          FROM PBL_UNID_DCTO_X_NUM_DOC V,
               (select min(dia) desde, max(dia) hasta
                  from (select trunc(sysdate) + 1 - rownum DIA
                          from lgt_lab
                         where rownum <= nDias)) G
         WHERE V.COD_GRUPO_CIA = '001'
           AND V.COD_PROD = cCodProd_in
           AND V.DIA_VTA BETWEEN G.DESDE AND G.HASTA
           and v.num_doc_id = cDNi_in;
  ...
  FUNCTION FID_IND_REDONDEO_PREC_CAMP(cCodCampana_in in char)
    ...
    ind char := 'N';
  BEGIN
     SELECT NVL(T.IND_REDONDEO_PRECIO,'N')
        INTO ind
        FROM vta_campana_cupon T
        WHERE t.cod_grupo_cia = '001'
        and   T.Cod_Camp_Cupon = cCodCampana_in;
  ...
  FUNCTION FID_CUR_TIPO_DOCUMENTO 
    RETURN FARMACURSOR IS
    curTipoDocumento FARMACURSOR;
  BEGIN
    OPEN curTipoDocumento FOR
      SELECT TRIM(COD_TIP_DOCUMENTO) CODIGO, 
             TRIM(DESC_TIP_DOCUMENTO) DESCRIPCION, 
             SUBSTR(MASCARA,4) MASCARA,
             SUBSTR(MASCARA,2,1) SEPARADOR
  ...
  FUNCTION FID_GET_COD_TIPO_DOC_CLIENTE(cNroDocumento_in IN PBL_CLIENTE.DNI_CLI%TYPE)
    RETURN CHAR IS
    vCodTipDocumento PBL_CLIENTE.COD_TIP_DOCUMENTO%TYPE;
  BEGIN
    BEGIN 
    SELECT CL.COD_TIP_DOCUMENTO
    INTO vCodTipDocumento
    FROM PBL_CLIENTE CL
    WHERE CL.DNI_CLI = cNroDocumento_in;
    EXCEPTION 
      WHEN OTHERS THEN
        vCodTipDocumento := ' ';
    END;
    RETURN NVL(vCodTipDocumento,' ');
  END;
  
  FUNCTION F_DETERMINA_TIPO_DOC(cNroDocumento_in IN PBL_CLIENTE.DNI_CLI%TYPE)
    RETURN CHAR IS
    CURSOR curTipoDocumentos IS
      SELECT A.COD_TIP_DOCUMENTO 
      FROM PBL_TIP_DOCUMENTOS A 
      WHERE A.SELECCIONABLE='S' AND A.MASCARA IS NOT NULL;
    filaTipoDocumento curTipoDocumentos%ROWTYPE;
    vTipoDocumento PBL_CLIENTE.COD_TIP_DOCUMENTO%TYPE;
    vEncontro CHAR(1) := 'N';
    vSeparador CHAR(1);
    
  BEGIN
    OPEN curTipoDocumentos;
    LOOP
      FETCH curTipoDocumentos INTO filaTipoDocumento;
      EXIT WHEN (curTipoDocumentos%NOTFOUND OR vEncontro = 'S');
        vTipoDocumento := filaTipoDocumento.COD_TIP_DOCUMENTO;
        
        SELECT TRIM(SUBSTR(A.MASCARA,2,1))
        INTO vSeparador
        FROM PBL_TIP_DOCUMENTOS A
        WHERE A.COD_TIP_DOCUMENTO = vTipoDocumento;
     
        SELECT DECODE(COUNT(1),0,'N','S') 
        INTO vEncontro
        FROM (
          SELECT (EXTRACTVALUE(xt.column_value, 'e')) MASCARA
                  FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' ||
                                     REPLACE( ( 
                                     SELECT TRIM(SUBSTR(A.MASCARA,4))
                                      FROM PBL_TIP_DOCUMENTOS A
                                      WHERE A.COD_TIP_DOCUMENTO = vTipoDocumento
                                     ),vSeparador,'</e><e>') ||'</e></coll>'),'/coll/e'))) xt
        ) A
        WHERE REGEXP_LIKE(cNroDocumento_in, A.MASCARA);
        
    END LOOP;
    CLOSE curTipoDocumentos;
    IF vEncontro = 'N' THEN
      vTipoDocumento := '';
    END IF;
  ...
  **************************************************************************/
  ...
  vTarj_aplica_camp VARCHAR2(1);
  BEGIN
      IF LENGTH(pNumeroTarjeta) < 30 THEN
          vTarj_aplica_camp := FID_IS_TARJETA_APLICA_CAMPANA(pGrupoCia,
                                                             pCodLocal,
                                                             pNumeroTarjeta);
          
          IF vTarj_aplica_camp = 'S' THEN
              vTarjetaEncriptada := farma_utility.F_ENCRIPTAR_TEXTO_SIMPLE(vCadenaTexto => pNumeroTarjeta,
                                                                           vLongitud    => 30);
          ELSE
              vTarjetaEncriptada := pNumeroTarjeta;                                                                
          END IF;
      ELSE
          vTarjetaEncriptada := pNumeroTarjeta;
      END IF; 
  ...
  FUNCTION F_CUR_BUSCA_CLIENTE(cCodGrupoCia_in     IN CHAR,
    ...
    curCli FarmaCursor;
  BEGIN
    IF (cTipoBusqueda_in = PKG_CONSTANTES.TIP_BUSQUEDA_RUC) THEN --RUC
        OPEN curCli FOR
          SELECT ' '/*C.DNI_CLI*/ COD_CLIENTE,
                 C.DNI_CLI NUM_DOCUMENTO,
                 C.NOM_CLI RAZON_SOCIAL,
                 NVL(C.DIR_CLI,' ') DIRECCION,
                 NVL(C.IND_AGENTE_RETENCION,'N') AGENTE_RETENCION,
                 NVL(C.IND_AGENTE_PERCEPCION,'N') AGENTE_PERCEPCION,
                 -- KMONCADA 06.06.2016 [PROY_M] INDICADOR DE CLIENTE DIGEMID
                 NVL(C.FLG_DIGEMID,'N') CLIENTE_DIGEMID
          FROM PBL_CLIENTE C
          WHERE C.DNI_CLI = cValorBusqueda_in
          AND C.COD_TIP_DOCUMENTO = PKG_CONSTANTES.TIPO_DOC_RUC
          AND C.IND_ESTADO = 'A';

      ELSIF (cTipoBusqueda_in = PKG_CONSTANTES.TIP_BUSQUEDA_RAZON_SOCIAL) THEN --RAZON SOCIAL
        OPEN curCli FOR
          SELECT ' '/*C.DNI_CLI*/ COD_CLIENTE,
                 C.DNI_CLI NUM_DOCUMENTO,
                 C.NOM_CLI RAZON_SOCIAL,
                 ...
                 NVL(C.FLG_DIGEMID,'N') CLIENTE_DIGEMID
          FROM PBL_CLIENTE C
          WHERE C.COD_TIP_DOCUMENTO = PKG_CONSTANTES.TIPO_DOC_RUC
          AND C.IND_ESTADO = 'A'
          AND C.NOM_CLI LIKE '%' || cValorBusqueda_in || '%';
      END IF;
      RETURN curCli;
  END;
  ...
  PROCEDURE P_MANTENIMIENTO_CLIENTE(pCodLocal_in           IN CHAR,
                                    ...
                                    pIsAgenteRetencion_in  IN CHAR DEFAULT NULL,
                                    pIsAgentePercepcion_in IN CHAR DEFAULT NULL,
                                    pClienteDigemid_in     IN CHAR DEFAULT 'N', --KMONCADA 06.06.2016
                                    pUser_in               IN CHAR
                                    ) is
  BEGIN
    ptoventa_fidelizacion.fid_p_insert_cliente(vdni_cli => pNumDocumento_in,
                                               vnom_cli => pRazonSocial_in,
                                               vdir_cli => pDireccion_in,
                                               pcodlocal => pCodLocal_in,
                                               puser => pUser_in,
                                               pindestado => 'A',
                                               ctipdoc => pTipoDocumento_in,
                                               cuservalida => pUser_in,
                                               pindagenteretencion_in => pIsAgenteRetencion_in,
                                               pindagentepercepcion_in => pIsAgentePercepcion_in,
                                               pClienteDigemid_in => pClienteDigemid_in);
  END;
END PTOVENTA_FIDELIZACION;
/
