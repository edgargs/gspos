...
-- 2014-12-15 JOLIVA: SE AGREGO ATENCION AL CLIENTE
      ...
      (
        SELECT
              ...
              NVL(TOT_COM, 0),
              nAtencionCliente,
              NVL(TOT_COM, 0) * (1 + nAtencionCliente/100)
        FROM
           (
            SELECT
                   CAB.COD_GRUPO_CIA,
            ...
            WHERE  CAB.COD_GRUPO_CIA = cCodGrupoCia_in
              AND  CAB.COD_LOCAL = cCodLocal_in
    	  			AND  CAB.FEC_PED_VTA BETWEEN TO_DATE(cFecha_Ini_in || ' 00:00:00','dd/MM/yyyy HH24:mi:ss')
                                   AND     TO_DATE(cFecha_Fin_in || ' 23:59:59','dd/MM/yyyy HH24:mi:ss')
              AND  CAB.EST_PED_VTA = 'C'
              AND  cab.tip_ped_vta != '03'   --- 19-AGO-14  TCT No debe considerar Venta Mayorista x Solic. Julio
              AND  DET.COD_GRUPO_CIA = CAB.COD_GRUPO_CIA
              AND  DET.COD_LOCAL = CAB.COD_LOCAL
              AND  DET.NUM_PED_VTA = CAB.NUM_PED_VTA
...
/*------------------------------------------------------------------------------------------------
Hist : 19-AGO-14  TCT  Reemplaza con nueva funcion que carga datos en linea
--------------------------------------------------------------------------------------------------*/
  FUNCTION REPORTE_VENTAS_VENDEDOR_IMP_1(cCodGrupoCia_in	IN CHAR,
       		   					   	             cCodLocal_in     IN CHAR,
			                       					 cFechaInicio 	  IN CHAR,
 		  						                     cFechaFin 		    IN CHAR
                                       )
...
/***********************************************************************************************************************************************/
  ...
  SELECT CONV.DES_CONVENIO || 'Ã' ||
              TIP_CP.DESC_COMP || 'Ã' ||
              --SUBSTR(VTA_CP.NUM_COMP_PAGO,0,3)||'-'||SUBSTR(VTA_CP.NUM_COMP_PAGO,4)
              Farma_Utility.GET_T_COMPROBANTE_2
                               (VTA_CP.COD_TIP_PROC_PAGO,VTA_CP.NUM_COMP_PAGO_E,VTA_CP.NUM_COMP_PAGO)
              || 'Ã' ||
              TO_CHAR(VTA_CP.FEC_CREA_COMP_PAGO,'DD/MM/YYYY')

              FROM VTA_COMP_PAGO VTA_CP,
              VTA_TIP_COMP TIP_CP,
              VTA_PEDIDO_VTA_CAB VTA_CAB,
              MAE_CONVENIO CONV
              WHERE VTA_CP.IND_COMP_ANUL='N'
              and  VTA_CAB.Est_Ped_Vta = 'C'
              AND VTA_CP.COD_GRUPO_CIA = cCodGrupoCia_in
              AND VTA_CP.COD_LOCAL = cCodLocal_in
              AND VTA_CP.TIP_CLIEN_CONVENIO = '2'
              AND TIP_CP.COD_GRUPO_CIA=VTA_CP.COD_GRUPO_CIA
              AND TIP_CP.TIP_COMP=VTA_CP.TIP_COMP_PAGO
              AND VTA_CP.FEC_CREA_COMP_PAGO BETWEEN
            TO_DATE( cFecInicial_in  || ' 00:00:00', 'dd/MM/yyyy HH24:MI:SS') AND
             TO_DATE(cFecFinal_in || ' 23:59:59', 'dd/MM/yyyy HH24:MI:SS')
             AND VTA_CAB.COD_GRUPO_CIA = VTA_CP.COD_GRUPO_CIA
              AND VTA_CAB.COD_LOCAL = VTA_CP.COD_LOCAL
              AND VTA_CAB.NUM_PED_VTA = VTA_CP.NUM_PED_VTA
              AND VTA_CAB.COD_CONVENIO IN (
          ...
          FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' ||
              ...
              AND VTA_CAB.TIP_PED_VTA != '03'
              AND VTA_CAB.COD_CONVENIO = CONV.COD_CONVENIO
              ORDER BY VTA_CP.FEC_CREA_COMP_PAGO ASC;
