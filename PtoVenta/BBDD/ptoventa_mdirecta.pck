CREATE OR REPLACE PACKAGE PTOVENTA."PTOVENTA_MDIRECTA" IS
  ...
  --Fecha        Usuario		    Comentario
  --18.09.2014   ERIOS              Se cambia el tipo de dato, del parametro cfecha_in
  FUNCTION MDIR_GRABA_OC_CAB_RECEP(cCodGrupoCia_in  IN CHAR,
            ...
            cNumer_Ord_Comp_in  IN CHAR, cfecha_in  IN VARCHAR2, cId_Docum_in  IN CHAR,
 ...
 --==============================================================================================
  ...
  --==============================================================================================
  -- Obtiene numero documento de la OC y la actualiza en num doc de kardex.
  --Fecha        Usuario		    Comentario
  --11/11/2014   RHERRERA         Creación
   PROCEDURE ACTUALIZA_NUMNOTA_KARDEX(cCodGrupoCia IN CHAR, cCodCia_in IN CHAR,
             cCodLocal  IN CHAR,cCodOC IN CHAR,nSecOCRecep IN NUMBER);
           
              
  --==============================================================================================
  -- Valida el lote en la devolución de un producto al proveedor
  --Fecha        Usuario		    Comentario
  --06/04/2016   CCASTILLO      Creación
  
   FUNCTION  F_VALIDA_LOTE(cCodGrupoCia_in  IN CHAR,
                           cCodLocal_in     IN CHAR,
                           cCodProducto_in  IN CHAR,
...
/*------------------------------------------------------------------------------------------------------------------
GOAL :LISTA ORDENES DE COMPRA NO TERMINADAS.
Hist : 05-SEP-14  TCT Modifica para solo Mostrar Ordenes no Anuladas
---------------------------------------------------------------------------------------------------------------------*/
 FUNCTION LISTA_ORDEN_COMPRA_CAB(cCodGrupoCia_in IN CHAR,
                                  cCodCia_in IN CHAR,
                                  cCodLocal_in IN CHAR,
    ...
    BEGIN
        OPEN cursListado FOR
           ...
           ---
           AND cab.est_oc_cab != 'N'    --- Anulado
           AND cab.fec_anu_oc IS NULL   --- Fecha Anulacion
           ---
      ...
      BEGIN
        ...
        OPEN cursListado FOR
          SELECT  (' ' || 'Ã' || 
                   ...
                   CASE
                      WHEN DET.CANT_RECEP >= DET.CANT_PED    --ASOSA - 18/09/2014
                      /*
                      BETWEEN DET.CANT_PED AND 
                        (DET.CANT_PED + GET_CANT_ADIC_PROD_X_PROVEEDOR(CAB.COD_PROV, DET.COD_GRUPO_CIA, DET.COD_PROD)) 
                        */
                        THEN 'T' 
                        --INI ASOSA - 24/09/2014
                        WHEN DET.CANT_RECEP = 0 THEN 'N'                             --NOTHING
                        --FIN ASOSA - 24/09/2014
                      ELSE 'A' END  || 'Ã' ||
                      DET.CANT_RECEP  --ASOSA - 30/09/2014
                   ) AS RESULTADO
          FROM LGT_OC_DET DET 
          JOIN LGT_PROD PROD 
  ...
  FUNCTION MDIR_GRABA_OC_CAB_RECEP(cCodGrupoCia_in   IN CHAR,
            ...
            cfecha_in           IN VARCHAR2, cId_Docum_in        IN CHAR,
      ...
      IS
       ...
       -- v_nNumNota LGT_NOTA_ES_CAB.NUM_NOTA_ES%TYPE;
        ...
        --    IF nDocumento = 1 THEN
              ...
              VALUES(
                    cCodGrupoCia_in,cCod_Local_in, cNumer_Ord_Comp_in,cId_Docum_in,TRIM(cSerie_Docm_in), TRIM(cNumer_Docm_in), 'A',
                    cCod_Cia_in,
                    TRIM(TO_NUMBER(nCant_Item_in, '99999')),
                    TRIM(TO_NUMBER(nImport_Total_in, '999,999.999')),
                    UPPER(vId_User_in),
                    TO_DATE(cfecha_in, 'dd/MM/yyyy'),
                    NULL,
                    NULL,
              ...
              SET FEC_RECEPCION  = TRUNC(SYSDATE),
                  ...
                  NUM_DOC_CAB    = TRIM(cNumer_Docm_in),
                  SER_DOC_CAB    = TRIM(cSerie_Docm_in),
  ...
  PROCEDURE MDIR_GRABA_OC_DET_RECEP(cCodGrupoCia_in       IN CHAR,
    ...
    IS
     ...
     vCantExceso number(9,3) := TO_NUMBER(nCant_Recep_in, '999,999.999') -     --ASOSA - 15/08/2014
                                                          TO_NUMBER(nCant_Solict_in, '999,999.999');
    ...
    v_FechaVencimiento CHAR(10) := cFechaVencimiento_in;
  BEGIN
    ...
    END IF;
     ...
     --INI ASOSA - 24/09/2014
      ...
      AND FF.COD_CIA = cCod_Cia_in
      AND FF.COD_LOCAL = cCod_Local_in
      AND FF.COD_OC = cNumer_Ord_Comp_in
      AND FF.COD_PROD = cCod_Prod_in;
      
      vCantExceso := vCantExceso + vAmountReceivedUntilNow;
      ...
      --FIN ASOSA - 24/09/2014
        ...
        SET CANT_RECEP = CANT_RECEP+nCant_Recep_in,
            ...
            CANT_EXCESO = vCantExceso    --ASOSA - 15/08/2014
        WHERE COD_GRUPO_CIA = cCodGrupoCia_in
        AND COD_LOCAL = cCod_Local_in
        AND COD_OC = cNumer_Ord_Comp_in
...
PROCEDURE ANULAR_INGRESO_RECEPCION(cCodGrupoCia_in IN CHAR,
  ...
  AS
    ...
    BEGIN
      ...
      -------------rherrera 11.11.2014 ------------
   --inicio
   --- capturo documento de proveedor y lo actualizo en kardex
      ACTUALIZA_NUMNOTA_KARDEX(cCodGrupoCia_in, cCodCia_in,cCodLocal_in,cCodOC_in,nSecCab_in);
   --fin---
 ...
 END;
  ...
  RETURN FarmaCursor
   ...
   PROCEDURE ACTUALIZA_NUMNOTA(cCodGrupoCia IN CHAR, cCodCia_in IN CHAR,cCodLocal  IN CHAR,cNumNota IN CHAR,cCodOC IN CHAR,nSecOCRecep IN NUMBER)
  AS
  --Declaro variables rherrera 11.11.2014
      cNumCompPago    lgt_kardex.num_comp_pago%type;
      cNumNotaEs      lgt_oc_cab_recep.num_nota_es%type;      
  ...
  BEGIN
   ...
   --inicio
   --- capturo documento de proveedor y lo actualizo en kardex
      ACTUALIZA_NUMNOTA_KARDEX(cCodGrupoCia, cCodCia_in,cCodLocal,cCodOC,nSecOCRecep);
   --fin---
...
--       AND B.EST_PROD_LOC='A'
 ...
 FUNCTION GET_DET_OC_SOBRANTE(vCodGrupoCia IN CHAR,
      ...
      BEGIN
        ...
        OPEN cursListado FOR
          SELECT  (' ' || 'Ã' || 
                   ...
                   PROD.desc_prod || ' - ' || prod.desc_unid_present || 'Ã' || 
                   det.cant_exceso || 'Ã' ||
                   ' ' || 'Ã' ||   --ASOSA - 19/08/2014 - Cantidad a devolver
                   TO_CHAR(det.prec_prod, '999,990.00') || 'Ã' ||
                   det.DET_IGV_PROD || 'Ã' ||
                   prod.cant_unid_present || 'Ã' ||
                   prl.val_frac_local || 'Ã' ||
                   --INI ASOSA - 20/08/2014
                   (SELECT L.NOM_LAB
                     FROM LGT_LAB L
                     WHERE L.COD_LAB = PROD.Cod_Lab) || 'Ã' ||  --NOM LABORATORIO
                   PRL.STK_FISICO || 'Ã' ||           --STOCK FISICO
                   ' ' || 'Ã' ||      --FECHA DE VENCIMIENTO
                   ' ' || 'Ã' ||      --NUMERO LOTE
                   --FIN ASOSA - 20/08/2014
                   -- kmoncada 15.07.2014 cantidad adicional por OC
                   GET_CANT_ADIC_PROD_X_PROVEEDOR(CAB.COD_PROV, DET.COD_GRUPO_CIA, DET.COD_PROD)|| 'Ã' ||
                   -- kmoncada 15.07.2014 valor de innerpack del producto
                   NVL(PROD.VAL_INNER_PACK,1) || 'Ã' ||
    ...
    INNER JOIN LGT_OC_CAB CAB 
          ...
          DET.COD_LOCAL=vCodLocal AND
          DET.COD_OC = vCodOrdenComp_in
          AND DET.CANT_EXCESO > 0
          AND DET.FECHA_DEV_EXCESO IS NULL;
...
begin
     ...
     and a.cod_cia = vCodCia
     and a.cod_local = vCodLocal
     and a.cod_oc = vCodOrdenComp_in;
 return codProv;
end;
 ...
 FUNCTION GET_ORD_COMP_PEND_REG(vCodGrupoCia IN CHAR,
      ...
      BEGIN
        OPEN cursListado FOR           
           ...
           AND CAB.COD_LOCAL = DET.COD_LOCAL
           AND CAB.COD_OC = DET.COD_OC
           AND CAB.cod_prov = PROV.cod_prov
           AND CAB.COD_GRUPO_CIA = vCodGrupoCia
           AND CAB.COD_CIA   = vCodCia
           AND CAB.COD_LOCAL = vCodLocal
           AND CAB.FECHA_DEV_EXCESO IS NULL
           AND DET.CANT_EXCESO > 0
           GROUP BY CAB.COD_OC ,
                    cab.COD_LOCAL ,
                    prov.ruc_prov ,
...
begin
     ...
     and a.cod_local = vCodLocal
     and a.cod_oc = vCodOrdenComp_in
     AND A.FECHA_DEV_EXCESO IS NOT NULL;
 ...
 --==============================================================================================
   ...
   function  GET_CANT_RECEP(vCodGrupoCia IN CHAR,
                                                     vCodCia IN CHAR,
                                                     vCodLocal IN CHAR ,
                                                     vCodOrdenComp_in IN VARCHAR2) 
  return varchar2
...
begin
 ...
 --==============================================================================================
  ...
  BEGIN
   ...
   --inicio
   --- capturo documento de proveedor y lo actualizo en kardex
   SELECT   trim(O.SER_DOC_CAB)||trim(O.NUM_DOC_CAB),NUM_NOTA_ES
            INTO         cNumCompPago,cNumNotaEs
   FROM    LGT_OC_CAB_RECEP O
   WHERE   O.COD_GRUPO_CIA=cCodGrupoCia
   AND O.COD_CIA = cCodCia_in
   AND O.COD_LOCAL=cCodLocal
   AND O.COD_OC=cCodOC
   AND O.Sec_Cab_Ord_Comp=nSecOCRecep;
   
   
   UPDATE LGT_KARDEX K 
   SET    K.NUM_COMP_PAGO = cNumCompPago
     WHERE COD_GRUPO_CIA = cCodGrupoCia
          AND   COD_LOCAL = cCodLocal
          AND   NUM_TIP_DOC = cNumNotaEs;
 ...
 --==============================================================================================
  ...
  FUNCTION  F_VALIDA_LOTE(cCodGrupoCia_in IN CHAR,cCodLocal_in IN CHAR,cCodProducto_in IN CHAR,vlote_in IN VARCHAR2)
    return varchar2
    is
      CURSOR c_cursor(p_cod_cia CHAR,p_cod_local CHAR,p_cod_pro CHAR,p_lote VARCHAR2) IS
      SELECT TO_CHAR(FECHA_VENCIMIENTO_LOTE,'DD/MM/YYYY') fvcto_lote
       FROM LGT_PROD_LOCAL_LOTE WHERE cod_grupo_cia=p_cod_cia AND cod_local=p_cod_local AND cod_prod=p_cod_pro AND lote=p_lote;
      
      l_reg c_cursor%ROWTYPE;
      
      p_salida VARCHAR2(10);
    
    BEGIN
      OPEN c_cursor(cCodGrupoCia_in,cCodLocal_in,cCodProducto_in,vlote_in);
      FETCH c_cursor INTO l_reg;
      IF c_cursor%NOTFOUND THEN
         p_salida:=' ';
      ELSE
         p_salida:=l_reg.fvcto_lote;
      END IF;
