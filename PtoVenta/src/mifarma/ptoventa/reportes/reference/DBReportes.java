...
public class DBReportes {
    ...
    public static void cargaListaProdPrecioVolumen(FarmaTableModel pTableModel) throws SQLException {
        pTableModel.clearTable();
        ArrayList parametros = new ArrayList();
        parametros.add(FarmaVariables.vCodGrupoCia);
        parametros.add(FarmaVariables.vCodLocal);
        
        log.debug("invocando a PTOVENTA_REPORTE.LTA_OBTIENE_INFO_PROD_PREC_VOL(?,?)");
        FarmaDBUtility.executeSQLStoredProcedure(pTableModel, "PTOVENTA_REPORTE.LTA_OBTIENE_INFO_PROD_PREC_VOL(?,?)",
                                                          parametros,false);
    }
