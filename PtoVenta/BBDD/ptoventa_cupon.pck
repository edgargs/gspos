...
/***************************************************************************/  
  --DESCRIPCON: OBTIENE CODIGO DE LOCAL DE LA CADENA DE CUPON INGRESADO
  --FECHA          Usuario		   Comentario
  --22/02/2016     KMONCADA      Creacion  
  FUNCTION F_GET_LOCAL_CUPON(cCadenaCupon_in IN VARCHAR2)
    RETURN CHAR;

/***************************************************************************/  
  --DESCRIPCON: OBTIENE NRO SECUENCIAL DE LA CADENA DEL CUPON INGRESADO
  --FECHA          Usuario		   Comentario
  --22/02/2016     KMONCADA      Creacion    
  FUNCTION F_GET_SECUENCIAL_CUPON(cCadenaCupon_in IN VARCHAR2)
    RETURN VARCHAR2;
    
/***************************************************************************/  
  --DESCRIPCON: VALIDA FORMATOS DE CUPONES (ANTIGUO EAN13 Y NUEVO)
  --FECHA          Usuario		   Comentario
  --22/02/2016     KMONCADA      Creacion
  FUNCTION F_GET_VALIDA_FORMATO_CUPON(cCadenaCupon_in IN VARCHAR2)
    RETURN CHAR;
    
/***************************************************************************/  
  --DESCRIPCON: OBTIENE CODIGO DE CAMPAÑA DEL CUPON
  --FECHA          Usuario		   Comentario
  --22/02/2016     KMONCADA      Creacion
  FUNCTION F_GET_COD_CAMPANA_CUPON(cCadenaCupon_in IN VARCHAR2)
    RETURN VARCHAR2;
    
/***************************************************************************/  
  --DESCRIPCON: OBTIENE PRECIO DEL PRODUCTO EN "CAMPAÑA CUPON PRECIO FIJO"
  --FECHA          Usuario		   Comentario
  --15/09/2016     ASOSA         Creacion    
  FUNCTION GET_F_VAL_PREC_PROD_CUPON(cCodGrupoCia_in IN CHAR,
                               cCodCampCupon_in    IN CHAR,
                               cCodProd_in     IN CHAR)
  RETURN NUMBER;
...
CREATE OR REPLACE PACKAGE BODY PTOVENTA."PTOVENTA_CUPON" is
  ...
  AS
    ...
    -- DUBILLUZ 24.12.2014
    SELECT COD_CUPON,FEC_INI,FEC_FIN
    -- DUBILLUZ 24.12.2014    
    FROM VTA_CUPON
  ...
  AS
    ...
    -- SELECT /*TRIM(COD_CUPON) AS COD_CUPON,FEC_INI,FEC_FIN
    -- DUBILLUZ 24.12.2014
    SELECT COD_CUPON AS COD_CUPON,FEC_INI,FEC_FIN
    -- DUBILLUZ 24.12.2014    
    FROM VTA_CUPON
    WHERE COD_GRUPO_CIA = cCodGrupoCia_in
          AND ESTADO = 'N'
...
17.07.2015 KMONCADA SE REALIZA EL CAMBIO PARA CASO DE PEDIDOS PENDIENTE DE COBRO
--------------------------------------------------------------------------------------------------------------------------*/
FUNCTION CUP_F_CUR_VALIDA_CUPON(cCodGrupoCia_in IN CHAR,
                                cCodLocal_in    IN CHAR,
                                cCadenaCupon_in IN CHAR,
  ...
  IS
    ...
    vDniCliente   VARCHAR2(20);

    -- fin jcallo --
    -- KMONCADA 10.05.2016 CONVENIOS CON CAMPAÑAS
  ...
  BEGIN
    ...
    IF F_GET_VALIDA_FORMATO_CUPON(cCadenaCupon_in)= 'N' THEN
    ...
    ---< /21-jul14  TCT NEW FORM CODIGO CAMP >
      ...
      -- DUBILLUZ 15.03.2016
      v_cCodLocalEmi := PTOVENTA_CUPON.F_GET_LOCAL_CUPON(cCadenaCupon_in);--SUBSTR(cCadenaCupon_in,7,3);
      v_cSecuencial := PTOVENTA_CUPON.F_GET_SECUENCIAL_CUPON(cCadenaCupon_in);--SUBSTR(cCadenaCupon_in,10);
      -- DUBILLUZ 15.03.2016
    ...
    IF cNumPedVta_in IS NOT NULL THEN
      SELECT TRIM(COD_CONVENIO)
      INTO vCodConvenio
      ...
      IF vCodConvenio IS NOT NULL THEN
        SELECT COUNT(1)
        INTO vAplicaConv 
        FROM REL_VTA_CAMPANA_CONVENIO_USO A
        WHERE A.COD_GRUPO_CIA = cCodGrupoCia_in
        AND A.COD_CAMP_CUPON = v_cCodigoCamp
        AND A.COD_CONVENIO = vCodConvenio;
        
        IF vAplicaConv = 0 THEN
          RAISE_APPLICATION_ERROR(-20012,'Cupón no aplica para este convenio.');
        END IF;
      ELSE
        SELECT COUNT(1)
        INTO vAplicaConv 
        ...
        AND A.COD_CAMP_CUPON = v_cCodigoCamp;
        IF vAplicaConv > 0 THEN
          RAISE_APPLICATION_ERROR(-20013,'Cupón aplica para ventas convenio.');
        END IF;
      END IF;
    ...
    --JCALLO 09.10.2008 --
    -- KMONCADA 17.07.2015  PARA CASO DE PEDIDOS PENDIENTES DE COBRO OBTIEN EL NRO DEL DNI DE LA CABECERA
    vDniCliente := cDniCliente;
     IF vDniCliente IS NULL THEN
       BEGIN
         SELECT FTP.DNI_CLI
         INTO vDniCliente
    ...
    IF vDniCliente IS NOT NULL AND LENGTH(TRIM(vDniCliente))>0 THEN
       --SI TIENE DNI QUIERE DECIR QUE ES UN CLIENTE FIDELIZADO
       --SE DEBE VALIDAR DE QUE EL CUPON SEA VALIDO YA SEA CUPO DE FID O NO.
       cValido := CUP_F_CHAR_VALIDA_CUPON_FID(cCodGrupoCia_in, cCodLocal_in, v_cCodigoCamp ,vDniCliente);
...
-------------------------------------------------------------------------------------------------------------------------*/
 FUNCTION CUP_F_CHAR_VALIDA_CUPON(cCodGrupoCia_in IN CHAR,
  ...
  BEGIN
    ...
    IF LENGTH(TRIM(cCadenaCupon_in)) = 13 AND CUP_F_CHAR_VALIDA_EAN13(cCadenaCupon_in)= 'N' THEN
       c_resultado:='N';
       RAISE_APPLICATION_ERROR(-20018,'Cupón no cumple el correcto formato correcto.');
    END IF;
    ...
    IF(cIndMultiUso_in='N')THEN
     ...
     --- < /21 - JUL - 14  TCT New Mode codigo camp >
      ...
      v_cCodLocalEmi := PTOVENTA_CUPON.F_GET_LOCAL_CUPON(cCadenaCupon_in);-- SUBSTR(cCadenaCupon_in,6,3);
      v_cSecuencial := PTOVENTA_CUPON.F_GET_SECUENCIAL_CUPON(cCadenaCupon_in); -- SUBSTR(cCadenaCupon_in,9,5);
    ...
    IF cCodConvenio_in IS NOT NULL THEN
      SELECT COUNT(1)
      INTO vAplicaConv 
      FROM REL_VTA_CAMPANA_CONVENIO_USO A
      WHERE A.COD_GRUPO_CIA = cCodGrupoCia_in
 ...
 FUNCTION CUP_F_CUR_CUP_REGALOS(cCodGrupoCia_in IN CHAR,
   ...
   SELECT substr(sys_context('USERENV','IP_ADDRESS'),1,50) INTO v_ip
      FROM DUAL;
        ...
        OPEN cur FOR
           ...
           WHERE X.COD_GRUPO_CIA=cCodGrupoCia_in
           AND X.COD_LOCAL=cCodLocal_in
           AND X.FEC_CREA_CUP_CAB>=TRUNC(SYSDATE)
  ...
  vcontador   NUMBER;
    ...
    BEGIN
      ...
      FROM VTA_CUPON A
      WHERE A.COD_GRUPO_CIA=cCodGrupoCia_in
      AND A.FEC_CREA_CUP_CAB>=TRUNC(SYSDATE)
      AND A.COD_LOCAL=cCodLocal_in
...
/***************************************************************************/  
  ...
  FUNCTION F_GET_LOCAL_CUPON(cCadenaCupon_in IN VARCHAR2)
    ...
    vCodLocalEmi CHAR(3);
  BEGIN
    IF LENGTH(TRIM(cCadenaCupon_in)) > 13 THEN
      vCodLocalEmi := SUBSTR(cCadenaCupon_in,7,3);
    ELSIF LENGTH(TRIM(cCadenaCupon_in)) = 13 THEN
      vCodLocalEmi := SUBSTR(cCadenaCupon_in,6,3);
    END IF;
    RETURN vCodLocalEmi;
  END;
...
/***************************************************************************/  
  ...
  FUNCTION F_GET_SECUENCIAL_CUPON(cCadenaCupon_in IN VARCHAR2)
    RETURN VARCHAR2 IS
    vSecuencialEmi VARCHAR2(10);
  BEGIN
    IF LENGTH(TRIM(cCadenaCupon_in)) > 13 THEN
      vSecuencialEmi := SUBSTR(cCadenaCupon_in,10,8);
    ELSIF LENGTH(TRIM(cCadenaCupon_in)) = 13 THEN
      vSecuencialEmi := SUBSTR(cCadenaCupon_in,9,5);
    END IF;
    RETURN vSecuencialEmi;
  END;
...
/***************************************************************************/  
  ...
  BEGIN
    /**
    NUEVO FORMATO: C77587A2800000006
    (1) C: IDENTIFICADOR DE CUPON
    (5) 77587: CODIGO DE CAMPAÑA
    (3) A28: CODIGO DE LOCAL
    (8) 0000006: SECUENCIAL
    */
    IF LENGTH(TRIM(cCadenaCupon_in)) = 13 AND CUP_F_CHAR_VALIDA_EAN13(cCadenaCupon_in)= 'S' THEN
      vResultado := 'S';
    ELSIF LENGTH(TRIM(cCadenaCupon_in)) > 13 THEN
      vResultado := 'S';
    END IF;
  ...
  FUNCTION F_GET_COD_CAMPANA_CUPON(cCadenaCupon_in IN VARCHAR2)
    RETURN VARCHAR2 IS
    vCodCamp VTA_CAMPANA_CUPON.COD_CAMP_CUPON%TYPE;
  BEGIN
    IF LENGTH(TRIM(cCadenaCupon_in)) = 13 THEN
      vCodCamp:= TRIM(substr(cCadenaCupon_in,1,5));
    ELSIF LENGTH(TRIM(cCadenaCupon_in)) > 13 THEN
      vCodCamp:= TRIM(substr(cCadenaCupon_in,2,5));
    ELSE
      vCodCamp:= TRIM(substr(cCadenaCupon_in,1,5));
    END IF;
   RETURN vCodCamp;
 ...
 /***************************************************************************/
  ...
  RETURN NUMBER
  IS
  v_vPrecio number(10,2) := 0.0;
  v_nCant number(3);
  BEGIN
    
      SELECT count(1)
      INTO v_nCant
      FROM VTA_CAMPANA_PROD_USO A,
           VTA_CAMPANA_CUPON B
      WHERE A.COD_GRUPO_CIA = B.COD_GRUPO_CIA
      AND A.COD_CAMP_CUPON = B.COD_CAMP_CUPON
      AND A.ESTADO = 'A'
      AND A.COD_GRUPO_CIA = cCodGrupoCia_in
      AND A.COD_CAMP_CUPON = cCodCampCupon_in
      AND A.COD_PROD = cCodProd_in
      AND B.TIP_CAMPANA = 'C'
      AND A.IMP_VALOR IS NOT NULL
      AND SYSDATE BETWEEN B.FECH_INICIO AND (B.FECH_FIN + 1 - 1 / 24 / 60 / 60);
