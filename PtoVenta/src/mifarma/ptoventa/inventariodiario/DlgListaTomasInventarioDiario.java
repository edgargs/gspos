...
public class DlgListaTomasInventarioDiario extends JDialog {
    ...
    private void initTableListaTomasInventario() {
        tableModelInventarioDiario =
                new FarmaTableModel(ConstantsInvDiario.columnsTomasInventario, ConstantsInvDiario.defaultValuesTomasInventario,
                                    0);
        FarmaUtility.initSimpleList(tblRelacionTomas, tableModelInventarioDiario,
                                    ConstantsInvDiario.columnsTomasInventario);
    ...
    private void chkKeyPressed(KeyEvent e) {
            ...
            if (cargaLogin()) {
                ...
                if (tieneRegistroSeleccionado(tblRelacionTomas)) {
                    ...
                    if (VariablesInvDiario.vIndSegConteo.equalsIgnoreCase("N")) {
                        DlgProductosPorTomaDiario dlgProductosPorTomaDiario =
                            new DlgProductosPorTomaDiario(myParentFrame, "", true);
                    ...
                    if (tieneRegistroSeleccionado(this.tblRelacionTomas)) {
                        ...
                        if (esTomaValida())
                            if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                                ...
                                try {
                                    if (obtieneIndicadorProceso_ForUpdate(VariablesInvDiario.vSecToma,
                                                                          VariablesInvDiario.vIndProceso)) {
                                        //cargarToma();
                                    ...
                                    if (sql.getErrorCode() == 20009)
                                        FarmaUtility.showMessage(this,
                                                                 "Existen productos que tienen stock comprometido, no se puede cargar la toma :\n" +
        ...
        //if (cargaLogin()){
        //if (tieneRegistroSeleccionado(this.tblRelacionTomas)) {
        //if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,"¿Esta seguro de anular la toma?")) {
        //cargarRegistroSeleccionado();
        // try {
    ...
    private void cargarRegistroSeleccionado() {
        ...
        VariablesInvDiario.vIndSegConteo =
                tblRelacionTomas.getValueAt(tblRelacionTomas.getSelectedRow(), 5).toString().trim();
    ...
    private void obtieneProductosPorLaboratorio() {
        try {
            ...
            VariablesInvDiario.vListaProductos =
                    DBInvDiario.obtieneProductosPorLaboratotio(VariablesInvDiario.vCodLab);
        ...
        for (int i = 0; i < VariablesInvDiario.vListaCodLab.size(); i++) {
            ...
            FarmaPrintService vPrint =
                new FarmaPrintService(66, "D:\\TomaInventario\\" + FarmaVariables.vCodLocal + "\\" +
                                      FarmaVariables.vCodLocal + "_" + VariablesInvDiario.vCodLab + ".txt", false);
            if (!vPrint.startPrintService()) {
                FarmaUtility.showMessage(this,
                                         "No se pudo inicializar el proceso de impresión\n para el laboratorio " +
                                         VariablesInvDiario.vCodLab + " - " + VariablesInvDiario.vNomLab,
                                         tblRelacionTomas);
                return;
            }
            try {
                ...
                vPrint.printBold("Laboratorio: " + VariablesInvDiario.vCodLab + " - " +
                                 VariablesInvDiario.vNomLab.trim(), true);
                ...
                for (int x = 0; x < VariablesInvDiario.vListaProductos.size(); x++) {
                    vPrint.printCondensed(FarmaPRNUtility.alinearIzquierda(((String)((ArrayList)VariablesInvDiario.vListaProductos.get(x)).get(0)),
                                                                           8) +
                                          FarmaPRNUtility.alinearIzquierda(((String)((ArrayList)VariablesInvDiario.vListaProductos.get(x)).get(1)),
                                                                           44) +
                                          FarmaPRNUtility.alinearIzquierda(((String)((ArrayList)VariablesInvDiario.vListaProductos.get(x)).get(2)),
                                                                           19) +
                                          FarmaPRNUtility.alinearIzquierda(((String)((ArrayList)VariablesInvDiario.vListaProductos.get(x)).get(3)),
                                                                           18) + " " +
                ...
                vPrint.printBold("Registros Impresos: " +
                                 FarmaUtility.formatNumber(VariablesInvDiario.vListaProductos.size(), ",##0"), true);
                vPrint.deactivateCondensed();
    ...
    public void envioTomaLocal() {
        try {
            if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                ...
                FarmaUtility.aceptarTransaccion();
                FarmaUtility.showMessage(this, "Se envio correctamente la Toma de Inventrio Diario al local",
                                         tblRelacionTomas);
            }
