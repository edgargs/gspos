--------------------------------------------------------
--  DDL for Package Body PTOVENTA_CA_CLIENTE
--------------------------------------------------------
  ...
  **/
  ...
  IS
	...
	cCodGrupoCia PTOVENTA.PBL_LOCAL.COD_GRUPO_CIA%TYPE;
	cCodCia      PTOVENTA.PBL_LOCAL.COD_CIA%TYPE;
	cCodLocal    PTOVENTA.PBL_LOCAL.COD_LOCAL%TYPE;
  BEGIN
	--ERIOS 29.05.2014 Fideliza al cliente. Mejorar con parametro del local.
	SELECT COD_GRUPO_CIA,COD_CIA,COD_LOCAL
            INTO cCodGrupoCia,cCodCia,cCodLocal
      FROM PTOVENTA.PBL_LOCAL
      WHERE COD_LOCAL = (SELECT COD_LOCAL FROM PTOVENTA.PBL_NUMERA WHERE COD_NUMERA = '001');
	  
	ptoventa_fid_reniec.p_genera_tarjeta_dni(cCodGrupoCia,
                                             cCodLocal,
                                             trim(cDNI_in));
