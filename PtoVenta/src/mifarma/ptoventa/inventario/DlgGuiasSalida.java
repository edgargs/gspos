...
public class DlgGuiasSalida extends JDialog implements KeyListener {
    ...
    private void getDatosVista() {
        ...
        VariablesInventario.vPlacaTransportista_Transf = VariablesInventario.vPlacTransp;
    }
    ...
    private void saveDatos() {
        ...
        try {
            ...
            if (!vFlag) {
                ...
                FarmaUtility.showMessage(this, "Error Fallo la Transaccíon " + VariablesInventario.vNumGuia.trim(),
                                         null);
            }
        } catch (Exception ex) {
            log.error("", ex);
    ...
    private void cargarTransportista() {
        ...
        try {
            ...
            if (array.size() > 0) {
                ...
                limpiaDatosTrans();
                FarmaUtility.showMessage(this, "No se encontro datos para el registro seleccionado.Verificar!!!",
                                         null);
    ...
    private void cargaListaMaestros(String pTipoMaestro, JTextField pJTextField_Cod, JTextField pJTextField_Desc) {
        ...
        if (FarmaVariables.vAceptar) { //No se debe setear el valor del codigo de transp ya que este se queda pegado y no es necesario .
            // pJTextField_Cod.setText(VariablesPtoVenta.vCodMaestro);
            try {
                pJTextField_Cod.setText(DBPtoVenta.getRucTransportista(VariablesPtoVenta.vCodMaestro));
            } catch (Exception ex) {
                log.error("Error al consultar el ruc del transportista en guia de salida--> " + ex.toString());
