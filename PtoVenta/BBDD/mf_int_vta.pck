CREATE OR REPLACE PACKAGE PTOVENTA.MF_INT_VTA AS
  ...
  */
  ...
  TIP_COMP_SAP_SERVICIO_NC  varchar2(5) := '15';
  ...
  from (select a.documento,
               ...
               sum((case
                     ...
                     when instr(IMP_DSCTO, '-') <> 0 then
                      ...
                      to_number(to_char(IMP_DSCTO, '99,999,990.00'),'99,999,990.00')
                   end)
...
IF cCodCia=MARCA_MIFARMA THEN
      ...
      select cp.COD_GRUPO_CIA,cp.COD_LOCAL,cp.NUM_PED_VTA,cp.SEC_COMP_PAGO,
             ...
             DECODE(T.TIP_pED_VTA,'03',
                    decode(cp.tip_comp_pago,'04',
                    nvl((SELECT cod_cliente_sap from MAE_CLIENTE_SAP_MAY where cod_cliente=t.cod_cli_conv_origen),'SIN_CCLIE'),
                    nvl((SELECT cod_cliente_sap from MAE_CLIENTE_SAP_MAY where cod_cliente=t.cod_cli_conv),'SIN_CCLIE')),
             (
      ...
      from   vta_comp_pago cp,
             (
               select c.cod_grupo_cia, c.cod_local, c.num_ped_vta,
                     ...
                     c.cod_cli_conv,
                     C.TIP_PED_VTA,
                     (select cod_cli_conv from vta_pedido_vta_cab where cod_grupo_cia=c.cod_grupo_cia
                               and cod_local=c.cod_local and num_ped_vta=c.num_ped_vta_origen) cod_cli_conv_origen
                from vta_pedido_vta_cab c
               where c.cod_grupo_cia = '001'
...
update vta_comp_pago p
 ...
 AND   (P.COD_GRUPO_CIA,P.COD_LOCAL,P.NUM_PED_VTA) IN
                                                   ...
                                                   and CC.fec_ped_vta between to_date(cFechProceso_in,'dd/mm/yyyy') and to_date(cFechProceso_in,'dd/mm/yyyy') + 1 -1/24/60/60
                                                   );
