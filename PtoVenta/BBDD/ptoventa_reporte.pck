...
/*******************************************************************************************************************************************/
  ...
  -- Descripcion: Indicador de ver rentables
  -- Fecha       Usuario		Comentario
  --16/09/2016   ERIOS          Creacion
  FUNCTION GET_VER_RENTABLES(cCodGrupoCia_in IN CHAR, cCodCia_in IN CHAR)
  RETURN CHAR;
...
-- 2009-10-01 JOLIVA: Se va a mostrar el total de venta (cabeceras) en el total de ventas
  ...
  BEGIN
   ...
   END;
	...
	--ERIOS 15.07.2016 Se valida la fecha minima en caso de locales Arcangel
	SELECT COD_CIA INTO cCodCia_in FROM PTOVENTA.PBL_LOCAL WHERE COD_GRUPO_CIA = cCodGrupoCia_in AND COD_LOCAL = cCodLocal_in;
	BEGIN
		SELECT FECH_INI_VIG INTO v_dFechIniVig
	...
	EXCEPTION 
		WHEN NO_DATA_FOUND THEN	
			v_dFechIniVig := NULL;
	END;
	IF v_dFechIniVig IS NOT NULL AND to_date(cFechaInicio,'dd/MM/yyyy') < v_dFechIniVig THEN
	  delete from TMP_VTA_VEND_LOCAL;
	  goto reporte;
	END IF;
...
-- 2009-10-01 JOLIVA: Se va a mostrar el total de venta (cabeceras) en el total de ventas
  ...
  BEGIN
   ...
   WHERE  R.COD_GRUPO_CIA = cCodGrupoCia_in
    ...
    -- KMONCADA 23.06.2016 [PUNTOS RENTABLES] SE AGREGA CALCULO DE MONTOS CON Y SIN IGV
...
SELECT
              ...
              NVL(MON_TOT_VTA_S_IGV,0) MON_TOT_VTA_S_IGV,
              NVL(MON_TOT_VTA,0) MON_TOT_VTA,
              NVL(MON_TOT_BONO_S_IGV,0)MON_TOT_BONO_S_IGV,
        ...
        FROM
           (
            SELECT
                   ...
                   -- KMONCADA 17.06.2016 MONTO SIN IGV
                   SUM(ROUND((DET.VAL_PREC_TOTAL/(1+(DET.VAL_IGV/100))),3)) MON_TOT_VTA_S_IGV,
                   ...
                   SUM(
                      CASE 
                        WHEN NVL(DET.PTS_PROG,0) > 0 AND ABS(DET.VAL_PREC_TOTAL) > 0.01 THEN 
				                  ROUND((DET.VAL_PREC_TOTAL/(1+(DET.VAL_IGV/100))),3)
                        ELSE 0 
                      END) MON_TOT_BONO_S_IGV,
                   SUM(
                      CASE 
                        WHEN NVL(DET.PTS_PROG,0) > 0 AND ABS(DET.VAL_PREC_TOTAL) > 0.01 THEN 
                          DET.VAL_PREC_TOTAL 
	...
	WITH
	PTOS AS (
		SELECT V.COD_GRUPO_CIA,V.COD_LOCAL,V.SEC_USU_LOCAL,
		SUM(V.MON_TOT_GRUPO_A) MON_TOT_GRUPO_A,
		SUM(V.MON_TOT_PP) MON_TOT_PP,
		SUM(V.CANT_PED) CANT_PED,
		CASE WHEN SUM(V.CANT_PED) = 0 THEN 0 ELSE SUM(V.MON_TOT_GRUPO_A)/SUM(V.CANT_PED) END VAL1,
		CASE WHEN SUM(V.CANT_PED) = 0 THEN 0 ELSE SUM(V.MON_TOT_PP)/SUM(V.CANT_PED) END VAL2,
		SUM(V.CANT_PED_ANUL) CANT_PED_ANUL,
		SUM(NVL(V.MON_TOT_VTA_S_IGV,0)) MON_TOT_VTA_S_IGV,
		SUM(NVL(V.MON_TOT_VTA,0)) MON_TOT_VTA,
		SUM(NVL(V.MON_TOT_BONO_S_IGV,0)) MON_TOT_BONO_S_IGV,
		SUM(NVL(V.MON_TOT_BONO,0)) MON_TOT_BONO,
		SUM(V.TOT_COM) TOT_COM
		FROM TMP_VTA_VEND_LOCAL V
		WHERE V.COD_GRUPO_CIA = cCodGrupoCia_in
			AND    V.COD_LOCAL     = cCodLocal_in
			AND    V.FEC_DIA_VENTA BETWEEN TO_DATE(cFechaInicio || ' 00:00:00','DD/MM/YYYY HH24:MI:SS')
								   AND     TO_DATE(cFechaFin || ' 23:59:59','DD/MM/YYYY HH24:MI:SS')
		GROUP BY V.COD_GRUPO_CIA,V.COD_LOCAL,V.SEC_USU_LOCAL
	),
	PPTO AS (
		SELECT PPTO.*
			   FROM PTOVENTA.PROG_RENTABLES RENT JOIN 
					PTOVENTA.PROG_RENTABLES_PPTO_LOC PPTO ON (RENT.COD_GRUPO_CIA = PPTO.COD_GRUPO_CIA
					AND RENT.COD_PROG = PPTO.COD_PROG
					AND PPTO.COD_LOCAL = cCodLocal_in)
				WHERE RENT.COD_GRUPO_CIA = cCodGrupoCia_in
				AND RENT.FEC_INI = TRUNC(TO_DATE(cFechaInicio || ' 00:00:00','DD/MM/YYYY HH24:MI:SS'),'MM')
	)
	SELECT NVL(USU.COD_TRAB,' ')  || 'Ã' ||
     	     USU.NOM_USU ||' '|| USU.APE_PAT ||' '|| NVL(USU.APE_MAT,' ')  || 'Ã' ||
      	   nvl(TO_CHAR(MON_TOT_GRUPO_A,'999,990.00') ,' ') || 'Ã' ||
       	   nvl(TO_CHAR(MON_TOT_PP,'999,990.00') ,' ') || 'Ã' ||
           nvl(TO_CHAR(CANT_PED,'999,990.00') ,' ')  || 'Ã' ||
           nvl(TO_CHAR(VAL1,'999,990.00')  ,' ')  || 'Ã' ||
           nvl(TO_CHAR(VAL2,'999,990.00')  ,' ')  || 'Ã' ||
           nvl(TO_CHAR(CANT_PED_ANUL,'999,990.00'),' ')|| 'Ã' ||
           nvl(TO_CHAR(MON_TOT_VTA_S_IGV,'999,990.00') ,' ')|| 'Ã' ||
           nvl(TO_CHAR(MON_TOT_VTA,'999,990.00') ,' ') || 'Ã' || --"Mto. Total",
           nvl(TO_CHAR(MON_TOT_BONO_S_IGV,'999,990.00')  ,' ')  || 'Ã' ||
           nvl(TO_CHAR(MON_TOT_BONO,'999,990.00')  ,' ')  || 'Ã' || --"Mto. Rent",           
           nvl(TO_CHAR(TOT_COM,'999,990.00') ,' ') || 'Ã' || --"Pto. Tot.",
           --ERIOS 15.07.2016 Para calculo de comision Albis
           CASE WHEN LOGRO_VTA IS NULL THEN ' ' ELSE CONCAT(TO_CHAR(LOGRO_VTA,'999,990.00') ,'%') END || 'Ã' || --logro_vta,
           CASE WHEN LOGRO_LLEE IS NULL THEN ' ' ELSE CONCAT(TO_CHAR(LOGRO_LLEE,'999,990.00'),'%') END || 'Ã' || --logro_llee,
           CASE WHEN LOGRO_VTA IS NULL THEN ' ' ELSE 
           TO_CHAR(
           CASE WHEN LOGRO_VTA >= 100 THEN 50 ELSE 0 END + --BONO_VTA,
           ROUND(MON_TOT_VTA_S_IGV*(
           CASE WHEN LOGRO_VTA >= 100 THEN              
                  CASE WHEN MON_TOT_VTA_S_IGV < 5000 THEN 0.25 WHEN MON_TOT_VTA_S_IGV < 35000 THEN 0.50 ELSE 0.75 END             
                ELSE 0 END
           )/100,2) + --PREMIO_VTA,
           ROUND(TOT_COM*(
           CASE WHEN LOGRO_VTA < 100 AND LOGRO_LLEE >= 75 THEN 
                  CASE WHEN LOGRO_VTA >= 85 THEN 98 WHEN LOGRO_VTA >= 65 THEN 90 ELSE 0 END
                WHEN LOGRO_VTA >= 100 AND LOGRO_LLEE >= 70 THEN 
                  100
                ELSE 0 END
           )/100,2) --COMISION
           ,'999,990.00') END monto_comision
    FROM   PTOS,
           PBL_USU_LOCAL USU,
           (SELECT COD_GRUPO_CIA,COD_LOCAL,SEC_USU_LOCAL,
           ROUND((MON_TOT_VTA_S_IGV/PPTO.PPTO_VOLUMEN)*100,2) LOGRO_VTA,
           ROUND((MON_TOT_BONO_S_IGV/PPTO.PPTO_LLEE)*100,2) LOGRO_LLEE
           FROM PPTO NATURAL JOIN PTOS) VOL
    WHERE  PTOS.COD_GRUPO_CIA = USU.COD_GRUPO_CIA
           AND PTOS.COD_LOCAL = USU.COD_LOCAL
           AND PTOS.SEC_USU_LOCAL = USU.SEC_USU_LOCAL
    AND    VOL.COD_GRUPO_CIA(+) = USU.COD_GRUPO_CIA
    AND    VOL.COD_LOCAL(+)     = USU.COD_LOCAL
    AND    VOL.SEC_USU_LOCAL(+) = USU.SEC_USU_LOCAL
		UNION
        SELECT ' '  || 'Ã' ||
    	   	 'TOTALES'  || 'Ã' ||
           ...
           nvl(TO_CHAR(SUM(NVL(V.MON_TOT_VTA_S_IGV,0)),'999,990.00') ,' ')|| 'Ã' ||
           nvl(TO_CHAR(SUM(NVL(V.MON_TOT_VTA,0)),'999,990.00') ,' ')|| 'Ã' ||
           nvl(TO_CHAR(SUM(NVL(V.MON_TOT_BONO_S_IGV,0)),'999,990.00')  ,' ')  || 'Ã' ||
           nvl(TO_CHAR(SUM(NVL(V.MON_TOT_BONO,0)),'999,990.00')  ,' ')  || 'Ã' ||
           nvl(TO_CHAR(SUM(V.TOT_COM),'999,990.00') ,' ') || 'Ã' ||
           ' ' || 'Ã' || ' ' || 'Ã' || ' '
    FROM   PTOS V
    GROUP BY ' ','TOTALES';
...
-- 2009-10-01 JOLIVA: Se va a mostrar el total de venta (cabeceras) en el total de ventas
  ...
  BEGIN
   ...
   END IF;
	...
	END;
	IF v_dFechIniVig IS NOT NULL AND to_date(cFechaInicio,'dd/MM/yyyy') < v_dFechIniVig THEN
	  delete from TMP_RES_VTA_VEND_LOCAL_MEZON;
	  delete from TMP_RES_VTA_VEND_LOCAL_DEL;
	  delete from TMP_RES_VTA_VEND_LOCAL_INS;
	  goto reporte;
	END IF;
...
-- KMONCADA 28.06.2016 ERROR CON LOCALES CON LETRAS
      sqltext := 'SELECT
...
/*******************************************************************************************************************************/
  ...
  BEGIN
    BEGIN
		SELECT EST_TAB_GRAL INTO vRetorno
		FROM PTOVENTA.PBL_TAB_GRAL
		WHERE COD_APL = 'FARMA_VENTA'
		AND COD_TAB_GRAL = 'IND_TIPO_COMISION'
		AND LLAVE_TAB_GRAL = cCodCia_in;
