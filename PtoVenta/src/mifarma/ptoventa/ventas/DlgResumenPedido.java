            ...
            for(int c = 0; c < VariablesVentas.vArrayList_ResumenPedido.size(); c++) {
                ...
                if (textoCod.trim().equals(codigo)) {
                    ...
                    if (!determinarIgualdadMultiplo(pFila)) {
                        if (JConfirmDialog.rptaConfirmDialogDefaultNo(this, "Esta escaneando producto ya existente en diferente presentación, \nSi continúa se elminara el anterior, \n¿desea continuar?")) {
                ...
                try
                {
                            ...
                            
                    {
                            ...
                            )
                        {
                            ...
                            if(VariablesVentas.vRuc_Cli_Ped.trim().length()==0 || VariablesVentas.vNom_Cli_Ped.trim().length()==0){
                    ...
                    DBVentas.grabarCabeceraPedido(pIngresoComprobanteManual,
                                                  VariablesCaja.pManualTipCompPago,
            ...
            if (diferencia < 0) {
                ...
                if (e.getKeyCode() == KeyEvent.VK_F4&&!pIngresoComprobanteManual) {
                    ...
                    /*}*/
                           ...
                           (e.getKeyChar() == '+'&&!pIngresoComprobanteManual)) {
                    // grabarPedidoVenta(ConstantsVentas.TIPO_COMP_BOLETA);          
                    /*if(cargaLogin_verifica())                    
                    {*/
                        lblNombreVendedor.setText(FarmaVariables.vNomUsu.trim() + " " + 
        ...
        else
        {
               ...
               VariablesConvenioBTLMF.vCodConvenio.length() > 0)
            {
                if (vkF.equalsIgnoreCase("F1")){
                    ...
                    try{
                        tipoDocBeneficiario = DBConvenioBTLMF.getComprobanteBeneficiario(VariablesConvenioBTLMF.vCodConvenio);
                    }catch(Exception ex){
                    ...
                    if("BOL".equalsIgnoreCase(tipoDocBeneficiario) || "M".equalsIgnoreCase(tipoDocBeneficiario)){
                        if(VariablesConvenioBTLMF.vDatosConvenio != null){
                            for (int j = 0; j < VariablesConvenioBTLMF.vDatosConvenio.size(); j++){
                                Map convenio = (Map)VariablesConvenioBTLMF.vDatosConvenio.get(j);
                                String pCodCampo      = (String)convenio.get(ConstantsConvenioBTLMF.COL_CODIGO_CAMPO);//COD_TIPO_CAMPO
                                String pDesCampo      = (String)convenio.get(ConstantsConvenioBTLMF.COL_VALOR_IN); // NOMBRE
                                String pCodValorCampo = (String)convenio.get(ConstantsConvenioBTLMF.COL_COD_VALOR_IN); // DNI
...
boolean band = false;
    ...
    public boolean existeSaldoCredDispBenif(JDialog dialog) {
        ...
        try {
            ...
            if (vEstado.equals("N")) {
                FarmaUtility.showMessage(dialog,"El Beneficiario ha sido Bloqueado en Matriz. Comuníquese con el área de Convenios.",
                                         "");
                ret = false;
            } else {
    ...
    private void cobraPedidoManual() {
        ...
        if(VariablesCaja.pManualTipCompPago.trim().equalsIgnoreCase("01")){
            ...
            FarmaUtility.showMessage(this, "Se va Ingresar la :\n" +
                                           "BOLETA : "+VariablesCaja.pManualSerieComprobante+VariablesCaja.pManualNumCompPago, txtDescProdOculto);
        }
        else 
            if(VariablesCaja.pManualTipCompPago.trim().equalsIgnoreCase("02")){
                vkF = "F4";
                FarmaUtility.showMessage(this, "Se va Ingresar la :\n" +
                                               "FACTURA : "+VariablesCaja.pManualSerieComprobante+VariablesCaja.pManualNumCompPago, txtDescProdOculto);                
            }
        agregarComplementarios(vkF);
    }
}
