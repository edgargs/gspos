...
public class DlgListaDetallePedido extends JDialog {
    ...
    private String vFracVta;
    private String vSecDet;
    private String vFracLocal;
    ...
    private void cargaListaDetallePedido() {
        try {
            contextProforma.cargaListaDetPedidos(tableModelDetallePedido, VariablesDelivery.vNumeroPedido,
    ...
    private void this_windowOpened(WindowEvent e) {
        ...
        if (!UtilityPtoVenta.isLocalVentaMayorista() && tipoVenta.equals(ConstantsVentas.TIPO_PEDIDO_INSTITUCIONAL)){
        ...
        if (!UtilityPtoVenta.isLocalVentaMayorista() && !ConstantsVentas.TIPO_PEDIDO_MESON.equals(tipoVenta)) {
    ...
    private void obtieneValores() {
        int valor = tblListaDetallePedido.getSelectedRow();
        VariablesDelivery.vCodProducto =
                FarmaUtility.getValueFieldJTable(tblListaDetallePedido, valor, 1);
        VariablesDelivery.vDescProducto =
                FarmaUtility.getValueFieldJTable(tblListaDetallePedido, valor, 2);
        VariablesDelivery.vCantSolicitada =
                FarmaUtility.getValueFieldJTable(tblListaDetallePedido, valor, 5);
        VariablesDelivery.vUnidVta =
                FarmaUtility.getValueFieldJTable(tblListaDetallePedido, valor, 3); //04.08.2014
        vFracVta = FarmaUtility.getValueFieldJTable(tblListaDetallePedido, valor, 10);
        vSecDet = FarmaUtility.getValueFieldJTable(tblListaDetallePedido, valor, 11);
        vFracLocal = FarmaUtility.getValueFieldJTable(tblListaDetallePedido, valor, 12);
    }

    private void  verificaCheckJTable() {
        int row = tblListaDetallePedido.getSelectedRow();
        Boolean valor = (Boolean)(tblListaDetallePedido.getValueAt(row, 0));
    ...
    private void deseleccionaProducto() {
        int valor = tblListaDetallePedido.getSelectedRow();
        VariablesDelivery.vCodProducto =
                ((String)(tblListaDetallePedido.getValueAt(valor, 1))).trim();
        vSecDet = FarmaUtility.getValueFieldJTable(tblListaDetallePedido, valor, 11);
    ...
    private void muestraIngresoCantidad() {
        ...
        dlgIngresoCantLote.setContext(contextProforma);
        dlgIngresoCantLote.setValFracVta(vFracVta, vFracLocal);
        dlgIngresoCantLote.setSecPedVta(vSecDet);
    ...
    private boolean eliminaVtaInstiProd() {
        try {
            obtieneValores();
            DBDelivery.eliminaDetalleProducto(VariablesDelivery.vNumeroPedido, VariablesDelivery.vCodProducto, vSecDet);
    ...
    private ArrayList obtieneProductosCantidadTotalSel() {
        ...
        try {
            myArray = contextProforma.obtieneProductosSeleccionTotalLote();
        } catch (Exception sql) {
    ...
    private void colocaCheckProdInicio() {
        ...
        String prodTable = "", tmpSecDet;
        String prodArray = "", secDetArray;
        if (myArray.size() > 0) {
            for (int i = 0; i < tblListaDetallePedido.getRowCount(); i++) {
                prodTable = FarmaUtility.getValueFieldJTable(tblListaDetallePedido, i, 1);
                tmpSecDet = FarmaUtility.getValueFieldJTable(tblListaDetallePedido, i, 11);
                for (int j = 0; j < myArray.size(); j++) {
                    prodArray = FarmaUtility.getValueFieldArrayList(myArray, j, 0);
                    secDetArray = FarmaUtility.getValueFieldArrayList(myArray, j, 1);
                    if (prodTable.equalsIgnoreCase(prodArray) &&
                        tmpSecDet.equalsIgnoreCase(secDetArray)) {
    ...
    public void setContext(ContextProforma contextProforma) {
    ...
    private void funcionF11() {
        VariablesDelivery.vIndProformaMenorStock = false;
        if (tblListaDetallePedido.getRowCount() > 0) {
            if (!validaChecksTable()) {
                if(ConstantsVentas.TIPO_PEDIDO_MESON.equals(tipoVenta)){
                    if (JConfirmDialog.rptaConfirmDialogDefaultNo(this, "NO SE HAN COMPLETADO LOS PRODUCTOS." +
                        "\n¿Está seguro de generar el pedido?")){
                        VariablesDelivery.vIndProformaMenorStock = true;
                        cerrarVentana(true);
                    }
                }else{
            ...
            }else if (JConfirmDialog.rptaConfirmDialog(this, "¿Está seguro de generar el pedido?")){
                cerrarVentana(true);
            }
        }
    }
