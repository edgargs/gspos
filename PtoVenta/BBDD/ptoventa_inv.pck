...
RETURN varchar2;
 ...
 FUNCTION F_OBTIENE_MOT_INSUMO(cCodMotKardex_in CHAR)
  ...
  --Descripcion: Valida precio competencia ingresado.
  --Fecha       Usuario		Comentario
  --14/07/2015  ERIOS       Creacion
  FUNCTION F_VALIDA_PRECIO_COMPETENCIA(cCodGrupoCia_in IN CHAR, cCodLocal_in IN CHAR, cCodProd_in IN CHAR, nPrecUnit_in IN NUMBER) RETURN VARCHAR2;

/*********************************************************************************/
  --Descripcion: MENSAJE SOBRE POLITICA DE CAMBIO
  --Fecha         Usuario		   Comentario
  --24/02/2016    KMONCADA     Creacion
  FUNCTION F_GET_MENSAJE_POLITICA_CAMBIO(cCodGrupoCia_in IN CHAR,
                                         cCodLocal_in    IN CHAR,
                                         cCodProd_in     IN CHAR)
    RETURN VARCHAR2;
  
  FUNCTION INV_LISTA_LOTE_PRODUCTO(cCod_Prod_in IN CHAR)
  RETURN FarmaCursor;
  
  FUNCTION INV_STOCK_PRODUCTO_LOCAL_LOTE(cCodProd_in IN CHAR, cLote_in IN CHAR, cFecVencLote_in IN CHAR) RETURN VARCHAR2;
  
  PROCEDURE INV_INGRESA_AJUSTE_KARDEX_LOTE(cCodGrupoCia_in  IN CHAR,
                                            cCodLocal_in      IN CHAR,
                                            cCodProd_in       IN CHAR,
                                            cCodMotKardex_in  IN CHAR,
                                            cNeoCant_in       IN CHAR,
                                            ...
                                            cUsu_in           IN CHAR,
                                            cLote_in IN CHAR,
                                            cFecVencLote_in IN CHAR,
                                            cIndCorreoAjuste IN CHAR DEFAULT 'S');

  --INVENTARIO GET STOCK

  --Descripcion: Busca el stock del producto por Lote
  --Fecha       Usuario		Comentario
  --14/01/2016  GMORAN     Creación
  FUNCTION INV_STK_PROD_LOTE(cCodGrupoCia_in IN CHAR,
                             cCodLocal_in IN CHAR,
                             cCodProd_in IN CHAR, 
                             clote_in IN CHAR,
                             cFechaVencLote  IN CHAR) RETURN FarmaCursor;       
                             
  --INVENTARIO GET STOCK

  --Descripcion: Insert de los beans de transferencia
  --Fecha       Usuario		Comentario
  --22/01/2016  GMORAN     Creación
                           
  PROCEDURE CONFIRMAR_TRANSF_WB(cGrupoCia_in IN CHAR, cCodLocal_in IN CHAR, cNumNotaEs_in IN CHAR, vIdUsu_in IN VARCHAR2);                                                                
...
CREATE OR REPLACE PACKAGE BODY PTOVENTA."PTOVENTA_INV" AS
  ...
  IF nCantSolicitado IS NOT NULL AND nOrdenCompra IS NOT NULL THEN --ASOSA - 03/10/2014
    ...
    IF FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in, cCodLocal_in) = 'S' THEN
      SELECT NVL(SUM(STK_FISICO),0)
      INTO v_nStkFisico
      FROM LGT_PROD_LOCAL_LOTE A
      WHERE A.COD_GRUPO_CIA = cCodGrupoCia_in
      AND A.COD_LOCAL = cCodLocal_in
      AND A.COD_PROD = cCodProd_in
      AND A.LOTE = vNumLote_in;
    END IF;
    ...
    ELSE
        INV_GRABAR_KARDEX(cCodGrupoCia_in => cCodGrupoCia_in ,
                          cCodLocal_in => cCodLocal_in,
                          cCodProd_in => cCodProd_in,
                          cCodMotKardex_in => cCodMotKardex_in,
                          cTipDocKardex_in => cTipDocKardex_in,
                          cNumTipDoc_in => cNumNota_in,
                          nStkAnteriorProd_in => v_nStkFisico,
                          --nCantMov_in,
                          nCantMovProd_in => (nCantMov_in - vCantExceso),  --ASOSA - 25/09/2014
                          --nCantSolicitado, --ASOSA - 15/08/2014
                          nValFrac_in => v_nValFrac, 
                          cDescUnidVta_in => v_vDescUnidVta,
                          cUsuCreaKardex_in => vUsu_in,
                          cCodNumera_in => COD_NUMERA_SEC_KARDEX,
                          cGlosa_in => '',
                          -- dubilluz 08.06.2016
                          cTipDoc_in  =>'',
                          cNumDoc_in  => '',
                          -- dubilluz 08.06.2016                          
                          vFecVecProd_in => vFecVecProd_in, 
                          vNumLote_in => vNumLote_in);
      ...
      IF FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in, cCodLocal_in) = 'S' THEN
        ...
        IF (v_Cantidad > 0) THEN 
          ...
          SET FECHA_VENCIMIENTO_LOTE = to_date(vFecVecProd_in,'dd/mm/yyyy'),
              A.STK_FISICO = A.STK_FISICO + nCantMov_in,
              USU_MOD_PROD_LOCAL_LOTE = vUsu_in,
        ...
        ELSE
          ...
          VALUES (cCodGrupoCia_in, cCodLocal_in, cCodProd_in, vNumLote_in, nCantMov_in,
              to_date(vFecVecProd_in,'dd/mm/yyyy'), vUsu_in, SYSDATE);
...
--          DECODE(P.IND_PROD_FRACCIONABLE,'S',L.UNID_VTA,P.DESC_UNID_PRESENT) || 'Ã' ||
 ...
 FUNCTION GET_DESC_ORIGEN(cCodGrupoCia IN CHAR, cTipoMaestro_in IN CHAR,cCodBusqueda_in IN CHAR,cNumNotaEs_in IN CHAR DEFAULT NULL)
  ...
  RETURN VARCHAR2
  IS
  --JMELGAR 02/11/2015 Se incrementa el valor de la variable.
    v_vDes VARCHAR2(120);
  BEGIN
    IF cTipOrigen_in = g_cTipoOrigenLocal THEN --LOCAL
      SELECT DESC_CORTA_LOCAL INTO v_vDes
...
/****************************************************************************************************/
  ...
  BEGIN
    ...
    VALUES(cCodGrupoCia_in,cCodLocal_in,v_nNumNota, SYSDATE,'P', cCodLocal_in,vTipDestino_in,cCodDestino_in,
    cTipMotivo_in,vDesEmp_in, vRucEmp_in , 
    --vDirEmp_in,
    Substr(vDirEmp_in,1,118),
    vDesTran_in, vRucTran_in, 
    --vDirTran_in,
    Substr(vDirTran_in,1,118),
    vPlacaTran_in, nCantItems_in,nValTotal_in, g_cTipoNotaSalida,vUsu_in,g_cTipCompGuia,cCodMotTransInterno_in);
...
-- transferencia desde local
  ...
  vUsu_in, v_nValFrac,v_vDescUnidVta);
    ...
    IF(cIndLote_in = 'M')THEN      
      SELECT COUNT(*) INTO v_count_prod 
      FROM LGT_PROD_LOCAL_LOTE
      WHERE  COD_GRUPO_CIA = cCodGrupoCia_in  
          AND    COD_LOCAL     = cCodLocal_in    
          AND    COD_PROD      = cCodProd_in
          AND    LOTE          = vNumLote_in
          AND    FECHA_VENCIMIENTO_LOTE = to_date(vFecVecProd_in, 'dd/mm/yyyy');
          
      IF(v_count_prod > 0) THEN 
          
          SELECT PLL.STK_FISICO
            INTO   v_nStkFisico_lote
          FROM   LGT_PROD_LOCAL_LOTE PLL
          WHERE  PLL.COD_GRUPO_CIA = cCodGrupoCia_in  
          AND    PLL.COD_LOCAL     = cCodLocal_in    
          AND    PLL.COD_PROD      = cCodProd_in
          AND    PLL.LOTE          = vNumLote_in
          AND    PLL.FECHA_VENCIMIENTO_LOTE = to_date(vFecVecProd_in, 'dd/mm/yyyy');
      
         --Actualizar Stock de Prod Local Lote
          UPDATE LGT_PROD_LOCAL_LOTE
          SET USU_MOD_PROD_LOCAL_LOTE = vUsu_in,
              FEC_MOD_PROD_LOCAL_LOTE = SYSDATE,
              STK_FISICO              = v_nStkFisico_lote - nCantMov_in
          WHERE  COD_GRUPO_CIA = cCodGrupoCia_in  
          AND    COD_LOCAL     = cCodLocal_in    
          AND    COD_PROD     = cCodProd_in
          AND    LOTE         = vNumLote_in
          ...
          -- obtener stk actual de la tabla lgt_prod_local
          SELECT P.STK_FISICO
                 INTO v_nStkFisico
          ...
          -- Actualizar el stock de la tabla lgt_prod_local
          UPDATE LGT_PROD_LOCAL
          SET USU_MOD_PROD_LOCAL = vUsu_in,
              FEC_MOD_PROD_LOCAL = SYSDATE,
              STK_FISICO              = v_nStkFisico - nCantMov_in
          WHERE  COD_GRUPO_CIA = cCodGrupoCia_in  
          AND    COD_LOCAL     = cCodLocal_in    
          AND    COD_PROD     = cCodProd_in;
      ...
      ELSE
          raise_application_error(-20022,'El producto no existe en el Lote '|| vNumLote_in ||' con fecha de vencimiento '|| vFecVecProd_in);
      END IF;
    ELSE
      --DESCONTAR STOCK FISICO,STOCK COMPROMETIDO
      UPDATE LGT_PROD_LOCAL 
      SET USU_MOD_PROD_LOCAL = vUsu_in,FEC_MOD_PROD_LOCAL = SYSDATE,
          STK_FISICO = STK_FISICO - nCantMov_in
      WHERE COD_GRUPO_CIA = cCodGrupoCia_in
            AND COD_LOCAL = cCodLocal_in
            AND COD_PROD = cCodProd_in;
...
--
  ...
  BEGIN
    OPEN curProd FOR
    SELECT PTOVENTA_VTA.F_GET_STOCK_PRODUCTO(cCodGrupoCia_in, cCodLocal_in, cCodProd_in, 'S') || 'Ã' ||
           --(STK_FISICO )  || 'Ã' ||
         IND_PROD_CONG  || 'Ã' ||
  ...
  PROCEDURE INV_ANULA_GUIA_INGRESO(cCodGrupoCia_in IN CHAR,cCodLocal_in IN CHAR,cNumNota_in IN CHAR,
                                    ...
                                    vIdUsu_in IN VARCHAR2)
  AS
    CURSOR curDet(vIsLocalM IN CHAR) IS
    SELECT SEC_DET_NOTA_ES,
           COD_PROD,
           CANT_MOV,
           VAL_FRAC,
           DECODE(vIsLocalM, 'S', NUM_LOTE_PROD, NULL) LOTE,
           DECODE(vIsLocalM, 'S', TO_CHAR(FEC_VCTO_PROD,'DD/MM/YYYY'), NULL) FECHA_VENCIMIENTO
    FROM LGT_NOTA_ES_DET
    WHERE COD_GRUPO_CIA = cCodGrupoCia_in
          AND COD_LOCAL = cCodLocal_in
          AND NUM_NOTA_ES = cNumNota_in;
  ...
  BEGIN
    ...
    FOR det IN curDet(FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in, cCodLocal_in))
    LOOP
      SELECT STK_FISICO,STK_FISICO,VAL_FRAC_LOCAL,UNID_VTA,L.IND_PROD_CONG
        INTO v_nFisico,v_nComprometido,v_nValFrac, v_vDescUnidVta, v_cProdCong
      FROM LGT_PROD_LOCAL L
      ...
      IF (v_nComprometido - v_nCant) >= 0 THEN
        ...
        -- KMONCADA 10.08.2016 [PROYECTO M] QUITA STOCK POR LOTE
        IF FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in, cCodLocal_in) = 'S' THEN
          UPDATE LGT_PROD_LOCAL_LOTE A
          SET A.USU_MOD_PROD_LOCAL_LOTE = vIdUsu_in,
              A.FEC_MOD_PROD_LOCAL_LOTE = SYSDATE,
              A.STK_FISICO = A.STK_FISICO - v_nCant
          WHERE COD_GRUPO_CIA = cCodGrupoCia_in
          AND COD_LOCAL = cCodLocal_in
          AND COD_PROD = det.COD_PROD
          AND A.LOTE = DET.LOTE;
        ...
        INV_GRABAR_KARDEX(cCodGrupoCia_in ,
                          ...
                          cGlosa_in => '',
                          -- KMONCADA 10.08.2016 [PROYECTO M]
                          vFecVecProd_in => det.fecha_vencimiento,
                          vNumLote_in => det.lote);
        --AGREGADO 13/06/2006 ERIOS
        --MODIFICADO 23/06/2006 ERIOS
  ...
  BEGIN
    ...
    INSERT INTO LGT_KARDEX(COD_GRUPO_CIA, COD_LOCAL, SEC_KARDEX, COD_PROD, COD_MOT_KARDEX,
     ...
     VALUES (cCodGrupoCia_in,  cCodLocal_in, v_cSecKardex, cCodProd_in, cCodMotKardex_in,
                                     ...
                                     vNumLote_in, to_date(vFecVecProd_in,'dd/mm/yyyy'));
...
/* FUNCTION INV_LISTA_ITEMS_AK(cCodGrupoCia_in IN CHAR ,
 ...
 IS
   curLabs FarmaCursor;
 BEGIN
  --JMELGAR 02/11/2015 Se agrega la condicion de tip_comp_pago
  OPEN curLabs FOR
     SELECT DISTINCT
...
PROCEDURE INV_INGRESA_AJUSTE_KARDEX(cCodGrupoCia_in  IN CHAR,
 ...
 END;
  ...
  IS
    curLote FarmaCursor;
  BEGIN
    -- KMONCADA 02.08.2016 [PROYECTO M] CARGAR SEGUN TABLA DE LOTES DE PRODUCTOS
    IF FARMA_UTILITY.F_IS_LOCAL_TIPO_VTA_M(cCodGrupoCia_in, cCodLocal_in) = 'S' THEN
      OPEN curLote FOR
        SELECT A.LOTE || 'Ã' || 
               DECODE(A.FECHA_VENCIMIENTO_LOTE,NULL,' ',TO_CHAR(A.FECHA_VENCIMIENTO_LOTE,'dd/MM/yyyy')) || 'Ã' ||
               TRIM(TO_CHAR(A.STK_FISICO,'99990.00'))
        FROM LGT_PROD_LOCAL_LOTE A
        WHERE COD_GRUPO_CIA = cCodGrupoCia_in
        AND COD_LOCAL = cCodLocal_in
        AND COD_PROD = cCodProd_in
        order by A.FECHA_VENCIMIENTO_LOTE ASC;
    ELSE
      OPEN curLote FOR
      SELECT NUM_LOTE_PROD || 'Ã' || 
             DECODE(FEC_VENC_LOTE,NULL,' ',TO_CHAR(FEC_VENC_LOTE,'dd/MM/yyyy')) || 'Ã' ||
             '0'
...
PROCEDURE INV_ACT_KARDEX_GUIA_REC(cGrupoCia_in   IN CHAR,
 ...
 END;
  ...
  RETURN VARCHAR2
  IS
    v_vDireccion PBL_LOCAL.DIREC_LOCAL_CORTA%TYPE;
  BEGIN
    SELECT DIREC_LOCAL_CORTA
      INTO v_vDireccion
    FROM PBL_LOCAL
    WHERE COD_GRUPO_CIA = cGrupoCia_in
          AND COD_LOCAL = cCodLocal_in;
...
BEGIN
 ...
 END;
  ...
  BEGIN
    ...
    SELECT trim(REPLACE(TEXTO_IMP, CHR(10), 'Ã'))
      INTO cLinea
      FROM lgt_nota_es_Cab
     WHERE COD_GRUPO_CIA = cCodGrupoCia_in
       AND COD_LOCAL = cCodLocal_in
    ...
    vchar := cLinea;
    -- KMONCADA 30.12.2014 PARA CONSIDERAR LA TECLA ENTER PRESIONADA Y LUEGO EL RANGO 
    -- DE 100 CARACTERES POR LINEA
    for lista in (
      SELECT EXTRACTVALUE(xt.column_value, 'e') VAL 
              FROM TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<coll><e>' ||
                                 REPLACE( (
                                 vchar
      ),'Ã','</e><e>') ||'</e></coll>'),'/coll/e'))) xt
      )loop
        if length(trim(lista.val))!=0 then

          co:=0;
          IF LENGTH(trim(lista.val))>cant THEN
            cantM := trunc((length(lista.val)/cant));
            while (co<cantM) loop
                  tvchar := tvchar || substr(lista.val,((co*(cant))+1),cant)||'Ã';
                  co := co+1;
            end loop;
            tvchar := tvchar || substr(lista.val,((co*cant)+1))||'Ã';
            
          ELSE
            if length(trim(tvchar))>0 then
              tvchar := tvchar ||'Ã'||lista.val;
            else
              tvchar:= lista.val;
            end if;
          END IF;
...
-- KMONCADA
 ...
 FUNCTION F_OBTIENE_MOT_INSUMO(cCodMotKardex_in CHAR)
  ...
  BEGIN
  WITH
  MN AS (SELECT (100-TO_NUMBER(DESC_CORTA))/100 VAL FROM PTOVENTA.PBL_TAB_GRAL WHERE COD_TAB_GRAL = 'MARGEN_PREC_COMP' AND LLAVE_TAB_GRAL = '01'),
  MX AS (SELECT (100+TO_NUMBER(DESC_CORTA))/100 VAL FROM PTOVENTA.PBL_TAB_GRAL WHERE COD_TAB_GRAL = 'MARGEN_PREC_COMP' AND LLAVE_TAB_GRAL = '02')
  SELECT CASE WHEN nPrecUnit_in BETWEEN
  MN.VAL*VAL_PREC_VTA AND
  MX.VAL*VAL_PREC_VTA THEN 'S'
  ELSE 'EL PRECIO NO ES VALIDO. VERIFIQUE.' END
    INTO vRetorno
  FROM PTOVENTA.LGT_PROD_LOCAL,MN,MX
  WHERE COD_GRUPO_CIA = cCodGrupoCia_in
  AND COD_LOCAL = cCodLocal_in
  AND COD_PROD = cCodProd_in
  ;
...
/*********************************************************************************/
  ...
  FUNCTION F_GET_MENSAJE_POLITICA_CAMBIO(cCodGrupoCia_in IN CHAR,
    ...
    vIsActivo CHAR(1);
    vMeses AUX_POLITICA_CANJE_LAB.MESES%TYPE;
    vFecha VARCHAR2(20);
    
    vMensajeAux VARCHAR2(3000);
    vMsjPolitica VARCHAR2(200);
    vMsjPoliticaCero VARCHAR2(200);
    vMsjPoliticaNula VARCHAR2(200);
  BEGIN 
    BEGIN 
    SELECT A.LLAVE_TAB_GRAL
    INTO vIsActivo
    FROM PBL_TAB_GRAL A
    WHERE A.ID_TAB_GRAL = 601;
    EXCEPTION 
      WHEN OTHERS THEN
        vIsActivo := 'N';
    END;
    ...
    IF vIsActivo = 'S' THEN
      ...
      INTO vMensajeAux
      FROM PBL_TAB_GRAL A
      WHERE A.ID_TAB_GRAL = 602;
      
      vMensajeAux := UPPER(vMensajeAux);
      
      vMsjPolitica := SUBSTR(vMensajeAux,0, INSTR(vMensajeAux,'@')-1);
      vMensajeAux := SUBSTR(vMensajeAux,INSTR(vMensajeAux,'@')+1);
  
      vMsjPoliticaCero := SUBSTR(vMensajeAux,0, INSTR(vMensajeAux,'@')-1);
      vMensajeAux := SUBSTR(vMensajeAux,INSTR(vMensajeAux,'@')+1);
  
      vMsjPoliticaNula := vMensajeAux;
       
      EXCEPTION
        WHEN OTHERS THEN
          IF vMensajeAux IS NULL THEN
          RAISE_APPLICATION_ERROR(-20005,'NO SE PUDO OBTENER LOS MENSAJES PARA LA POLITICA DE CANJE');
          END IF;
          
          IF vMsjPolitica IS NULL THEN
            vMsjPolitica := NULL;
            vMsjPoliticaCero := NULL;
            vMsjPoliticaNula := NULL;
          ELSIF vMsjPoliticaCero IS NULL THEN
            vMsjPoliticaCero := NULL;
            vMsjPoliticaNula := NULL;
          ELSIF vMsjPoliticaNula IS NULL THEN
            vMsjPoliticaNula := NULL;
          END IF;
      END;
      ...
      BEGIN
        ...
        INTO vMeses
        FROM LGT_PROD P,
             AUX_POLITICA_CANJE_LAB C
        WHERE P.COD_GRUPO_CIA = cCodGrupoCia_in
        AND P.COD_PROD = cCodProd_in
        AND C.COD_LAB = P.COD_LAB;
      EXCEPTION
        WHEN NO_DATA_FOUND THEN 
          vMeses := NULL;
      END;
      IF vMeses IS NULL THEN
        vMensaje := vMsjPoliticaNula;
      ELSIF vMeses = 0 THEN
        vMensaje := vMsjPoliticaCero;
      ELSE
        vFecha := TO_CHAR(ADD_MONTHS(SYSDATE, vMeses),'DD/MM/YYYY');
        vMsjPolitica := REPLACE(vMsjPolitica,'<<FECHA>>',vFecha);
        vMensaje := vMsjPolitica;
      END IF;
    ELSE
      vMensaje := NULL;
    END IF;
  ...
  -- gmoran 12012016 busqueda del stock del producto segun lote y fecha de vencimiento
  FUNCTION INV_STOCK_PRODUCTO_LOCAL_LOTE (cCodProd_in IN CHAR,
                                          cLote_in IN CHAR,
                                          cFecVencLote_in IN CHAR) 
  RETURN VARCHAR2
  IS
     vRetornoStock VARCHAR2(100);
  BEGIN 
     vRetornoStock :=' ';   
  BEGIN
     SELECT stk_fisico
     ...
     WHERE Cod_Prod = cCodProd_in
     AND lote = cLote_in
     AND to_char(fecha_vencimiento_lote, 'dd/mm/yyyy') = cFecVencLote_in;
     EXCEPTION
      WHEN no_data_found THEN
           vRetornoStock:=' ';
      END;
    
      RETURN vRetornoStock;
     
  END;                                           
  -----------------------------------------------------------------------------------  

  -- gmoran 11012016 creación de funcion tabla lote -----
  ...
  FUNCTION INV_LISTA_LOTE_PRODUCTO(cCod_Prod_in IN CHAR)
   ...
   BEGIN
    ...
    OPEN curLabs FOR
       SELECT
             NVL(L.lote,0)              || 'Ã' ||
             L.fecha_vencimiento_lote   || 'Ã' || 
             NVL(L.stk_fisico,0)          
      FROM lgt_prod_local_lote L
      WHERE L.COD_PROD     = cCod_Prod_in;
      
     RETURN curLabs;
   END;
 -----------------------------------------------------
 
 -- gmoran 12012016 se crea el procedure para el ajuste de kardex ya que se va a realizar por LOTES
...
-- seudocodigo validar  si existe el producto lote en la tabla lgt_prod_local_lote X cod_prod
-- si no existe " raise_application_error(-20022,'El producto no existe');"
    -- VALIDAR SI EL PRODUCTO EXISTE EN LA TABLA LGT_PROD_LOCAL_LOTE
    SELECT COUNT(*) INTO v_count_prod 
    FROM LGT_PROD_LOCAL_LOTE
    WHERE  COD_GRUPO_CIA = cCodGrupoCia_in  
          AND    COD_LOCAL     = cCodLocal_in    
          AND    COD_PROD      = cCodProd_in
          AND    LOTE          = cLote_in
          AND    FECHA_VENCIMIENTO_LOTE = to_date(cFecVencLote_in, 'dd/mm/yyyy');
    
    
    IF(v_count_prod > 0) THEN 
          -- obtiene motivo insumo
          v_cCod_Mot:= F_OBTIENE_MOT_INSUMO(cCodMotKardex_in);
      
          --Obtener STK Actual prod local lote
          SELECT PLL.STK_FISICO,PL.VAL_FRAC_LOCAL,PL.UNID_VTA
            INTO   v_nStkFisico_lote,v_nValFrac,v_vDescUnidVta
          FROM   LGT_PROD_LOCAL_LOTE PLL
          INNER JOIN LGT_PROD_LOCAL PL ON PL.COD_PROD = PLL.COD_PROD 
          WHERE  PLL.COD_GRUPO_CIA = cCodGrupoCia_in  
          AND    PLL.COD_LOCAL     = cCodLocal_in    
          AND    PLL.COD_PROD      = cCodProd_in
          AND    PLL.LOTE          = cLote_in
          AND    PLL.FECHA_VENCIMIENTO_LOTE = to_date(cFecVencLote_in, 'dd/mm/yyyy');
      
          vCantMov_in:= cNeoCant_in - v_nStkFisico_lote;
          v_nNeoCod:=Farma_Utility.COMPLETAR_CON_SIMBOLO(Farma_Utility.OBTENER_NUMERACION(cCodGrupoCia_in,cCodLocal_in,COD_NUMERA_SEC_MOV_AJUSTE_KARD),10,'0','I' );
          Farma_Utility.ACTUALIZAR_NUMERA_SIN_COMMIT(cCodGrupoCia_in,cCodLocal_in,COD_NUMERA_SEC_MOV_AJUSTE_KARD, cUsu_in);
      
         --Actualizar Stock de Prod Local Lote
          UPDATE LGT_PROD_LOCAL_LOTE
          SET USU_MOD_PROD_LOCAL_LOTE = cUsu_in,
              FEC_MOD_PROD_LOCAL_LOTE = SYSDATE,
              STK_FISICO              = TO_NUMBER(cNeoCant_in)
          WHERE  COD_GRUPO_CIA = cCodGrupoCia_in  
          AND    COD_LOCAL     = cCodLocal_in    
          ...
          SET USU_MOD_PROD_LOCAL = cUsu_in,
              FEC_MOD_PROD_LOCAL = SYSDATE,
              STK_FISICO              = v_nStkFisico + TO_NUMBER(vCantMov_in)
          WHERE  COD_GRUPO_CIA = cCodGrupoCia_in  
          AND    COD_LOCAL     = cCodLocal_in    
          AND    COD_PROD     = cCodProd_in;
         ...
         --INSERTAR KARDEX
          ...
          INSERT INTO LGT_KARDEX(COD_GRUPO_CIA, COD_LOCAL, SEC_KARDEX, COD_PROD, COD_MOT_KARDEX,
                                           ...
                                           STK_FINAL_PROD, VAL_FRACC_PROD, DESC_UNID_VTA, USU_CREA_KARDEX,DESC_GLOSA_AJUSTE,
                                           NRO_LOTE, FECHA_VENCIMIENTO_LOTE)
          VALUES (cCodGrupoCia_in,  cCodLocal_in, v_cSecKardex, cCodProd_in, v_cCod_Mot,
                                           cTipDoc_in, v_nNeoCod, v_nStkFisico_lote, vCantMov_in,
                                           (v_nStkFisico_lote + vCantMov_in), v_nValFrac, v_vDescUnidVta, cUsu_in, cGlosa_in,
                                           cLote_in, to_date(cFecVencLote_in,'dd/mm/yyyy'));
          Farma_Utility.ACTUALIZAR_NUMERA_SIN_COMMIT(cCodGrupoCia_in, cCodLocal_in, COD_NUMERA_SEC_KARDEX, cUsu_in);
          
          IF cIndCorreoAjuste='S' AND
             v_cCod_Mot <> MOT_CONSUMO_INTERNO  THEN
            --ENVIAR MAIL DE AJUSTE
            INT_ENVIA_CORREO_AJUSTE(cCodGrupoCia_in,cCodLocal_in,
                                    'AJUSTE DE PRODUCTO: ',
                                    'AJUSTE',
          ...
          END IF;                                      
    ELSE
        raise_application_error(-20022,'El producto no existe en el Lote '|| cLote_in ||' con fecha de vencimiento '|| cFecVencLote_in);
    END IF;
 END;
---------------------------------------------------------- 
 FUNCTION INV_STK_PROD_LOTE(cCodGrupoCia_in IN CHAR,
                              ...
                              cCodProd_in      IN CHAR,
                            clote_in        IN CHAR,
                            cFechaVencLote  IN CHAR)
  RETURN FarmaCursor
  IS
    curVta FarmaCursor;
  BEGIN
    OPEN curVta FOR
    SELECT (PROD_LOCAL_LOTE.STK_FISICO)
    FROM   LGT_PROD_LOCAL_LOTE PROD_LOCAL_LOTE
    WHERE  PROD_LOCAL_LOTE.COD_GRUPO_CIA = cCodGrupoCia_in
    AND     PROD_LOCAL_LOTE.COD_LOCAL = cCodLocal_in
    AND    PROD_LOCAL_LOTE.LOTE = clote_in
    AND    PROD_LOCAL_LOTE.FECHA_VENCIMIENTO_LOTE = to_date(cFechaVencLote, 'dd/mm/yyyy')
    AND     PROD_LOCAL_LOTE.COD_PROD = cCodProd_in FOR UPDATE;   
    RETURN curVta;
  ...
  BEGIN
    ...
    IF v_cTipOrigen = g_cTipoOrigenMatriz THEN
      ...
      --INSERT DE LAS TABLAS DE CAB TRANSFERENCIA.

      INSERT INTO CAB_TRANSFERENCIA(COD_GRUPO_CIA,COD_LOCAL,NUM_NOTA_ES,FEC_NOTA_ES_CAB,EST_NOTA_ES_CAB,
        TIP_DOC,NUM_DOC,COD_ORIGEN_NOTA_ES,CANT_ITEMS,VAL_TOTAL_NOTA_ES_CAB,COD_DESTINO_NOTA_ES,DESC_EMPRESA,
        RUC_EMPRESA,DIR_EMPRESA,DESC_TRANS,RUC_TRANS,DIR_TRANS,PLACA_TRANS,TIP_NOTA_ES,TIP_ORIGEN_NOTA_ES,
        TIP_MOT_NOTA_ES,EST_RECEPCION,USU_CREA_NOTA_ES_CAB,FEC_CREA_NOTA_ES_CAB,USU_MOD_NOTA_ES_CAB,
        FEC_MOD_NOTA_ES_CAB,IND_NOTA_IMPRESA,FEC_PROCESO_SAP,FEC_PROCESO_CE,COD_MOTIVO_INTERNO_TRANS,
        IND_TRANS_AUTOMATICA,COD_GRUPO_CIA_DEL,COD_LOCAL_DEL,NUM_PED_VTA_DEL,SEC_GRUPO_DEL,FEC_INI_CREACION,
        FEC_FIN_CREACION,FEC_INI_CONFIRMACION,FEC_FIN_CONFIRMACION,FEC_INI_CONFIRMACION_LINEA,
        FEC_FIN_CONFIRMACION_LINEA,FEC_INI_CREA_IMPR_GUIAS,FEC_FIN_CREA_IMPR_GUIAS,FEC_INI_ANULACION,
        FEC_FIN_ANULACION,TEXTO_IMP,TRA_CIA,TRA_COD_LOCAL,TRA_NUM_PROFORMA,FECHA_PROC_INT)
      SELECT COD_GRUPO_CIA,COD_LOCAL,NUM_NOTA_ES,FEC_NOTA_ES_CAB,EST_NOTA_ES_CAB,
        TIP_DOC,NUM_DOC,COD_ORIGEN_NOTA_ES,CANT_ITEMS,VAL_TOTAL_NOTA_ES_CAB,COD_DESTINO_NOTA_ES,DESC_EMPRESA,
        RUC_EMPRESA,DIR_EMPRESA,DESC_TRANS,RUC_TRANS,DIR_TRANS,PLACA_TRANS,TIP_NOTA_ES,TIP_ORIGEN_NOTA_ES,
        TIP_MOT_NOTA_ES,EST_RECEPCION,USU_CREA_NOTA_ES_CAB,FEC_CREA_NOTA_ES_CAB,USU_MOD_NOTA_ES_CAB,
        FEC_MOD_NOTA_ES_CAB,IND_NOTA_IMPRESA,FEC_PROCESO_SAP,FEC_PROCESO_CE,COD_MOTIVO_INTERNO_TRANS,
      ...
      INSERT INTO GUIA_TRANSFERENCIA_REM(COD_GRUPO_CIA,COD_LOCAL,NUM_NOTA_ES,SEC_GUIA_REM,NUM_GUIA_REM,
        FEC_CREA_GUIA_REM,USU_CREA_GUIA_REM,FEC_MOD_GUIA_REM,USU_MOD_GUIA_REM,EST_GUIA_REM,NUM_ENTREGA,
        IND_GUIA_CERRADA,IND_GUIA_IMPRESA,TIPO_PED_REP,IND_GUIA_LIBERADA,FEC_PROCESO_INT_SAP,IND_ENV_LEGACY)
      SELECT COD_GRUPO_CIA,COD_LOCAL,NUM_NOTA_ES,SEC_GUIA_REM,NUM_GUIA_REM,
        FEC_CREA_GUIA_REM,USU_CREA_GUIA_REM,FEC_MOD_GUIA_REM,USU_MOD_GUIA_REM,EST_GUIA_REM,NUM_ENTREGA,
        IND_GUIA_CERRADA,IND_GUIA_IMPRESA,TIPO_PED_REP,IND_GUIA_LIBERADA,FEC_PROCESO_INT_SAP,IND_ENV_LEGACY
      FROM LGT_GUIA_REM
      WHERE COD_GRUPO_CIA = cGrupoCia_in
      ...
      INSERT INTO DET_TRANSFERENCIA(COD_GRUPO_CIA,COD_LOCAL,NUM_NOTA_ES,SEC_DET_NOTA_ES,COD_PROD,SEC_GUIA_REM,
        VAL_PREC_UNIT,VAL_PREC_TOTAL,CANT_MOV,VAL_FRAC,EST_NOTA_ES_DET,FEC_NOTA_ES_DET,DESC_UNID_VTA,FEC_VCTO_PROD,
        NUM_LOTE_PROD,CANT_ENVIADA_MATR,NUM_PAG_RECEP,IND_PROD_AFEC,USU_CREA_NOTA_ES_DET,FEC_CREA_NOTA_ES_DET,
        USU_MOD_NOTA_ES_DET,FEC_MOD_NOTA_ES_DET,FEC_PROCESO_SAP,POSICION,NUM_ENTREGA,IND_AJUSTE_DIF,CANT_CONTADA,
        CANT_RECEPCIONADA,SEC_RESPALDO_STK,CANT_EXCESO,CANT_USADA_EXCESO,FECHA_PROC_INT)
      SELECT COD_GRUPO_CIA,COD_LOCAL,NUM_NOTA_ES,SEC_DET_NOTA_ES,COD_PROD,SEC_GUIA_REM,VAL_PREC_UNIT,
        VAL_PREC_TOTAL,CANT_MOV,VAL_FRAC,EST_NOTA_ES_DET,FEC_NOTA_ES_DET,DESC_UNID_VTA,FEC_VCTO_PROD,NUM_LOTE_PROD,
        CANT_ENVIADA_MATR,NUM_PAG_RECEP,IND_PROD_AFEC,USU_CREA_NOTA_ES_DET,FEC_CREA_NOTA_ES_DET,USU_MOD_NOTA_ES_DET,
        FEC_MOD_NOTA_ES_DET,FEC_PROCESO_SAP,POSICION,NUM_ENTREGA,IND_AJUSTE_DIF,CANT_CONTADA,CANT_RECEPCIONADA,
        SEC_RESPALDO_STK,CANT_EXCESO,CANT_USADA_EXCESO,FECHA_PROC_INT
      FROM LGT_NOTA_ES_DET
      WHERE COD_GRUPO_CIA = cGrupoCia_in
            AND COD_LOCAL = cCodLocal_in
            AND NUM_NOTA_ES = cNumNotaEs_in;
