...
/* ************************************************************************ */
    ...
    {   
        ...
        {   
             ...
             * @since 29.09.2014
             * */
            if(!UtilityEposTrx.vPermiteAnular())return;
            //LTAVARA
            ...
            {   if(tblCabeceraPedido.getRowCount()>0)
                {
                    ...
                    if ("1".equals(VariablesVentas.vCodTipProcPago)){
                            if (UtilityEposTrx.validacionEmiteElectronico()== false){
                                    FarmaUtility.showMessage(this,"No se puede generar una NC de contingencia al comprobante Electronico"
                                                                  ,txtCorrelativo);
                                    cerrarVentana(false);
                                   return;                 
                                }
  ...
  /*
   * Se comento para crear el metodo anularPedidoBTLMF que Genenera NC
   * LTAVARA 11.09.2014
   * */
  
  private void anularPedidoBTLMFOld()
  {
    cargarVariables();
      ...
      if(vCantCuponesEmitidosUsados > 0)
        ...
        if(comprobarProductosRestantes())
        {
            ...
            if(UtilityEposTrx.isActFuncElectronica()){//PROCESO ELECTRONICO ACTIVO
                ...
                FarmaUtility.showMessage(this,"Se anulará dicho pedido, pero se generará una solicitud de Nota de Crédito.",pObj);
                DlgNotaCreditoNueva dlgNotaCreditoNueva = new DlgNotaCreditoNueva(myParentFrame,"",true);
                dlgNotaCreditoNueva.setVisible(true);
                ...
                if(emitioAntesElectronico())
                    FarmaUtility.showMessage(this,"Para anular el pedido, el local debe estar activo el \n PROCESO ELECTRONICO de lo contrario no se anulara.",pObj);
                else{
   ...
   /* private void mostrarMensajeRecargaExito() {
    ...
    private boolean validarRecargas()
    {
        ...
        {   boolean retorno = false;
            ...
            String resp = "S"; //urv.obtieneMensajeRecargaAnul(txtCorrelativo.getText().trim(), VariablesVentas.vFechaPedVta_new);  //ASOSA - 09/07/2014
