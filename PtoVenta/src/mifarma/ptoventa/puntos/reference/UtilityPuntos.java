                        ...
                        if (!pPuntosCero && !pPuntosNulo && pPuntosMayorCero) {
                            ...
                            if (beanSaldo == null) {
                                FarmaUtility.showMessage(pDialog, "No se pudo recuperar puntos por:\n" +
                                      ...
                                      beanSaldo.getEstadoOperacion().equalsIgnoreCase(FarmaPuntosConstante.NUMERO_YA_APLICADA) ){//[Desarrollo5] 23.10.2015 se le agrego esta validacion para cuando se haya perdido la conexion con orbis por time out
                                /* *********************************************************************************/
                  ...
                  //  FarmaUtility.showMessage(pJDialog, "PROGRAMA DE PUNTOS:\nVenta en modo OFFLINE", campoTexto);
                    ...
                    if(!isLectorTarjeta){
                        ...
                        if(JConfirmDialog.rptaConfirmDialog(pJDialog, mensaje)){
                            ...
                            boolean resultado = FarmaVariables.vAceptar;
                            if(resultado){
                                VariablesPuntos.frmPuntos.getTarjetaBean().setDni(nroDocumentoAux);
                        ...
                        if(VariablesPuntos.frmPuntos.getTarjetaBean().getNumeroTarjeta()== null || 
                            (VariablesPuntos.frmPuntos.getTarjetaBean().getNumeroTarjeta()!= null &&VariablesPuntos.frmPuntos.getTarjetaBean().getNumeroTarjeta().trim().length()==0)
                          ){
                            if(!campoTextoField.equalsIgnoreCase(nroDocumentoAux)){
                                VariablesPuntos.frmPuntos.getTarjetaBean().setNumeroTarjeta(campoTextoField);
                            }
                          }
                        // seteando el nro de dni
                        if(VariablesPuntos.frmPuntos.getTarjetaBean().getDni()== null || 
                            (VariablesPuntos.frmPuntos.getTarjetaBean().getDni()!= null &&VariablesPuntos.frmPuntos.getTarjetaBean().getDni().trim().length()==0)
                          ){
                            if(nroDocumentoAux.trim().length()>0){
                                VariablesPuntos.frmPuntos.getTarjetaBean().setDni(nroDocumentoAux);
                            }else{
                                try{
                                    VariablesPuntos.frmPuntos.getTarjetaBean().setDni(DBFidelizacion.getDniClienteFidelizado(campoTextoField));
                                }catch(Exception ex){
                                    log.error(" ", ex);
                                }
