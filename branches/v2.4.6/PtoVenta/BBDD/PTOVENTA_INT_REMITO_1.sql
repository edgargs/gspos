...
--------------------------------------------------------
  ...
  AS
    ...
    SELECT ICE.COD_LOCAL||
           ...
           ICE.COD_REMITO||
           DECODE(p_cia,CIA_MIFARMA,'',decode(p_cia,CIA_FASA,'        SCOT','        BBVA'))
           RESUMEN
...
CURSOR CurSobres_du(p_codRemito CHAR,nCambiarFecha_in char,p_cia CHAR) IS
    SELECT DR.COD_GRUPO_CIA COD_GRUPO_CIA,
           DR.COD_LOCAL COD_LOCAL,
           CASE p_cia
           WHEN CIA_MIFARMA THEN TO_CHAR(decode(nCambiarFecha_in,'N',DR.FEC_DIA_VTA,trunc(sysdate,'MM')),'DD/MM/YYYY')
           ...
           CASE p_cia
           WHEN CIA_MIFARMA THEN decode(nCambiarFecha_in,'N',DR.FEC_DIA_VTA,trunc(sysdate,'MM'))
