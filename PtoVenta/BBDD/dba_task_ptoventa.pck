CREATE OR REPLACE PACKAGE PTOVENTA.DBA_TASK_PTOVENTA IS
  ...
  PROCEDURE genera_bat_ftp_int_5v;
  ...
  PROCEDURE genera_comandos_ftp_int_5v(cCodLocal_in	   IN CHAR);
  ...
  PROCEDURE genera_bat_exporta_int_5v(cCodLocal_in	   IN CHAR);
...
--revisado corregido y aumentado 20070831
  ...
  PROCEDURE genera_bat_exporta(cCodLocal_in     IN CHAR) IS
    ...
    CURSOR C1 IS select distinct(substr(utl_inaddr.get_host_address,0,length(utl_inaddr.get_host_address)-1))||'3' from ptoventa.vta_impr_local;
    Iplocal VARCHAR2(15);
...
--========== 2007 generacion de _xcorre_envia.bat para que envie el archivo y lo trae a otra ruta
    ...
    BEGIN
         OPEN C1;
              LOOP
      FETCH C1 INTO iplocal;
      EXIT WHEN C1%NOTFOUND;
      DBMS_OUTPUT.PUT_LINE(iplocal);
              END LOOP;
      CLOSE C1;
    END;
...
--    cline  :='CALL exp system/namego@xe file='||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-1),'yyyymmdd')||'.dmp '||''||'log='||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-1),'yyyymmdd')||'.log owner=ptoventa statistics=NONE';
    ...
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-7),'yyyymmdd')||'.dmp';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-7),'yyyymmdd')||'.log';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-8),'yyyymmdd')||'.dmp';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-8),'yyyymmdd')||'.log';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-9),'yyyymmdd')||'.dmp';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-9),'yyyymmdd')||'.log';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-10),'yyyymmdd')||'.dmp';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-10),'yyyymmdd')||'.log';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-11),'yyyymmdd')||'.dmp';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-11),'yyyymmdd')||'.log';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-12),'yyyymmdd')||'.dmp';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-12),'yyyymmdd')||'.log';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-13),'yyyymmdd')||'.dmp';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-13),'yyyymmdd')||'.log';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-14),'yyyymmdd')||'.dmp';
    utl_file.put_line (output_file, cline);
    cline  := 'call del \\'||Iplocal||'\d$\backup\d_exportador\BACKUP_DATA_LOCAL\'||cCodLocal_in||''||TO_CHAR(TRUNC(SYSDATE-14),'yyyymmdd')||'.log';
    utl_file.put_line (output_file, cline);
...
--==============modificacion 20070720 pedido de Rolando Castro que reintente
  ...
  BEGIN
    select distinct a.cod_cia into   vc_CodCia from   vta_impr_local a;
...
--    cline  :='open 10.11.1.30';
    ...
    END IF;
-- se agrego el cod_cia '003' locales BTL -- msaavedra 19.06.2014
     IF trim(vc_CodCia) = '002' or  trim(vc_CodCia) = '003' THEN
...
--    cline  :='open 10.11.1.250';
  ...
  BEGIN
     ...
     BEGIN
      NULL;
         /*
         11-JUL-2014  TCT Comenta ya no se usara las tablas temporales (autorizado por Julio)
...
--20131104 PARTE 1 DE EXPORTADOR OC,DP,GCONF
    ...
    utl_file.put_line (output_file, cline);
    cline  :='call dir /b D:\INTERFACES\END_IN_OC*.*   >>D:\EXPORTADOR\listadoOC.tot';
    utl_file.put_line (output_file, cline);
    ...
    utl_file.put_line (output_file, cline);
    cline  :='call dir /b D:\INTERFACES\END_IN_DP*.*   >>D:\EXPORTADOR\listadoDP.tot';
    utl_file.put_line (output_file, cline);
...
--20070829 PARTE 2 DE EXPORTADOR CI CC
    ...
    utl_file.put_line (output_file, cline);
    cline  :='call dir /b D:\INTERFACES\OCTMP\END_IN_OC*.*  >>D:\EXPORTADOR\listadoOC2.tot';
    utl_file.put_line (output_file, cline);
    ...
    utl_file.put_line (output_file, cline);
    cline  :='call dir /b D:\INTERFACES\DPTMP\END_IN_DP*.*  >>D:\EXPORTADOR\listadoDP2.tot';
    utl_file.put_line (output_file, cline);
...
------
PROCEDURE genera_bat_ftp_int_5v
  --genera programa que invoca a programas que envia,trae, comprueba, elimina los arhivos SB y FT hacia Matriz
  is
    cpath       VARCHAR2(60);
    cfilename   VARCHAR2(80);
    cline       VARCHAR2(80);
  ...
  BEGIN
    cpath       :='DIR_EXPORTADOR';
    cfilename   :='envia_ftp_int_5v.bat';
    output_file := utl_file.fopen (cpath,cfilename, 'W');
--20131104 PARTE 1 DE EXPORTADOR OC,DP,GCONF
    cline  :='call del D:\EXPORTADOR\listadoFT.tot';
    utl_file.put_line (output_file, cline);
    cline  :='call time /t >>D:\EXPORTADOR\listadoFT.tot';
    utl_file.put_line (output_file, cline);
    cline  :='call dir /b D:\INTERFACES\FT*.*   >>D:\EXPORTADOR\listadoFT.tot';
    utl_file.put_line (output_file, cline);
    cline  :='call dir /b D:\INTERFACES\END_IN_V*.*   >>D:\EXPORTADOR\listadoFT.tot';
    utl_file.put_line (output_file, cline);
    cline  :='call del D:\EXPORTADOR\listadoSB.tot';
    utl_file.put_line (output_file, cline);
    cline  :='call time /t >>D:\EXPORTADOR\listadoSB.tot';
    utl_file.put_line (output_file, cline);
    cline  :='call dir /b D:\INTERFACES\SB*.*   >>D:\EXPORTADOR\listadoSB.tot';
    utl_file.put_line (output_file, cline);
--  envia los archivos SB,FT
    cline  :='CALL ftp -n -v -s:D:\EXPORTADOR\comandos_int_5v.ftp';
    utl_file.put_line (output_file, cline);
--  trae  los archivos SB y FT
    cline  :='CALL ftp -n -v -s:D:\EXPORTADOR\comandos_int_5v_got.ftp';
    utl_file.put_line (output_file, cline);
--20070829 PARTE 2 DE EXPORTADOR FT SB
    cline  :='call del D:\EXPORTADOR\listadoFT2.tot';
    utl_file.put_line (output_file, cline);
    cline  :='call time/t >>D:\EXPORTADOR\listadoFT2.tot';
    utl_file.put_line (output_file, cline);
    cline  :='call dir /b D:\INTERFACES\FTTMP\FT*.*  >>D:\EXPORTADOR\listadoFT2.tot';
    utl_file.put_line (output_file, cline);
    cline  :='call dir /b D:\INTERFACES\FTTMP\END_IN_V*.*  >>D:\EXPORTADOR\listadoFT2.tot';
    utl_file.put_line (output_file, cline);
    cline  :='call sqlplus ptoventa/caba123lleria@xe @D:\EXPORTADOR\evalua_FT.sql';
    utl_file.put_line (output_file, cline);
    cline  :='CALL D:\EXPORTADOR\elimina_mueve_FT.bat';
    utl_file.put_line (output_file, cline);
    --
    cline  :='call del D:\EXPORTADOR\listadoSB2.tot';
    utl_file.put_line (output_file, cline);
    cline  :='call time/t >>D:\EXPORTADOR\listadoSB2.tot';
    utl_file.put_line (output_file, cline);
    cline  :='call dir /b D:\INTERFACES\SBTMP\SB*.*  >>D:\EXPORTADOR\listadoSB2.tot';
    utl_file.put_line (output_file, cline);
    cline  :='call sqlplus ptoventa/caba123lleria@xe @D:\exportador\evalua_SB.sql';
    utl_file.put_line (output_file, cline);
    cline  :='CALL D:\EXPORTADOR\elimina_mueve_SB.bat';
    utl_file.put_line (output_file, cline);
    utl_file.fclose(output_file);
  end;
------
...
---
  ...
  BEGIN
    ...
    utl_file.fclose(output_file);
    END IF;
-- se agrego el cod_cia '003' locales BTL -- ehuaman 07.08.2014
    IF trim(vc_CodCia) = '002' or trim(vc_CodCia)= '003' THEN
...
--    cline  :='mput D:\INTERFACES\CCTMP\CC*.*';
    cline  :='mput OC*.*';
    utl_file.put_line (output_file, cline);
    cline  :='mput END_IN_OC*.*';
    utl_file.put_line (output_file, cline);
--    cline  :='mput D:\INTERFACES\CITMP\CI*.*';
    cline  :='mput DP*.*';
    utl_file.put_line (output_file, cline);
    cline  :='mput END_IN_DP*.*';
    utl_file.put_line (output_file, cline);
    cline  :='bye';
    utl_file.put_line (output_file, cline);
    cline  :=' ';
...
--    cline  :='mput D:\INTERFACES\CCTMP\CC*.*';
    ...
    utl_file.put_line (output_file, cline);
    cline  :='mput D:\INTERFACES\END_IN_OC*.*';
    utl_file.put_line (output_file, cline);
--    cline  :='mput D:\INTERFACES\CITMP\CI*.*';
    cline  :='mput D:\INTERFACES\DP*.*';
    utl_file.put_line (output_file, cline);
    cline  :='mput D:\INTERFACES\END_IN_DP*.*';
    utl_file.put_line (output_file, cline);
...
--    cline  :='mget d:\interfaces\OCTMP\OC*.*';
    ...
    utl_file.put_line (output_file, cline);
    cline  :='mget /usr/mifarma/ftproot/interfaces/'||cCodLocal_in||'/END_IN_OC*.*';
    utl_file.put_line (output_file, cline);
    cline  :='lcd d:\interfaces\DPTMP';
    utl_file.put_line (output_file, cline);
--    cline  :='mget /usr/mifarma/ftproot/interfaces/'||cCodLocal_in||'/CI*.*';
--    cline  :='mget d:\interfaces\DPTMP\DP*.*';
    ...
    utl_file.put_line (output_file, cline);
    cline  :='mget /usr/mifarma/ftproot/interfaces/'||cCodLocal_in||'/END_IN_DP*.*';
    utl_file.put_line (output_file, cline);
...
---
---
PROCEDURE genera_comandos_ftp_int_5v(cCodLocal_in     IN CHAR) IS
  --movimiento de arhivos OC,DP,GCONF
    cpath       VARCHAR2(60);
    cfilename   VARCHAR2(80);
    cline       VARCHAR2(80);
...
--    cline  :='cd /interfaces/'||cCodLocal_in;
    utl_file.put_line (output_file, cline);
    cline  :='cd c:\Inetpub\ftproot\INTERFACES\'||cCodLocal_in;
    utl_file.put_line (output_file, cline);
    cline  :='prompt';
    utl_file.put_line (output_file, cline);
--    cline  :='mput D:\INTERFACES\CCTMP\CC*.*';
    cline  :='mput D:\INTERFACES\FT*.*';
    utl_file.put_line (output_file, cline);
--    cline  :='mput D:\INTERFACES\CITMP\CI*.*';
    cline  :='mput D:\INTERFACES\SB*.*';
    utl_file.put_line (output_file, cline);
    cline  :='mput D:\INTERFACES\END_IN_V*.*';
    utl_file.put_line (output_file, cline);
    cline  :='bye';
    utl_file.put_line (output_file, cline);
    cline  :=' ';
    ...
    --archivo de comprobacion comandos_int_4v_got.ftp
    cpath       :='DIR_EXPORTADOR';
    cfilename   :='comandos_int_5v_got.ftp';
    output_file := utl_file.fopen (cpath,cfilename, 'W');
--    cline  :='open 10.85.8.46';
    cline  :='open 10.11.1.250';
    utl_file.put_line (output_file, cline);
...
--    cline  :='cd /interfaces/'||cCodLocal_in;
    utl_file.put_line (output_file, cline);
--    cline  :='cd /usr/mifarma/ftproot/interfaces/'||cCodLocal_in;
    cline  :='lcd d:\interfaces\FTTMP';
    utl_file.put_line (output_file, cline);
    cline  :='prompt';
    utl_file.put_line (output_file, cline);
--    cline  :='mget /usr/mifarma/ftproot/interfaces/'||cCodLocal_in||'/CC*.*';
--    cline  :='mget d:\interfaces\OCTMP\OC*.*';
    cline  :='mget c:\Inetpub\ftproot\INTERFACES\'||cCodLocal_in||'\FT*.*';
    utl_file.put_line (output_file, cline);
    cline  :='lcd d:\interfaces\SBTMP';
    utl_file.put_line (output_file, cline);
--    cline  :='mget /usr/mifarma/ftproot/interfaces/'||cCodLocal_in||'/CI*.*';
--    cline  :='mget d:\interfaces\DPTMP\DP*.*';
    cline  :='mget c:\Inetpub\ftproot\INTERFACES\'||cCodLocal_in||'\SB*.*';
    utl_file.put_line (output_file, cline);
    cline  :='bye';
    utl_file.put_line (output_file, cline);
    cline  :=' ';
...
--    cline  :='mget /usr/mifarma/ftproot/interfaces/'||cCodLocal_in||'/CC*.*';
--    cline  :='mget d:\interfaces\OCTMP\OC*.*';
    cline  :='mget /usr/mifarma/ftproot/interfaces/'||cCodLocal_in||'/FT*.*';
    utl_file.put_line (output_file, cline);
    cline  :='mget /usr/mifarma/ftproot/interfaces/'||cCodLocal_in||'/END_IN_V*.*';
    utl_file.put_line (output_file, cline);
    cline  :='lcd d:\interfaces\SBTMP';
    utl_file.put_line (output_file, cline);
--    cline  :='mget /usr/mifarma/ftproot/interfaces/'||cCodLocal_in||'/CI*.*';
--    cline  :='mget d:\interfaces\DPTMP\DP*.*';
    cline  :='mget /usr/mifarma/ftproot/interfaces/'||cCodLocal_in||'/SB*.*';
    utl_file.put_line (output_file, cline);
    cline  :='bye';
    utl_file.put_line (output_file, cline);
    cline  :=' ';
...
----
----
PROCEDURE genera_bat_exporta_int_5v(cCodLocal_in     IN CHAR) IS
  -- genera el bat que se ejecuta con la tarea del sistema operativo
  -- para el envio de archivos CC y de CI hacia matriz
    cpath       VARCHAR2(60);
    cfilename   VARCHAR2(80);
  ...
  BEGIN
    cpath       :='DIR_EXPORTADOR';
    cfilename   :='_xcorre_exportacion_int_5v.bat';
    output_file := utl_file.fopen (cpath,cfilename, 'W');

    cline  :='D:\EXPORTADOR\envia_ftp_int_5v.bat'||' & ';
    utl_file.put (output_file, cline);
    cline  :='del D:\EXPORTADOR\envia_ftp_int_5v.bat'||' & ';
    utl_file.put (output_file, cline);
    cline  :='del D:\EXPORTADOR\comandos_int_5v.ftp'||' & ';
    utl_file.put (output_file, cline);
    utl_file.fclose(output_file);
    genera_bat_ftp_int_5v;
    genera_comandos_ftp_int_5v(cCodLocal_in);
   END;
----
...
--  where  a.fec_cierre_dia_vta>sysdate-30;
  ...
  FECHAINICIO_VTA_IN := TO_CHAR(TRUNC(SYSDATE) -NDIAS,'dd/MM/yyyy');
  FECHAFIN_VTA_IN    := TO_CHAR(TRUNC(SYSDATE) -NDIAS,'dd/MM/yyyy');
  ...
  --PARA LAS INTERFACES DE VENTA DE LAS TRES MARCAS
   IF trim(vc_CodCia) = '001' THEN  --MIFARMA
      Ptoventa.mf_int_vta.int_eject_resumen_rango_dia(CODIGOCOMPANIA_IN,FECHAINICIO_IN,FECHAFIN_IN);
   END IF;

   IF trim(vc_CodCia) = '002' THEN  --FASA
      Ptoventa.mf_int_vta.int_eject_resumen_rango_dia('001',FECHAINICIO_VTA_IN,FECHAFIN_VTA_IN);
   END IF;

   IF trim(vc_CodCia) = '003' THEN  --BTL
      Ptoventa.mf_int_vta.int_eject_resumen_rango_dia('001',FECHAINICIO_VTA_IN,FECHAFIN_VTA_IN);
   END IF;


   IF trim(vc_CodCia) = '001' THEN
      -- COTIZACIONES DE COMPETENCIA
      Ptoventa.Ptoventa_Int.INT_RESUMEN_CC(CODIGOCOMPANIA_IN, CCODLOCAL_IN,1);
      --GENERA KARDEX PARA AJUSTES RD
      PTOVENTA.PTOVENTA_INT.INT_GENERA_GUIA_PROV(CODIGOCOMPANIA_IN,CCODLOCAL_IN,0);
      --AJUSTES RD DE LOCALES MIFARMA
   ...
   IF trim(vc_CodCia) = '002' THEN
          --GENERA KARDEX PARA AJUSTES RD
          PTOVENTA.PTOVENTA_INT.INT_GENERA_GUIA_PROV(CODIGOCOMPANIA_IN,CCODLOCAL_IN,0);
          --AJUSTES RD DE LOCALES FASA
          Ptoventa.pkg_fasa_interf.SP_RVIRTUAL_INT_FASA(CODIGOCOMPANIA_IN, CCODLOCAL_IN);
   END IF;

   IF trim(vc_CodCia) = '003' THEN
      --GENERA KARDEX PARA AJUSTES RD
      PTOVENTA.PTOVENTA_INT.INT_GENERA_GUIA_PROV(CODIGOCOMPANIA_IN,CCODLOCAL_IN,0);
      --AJUSTES RD DE LOCALES FASA
      Ptoventa.pkg_fasa_interf.SP_RVIRTUAL_INT_FASA(CODIGOCOMPANIA_IN, CCODLOCAL_IN,0);
   END IF;
...
--  where  a.fec_cierre_dia_vta>sysdate-30;
 ...
 --ccasas-activado por dubilluz -20131014
  ...
  IF trim(vc_CodCia) = '003' THEN
          begin
          --CONFIRMACIONES DE LOCALES BTL CON FARMAVENTA
          Btl_Sac_Int_Recep_Ciega.P_SOBRANTES_FALTANTES;
          end;

          --AJUSTES AJ DE LOCALES BTL CON FARMAVENTA
          Ptoventa.pkg_fasa_interf.SP_MOV2_INT_FASA(CODIGOCOMPANIA_IN,CCODLOCAL_IN,0);
  END IF;
...
-- CORRIGE ERROR DE DUPLICIDAD DE OCS
   EXECUTE IMMEDIATE 'ALTER SESSION SET NLS_DATE_FORMAT = ''DD/MM/RR'' ';
   EXECUTE IMMEDIATE 'ALTER SESSION SET NLS_NUMERIC_CHARACTERS = '',.'' ';

--  ORDENES DE COMPRA DE MERCADERIA DIRECTA
    ptoventa.pkg_fasa_interf.SP_OCDIR_INT_FASA(CCODGRUPOCIA_IN, CCODLOCAL_IN,nDiasAtras_in-1);
--  DEVOLUCIONES DE ORDENES DE COMPRA DE MERCADERIA DIRECTA
    ptoventa.pkg_fasa_interf.sp_devpro_int_fasa(CCODGRUPOCIA_IN, CCODLOCAL_IN,nDiasAtras_in-1);
