...
-- 2014-08-13 JOLIVA: SE VALIDA QUE NO SE CALCULE MASTER PACK PARA LOS LOCALES EXCLUIDOS DEL MISMO (TABLA AUX_EXCLUYE_MASTERPACK)
  V_CANT_LOCAL       INTEGER;
...
-- 2014-08-13 JOLIVA: SE VALIDA QUE NO SE CALCULE MASTER PACK PARA LOS LOCALES EXCLUIDOS DEL MISMO (TABLA AUX_EXCLUYE_MASTERPACK)
   SELECT COUNT(*)
   INTO V_CANT_LOCAL
   FROM AUX_EXCLUYE_MASTERPACK
   WHERE COD_LOCAL = cCodLocal_in;

   -- SOLO SE CALCULA MASTER PACK SI EL LOCAL NO ESTÁ EXCLUIDO DEL MISMO
   IF V_CANT_LOCAL = 0 THEN
      BEGIN
    ...
    -- Author    : JOLIVA
    -- Fecha     : 2014-07-18
    -- Propósito : Se modifica calculo de cantidad a reponer en casos de MasterPack (solicitado por Fernando Chau)
--                                                                       when  d.CANT_MAX_STK >= mp.CANT_PACK*1 then
--                                                                             round(d.cant_solicitada/mp.CANT_PACK*1)*mp.CANT_PACK*1
                                                                           WHEN D.UN_VTA_M5 >= MP.CANT_PACK*1 THEN
                                                                                CASE
                                                                                     WHEN D.CANT_SOLICITADA / MP.CANT_PACK * 1 BETWEEN 0 AND 1 THEN MP.CANT_PACK * 1
-- 2014-08-20 JOLIVA: SE MODIFICA EL CALCULO DE MASTERPACK CUANDO ES MAYOR A 1 DE MODO QUE HAGA TRUNC EN LUGAR DE ROUND
--                                                                                     ELSE ROUND(D.CANT_SOLICITADA / MP.CANT_PACK * 1) * MP.CANT_PACK * 1
                                                                                     ELSE TRUNC(D.CANT_SOLICITADA / MP.CANT_PACK * 1) * MP.CANT_PACK * 1
                                                                                END
                                                                           else
...
*/
  CURSOR curProdGrupo(cCodGrupo CHAR) IS
       ...
       WHERE P.COD_GRUPO_CIA = vCodGrupoCia
		 ...
		 AND PL.COD_GRUPO_CIA = T.COD_GRUPO_CIA
		 AND PL.COD_LOCAL = T.COD_LOCAL
		 AND PL.COD_PROD = T.COD_PROD
...
BEGIN
     ...
     -- 2014-10-22 JOLIVA: SE CORRIGE CONSULTA A RECOMENDACION DE JOSE LUNA
/*
  -- 2009-05-20 DOCUMENTADO POR JOLIVA
...
*/
    UPDATE LGT_PROD_LOCAL_REP LPLRP
       SET LPLRP.UNID_MIN_SUG    = CANT_SUG *
                                   (SELECT CANT_EQ_GRU_SIM
                                      FROM LGT_PROD P
                                     WHERE P.COD_GRUPO_CIA = LPLRP.COD_GRUPO_CIA
                                       AND P.COD_PROD = LPLRP.COD_PROD),
           LPLRP.STK_UNID_MIN    = LPLRP.STK_FISICO *
                                   (SELECT CANT_EQ_GRU_SIM
                                      FROM LGT_PROD P
                                     WHERE P.COD_GRUPO_CIA = LPLRP.COD_GRUPO_CIA
                                       AND P.COD_PROD = LPLRP.COD_PROD) /
                                   LPLRP.VAL_FRAC_LOCAL,
           LPLRP.CANT_SUG_BK     = CANT_SUG,
           LPLRP.IND_STK_ALMACEN = (SELECT IND_STK_ALMACEN
                                      FROM TMP_PBL_INDICADORES_STK T
                                     WHERE T.COD_PROD = LPLRP.COD_PROD
    ...
    WHERE EXISTS (SELECT 1
              FROM LGT_PROD_GRUPO_SIMILAR D
             WHERE lplrp.COD_GRUPO_CIA = D.COD_GRUPO_CIA
               AND lplrp.COD_PROD = D.COD_PROD);
          ...
          FROM
               (
                  SELECT
                         ...
                         TRIM(PL.IND_ZAN),
                         PLR.IND_STK_ALMACEN,
                         RANK() OVER(ORDER BY NVL(TRIM(PL.IND_ZAN),' ') DESC, P.COD_PROD DESC) POS
                  FROM LGT_PROD P,
				       LGT_PROD_LOCAL PL,
                       LGT_PROD_GRUPO_SIMILAR GP,
                       LGT_PROD_LOCAL_REP PLR
                  WHERE GP.COD_GRUPO = v_cur_Grupos.COD_GRUPO
                    ...
                    AND PLR.COD_PROD = GP.COD_PROD
					AND PL.COD_GRUPO_CIA = PLR.COD_GRUPO_CIA
					AND PL.COD_LOCAL = PLR.COD_LOCAL
					AND PL.COD_PROD = PLR.COD_PROD
...
/***************************************************************************/
  ...
  BEGIN
    ...
    */
    ...
    --agregado por jluna 20141002 siguiente linea
