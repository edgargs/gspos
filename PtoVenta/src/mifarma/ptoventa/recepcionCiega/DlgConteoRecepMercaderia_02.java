...
public class DlgConteoRecepMercaderia_02 extends JDialog {
    ...
    private static final Logger log = LoggerFactory.getLogger(DlgConteoRecepMercaderia_02.class);
    ...
    private JLabelFunction lbfNuevoBulto = new JLabelFunction();
    private JLabelWhite lblCorrBulto = new JLabelWhite();

    // **************************************************************************
    // Constructores
    // **************************************************************************

    public DlgConteoRecepMercaderia_02() {
        this(null, "", false);
    }

    public DlgConteoRecepMercaderia_02(Frame parent, String title, boolean modal) {
    ...
    private void jbInit() throws Exception {
        ...
        lbfNuevoBulto.setBounds(new Rectangle(85, 360, 170, 20));
        lbfNuevoBulto.setText("[ F5 ] Nuevo/Cambio Bulto");
        lblCorrBulto.setText("jLabelWhite2");
        lblCorrBulto.setBounds(new Rectangle(380, 15, 155, 15));
        jPanelTitleBuscar.add(lblCorrBulto, null);
        jPanelTitleBuscar.add(txtBuscar, null);
        jPanelTitleBuscar.add(btnBuscar, null);
        jContentPane.add(lbfNuevoBulto, null);
    ...
    private void initialize() {
        ...
        */
        lblCorrBulto.setText("");   //ASOSA - 12/06/2015 - RCIEGAM
    }


    private void initTableListaProdConteo() {
        tableModelProductosConteo =
                new FarmaTableModel(ConstantsRecepCiega.columnsListaProductosConteo02, ConstantsRecepCiega.defaultValuesListaProductosConteo02,
                                    0);
        FarmaUtility.initSimpleList(tblRelacionProductosConteo, tableModelProductosConteo,
                                    ConstantsRecepCiega.columnsListaProductosConteo02);
    ...
    private void this_windowOpened(WindowEvent e) {
        ...
        setearVariableBeep();   //ASOSA - 22/06/2015 - RCIEGAMCHANGES01
        mostrarIngresoBulto();  //ASOSA - 05/06/2015 - RCIEGAM
    }
    
    /**
     * setearVariableBeep
     * @author ASOSA
     * @since 05/06/2015 - 12/06/2015
     * @type RCIEGAMCHANGES01
     */
    private void setearVariableBeep() {
        VariablesRecepCiega.isLocalBeepBios = UtilityRecepCiega.isLocalBeepBios();
    }
     ...
     * @type RCIEGAM
     */
    private void mostrarIngresoBulto() {
        DlgIngresaBulto dlgIngresaBulto = new DlgIngresaBulto(myParentFrame, "", true);
        dlgIngresaBulto.setVisible(true);
        if (!FarmaVariables.vAceptar) {
            cerrarVentana(false);            
        } else {
            lblCorrBulto.setText("Bulto Nro: " + VariablesRecepCiega.vCorrBulto);
        }
    }
    ...
    private void chkKeyPressed(KeyEvent e) {
        ...
        if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
            boolean mensSalida = JConfirmDialog.rptaConfirmDialog(this, "¿Está seguro que desea Salir?");
            if (mensSalida) {
                try {
                    //Limpiar Variables
                    VariablesRecepCiega.vNroBloque = "";
            ...
            String corrBultoImp = VariablesRecepCiega.vCorrBulto;   //paso el correlativo a una variable local porque luego de ingresar el nuevo bulto se borrara el valor anterior.
            imprimirVoucherCerrado(corrBultoImp); //C01
            DlgIngresaBulto dlgIngresaBulto = new DlgIngresaBulto(myParentFrame, "", true);
            dlgIngresaBulto.setVisible(true);
            if (FarmaVariables.vAceptar) {
                //imprimirVoucherCerrado(corrBultoImp);     //C01
    ...
    private void imprimirVoucherCerrado(String corrBultoImp) {
        UtilityRecepCiega.imprVouBultoCerrado02(corrBultoImp);        
        FarmaUtility.showMessage(this, 
                                 "Se cerro el bulto " + corrBultoImp + ", recoger voucher", 
                                 txtBuscar);
    }
    ...
    private void txtBuscar_keyPressed(KeyEvent e) {
        ...
        if (e.getKeyCode() == KeyEvent.VK_ENTER) {
            ...
            if (txtBuscar.getText().trim().length() > 15) {
                UtilityRecepCiega.ejecutarSonido(ConstantsRecepCiega.ARCHIVO_ERROR);  //ASOSA - 10/06/2015 - RCIEGAM
    ...
    private void ingresaProducto(String vCodBarra) {
        ...
        log.debug("indNoFound2.1: " + VariablesRecepCiega.vIndNoFound);
        //VariablesRecepCiega.vTempCodBarra = vCodBarra;  no se utiliza

        validaCodBarra(vCodBarra);
        log.debug("indNoFound2.2: " + VariablesRecepCiega.vIndNoFound);
        ...
        try {
            ...
            if (VariablesRecepCiega.vIndAgregaConteo) {
                ...
                //almaceno AuxConteo
                log.debug("contadorFila Antes: " + VariablesRecepCiega.contadorFila);
                VariablesRecepCiega.vLastSecProd = String.valueOf(VariablesRecepCiega.contadorFila);
                log.debug("contadorFila: " + VariablesRecepCiega.contadorFila);
                log.debug("LastSecProd: " + VariablesRecepCiega.vLastSecProd);
                //inserto a BD
                //ASOSA - 05/06/2015 - RCIEGAM
                DBRecepCiega.insertAuxConteo02(VariablesRecepCiega.vSecRecepGuia, VariablesRecepCiega.contadorFila,
                                             VariablesRecepCiega.vLastCodBarra, VariablesRecepCiega.vLastCant,
                                             VariablesRecepCiega.vIndDeteriorado, VariablesRecepCiega.vIndFueraLote,
                                             VariablesRecepCiega.vIndNoFound,
                                            VariablesRecepCiega.vCodBulto,
                                            VariablesRecepCiega.vCorrBulto,
                                            VariablesRecepCiega.vIndLectoraBulto
                                               );
            ...
            FarmaUtility.ordenar(tblRelacionProductosConteo, tableModelProductosConteo, Col_Aux_Conteo,
            ...
            if (VariablesRecepCiega.vIndNoFound.trim().equalsIgnoreCase("S")) {
                ...
                DBRecepCiega.enviaErrorCorreoPorDB(null, VariablesRecepCiega.vSecRecepGuia);
                FarmaUtility.showMessage(this, "Código de Barra No Existe. \n" +
                        "Se enviará alerta vía e-mail.", txtBuscar);

                // 27.11.09
                //Coger el ultimo AuxCodBarra de tabla
                VariablesRecepCiega.vAuxCodBarra =
                        FarmaUtility.getValueFieldJTable(tblRelacionProductosConteo, 0, Col_Cod_Barra);
                log.error("VariablesRecepCiega.vAuxCodBarra :" + VariablesRecepCiega.vAuxCodBarra);

            } else {
                UtilityRecepCiega.ejecutarSonido(ConstantsRecepCiega.ARCHIVO_ATENDIDO);  //ASOSA - 10/06/2015 - RCIEGAM
                VariablesRecepCiega.vAuxSecProd = VariablesRecepCiega.vLastSecProd;
            }
            ...
            //DUBILLUZ  - 14.01.2010
            FarmaUtility.liberarTransaccion();
            FarmaUtility.showMessage(this, "Error al momento de agregar el código de barra.\n" +
    ...
    private void muestraInfoProd() {
        ...
        VariablesRecepCiega.vTempAuxSecConteo =
                FarmaUtility.getValueFieldJTable(tblRelacionProductosConteo, vFila, Col_Aux_Conteo); //secAuxConteo
        log.debug("Otiene vTempAuxSecConteo: " + VariablesRecepCiega.vTempAuxSecConteo);
        VariablesRecepCiega.vTempNroBloque =
                FarmaUtility.getValueFieldJTable(tblRelacionProductosConteo, vFila, Col_Nro_Bloque);
        VariablesRecepCiega.vTempCodBarra =
                FarmaUtility.getValueFieldJTable(tblRelacionProductosConteo, vFila, Col_Cod_Barra);
        VariablesRecepCiega.vTempDescProd =
                FarmaUtility.getValueFieldJTable(tblRelacionProductosConteo, vFila, Col_Desc_Prod);
        VariablesRecepCiega.vTempCant = FarmaUtility.getValueFieldJTable(tblRelacionProductosConteo, vFila, Col_Cant);
        log.debug("TempnroBloque: " + VariablesRecepCiega.vTempNroBloque + " -- codBarra:" +
                  VariablesRecepCiega.vTempCodBarra);
        ...
        if (!VariablesRecepCiega.vIndModificoCantActivo) {
            ...
            if (VariablesRecepCiega.vLastCodBarra.trim().equalsIgnoreCase(VariablesRecepCiega.vTempCodBarra.trim()) &&
                VariablesRecepCiega.vLastSecProd.trim().equalsIgnoreCase(VariablesRecepCiega.vTempAuxSecConteo.trim())) {
                //Puede modificar porque es el último
            ...
            //Modifica si ingreso incorrecto antes
                     ...
                     VariablesRecepCiega.vAuxSecProd.trim().equalsIgnoreCase(VariablesRecepCiega.vTempAuxSecConteo.trim())) {
                VariablesRecepCiega.vIndModificarCant = true;
                mostrarIngresoCantPrimerConteo();
                actualizaListaPrimerConteo();
                //VariablesRecepCiega.vIndModificoCantActivo = true; //YA MODIFICO, YA NO PODRA MODIFICAR
              ...
              */
                FarmaUtility.showMessage(this, "Ud. no puede modificar este registro.\n" +
                        "Sólo se puede modificar el último ingresado, una vez modificado no se aceptan más cambios.",
                        tblRelacionProductosConteo);
    ...
    private void updateEstadoRecepMercaderia(String pEstado) throws SQLException {
        ...
        DBRecepCiega.actualizaEstadoRecep(VariablesRecepCiega.vSecRecepGuia, pEstado);
    ...
    private String obtieneEstadoRecepCiega(String pSecRecepCiega) {
        ...
        try {
            ...
            log.error("", sql);
            FarmaUtility.liberarTransaccion();
            FarmaUtility.showMessage(this, "Error al obtener el estado de la Recepción.\n" +
                    sql.getMessage(), txtBuscar);
        }
        return rpta;
    }
    ...
    private void actualizaListaPrimerConteo() {
        try {
            //ASOSA - 08/06/2015 - RCIEGAM
            DBRecepCiega.obtieneListaPrimerConteo02(tableModelProductosConteo, 
    ...
    private void verificaRolUsuario() {
        if (FarmaVariables.dlgLogin.verificaRol(FarmaConstants.ROL_VENDEDOR)) {
            ...
            FarmaVariables.vAceptar = false;
            FarmaUtility.showMessage(this,
                                     "Ud. no tiene Permiso para Finalizar el conteo." + "\nIngrese Usuarios Correcto.",
                                     txtBuscar);
            return;
        }
    }
    ...
    private void pFinalizaConteo() {
        ...
        String pEstado = UtilityRecepCiega.pEstadoRecepcion(VariablesRecepCiega.vSecRecepGuia);
        ...
        if (pEstado.trim().equalsIgnoreCase(ConstantsRecepCiega.EstadoConteo) &&
            ...
            try {
                String indNoContado = DBRecepCiega.validaRegistroAuxConteo(VariablesRecepCiega.vSecRecepGuia);
                if (indNoContado.trim().equalsIgnoreCase("S")) {
                    validaRol();
                    if (FarmaVariables.vAceptar) {
                        if (JConfirmDialog.rptaConfirmDialog(this,
                                                                                        "¿Está seguro que desea Finalizar el Conteo?")) {
                            DBRecepCiega.insertConteo02(VariablesRecepCiega.vSecRecepGuia);
                            updateEstadoRecepMercaderia(ConstantsRecepCiega.EstadoRevisado);
                            FarmaUtility.aceptarTransaccion();
                            UtilityRecepCiega.imprVouBultoCerrado02(VariablesRecepCiega.vCorrBulto);  //C02
                            FarmaUtility.showMessage(this, 
                                                     "Se Ha Finalizado el Conteo con éxito!\nSe cerro el bulto " + VariablesRecepCiega.vCorrBulto + ", recoger voucher", 
                                                     txtBuscar);
                            //imprimirVoucherCerrado(VariablesRecepCiega.vCorrBulto); //ASOSA - 12/06/2015 - RCIEGA //C02
                            cerrarVentana(true);
                        } else {
                            return;
                        }
                ...
                log.error("", sql);
                FarmaUtility.liberarTransaccion();
                FarmaUtility.showMessage(this, "Error al momento de finalizar conteo.\n" +
                        sql.getMessage(), txtBuscar);
            }
        } else {
            ...
            FarmaUtility.showMessage(this, pMensaje, txtBuscar);
