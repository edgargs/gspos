  ...
  BEGIN
    OPEN curCaj FOR
         SELECT VTA_CAB.NUM_PED_VTA || 'Ã' ||
               ...
               DECODE(VTA_CAB.TIP_COMP_PAGO,COD_TIP_COMP_BOLETA,'BOLETA',COD_TIP_COMP_FACTURA,'FACTURA',COD_TIP_COMP_TICKET,'TICKET','OTRO') || 'Ã' ||
    ...
    --RHERRERA: SE OBTIENE INFORMACION DEL PEDIDO DE LA CABECERA DEL PEDIDO
           --  NVL(VC.COD_CONVENIO,' ') || 'Ã' ||
             ...
             /*AGREGADO PARA VENTA EMPRESA*/
               ...
               (CASE
                  WHEN VTA_CAB_EMP.COD_POLIZA IS NOT NULL THEN 'ATENCION BOTIQUIN '||VTA_CAB_EMP.NOMBRE_CLIENTE_POLIZA||' POLIZA N° '||VTA_CAB_EMP.COD_POLIZA||'.'
                  ELSE ' ' END)|| 'Ã'||
               (
               CASE
...
--          AND     NUM_PED_VTA = VTA_CAB.NUM_PED_VTA)
         ...
         VTA_CAB.USU_CREA_PED_VTA_CAB|| ' Ã ' ||--' ' JMIRANDA 17/09/2009
         NVL(VTA_CAB.COD_CONVENIO,' ') || ' Ã ' ||--RHERRERA: 11/04/2014 envia codigo de convenio
         ...
         NVL(VTA_CAB.PUNTO_LLEGADA,' ' ) || ' Ã ' ||
         NVL((SELECT CASE
          WHEN V_EMP.COD_POLIZA IS NOT NULL THEN 'ATENCION BOTIQUIN '||V_EMP.NOMBRE_CLIENTE_POLIZA||' POLIZA N° '||V_EMP.COD_POLIZA||'.'
          ELSE ' ' END
         FROM VTA_PEDIDO_VTA_CAB_EMP V_EMP
         WHERE V_EMP.COD_GRUPO_CIA=VTA_CAB.COD_GRUPO_CIA
         AND V_EMP.COD_LOCAL=VTA_CAB.COD_LOCAL
               ...
               (MM.CANT_NC_TICKETS, MM.MON_NC_TICKETS) =
                    (
                       --ERIOS 2.4.3 Se calcula el valor para NC de tickets y guias
						SELECT COUNT(1) CANTIDAD,SUM(VCP2.VAL_NETO_COMP_PAGO+VCP2.val_redondeo_comp_pago) TOTAL_NETO_REDONDEO--,VCP2.TIP_COMP_PAGO
						FROM  (
						...
						JOIN  VTA_COMP_PAGO VCP2     ON (VCP2.COD_GRUPO_CIA = cCodGrupoCia_in
														  ...
														  AND VCP2.TIP_COMP_PAGO IN ('05','03')
...
END;
  ...
  BEGIN
       ...
       AND    VIR.COD_PROD IN (SELECT COD_PROD
                                                           ...
                                                           AND    DET.COD_LOCAL = cCodLocal_in
                                                           AND    DET.NUM_PED_VTA = cNumPedVta_in
                                                           AND    CAB.COD_RPTA_RECARGA = COD_EXITO
                               );
    RETURN v_nCant;
  END;
