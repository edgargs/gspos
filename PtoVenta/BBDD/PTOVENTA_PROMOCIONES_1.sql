--------------------------------------------------------
--  DDL for Package Body PTOVENTA_PROMOCIONES
--------------------------------------------------------
  ...
  AS
    ...
    CURSOR curPromociones(v_cod_local CHAR,v_cod_grupo_cia CHAR) IS
      ...
      BEGIN
        ...
        SELECT DISTINCT cod_local, cod_grupo_cia
               INTO v_cod_local, v_cod_grupo_cia
        FROM   vta_pedido_vta_cab;
        CCReceiverAddress := FARMA_EMAIL.GET_RECEIVER_ADDRESS_LOCAL(v_cod_local);

        SELECT COD_LOCAL ||' - '|| DESC_LOCAL
               INTO v_vDescLocal
        FROM   PBL_LOCAL
        WHERE  COD_GRUPO_CIA = v_cod_grupo_cia
  ...
  --              mesg_body := mesg_body||'      <th><small>DESCRIPCION</small></th>';
                ...
                FOR v_rCurPromociones IN curPromociones (v_cod_local, v_cod_grupo_cia)
                LOOP
                  mesg_body := mesg_body||'   <tr>'||
 ...
 CURSOR cur IS
   ...
   /* WHERE (SELECT SYSDATE FROM DUAL)
    ...
    AND    P.COD_PAQUETE_2 IN (SELECT V.COD_PAQUETE
                               ...
                               WHERE  T.PORC_DCTO = 100
                               GROUP BY T.COD_PAQUETE
                               HAVING  COUNT(1) =
                                       (SELECT COUNT(1) FROM VTA_PROD_PAQUETE WHERE COD_PAQUETE = T.COD_PAQUETE)
                                     )V
                               )
