...
public class DlgDevolucionIngresoCantidad extends JDialog implements KeyListener {
    ...
    private void jbInit() throws Exception {
        ...
        /** Inicio ccastillo 07/04/2016 **/
        //--El lote no debe ser editado para isLocalTipoVentaM='S'
        if (UtilityPtoVenta.isLocalVentaMayorista())
        txtFechaVenc.setEditable(false);
        /** Fin ccastillo 07/04/2016 **/
   ...
   /*private void txtFechaVenc_keyPressed(KeyEvent e) {
    ...
    private boolean validarCampos() {
        ...
        /** Inicio ccastillo 06/04/2016 **/
        //1-Se comento logica anterior (validar estructura de la fecha de vencimiento)
        //2-Se agrega validacion para el lote del producto a devolver
        ...
        /*else if (txtFechaVenc.getText().trim().length() != 0) {
            ...
            if(txtLote.getText().trim().length() == 0) {
                ...
                try{
                    String fvctolote = DBInventario.validaLoteDevolucion(lblCodigoD.getText(),txtLote.getText());
                    if((fvctolote.trim()).equals("")){
                        FarmaUtility.showMessage(this, "El Lote Ingresado no es Valido.", txtLote);
                        retorno = false;
                    }else{  
                        txtFechaVenc.setText(fvctolote);
