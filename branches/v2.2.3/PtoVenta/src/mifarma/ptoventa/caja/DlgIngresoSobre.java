  ...
  private void chkKeyPressed(KeyEvent e)
  {
    ...
    }else if(mifarma.ptoventa.reference.UtilityPtoVenta.verificaVK_F2(e))
    {
        ...
        if(ind.trim().equalsIgnoreCase("S")){
                if(com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this, "¿Esta seguro de eliminar el sobre con código?")){
                  borrarSobre();    
                  lblRegistros.setText(tblSobres.getRowCount()+"");
                 }
         }else {
                if(com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this, "¿Esta seguro de eliminar el sobre agregado?")){
                   borrarSobre();    
                    lblRegistros.setText(tblSobres.getRowCount()+"");
               }
         }
       ...
       if(tblSobres.getRowCount()>0){
            ...
            if(cant>0){
                if(com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this, "¿Esta seguro de registrar los sobres agregados?")){
                    if (cargaValidaLogin()) {
                        guardarCreados();
                    }
                    else {
