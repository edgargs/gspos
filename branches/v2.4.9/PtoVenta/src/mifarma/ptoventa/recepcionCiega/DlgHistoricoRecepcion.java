...
import mifarma.ptoventa.delivery.DlgStockFaltante;
        ...
        try{
            if(tableModelRecepcion.getRowCount()== -1){
                ...
                if (JConfirmDialog.rptaConfirmDialog(this,"¿Está seguro de Eliminar la recepción?")) {
                    int vFila = tblRecepcion.getSelectedRow();
                    String pCodigoRecepcion = FarmaUtility.getValueFieldArrayList(tableModelRecepcion.data,vFila,0).toString().trim();
            ...
            if (sql.getErrorCode() == 20601||sql.getErrorCode() == 20602||sql.getErrorCode() == 20603) {
                FarmaUtility.showMessage(this, sql.getMessage().substring(11,sql.getMessage().indexOf("ORA-06512")), txtFechaIni);
            }else{
                log.error("",sql);
