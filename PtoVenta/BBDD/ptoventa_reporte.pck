...
/*******************************************************************************************************************************************/
  ...
  RETURN FarmaCursor;
/*--------------------------------------------------------------------------------------------------------------------------
GOAL : Cargar las Tablas de Ventas en Local
...
-- 2014-12-15 JOLIVA: SE AGREGO ATENCION AL CLIENTE
  ...
  --ERIOS 24.03.2015 Cambios de JLUNA
        ...
        FROM
           (
            SELECT
                   ...
                   SUM
                      (--ASOSA - 25/05/2015 - NG
					   --ERIOS 28.05.2015 ¿Y los historicos?
                            (NVL(DET.PORC_ZAN,0.4) * DET.VAL_PREC_TOTAL / (1+VAL_IGV/100) / 100)
                       ) "TOT_COM"
...
BEGIN
 ...
 /* vc_Fec_Ini := vc_Fec_Ini ||' 00:00:00';
  ...
  ptoventa_reporte.sp_load_aux_ventas(ac_fec_ini => vc_Fec_Ini,
  ...
  BEGIN
    OPEN vCursor FOR 
      ...
      FROM (SELECT TRIM(B.DESC_CATEGORIA_GIGANTE) CATEGORIA,
                   ...
                   NVL((SELECT SUM(C.MON_TOT_C_IGV)
