    ...
    private void seleccionarProducto() {
        ...
        if (!seleccion) {
            ...
            if (FarmaVariables.vAceptar) {
                agregarProducto();
                //KMONCADA 17.08.2016 [PROYECTO M] PERMITIR MAS DE UN LOTE EN COMPETENCIA
    ...
    private void borrarProducto() {
        ...
        for (int i = 0; i < arrayProductos.size(); i++) {
            if (((ArrayList)arrayProductos.get(i)).contains(cod)) {
                ...
                //KMONCADA 17.08.2016 [PROYECTO M] PERMITIR MAS DE UN LOTE AL REGISTRAR PRECIO COMPETENCIA
                if(!UtilityPtoVenta.isLocalVentaMayorista()){
                    break;
                }
            }
