...
import mifarma.ptoventa.pinpad.mastercard.HiloProcesoAnularPinpadMC;
...
public class DlgAnularTransPinpad extends JDialog {
    ...
    public void setValores(String numPedido, String monto, String fecha, String numReferencia, String strCodFormaPago,
                           boolean flagAnulPed) {
    ...
    private void this_windowOpened(WindowEvent e) {
        if (UtilityPinpad.validarLibrerias()) {
            ...
            if (FarmaVariables.vTipCambio == 0) {
                ...
                //Si es Mastercard
                //ERIOS 2.4.7 Los pagos con CMR pasan por el pinpad de MC
    ...
    private void chkkeyPressed(KeyEvent e) {
        if (lblF11.isEnabled() && UtilityPtoVenta.verificaVK_F11(e)) { //conciliacionAnulacionPinpad();
            cerrarVentana(true);
        }
        if (lblF5.isEnabled() && e.getKeyCode() == KeyEvent.VK_F5) {
            ...
            if (JConfirmDialog.rptaConfirmDialog(this,
                                                 "ATENCION: El ejecutar esta opción afectara en el cierre de caja\n¿Esta seguro de continuar?")) {
                DlgLogin dlgLogin = new DlgLogin(myParent, ConstantsPtoVenta.MENSAJE_LOGIN, true);
                dlgLogin.setRolUsuario(FarmaConstants.ROL_ADMLOCAL);
                dlgLogin.setVisible(true);

                if (FarmaVariables.vAceptar) { //LLEIVA 11-Dic-2013 Se indica el flag de la transacción que no se pudo anular por que el lote se encuentra cerrado
                    try {
    ...
    public void conciliacionAnulacionPinpad() {
        try {
            ...
            ArrayList<String> listTemp =
                facadeRecaudacion.getNumTarjCodAutoriz(lblDatoPedidoEnv.getText()).get(0);
            String numTarjeta = listTemp.get(0);
            String numAutorizacion = listTemp.get(1);
            ...
            String PCODIAUTOORIG =
                numAutorizacion; //lblDatoNumAutorizacion.getText(); //Anulacion-Codigo autorizacion Origen
