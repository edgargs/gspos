...
public class FacadeRecaudacion {
    ...
    public String grabaCabeRecau(ArrayList arrayCabecera, String pDni) {
        String tmpCodigo = "";
        try {
            if (arrayCabecera.size() > 0) {
                tmpCodigo =
                        daoRecaudacion.grabaCabRecaudacion(arrayCabecera.get(0).toString().trim(), arrayCabecera.get(1).toString().trim(),
    ...
    public boolean grabaFormPagoRecau(ArrayList lstDetallePago, String pVuelto, String codAutorizacion, Long codTrssc,
        ...
        try {
            ...
            for (int i = 0; i < lstDetallePago.size(); i++) {
                daoRecaudacion.grabaFormPagoRCD(((ArrayList)lstDetallePago.get(i)).get(0).toString().trim(),
            ...
            daoRecaudacion.cambiarIndicadorRCD(((ArrayList)lstDetallePago.get(0)).get(0).toString().trim(),
                                               ((ArrayList)lstDetallePago.get(0)).get(1).toString().trim(),
                                               ((ArrayList)lstDetallePago.get(0)).get(2).toString().trim(),
                                               vCodigoRecaudacion, ConstantsRecaudacion.ESTADO_COBRADO,
                                               FarmaVariables.vIdUsu.trim(), "", ConstantsRecaudacion.TIP_IMPRE,
                                               // Parametro para actualizar el estado de impresion.  RLLantoy 25.06.2013
                    codTrssc, estTrsscSix, codAutorizacion, VariablesCaja.vSecMovCaja, strFechaOrigen, "", "", "");
    ...
    private void colocaVueltoDetallePago(String pVuelto) {
        ...
        for (int i = 0; i < lstDetalle.size(); i++) {
            if ((((ArrayList)lstDetalle.get(i)).get(4)).toString().equalsIgnoreCase(ConstantsCaja.FORMA_PAGO_EFECTIVO_SOLES)) {
                existeSoles = true;
                filaSoles = i;
                break;
    ...
    public ArrayList listarRcdCanceladas(String strCodGrupoCia, String strCodCia, String strCodLocal, String pCodRcd,
