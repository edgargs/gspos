...
BEGIN
  ...
  WHERE L.COD_GRUPO_CIA = :NEW.COD_GRUPO_CIA
  AND L.COD_LOCAL = :NEW.COD_LOCAL;
  EXCEPTION
    WHEN OTHERS THEN
      vTipoLocalVenta :=  NULL;
  END;
  
  IF vTipoLocalVenta = PKG_CONSTANTES.LOCAL_TIPO_VTA_M THEN
   SELECT PKG_CONSTANTES.VAL_FRAC_ENTERO,
          PROD_LOCAL.VAL_FRAC_LOCAL,
          PROD_LOCAL.VAL_PREC_VTA * PROD_LOCAL.VAL_FRAC_LOCAL,
          PROD_LOCAL.IND_ZAN,
          PROD_LOCAL.PORC_ZAN,
