...
public class DlgAsignarMontoFondoSencillo extends JDialog {
    ...
    private static final Logger log = LoggerFactory.getLogger(DlgAsignarMontoFondoSencillo.class);
    ...
    public DlgAsignarMontoFondoSencillo(Frame parent, String title, boolean modal) {
    ...
    private void this_windowClosing(WindowEvent e) {
        FarmaUtility.showMessage(this, "Debe presionar la tecla ESC para cerrar la ventana.", txtMonto);
    }
    ...
    private void cargarDatosCajero() {
        txtCajero.setText(VariablesFondoSencillo.vNombresCajero + " " + VariablesFondoSencillo.vApePaterno + " " +
        ...
        try {
            ...
            if (!Rpta.trim().equalsIgnoreCase("S") &&
                ...
                VariablesFondoSencillo.vHisSecFondoSen =
                        DBFondoSencillo.insAsignacionSencillo2(pMonto, VariablesFondoSencillo.vSecUsuCajero, // DESTINO
                            FarmaVariables.vNuSecUsu, // ORIGEN
                            ConstantsFondoSencillo.EstadoEmitido, ConstantsFondoSencillo.TipoFondoAsigna,
                            FarmaVariables.vIdUsu, FarmaVariables.vIpPc).trim();
                FarmaUtility.aceptarTransaccion();
                flag = true;
            } else {
                flag = false;
                FarmaUtility.showMessage(this,
                                         "La caja fue abierta desde otro punto o ya tiene fondo Asignado." + "\nNo se le puede asignar monto hasta que Cierre la caja.",
                                         txtMonto);
                cerrarVentana(false);
            }
            ...
            if (sql.getErrorCode() > 20000) {
                ...
                FarmaUtility.showMessage(this, "Error al asignar Monto a Cajero.\n" +
