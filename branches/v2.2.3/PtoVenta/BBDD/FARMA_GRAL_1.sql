--------------------------------------------------------
--  DDL for Package Body FARMA_GRAL
--------------------------------------------------------
    ...
    if v_lineaTiempo_anulacion = '0' then
       v_lineaTiempo_anulacion:= '  (LA RECARGA NO PUEDE ANULARSE)';
        ...
        v_lineaTiempo_anulacion:=
            '  (Tiempo Maximo de Anulacion de '||v_lineaTiempo_anulacion|| ' minutos.)';
    end if;
...
/* ******************************************************************* */
  ...
  BEGIN
     ...
     SELECT COD_LOCAL,
            TIP_COMP_PAGO,
            SERIE,
            (SELECT MIN(CP.NUM_COMP_PAGO) FROM VTA_COMP_PAGO CP WHERE CP.COD_GRUPO_CIA = cGrupoCia_in AND CP.COD_LOCAL = CD.COD_LOCAL AND CP.TIP_COMP_PAGO = CD.TIP_COMP_PAGO AND SUBSTR(CP.NUM_COMP_PAGO,1,3) = CD.SERIE AND CP.NUM_PED_VTA = CD.NUM_PED_MIN) MIN_COMP,
            (SELECT MAX(CP.NUM_COMP_PAGO) FROM VTA_COMP_PAGO CP WHERE CP.COD_GRUPO_CIA = cGrupoCia_in AND CP.COD_LOCAL = CD.COD_LOCAL AND CP.TIP_COMP_PAGO = CD.TIP_COMP_PAGO AND SUBSTR(CP.NUM_COMP_PAGO,1,3) = CD.SERIE AND CP.NUM_PED_VTA = CD.NUM_PED_MAX) MAX_COMP
     FROM
          (
             SELECT
                    C.COD_LOCAL,
                   C.TIP_COMP_PAGO TIP_COMP_PAGO,
...
*/
  ...
  BEGIN
        ...
        AND    ce.fec_crea_cuadratura_caja
                      BETWEEN TO_DATE('01/12/2008' || ' 00:00:00','dd/MM/yyyy HH24:mi:ss')
...
FUNCTION F_IMPR_TERMICA_STICKER_PROD(cCodGrupoCia_in   IN CHAR,
  ...
  BEGIN
      ...
      V_SQL := ' SELECT DISTINCT ' || ''''
            ...
            || 'ZB?'
            || 'A80,20,0,1,1,1,N,"' || ''''
            || ' || SUBSTR(trim(d.desc_prod),1,27) || ' ||''''|| ' / ' || '''' || ' || trim(D.DESC_UNID_PRESENT) || '|| ''''
            || '"?'
            || 'A200,50,0,1,1,1,N,"'|| ''''
            || ' || trim(d.cod_prod) || '|| ''''
            || '"?'
            ...
            || '"?'
            || 'B320,80,0,E30,3,8,80,B,"'|| ''''
            ||' || NVL(FARMA_GRAL.F_VAR_GET_COD_BARRAS(trim(d.cod_prod)),' || '''' || '0000000000000' || '''' || ') || ' || ''''
            || '"?'
            || 'P1?' || ''''
            || ' AS HTML from lgt_prod d,aux_cambio_precio e '
     ...
     BEGIN
          ...
          FOR
              SELECT (PL.COD_PROD || 'Ã' ||
                      P.DESC_PROD  || 'Ã' ||
                      NVL(PL.UNID_VTA,P.DESC_UNID_PRESENT) || 'Ã' ||
                      TO_CHAR(PL.VAL_PREC_VTA,'9,999,990.00')) AS RESULTADO
               ...
               WHERE PL.COD_PROD = P.COD_PROD
                 ...
                 AND P.DESC_PROD LIKE '%' || vDescripcion || '%';
          RETURN V_RCURPROD;
     END;
  ...
  --02/10/2013  CVILCA      	Creación
   ...
   BEGIN
        SELECT DISTINCT
              'R50,0?N?S2?D5?ZB?A80,20,0,1,1,1,N,"'
              || SUBSTR(TRIM(P.DESC_PROD),1,27) || ' / ' || TRIM(NVL(PL.UNID_VTA,P.DESC_UNID_PRESENT))
              || '"?A200,50,0,1,1,1,N,"' || TRIM(P.COD_PROD)
              || '"?A500,50,0,1,1,1,N,"' || TRIM(TO_CHAR(PL.FEC_MOD_PROD_LOCAL,'dd-mm-yyyy'))
              || '"?A130,80,0,5,1,2,N,"' || SUBSTR(TRIM(TO_CHAR(PL.VAL_PREC_VTA,'9,999,990.00')),1,INSTR(TRIM(TO_CHAR(PL.VAL_PREC_VTA,'9,999,990.00')),'.'))
              || '"?A' || CASE WHEN PL.VAL_PREC_VTA > 100 THEN 260 WHEN PL.VAL_PREC_VTA < 10 THEN 190 ELSE 220 END || ',80,0,4,1,2,N,"'
              || SUBSTR(TRIM(TO_CHAR(PL.VAL_PREC_VTA,'9,999,990.00')),INSTR(TRIM(TO_CHAR(PL.VAL_PREC_VTA,'9,999,990.00')),'.')+1)
              || '"?B320,80,0,E30,3,8,80,B,"'
 ...
 /**********************************************************************************************/
  ...
  FUNCTION F_GET_PRODUC_COD_BARRA(cCodGrupoCia IN CHAR, cCodLocal IN CHAR,  cCodBarra IN CHAR )
   ...
   IS
    ...
    B.COD_PROD || 'Ã' ||
    C.DESC_PROD  || 'Ã' ||
    NVL(B.UNID_VTA,C.DESC_UNID_PRESENT)  || 'Ã' ||
     TO_CHAR(B.VAL_PREC_VTA,'9,999,990.00')
     ) AS RESULTADO
    ...
    WHERE 
    A.COD_PROD=B.COD_PROD AND B.COD_PROD=C.COD_PROD
    --and c.cod_grupo_rep_edmundo IN('003','007','006','004')
    AND A.COD_GRUPO_CIA=cCodGrupoCia
    AND B.COD_LOCAL=cCodLocal
    AND A.COD_BARRA=cCodBarra
    AND C.EST_PROD='A'
